...
Expr<Type<TypeCategory::Character, KIND>>::LEN() const {
    ...
    common::visitors{
        ...
        [](const Designator<Result> &dr) {
            return dr.LEN();
        },
        [](const FunctionRef<Result> &fr) {
            return fr.LEN();
        },
...
void GenericExprWrapper::Deleter(GenericExprWrapper *p) {
    delete p;
}
...
template <TypeCategory CAT> int Expr<SomeKind<CAT>>::GetKind() const {
    return std::visit(
    [](const auto &kx) {
        return std::decay_t<decltype(kx)>::Result::kind;
    },
    u);
}
...
std::optional<Expr<SubscriptInteger>> Expr<SomeCharacter>::LEN() const {
    return std::visit([](const auto &kx) {
        return kx.LEN();
    }, u);
}
