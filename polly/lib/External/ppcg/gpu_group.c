...
gpu_array_ref_group_print_name(struct gpu_array_ref_group *group,
...
gpu_array_ref_group_access_relation(struct gpu_array_ref_group *group, int read,
                                    int write) {
  int i;
...
gpu_array_ref_group_type(struct gpu_array_ref_group *group) {
...
gpu_array_ref_group_tile(struct gpu_array_ref_group *group) {
...
static void extract_stride(__isl_keep isl_constraint *c,
                           ...
                           __isl_keep isl_val *stride, int sign) {
  int i;
  isl_val *v;
  isl_space *space;
  unsigned nparam;
...
static isl_stat check_stride_constraint(__isl_take isl_constraint *c,
  ...
  if (n_div && (isl_val_is_one(v) || isl_val_is_negone(v))) {
    ...
    if (!isl_val_is_zero(stride) && isl_val_gt(stride, bound->stride))
...
check_stride(struct gpu_array_bound *bound, __isl_take isl_basic_map *bounds) {
  isl_space *space;
  isl_basic_map *hull;
...
static int can_tile(__isl_keep isl_map *access, struct gpu_array_tile *tile) {
  ...
  for (i = 0; i < tile->n; ++i) {
    ...
    access_i = isl_map_project_out(access_i, isl_dim_out, 1, tile->n - (i + 1));
...
static int access_is_coalesced(struct gpu_group_data *data,
  ...
  access =
      isl_union_map_apply_domain(access, isl_union_map_copy(data->full_sched));
...
localize_access(struct gpu_group_data *data, __isl_take isl_union_map *access) {
  int n;
  isl_space *space;
  isl_set *param;
  isl_union_map *umap;
...
static int populate_array_references(struct gpu_local_array_info *local,
  ...
  for (i = 0; i < local->array->n_ref; ++i) {
    ...
    umap =
        isl_union_map_apply_domain(umap, isl_union_map_copy(data->copy_sched));
...
gpu_array_ref_group_free(struct gpu_array_ref_group *group) {
  if (!group)
    return NULL;
...
join_groups(struct gpu_array_ref_group *group1,
            struct gpu_array_ref_group *group2) {
  int i;
  isl_ctx *ctx;
  struct gpu_array_ref_group *group;
  ...
  group->refs = isl_alloc_array(ctx, struct gpu_stmt_access *, group->n_ref);
  if (!group->refs)
 ...
 */
static struct gpu_array_ref_group *
join_groups_and_free(struct gpu_array_ref_group *group1,
                     struct gpu_array_ref_group *group2) {
  struct gpu_array_ref_group *group;
...
static void report_no_reuse_and_coalesced(struct ppcg_kernel *kernel,
  ...
  p = isl_printer_print_str(
      p, " not considered for mapping to shared memory in kernel");
  p = isl_printer_print_int(p, kernel->id);
  p = isl_printer_print_str(
      p, " because it exhibits no reuse and is considered to be coalesced");
...
static int compute_group_bounds_core(struct ppcg_kernel *kernel,
  ...
  if (r >= 0 && kernel->options->debug->verbose && use_shared && no_reuse &&
      coalesced)
    report_no_reuse_and_coalesced(kernel, access);

  if (use_shared && (!no_reuse || !coalesced)) {
    group->shared_tile = gpu_array_tile_create(ctx, group->array->n_index);
  ...
  acc =
      isl_map_project_out(acc, isl_dim_in, data->thread_depth, data->n_thread);
  ...
  if (force_private && !group->private_tile)
    isl_die(ctx, isl_error_internal,
            "unable to map array reference group to registers", return -1);
 ...
 * Return -1 on error.
 */
static int group_writes(struct ppcg_kernel *kernel, int n,
                        struct gpu_array_ref_group **groups,
                        int (*overlap)(struct gpu_array_ref_group *group1,
                                       struct gpu_array_ref_group *group2),
                        int compute_bounds, struct gpu_group_data *data) {
  int i, j;
  ...
  for (i = 0; i < n; ++i) {
    for (j = n - 1; j > i; --j) {
      ...
      if (!groups[i])
        return -1;
      if (compute_bounds && compute_group_bounds(kernel, groups[i], data) < 0)
        return -1;
    }
  }
 ...
 * Return the updated number of groups.
 */
static int group_overlapping_writes(struct ppcg_kernel *kernel, int n,
                                    struct gpu_array_ref_group **groups,
                                    struct gpu_group_data *data) {
 ...
 * Return the updated number of groups.
 */
static int group_depth_overlapping_writes(struct ppcg_kernel *kernel, int n,
                                          struct gpu_array_ref_group **groups,
                                          struct gpu_group_data *data) {
...
static int smaller_tile(struct gpu_array_tile *tile,
                        struct gpu_array_tile *tile1,
                        struct gpu_array_tile *tile2) {
  int smaller;
...
static int group_common_shared_memory_tile(struct ppcg_kernel *kernel,
  ...
  for (i = 0; i < n; ++i) {
    ...
    for (j = n - 1; j > i; --j) {
      ...
      if (!group->shared_tile ||
          !smaller_tile(group->shared_tile, groups[i]->shared_tile,
 ...
 */
static void set_array_groups(struct gpu_local_array_info *array, int n,
                             struct gpu_array_ref_group **groups) {
  int i;
...
static int group_array_references(struct ppcg_kernel *kernel,
  ...
  groups =
      isl_calloc_array(ctx, struct gpu_array_ref_group *, local->array->n_ref);
  if (!groups)
    return -1;
 ...
 */
static void
check_can_be_private_live_ranges(struct ppcg_kernel *kernel,
  ...
  for (i = 0; i < kernel->n_array; ++i) {
    ...
    order = isl_union_map_intersect_domain(order, isl_union_set_copy(domain));
    order = isl_union_map_intersect_range(order, isl_union_set_copy(domain));
    order = isl_union_map_eq_at_multi_union_pw_aff(
        order, isl_multi_union_pw_aff_copy(prefix));
 ...
 */
static __isl_give isl_union_map *
expand(__isl_take isl_union_map *s,
...
static void compute_privatization(struct gpu_group_data *data,
  ...
  for (i = 0; i < data->n_thread; ++i) {
    ...
    isl_id_free(id);
    aff2 = isl_aff_var_on_domain(isl_local_space_copy(ls), isl_dim_param, pos);
 ...
 */
static __isl_give isl_union_map *
prefix_with_equalities(__isl_keep isl_schedule_node *node) {
...
int gpu_group_references(struct ppcg_kernel *kernel,
  ...
  data.thread_sched = isl_union_map_flat_range_product(
      data.thread_sched,
...
strided_tile(struct gpu_array_tile *tile, __isl_keep isl_space *space,
  ...
  for (i = 0; i < tile->n; ++i) {
    ...
    if (tile->bound[i].shift) {
      ...
      shift_i = isl_aff_zero_on_domain(isl_local_space_copy(ls));
    }
