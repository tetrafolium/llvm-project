...
Expr<SubscriptInteger> Triplet::stride() const {
...
int ComplexPart::Rank() const {
    return complex_.Rank();
}
...
template <typename T> BaseObject Designator<T>::GetBaseObject() const {
    ...
    common::visitors{
        [](SymbolRef symbol) {
            return BaseObject{symbol};
        },
        [](const Substring &sstring) {
            return sstring.GetBaseObject();
        },
        [](const auto &x) {
...
NamedEntity CoarrayRef::GetBase() const {
    return AsNamedEntity(base_);
}
...
bool Subscript::operator==(const Subscript &that) const {
    return u == that.u;
}
