...
static __isl_give isl_mat *particular_solution(__isl_keep isl_mat *B,
  ...
  for (i = 0; i < B->n_row; ++i) {
    ...
    for (j = 0; j < B->n_col - 1; ++j)
      isl_int_fdiv_r(M->row[i][B->n_row + j], B->row[i][1 + j], M->row[i][i]);
  }
...
static __isl_give isl_mat *parameter_compression_multi(__isl_keep isl_mat *B,
  ...
  for (i = 0; i < B->n_row; ++i) {
    ...
    for (j = 1; j < U->n_row; ++j)
      for (k = 0; k < U->n_col; ++k)
        isl_int_mul(A->row[k][i * size + j], D, U->row[j][k]);
  }
...
__isl_give isl_mat *isl_mat_parameter_compression(__isl_take isl_mat *B,
  ...
  for (i = 0; i < B->n_row; ++i) {
    ...
    if (isl_int_is_zero(D)) {
      ...
      if (!B || !d)
        goto error2;
      isl_seq_cpy(d->block.data + i, d->block.data + i + 1, d->size - (i + 1));
      d->size--;
      i--;
      continue;
    }
 ...
 */
__isl_give isl_mat *
isl_mat_final_variable_compression(__isl_take isl_mat *B, int first,
                                   __isl_give isl_mat **T2) {
  int i, n;
  isl_ctx *ctx;
...
return_with_identity(__isl_take isl_basic_set *bset, __isl_give isl_mat **T,
                     __isl_give isl_mat **T2) {
  isl_size dim;
  isl_mat *id;
 ...
 */
static __isl_give isl_basic_set *
compress_variables(__isl_take isl_basic_set *bset, __isl_give isl_mat **T,
                   __isl_give isl_mat **T2) {
  struct isl_mat *B, *TC;
  isl_size dim;
...
isl_basic_set_remove_equalities(__isl_take isl_basic_set *bset,
...
isl_stat isl_basic_set_dim_residue_class(__isl_keep isl_basic_set *bset,
  ...
  isl_seq_gcd(U->row[nparam + pos] + bset->n_eq, total - bset->n_eq, modulo);
...
isl_stat isl_set_dim_residue_class_val(__isl_keep isl_set *set, int pos,
  ...
  *residue = isl_val_alloc(isl_set_get_ctx(set));
  if (!*modulo || !*residue)
    goto error;
  if (isl_set_dim_residue_class(set, pos, &(*modulo)->n, &(*residue)->n) < 0)
    goto error;
