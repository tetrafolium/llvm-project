...
private:
    ...
    bool inbounds(StratifiedIndex Idx) const {
...
template <typename T> class StratifiedSetsBuilder {
    ...
    struct BuilderLink {
        ...
        bool isRemapped() const {
            return Remap != StratifiedLink::SetSentinel;
        }
        ...
        const StratifiedLink &getLink() const {
            return Link;
        }
...
private:
    ...
    StratifiedIndex getNewUnlinkedIndex() {
        return addLinks();
    }
    ...
    bool inbounds(StratifiedIndex N) const {
        return N < Links.size();
    }
};
}
