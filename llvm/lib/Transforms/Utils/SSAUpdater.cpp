...
Value *SSAUpdater::GetValueInMiddleOfBlock(BasicBlock *BB) {
  ...
  PHINode *InsertedPHI =
      PHINode::Create(ProtoType, PredValues.size(), ProtoName, &BB->front());
...
public:
  ...
  ///
  static PHINode *InstrIsPHI(Instruction *I) { return dyn_cast<PHINode>(I); }
...
void LoadAndStorePromoter::run(const SmallVectorImpl<Instruction *> &Insts) {
  ...
  for (LoadInst *ALoad : LiveInLoads) {
    ...
    if (NewVal == ALoad)
      NewVal = UndefValue::get(NewVal->getType());
...
bool LoadAndStorePromoter::isInstInList(
    Instruction *I, const SmallVectorImpl<Instruction *> &Insts) const {
