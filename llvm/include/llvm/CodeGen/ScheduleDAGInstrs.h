...
struct VReg2SUnitOperIdx : public VReg2SUnit {
  ...
  VReg2SUnitOperIdx(unsigned VReg, LaneBitmask LaneMask, unsigned OperandIndex,
                    SUnit *SU)
...
protected:
  ...
  /// and those below it.
  void reduceHugeMemNodeMaps(Value2SUsMap &stores, Value2SUsMap &loads,
                             unsigned N);
...
public:
  explicit ScheduleDAGInstrs(MachineFunction &mf, const MachineLoopInfo *mli,
  ...
  /// operands.
  void buildSchedGraph(AAResults *AA, RegPressureTracker *RPTracker = nullptr,
