...
static void free_child(struct isl_arg *arg, void *opt) {
  if (arg->offset == ISL_ARG_OFFSET_NONE)
    ...
    isl_args_free(arg->u.child.child, *(void **)(((char *)opt) + arg->offset));
}
...
static void print_default_flags(struct isl_arg *decl, void *opt, int pos) {
  ...
  for (i = 0; decl->u.flags.flags[i].name; ++i)
    if ((*p & decl->u.flags.flags[i].mask) == decl->u.flags.flags[i].value)
...
static void print_flags_help(struct isl_arg *decl,
  ...
  for (i = 0; decl->u.flags.flags[i].name; ++i) {
    ...
    for (j = i; decl->u.flags.flags[j].mask == decl->u.flags.flags[i].mask;
         ++j) {
      if (j != i) {
        printf("|");
        pos++;
...
static void print_bool_help(struct isl_arg *decl, struct isl_prefixes *prefixes,
                            void *opt) {
  int pos;
...
static int print_argument_name(struct isl_arg *decl, const char *name,
                               int pos) {
  printf("%c<%s>", decl->long_name ? '=' : ' ', name);
  return pos + 3 + strlen(name);
}

static void print_int_help(struct isl_arg *decl, struct isl_prefixes *prefixes,
                           void *opt) {
  int pos;
  char val[20];
...
static void print_long_help(struct isl_arg *decl, struct isl_prefixes *prefixes,
                            void *opt) {
  int pos;
...
static void print_str_help(struct isl_arg *decl, struct isl_prefixes *prefixes,
                           void *opt) {
  int pos;
...
static void print_help(struct isl_arg *arg, struct isl_prefixes *prefixes,
                       void *opt) {
  int i;
  int any = 0;
...
static int match_long_name(struct isl_arg *decl, const char *start,
                           const char *end) {
  do {
...
static const char *skip_name(struct isl_arg *decl, const char *arg,
                             struct isl_prefixes *prefixes, int need_argument,
                             int *has_argument) {
  const char *equal;
  const char *name;
  const char *end;
...
static int parse_option(struct isl_arg *decl, char **arg,
  ...
  for (i = 0; decl[i].type != isl_arg_end; ++i) {
    ...
    switch (decl[i].type) {
    case isl_arg_choice:
      parsed = parse_choice_option(&decl[i], arg, prefixes, opt);
      break;
    case isl_arg_flags:
      parsed = parse_flags_option(&decl[i], arg, prefixes, opt);
      break;
    case isl_arg_int:
      parsed = parse_int_option(&decl[i], arg, prefixes, opt);
      break;
    case isl_arg_long:
      parsed = parse_long_option(&decl[i], arg, prefixes, opt);
      break;
    case isl_arg_ulong:
      parsed = parse_ulong_option(&decl[i], arg, prefixes, opt);
      break;
    case isl_arg_bool:
      parsed = parse_bool_option(&decl[i], arg, prefixes, opt);
      break;
    case isl_arg_str:
      parsed = parse_str_option(&decl[i], arg, prefixes, opt);
      break;
    case isl_arg_str_list:
      parsed = parse_str_list_option(&decl[i], arg, prefixes, opt);
      break;
    case isl_arg_child:
      parsed = parse_child_option(&decl[i], arg, prefixes, opt);
      break;
    case isl_arg_alias:
    case isl_arg_arg:
    case isl_arg_footer:
...
int isl_args_parse(struct isl_args *args, int argc, char **argv, void *opt,
  ...
  while (argc > 1 + skip) {
    ...
    parsed = parse_option(args->args, &argv[1 + skip], &prefixes, opt);
    if (parsed)
