...
public:
    ...
    uint32_t getInstructionLogSize() const {
        return m_insn_log_size;
    }
    ...
    /// \param[in] size
    ///     Value to be set.
    void setInstructionLogSize(uint32_t size) {
        m_insn_log_size = size;
    }
...
private:
    ...
    public:
        ...
        Instructions &GetInstructionLog() {
            return m_instruction_log;
        }

        uint32_t GetStopID() const {
            return m_thread_stop_id;
        }

        void SetStopID(uint32_t stop_id) {
            m_thread_stop_id = stop_id;
        }
