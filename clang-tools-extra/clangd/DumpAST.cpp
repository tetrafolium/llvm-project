...
class DumpVisitor : public RecursiveASTVisitor<DumpVisitor> {
    ...
    SourceRange getSourceRange(const TemplateName &Node) {
        return SourceRange();
    }
    // Attr just uses a weird method name. Maybe we should fix it instead?
    SourceRange getSourceRange(const Attr *Node) {
    ...
    std::string getKind(const Decl *D) {
...
#undef ATTR
    ...
    std::string getDetail(const Decl *D) {
        ...
        if (!ND || llvm::isa_and_nonnull<CXXConstructorDecl>(ND->getAsFunction()) ||
            ...
            return "";
        std::string Name = toString([&](raw_ostream &OS) {
            ND->printName(OS);
        });
        if (Name.empty())
            return "(anonymous)";
    ...
    std::string getArcana(const NestedNameSpecifierLoc &NNS) {
        return "";
    }
    std::string getArcana(const TemplateName &NNS) {
        return "";
    }
    std::string getArcana(const CXXBaseSpecifier &CBS) {
        return "";
    }
