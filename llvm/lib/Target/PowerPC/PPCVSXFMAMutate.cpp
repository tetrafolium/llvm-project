...
static cl::opt<bool>
    DisableVSXFMAMutate("disable-ppc-vsx-fma-mutation",
...
protected:
  bool processBlock(MachineBasicBlock &MBB) {
    ...
    for (MachineBasicBlock::iterator I = MBB.begin(), IE = MBB.end(); I != IE;
      ...
      if (LIS->getInterval(Reg2).Query(FMAIdx).isKill() && Reg2 != OldFMAReg) {
        KilledProdOp = 2;
        OtherProdOp = 3;
      ...
      if (!MRI.constrainRegClass(KilledProdReg, MRI.getRegClass(OldFMAReg)))
        continue;
...
INITIALIZE_PASS_BEGIN(PPCVSXFMAMutate, DEBUG_TYPE, "PowerPC VSX FMA Mutation",
                      false, false)
INITIALIZE_PASS_DEPENDENCY(LiveIntervals)
INITIALIZE_PASS_DEPENDENCY(SlotIndexes)
INITIALIZE_PASS_DEPENDENCY(MachineDominatorTree)
INITIALIZE_PASS_END(PPCVSXFMAMutate, DEBUG_TYPE, "PowerPC VSX FMA Mutation",
                    false, false)
