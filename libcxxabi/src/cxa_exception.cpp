...
failed_throw(__cxa_exception* exception_header) {
  //  Section 2.5.3 says:
...
void __cxa_throw(void* thrown_object, std::type_info* tinfo,
  ...
  exception_header->referenceCount =
...
void* __cxa_get_exception_ptr(void* unwind_exception) throw() {
  ...
  return reinterpret_cast<void*>(
      static_cast<_Unwind_Control_Block*>(unwind_exception)
...
bool __cxa_begin_cleanup(void* unwind_arg) throw() {
  ...
  if (__isOurExceptionClass(unwind_exception)) {
    if (0 == exception_header->propagationCount) {
      exception_header->nextPropagatingException =
...
#if defined(_LIBCXXABI_ARM_EHABI)
    return reinterpret_cast<void*>(
        exception_header->unwindHeader.barrier_cache.bitpattern[0]);
#else
