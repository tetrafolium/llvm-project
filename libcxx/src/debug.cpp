...
__libcpp_abort_debug_function(__libcpp_debug_info const& info) {
  std::fprintf(stderr, "%s\n", info.what().c_str());
  std::abort();
}

_LIBCPP_SAFE_STATIC __libcpp_debug_function_type __libcpp_debug_function =
...
_LIBCPP_FUNC_VIS
const __libcpp_db* __get_const_db() { return __get_db(); }
...
__libcpp_db::__libcpp_db()
    : __cbeg_(nullptr), __cend_(nullptr), __csz_(0), __ibeg_(nullptr),
      __iend_(nullptr), __isz_(0) {}
...
void* __libcpp_db::__find_c_from_i(void* __i) const {
#ifndef _LIBCPP_HAS_NO_THREADS
  RLock _(mut());
#endif
...
void __libcpp_db::__insert_ic(void* __i, const void* __c) {
#ifndef _LIBCPP_HAS_NO_THREADS
  WLock _(mut());
#endif
  if (__cbeg_ == __cend_)
...
void __libcpp_db::__insert_c(void* __c, __libcpp_db::_InsertConstruct* __fn) {
#ifndef _LIBCPP_HAS_NO_THREADS
  WLock _(mut());
#endif
...
void __libcpp_db::__erase_i(void* __i) {
#ifndef _LIBCPP_HAS_NO_THREADS
  WLock _(mut());
#endif
  if (__ibeg_ != __iend_) {
...
void __libcpp_db::__invalidate_all(void* __c) {
#ifndef _LIBCPP_HAS_NO_THREADS
  WLock _(mut());
#endif
  if (__cend_ != __cbeg_) {
...
__c_node* __libcpp_db::__find_c_and_lock(void* __c) const {
#ifndef _LIBCPP_HAS_NO_THREADS
  mut().lock();
#endif
  if (__cend_ == __cbeg_) {
...
__c_node* __libcpp_db::__find_c(void* __c) const {
  size_t hc = hash<void*>()(__c) % static_cast<size_t>(__cend_ - __cbeg_);
  __c_node* p = __cbeg_[hc];
...
void __libcpp_db::unlock() const {
#ifndef _LIBCPP_HAS_NO_THREADS
  mut().unlock();
#endif
}

void __libcpp_db::__erase_c(void* __c) {
#ifndef _LIBCPP_HAS_NO_THREADS
  WLock _(mut());
#endif
  if (__cend_ != __cbeg_) {
    ...
    while (p->__c_ != __c) {
      ...
      if (p == nullptr)
        return;
      _LIBCPP_ASSERT(p != nullptr,
                     "debug mode internal logic error __erase_c B");
    }
    if (q == nullptr)
...
void __libcpp_db::__iterator_copy(void* __i, const void* __i0) {
#ifndef _LIBCPP_HAS_NO_THREADS
  WLock _(mut());
#endif
  __i_node* i = __find_iterator(__i);
...
bool __libcpp_db::__dereferenceable(const void* __i) const {
#ifndef _LIBCPP_HAS_NO_THREADS
  RLock _(mut());
#endif
  __i_node* i = __find_iterator(__i);
...
bool __libcpp_db::__decrementable(const void* __i) const {
#ifndef _LIBCPP_HAS_NO_THREADS
  RLock _(mut());
#endif
  __i_node* i = __find_iterator(__i);
...
bool __libcpp_db::__addable(const void* __i, ptrdiff_t __n) const {
#ifndef _LIBCPP_HAS_NO_THREADS
  RLock _(mut());
#endif
  __i_node* i = __find_iterator(__i);
...
bool __libcpp_db::__subscriptable(const void* __i, ptrdiff_t __n) const {
#ifndef _LIBCPP_HAS_NO_THREADS
  RLock _(mut());
#endif
  __i_node* i = __find_iterator(__i);
  return i != nullptr && i->__c_ != nullptr &&
         i->__c_->__subscriptable(__i, __n);
}

bool __libcpp_db::__less_than_comparable(const void* __i,
                                         const void* __j) const {
#ifndef _LIBCPP_HAS_NO_THREADS
  RLock _(mut());
#endif
  __i_node* i = __find_iterator(__i);
...
void __libcpp_db::swap(void* c1, void* c2) {
#ifndef _LIBCPP_HAS_NO_THREADS
  WLock _(mut());
#endif
...
void __libcpp_db::__insert_i(void* __i) {
#ifndef _LIBCPP_HAS_NO_THREADS
  WLock _(mut());
#endif
...
void __c_node::__add(__i_node* i) {
  if (end_ == cap_) {
    size_t nc = 2 * static_cast<size_t>(cap_ - beg_);
    if (nc == 0)
      nc = 1;
    __i_node** beg = static_cast<__i_node**>(malloc(nc * sizeof(__i_node*)));
    if (beg == nullptr)
      __throw_bad_alloc();
...
_LIBCPP_HIDDEN
__i_node* __libcpp_db::__insert_iterator(void* __i) {
  ...
  size_t hi = hash<void*>()(__i) % static_cast<size_t>(__iend_ - __ibeg_);
  __i_node* p = __ibeg_[hi];
  __i_node* r = __ibeg_[hi] = static_cast<__i_node*>(malloc(sizeof(__i_node)));
  if (r == nullptr)
    __throw_bad_alloc();
...
_LIBCPP_HIDDEN
__i_node* __libcpp_db::__find_iterator(const void* __i) const {
  __i_node* r = nullptr;
  if (__ibeg_ != __iend_) {
    size_t h =
        hash<const void*>()(__i) % static_cast<size_t>(__iend_ - __ibeg_);
...
_LIBCPP_HIDDEN
void __c_node::__remove(__i_node* p) {
  __i_node** r = find(beg_, end_, p);
  _LIBCPP_ASSERT(r != end_,
