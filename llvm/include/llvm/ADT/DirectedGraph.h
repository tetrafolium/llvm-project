...
public:
    ...
    explicit DGNode(EdgeType &E) : Edges() {
        Edges.insert(&E);
    }
    DGNode() = default;
...
protected:
    ...
    const_iterator findEdgeTo(const NodeType &N) const {
        return llvm::find_if(
        Edges, [&N](const EdgeType *E) {
            return E->getTargetNode() == N;
        });
    }
...
public:
    ...
    explicit DirectedGraph(NodeType &N) : Nodes() {
        addNode(N);
    }
