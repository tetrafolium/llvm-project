...
nub_size_t MachTask::ReadMemory(nub_addr_t addr, nub_size_t size, void *buf) {
  ...
  if (task != TASK_NULL) {
    ...
    DNBLogThreadedIf(LOG_MEMORY,
                     "MachTask::ReadMemory ( addr = 0x%8.8llx, "
...
nub_size_t MachTask::WriteMemory(nub_addr_t addr, nub_size_t size,
  ...
  if (task != TASK_NULL) {
    ...
    DNBLogThreadedIf(LOG_MEMORY,
                     "MachTask::WriteMemory ( addr = 0x%8.8llx, "
...
int MachTask::GetMemoryRegionInfo(nub_addr_t addr, DNBRegionInfo *region_info) {
  ...
  DNBLogThreadedIf(LOG_MEMORY,
                   "MachTask::MemoryRegionInfo ( addr = 0x%8.8llx "
...
std::string MachTask::GetProfileData(DNBProfileDataScanType scanType) {
  ...
  if (m_vm_memory.GetMemoryProfile(
          scanType, task, task_info, m_process->GetCPUType(), pid, vminfo,
          physical_memory, anonymous, phys_footprint, memory_cap)) {
      ...
      if (exception_message.CatchExceptionRaise(task)) {
        if (exception_message.state.task_port != task) {
          if (exception_message.state.IsValid()) {
            ...
            DNBLogThreadedIf(LOG_EXCEPTIONS,
                             "task port changed from 0x%4.4x to 0x%4.4x", task,
                             exception_message.state.task_port);
