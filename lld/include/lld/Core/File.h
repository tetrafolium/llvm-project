...
public:
    ...
    StringRef archivePath() const {
        return _archivePath;
    }
    void setArchivePath(StringRef path) {
        _archivePath = std::string(path);
    }

    /// Returns the path name of this file. It doesn't include archive file name.
    StringRef memberPath() const {
        return _path;
    }
    ...
    bool hasOrdinal() const {
        return (_ordinal != UINT64_MAX);
    }

    /// Sets the command line order of the file.
    void setOrdinal(uint64_t ordinal) const {
        _ordinal = ordinal;
    }
    ...
    void setLastError(std::error_code err) {
        _lastError = err;
    }
...
protected:
    ...
    virtual std::error_code doParse() {
