...
class AArch64FastISel final : public FastISel {
  ...
  public:
    ...
    void setOffsetReg(unsigned Reg) { OffsetReg = Reg; }

    unsigned getOffsetReg() const { return OffsetReg; }
...
private:
  ...
  unsigned emitAddSub_rx(bool UseAdd, MVT RetVT, unsigned LHSReg,
                         bool LHSIsKill, unsigned RHSReg, bool RHSIsKill,
                         AArch64_AM::ShiftExtendType ExtType, uint64_t ShiftImm,
                         bool SetFlags = false, bool WantResult = true);
  ...
  unsigned emitMul_rr(MVT RetVT, unsigned Op0, bool Op0IsKill, unsigned Op1,
                      bool Op1IsKill);
  unsigned emitSMULL_rr(MVT RetVT, unsigned Op0, bool Op0IsKill, unsigned Op1,
                        bool Op1IsKill);
  unsigned emitUMULL_rr(MVT RetVT, unsigned Op0, bool Op0IsKill, unsigned Op1,
                        bool Op1IsKill);
...
unsigned AArch64FastISel::materializeInt(const ConstantInt *CI, MVT VT) {
  ...
  const TargetRegisterClass *RC =
      (VT == MVT::i64) ? &AArch64::GPR64RegClass : &AArch64::GPR32RegClass;
...
unsigned AArch64FastISel::materializeFP(const ConstantFP *CFP, MVT VT) {
  ...
  int Imm = Is64Bit ? AArch64_AM::getFP64Imm(Val) : AArch64_AM::getFP32Imm(Val);
  if (Imm != -1) {
...
bool AArch64FastISel::simplifyAddress(Address &Addr, MVT VT) {
  ...
  if ((ImmediateOffsetNeedsLowering || Addr.getOffsetReg()) &&
      Addr.isFIBase()) {
    unsigned ResultReg = createResultReg(&AArch64::GPR64spRegClass);
    BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(AArch64::ADDXri),
            ResultReg)
  ...
  if (ImmediateOffsetNeedsLowering) {
    ...
    if (Addr.getReg())
      ...
      ResultReg =
          emitAdd_ri_(MVT::i64, Addr.getReg(), /*IsKill=*/false, Offset);
    else
...
unsigned AArch64FastISel::emitAddSub(bool UseAdd, MVT RetVT, const Value *LHS,
  ...
  if (RHS->hasOneUse() && isValueAvailable(RHS)) {
    if (isMulPowOf2(RHS)) {
      ...
      ResultReg =
          emitAddSub_rs(UseAdd, RetVT, LHSReg, LHSIsKill, RHSReg, RHSIsKill,
    ...
    if (const auto *SI = dyn_cast<BinaryOperator>(RHS)) {
      if (const auto *C = dyn_cast<ConstantInt>(SI->getOperand(1))) {
        ...
        default:
          break;
        case Instruction::Shl:
          ShiftType = AArch64_AM::LSL;
          break;
        case Instruction::LShr:
          ShiftType = AArch64_AM::LSR;
          break;
        case Instruction::AShr:
          ShiftType = AArch64_AM::ASR;
          break;
        }
        ...
        if (ShiftType != AArch64_AM::InvalidShiftExtend) {
          ...
          ResultReg =
              emitAddSub_rs(UseAdd, RetVT, LHSReg, LHSIsKill, RHSReg, RHSIsKill,
                            ShiftType, ShiftVal, SetFlags, WantResult);
          if (ResultReg)
            return ResultReg;
        }
      }
...
unsigned AArch64FastISel::emitAddSub_rr(bool UseAdd, MVT RetVT, unsigned LHSReg,
  ...
  static const unsigned OpcTable[2][2][2] = {
      {{AArch64::SUBWrr, AArch64::SUBXrr}, {AArch64::ADDWrr, AArch64::ADDXrr}},
...
unsigned AArch64FastISel::emitAddSub_ri(bool UseAdd, MVT RetVT, unsigned LHSReg,
  ...
  static const unsigned OpcTable[2][2][2] = {
      {{AArch64::SUBWri, AArch64::SUBXri}, {AArch64::ADDWri, AArch64::ADDXri}},
...
unsigned AArch64FastISel::emitAddSub_rs(bool UseAdd, MVT RetVT, unsigned LHSReg,
  ...
  static const unsigned OpcTable[2][2][2] = {
      {{AArch64::SUBWrs, AArch64::SUBXrs}, {AArch64::ADDWrs, AArch64::ADDXrs}},
...
unsigned AArch64FastISel::emitAddSub_rx(bool UseAdd, MVT RetVT, unsigned LHSReg,
  ...
  static const unsigned OpcTable[2][2][2] = {
      {{AArch64::SUBWrx, AArch64::SUBXrx}, {AArch64::ADDWrx, AArch64::ADDXrx}},
...
unsigned AArch64FastISel::emitLogicalOp_ri(unsigned ISDOpc, MVT RetVT,
  ...
  static const unsigned OpcTable[3][2] = {{AArch64::ANDWri, AArch64::ANDXri},
...
unsigned AArch64FastISel::emitLogicalOp_rs(unsigned ISDOpc, MVT RetVT,
  ...
  static_assert((ISD::AND + 1 == ISD::OR) && (ISD::AND + 2 == ISD::XOR),
                "ISD nodes are not consecutive!");
  static const unsigned OpcTable[3][2] = {{AArch64::ANDWrs, AArch64::ANDXrs},
...
unsigned AArch64FastISel::emitLoad(MVT VT, MVT RetVT, Address Addr,
  ...
  static const unsigned GPOpcTable[2][8][4] = {
      // Sign-extend.
      {{AArch64::LDURSBWi, AArch64::LDURSHWi, AArch64::LDURWi, AArch64::LDURXi},
       {AArch64::LDURSBXi, AArch64::LDURSHXi, AArch64::LDURSWi,
        AArch64::LDURXi},
       {AArch64::LDRSBWui, AArch64::LDRSHWui, AArch64::LDRWui, AArch64::LDRXui},
  ...
  case MVT::i8:
    ...
    RC = (IsRet64Bit && !WantZExt) ? &AArch64::GPR64RegClass
...
bool AArch64FastISel::emitCompareAndBranch(const BranchInst *BI) {
  ...
  static const unsigned OpcTable[2][2][2] = {
      ...
      {{AArch64::TBZW, AArch64::TBZX}, {AArch64::TBNZW, AArch64::TBNZX}}};
...
bool AArch64FastISel::selectCmp(const Instruction *I) {
  ...
  static unsigned CondCodeTable[2][2] = {{AArch64CC::NE, AArch64CC::VC},
...
bool AArch64FastISel::selectSelect(const Instruction *I) {
  ...
  if (foldXALUIntrinsic(CC, I, Cond)) {
    ...
    BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, II, AArch64::WZR)
  ...
  unsigned ResultReg =
      fastEmitInst_rri(Opc, RC, Src1Reg, Src1IsKill, Src2Reg, Src2IsKill, CC);
  updateValueMap(I, ResultReg);
  return true;
}
...
bool AArch64FastISel::selectIntToFP(const Instruction *I, bool Signed) {
  ...
  unsigned ResultReg =
      fastEmitInst_r(Opc, TLI.getRegClassFor(DestVT), SrcReg, SrcIsKill);
  updateValueMap(I, ResultReg);
  return true;
}
...
bool AArch64FastISel::fastLowerIntrinsicCall(const IntrinsicInst *II) {
  ...
  case Intrinsic::memmove: {
    ...
    if (isa<ConstantInt>(MTI->getLength()) && IsMemCpy) {
      ...
      unsigned Alignment =
          MinAlign(MTI->getDestAlignment(), MTI->getSourceAlignment());
      if (isMemCpySmall(Len, Alignment)) {
        Address Dest, Src;
  ...
  case Intrinsic::umul_with_overflow: {
    ...
    if (MulReg) {
      ...
      BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc,
              TII.get(TargetOpcode::COPY), ResultReg1)
          .addReg(MulReg);
    }
...
unsigned AArch64FastISel::emitSMULL_rr(MVT RetVT, unsigned Op0, bool Op0IsKill,
  ...
  return fastEmitInst_rrr(AArch64::SMADDLrrr, &AArch64::GPR64RegClass, Op0,
                          Op0IsKill, Op1, Op1IsKill, AArch64::XZR,
                          /*IsKill=*/true);
}
...
unsigned AArch64FastISel::emitUMULL_rr(MVT RetVT, unsigned Op0, bool Op0IsKill,
  ...
  return fastEmitInst_rrr(AArch64::UMADDLrrr, &AArch64::GPR64RegClass, Op0,
                          Op0IsKill, Op1, Op1IsKill, AArch64::XZR,
                          /*IsKill=*/true);
}
...
unsigned AArch64FastISel::emitLSL_rr(MVT RetVT, unsigned Op0Reg, bool Op0IsKill,
  ...
  unsigned ResultReg =
      fastEmitInst_rr(Opc, RC, Op0Reg, Op0IsKill, Op1Reg, Op1IsKill);
  if (NeedTrunc)
    ResultReg = emitAnd_ri(MVT::i32, ResultReg, /*IsKill=*/true, Mask);
  return ResultReg;
}
...
unsigned AArch64FastISel::emitLSL_ri(MVT RetVT, MVT SrcVT, unsigned Op0,
  ...
  static const unsigned OpcTable[2][2] = {{AArch64::SBFMWri, AArch64::SBFMXri},
...
bool AArch64FastISel::selectRem(const Instruction *I, unsigned ISDOpcode) {
  ...
  // MSUB instruction.
  unsigned ResultReg =
      fastEmitInst_rrr(MSubOpc, RC, QuotReg, /*IsKill=*/true, Src1Reg,
                       Src1IsKill, Src0Reg, Src0IsKill);
  updateValueMap(I, ResultReg);
  return true;
}
    ...
    if (const auto *ZExt = dyn_cast<ZExtInst>(Op0)) {
      if (!isIntExtFree(ZExt)) {
        ...
        if (isValueAvailable(ZExt) &&
            isTypeSupported(ZExt->getSrcTy(), TmpVT)) {
          SrcVT = TmpVT;
          IsZExt = true;
      ...
      if (!isIntExtFree(SExt)) {
        ...
        if (isValueAvailable(SExt) &&
            isTypeSupported(SExt->getSrcTy(), TmpVT)) {
          SrcVT = TmpVT;
          IsZExt = false;
...
bool AArch64FastISel::selectSDiv(const Instruction *I) {
  ...
  unsigned SelectReg = fastEmitInst_rri(SelectOpc, RC, AddReg, /*IsKill=*/true,
                                        Src0Reg, Src0IsKill, AArch64CC::LT);
  if (!SelectReg)
    return false;
