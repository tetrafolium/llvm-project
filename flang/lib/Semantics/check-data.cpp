...
public:
  ...
  bool operator()(const Symbol &symbol) { // C876
    ...
    if (const char *whyNot{IsAutomatic(symbol)
                ? "Automatic variable"
                : IsDummy(symbol) ? "Dummy argument"
                        ...
                        : IsAllocatable(symbol) ? "Allocatable"
                                ...
                                ? "Default-initialized"
                                : IsInBlankCommon(symbol)
                                    ? "Blank COMMON object"
                                    : IsProcedure(symbol) && !IsPointer(symbol)
                                        ? "Procedure"
                                        // remaining checks don't apply to
                                        // components
                                        : !isFirstSymbol
                                            ? nullptr
                                            : IsHostAssociated(symbol, scope)
                                                ? "Host-associated object"
                                                : IsUseAssociated(symbol, scope)
