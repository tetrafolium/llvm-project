...
static __isl_give isl_printer *print_cuda_macros(__isl_take isl_printer *p) {
  const char *macros = "#define cudaCheckReturn(ret) \\\n"
                       "  do { \\\n"
...
declare_device_array(__isl_take isl_printer *p, struct gpu_array_info *array) {
  int i;
...
static __isl_give isl_printer *allocate_device_arrays(__isl_take isl_printer *p,
                                                      struct gpu_prog *prog) {
  int i;
  ...
  for (i = 0; i < prog->n_array; ++i) {
    ...
    p = isl_printer_start_line(p);
    p = isl_printer_print_str(p, "cudaCheckReturn(cudaMalloc((void **) &dev_");
 ...
 */
static __isl_give isl_printer *
copy_array_to_device(__isl_take isl_printer *p, struct gpu_array_info *array) {
  p = isl_printer_start_line(p);
 ...
 */
static __isl_give isl_printer *
copy_array_from_device(__isl_take isl_printer *p,
...
static __isl_give isl_printer *print_reverse_list(__isl_take isl_printer *p,
                                                  int len, int *list) {
  int i;
 ...
 */
static __isl_give isl_printer *
print_kernel_arguments(__isl_take isl_printer *p, struct gpu_prog *prog,
  ...
  for (i = 0; i < prog->n_array; ++i) {
    ...
    if (types)
      p = gpu_array_info_print_declaration_argument(p, &prog->array[i], NULL);
    else
      p = gpu_array_info_print_call_argument(p, &prog->array[i]);
...
static void print_iterators(FILE *out, const char *type,
  ...
  for (i = 0; i < n; ++i) {
    ...
    fprintf(out, "%s = %s", isl_id_get_name(id), cuda_dims[n - 1 - i]);
    isl_id_free(id);
  }
  fprintf(out, ";\n");
}
...
static void print_kernel_iterators(FILE *out, struct ppcg_kernel *kernel) {
  ...
  const char *thread_dims[] = {"threadIdx.x", "threadIdx.y", "threadIdx.z"};
 ...
 */
static __isl_give isl_printer *
print_kernel_stmt(__isl_take isl_printer *p,
...
static void print_kernel(struct gpu_prog *prog, struct ppcg_kernel *kernel,
  ...
  print_options = isl_ast_print_options_set_print_user(
      print_options, &print_kernel_stmt, NULL);
