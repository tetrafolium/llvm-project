...
//// after the escaping function 'use' gets arg promotion (int* -> int).
///Currently / we lose track of param after the loop header.

int g = 0;
//// A no-inline, read-only function with internal linkage is a good candidate
//// for arg promotion.
__attribute__((__noinline__)) static void use(const int *p) {
...
__attribute__((__noinline__)) void do_thing(int x) { g *= x; }
