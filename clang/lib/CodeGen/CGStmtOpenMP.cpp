...
void CodeGenFunction::EmitOMPReductionClauseInit(
    ...
    for (const Expr *IRef : Shareds) {
        ...
        RedCG.getBaseDecl(Count), [BaseAddr]() {
            return BaseAddr;
        });
        assert(IsRegistered && "private var already registered as private");
        // Silence the warning about unused variable.
...
static void emitOMPSimdRegion(CodeGenFunction &CGF, const OMPLoopDirective &S,
    ...
    (void)CGF.EmitOMPLinearClauseInit(S);
    {
        ...
        CGF.EmitOMPSimdFinal(S, [](CodeGenFunction &) {
            return nullptr;
        });
...
public:
    ...
    ArrayRef<const VarDecl *> getPrivateDecls() const {
        return PrivateDecls;
    }
};
} // anonymous namespace
...
void CodeGenFunction::EmitOMPTargetTaskBasedDirective(
    ...
    if (InputInfo.NumberOfTargetItems > 0) {
        ...
        BPVD, [&InputInfo]() {
            return InputInfo.BasePointersArray;
        });
            ...
            for (const auto *C : D.getClausesOfKind<OMPFirstprivateClause>()) {
                for (const Expr *Ref : C->varlists()) {
                    ...
                    if (!CGF.LocalDeclMap.count(VD)) {
                        ...
                        VD, [&GlobLVal, &CGF]() {
                            return GlobLVal.getAddress(CGF);
                        });
                    }
                }
            }
