...
template <typename T> struct ProgramStatePartialTrait<llvm::ImmutableList<T>> {
  ...
  static bool Contains(data_type L, key_type K) { return L.contains(K); }
...
template <> struct ProgramStatePartialTrait<void *> {
  ...
  static data_type MakeData(void *const *p) { return p ? *p : data_type(); }

  static void *MakeVoidPtr(data_type d) { return d; }
};
...
template <> struct ProgramStatePartialTrait<const void *> {
  ...
  static void *MakeVoidPtr(data_type d) { return const_cast<void *>(d); }
};
