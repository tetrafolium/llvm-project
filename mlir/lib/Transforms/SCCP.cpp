...
public:
    ...
    bool isUnknown() const {
        return constantAndTag.getInt() == Kind::Unknown;
    }
    ...
    Attribute getConstant() const {
        return constantAndTag.getPointer();
    }
...
void SCCPSolver::initializeSymbolCallables(Operation *op) {
    ...
    auto walkFn = [&](Operation *symTable, bool allUsesVisible) {
        ...
        if (!uses) {
            ...
            op->walk([&](CallableOpInterface op) {
                callableLatticeState.erase(op);
            });
            return;
        }
...
void SCCPSolver::visitRegionSuccessors(
    ...
    for (const RegionSuccessor &it : regionSuccessors) {
        ...
        if (!region) {
            ...
            if (llvm::all_of(results, [&](Value res) {
            return isOverdefined(res);
            }))
            continue;
        ...
        if (llvm::all_of(arguments, [&](Value arg) {
        return isOverdefined(arg);
        }))
        continue;
