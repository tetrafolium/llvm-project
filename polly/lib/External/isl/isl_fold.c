...
static __isl_give isl_qpolynomial_fold *
qpolynomial_fold_alloc(enum isl_fold type, __isl_take isl_space *space, int n) {
...
isl_qpolynomial_fold_reset_domain_space(__isl_take isl_qpolynomial_fold *fold,
  ...
  for (i = 0; i < fold->n; ++i) {
    fold->qp[i] =
        isl_qpolynomial_reset_domain_space(fold->qp[i], isl_space_copy(space));
    if (!fold->qp[i])
      goto error;
  }
...
int isl_qpolynomial_fold_involves_dims(__isl_keep isl_qpolynomial_fold *fold,
  ...
  for (i = 0; i < fold->n; ++i) {
    int involves = isl_qpolynomial_involves_dims(fold->qp[i], type, first, n);
...
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_set_dim_name(__isl_take isl_qpolynomial_fold *fold,
  ...
  for (i = 0; i < fold->n; ++i) {
    fold->qp[i] = isl_qpolynomial_set_dim_name(fold->qp[i], type, pos, s);
    if (!fold->qp[i])
      goto error;
  }
...
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_drop_dims(__isl_take isl_qpolynomial_fold *fold,
                               enum isl_dim_type type, unsigned first,
                               unsigned n) {
  int i;
  ...
  for (i = 0; i < fold->n; ++i) {
    fold->qp[i] = isl_qpolynomial_drop_dims(fold->qp[i], type, first, n);
    if (!fold->qp[i])
      goto error;
  }
...
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_insert_dims(__isl_take isl_qpolynomial_fold *fold,
                                 enum isl_dim_type type, unsigned first,
                                 unsigned n) {
  int i;
  ...
  for (i = 0; i < fold->n; ++i) {
    fold->qp[i] = isl_qpolynomial_insert_dims(fold->qp[i], type, first, n);
    if (!fold->qp[i])
      goto error;
  }
...
static int isl_qpolynomial_aff_sign(__isl_keep isl_set *set,
  ...
  res = isl_set_solve_lp(set, 0, aff->el + 1, aff->el[0], &opt, NULL, NULL);
  if (res == isl_lp_error)
    goto done;
  if (res == isl_lp_empty || (res == isl_lp_ok && !isl_int_is_neg(opt))) {
    sgn = 1;
    goto done;
  }

  res = isl_set_solve_lp(set, 1, aff->el + 1, aff->el[0], &opt, NULL, NULL);
 ...
 */
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_fold_on_domain(__isl_keep isl_set *set,
  ...
  for (i = 0; i < fold2->n; ++i) {
    for (j = n1 - 1; j >= 0; --j) {
      isl_qpolynomial *d;
      int sgn, equal;
      equal = isl_qpolynomial_plain_is_equal(res->qp[j], fold2->qp[i]);
      if (equal < 0)
        goto error;
      if (equal)
        break;
      d = isl_qpolynomial_sub(isl_qpolynomial_copy(res->qp[j]),
...
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_add_qpolynomial(__isl_take isl_qpolynomial_fold *fold,
  ...
  for (i = 0; i < fold->n; ++i) {
    fold->qp[i] = isl_qpolynomial_add(fold->qp[i], isl_qpolynomial_copy(qp));
    if (!fold->qp[i])
      goto error;
  }
...
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_add_on_domain(__isl_keep isl_set *dom,
                                   __isl_take isl_qpolynomial_fold *fold1,
                                   __isl_take isl_qpolynomial_fold *fold2) {
  int i;
  ...
  if (fold2->n == 1) {
    res = isl_qpolynomial_fold_add_qpolynomial(
        fold1, isl_qpolynomial_copy(fold2->qp[0]));
    isl_qpolynomial_fold_free(fold2);
    return res;
  }

  res = isl_qpolynomial_fold_add_qpolynomial(
      isl_qpolynomial_fold_copy(fold1), isl_qpolynomial_copy(fold2->qp[0]));

  for (i = 1; i < fold2->n; ++i) {
    isl_qpolynomial_fold *res_i;
    res_i = isl_qpolynomial_fold_add_qpolynomial(
        isl_qpolynomial_fold_copy(fold1), isl_qpolynomial_copy(fold2->qp[i]));
...
isl_qpolynomial_fold_gist(__isl_take isl_qpolynomial_fold *fold,
  ...
  for (i = 0; i < fold->n; ++i) {
    fold->qp[i] = isl_qpolynomial_gist(fold->qp[i], isl_set_copy(context));
    if (!fold->qp[i])
      goto error;
  }
 ...
 */
static __isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_zero_in_space(__isl_take isl_space *space,
                                   enum isl_fold type) {
...
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_dup(__isl_keep isl_qpolynomial_fold *fold) {
  int i;
  isl_qpolynomial_fold *dup;

  if (!fold)
    return NULL;
  dup = qpolynomial_fold_alloc(fold->type, isl_space_copy(fold->dim), fold->n);
  if (!dup)
    return NULL;
...
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_cow(__isl_take isl_qpolynomial_fold *fold) {
  if (!fold)
    return NULL;
...
isl_pw_qpolynomial_fold_fold(__isl_take isl_pw_qpolynomial_fold *pw1,
  ...
  if (pw1->type != pw2->type)
    isl_die(pw1->dim->ctx, isl_error_invalid, "fold types don't match",
            goto error);

  n = (pw1->n + 1) * (pw2->n + 1);
  res = isl_pw_qpolynomial_fold_alloc_size(isl_space_copy(pw1->dim), pw1->type,
                                           n);
  ...
  for (i = 0; i < pw1->n; ++i) {
    ...
    for (j = 0; j < pw2->n; ++j) {
      ...
      set = isl_set_subtract(set, isl_set_copy(pw2->p[j].set));
      ...
      sum = isl_qpolynomial_fold_fold_on_domain(
          common, isl_qpolynomial_fold_copy(pw1->p[i].fold),
          isl_qpolynomial_fold_copy(pw2->p[j].fold));

      res = isl_pw_qpolynomial_fold_add_piece(res, common, sum);
    }
    res = isl_pw_qpolynomial_fold_add_piece(
        res, set, isl_qpolynomial_fold_copy(pw1->p[i].fold));
  }
  ...
  for (j = 0; j < pw2->n; ++j) {
    ...
    res = isl_pw_qpolynomial_fold_add_piece(
        res, set, isl_qpolynomial_fold_copy(pw2->p[j].fold));
  }
...
isl_union_pw_qpolynomial_fold_fold_pw_qpolynomial_fold(
  ...
  if (!entry->data)
    ...
    entry->data = isl_pw_qpolynomial_fold_fold(
        entry->data, isl_pw_qpolynomial_fold_copy(part));
    if (!entry->data)
      goto error;
...
__isl_give isl_union_pw_qpolynomial_fold *isl_union_pw_qpolynomial_fold_fold(
  ...
  if (isl_union_pw_qpolynomial_fold_foreach_pw_qpolynomial_fold(u2, &fold_part,
                                                                &u1) < 0)
    goto error;
...
__isl_give isl_pw_qpolynomial_fold *isl_pw_qpolynomial_fold_from_pw_qpolynomial(
  ...
  pwf = isl_pw_qpolynomial_fold_alloc_size(isl_space_copy(pwqp->dim), type,
                                           pwqp->n);

  for (i = 0; i < pwqp->n; ++i)
    pwf = isl_pw_qpolynomial_fold_add_piece(
        pwf, isl_set_copy(pwqp->p[i].set),
        isl_qpolynomial_fold_alloc(type, isl_qpolynomial_copy(pwqp->p[i].qp)));
...
isl_qpolynomial_fold_eval(__isl_take isl_qpolynomial_fold *fold,
  ...
  isl_assert(pnt->dim->ctx, isl_space_is_equal(pnt->dim, fold->dim),
             goto error);
  ...
  if (fold->n == 0)
    ...
    for (i = 1; i < fold->n; ++i) {
      isl_val *v_i;
      v_i = isl_qpolynomial_eval(isl_qpolynomial_copy(fold->qp[i]),
                                 isl_point_copy(pnt));
...
isl_qpolynomial_fold_opt_on_domain(__isl_take isl_qpolynomial_fold *fold,
  ...
  for (i = 1; i < fold->n; ++i) {
    isl_val *opt_i;
    opt_i = isl_qpolynomial_opt_on_domain(isl_qpolynomial_copy(fold->qp[i]),
                                          isl_set_copy(set), max);
    if (max)
 ...
 */
static isl_bool
qpolynomial_fold_covers_on_domain(__isl_keep isl_set *set,
                                  __isl_keep isl_qpolynomial_fold *fold1,
                                  __isl_keep isl_qpolynomial_fold *fold2) {
  int i, j;
  int covers;
  ...
  for (i = 0; i < fold2->n; ++i) {
    for (j = 0; j < fold1->n; ++j) {
      ...
      d = isl_qpolynomial_sub(isl_qpolynomial_copy(fold1->qp[j]),
                              isl_qpolynomial_copy(fold2->qp[i]));
      sgn = isl_qpolynomial_sign(set, d);
      isl_qpolynomial_free(d);
      if (sgn == covers)
 ...
 */
isl_bool
isl_pw_qpolynomial_fold_covers(__isl_keep isl_pw_qpolynomial_fold *pwf1,
  ...
  for (i = 0; i < pwf2->n; ++i) {
    for (j = 0; j < pwf1->n; ++j) {
      ...
      covers = qpolynomial_fold_covers_on_domain(common, pwf1->p[j].fold,
                                                 pwf2->p[i].fold);
      isl_set_free(common);
      if (covers < 0 || !covers)
        return covers;
...
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_morph_domain(__isl_take isl_qpolynomial_fold *fold,
  ...
  for (i = 0; i < fold->n; ++i) {
    fold->qp[i] =
        isl_qpolynomial_morph_domain(fold->qp[i], isl_morph_copy(morph));
    if (!fold->qp[i])
      goto error;
  }
...
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_lift(__isl_take isl_qpolynomial_fold *fold,
                          __isl_take isl_space *space) {
  int i;
  ...
  for (i = 0; i < fold->n; ++i) {
    fold->qp[i] = isl_qpolynomial_lift(fold->qp[i], isl_space_copy(space));
    if (!fold->qp[i])
      goto error;
  }
...
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_move_dims(__isl_take isl_qpolynomial_fold *fold,
  ...
  for (i = 0; i < fold->n; ++i) {
    fold->qp[i] = isl_qpolynomial_move_dims(fold->qp[i], dst_type, dst_pos,
                                            src_type, src_pos, n);
    if (!fold->qp[i])
      goto error;
  }
...
isl_qpolynomial_fold_substitute(__isl_take isl_qpolynomial_fold *fold,
  ...
  for (i = 0; i < fold->n; ++i) {
    fold->qp[i] = isl_qpolynomial_substitute(fold->qp[i], type, first, n, subs);
    if (!fold->qp[i])
      goto error;
  }
...
static isl_stat add_pwqp(__isl_take isl_pw_qpolynomial *pwqp, void *user) {
  ...
  entry = isl_union_pw_qpolynomial_fold_find_part_entry(*upwf, pwqp->dim, 1);
  if (!entry)
    goto error;
  ...
  if (!entry->data)
    ...
    if (isl_pw_qpolynomial_fold_is_zero(entry->data))
      *upwf = isl_union_pw_qpolynomial_fold_remove_part_entry(*upwf, entry);
  }
...
isl_union_pw_qpolynomial_fold_add_union_pw_qpolynomial(
  ...
  upwf = isl_union_pw_qpolynomial_fold_align_params(
      upwf, isl_union_pw_qpolynomial_get_space(upwqp));
  upwqp = isl_union_pw_qpolynomial_align_params(
      upwqp, isl_union_pw_qpolynomial_fold_get_space(upwf));

  upwf = isl_union_pw_qpolynomial_fold_cow(upwf);
  if (!upwf || !upwqp)
    goto error;

  if (isl_union_pw_qpolynomial_foreach_pw_qpolynomial(upwqp, &add_pwqp, &upwf) <
      0)
    goto error;
...
isl_map_apply_pw_qpolynomial_fold(__isl_take isl_map *map,
  ...
  if (ok < 0)
    goto error;
  if (!ok)
    isl_die(ctx, isl_error_invalid, "incompatible dimensions", goto error);
  ...
  pwf = isl_pw_qpolynomial_fold_reset_domain_space(pwf, isl_set_get_space(dom));
...
static isl_stat
pw_qpolynomial_fold_apply(__isl_take isl_pw_qpolynomial_fold *pwf, void *user) {
  isl_space *map_dim;
  isl_space *pwf_dim;
  ...
  if (ok < 0)
    return isl_stat_error;
  if (ok) {
    pwf = isl_map_apply_pw_qpolynomial_fold(isl_map_copy(data->map), pwf,
                                            data->tight ? &data->tight : NULL);
    data->res =
        isl_union_pw_qpolynomial_fold_fold_pw_qpolynomial_fold(data->res, pwf);
  } else
    isl_pw_qpolynomial_fold_free(pwf);
...
isl_union_map_apply_union_pw_qpolynomial_fold(
  ...
  upwf = isl_union_pw_qpolynomial_fold_align_params(
      upwf, isl_union_map_get_space(umap));
  umap = isl_union_map_align_params(
      umap, isl_union_pw_qpolynomial_fold_get_space(upwf));
 ...
 */
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_realign_domain(__isl_take isl_qpolynomial_fold *fold,
  ...
  for (i = 0; i < fold->n; ++i) {
    fold->qp[i] =
        isl_qpolynomial_realign_domain(fold->qp[i], isl_reordering_copy(r));
    if (!fold->qp[i])
      goto error;
  }
...
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_mul_isl_int(__isl_take isl_qpolynomial_fold *fold,
                                 isl_int v) {
  int i;
...
__isl_give isl_qpolynomial_fold *
isl_qpolynomial_fold_scale(__isl_take isl_qpolynomial_fold *fold, isl_int v) {
...
isl_qpolynomial_fold_scale_val(__isl_take isl_qpolynomial_fold *fold,
  ...
  if (isl_val_is_neg(v))
    fold->type = isl_fold_type_negate(fold->type);
  for (i = 0; i < fold->n; ++i) {
    fold->qp[i] = isl_qpolynomial_scale_val(fold->qp[i], isl_val_copy(v));
    if (!fold->qp[i])
      goto error;
  }
