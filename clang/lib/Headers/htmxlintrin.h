...
static __inline void __attribute__((__always_inline__)) __TM_abort() {
...
__TM_is_footprint_exceeded(void *const __tdb_ptr) {
  ...
  return (tdb->format == 1 && (tdb->abort_code == 7 /* fetch overflow */
...
__TM_is_conflict(void *const __tdb_ptr) {
  ...
  return (tdb->format == 1 && (tdb->abort_code == 9 /* fetch conflict */
