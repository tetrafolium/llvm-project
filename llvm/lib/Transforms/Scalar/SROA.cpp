...
public:
    ...
    void makeUnsplittable() {
        UseAndIsSplittable.setInt(false);
    }
    ...
    bool isDead() const {
        return getUse() == nullptr;
    }
    void kill() {
        UseAndIsSplittable.setPointer(nullptr);
    }
...
AllocaSlices::AllocaSlices(const DataLayout &DL, AllocaInst &AI)
    ...
    llvm::erase_if(Slices, [](const Slice &S) {
        return S.isDead();
    });
...
bool SROA::presplitLoadsAndStores(AllocaInst &AI, AllocaSlices &AS) {
    ...
    llvm::erase_if(AS, [](const Slice &S) {
        return S.isDead();
    });
