...
__attribute__((__noinline__)) void thing(int x) { g = x; }
...
__attribute__((__noinline__)) int fun(int param) {
  ...
  if (param == 0) { //// end of alloca live range
    ...
    //// Instcombine runs LowerDbgDeclare and inserts a dbg.value after the
    ///load. / SelectionDAG combines the load and cmp. We go from this IR: / %0
    ///= load i32, i32* %param.addr, align 4, !dbg !42, !tbaa !20 /   call void
    ///@llvm.dbg.value(metadata i32 %0, ... /   %cmp = icmp eq i32 %0, 0, !dbg
    ///!44 / to this MIR: /   DBG_VALUE $noreg, $noreg, !"param"... /   CMP32mi8
    ///%param.addr, 1, $noreg, 0, $noreg, 0, implicit-def $eflags,
    ///debug-location !44
    thing(param);
  }
