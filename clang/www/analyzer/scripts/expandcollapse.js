...
function initExpandCollapse() {
  ...
  for (var i = 0; i < rows.length; i += 1) {
    ...
    for (var j = 0; j < divs.length; j += 1) {
      ...
      with(data.style) {
        height = (CLIP_HEIGHT - EXPANDER_HEIGHT) + "px";
        overflow = "hidden"
      }
      ...
      expander.onclick =
        ...
        this.src =
            imgPath + (getCellInfo(this.id).expanded ? "arrows_light.gif"
                                                     : "ellipses_light.gif");
      } expander.onmouseover =
              function() {
        this.src =
            imgPath + (getCellInfo(this.id).expanded ? "arrows_dark.gif"
                                                     : "ellipses_dark.gif");
      } expander.onmouseout =
                  function() {
        this.src =
      ...
      CellsInfo[groupCount][cellCount] = {
