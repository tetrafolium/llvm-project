...
namespace LLVM.ClangFormat {
  internal sealed class Vsix {
    ...
    public static IWpfTextView GetCurrentView() {
      // The SVsTextManager is a service through which we can get the active
      // view.
      var textManager =
          (IVsTextManager) Package.GetGlobalService(typeof(SVsTextManager));
      IVsTextView textView;
      textManager.GetActiveView(1, null, out textView);

      // Now we have the active view as IVsTextView, but the text interfaces we
      // need are in the IWpfTextView.
    ...
    public static ITextDocument GetTextDocument(IWpfTextView view) {
      ...
      if (view != null && view.TextBuffer.Properties.TryGetProperty(
