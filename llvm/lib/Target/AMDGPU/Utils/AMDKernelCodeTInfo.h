...
#define PRINTCODEPROP(name)                                                    \
  printBitField<FLD_T(code_properties), AMD_CODE_PROPERTY_##name##_SHIFT,      \
                AMD_CODE_PROPERTY_##name##_WIDTH>

#define PARSECODEPROP(name)                                                    \
  parseBitField<FLD_T(code_properties), AMD_CODE_PROPERTY_##name##_SHIFT,      \
                AMD_CODE_PROPERTY_##name##_WIDTH>
...
#define PRINTCOMP(GetMacro, Shift)                                             \
  [](StringRef Name, const amd_kernel_code_t &C, raw_ostream &OS) {            \
    printName(OS, Name) << (int)GetMacro(C.compute_pgm_resource_registers >>   \
                                         Shift);                               \
  }
#define PARSECOMP(SetMacro, Shift)                                             \
  [](amd_kernel_code_t &C, MCAsmParser &MCParser, raw_ostream &Err) {          \
    ...
    if (!expectAbsExpression(MCParser, Value, Err))                            \
      return false;                                                            \
    C.compute_pgm_resource_registers &=                                        \
        ~(SetMacro(0xFFFFFFFFFFFFFFFFULL) << Shift);                           \
...
FIELD2(amd_code_version_major, kernel_code_version_major,
    ...
    FIELD2(amd_code_version_minor, kernel_code_version_minor,
           amd_kernel_code_version_minor),
    FIELD2(amd_machine_kind, machine_kind, amd_machine_kind),
    FIELD2(amd_machine_version_major, machine_version_major,
           amd_machine_version_major),
    FIELD2(amd_machine_version_minor, machine_version_minor,
           amd_machine_version_minor),
    FIELD2(amd_machine_version_stepping, machine_version_stepping,
           amd_machine_version_stepping),

    FIELD(kernel_code_entry_byte_offset), FIELD(kernel_code_prefetch_byte_size),
    ...
    COMPPGM1(float_mode, compute_pgm_rsrc1_float_mode,
             FLOAT_MODE), // TODO: split float_mode
    ...
    COMPPGM2(enable_sgpr_private_segment_wave_byte_offset,
             compute_pgm_rsrc2_scratch_en, SCRATCH_EN),
    ...
    COMPPGM2(enable_sgpr_workgroup_id_x, compute_pgm_rsrc2_tgid_x_en,
             TGID_X_EN),
    COMPPGM2(enable_sgpr_workgroup_id_y, compute_pgm_rsrc2_tgid_y_en,
             TGID_Y_EN),
    COMPPGM2(enable_sgpr_workgroup_id_z, compute_pgm_rsrc2_tgid_z_en,
             TGID_Z_EN),
    COMPPGM2(enable_sgpr_workgroup_info, compute_pgm_rsrc2_tg_size_en,
             TG_SIZE_EN),
    COMPPGM2(enable_vgpr_workitem_id, compute_pgm_rsrc2_tidig_comp_cnt,
             TIDIG_COMP_CNT),
    COMPPGM2(enable_exception_msb, compute_pgm_rsrc2_excp_en_msb,
             EXCP_EN_MSB), // TODO: split enable_exception_msb
    COMPPGM2(granulated_lds_size, compute_pgm_rsrc2_lds_size, LDS_SIZE),
    COMPPGM2(enable_exception, compute_pgm_rsrc2_excp_en,
             EXCP_EN), // TODO: split enable_exception
    ...
    FIELD(workgroup_group_segment_byte_size), FIELD(gds_segment_byte_size),
    FIELD(kernarg_segment_byte_size), FIELD(workgroup_fbarrier_count),
    FIELD(wavefront_sgpr_count), FIELD(workitem_vgpr_count),
    FIELD(reserved_vgpr_first), FIELD(reserved_vgpr_count),
    FIELD(reserved_sgpr_first), FIELD(reserved_sgpr_count),
    FIELD(debug_wavefront_private_segment_offset_sgpr),
    FIELD(debug_private_segment_buffer_sgpr), FIELD(kernarg_segment_alignment),
    FIELD(group_segment_alignment), FIELD(private_segment_alignment),
    FIELD(wavefront_size), FIELD(call_convention),
