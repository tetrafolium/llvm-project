...
uset_convex_hull_wrap_bounded(__isl_take isl_set *set);
...
isl_basic_set_remove_redundancies(__isl_take isl_basic_set *bset) {
  return bset_from_bmap(isl_basic_map_remove_redundancies(bset_to_bmap(bset)));
}
 ...
 */
static isl_bool uset_is_bound(__isl_keep isl_set *set, isl_int *c,
                              unsigned len) {
  int first;
  int j;
  isl_int opt;
  ...
  for (j = 0; j < set->n; ++j) {
    ...
    res = isl_basic_set_solve_lp(set->p[j], 0, c, set->ctx->one, &opt,
                                 &opt_denom, NULL);
...
isl_set_add_basic_set_equality(__isl_take isl_set *set, isl_int *c) {
  int i;
...
isl_int *isl_set_wrap_facet(__isl_keep isl_set *set, isl_int *facet,
  ...
  isl_int_init(num);
  isl_int_init(den);
  res = isl_basic_set_solve_lp(lp, 0, obj->block.data, ctx->one, &num, &den,
                               NULL);
  if (res == isl_lp_ok) {
    isl_int_neg(num, num);
  ...
  if (res == isl_lp_error)
    return NULL;
  isl_assert(ctx, res == isl_lp_ok || res == isl_lp_unbounded, return NULL);
  return facet;
error:
  isl_basic_set_free(lp);
  isl_mat_free(T);
  ...
  for (;;) {
    ...
    for (i = 0; i < face->n_eq; ++i)
      if (!isl_seq_eq(bounds->row[0], face->eq[i], 1 + dim) &&
          !isl_seq_is_neg(bounds->row[0], face->eq[i], 1 + dim))
        break;
  ...
  for (i = 0; i < hull->n_ineq; ++i) {
    ...
    for (j = 0; j < facet->n_ineq; ++j) {
      for (f = 0; f < hull_facet->n_ineq; ++f)
        if (isl_seq_eq(facet->ineq[j], hull_facet->ineq[f], 1 + dim))
          break;
 ...
 */
static __isl_give isl_basic_set *
convex_hull_pair_elim(__isl_take isl_basic_set *bset1,
  ...
  for (i = 0; i < 2; ++i) {
    for (j = 0; j < bset[i]->n_eq; ++j) {
      ...
      isl_seq_cpy(hull->eq[k] + (i + 1) * (1 + dim), bset[i]->eq[j], 1 + dim);
    }
    for (j = 0; j < bset[i]->n_ineq; ++j) {
      ...
      isl_seq_cpy(hull->ineq[k] + (i + 1) * (1 + dim), bset[i]->ineq[j],
                  1 + dim);
    }
    k = isl_basic_set_alloc_inequality(hull);
    if (k < 0)
...
isl_bool isl_basic_map_image_is_bounded(__isl_keep isl_basic_map *bmap) {
  ...
  bmap =
      isl_basic_map_move_dims(bmap, isl_dim_param, nparam, isl_dim_in, 0, n_in);
 ...
 */
static __isl_give isl_basic_set *
induced_lineality_space(__isl_take isl_basic_set *bset1,
                        __isl_take isl_basic_set *bset2) {
  int i, k;
 ...
 */
static __isl_give isl_basic_set *
modulo_lineality(__isl_take isl_set *set, __isl_take isl_basic_set *lin) {
 ...
 */
static __isl_give isl_basic_set *
valid_direction_lp(__isl_take isl_basic_set *bset1,
                   __isl_take isl_basic_set *bset2) {
  isl_space *space;
  struct isl_basic_set *lp;
  unsigned d;
  ...
  if (total < 0 || !bset2)
    goto error;
  d = 1 + total;
  n = 2 + 2 * bset1->n_eq + bset1->n_ineq + 2 * bset2->n_eq + bset2->n_ineq;
...
static __isl_give isl_vec *valid_direction(__isl_take isl_basic_set *bset1,
  ...
  if (!bset1 || !bset2)
    goto error;
  lp = valid_direction_lp(isl_basic_set_copy(bset1), isl_basic_set_copy(bset2));
  ...
  n++;
  for (i = 0; i < bset1->n_eq; ++i) {
    isl_int_sub(sample->block.data[n], sample->block.data[n],
                sample->block.data[n + 1]);
    isl_seq_combine(dir->block.data, bset1->ctx->one, dir->block.data,
 ...
 */
static __isl_give isl_basic_set *
convex_hull_pair_pointed(__isl_take isl_basic_set *bset1,
                         __isl_take isl_basic_set *bset2) {
  ...
  dir = valid_direction(isl_basic_set_copy(bset1), isl_basic_set_copy(bset2));
  if (!dir)
    goto error;
...
static __isl_give isl_basic_set *
modulo_affine_hull(__isl_take isl_set *set,
 ...
 */
static __isl_give isl_basic_set *
convex_hull_pair(__isl_take isl_basic_set *bset1,
                 __isl_take isl_basic_set *bset2) {
...
isl_basic_set_lineality_space(__isl_take isl_basic_set *bset) {
  ...
  lin = isl_basic_set_alloc_space(isl_basic_set_get_space(bset), n_div, dim, 0);
  for (i = 0; i < n_div; ++i)
...
isl_set_combined_lineality_space(__isl_take isl_set *set) {
  ...
  for (i = 0; i < set->n; ++i)
    lin = isl_set_add_basic_set(
        lin, isl_basic_set_lineality_space(isl_basic_set_copy(set->p[i])));
  isl_set_free(set);
 ...
 */
static __isl_give isl_basic_set *
uset_convex_hull_unbounded(__isl_take isl_set *set) {
  ...
  while (list) {
    ...
    if (n < 0)
      goto error;
    if (n < 2)
      isl_die(isl_basic_set_list_get_ctx(list), isl_error_internal,
...
static isl_stat update_constraint(struct isl_ctx *ctx,
                                  struct isl_hash_table *table, isl_int *con,
                                  unsigned len, int n, int ineq) {
  ...
  entry =
      isl_hash_table_find(ctx, table, c_hash, max_constraint_equal, con + 1, 0);
  if (!entry)
 ...
 */
static __isl_give isl_basic_set *
common_constraints(__isl_take isl_basic_set *hull, __isl_keep isl_set *set,
                   int *is_hull) {
  int i, j, s, n;
  int min_constraints;
  int best;
  ...
  *is_hull = 0;
  ...
  constraints =
      isl_calloc_array(hull->ctx, struct max_constraint, min_constraints);
  if (!constraints)
    return hull;
  ...
  for (i = 0; i < set->p[best]->n_ineq; ++i) {
    constraints[i].c = isl_mat_sub_alloc6(hull->ctx, set->p[best]->ineq + i, 0,
                                          1, 0, 1 + total);
    if (!constraints[i].c)
      goto error;
  ...
  for (i = 0; i < min_constraints; ++i) {
    ...
    entry = isl_hash_table_find(hull->ctx, table, c_hash, max_constraint_equal,
                                constraints[i].c->row[0] + 1, 1);
    if (!entry)
      goto error;
  ...
  for (s = 0; s < set->n; ++s) {
    ...
    for (i = 0; i < set->p[s]->n_eq; ++i) {
      ...
      for (j = 0; j < 2; ++j) {
        ...
        if (update_constraint(hull->ctx, table, eq, total, n, 0) < 0)
          goto error;
      }
    }
...
static __isl_give isl_basic_set *
uset_convex_hull_wrap(__isl_take isl_set *set) {
  struct isl_basic_set *hull;
  int is_hull;
...
modulo_affine_hull(__isl_take isl_set *set,
  ...
  dummy =
      isl_basic_set_remove_equalities(isl_basic_set_copy(affine_hull), &T, &T2);
  if (!dummy)
    goto error;
...
replace_map_by_empty_basic_map(__isl_take isl_map *map) {
  isl_space *space;
...
static struct sh_data *sh_data_alloc(__isl_keep isl_set *set, unsigned n_ineq) {
  ...
  for (i = 0; i < set->n; ++i) {
    data->p[i].table =
        isl_hash_table_alloc(set->ctx, 2 * set->p[i]->n_eq + set->p[i]->n_ineq);
    if (!data->p[i].table)
      goto error;
...
static int is_bound(struct sh_data *data, __isl_keep isl_set *set, int j,
  ...
  res = isl_tab_min(data->p[j].tab, ineq, data->ctx->one, &opt, NULL, 0);
  ...
  return (res == isl_lp_ok || res == isl_lp_empty)
             ? 1
...
static isl_stat set_max_constant_term(struct sh_data *data,
                                      __isl_keep isl_set *set, int i,
                                      isl_int *ineq, uint32_t c_hash,
                                      struct ineq_cmp_data *v) {
  int j;
  isl_ctx *ctx;
  struct isl_hash_table_entry *entry;
  ...
  for (j = i + 1; j < set->n; ++j) {
    ...
    entry = isl_hash_table_find(ctx, data->p[j].table, c_hash, &has_ineq, v, 0);
    if (!entry)
      return isl_stat_error;
    if (entry == isl_hash_table_entry_none)
      continue;
...
static __isl_give isl_basic_set *add_bound(__isl_take isl_basic_set *hull,
  ...
  entry =
      isl_hash_table_find(hull->ctx, data->hull_table, c_hash, has_ineq, &v, 0);
  if (!entry)
    return isl_basic_set_free(hull);
  if (entry != isl_hash_table_entry_none)
    return hull;
  ...
  for (j = 0; j < i; ++j) {
    entry = isl_hash_table_find(hull->ctx, data->p[j].table, c_hash, has_ineq,
                                &v, 0);
    if (!entry)
      return isl_basic_set_free(hull);
    if (entry != isl_hash_table_entry_none)
  ...
  entry =
      isl_hash_table_find(hull->ctx, data->hull_table, c_hash, has_ineq, &v, 1);
  if (!entry)
    goto error;
...
static __isl_give isl_basic_set *add_bounds(__isl_take isl_basic_set *bset,
  ...
  for (j = 0; j < set->p[i]->n_eq; ++j) {
    for (k = 0; k < 2; ++k) {
      ...
      bset = add_bound(bset, data, set, i, set->p[i]->eq[j], shift);
    }
  }
  for (j = 0; j < set->p[i]->n_ineq; ++j)
 ...
 */
static __isl_give isl_basic_map *
map_simple_hull_trivial(__isl_take isl_map *map) {
  isl_basic_map *hull;
 ...
 */
static __isl_give isl_basic_map *
select_shared_inequalities(__isl_take isl_basic_map *bmap1,
  ...
  while (bmap1 && i1 >= 0 && i2 >= 0) {
    ...
    cmp = isl_basic_map_constraint_cmp(bmap1, bmap1->ineq[i1], bmap2->ineq[i2]);
    if (cmp < 0) {
      --i2;
      continue;
    }
 ...
 */
static __isl_give isl_basic_map *
select_shared_equalities(__isl_take isl_basic_map *bmap1,
                         __isl_keep isl_basic_map *bmap2) {
  int i1, i2;
  isl_size total;
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_plain_unshifted_simple_hull(__isl_take isl_basic_map *bmap1,
 ...
 */
static __isl_give isl_basic_set *
add_bound_from_constraint(__isl_take isl_basic_set *hull, struct sh_data *data,
  ...
  for (i = 0; i < set->n; ++i) {
    ...
    entry =
        isl_hash_table_find(ctx, data->p[i].table, c_hash, &has_ineq, &v, 0);
    if (!entry)
      return isl_basic_set_free(hull);
    if (entry != isl_hash_table_entry_none) {
 ...
 */
static __isl_give isl_basic_set *
uset_unshifted_simple_hull_from_constraints(__isl_take isl_set *set, int n_ineq,
                                            isl_int **ineq) {
  int i;
  int last_added = 0;
  struct sh_data *data = NULL;
  ...
  for (i = 0; i < n_ineq; ++i) {
    ...
    parallel = i > 0 && isl_seq_eq(ineq[i - 1] + 1, ineq[i] + 1, dim);
    if (parallel && (last_added || isl_int_eq(ineq[i - 1][0], ineq[i][0])))
      continue;
...
collect_inequalities(__isl_take isl_mat *mat,
 ...
 */
static __isl_give isl_basic_map_list *
collect_basic_maps(__isl_take isl_map_list *list) {
  int i;
  isl_size n;
  isl_ctx *ctx;
  ...
  for (i = 0; i < dim; ++i) {
    ...
    for (j = 0; j < hull->n_eq; ++j) {
      if (isl_int_is_zero(hull->eq[j][1 + nparam + i]))
        continue;
      if (isl_seq_first_non_zero(hull->eq[j] + 1 + nparam + i + 1, left) == -1)
        break;
    }
    if (j < hull->n_eq)
      continue;
    ...
    for (j = 0; j < hull->n_ineq; ++j) {
      if (isl_int_is_zero(hull->ineq[j][1 + nparam + i]))
        continue;
      if (isl_seq_first_non_zero(hull->ineq[j] + 1 + nparam + i + 1, left) !=
              -1 ||
