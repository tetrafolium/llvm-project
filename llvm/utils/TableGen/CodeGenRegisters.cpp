...
CodeGenRegister::CodeGenRegister(Record *R, unsigned Enum)
    : TheDef(R), EnumValue(Enum), CostPerUse(R->getValueAsInt("CostPerUse")),
...
void CodeGenRegister::computeSuperRegs(CodeGenRegBank &RegBank) {
  ...
  for (SubRegMap::const_iterator I = SubRegs.begin(), E = SubRegs.end(); I != E;
       ++I)
...
unsigned CodeGenRegister::getWeight(const CodeGenRegBank &RegBank) const {
  unsigned Weight = 0;
  for (RegUnitList::iterator I = RegUnits.begin(), E = RegUnits.end(); I != E;
       ++I) {
  ...
  void expand(SetTheory &ST, Record *Def, SetTheory::RecSet &Elts) override {
    ...
    for (unsigned n = 0; n != Length; ++n) {
      ...
      for (unsigned i = 0; i != Dim; ++i) {
        ...
        CostPerUse =
            std::max(CostPerUse, unsigned(Reg->getValueAsInt("CostPerUse")));
      }
...
void CodeGenRegBank::computeRegUnitLaneMasks() {
  for (auto &Register : Registers) {
    ...
    for (SubRegMap::const_iterator S = SubRegs.begin(), SE = SubRegs.end();
         S != SE; ++S) {
...
void CodeGenRegBank::inferMatchingSuperRegClass(
  ...
  SmallVector<std::pair<const CodeGenRegister *, const CodeGenRegister *>, 16>
      SSPairs;
  BitVector TopoSigs(getNumTopoSigs());
