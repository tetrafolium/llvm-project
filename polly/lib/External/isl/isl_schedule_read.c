...
isl_stream_read_schedule_tree(__isl_keep isl_stream *s);
...
static __isl_give isl_schedule_tree *read_context(__isl_keep isl_stream *s) {
  ...
  if (!more) {
    ...
    if (key != isl_schedule_key_child)
      isl_die(ctx, isl_error_invalid, "expecting child", goto error);
    if (isl_stream_yaml_next(s) < 0)
      goto error;
...
static __isl_give isl_schedule_tree *read_expansion(isl_stream *s) {
  ...
  do {
    ...
    case isl_schedule_key_contraction:
      ...
      tok = isl_stream_next_token(s);
      str = isl_token_get_str(ctx, tok);
      contraction = isl_union_pw_multi_aff_read_from_str(ctx, str);
      free(str);
      isl_token_free(tok);
      if (!contraction)
        goto error;
    ...
    case isl_schedule_key_child:
      ...
      break;
    default:
      isl_die(ctx, isl_error_invalid, "unexpected key", goto error);
    }
  ...
  if (!contraction)
    isl_die(ctx, isl_error_invalid, "missing contraction", goto error);
  if (!expansion)
    isl_die(ctx, isl_error_invalid, "missing expansion", goto error);
 ...
 */
static __isl_give isl_schedule_band *
set_coincident(__isl_take isl_schedule_band *band,
  ...
  for (i = 0; i < n && i < m; ++i) {
    ...
    if (!v)
      band = isl_schedule_band_free(band);
    band =
        isl_schedule_band_member_set_coincident(band, i, !isl_val_is_zero(v));
    isl_val_free(v);
  }
...
static __isl_give isl_schedule_tree *read_band(isl_stream *s) {
  ...
  do {
    ...
    case isl_schedule_key_schedule:
      ...
      str = isl_token_get_str(ctx, tok);
      schedule = isl_multi_union_pw_aff_read_from_str(ctx, str);
      free(str);
      isl_token_free(tok);
      if (!schedule)
        goto error;
 ...
 */
static __isl_give isl_schedule_tree *
read_children(isl_stream *s, enum isl_schedule_node_type type) {
  isl_ctx *ctx;
 ...
 */
static __isl_give isl_schedule_tree *
isl_stream_read_schedule_tree(struct isl_stream *s) {
  enum isl_schedule_key key;
  struct isl_token *tok;
  isl_schedule_tree *tree = NULL;
  int more;
