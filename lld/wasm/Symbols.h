...
public:
    ...
    bool hasGOTIndex() const {
        return gotIndex != INVALID_INDEX;
    }
...
public:
    ...
    const WasmGlobalType *getGlobalType() const {
        return globalType;
    }
...
public:
    static bool classof(const Symbol *s) {
        return s->kind() == DefinedEventKind;
    }

    const WasmEventType *getEventType() const {
        return eventType;
    }
