...
enum isl_token_type isl_stream_register_keyword(__isl_keep isl_stream *s,
  ...
  entry =
      isl_hash_table_find(s->ctx, s->keywords, name_hash, same_name, name, 1);
  if (!entry)
    return ISL_TOKEN_ERROR;
  if (entry->data) {
    keyword = entry->data;
...
struct isl_token *isl_token_new(isl_ctx *ctx, int line, int col,
...
__isl_give isl_val *isl_token_get_val(isl_ctx *ctx, struct isl_token *tok) {
  ...
  if (tok->type != ISL_TOKEN_VALUE)
    isl_die(ctx, isl_error_invalid, "not a value token", return NULL);
...
__isl_give char *isl_token_get_str(isl_ctx *ctx, struct isl_token *tok) {
  ...
  if (!tok->u.s)
    isl_die(ctx, isl_error_invalid,
            "token does not have a string representation", return NULL);
...
static struct isl_token *next_token(__isl_keep isl_stream *s, int same_line) {
  ...
  if (isalpha(c) || c == '_') {
    ...
    while ((c = isl_stream_getc(s)) != -1 && (isalnum(c) || c == '_'))
      isl_stream_push_char(s, c);
    if (c != -1)
      isl_stream_ungetc(s, c);
    while ((c = isl_stream_getc(s)) != -1 && c == '\'')
...
static int push_state(__isl_keep isl_stream *s, enum isl_yaml_state state) {
  if (s->yaml_size < s->yaml_depth + 1) {
    ...
    state = isl_realloc_array(s->ctx, s->yaml_state, enum isl_yaml_state,
                              s->yaml_depth + 1);
    if (!state)
      return -1;
    s->yaml_state = state;

    indent = isl_realloc_array(s->ctx, s->yaml_indent, int, s->yaml_depth + 1);
    if (!indent)
      return -1;
...
static int pop_state(__isl_keep isl_stream *s) {
  if (!s)
    return -1;
  if (s->yaml_depth < 1)
    isl_die(isl_stream_get_ctx(s), isl_error_invalid, "not in YAML construct",
            return -1);
...
static int set_yaml_indent(__isl_keep isl_stream *s, int indent) {
  if (s->yaml_depth < 1)
    isl_die(s->ctx, isl_error_internal, "not in YAML element", return -1);
...
int isl_stream_yaml_next(__isl_keep isl_stream *s) {
  ...
  if (state == isl_yaml_none)
    isl_die(s->ctx, isl_error_invalid, "not in YAML element", return -1);
  switch (state) {
  ...
  case isl_yaml_sequence:
    ...
    return 1;
  default:
    isl_die(s->ctx, isl_error_internal, "unexpected state", return 0);
  }
}
...
int isl_stream_yaml_read_end_mapping(__isl_keep isl_stream *s) {
  ...
  if (indent >= get_yaml_indent(s))
    isl_die(isl_stream_get_ctx(s), isl_error_invalid, "mapping not finished",
            return -1);
...
int isl_stream_yaml_read_end_sequence(__isl_keep isl_stream *s) {
  ...
  if (indent >= get_yaml_indent(s) && dash)
    isl_die(isl_stream_get_ctx(s), isl_error_invalid, "sequence not finished",
            return -1);
