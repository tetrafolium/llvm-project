...
static char *get_filename(isl_ctx *ctx, const char *name, const char *suffix) {
  ...
  length =
      strlen(pattern) - 6 + strlen(srcdir) + strlen(name) + strlen(suffix) + 1;
...
int test_parse_map_equal(isl_ctx *ctx, const char *str, const char *str2) {
  ...
  if (equal < 0)
    return -1;
  if (!equal)
    isl_die(ctx, isl_error_unknown, "maps not equal", return -1);
...
struct {
  ...
  const char *map2;
    ...
    {"{ [x,y]  : [([x/2]+y)/3] >= 1 }", "{ [x, y] : 2y >= 6 - x }"},
    {"{ [x,y] : x <= min(y, 2*y+3) }", "{ [x,y] : x <= y, 2*y + 3 }"},
    {"{ [x,y] : x >= min(y, 2*y+3) }",
    ...
    {"{ [i,j] -> [i] : i < j; [i,j] -> [j] : j <= i }",
     "{ [i,j] -> [min(i,j)] }"},
    {"{ [i,j] : i != j }", "{ [i,j] : i < j or i > j }"},
    {"{ [i,j] : (i+1)*2 >= j }", "{ [i, j] : j <= 2 + 2i }"},
    {"{ [i] -> [i > 0 ? 4 : 5] }",
     "{ [i] -> [5] : i <= 0; [i] -> [4] : i >= 1 }"},
    {"[N=2,M] -> { [i=[(M+N)/4]] }",
    ...
    {"{ [i] : ((i > 10)) }", "{ [i] : i >= 11 }"},
    {"{ [i] -> [0] }", "{ [i] -> [0 * i] }"},
    {"{ [a] -> [b] : (not false) }", "{ [a] -> [b] : true }"},
    {"{ [i] : i/2 <= 5 }", "{ [i] : i <= 10 }"},
    {"{Sym=[n] [i] : i <= n }", "[n] -> { [i] : i <= n }"},
    {"{ [*] }", "{ [a] }"},
    {"{ [i] : 2*floor(i/2) = i }", "{ [i] : exists a : i = 2 a }"},
    {"{ [a] -> [b] : a = 5 implies b = 5 }",
     "{ [a] -> [b] : a != 5 or b = 5 }"},
    {"{ [a] -> [a - 1 : a > 0] }", "{ [a] -> [a - 1] : a > 0 }"},
    {"{ [a] -> [a - 1 : a > 0; a : a <= 0] }",
    ...
    {"{ [x] -> [] : exists (a, b: 0 <= a <= 1 and 0 <= b <= 3 and "
     ...
     "{ [x] -> [] : 0 <= x <= 15 }"},
    {"{ [x] -> [x] : }", "{ [x] -> [x] }"},
    {"{ [x=4:5] -> [x + 1] }", "{ [x] -> [x + 1] : 4 <= x <= 5 }"},
    {"{ [x=4:5] -> [x + 1 : x + 1] }", "{ [x=4:5] -> [x + 1] }"},
    {"{ [x] -> [x - 1 : x + 1] }", "{ [x] -> [y] : x - 1 <= y <= x + 1 }"},
    {"{ [x=4:] -> [x + 1] }", "{ [x] -> [x + 1] : 4 <= x }"},
    {"{ [x=:5] -> [x + 1] }", "{ [x] -> [x + 1] : x <= 5 }"},
    {"{ [x=:] -> [x + 1] }", "{ [x] -> [x + 1] }"},
    {"{ [:] -> [:] }", "{ [x] -> [y] }"},
...
static int test_read(isl_ctx *ctx) {
  ...
  if (equal < 0)
    return -1;
  if (!equal)
    isl_die(ctx, isl_error_unknown, "read sets not equal", return -1);
...
static int test_bounded(isl_ctx *ctx) {
  ...
  if (bounded < 0)
    return -1;
  if (!bounded)
    isl_die(ctx, isl_error_unknown, "set not considered bounded", return -1);
  ...
  if (bounded < 0)
    return -1;
  if (bounded)
    isl_die(ctx, isl_error_unknown, "set considered bounded", return -1);
...
static int test_construction_2(isl_ctx *ctx) {
  ...
  bset1 = isl_basic_set_upper_bound_val(bset1, isl_dim_set, 0, isl_val_copy(v));
  bset1 = isl_basic_set_lower_bound_val(bset1, isl_dim_set, 0, isl_val_neg(v));
  ...
  if (equal < 0)
    return -1;
  if (!equal)
    isl_die(ctx, isl_error_unknown, "failed construction", return -1);
...
static int test_dim(isl_ctx *ctx) {
  ...
  map1 = isl_map_read_from_str(
      ctx, "[n] -> { [i] -> [j] : exists (a = [i/10] : i - 10a <= n ) }");
  map1 = isl_map_add_dims(map1, isl_dim_in, 1);
  map2 = isl_map_read_from_str(
      ctx, "[n] -> { [i,k] -> [j] : exists (a = [i/10] : i - 10a <= n ) }");
...
struct {
  __isl_give isl_val *(*fn)(__isl_take isl_val *v1, __isl_take isl_val *v2);
    ...
    ['+'] = {&isl_val_add}, ['-'] = {&isl_val_sub}, ['*'] = {&isl_val_mul},
    ['/'] = {&isl_val_div}, ['g'] = {&isl_val_gcd}, ['m'] = {&isl_val_min},
    ['M'] = {&isl_val_max},
};
...
static int test_elimination(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(elimination_tests); ++i) {
    ...
    if (n < 0)
      return -1;
    if (n != 0)
      isl_die(ctx, isl_error_unknown, "expecting no existentials", return -1);
  }
...
static int test_div(isl_ctx *ctx) {
  ...
  */
  ...
  if (empty < 0)
    return -1;
  if (!empty)
    isl_die(ctx, isl_error_unknown, "result not as accurate as expected",
            return -1);
...
static int test_recession_cone(struct isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(recession_cone_tests); ++i) {
    ...
    if (equal < 0)
      return -1;
    if (!equal)
      isl_die(ctx, isl_error_unknown, "unexpected cone", return -1);
  }
...
int test_affine_hull(struct isl_ctx *ctx) {
  ...
  if (n < 0)
    return -1;
  if (n != 0)
    isl_die(ctx, isl_error_unknown, "not expecting any divs", return -1);
  ...
  if (subset < 0)
    return -1;
  if (!subset)
    isl_die(ctx, isl_error_unknown, "not as accurate as expected", return -1);
...
static isl_stat test_plain_unshifted_simple_hull_special(isl_ctx *ctx) {
  ...
  if (is_universe < 0)
    return isl_stat_error;
  if (!is_universe)
    isl_die(ctx, isl_error_unknown, "hull should not have any constraints",
            return isl_stat_error);
...
static int test_plain_unshifted_simple_hull(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(plain_unshifted_simple_hull_tests); ++i) {
    ...
    if (equal < 0)
      return -1;
    if (!equal)
      isl_die(ctx, isl_error_unknown, "unexpected hull", return -1);
  }
...
static int test_simple_hull(struct isl_ctx *ctx) {
  ...
  if (is_empty == isl_bool_false)
    isl_die(ctx, isl_error_unknown, "Empty set should be detected", return -1);
...
static struct {
  ...
  const char *size;
    ...
    {"[N] -> { S[x, y] : N <= x, y < N + 10 }", "[N] -> { S[N, N] }",
     "{ S[10, 10] }"},
    {"{ S[x, y] : 0 <= x + y, x - y < 10 }", "{ S[0, -4] }", "{ S[10, 9] }"},
...
static int test_convex_hull_algo(isl_ctx *ctx, int convex) {
  ...
  for (i = 0; i < ARRAY_SIZE(convex_hull_tests); ++i) {
    ...
    if (equal < 0)
      return -1;
    if (!equal)
      isl_die(ctx, isl_error_unknown, "unexpected convex hull", return -1);
  }
...
static int test_plain_gist(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(plain_gist_tests); ++i) {
    ...
    if (equal < 0)
      return -1;
    if (!equal)
      isl_die(ctx, isl_error_unknown, "incorrect gist result", return -1);
  }
...
static isl_stat test_gist_fail(struct isl_ctx *ctx) {
  ...
  if (i < n)
    isl_die(ctx, isl_error_unknown, "operation not expected to succeed",
            return isl_stat_error);
...
struct {
  ...
  const char *gist;
    ...
    {"{ [m, n, a, b] : a <= 2147 + n }",
     ...
     "b >= 0) }",
     "{ [m, n, ku, kl] }"},
    {"{ [a, a, b] : a >= 10 }", "{ [a, b, c] : c >= a and c <= b and c >= 2 }",
     "{ [a, a, b] : a >= 10 }"},
...
static int test_gist(struct isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(gist_tests); ++i) {
    ...
    if (!equal)
      isl_die(ctx, isl_error_unknown, "incorrect gist result", return -1);
    if (!equal_input)
      isl_die(ctx, isl_error_unknown, "gist modified input", return -1);
    if (!equal_input)
      isl_die(ctx, isl_error_unknown, "inconsistent gist test case", return -1);
  }
  ...
  if (n_div < 0)
    return -1;
  if (n_div != 1)
    isl_die(ctx, isl_error_unknown, "expecting single div", return -1);
...
int test_coalesce_set(isl_ctx *ctx, const char *str, int check_one) {
  ...
  if (equal < 0)
    return -1;
  if (!equal)
    isl_die(ctx, isl_error_unknown, "coalesced set not equal to input",
            return -1);
  if (check_one && !one)
    isl_die(ctx, isl_error_unknown, "coalesced set should not be a union",
            return -1);
 ...
 */
static isl_stat test_coalesce_intersection(isl_ctx *ctx, const char *s1,
                                           const char *s2) {
  isl_set *set1, *set2;
...
static __isl_give isl_map *cocoa_fig_1_right_graph(isl_ctx *ctx) {
  ...
  dom = isl_set_read_from_str(
      ctx, "{ [x,y] : x >= 0 and -2 x + 3 y >= 0 and x <= 3 and "
           "2 x - 3 y + 3 >= 0 }");
...
static int test_closure(isl_ctx *ctx) {
  ...
  map = isl_map_read_from_str(
      ctx, "[n] -> { [i,j] -> [i2,j2] : i2 = i + 1 and j2 = j + 1 and "
  ...
  assert(exact);
  map2 = isl_map_read_from_str(
      ctx, "[n] -> { [i,j] -> [i2,j2] : exists (k1,k2,k : "
  ...
  map = isl_map_read_from_str(
      ctx, "[n] -> { [x] -> [y] : y = x + 1 and 0 <= x and x <= n and "
           " 0 <= y and y <= n }");
  map = isl_map_transitive_closure(map, &exact);
  map2 = isl_map_read_from_str(
      ctx, "[n] -> { [x] -> [y] : y > x and 0 <= x and x <= n and "
           " 0 <= y and y <= n }");
  assert(isl_map_is_equal(map, map2));
  isl_map_free(map2);
  isl_map_free(map);
  ...
  /* COCOA example 2 */
  map = isl_map_read_from_str(
      ctx, "[n] -> { [i,j] -> [i2,j2] : i2 = i + 2 and j2 = j + 2 and "
  ...
  /* COCOA Fig.2 left */
  map = isl_map_read_from_str(
      ctx, "[n] -> { [i,j] -> [i2,j2] : i2 = i + 2 and j2 = j and "
  ...
  /* COCOA Fig.2 right */
  map = isl_map_read_from_str(
      ctx, "[n] -> { [i,j] -> [i2,j2] : i2 = i + 3 and j2 = j and "
  ...
  assert(exact);
  map2 = isl_map_read_from_str(
      ctx, "[n] -> { [i,j] -> [i2,j2] : exists (k1,k2,k3,k : "
  ...
  map = isl_map_read_from_str(
      ctx, "{ [i,j] -> [i2,j2] : i = 0 and 0 <= j and j <= 1 and "
           "i2 = 1 and j2 = j or "
  ...
  map = isl_map_read_from_str(
      ctx, "[m,n] -> { [i,j] -> [i2,j2] : i2 = i and j2 = j + 2 and "
  ...
  map = isl_map_read_from_str(
      ctx, "[n] -> { [i,j] -> [i2,j2] : i2 = i and j2 = j + 1 and "
           "1 <= i,j,j+1 <= n or "
  ...
  /* Omega's closure4 */
  map = isl_map_read_from_str(
      ctx, "[m,n] -> { [x,y] -> [x2,y2] : x2 = x and y2 = y + 1 and "
           "1 <= x,y <= 10 or "
           "x2 = x + 1 and y2 = y and "
           "1 <= x <= 20 && 5 <= y <= 15 }");
  map = isl_map_transitive_closure(map, &exact);
...
static isl_stat check_bool(isl_ctx *ctx, isl_bool actual, isl_bool expected) {
  if (actual != expected)
    isl_die(ctx, isl_error_unknown, "incorrect boolean operation",
            return isl_stat_error);
  return isl_stat_ok;
}
...
static int test_lex(struct isl_ctx *ctx) {
  ...
  if (empty < 0)
    return -1;
  if (empty)
    isl_die(ctx, isl_error_unknown, "expecting non-empty result", return -1);
...
static int test_min_special(isl_ctx *ctx) {
  ...
  if (!res)
    return -1;
  if (!ok)
    isl_die(ctx, isl_error_unknown, "unexpected minimum", return -1);
...
struct {
  ...
  const char *res;
    ...
    {"{ [a, b] : 0 <= a, b <= 100 and b mod 2 = 0}",
     "{ [a, b] -> [floor((b - 2*floor((-a)/4))/5)] }", &isl_set_max_val, "30"},
...
static int test_min(struct isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(opt_tests); ++i) {
    ...
    if (ok < 0)
      return -1;
    if (!ok)
      isl_die(ctx, isl_error_unknown, "unexpected optimum", return -1);
  }
...
static int map_check_equal(__isl_keep isl_map *map, const char *str) {
  ...
  if (equal < 0)
    return -1;
  if (!equal)
    isl_die(isl_map_get_ctx(map), isl_error_unknown, "result not as expected",
            return -1);
  return 0;
}
...
static isl_stat set_check_equal(__isl_keep isl_set *set, const char *str) {
  ...
  if (equal < 0)
    return isl_stat_error;
  if (!equal)
    isl_die(isl_set_get_ctx(set), isl_error_unknown, "result not as expected",
            return isl_stat_error);
  return isl_stat_ok;
}
...
int test_sv(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(sv_tests); ++i) {
    ...
    if (sv < 0)
      return -1;
    if (sv_tests[i].sv && !sv)
      isl_die(ctx, isl_error_internal, "map not detected as single valued",
              return -1);
    if (!sv_tests[i].sv && sv)
      isl_die(ctx, isl_error_internal, "map detected as single valued",
              return -1);
  }
...
static int test_bijective(struct isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(bijective_tests); ++i) {
    ...
    if (bijective_tests[i].bijective && !bijective)
      isl_die(ctx, isl_error_internal, "map not detected as bijective",
              return -1);
    if (!bijective_tests[i].bijective && bijective)
      isl_die(ctx, isl_error_internal, "map detected as bijective", return -1);
  }
...
static isl_stat test_pwqp_gist(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(pwqp_gist_tests); ++i) {
    ...
    if (equal < 0)
      return isl_stat_error;
    if (!equal)
      isl_die(ctx, isl_error_unknown, "unexpected result",
              return isl_stat_error);
  }
...
static int test_pwqp(struct isl_ctx *ctx) {
  ...
  pwqp1 =
      isl_pw_qpolynomial_move_dims(pwqp1, isl_dim_param, 0, isl_dim_in, 1, 1);
  ...
  str = "{ [a,b,c] -> (([(2*[a/3]+b)/5]) * ([(2*[c/3]+b)/5])) }";
  pwqp1 = isl_pw_qpolynomial_read_from_str(ctx, str);
  pwqp1 = isl_pw_qpolynomial_fix_val(pwqp1, isl_dim_set, 1, isl_val_one(ctx));
  equal = isl_pw_qpolynomial_plain_is_equal(pwqp1, pwqp2);
  isl_pw_qpolynomial_free(pwqp1);
  isl_pw_qpolynomial_free(pwqp2);
  if (equal < 0)
...
static int test_union(isl_ctx *ctx) {
  ...
  if (equal < 0)
    return -1;
  if (!equal)
    isl_die(ctx, isl_error_unknown, "union maps not equal", return -1);
  ...
  if (equal < 0)
    return -1;
  if (!equal)
    isl_die(ctx, isl_error_unknown, "union sets not equal", return -1);
...
static int test_bound_unbounded_domain(isl_ctx *ctx) {
  ...
  if (equal < 0)
    return -1;
  if (!equal)
    isl_die(ctx, isl_error_unknown, "expecting infinite polynomial bound",
            return -1);
...
static int test_bound(isl_ctx *ctx) {
  ...
  if (dim < 0)
    return -1;
  if (dim != 4)
    isl_die(ctx, isl_error_unknown, "unexpected input dimension", return -1);
...
static int test_get_list(isl_ctx *ctx) {
  ...
  if (test_get_list_set_from_union_set(ctx, "{ A[0]; A[2]; B[3] }"))
    return -1;
  if (test_get_list_basic_map_from_map(
          ctx, "{ [0] -> [0]; [2] -> [0]; [3] -> [0] }"))
    return -1;
  if (test_get_list_map_from_union_map(
          ctx, "{ A[0] -> [0]; A[2] -> [0]; B[3] -> [0] }"))
    return -1;
...
struct {
  ...
  int subset;
    ...
    {"{ [65] }",
     ...
     "61e3 <= 52i + 12e2 and 61e3 >= -60 + 52i + 12e2) }",
     1},
    ...
    {"{ [t, i] : (exists (e0 = [(2 + t)/4]: 4e0 <= 2 + t and "
     ...
     "4e0 >= -61 + i0 + i1)) or "
     "(i1 <= 66 - i0 and i0 >= 2 and i1 >= 59 + i0) }",
     1},
...
static int test_subset(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(subset_tests); ++i) {
    ...
    if (subset != subset_tests[i].subset)
      isl_die(ctx, isl_error_unknown, "incorrect subset result", return -1);
  }
...
static int test_subtract(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(subtract_domain_tests); ++i) {
    umap1 = isl_union_map_read_from_str(ctx, subtract_domain_tests[i].minuend);
    uset =
        isl_union_set_read_from_str(ctx, subtract_domain_tests[i].subtrahend);
    umap2 =
        isl_union_map_read_from_str(ctx, subtract_domain_tests[i].difference);
    ...
    if (equal < 0)
      return -1;
    if (!equal)
      isl_die(ctx, isl_error_unknown, "incorrect subtract domain result",
              return -1);
  }

  for (i = 0; i < ARRAY_SIZE(subtract_domain_tests); ++i) {
    upma1 = isl_union_pw_multi_aff_read_from_str(
        ctx, subtract_domain_tests[i].minuend);
    uset =
        isl_union_set_read_from_str(ctx, subtract_domain_tests[i].subtrahend);
    upma2 = isl_union_pw_multi_aff_read_from_str(
        ctx, subtract_domain_tests[i].difference);
...
static int test_intersect_1(isl_ctx *ctx) {
  ...
  if (n1 != n2)
    isl_die(ctx, isl_error_unknown,
            "number of constraints of empty set changed", return -1);
...
int test_factorize(isl_ctx *ctx) {
  ...
  if (!f)
    isl_die(ctx, isl_error_unknown, "failed to construct factorizer",
            return -1);
...
int test_one_schedule(isl_ctx *ctx, const char *d, const char *w, const char *r,
                      const char *s, int tilable, int parallel) {
  int i;
  isl_union_set *D;
  isl_union_map *W, *R, *S;
  isl_union_map *empty;
  ...
  isl_union_map_compute_flow(isl_union_map_copy(R), isl_union_map_copy(W),
                             empty, isl_union_map_copy(S), &dep_raw, NULL, NULL,
                             NULL);
  isl_union_map_compute_flow(isl_union_map_copy(W), isl_union_map_copy(W),
                             isl_union_map_copy(R), isl_union_map_copy(S),
  ...
  if (is_nonneg < 0 || is_parallel < 0 || is_tilable < 0 || is_injection < 0 ||
      is_complete < 0)
    return -1;
  if (!is_complete)
  ...
  if (!is_injection)
    isl_die(ctx, isl_error_unknown,
            "generated schedule not injective on each statement", return -1);
  if (!is_nonneg)
    isl_die(ctx, isl_error_unknown,
            "negative dependences in generated schedule", return -1);
  if (!is_tilable)
    isl_die(ctx, isl_error_unknown,
            "generated schedule not as tilable as expected", return -1);
  if (!is_parallel)
    isl_die(ctx, isl_error_unknown,
            "generated schedule not as parallel as expected", return -1);
...
compute_schedule_with_context(isl_ctx *ctx, const char *domain,
...
int test_special_schedule(isl_ctx *ctx, const char *domain,
  ...
  if (equal < 0)
    return -1;
  if (!equal)
    isl_die(ctx, isl_error_unknown, "unexpected schedule", return -1);
...
static int test_special_conditional_schedule_constraints(isl_ctx *ctx) {
  ...
  if (empty < 0)
    return -1;
  if (empty)
    isl_die(ctx, isl_error_unknown, "conditional validity not respected",
            return -1);
...
static int test_special_conditional_schedule_constraints_2(isl_ctx *ctx) {
  ...
  sc = isl_schedule_constraints_set_conditional_validity(
      sc, condition, isl_union_map_copy(validity));
  schedule = isl_schedule_constraints_compute_schedule(sc);
  umap = isl_schedule_get_map(schedule);
  isl_schedule_free(schedule);
  validity = isl_union_map_apply_domain(validity, isl_union_map_copy(umap));
  ...
  if (!empty)
    isl_die(ctx, isl_error_unknown,
            "conditional validity constraints not satisfied", return -1);
...
static int test_conditional_schedule_constraints(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(live_range_tests); ++i) {
    domain = isl_union_set_read_from_str(ctx, live_range_tests[i].domain);
    flow = isl_union_map_read_from_str(ctx, live_range_tests[i].flow);
    condition = isl_union_map_read_from_str(ctx, live_range_tests[i].condition);
    validity = isl_union_map_read_from_str(
        ctx, live_range_tests[i].conditional_validity);
    sc = isl_schedule_constraints_on_domain(domain);
    sc = isl_schedule_constraints_set_validity(sc, isl_union_map_copy(flow));
    sc = isl_schedule_constraints_set_proximity(sc, flow);
    sc = isl_schedule_constraints_set_conditional_validity(sc, condition,
                                                           validity);
    schedule = isl_schedule_constraints_compute_schedule(sc);
    node = isl_schedule_get_root(schedule);
    while (node && isl_schedule_node_get_type(node) != isl_schedule_node_band)
    ...
    if (n_member != live_range_tests[i].outer_band_n)
      isl_die(ctx, isl_error_unknown,
              "unexpected number of members in outer band", return -1);
  }
  return 0;
}
...
static int test_strongly_satisfying_schedule(isl_ctx *ctx) {
  ...
  if (empty < 0)
    return -1;
  if (!empty)
    isl_die(ctx, isl_error_unknown, "dependences not strongly satisfied",
            return -1);
...
static int test_conflicting_context_schedule(isl_ctx *ctx) {
  ...
  domain = "[n] -> { A[] : n >= 0 }";
  context = "[n] -> { : n < 0 }";
  schedule = compute_schedule_with_context(ctx, domain, "{}", "{}", context);
...
int test_schedule(isl_ctx *ctx) {
  ...
  D = "[n] -> { S_0[j, k] : j <= -1 + n and j >= 0 and "
      "k <= -1 + n and k >= 0 }";
  W = "[n] -> { S_0[j, k] -> B[j] : j <= -1 + n and j >= 0 and "
      "k <= -1 + n and k >= 0 }";
...
int test_plain_injective(isl_ctx *ctx, const char *str, int injective) {
  ...
  if (test == injective)
    return 0;
  if (injective)
    isl_die(ctx, isl_error_unknown, "map not detected as injective", return -1);
  else
    isl_die(ctx, isl_error_unknown, "map detected as injective", return -1);
}
...
static struct {
  ...
  isl_bool res;
    ...
    {&isl_union_pw_multi_aff_involves_locals, "{ A[] -> [0]; B[0] -> [1] }",
     isl_bool_false},
    {&isl_union_pw_multi_aff_involves_locals,
     "{ A[] -> [0]; B[x] -> [1] : x mod 2 = 0 }", isl_bool_true},
    {&isl_union_pw_multi_aff_involves_locals,
     "{ A[] -> [0]; B[x] -> [x // 2] }", isl_bool_true},
    {&isl_union_pw_multi_aff_involves_locals,
     "{ A[i] -> [i // 2]; B[0] -> [1] }", isl_bool_true},
};
...
struct {
  __isl_give
      isl_union_pw_multi_aff *(*fn)(__isl_take isl_union_pw_multi_aff *upma1,
...
static int test_bin_upma(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(upma_bin_tests); ++i) {
    upma1 = isl_union_pw_multi_aff_read_from_str(ctx, upma_bin_tests[i].arg1);
    upma2 = isl_union_pw_multi_aff_read_from_str(ctx, upma_bin_tests[i].arg2);
    res = isl_union_pw_multi_aff_read_from_str(ctx, upma_bin_tests[i].res);
...
static int test_bin_upma_fail(isl_ctx *ctx) {
  ...
  for (i = 0; i < n; ++i) {
    upma1 =
        isl_union_pw_multi_aff_read_from_str(ctx, upma_bin_fail_tests[i].arg1);
    upma2 =
        isl_union_pw_multi_aff_read_from_str(ctx, upma_bin_fail_tests[i].arg2);
...
struct {
  __isl_give
      isl_union_pw_multi_aff *(*fn)(__isl_take isl_union_pw_multi_aff *upma,
                                    __isl_take isl_union_set *uset);
  const char *arg1;
  const char *arg2;
  const char *res;
} upma_uset_tests[] = {
    {&isl_union_pw_multi_aff_intersect_domain_wrapped_range, "{ A[i] -> B[i] }",
     "{ B[0] }", "{ }"},
...
static isl_stat test_upma_uset(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(upma_uset_tests); ++i) {
    upma = isl_union_pw_multi_aff_read_from_str(ctx, upma_uset_tests[i].arg1);
    uset = isl_union_set_read_from_str(ctx, upma_uset_tests[i].arg2);
    res = isl_union_pw_multi_aff_read_from_str(ctx, upma_uset_tests[i].res);
...
struct {
  ...
  const char *res;
    ...
    {&isl_multi_pw_aff_range_factor_range,
     ...
     "{ A[y] -> C[(2 : y <= 10)] }"},
    {&isl_multi_pw_aff_range_factor_domain,
     "{ A[x] -> [B[(1 : x >= 5)] -> C[]] }", "{ A[x] -> B[(1 : x >= 5)] }"},
    {&isl_multi_pw_aff_range_factor_range,
     "{ A[x] -> [B[(1 : x >= 5)] -> C[]] }", "{ A[y] -> C[] }"},
    {&isl_multi_pw_aff_range_factor_domain,
     "{ A[x] -> [B[] -> C[(2 : x <= 10)]] }", "{ A[x] -> B[] }"},
    {&isl_multi_pw_aff_range_factor_range,
     "{ A[x] -> [B[] -> C[(2 : x <= 10)]] }", "{ A[y] -> C[(2 : y <= 10)] }"},
    {&isl_multi_pw_aff_range_factor_domain, "{ A[x] -> [B[] -> C[]] }",
     "{ A[x] -> B[] }"},
    {&isl_multi_pw_aff_range_factor_range, "{ A[x] -> [B[] -> C[]] }",
     "{ A[y] -> C[] }"},
    {&isl_multi_pw_aff_factor_range, "{ [B[] -> C[]] }", "{ C[] }"},
    {&isl_multi_pw_aff_range_factor_domain, "{ A[x] -> [B[] -> C[]] : x >= 0 }",
     "{ A[x] -> B[] : x >= 0 }"},
    {&isl_multi_pw_aff_range_factor_range, "{ A[x] -> [B[] -> C[]] : x >= 0 }",
     "{ A[y] -> C[] : y >= 0 }"},
    {&isl_multi_pw_aff_factor_range, "[N] -> { [B[] -> C[]] : N >= 0 }",
     "[N] -> { C[] : N >= 0 }"},
};
 ...
 */
struct {
  __isl_give isl_multi_pw_aff *(*fn)(__isl_take isl_multi_pw_aff *mpa1,
  ...
  const char *res;
    ...
    {&isl_multi_pw_aff_add, "{ A[x] -> [(1 : x >= 5)] }",
     "{ A[x] -> [(x : x <= 10)] }", "{ A[x] -> [(1 + x : 5 <= x <= 10)] }"},
    {&isl_multi_pw_aff_add, "{ A[x] -> [] : x >= 5 }",
     "{ A[x] -> [] : x <= 10 }", "{ A[x] -> [] : 5 <= x <= 10 }"},
    {&isl_multi_pw_aff_add, "{ A[x] -> [] : x >= 5 }",
     "[N] -> { A[x] -> [] : x <= N }", "[N] -> { A[x] -> [] : 5 <= x <= N }"},
    {&isl_multi_pw_aff_add, "[N] -> { A[x] -> [] : x <= N }",
     "{ A[x] -> [] : x >= 5 }", "[N] -> { A[x] -> [] : 5 <= x <= N }"},
    ...
    {&isl_multi_pw_aff_product, "{ A[] -> B[] }", "{ A[] -> C[] }",
     "{ [A[] -> A[]] -> [B[] -> C[]] }"},
    {&isl_multi_pw_aff_pullback_multi_pw_aff, "{ B[i,j] -> C[i + 2j] }",
     "{ A[a,b] -> B[b,a] }", "{ A[a,b] -> C[b + 2a] }"},
    {&isl_multi_pw_aff_pullback_multi_pw_aff, "{ B[i,j] -> C[i + 2j] }",
     "{ A[a,b] -> B[(b : b > a),(a : b > a)] }",
     "{ A[a,b] -> C[(b + 2a : b > a)] }"},
    {&isl_multi_pw_aff_pullback_multi_pw_aff,
     ...
     "{ A[a,b] -> B[(b : b > a),(a : b > a)] }",
     "{ A[a,b] -> C[(b + 2a : b > a > 4)] }"},
    {&isl_multi_pw_aff_pullback_multi_pw_aff, "{ B[i,j] -> C[] }",
     "{ A[a,b] -> B[(b : b > a),(a : b > a)] }", "{ A[a,b] -> C[] }"},
    {&isl_multi_pw_aff_pullback_multi_pw_aff, "{ B[i,j] -> C[] : i > j }",
     "{ A[a,b] -> B[b,a] }", "{ A[a,b] -> C[] : b > a }"},
    {&isl_multi_pw_aff_pullback_multi_pw_aff, "{ B[i,j] -> C[] : j > 5 }",
     "{ A[a,b] -> B[(b : b > a),(a : b > a)] }",
     "{ A[a,b] -> C[] : b > a > 5 }"},
    {&isl_multi_pw_aff_pullback_multi_pw_aff,
     "[N] -> { B[i,j] -> C[] : j > N }",
     "{ A[a,b] -> B[(b : b > a),(a : b > a)] }",
     "[N] -> { A[a,b] -> C[] : b > a > N }"},
    {&isl_multi_pw_aff_pullback_multi_pw_aff, "[M,N] -> { B[] -> C[] : N > 5 }",
     "[M,N] -> { A[] -> B[] : M > N }", "[M,N] -> { A[] -> C[] : M > N > 5 }"},
};
...
static int test_bin_mpa(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(mpa_bin_tests); ++i) {
    mpa1 = isl_multi_pw_aff_read_from_str(ctx, mpa_bin_tests[i].arg1);
    mpa2 = isl_multi_pw_aff_read_from_str(ctx, mpa_bin_tests[i].arg2);
    res = isl_multi_pw_aff_read_from_str(ctx, mpa_bin_tests[i].res);
...
struct {
  __isl_give
      isl_multi_union_pw_aff *(*fn)(__isl_take isl_multi_union_pw_aff *mupa);
  const char *arg;
  const char *res;
} mupa_un_tests[] = {
    {&isl_multi_union_pw_aff_factor_range,
     "[B[{ A[] -> [1] }] -> C[{ A[] -> [2] }]]", "C[{ A[] -> [2] }]"},
    {&isl_multi_union_pw_aff_factor_range, "[B[] -> C[{ A[] -> [2] }]]",
     "C[{ A[] -> [2] }]"},
    {&isl_multi_union_pw_aff_factor_range, "[B[{ A[] -> [1] }] -> C[]]", "C[]"},
    {&isl_multi_union_pw_aff_factor_range, "[B[] -> C[]]", "C[]"},
    {&isl_multi_union_pw_aff_factor_range, "([B[] -> C[]] : { A[x] : x >= 0 })",
     "(C[] : { A[x] : x >= 0 })"},
    {&isl_multi_union_pw_aff_factor_range,
     "[N] -> ([B[] -> C[]] : { A[x] : x <= N })",
     "[N] -> (C[] : { A[x] : x <= N })"},
...
static int test_un_mupa(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(mupa_un_tests); ++i) {
    mupa = isl_multi_union_pw_aff_read_from_str(ctx, mupa_un_tests[i].arg);
    res = isl_multi_union_pw_aff_read_from_str(ctx, mupa_un_tests[i].res);
...
struct {
  __isl_give
      isl_multi_union_pw_aff *(*fn)(__isl_take isl_multi_union_pw_aff *mupa1,
  ...
  const char *res;
    ...
    {&isl_multi_union_pw_aff_sub, "[{ A[] -> [1] }]", "[{ A[] -> [2] }]",
     "[{ A[] -> [-1] }]"},
    {&isl_multi_union_pw_aff_add, "[{ A[] -> [1]; B[] -> [4] }]",
     "[{ A[] -> [2]; C[] -> [5] }]", "[{ A[] -> [3] }]"},
    {&isl_multi_union_pw_aff_union_add, "[{ A[] -> [1]; B[] -> [4] }]",
     "[{ A[] -> [2]; C[] -> [5] }]",
     "[{ A[] -> [3]; B[] -> [4]; C[] -> [5] }]"},
    ...
    {&isl_multi_union_pw_aff_union_add, "([] : { A[x] : x >= 0 })",
     "[N] -> ([] : { A[x] : x >= N })",
     "[N] -> ([] : { A[x] : x >= 0 or x >= N })"},
    {&isl_multi_union_pw_aff_union_add, "[N] -> ([] : { A[] : N >= 0})",
     "[N] -> ([] : { A[] : N <= 0})", "[N] -> ([] : { A[] })"},
    {&isl_multi_union_pw_aff_union_add, "[N] -> ([] : { A[] })",
     "[N] -> ([] : { : })", "[N] -> ([] : { : })"},
    {&isl_multi_union_pw_aff_union_add, "[N] -> ([] : { : })",
     "[N] -> ([] : { A[] })", "[N] -> ([] : { : })"},
    {&isl_multi_union_pw_aff_union_add, "[N] -> ([] : { : N >= 0})",
     "[N] -> ([] : { : N <= 0})", "[N] -> ([] : { : })"},
    {&isl_multi_union_pw_aff_range_product, "B[{ A[] -> [1] }]",
     "C[{ A[] -> [2] }]", "[B[{ A[] -> [1] }] -> C[{ A[] -> [2] }]]"},
    {&isl_multi_union_pw_aff_range_product, "(B[] : { A[x] : x >= 5 })",
     "(C[] : { A[x] : x <= 10 })", "([B[] -> C[]] : { A[x] : 5 <= x <= 10 })"},
    {&isl_multi_union_pw_aff_range_product, "B[{ A[x] -> [x + 1] : x >= 5 }]",
     "(C[] : { A[x] : x <= 10 })",
     "[B[{ A[x] -> [x + 1] : 5 <= x <= 10 }] -> C[]]"},
    {&isl_multi_union_pw_aff_range_product, "(C[] : { A[x] : x <= 10 })",
     "B[{ A[x] -> [x + 1] : x >= 5 }]",
     "[C[] -> B[{ A[x] -> [x + 1] : 5 <= x <= 10 }]]"},
    {&isl_multi_union_pw_aff_range_product, "B[{ A[x] -> [x + 1] : x >= 5 }]",
     "[N] -> (C[] : { A[x] : x <= N })",
     "[N] -> [B[{ A[x] -> [x + 1] : 5 <= x <= N }] -> C[]]"},
    {&isl_multi_union_pw_aff_range_product, "[N] -> (C[] : { A[x] : x <= N })",
     "B[{ A[x] -> [x + 1] : x >= 5 }]",
     "[N] -> [C[] -> B[{ A[x] -> [x + 1] : 5 <= x <= N }]]"},
    {&isl_multi_union_pw_aff_range_product, "B[{ A[] -> [1]; D[] -> [3] }]",
     "C[{ A[] -> [2] }]",
     "[B[{ A[] -> [1]; D[] -> [3] }] -> C[{ A[] -> [2] }]]"},
    {&isl_multi_union_pw_aff_range_product, "B[] }]", "(C[] : { A[x] })",
     "([B[] -> C[]] : { A[x] })"},
    {&isl_multi_union_pw_aff_range_product, "(B[] : { A[x] })", "C[] }]",
     "([B[] -> C[]] : { A[x] })"},
};
...
static int test_bin_mupa(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(mupa_bin_tests); ++i) {
    mupa1 = isl_multi_union_pw_aff_read_from_str(ctx, mupa_bin_tests[i].arg1);
    mupa2 = isl_multi_union_pw_aff_read_from_str(ctx, mupa_bin_tests[i].arg2);
    res = isl_multi_union_pw_aff_read_from_str(ctx, mupa_bin_tests[i].res);
...
struct {
  ...
  const char *res;
    ...
    {&isl_multi_union_pw_aff_intersect_range, "C[{ B[i,j] -> [i + 2j] }]",
     "{ C[1] }", "C[{ B[i,j] -> [i + 2j] : i + 2j = 1 }]"},
    {&isl_multi_union_pw_aff_intersect_range, "C[{ B[i,j] -> [i + 2j] }]",
     "[N] -> { C[N] }", "[N] -> C[{ B[i,j] -> [i + 2j] : i + 2j = N }]"},
    {&isl_multi_union_pw_aff_intersect_range,
     "[N] -> C[{ B[i,j] -> [i + 2j + N] }]", "{ C[1] }",
     "[N] -> C[{ B[i,j] -> [i + 2j + N] : i + 2j + N = 1 }]"},
    {&isl_multi_union_pw_aff_intersect_range, "C[{ B[i,j] -> [i + 2j] }]",
     "[N] -> { C[x] : N >= 0 }", "[N] -> C[{ B[i,j] -> [i + 2j] : N >= 0 }]"},
    {&isl_multi_union_pw_aff_intersect_range, "C[]", "{ C[] }", "C[]"},
    {&isl_multi_union_pw_aff_intersect_range, "[N] -> (C[] : { : N >= 0 })",
     "{ C[] }", "[N] -> (C[] : { : N >= 0 })"},
    {&isl_multi_union_pw_aff_intersect_range, "(C[] : { A[a,b] })", "{ C[] }",
     "(C[] : { A[a,b] })"},
    {&isl_multi_union_pw_aff_intersect_range,
     "[N] -> (C[] : { A[a,b] : a,b <= N })", "{ C[] }",
     "[N] -> (C[] : { A[a,b] : a,b <= N })"},
    {&isl_multi_union_pw_aff_intersect_range, "C[]", "[N] -> { C[] : N >= 0 }",
     "[N] -> (C[] : { : N >= 0 })"},
    {&isl_multi_union_pw_aff_intersect_range, "(C[] : { A[a,b] })",
     "[N] -> { C[] : N >= 0 }", "[N] -> (C[] : { A[a,b] : N >= 0 })"},
    {&isl_multi_union_pw_aff_intersect_range, "[N] -> (C[] : { : N >= 0 })",
     "[N] -> { C[] : N < 1024 }", "[N] -> (C[] : { : 0 <= N < 1024 })"},
    {&isl_multi_union_pw_aff_intersect_params, "C[{ B[i,j] -> [i + 2j] }]",
     "[N] -> { : N >= 0 }", "[N] -> C[{ B[i,j] -> [i + 2j] : N >= 0}]"},
    ...
    {&isl_multi_union_pw_aff_intersect_params,
     ...
     "[N] -> C[{ B[i,j] -> [i + 2j] : N <= 256 }]"},
    {&isl_multi_union_pw_aff_intersect_params, "C[]", "[N] -> { : N >= 0 }",
     "[N] -> (C[] : { : N >= 0 })"},
    {&isl_multi_union_pw_aff_intersect_params, "(C[] : { A[a,b] })",
     "[N] -> { : N >= 0 }", "[N] -> (C[] : { A[a,b] : N >= 0 })"},
    {&isl_multi_union_pw_aff_intersect_params, "[N] -> (C[] : { A[a,N] })",
     "{ : }", "[N] -> (C[] : { A[a,N] })"},
    {&isl_multi_union_pw_aff_intersect_params,
...
static int test_mupa_set(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(mupa_set_tests); ++i) {
    mupa = isl_multi_union_pw_aff_read_from_str(ctx, mupa_set_tests[i].arg1);
    set = isl_set_read_from_str(ctx, mupa_set_tests[i].arg2);
    res = isl_multi_union_pw_aff_read_from_str(ctx, mupa_set_tests[i].res);
...
struct {
  ...
  const char *res;
    ...
    {&isl_multi_union_pw_aff_intersect_domain, "C[{ B[i,j] -> [i + 2j] }]",
     "{ B[i,i] }", "C[{ B[i,i] -> [3i] }]"},
    {&isl_multi_union_pw_aff_intersect_domain, "(C[] : { B[i,j] })",
     "{ B[i,i] }", "(C[] : { B[i,i] })"},
    {&isl_multi_union_pw_aff_intersect_domain, "(C[] : { B[i,j] })",
     "[N] -> { B[N,N] }", "[N] -> (C[] : { B[N,N] })"},
    {&isl_multi_union_pw_aff_intersect_domain, "C[]", "{ B[i,i] }",
     "(C[] : { B[i,i] })"},
    {&isl_multi_union_pw_aff_intersect_domain, "[N] -> (C[] : { : N >= 0 })",
     "{ B[i,i] }", "[N] -> (C[] : { B[i,i] : N >= 0 })"},
};
...
static int test_mupa_uset(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(mupa_uset_tests); ++i) {
    mupa = isl_multi_union_pw_aff_read_from_str(ctx, mupa_uset_tests[i].arg1);
    uset = isl_union_set_read_from_str(ctx, mupa_uset_tests[i].arg2);
    res = isl_multi_union_pw_aff_read_from_str(ctx, mupa_uset_tests[i].res);
...
struct {
  ...
  const char *res;
    ...
    {&isl_multi_union_pw_aff_apply_multi_aff,
     ...
     "{ A[i,j] -> [i] : i >= 0; B[i,j] -> [j] }]"},
    {&isl_multi_union_pw_aff_apply_multi_aff,
     "C[{ A[i,j] -> [i]; B[i,j] -> [j] }]", "[N] -> { C[a] -> D[a + N] }",
     "[N] -> D[{ A[i,j] -> [i + N]; B[i,j] -> [j + N] }] "},
    {&isl_multi_union_pw_aff_apply_multi_aff, "C[]", "{ C[] -> D[] }", "D[]"},
    {&isl_multi_union_pw_aff_apply_multi_aff, "[N] -> (C[] : { : N >= 0 })",
     "{ C[] -> D[] }", "[N] -> (D[] : { : N >= 0 })"},
    {&isl_multi_union_pw_aff_apply_multi_aff, "C[]", "[N] -> { C[] -> D[N] }",
     "[N] -> D[{ [N] }]"},
    {&isl_multi_union_pw_aff_apply_multi_aff, "(C[] : { A[i,j] : i >= j })",
     "{ C[] -> D[] }", "(D[] : { A[i,j] : i >= j })"},
    {&isl_multi_union_pw_aff_apply_multi_aff,
     "[N] -> (C[] : { A[i,j] : N >= 0 })", "{ C[] -> D[] }",
     "[N] -> (D[] : { A[i,j] : N >= 0 })"},
    {&isl_multi_union_pw_aff_apply_multi_aff, "(C[] : { A[i,j] : i >= j })",
     "[N] -> { C[] -> D[N] }", "[N] -> (D[{ A[i,j] -> [N] : i >= j }])"},
};
...
static int test_mupa_ma(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(mupa_ma_tests); ++i) {
    mupa = isl_multi_union_pw_aff_read_from_str(ctx, mupa_ma_tests[i].arg1);
    ma = isl_multi_aff_read_from_str(ctx, mupa_ma_tests[i].arg2);
    res = isl_multi_union_pw_aff_read_from_str(ctx, mupa_ma_tests[i].res);
 ...
 */
struct {
  __isl_give isl_union_pw_aff *(*fn)(__isl_take isl_multi_union_pw_aff *mupa,
  ...
  const char *res;
    ...
    {&isl_multi_union_pw_aff_apply_pw_aff, "C[]", "[N] -> { C[] -> [N] }",
     "[N] -> { [N] }"},
    {&isl_multi_union_pw_aff_apply_pw_aff, "C[]",
     "[N] -> { C[] -> [N] : N >= 0; C[] -> [-N] : N < 0 }",
     "[N] -> { [N] : N >= 0; [-N] : N < 0 }"},
    {&isl_multi_union_pw_aff_apply_pw_aff, "[N] -> (C[] : { : N >= 0 })",
     "[N] -> { C[] -> [N] }", "[N] -> { [N] : N >= 0 }"},
    {&isl_multi_union_pw_aff_apply_pw_aff, "[N] -> (C[] : { : N >= 0 })",
     "[N] -> { C[] -> [N] : N >= 0; C[] -> [-N] : N < 0 }",
     "[N] -> { [N] : N >= 0 }"},
    {&isl_multi_union_pw_aff_apply_pw_aff, "[N] -> (C[] : { : N >= 0 })",
     "{ C[] -> [0] }", "[N] -> { [0] : N >= 0 }"},
    {&isl_multi_union_pw_aff_apply_pw_aff, "(C[] : { A[i,j] : i >= j })",
     "[N] -> { C[] -> [N] }", "[N] -> { A[i,j] -> [N] : i >= j }"},
    {&isl_multi_union_pw_aff_apply_pw_aff, "(C[] : { A[i,j] : i >= j })",
     "[N] -> { C[] -> [N] : N >= 0 }",
...
static int test_mupa_pa(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(mupa_pa_tests); ++i) {
    mupa = isl_multi_union_pw_aff_read_from_str(ctx, mupa_pa_tests[i].arg1);
    pa = isl_pw_aff_read_from_str(ctx, mupa_pa_tests[i].arg2);
    res = isl_union_pw_aff_read_from_str(ctx, mupa_pa_tests[i].res);
...
struct {
  ...
  const char *res;
    ...
    {&isl_multi_union_pw_aff_apply_pw_multi_aff, "C[]", "{ C[] -> D[] }",
     "D[]"},
    {&isl_multi_union_pw_aff_apply_pw_multi_aff, "[N] -> (C[] : { : N >= 0 })",
     "{ C[] -> D[] }", "[N] -> (D[] : { : N >= 0 })"},
    {&isl_multi_union_pw_aff_apply_pw_multi_aff, "C[]",
     "[N] -> { C[] -> D[N] }", "[N] -> D[{ [N] }]"},
    {&isl_multi_union_pw_aff_apply_pw_multi_aff, "(C[] : { A[i,j] : i >= j })",
     "{ C[] -> D[] }", "(D[] : { A[i,j] : i >= j })"},
    {&isl_multi_union_pw_aff_apply_pw_multi_aff,
     "[N] -> (C[] : { A[i,j] : N >= 0 })", "{ C[] -> D[] }",
     "[N] -> (D[] : { A[i,j] : N >= 0 })"},
...
static int test_mupa_pma(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(mupa_pma_tests); ++i) {
    mupa = isl_multi_union_pw_aff_read_from_str(ctx, mupa_pma_tests[i].arg1);
    pma = isl_pw_multi_aff_read_from_str(ctx, mupa_pma_tests[i].arg2);
    res = isl_multi_union_pw_aff_read_from_str(ctx, mupa_pma_tests[i].res);
...
struct {
  ...
  const char *res;
    ...
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff,
     ...
     "C[{ A[a,b] -> [b + 2a] }]"},
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff,
     "C[{ B[i,j] -> [i + 2j] }]", "{ A[a,b] -> B[b,a] : b > a }",
     "C[{ A[a,b] -> [b + 2a] : b > a }]"},
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff,
     "C[{ B[i,j] -> [i + 2j] : j > 4 }]", "{ A[a,b] -> B[b,a] : b > a }",
     "C[{ A[a,b] -> [b + 2a] : b > a > 4 }]"},
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff,
     "C[{ B[i,j] -> [i + 2j] }]",
     "{ A[a,b] -> B[b,a] : a > b; A[a,b] -> B[a,b] : a <= b }",
     "C[{ A[a,b] -> [b + 2a] : a > b; A[a,b] -> [a + 2b] : a <= b }]"},
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff, "(C[] : { B[a,b] })",
     "{ A[a,b] -> B[b,a] }", "(C[] : { A[a,b] })"},
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff, "(C[] : { B[a,b] })",
     "{ B[a,b] -> A[b,a] }", "(C[] : { })"},
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff, "(C[] : { B[a,b] })",
     "{ A[a,b] -> B[b,a] : a > b }", "(C[] : { A[a,b] : a > b })"},
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff,
     "(C[] : { B[a,b] : a > b })", "{ A[a,b] -> B[b,a] }",
     "(C[] : { A[a,b] : b > a })"},
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff,
     "[N] -> (C[] : { B[a,b] : a > N })", "{ A[a,b] -> B[b,a] : a > b }",
     "[N] -> (C[] : { A[a,b] : a > b > N })"},
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff,
     "(C[] : { B[a,b] : a > b })", "[N] -> { A[a,b] -> B[b,a] : a > N }",
     "[N] -> (C[] : { A[a,b] : b > a > N })"},
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff, "C[]",
     "{ A[a,b] -> B[b,a] }", "C[]"},
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff,
     "[N] -> (C[] : { : N >= 0 })", "{ A[a,b] -> B[b,a] }",
     "[N] -> (C[] : { : N >= 0 })"},
    {&isl_multi_union_pw_aff_pullback_union_pw_multi_aff, "C[]",
     "[N] -> { A[a,b] -> B[b,a] : N >= 0 }", "[N] -> (C[] : { : N >= 0 })"},
};
...
static int test_mupa_upma(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(mupa_upma_tests); ++i) {
    mupa = isl_multi_union_pw_aff_read_from_str(ctx, mupa_upma_tests[i].arg1);
    upma = isl_union_pw_multi_aff_read_from_str(ctx, mupa_upma_tests[i].arg2);
    res = isl_multi_union_pw_aff_read_from_str(ctx, mupa_upma_tests[i].res);
...
static struct {
  ...
  const char *res;
    ...
    {"{ A[M, N] : M mod 2 = 0 and N mod 8 = 3 }", "{ A[M, N] }",
     "[M, N] -> { : M mod 2 = 0 and N mod 8 = 3 }"},
    {"{ B[N, M] : M mod 2 = 0 and N mod 8 = 3 }", "{ B[N, M] }",
     "[M, N] -> { : M mod 2 = 0 and N mod 8 = 3 }"},
    {"[M] -> { C[N] : M mod 2 = 0 and N mod 8 = 3 }", "{ C[N] }",
     "[M, N] -> { : M mod 2 = 0 and N mod 8 = 3 }"},
    {"[M] -> { D[x, N] : x mod 2 = 0 and N mod 8 = 3 and M >= 0 }",
     "{ D[M, N] }", "[M, N] -> { : M mod 2 = 0 and N mod 8 = 3 and M >= 0 }"},
};
...
struct {
  ...
  const char *res;
    ...
    {"{ A[M, N] -> [M + floor(N/2)] }", "{ A[M, N] }",
     "[M, N] -> { [M + floor(N/2)] }"},
    {"{ B[N, M] -> [M + floor(N/2)] }", "{ B[N, M] }",
     "[N, M] -> { [M + floor(N/2)] }"},
    {"[M] -> { C[N] -> [M + floor(N/2)] }", "{ C[N] }",
     "[M, N] -> { [M + floor(N/2)] }"},
    {"[M] -> { C[x, N] -> [x + floor(N/2)] }", "{ C[M, N] }",
     "[M, N] -> { [M + floor(N/2)] }"},
    {"[M] -> { C[x, N] -> [M + floor(N/2)] }", "{ C[M, N] }",
     "[M, N] -> { [M + floor(N/2)] }"},
    {"[A, M] -> { C[N, x] -> [x + floor(N/2)] }", "{ C[N, M] }",
     "[A, N, M] -> { [M + floor(N/2)] }"},
};
...
struct {
  ...
  const char *res;
    ...
    {"{ B[N, M] -> A[M, N] : M mod 2 = 0 and N mod 8 = 3 }", "{ A[M, N] }",
     "[M, N] -> { B[N, M] : M mod 2 = 0 and N mod 8 = 3 }"},
    {"{ B[N, M] -> A[M, N] : M mod 2 = 0 and N mod 8 = 3 }", "{ B[M, N] }",
     "{ }"},
    {"{ A[] -> B[]; C[] -> D[]; E[] -> B[] }", "{ B[] }", "{ A[]; E[] }"},
};
...
struct {
  ...
  const char *res;
    ...
    {"{ [A[M, N] -> B[]] -> [M + floor(N/2)] }", "{ A[M, N] }",
     "[M, N] -> { B[] -> [M + floor(N/2)] }"},
    {"{ [B[N, M] -> D[]] -> [M + floor(N/2)] }", "{ B[N, M] }",
     "[N, M] -> { D[] -> [M + floor(N/2)] }"},
    {"[M] -> { [C[N] -> B[x]] -> [x + M + floor(N/2)] }", "{ C[N] }",
     "[M, N] -> { B[x] -> [x + M + floor(N/2)] }"},
    {"[M] -> { [C[x, N] -> B[]] -> [x + floor(N/2)] }", "{ C[M, N] }",
     "[M, N] -> { B[] -> [M + floor(N/2)] }"},
    {"[M] -> { [C[x, N] -> B[]] -> [M + floor(N/2)] }", "{ C[M, N] }",
     "[M, N] -> { B[] -> [M + floor(N/2)] }"},
    {"[A, M] -> { [C[N, x] -> B[]] -> [x + floor(N/2)] }", "{ C[N, M] }",
     "[A, N, M] -> { B[] -> [M + floor(N/2)] }"},
};
...
struct {
  ...
  const char *res;
    ...
    {"[M, N] -> { : M mod 2 = 0 and N mod 8 = 3 }", "{ A[M, N] }",
     "{ A[M, N] : M mod 2 = 0 and N mod 8 = 3 }"},
    {"[M, N] -> { : M mod 2 = 0 and N mod 8 = 3 }", "{ B[N, M] }",
     "{ B[N, M] : M mod 2 = 0 and N mod 8 = 3 }"},
    {"[M, N] -> { : M mod 2 = 0 and N mod 8 = 3 }", "{ C[N] }",
     "[M] -> { C[N] : M mod 2 = 0 and N mod 8 = 3 }"},
    {"[M, N] -> { : M mod 2 = 0 and N mod 8 = 3 }", "{ D[T, N] }",
...
struct {
  ...
  const char *res;
    ...
    {"[M, N] -> { [M + floor(N/2)] }", "{ A[M, N] }",
     "{ A[M, N] -> [M + floor(N/2)] }"},
    {"[M, N] -> { [M + floor(N/2)] }", "{ B[N, M] }",
     "{ B[N, M] -> [M + floor(N/2)] }"},
    {"[M, N] -> { [M + floor(N/2)] }", "{ C[N] }",
     "[M] -> { C[N] -> [M + floor(N/2)] }"},
    {"[M, N] -> { [M + floor(N/2)] }", "{ D[A, B, C, N, Z] }",
...
static struct {
  ...
  const char *res;
    ...
    {"[M, N] -> { [M + floor(N/2)] }", "{ D[A, B, C, N, Z] }",
     "[M] -> { D[A, B, C, N, Z] -> [M + floor(N/2)] }"},
    {"[M, N] -> { T[M + floor(N/2), N + floor(M/3)] }", "{ A[M, N] }",
...
static int check_single_piece(isl_ctx *ctx, __isl_take isl_pw_aff *pa) {
  ...
  if (n < 0)
    return -1;
  if (n != 1)
    isl_die(ctx, isl_error_unknown, "expecting single expression", return -1);
...
int test_equal(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(pma_equal_tests); ++i) {
    ...
    if (equal < 0)
      return -1;
    if (equal != expected)
      isl_die(ctx, isl_error_unknown, "unexpected equality result", return -1);
  }
...
static int test_plain_fixed(isl_ctx *ctx, __isl_take isl_map *map,
  ...
  if (test == fixed)
    return 0;
  if (fixed)
    isl_die(ctx, isl_error_unknown, "map not detected as fixed", return -1);
  else
    isl_die(ctx, isl_error_unknown, "map detected as fixed", return -1);
}
...
int test_vertices(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(vertices_tests); ++i) {
    ...
    if (n < 0)
      return -1;
    if (!ok)
      isl_die(ctx, isl_error_unknown, "unexpected vertices", return -1);
  }
...
static struct {
  ...
  const char *res;
    ...
    {&isl_union_map_range_reverse,
     ...
     "{ A[] -> [C[] -> B[]]; A[0] -> N[B[2] -> B[1]] }"},
    {&isl_union_map_range_reverse, "{ A[] -> N[B[] -> C[]] }",
     "{ A[] -> [C[] -> B[]] }"},
    {&isl_union_map_range_reverse, "{ A[] -> N[B[x] -> B[y]] }",
     "{ A[] -> N[B[*] -> B[*]] }"},
};
...
static struct {
  ...
  const char *res;
    ...
    {&isl_union_map_intersect, "[n] -> { A[i] -> [] : 0 <= i <= n; B[] -> [] }",
     "[m] -> { A[i] -> [] : 0 <= i <= m; C[] -> [] }",
     "[m, n] -> { A[i] -> [] : 0 <= i <= n and i <= m }"},
    {&isl_union_map_intersect_domain_factor_range,
     "{ [A[i] -> B[i + 1]] -> C[i + 2] }", "[N] -> { B[i] -> C[N] }",
     "[N] -> { [A[N - 2] -> B[N - 1]] -> C[N] }"},
    {&isl_union_map_intersect_domain_factor_range,
     "{ T[A[i] -> B[i + 1]] -> C[i + 2] }", "[N] -> { B[i] -> C[N] }",
     "[N] -> { T[A[N - 2] -> B[N - 1]] -> C[N] }"},
    {&isl_union_map_intersect_domain_factor_range,
     "{ [A[i] -> B[i + 1]] -> C[i + 2] }", "[N] -> { A[i] -> C[N] }", "{ }"},
    {&isl_union_map_intersect_range_factor_domain,
     "{ A[i] -> [B[i + 1] -> C[i + 2]] }", "[N] -> { A[i] -> B[N] }",
     "[N] -> { A[N - 1] -> [B[N] -> C[N + 1]] }"},
    {&isl_union_map_intersect_range_factor_domain,
     "{ A[i] -> T[B[i + 1] -> C[i + 2]] }", "[N] -> { A[i] -> B[N] }",
     "[N] -> { A[N - 1] -> T[B[N] -> C[N + 1]] }"},
    {&isl_union_map_intersect_range_factor_domain,
     "{ A[i] -> [B[i + 1] -> C[i + 2]] }", "[N] -> { A[i] -> C[N] }", "{ }"},
...
struct {
  ...
  const char *res;
    ...
    {&isl_multi_union_pw_aff_zero_union_set,
     "F[{ A[i,j] -> [i]; B[i,j] -> [i] }]", "{ A[0,j]; B[0,j] }"},
    {&isl_multi_union_pw_aff_zero_union_set,
     "F[{ A[i,j] -> [i-j]; B[i,j] -> [i-j] : i >= 0 }]",
     "{ A[i,i]; B[i,i] : i >= 0 }"},
    {&isl_multi_union_pw_aff_zero_union_set,
     "(F[] : { A[i,j]; B[i,i] : i >= 0 })", "{ A[i,j]; B[i,i] : i >= 0 }"},
};
...
static int test_un_locus(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(un_locus_tests); ++i) {
    mupa = isl_multi_union_pw_aff_read_from_str(ctx, un_locus_tests[i].arg);
...
struct {
  ...
  const char *res;
    ...
    {&isl_union_map_eq_at_multi_union_pw_aff, "{ A[i,j] -> B[i',j'] }",
     "F[{ A[i,j] -> [i]; B[i,j] -> [i] }]", "{ A[i,j] -> B[i,j'] }"},
    {&isl_union_map_eq_at_multi_union_pw_aff, "{ A[i,j] -> B[i',j'] }",
     "F[{ A[i,j] -> [i]; B[i,j] -> [i] }, "
     "{ A[i,j] -> [j]; B[i,j] -> [j] }]",
     "{ A[i,j] -> B[i,j] }"},
    ...
    {&isl_union_map_eq_at_multi_union_pw_aff, "{ A[i,j] -> B[i',j'] }",
     ...
     "{ A[i,j] -> B[i,j'] : i > j }"},
    {&isl_union_map_lex_lt_at_multi_union_pw_aff, "{ A[i,j] -> B[i',j'] }",
     "F[{ A[i,j] -> [i]; B[i,j] -> [i] }, "
     "{ A[i,j] -> [j]; B[i,j] -> [j] }]",
     "{ A[i,j] -> B[i',j'] : i,j << i',j' }"},
    {&isl_union_map_lex_gt_at_multi_union_pw_aff, "{ A[i,j] -> B[i',j'] }",
     "F[{ A[i,j] -> [i]; B[i,j] -> [i] }, "
     "{ A[i,j] -> [j]; B[i,j] -> [j] }]",
     "{ A[i,j] -> B[i',j'] : i,j >> i',j' }"},
    {&isl_union_map_eq_at_multi_union_pw_aff,
     "{ A[i,j] -> B[i',j']; A[i,j] -> C[i',j'] }", "(F[] : { A[i,j]; B[i,j] })",
     "{ A[i,j] -> B[i',j'] }"},
    {&isl_union_map_eq_at_multi_union_pw_aff, "{ A[i,j] -> B[i',j'] }",
...
static int test_bin_locus(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(bin_locus_tests); ++i) {
    umap = isl_union_map_read_from_str(ctx, bin_locus_tests[i].arg1);
    mupa = isl_multi_union_pw_aff_read_from_str(ctx, bin_locus_tests[i].arg2);
...
static struct {
  ...
  const char *res;
    ...
    {&isl_multi_aff_lex_le_set, "{ A[i] -> [i] }", "{ A[i] -> [0] }",
     "{ A[i] : i <= 0 }"},
    {&isl_multi_aff_lex_lt_set, "{ A[i] -> [i] }", "{ A[i] -> [0] }",
     "{ A[i] : i < 0 }"},
    {&isl_multi_aff_lex_le_set, "{ A[i] -> [i, i] }", "{ A[i] -> [0, 0] }",
     "{ A[i] : i <= 0 }"},
    {&isl_multi_aff_lex_le_set, "{ A[i] -> [i, 0] }", "{ A[i] -> [0, 0] }",
     "{ A[i] : i <= 0 }"},
    {&isl_multi_aff_lex_le_set, "{ A[i] -> [i, 1] }", "{ A[i] -> [0, 0] }",
     "{ A[i] : i < 0 }"},
};
...
static int test_output_set(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(output_tests); ++i) {
    ...
    if (equal < 0)
      return -1;
    if (!equal)
      isl_die(ctx, isl_error_unknown, "parsed output not the same", return -1);
  }
...
static isl_stat test_output_ma(isl_ctx *ctx) {
  ...
  if (equal < 0)
    return isl_stat_error;
  if (!equal)
    isl_die(ctx, isl_error_unknown, "bad conversion", return isl_stat_error);
...
static int test_residue_class(isl_ctx *ctx) {
  ...
  if (res >= 0 && (isl_int_cmp_si(m, 2) != 0 || isl_int_cmp_si(r, 1) != 0))
...
static int test_align_parameters_1(isl_ctx *ctx) {
  ...
  if (equal < 0)
    return -1;
  if (!equal)
    isl_die(ctx, isl_error_unknown, "result not as expected", return -1);
...
static int test_list(isl_ctx *ctx) {
  ...
  if (n != 3) {
    ...
    isl_die(ctx, isl_error_unknown, "unexpected number of elements in list",
            return -1);
  }
  ...
  if (!ok)
    isl_die(ctx, isl_error_unknown, "unexpected elements in list", return -1);
...
static int test_curry(isl_ctx *ctx) {
  ...
  if (equal)
    isl_die(ctx, isl_error_unknown,
            "curried map should not be equal to original", return -1);
...
static int test_preimage_basic_set(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(preimage_tests); ++i) {
    ...
    if (equal < 0)
      return -1;
    if (!equal)
      isl_die(ctx, isl_error_unknown, "bad preimage", return -1);
  }
...
struct {
  ...
  const char *res;
    ...
    {"{ B[i] -> C[i]; D[i] -> E[i] }", "{ A[i] -> B[i + 1] }",
     "{ A[i] -> C[i + 1] }"},
    {"{ B[i] -> C[i]; B[i] -> E[i] }", "{ A[i] -> B[i + 1] }",
     "{ A[i] -> C[i + 1]; A[i] -> E[i + 1] }"},
    {"{ B[i] -> C[([i/2])] }", "{ A[i] -> B[2i] }", "{ A[i] -> C[i] }"},
    {"{ B[i,j] -> C[([i/2]), ([(i+j)/3])] }", "{ A[i] -> B[([i/5]), ([i/7])] }",
     "{ A[i] -> C[([([i/5])/2]), ([(([i/5])+([i/7]))/3])] }"},
    {"[N] -> { B[i] -> C[([N/2]), i, ([N/3])] }",
     "[N] -> { A[] -> B[([N/5])] }",
     "[N] -> { A[] -> C[([N/2]), ([N/5]), ([N/3])] }"},
    {"{ B[i] -> C[i] : exists a : i = 5 a }", "{ A[i] -> B[2i] }",
...
static int test_preimage_union_map(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(preimage_domain_tests); ++i) {
    umap1 = isl_union_map_read_from_str(ctx, preimage_domain_tests[i].map);
    ma = isl_multi_aff_read_from_str(ctx, preimage_domain_tests[i].ma);
    umap2 = isl_union_map_read_from_str(ctx, preimage_domain_tests[i].res);
...
struct {
  ...
  const char *res;
    ...
    {"{ B[i] -> C[(i)/2] }", "{ A[a] -> B[(a)/3] }", "{ A[a] -> C[(a)/6] }"},
    {"{ B[i] -> C[2i] }", "{ A[a] -> B[5a] }", "{ A[a] -> C[10a] }"},
    {"{ B[i] -> C[2i] }", "{ A[a] -> B[(a)/3] }", "{ A[a] -> C[(2a)/3] }"},
...
static int test_pullback(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(pullback_tests); ++i) {
    ...
    if (equal < 0)
      return -1;
    if (!equal)
      isl_die(ctx, isl_error_unknown, "bad pullback", return -1);
  }
...
static __isl_give isl_id *before_for(__isl_keep isl_ast_build *build,
  ...
  if (data->before >= 3)
    isl_die(ctx, isl_error_unknown, "unexpected number of for nodes",
            return NULL);
  if (data->depth < 0 || data->depth >= 2)
    isl_die(ctx, isl_error_unknown, "unexpected depth", return NULL);
  ...
  if (n != 1) {
    ...
    if (n < 0)
      return NULL;
    isl_die(ctx, isl_error_unknown, "expecting single range space",
            return NULL);
  }
  ...
  if (empty < 0)
    return NULL;
  if (empty)
    isl_die(ctx, isl_error_unknown, "spaces don't match", return NULL);
...
static __isl_give isl_ast_node *after_for(__isl_take isl_ast_node *node,
  ...
  if (data->after > data->before)
    isl_die(isl_ast_node_get_ctx(node), isl_error_unknown,
            "mismatch in number of for nodes", return isl_ast_node_free(node));

  id = isl_ast_node_get_annotation(node);
  if (!id)
    isl_die(isl_ast_node_get_ctx(node), isl_error_unknown, "missing annotation",
  ...
  if (!valid)
    isl_die(isl_ast_node_get_ctx(node), isl_error_unknown, "wrong annotation",
            return isl_ast_node_free(node));
...
static int test_ast_gen1(isl_ctx *ctx) {
  ...
  build = isl_ast_build_set_after_each_for(build, &after_for, &data);
...
static int test_ast_gen3(isl_ctx *ctx) {
  ...
  build = isl_ast_build_from_context(set);
  build = isl_ast_build_set_options(build, options);
  build = isl_ast_build_set_at_each_domain(build, &count_domains, &n_domain);
  tree = isl_ast_build_node_from_schedule_map(build, schedule);
  isl_ast_build_free(build);
  if (!tree)
    return -1;
...
static int test_ast_gen4(isl_ctx *ctx) {
  ...
  if (type == isl_ast_node_if)
    isl_die(ctx, isl_error_unknown, "not expecting if node", return -1);
  ...
  if (type != isl_ast_node_if)
    isl_die(ctx, isl_error_unknown, "expecting if node", return -1);
...
static int test_ast_expr(isl_ctx *ctx) {
  ...
  if (!expr)
    return -1;
  if (is_min)
    isl_die(ctx, isl_error_unknown, "expressions should not be combined",
            return -1);
...
static int test_pw_multi_aff(isl_ctx *ctx) {
  ...
  if (equal < 0)
    return -1;
  if (!equal)
    isl_die(ctx, isl_error_unknown, "expressions not equal", return -1);
...
static int test_multi_pw_aff_1(isl_ctx *ctx) {
  ...
  if (equal_domain < 0)
    return -1;
  if (equal_domain)
    isl_die(ctx, isl_error_unknown, "domains unexpectedly equal", return -1);
...
static int test_simplify_1(isl_ctx *ctx) {
  ...
  if (!bset)
    return -1;
  if (!empty)
    isl_die(ctx, isl_error_unknown, "basic set should be empty", return -1);
...
static int test_simplify_2(isl_ctx *ctx) {
  ...
  if (universe < 0)
    return -1;
  if (!universe)
    isl_die(ctx, isl_error_unknown, "equality not simplified away", return -1);
  return 0;
}
...
static int test_schedule_tree_prefix(isl_ctx *ctx) {
  ...
  if (equal < 0)
    return -1;
  if (!equal)
    isl_die(ctx, isl_error_unknown, "unexpected prefix schedule", return -1);
...
static int test_schedule_tree_group_2(isl_ctx *ctx) {
  ...
  if (!equal)
    isl_die(ctx, isl_error_unknown, "expressions not equal", return -1);
  if (!equal_expansion)
    isl_die(ctx, isl_error_unknown, "unexpected expansion", return -1);
...
struct {
  ...
  const char *dual;
    ...
    {"{ rat: [i] : FALSE }", "{ rat: coefficients[[cst] -> [a]] }"},
    {"{ rat: [i] : }", "{ rat: coefficients[[cst] -> [0]] : cst >= 0 }"},
    {"{ [0:,1,2:3] }", "{ rat: coefficients[[c_cst] -> [a, b, c]] : "
...
struct {
  ...
  const char *dual;
    ...
    {"{ rat: coefficients[[cst] -> [a]] : cst >= 0 and 10a + cst >= 0 }",
     "{ rat: [i] : 0 <= i <= 10 }"},
    {"{ rat: coefficients[[cst] -> [a]] : FALSE }", "{ rat: [i] }"},
    {"{ rat: coefficients[[cst] -> [a]] }", "{ rat: [i] : FALSE }"},
};
...
static int test_dual(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(coef_tests); ++i) {
    ...
    if (equal < 0)
      return -1;
    if (!equal)
      isl_die(ctx, isl_error_unknown, "incorrect dual", return -1);
  }
...
static int test_domain_hash(isl_ctx *ctx) {
  ...
  if (!space)
    return -1;
  if (hash1 != hash2)
    isl_die(ctx, isl_error_unknown, "domain hash not equal to hash of domain",
            return -1);
...
static int test_universe(isl_ctx *ctx) {
  ...
  if (is_univ < 0)
    return -1;
  if (!is_univ)
    isl_die(ctx, isl_error_unknown, "not recognized as universe set",
            return -1);
...
static isl_stat check_pairwise_disjoint(__isl_keep isl_basic_set_list *list) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    for (j = i + 1; j < n; ++j) {
      ...
      if (!disjoint)
        isl_die(isl_basic_set_list_get_ctx(list), isl_error_unknown,
                "not disjoint", break);
...
static int test_chambers(isl_ctx *ctx) {
  ...
  for (i = 0; i < ARRAY_SIZE(chambers_tests); ++i) {
    ...
    if (isl_vertices_foreach_disjoint_cell(vertices, &add_cell, &cells) < 0)
