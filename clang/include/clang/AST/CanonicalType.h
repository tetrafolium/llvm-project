...
template <typename T> struct simplify_type<::clang::CanQual<T>> {
...
template <typename T> struct PointerLikeTypeTraits<clang::CanQual<T>> {
...
template <>
struct CanProxyAdaptor<ObjCObjectType> : public CanProxyBase<ObjCObjectType> {
  LLVM_CLANG_CANPROXY_TYPE_ACCESSOR(getBaseType)
  LLVM_CLANG_CANPROXY_SIMPLE_ACCESSOR(const ObjCInterfaceDecl *, getInterface)
