...
class Bound {
public:
    static Bound Assumed() {
        return Bound(Category::Assumed);
    }
    static Bound Deferred() {
        return Bound(Category::Deferred);
    }
...
inline bool ArraySpec::IsImpliedShape() const {
    ...
    CheckAll([](const ShapeSpec &x) {
        return x.ubound().isAssumed();
    });
}
