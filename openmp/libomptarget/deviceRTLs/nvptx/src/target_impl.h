...
INLINE uint32_t __kmpc_impl_ffs(uint32_t x) {
    return __ffs(x);
}

INLINE uint32_t __kmpc_impl_popc(uint32_t x) {
    return __popc(x);
}
...
INLINE void __kmpc_impl_threadfence(void) {
    __threadfence();
}
INLINE void __kmpc_impl_threadfence_block(void) {
    __threadfence_block();
}
INLINE void __kmpc_impl_threadfence_system(void) {
...
INLINE unsigned GetLaneId() {
    return GetThreadIdInBlock() & (WARPSIZE - 1);
}
...
// Memory
INLINE void *__kmpc_impl_malloc(size_t x) {
    return malloc(x);
}
INLINE void __kmpc_impl_free(void *x) {
    free(x);
}
