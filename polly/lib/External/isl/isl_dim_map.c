...
__isl_give isl_dim_map *isl_dim_map_alloc(isl_ctx *ctx, unsigned len) {
  ...
  dim_map = isl_alloc(ctx, struct isl_dim_map,
                      sizeof(struct isl_dim_map) +
                          len * sizeof(struct isl_dim_map_entry));
  if (!dim_map)
    return NULL;
  dim_map->len = 1 + len;
  dim_map->m[0].pos = 0;
...
void isl_dim_map_range(__isl_keep isl_dim_map *dim_map, unsigned dst_pos,
                       int dst_stride, unsigned src_pos, int src_stride,
                       unsigned n, int sign) {
  int i;
...
void isl_dim_map_dump(struct isl_dim_map *dim_map) {
  ...
  for (i = 0; i < dim_map->len; ++i)
    fprintf(stderr, "%d -> %d * %d; ", i, dim_map->m[i].sgn, dim_map->m[i].pos);
  fprintf(stderr, "\n");
}
...
isl_basic_map_add_constraints_dim_map(__isl_take isl_basic_map *dst,
...
isl_basic_set_add_constraints_dim_map(__isl_take isl_basic_set *dst,
...
isl_dim_map_from_reordering(__isl_keep isl_reordering *exp) {
  int i;
  isl_ctx *ctx;
  isl_size dim;
  isl_space *space;
