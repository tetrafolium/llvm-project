...
public:
    ...
    void set_isImplicitNoneType(bool x) {
        isImplicitNoneType_ = x;
    }
    void set_isImplicitNoneExternal(bool x) {
        isImplicitNoneExternal_ = x;
    }
    void set_inheritFromParent(bool x) {
        inheritFromParent_ = x;
    }
...
public:
    MessageHandler() {
        DIE("MessageHandler: default-constructed");
    }
    explicit MessageHandler(SemanticsContext &c) : context_{&c} {}
    Messages &messages() {
        return context_->messages();
    };
...
public:
    BaseVisitor() {
        DIE("BaseVisitor: default-constructed");
    }
    BaseVisitor(
...
private:
    ...
    const GenericInfo &GetGenericInfo() const {
        return genericInfo_.top();
    }
    void SetGenericSymbol(Symbol &symbol) {
        genericInfo_.top().symbol = &symbol;
    }
...
bool AttrsVisitor::SetPassNameOn(Symbol &symbol) {
    ...
    std::visit(common::visitors{
        [&](ProcEntityDetails &x) {
            x.set_passName(*passName_);
        },
        [&](ProcBindingDetails &x) {
            x.set_passName(*passName_);
        },
        [](auto &) {
            common::die("unexpected pass name");
        },
    },
    symbol.details());
    return true;
...
bool AttrsVisitor::SetBindNameOn(Symbol &symbol) {
    ...
    common::visitors{
        [&](EntityDetails &x) {
            x.set_bindName(std::move(bindName_));
        },
        ...
        [&](CommonBlockDetails &x) {
            x.set_bindName(std::move(bindName_));
        },
        [](auto &) {
            common::die("unexpected bind name");
        },
    },
    symbol.details());
    return true;
...
Scope &ScopeHandler::InclusiveScope() {
    return GetInclusiveScope(currScope());
}
...
bool SubprogramVisitor::Pre(const parser::EntryStmt &) {
    return BeginAttrs();
}
...
void SubprogramVisitor::Post(const parser::EntryStmt &stmt) {
    ...
    if (inFunction) {
        ...
        if (resultSymbol) { // C1574
            std::visit(
            common::visitors{[](EntityDetails &x) {
                x.set_funcResult(true);
            },
            [](ObjectEntityDetails &x) {
                x.set_funcResult(true);
            },
...
bool ConstructVisitor::Pre(const parser::DataIDoObject &x) {
    ...
    common::visitors{
        ...
        [&](const Indirection<parser::DataImpliedDo> &y) {
            Walk(y.value());
        },
    },
    x.u);
...
void ConstructVisitor::Post(const parser::ForallConstruct &) {
    PopScope();
}
...
void ConstructVisitor::Post(const parser::ForallStmt &) {
    PopScope();
}
...
void ResolveNamesVisitor::HandleCall(
    ...
    common::visitors{
        [&](const parser::Name &x) {
            HandleProcedureName(procFlag, x);
        },
...
void ResolveNamesVisitor::PreSpecificationConstruct(
    ...
    common::visitors{
        ...
        [&](const Indirection<parser::InterfaceBlock> &y) {
            const auto &stmt{
                std::get<parser::Statement<parser::InterfaceStmt>>(
                            y.value().t)};
...
public:
    ...
    template <typename A> bool Pre(const A &) {
        return true;
    }
...
public:
    ...
    template <typename A> void Walk(const A &x) {
        parser::Walk(x, *this);
    }
