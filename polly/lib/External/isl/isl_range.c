...
static isl_bool has_sign(__isl_keep isl_basic_set *bset,
                         __isl_keep isl_qpolynomial *poly, int sign,
                         int *signs) {
  ...
  poly =
      isl_qpolynomial_move_dims(poly, isl_dim_in, 0, isl_dim_param, 0, nparam);
  ...
  if (!opt)
    r = isl_bool_error;
  else if (isl_val_is_nan(opt) || isl_val_is_infty(opt) ||
...
static int monotonicity(__isl_keep isl_basic_set *bset,
  ...
  sub = isl_qpolynomial_var_on_domain(isl_space_copy(space), isl_dim_set,
                                      nvar - 1);
  sub = isl_qpolynomial_add(
      sub, isl_qpolynomial_rat_cst_on_domain(space, ctx->one, ctx->one));

  diff = isl_qpolynomial_substitute(isl_qpolynomial_copy(poly), isl_dim_in,
                                    nvar - 1, 1, &sub);
...
isl_qpolynomial_terms_of_sign(__isl_keep isl_qpolynomial *poly, int *signs,
                              int sign) {
  isl_space *space;
...
plug_in_at_pos(__isl_take isl_qpolynomial *poly, int pos,
...
static isl_stat basic_guarded_poly_bound(__isl_take isl_basic_set *bset,
  ...
  if (isl_basic_set_dims_get_sign(bset, isl_dim_set, 0, dim,
                                  data->signs + nparam) < 0)
    goto error;
  if (isl_basic_set_dims_get_sign(bset, isl_dim_param, 0, nparam, data->signs) <
      0)
    goto error;
...
static isl_stat
qpolynomial_bound_on_domain_range(__isl_take isl_basic_set *bset,
                                  __isl_take isl_qpolynomial *poly,
                                  struct range_data *data) {
  isl_size nparam = isl_basic_set_dim(bset, isl_dim_param);
