...
#define GBR_set_num_neg(a, b)                                                  \
  do {                                                                         \
    ...
    impz_neg(mp_rat_numer_ref(*a), isl_sioimath_bigarg_src(*b, &scratch));     \
  } while (0)
#define GBR_set_den(a, b)                                                      \
  do {                                                                         \
    isl_sioimath_scratchspace_t scratch;                                       \
    impz_set(mp_rat_denom_ref(*a), isl_sioimath_bigarg_src(*b, &scratch));     \
  } while (0)
...
static int solve_lp(struct tab_lp *lp) {
  ...
  res = isl_tab_min(lp->tab, lp->row->el, lp->ctx->one, &lp->opt,
                    &lp->opt_denom, flags);
  ...
  if (isl_int_abs_lt(lp->opt, lp->opt_denom)) {
    ...
    isl_seq_inner_product(lp->obj, sample->el + 1 + lp->dim, lp->dim,
                          &lp->tmp2);
...
static int cut_lp_to_hyperplane(struct tab_lp *lp, isl_int *row) {
  ...
  res = isl_tab_min(lp->tab, lp->row->el, lp->ctx->one, &lp->tmp, NULL, 0);
  if (res != isl_lp_ok)
    return -1;
