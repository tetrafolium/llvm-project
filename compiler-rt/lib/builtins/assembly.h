...
#define GNU_PROPERTY(type, value)                                              \
  .pushsection.note.gnu.property,                                              \
      "a" SEPARATOR.p2align 3 SEPARATOR.word 4 SEPARATOR.word 16 SEPARATOR     \
          .word NT_GNU_PROPERTY_TYPE_0 SEPARATOR.asciz                         \
      "GNU" SEPARATOR.word type SEPARATOR.word 4 SEPARATOR.word value          \
          SEPARATOR.word 0 SEPARATOR.popsection
  ...
  FILE_LEVEL_DIRECTIVE SEPARATOR.globl SYMBOL_NAME(name)                       \
      SEPARATOR SYMBOL_IS_FUNC(SYMBOL_NAME(name))                              \
          SEPARATOR DECLARE_SYMBOL_VISIBILITY(name)                            \
              DECLARE_FUNC_ENCODING SYMBOL_NAME(name)                          \
      :

#define DEFINE_COMPILERRT_THUMB_FUNCTION(name)                                 \
  DEFINE_CODE_STATE                                                            \
  FILE_LEVEL_DIRECTIVE SEPARATOR                                               \
      .globl SYMBOL_NAME(name) SEPARATOR SYMBOL_IS_FUNC(SYMBOL_NAME(name))     \
          SEPARATOR DECLARE_SYMBOL_VISIBILITY(name)                            \
              SEPARATOR.thumb_func SEPARATOR SYMBOL_NAME(name)                 \
      :
  ...
  FILE_LEVEL_DIRECTIVE SEPARATOR.globl SYMBOL_NAME(name)                       \
      SEPARATOR SYMBOL_IS_FUNC(SYMBOL_NAME(name))                              \
          SEPARATOR HIDDEN(SYMBOL_NAME(name))                                  \
              SEPARATOR DECLARE_FUNC_ENCODING SYMBOL_NAME(name)                \
      :

#define DEFINE_COMPILERRT_PRIVATE_FUNCTION_UNMANGLED(name)                     \
  DEFINE_CODE_STATE.globl name SEPARATOR SYMBOL_IS_FUNC(name)                  \
      SEPARATOR HIDDEN(name) SEPARATOR DECLARE_FUNC_ENCODING name:

#define DEFINE_COMPILERRT_OUTLINE_FUNCTION_UNMANGLED(name)                     \
  DEFINE_CODE_STATE                                                            \
  FUNC_ALIGN.globl name SEPARATOR SYMBOL_IS_FUNC(name)                         \
      SEPARATOR DECLARE_SYMBOL_VISIBILITY(name)                                \
          SEPARATOR CFI_START SEPARATOR DECLARE_FUNC_ENCODING name             \
      : SEPARATOR BTI_C

#define DEFINE_COMPILERRT_FUNCTION_ALIAS(name, target)                         \
  .globl SYMBOL_NAME(name) SEPARATOR SYMBOL_IS_FUNC(SYMBOL_NAME(name))         \
      SEPARATOR DECLARE_SYMBOL_VISIBILITY(SYMBOL_NAME(name))                   \
          SEPARATOR.set SYMBOL_NAME(name),                                     \
