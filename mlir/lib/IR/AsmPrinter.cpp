...
private:
    ...
    raw_ostream &getStream() const override {
        return llvm::nulls();
    }
...
void ModulePrinter::printType(Type type) {
    ...
    .Case<FunctionType>([&](FunctionType funcTy) {
        os << '(';
        interleaveComma(funcTy.getInputs(), [&](Type ty) {
            printType(ty);
        });
        os << ") -> ";
        ...
        if (results.size() == 1 && !results[0].isa<FunctionType>()) {
            ...
            os << '(';
            interleaveComma(results, [&](Type ty) {
                printType(ty);
            });
            os << ')';
        }
...
public:
    ...
    void printOperand(Value value) override {
        printValueID(value);
    }
...
void OperationPrinter::printGenericOp(Operation *op) {
    ...
    os << "\"(";
    interleaveComma(op->getOperands(), [&](Value value) {
        printValueID(value);
    });
    os << ')';
...
void Type::print(raw_ostream &os) {
    ModulePrinter(os).printType(*this);
}
