...
public:
    ...
    void setOMPAllocatorHandleT(QualType Ty) {
        OMPAllocatorHandleT = Ty;
    }
    /// Gets omp_allocator_handle_t type.
    QualType getOMPAllocatorHandleT() const {
        return OMPAllocatorHandleT;
    }
    /// Sets omp_alloctrait_t type.
    void setOMPAlloctraitT(QualType Ty) {
        OMPAlloctraitT = Ty;
    }
    /// Gets omp_alloctrait_t type.
    QualType getOMPAlloctraitT() const {
    ...
    void setOMPDependT(QualType Ty) {
        OMPDependT = Ty;
    }
    /// Gets omp_depend_t type.
    QualType getOMPDependT() const {
        return OMPDependT;
    }

    /// Sets omp_event_handle_t type.
    void setOMPEventHandleT(QualType Ty) {
        OMPEventHandleT = Ty;
    }
    /// Gets omp_event_handle_t type.
    QualType getOMPEventHandleT() const {
        return OMPEventHandleT;
    }

    bool isClauseParsingMode() const {
    ...
    void setClauseParsingMode(OpenMPClauseKind K) {
        ClauseKindMode = K;
    }
    ...
    bool isForceVarCapturing() const {
    ...
    void setContext(DeclContext *DC) {
        getTopOfStack().Context = DC;
    }
...
OpenMPClauseKind Sema::isOpenMPPrivateDecl(ValueDecl *D, unsigned Level,
    ...
    return isOpenMPTaskingDirective(K);
    },
    Level)) {
...
public:
    ...
    ValueDecl *getLoopDecl() const {
        return LCDecl;
    }
    /// Return the reference expression to loop counter variable.
    Expr *getLoopDeclRefExpr() const {
        return LCRef;
    }
    /// Source range of the loop init.
    SourceRange getInitSrcRange() const {
        return InitSrcRange;
    }
    /// Source range of the loop condition.
    SourceRange getConditionSrcRange() const {
    ...
    bool shouldSubtractStep() const {
        return SubtractStep;
    }
    /// True, if the compare operator is strict (<, > or !=).
    bool isStrictTestOp() const {
...
OMPClause *Sema::ActOnOpenMPUsesAllocatorClause(
    ...
    if (llvm::any_of(
                Data,
    [](const UsesAllocatorsData &D) {
    return D.AllocatorTraits;
}) &&
