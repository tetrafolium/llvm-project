  ...
  do {
    ...
    if (isl_int_eq(mu[0], mu[1]))
      ...
      for (j = 0; j <= 1; ++j) {
        ...
        isl_seq_combine(b_tmp->el, ctx->one, B->row[1 + i + 1] + 1, tmp,
                        B->row[1 + i] + 1, dim);
    ...
    isl_seq_combine(B->row[1 + i + 1] + 1, ctx->one, B->row[1 + i + 1] + 1, tmp,
                    B->row[1 + i] + 1, dim);
 ...
 */
__isl_give isl_mat *
isl_basic_set_reduced_basis(__isl_keep isl_basic_set *bset) {
  ...
  if (bset->n_eq == 0)
    ...
    if (nvar < 0)
      goto error;
    eq = isl_mat_sub_alloc6(bset->ctx, bset->eq, 0, bset->n_eq, 1, nvar);
