...
isl_constraint_alloc_vec(int eq, __isl_take isl_local_space *ls,
...
__isl_give isl_constraint *
isl_constraint_alloc(int eq, __isl_take isl_local_space *ls) {
  isl_size dim;
  isl_ctx *ctx;
  isl_vec *v;
...
__isl_give isl_constraint *
isl_basic_map_constraint(__isl_take isl_basic_map *bmap, isl_int **line) {
  int eq;
  isl_size dim;
  isl_ctx *ctx;
  isl_vec *v;
...
__isl_give isl_constraint *
isl_basic_set_constraint(__isl_take isl_basic_set *bset, isl_int **line) {
  return isl_basic_map_constraint(bset_to_bmap(bset), line);
}

__isl_give isl_constraint *
isl_constraint_alloc_equality(__isl_take isl_local_space *ls) {
  return isl_constraint_alloc(1, ls);
}

__isl_give isl_constraint *
isl_constraint_alloc_inequality(__isl_take isl_local_space *ls) {
...
__isl_give isl_constraint *
isl_constraint_copy(__isl_keep isl_constraint *constraint) {
  if (!constraint)
    return NULL;
...
isl_stat isl_basic_map_foreach_constraint(
  ...
  for (i = 0; i < bmap->n_eq; ++i) {
    c = isl_basic_map_constraint(isl_basic_map_copy(bmap), &bmap->eq[i]);
    if (!c)
      return isl_stat_error;
    if (fn(c, user) < 0)
      return isl_stat_error;
  ...
  for (i = 0; i < bmap->n_ineq; ++i) {
    c = isl_basic_map_constraint(isl_basic_map_copy(bmap), &bmap->ineq[i]);
    if (!c)
      return isl_stat_error;
    if (fn(c, user) < 0)
      return isl_stat_error;
...
isl_basic_map_get_constraint_list(__isl_keep isl_basic_map *bmap) {
  isl_size n;
  isl_bool known;
  isl_ctx *ctx;
  ...
  ctx = isl_basic_map_get_ctx(bmap);
  if (!known)
    isl_die(ctx, isl_error_invalid, "input involves unknown divs", return NULL);

  n = isl_basic_map_n_constraint(bmap);
  if (n < 0)
    return NULL;
  list = isl_constraint_list_alloc(ctx, n);
  if (isl_basic_map_foreach_constraint(bmap, &collect_constraint, &list) < 0)
 ...
 */
__isl_give isl_constraint_list *
isl_basic_set_get_constraint_list(__isl_keep isl_basic_set *bset) {
...
isl_basic_map_add_constraint(__isl_take isl_basic_map *bmap,
  ...
  bmap =
      isl_basic_map_intersect(bmap, isl_basic_map_from_constraint(constraint));
  return bmap;
error:
  isl_basic_map_free(bmap);
  isl_constraint_free(constraint);
...
isl_basic_set_add_constraint(__isl_take isl_basic_set *bset,
                             __isl_take isl_constraint *constraint) {
  return bset_from_bmap(
      isl_basic_map_add_constraint(bset_to_bmap(bset), constraint));
}

__isl_give isl_map *
isl_map_add_constraint(__isl_take isl_map *map,
                       __isl_take isl_constraint *constraint) {
  isl_basic_map *bmap;
...
__isl_give isl_set *
isl_set_add_constraint(__isl_take isl_set *set,
                       __isl_take isl_constraint *constraint) {
...
isl_constraint_peek_space(__isl_keep isl_constraint *constraint) {
  return constraint ? isl_local_space_peek_space(constraint->ls) : NULL;
}

__isl_give isl_space *
isl_constraint_get_space(__isl_keep isl_constraint *constraint) {
  return constraint ? isl_local_space_get_space(constraint->ls) : NULL;
}

__isl_give isl_local_space *
isl_constraint_get_local_space(__isl_keep isl_constraint *constraint) {
...
#include "check_type_range_templ.c"
    ...
    isl_bool
    isl_constraint_involves_dims(__isl_keep isl_constraint *constraint,
  ...
  active = isl_local_space_get_active(constraint->ls, constraint->v->el + 1);
  if (!active)
    goto error;
...
const char *isl_constraint_get_dim_name(__isl_keep isl_constraint *constraint,
                                        enum isl_dim_type type, unsigned pos) {
  return constraint ? isl_local_space_get_dim_name(constraint->ls, type, pos)
                    : NULL;
}
 ...
 */
__isl_give isl_val *
isl_constraint_get_constant_val(__isl_keep isl_constraint *constraint) {
  isl_ctx *ctx;
 ...
 */
__isl_give isl_val *
isl_constraint_get_coefficient_val(__isl_keep isl_constraint *constraint,
                                   enum isl_dim_type type, int pos) {
  isl_ctx *ctx;
...
__isl_give isl_aff *
isl_constraint_get_div(__isl_keep isl_constraint *constraint, int pos) {
  if (!constraint)
    return NULL;
...
__isl_give isl_constraint *
isl_constraint_set_constant(__isl_take isl_constraint *constraint, isl_int v) {
 ...
 */
__isl_give isl_constraint *
isl_constraint_set_constant_val(__isl_take isl_constraint *constraint,
...
__isl_give isl_constraint *
isl_constraint_set_constant_si(__isl_take isl_constraint *constraint, int v) {
  constraint = isl_constraint_cow(constraint);
  if (!constraint)
    return NULL;
...
__isl_give isl_constraint *
isl_constraint_set_coefficient(__isl_take isl_constraint *constraint,
 ...
 */
__isl_give isl_constraint *
isl_constraint_set_coefficient_val(__isl_take isl_constraint *constraint,
                                   enum isl_dim_type type, int pos,
                                   __isl_take isl_val *v) {
  constraint = isl_constraint_cow(constraint);
  if (!constraint || !v)
...
__isl_give isl_constraint *
isl_constraint_set_coefficient_si(__isl_take isl_constraint *constraint,
...
__isl_give isl_constraint *
isl_constraint_negate(__isl_take isl_constraint *constraint) {
  isl_ctx *ctx;
...
isl_bool
isl_constraint_is_div_constraint(__isl_keep isl_constraint *constraint) {
  int i;
  isl_size n_div;
  ...
  for (i = 0; i < n_div; ++i) {
    isl_bool is_div;
    is_div =
        isl_local_space_is_div_constraint(constraint->ls, constraint->v->el, i);
    if (is_div < 0 || is_div)
      return is_div;
  }
...
isl_bool isl_constraint_is_div_equality(__isl_keep isl_constraint *constraint,
  ...
  if (equality < 0 || !equality)
    return equality;
  return isl_local_space_is_div_equality(constraint->ls, constraint->v->el,
                                         div);
}
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_from_constraint(__isl_take isl_constraint *constraint) {
  int k;
  isl_local_space *ls;
...
__isl_give isl_basic_set *
isl_basic_set_from_constraint(__isl_take isl_constraint *constraint) {
  isl_space *space;
...
isl_bool isl_basic_set_has_defining_equality(__isl_keep isl_basic_set *bset,
                                             enum isl_dim_type type, int pos,
                                             __isl_give isl_constraint **c) {
  return isl_basic_map_has_defining_equality(bset_to_bmap(bset), type, pos, c);
}

isl_bool isl_basic_set_has_defining_inequalities(
    struct isl_basic_set *bset, enum isl_dim_type type, int pos,
    struct isl_constraint **lower, struct isl_constraint **upper) {
  int i, j;
  unsigned offset;
  isl_size total;
  isl_int m;
  ...
  for (i = 0; i < bset->n_ineq; ++i) {
    ...
    for (j = i + 1; j < bset->n_ineq; ++j) {
      if (!isl_seq_is_neg(bset->ineq[i] + 1, bset->ineq[j] + 1, total))
        continue;
      ...
      *lower = isl_basic_set_constraint(isl_basic_set_copy(bset), lower_line);
      *upper = isl_basic_set_constraint(isl_basic_set_copy(bset), upper_line);
      isl_int_clear(m);
      return isl_bool_true;
    }
  }
...
add_larger_bound_constraint(__isl_take isl_basic_set *bset, isl_int *a,
                            isl_int *b, unsigned abs_pos, int strict) {
  int k;
  isl_int t;
  isl_size total;
  ...
  isl_seq_combine(bset->ineq[k] + 1 + abs_pos, t, a + 1 + abs_pos + 1,
                  a[1 + abs_pos], b + 1 + abs_pos + 1, total - abs_pos);
 ...
 */
static __isl_give isl_basic_set *
set_smallest_upper_bound(__isl_keep isl_basic_set *context,
                         __isl_keep isl_basic_set *bset, unsigned abs_pos,
                         int n_upper, int u) {
  int j;
  ...
  for (j = 0; j < bset->n_ineq; ++j) {
    ...
    if (!isl_int_is_neg(bset->ineq[j][1 + abs_pos]))
      continue;
    context = add_larger_bound_constraint(context, bset->ineq[j], bset->ineq[u],
                                          abs_pos, j > u);
  }
 ...
 */
static __isl_give isl_basic_set *
set_largest_lower_bound(__isl_keep isl_basic_set *context,
                        __isl_keep isl_basic_set *bset, unsigned abs_pos,
                        int n_lower, int l) {
  int j;
  ...
  for (j = 0; j < bset->n_ineq; ++j) {
    ...
    if (!isl_int_is_pos(bset->ineq[j][1 + abs_pos]))
      continue;
    context = add_larger_bound_constraint(context, bset->ineq[l], bset->ineq[j],
                                          abs_pos, j > l);
  }
...
static isl_stat
foreach_upper_bound(__isl_keep isl_basic_set *bset, enum isl_dim_type type,
                    unsigned abs_pos, __isl_take isl_basic_set *context,
                    int n_upper,
  ...
  for (i = 0; i < bset->n_ineq; ++i) {
    ...
    context_i = set_smallest_upper_bound(context, bset, abs_pos, n_upper, i);
    if (isl_basic_set_is_empty(context_i)) {
      isl_basic_set_free(context_i);
      continue;
    }
    upper = isl_basic_set_constraint(isl_basic_set_copy(bset), &bset->ineq[i]);
    if (!upper || !context_i)
      goto error;
...
static isl_stat
foreach_lower_bound(__isl_keep isl_basic_set *bset, enum isl_dim_type type,
                    unsigned abs_pos, __isl_take isl_basic_set *context,
                    int n_lower,
                    isl_stat (*fn)(__isl_take isl_constraint *lower,
                                   __isl_take isl_constraint *upper,
  ...
  for (i = 0; i < bset->n_ineq; ++i) {
    ...
    context_i = set_largest_lower_bound(context, bset, abs_pos, n_lower, i);
    if (isl_basic_set_is_empty(context_i)) {
      isl_basic_set_free(context_i);
      continue;
    }
    lower = isl_basic_set_constraint(isl_basic_set_copy(bset), &bset->ineq[i]);
    if (!lower || !context_i)
      goto error;
...
static isl_stat
foreach_bound_pair(__isl_keep isl_basic_set *bset, enum isl_dim_type type,
                   unsigned abs_pos, __isl_take isl_basic_set *context,
  ...
  for (i = 0; i < bset->n_ineq; ++i) {
    ...
    for (j = 0; j < bset->n_ineq; ++j) {
      ...
      context_j =
          set_smallest_upper_bound(context_i, bset, abs_pos, n_upper, j);
      context_j = isl_basic_set_extend_constraints(context_j, 0, 1);
      context_j = add_larger_bound_constraint(context_j, bset->ineq[i],
                                              bset->ineq[j], abs_pos, 0);
      ...
      lower =
          isl_basic_set_constraint(isl_basic_set_copy(bset), &bset->ineq[i]);
      upper =
          isl_basic_set_constraint(isl_basic_set_copy(bset), &bset->ineq[j]);
 ...
 * act accordingly.
 */
isl_stat isl_basic_set_foreach_bound_pair(
  ...
  for (i = 0; i < bset->n_eq; ++i) {
    ...
    lower = isl_basic_set_constraint(isl_basic_set_copy(bset), &bset->eq[i]);
    upper = isl_constraint_copy(lower);
    context = isl_basic_set_remove_dims(isl_basic_set_copy(bset), type, pos, 1);
  ...
  if (!n_lower)
    return foreach_upper_bound(bset, type, abs_pos, context, n_upper, fn, user);
  if (!n_upper)
    return foreach_lower_bound(bset, type, abs_pos, context, n_lower, fn, user);
  return foreach_bound_pair(bset, type, abs_pos, context, n_lower, n_upper, fn,
                            user);
error:
  isl_constraint_free(lower);
  isl_constraint_free(upper);
...
__isl_give isl_aff *
isl_constraint_get_bound(__isl_keep isl_constraint *constraint,
                         enum isl_dim_type type, int pos) {
  isl_space *space;
  isl_aff *aff;
  isl_ctx *ctx;
 ...
 */
__isl_give isl_aff *
isl_constraint_get_aff(__isl_keep isl_constraint *constraint) {
  isl_aff *aff;
...
isl_constraint_alloc_aff(int eq, __isl_take isl_aff *aff) {
  isl_local_space *ls;
  isl_vec *v;
