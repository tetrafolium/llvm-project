...
public:
  ...
  _LIBCPP_HIDDEN
  __barrier_algorithm_base(ptrdiff_t& __expected) : __expected(__expected) {
  ...
  bool __arrive(__barrier_phase_t __old_phase) {
    ...
    size_t __current_expected = __expected,
           __current = hash<thread::id>()(this_thread::get_id()) %
                       ((__expected + 1) >> 1);
    for (int __round = 0;; ++__round) {
      ...
      for (;; ++__current) {
        ...
        if (__current == __last_node && (__current_expected & 1)) {
          if (__state[__current]
                  .__tickets[__round]
                  .__phase.compare_exchange_strong(expect, __full_step,
                                                   memory_order_acq_rel))
            break; // I'm 1 in 1, go to next __round
        } else if (__state[__current]
                       .__tickets[__round]
                       .__phase.compare_exchange_strong(expect, __half_step,
                                                        memory_order_acq_rel)) {
