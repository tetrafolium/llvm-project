...
__isl_give isl_vec *isl_vec_expand(__isl_take isl_vec *vec, int pos, int n,
  ...
  if (!vec)
    return NULL;
  if (expanded < n)
    isl_die(isl_vec_get_ctx(vec), isl_error_invalid, "not an expansion",
            return isl_vec_free(vec));
  if (expanded == n)
    return vec;
  if (pos < 0 || n < 0 || pos + n > vec->size)
    isl_die(isl_vec_get_ctx(vec), isl_error_invalid, "position out of bounds",
            return isl_vec_free(vec));
  ...
  for (i = expanded - 1; i >= 0; --i) {
    if (j >= 0 && exp[j] == i) {
      if (i != j)
        isl_int_swap(vec->el[pos + i], vec->el[pos + j]);
      j--;
    } else {
...
__isl_give isl_val *isl_vec_get_element_val(__isl_keep isl_vec *vec, int pos) {
  ...
  if (pos < 0 || pos >= vec->size)
    isl_die(ctx, isl_error_invalid, "position out of range", return NULL);
  return isl_val_int_from_isl_int(ctx, vec->el[pos]);
}

__isl_give isl_vec *isl_vec_set_element(__isl_take isl_vec *vec, int pos,
                                        isl_int v) {
  vec = isl_vec_cow(vec);
  if (!vec)
    return NULL;
  if (pos < 0 || pos >= vec->size)
    isl_die(vec->ctx, isl_error_invalid, "position out of range", goto error);
...
__isl_give isl_vec *isl_vec_set_element_si(__isl_take isl_vec *vec, int pos,
                                           int v) {
  vec = isl_vec_cow(vec);
  if (!vec)
    return NULL;
 ...
 */
__isl_give isl_vec *isl_vec_set_element_val(__isl_take isl_vec *vec, int pos,
...
int isl_vec_cmp_element(__isl_keep isl_vec *vec1, __isl_keep isl_vec *vec2,
  ...
  if (pos < 0 || pos >= vec1->size || pos >= vec2->size)
    isl_die(isl_vec_get_ctx(vec1), isl_error_invalid, "position out of range",
            return 0);
...
__isl_give isl_vec *isl_vec_add(__isl_take isl_vec *vec1,
  ...
  isl_seq_combine(vec1->el, vec1->ctx->one, vec1->el, vec1->ctx->one, vec2->el,
                  vec1->size);
...
__isl_give isl_vec *isl_vec_drop_els(__isl_take isl_vec *vec, unsigned pos,
                                     unsigned n) {
  if (n == 0)
    return vec;
  vec = isl_vec_cow(vec);
  ...
  if (pos + n > vec->size)
    isl_die(vec->ctx, isl_error_invalid, "range out of bounds", goto error);

  if (pos + n != vec->size)
    isl_seq_cpy(vec->el + pos, vec->el + pos + n, vec->size - pos - n);
...
__isl_give isl_vec *isl_vec_insert_els(__isl_take isl_vec *vec, unsigned pos,
                                       unsigned n) {
  isl_vec *ext = NULL;
  ...
  if (pos > vec->size)
    isl_die(vec->ctx, isl_error_invalid, "position out of bounds", goto error);
 ...
 * than "src_pos".
 */
__isl_give isl_vec *isl_vec_move_els(__isl_take isl_vec *vec, unsigned dst_pos,
  ...
  if (src_pos + n > vec->size)
    isl_die(vec->ctx, isl_error_invalid, "source range out of bounds",
            return isl_vec_free(vec));
  if (dst_pos + n > vec->size)
    isl_die(vec->ctx, isl_error_invalid, "destination range out of bounds",
            return isl_vec_free(vec));
  ...
  if (dst_pos < src_pos) {
    ...
    isl_seq_cpy(res->el + dst_pos + n, vec->el + dst_pos, src_pos - dst_pos);
    isl_seq_cpy(res->el + src_pos + n, vec->el + src_pos + n,
                res->size - src_pos - n);
  } else {
    isl_seq_cpy(res->el, vec->el, src_pos);
    isl_seq_cpy(res->el + src_pos, vec->el + src_pos + n, dst_pos - src_pos);
    isl_seq_cpy(res->el + dst_pos, vec->el + src_pos, n);
    isl_seq_cpy(res->el + dst_pos + n, vec->el + dst_pos + n,
                res->size - dst_pos - n);
  }
