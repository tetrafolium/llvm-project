...
bool ObjCARCOpt::VisitInstructionTopDown(Instruction *Inst,
                                         DenseMap<Value *, RRInfo> &Releases,
                                         BBState &MyStates) {
  bool NestingDetected = false;
  ARCInstKind Class = GetARCInstKind(Inst);
...
bool ObjCARCOpt::VisitTopDown(BasicBlock *BB,
                              DenseMap<const BasicBlock *, BBState> &BBStates,
                              DenseMap<Value *, RRInfo> &Releases) {
...
void ObjCARCOpt::MoveCalls(Value *Arg, RRInfo &RetainsToMove,
  ...
  for (Instruction *InsertPt : ReleasesToMove.ReverseInsertPts) {
    Value *MyArg =
        ArgTy == ParamTy ? Arg : new BitCastInst(Arg, ParamTy, "", InsertPt);
...
void ObjCARCOpt::releaseMemory() { PA.clear(); }
