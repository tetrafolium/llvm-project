...
protected:
  ...
  // Compares.
  bool evaluateCMPrr(uint32_t Cmp, const RegisterSubReg &R1,
                     const RegisterSubReg &R2, const CellMap &Inputs,
                     bool &Result);
...
bool MachineConstEvaluator::getCell(const RegisterSubReg &R,
...
bool MachineConstEvaluator::evaluateCMPrr(uint32_t Cmp,
                                          const RegisterSubReg &R1,
                                          const RegisterSubReg &R2,
                                          const CellMap &Inputs, bool &Result) {
...
bool MachineConstEvaluator::evaluateCMPri(uint32_t Cmp,
                                          const RegisterSubReg &R1,
                                          const APInt &A2,
                                          const CellMap &Inputs, bool &Result) {
...
bool MachineConstEvaluator::evaluateCMPrp(uint32_t Cmp,
                                          const RegisterSubReg &R1,
                                          uint64_t Props2,
                                          const CellMap &Inputs, bool &Result) {
  assert(Inputs.has(R1.Reg));
...
bool MachineConstEvaluator::evaluateZEXTr(const RegisterSubReg &R1,
...
bool MachineConstEvaluator::evaluateSEXTr(const RegisterSubReg &R1,
                                          unsigned Width, unsigned Bits,
                                          const CellMap &Inputs,
                                          LatticeCell &Result) {
  assert(Inputs.has(R1.Reg));
...
private:
  ...
  bool evaluateHexRSEQ32(RegisterSubReg RL, RegisterSubReg RH,
                         const CellMap &Inputs, LatticeCell &Result);
...
bool HexagonConstEvaluator::evaluateHexRSEQ32(RegisterSubReg RL,
                                              RegisterSubReg RH,
                                              const CellMap &Inputs,
                                              LatticeCell &Result) {
