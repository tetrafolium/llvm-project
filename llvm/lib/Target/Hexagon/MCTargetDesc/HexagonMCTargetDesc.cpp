...
unsigned llvm::HexagonGetLastSlot() {
    return HexagonItinerariesV5FU::SLOT3;
}
...
FeatureBitset Hexagon_MC::completeHVXFeatures(const FeatureBitset &S) {
    ...
    for (unsigned F : {
                ArchV67, ArchV66, ArchV65, ArchV62, ArchV60, ArchV55,
                ArchV5
            }) {
        if (!FB.test(F))
            continue;
    ...
    bool UseHvx = false;
    for (unsigned F : {
                ExtensionHVX, ExtensionHVX64B, ExtensionHVX128B
            }) {
        if (!FB.test(F))
            continue;
        UseHvx = true;
    ...
    for (unsigned F : {
                ExtensionHVXV60, ExtensionHVXV62, ExtensionHVXV65,
