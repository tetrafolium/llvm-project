...
ProgramStateRef MallocChecker::FreeMemAux(
  ...
  if (RsBase) {
    ...
    if ((RsBase->isReleased() || RsBase->isRelinquished()) &&
      ...
      // it, check that the call to free is proper.
...
bool MallocChecker::SummarizeRegion(raw_ostream &os, const MemRegion *MR) {
  switch (MR->getKind()) {
...
void MallocChecker::checkDeadSymbols(SymbolReaper &SymReaper,
  ...
  if (!Errors.empty()) {
    ...
    if (N) {
      for (SmallVectorImpl<SymbolRef>::iterator I = Errors.begin(),
                                                E = Errors.end();
           I != E; ++I) {
        HandleLeak(*I, N, C);
      }
