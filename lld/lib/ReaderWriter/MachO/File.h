...
public:
    ...
    MachOLinkingContext::Arch arch() const {
        return _arch;
    }
    void setArch(MachOLinkingContext::Arch arch) {
        _arch = arch;
    }

    MachOLinkingContext::OS OS() const {
        return _os;
    }
    void setOS(MachOLinkingContext::OS os) {
        _os = os;
    }
    ...
    void setSwiftVersion(uint32_t v) {
        _swiftVersion = v;
    }
    ...
    void setFlags(normalized::FileFlags v) {
        _flags = v;
    }
    ...
    std::unique_ptr<DebugInfo> takeDebugInfo() {
        return std::move(_debugInfo);
    }
...
public:
    ...
    void setCompatVersion(uint32_t version) {
        _compatVersion = version;
    }
...
public:
    ...
    std::error_code doParse() override {
        ...
        std::unique_ptr<llvm::MachO::InterfaceFile> interface {
            std::move(*result)
        };
