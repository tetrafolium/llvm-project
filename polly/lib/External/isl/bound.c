...
ISL_ARG_BOOL(struct bound_options, continue_on_error, '\0', "continue-on-error",
             0, NULL)
ISL_ARGS_END
...
static isl_stat verify_point(__isl_take isl_point *pnt, void *user) {
  ...
  bound = isl_pw_qpolynomial_fold_eval(isl_pw_qpolynomial_fold_copy(vpb->bound),
                                       isl_point_copy(pnt));
  ...
  if (!bounded)
    opt = isl_pw_qpolynomial_fold_eval(isl_pw_qpolynomial_fold_copy(pwf),
...
int main(int argc, char **argv) {
  ...
  if (obj.type == isl_obj_pw_qpolynomial)
    pwf = isl_pw_qpolynomial_fold_from_pw_qpolynomial(isl_fold_max, obj.v);
