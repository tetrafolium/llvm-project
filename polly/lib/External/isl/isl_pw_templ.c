...
__isl_give PW *FN(PW, alloc_size)(__isl_take isl_space *space OPT_TYPE_PARAM,
                                  int n) {
  isl_ctx *ctx;
  struct PW *pw;
 ...
 */
__isl_give PW *FN(PW, add_dup_piece)(__isl_take PW *pw, __isl_take isl_set *set,
                                     __isl_take EL *el) {
  isl_ctx *ctx;
  ...
  if (!OPT_EQUAL_TYPES(pw->, el->))
    isl_die(ctx, isl_error_invalid, "fold types don't match", goto error);
 ...
 */
__isl_give PW *FN(PW, add_piece)(__isl_take PW *pw, __isl_take isl_set *set,
                                 __isl_take EL *el) {
  isl_bool skip;
...
__isl_give PW *FN(PW, alloc)(OPT_TYPE_PARAM_FIRST __isl_take isl_set *set,
                             __isl_take EL *el) {
  PW *pw;
...
__isl_give PW *FN(PW, dup)(__isl_keep PW *pw) {
  ...
  dup = FN(PW, alloc_size)(isl_space_copy(pw->dim) OPT_TYPE_ARG(pw->), pw->n);
  if (!dup)
    return NULL;
...
static __isl_give PW *FN(FN(FN(PW, from), BASE),
                         type_base)(__isl_take EL *el OPT_TYPE_PARAM) {
 ...
 */
static __isl_give PW *FN(FN(FN(PW, from), BASE),
                         type)(__isl_take EL *el OPT_TYPE_PARAM) {
  isl_bool is_zero;
  isl_space *space;
...
__isl_give isl_id *FN(PW, get_tuple_id)(__isl_keep PW *pw,
...
__isl_give PW *FN(PW, realign_domain)(__isl_take PW *pw,
  ...
  for (i = 0; i < pw->n; ++i) {
    pw->p[i].set = isl_set_realign(pw->p[i].set, isl_reordering_copy(exp));
    if (!pw->p[i].set)
      goto error;
    pw->p[i].FIELD =
        FN(EL, realign_domain)(pw->p[i].FIELD, isl_reordering_copy(exp));
    if (!pw->p[i].FIELD)
      goto error;
  }
 ...
 */
__isl_give PW *FN(PW, align_params)(__isl_take PW *pw,
  ...
  if (!equal_params) {
    ...
    exp = isl_reordering_extend_space(exp, FN(PW, get_domain_space)(pw));
...
#include "isl_type_has_equal_space_bin_templ.c"
     ...
     */
    static __isl_give PW *
    FN(PW, union_add_)(__isl_take PW *pw1, __isl_take PW *pw2) {
  int i, j, n;
  struct PW *res;
  isl_ctx *ctx;
  isl_set *set;
  ...
  n = (pw1->n + 1) * (pw2->n + 1);
  res = FN(PW, alloc_size)(isl_space_copy(pw1->dim) OPT_TYPE_ARG(pw1->), n);
  ...
  for (i = 0; i < pw1->n; ++i) {
    ...
    for (j = 0; j < pw2->n; ++j) {
      ...
      sum = FN(EL, add_on_domain)(common, FN(EL, copy)(pw1->p[i].FIELD),
...
__isl_give PW *FN(PW, add_disjoint)(__isl_take PW *pw1, __isl_take PW *pw2) {
  ...
  for (i = 0; i < pw2->n; ++i)
    pw1 = FN(PW, add_piece)(pw1, isl_set_copy(pw2->p[i].set),
 ...
 */
static __isl_give PW *FN(PW, on_shared_domain_in)(
    __isl_take PW *pw1, __isl_take PW *pw2, __isl_take isl_space *space,
...
static __isl_give PW *FN(PW, on_shared_domain_in)(
  ...
  for (i = 0; i < pw1->n; ++i) {
    for (j = 0; j < pw2->n; ++j) {
      ...
      res_ij = fn(FN(EL, copy)(pw1->p[i].FIELD), FN(EL, copy)(pw2->p[j].FIELD));
...
/* This function is currently only used from isl_aff.c
 */
static __isl_give PW *FN(PW, on_shared_domain)(
...
__isl_give PW *FN(PW, from_range)(__isl_take PW *pw) {
  ...
  if (!isl_space_is_set(pw->dim))
    isl_die(FN(PW, get_ctx)(pw), isl_error_invalid, "not living in a set space",
            return FN(PW, free)(pw));
 ...
 */
static __isl_give PW *FN(PW, restrict_domain_aligned)(
 ...
 */
__isl_give PW *FN(PW,
                  intersect_domain_wrapped_domain)(__isl_take PW *pw,
...
static __isl_give PW *FN(PW, gist_last)(
    __isl_take PW *pw, __isl_take isl_set *context,
 ...
 */
static __isl_give PW *FN(PW, gist_aligned)(
    __isl_take PW *pw, __isl_take isl_set *context,
    __isl_give EL *(*fn_el)(__isl_take EL *el, __isl_take isl_set *set),
  ...
  for (i = pw->n - 1; i >= 0; --i) {
    ...
    set_i =
        isl_set_intersect(isl_set_copy(pw->p[i].set), isl_set_copy(context));
...
__isl_give PW *FN(PW, sort)(__isl_take PW *pw) {
  ...
  if (pw->n <= 1)
    return pw;
  if (isl_sort(pw->p, pw->n, sizeof(pw->p[0]), &FN(PW, sort_field_cmp), NULL) <
      0)
    return FN(PW, free)(pw);
  for (i = pw->n - 1; i >= 1; --i) {
...
isl_bool FN(PW, involves_dims)(__isl_keep PW *pw, enum isl_dim_type type,
  ...
  for (i = 0; i < pw->n; ++i) {
    isl_bool involves = FN(EL, involves_dims)(pw->p[i].FIELD, type, first, n);
    if (involves < 0 || involves)
      return involves;
    involves = isl_set_involves_dims(pw->p[i].set, set_type, first, n);
    if (involves < 0 || involves)
      return involves;
  }
  return isl_bool_false;
...
__isl_give PW *FN(PW, set_dim_name)(__isl_take PW *pw, enum isl_dim_type type,
  ...
  for (i = 0; i < pw->n; ++i) {
    pw->p[i].set = isl_set_set_dim_name(pw->p[i].set, set_type, pos, s);
    if (!pw->p[i].set)
      goto error;
...
__isl_give PW *FN(PW, drop_dims)(__isl_take PW *pw, enum isl_dim_type type,
                                 unsigned first, unsigned n) {
  int i;
  enum isl_dim_type set_type;
 ...
 */
__isl_give PW *FN(PW, project_out)(__isl_take PW *pw, enum isl_dim_type type,
                                   unsigned first, unsigned n) {
  int i;
  enum isl_dim_type set_type;
  ...
  if (!pw->dim)
    goto error;
  for (i = 0; i < pw->n; ++i) {
    pw->p[i].set = isl_set_project_out(pw->p[i].set, set_type, first, n);
    if (!pw->p[i].set)
      goto error;
    pw->p[i].FIELD = FN(EL, drop_dims)(pw->p[i].FIELD, type, first, n);
    if (!pw->p[i].FIELD)
...
__isl_give PW *FN(PW, fix_dim)(__isl_take PW *pw, enum isl_dim_type type,
                               unsigned pos, isl_int v) {
  int i;
 ...
 * to be equal to "v".
 */
__isl_give PW *FN(PW, fix_val)(__isl_take PW *pw, enum isl_dim_type type,
                               unsigned pos, __isl_take isl_val *v) {
  if (!v)
    return FN(PW, free)(pw);
  if (!isl_val_is_int(v))
    isl_die(FN(PW, get_ctx)(pw), isl_error_invalid, "expecting integer value",
            goto error);
...
__isl_give PW *FN(PW, split_dims)(__isl_take PW *pw, enum isl_dim_type type,
                                  unsigned first, unsigned n) {
  int i;
...
static isl_stat FN(PW, check_pos)(__isl_keep PW *pw, int pos) {
  if (!pw)
    return isl_stat_error;
  if (pos < 0 || pos >= pw->n)
    isl_die(FN(PW, get_ctx)(pw), isl_error_internal, "position out of bounds",
            return isl_stat_error);
  return isl_stat_ok;
}
 ...
 */
int FN(PW, find_dim_by_name)(__isl_keep PW *pw, enum isl_dim_type type,
                             const char *name) {
  if (!pw)
    return -1;
 ...
 * Return -1 if no such dimension can be found.
 */
static int FN(PW, find_dim_by_id)(__isl_keep PW *pw, enum isl_dim_type type,
 ...
 */
static __isl_give PW *
FN(PW, reset_space_and_domain)(__isl_take PW *pw, __isl_take isl_space *space,
  ...
  for (i = 0; i < pw->n; ++i) {
    pw->p[i].set = isl_set_reset_space(pw->p[i].set, isl_space_copy(domain));
    if (!pw->p[i].set)
      goto error;
    pw->p[i].FIELD = FN(EL, reset_space_and_domain)(
        pw->p[i].FIELD, isl_space_copy(space), isl_space_copy(domain));
    if (!pw->p[i].FIELD)
      goto error;
  }
 ...
 */
__isl_give PW *FN(PW, reset_tuple_id)(__isl_take PW *pw,
                                      enum isl_dim_type type) {
  isl_space *space;
...
__isl_give PW *FN(PW, set_dim_id)(__isl_take PW *pw, enum isl_dim_type type,
...
isl_stat FN(PW, foreach_piece)(__isl_keep PW *pw,
  ...
  for (i = 0; i < pw->n; ++i)
    if (fn(isl_set_copy(pw->p[i].set), FN(EL, copy)(pw->p[i].FIELD), user) < 0)
      return isl_stat_error;
 ...
 */
static __isl_give PW *FN(PW, negate_type)(__isl_take PW *pw) { return pw; }
#endif
...
__isl_give PW *FN(PW, scale_val)(__isl_take PW *pw, __isl_take isl_val *v) {
  ...
  for (i = 0; i < pw->n; ++i) {
    pw->p[i].FIELD = FN(EL, scale_val)(pw->p[i].FIELD, isl_val_copy(v));
    if (!pw->p[i].FIELD)
      goto error;
  }
 ...
 */
__isl_give PW *FN(PW, scale_down_val)(__isl_take PW *pw,
  ...
  if (!isl_val_is_rat(v))
    isl_die(isl_val_get_ctx(v), isl_error_invalid, "expecting rational factor",
            goto error);
  ...
  for (i = 0; i < pw->n; ++i) {
    pw->p[i].FIELD = FN(EL, scale_down_val)(pw->p[i].FIELD, isl_val_copy(v));
    if (!pw->p[i].FIELD)
      goto error;
  }
