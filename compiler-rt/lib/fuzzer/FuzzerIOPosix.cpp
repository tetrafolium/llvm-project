...
intptr_t GetHandleFromFd(int fd) { return static_cast<intptr_t>(fd); }
...
void RawPrint(const char *Str) { (void)write(2, Str, strlen(Str)); }

void MkDir(const std::string &Path) { mkdir(Path.c_str(), 0700); }

void RmDir(const std::string &Path) { rmdir(Path.c_str()); }
