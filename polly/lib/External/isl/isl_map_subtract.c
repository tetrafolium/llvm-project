 ...
 * coefficients.
 */
static void expand_constraint(isl_vec *v, unsigned dim, isl_int *c,
                              int *div_map, unsigned n_div) {
  int i;
...
static isl_stat tab_add_constraint(struct isl_tab *tab,
  ...
  if (c < 2 * bmap->n_eq) {
    if ((c % 2) != oppose)
      isl_seq_neg(bmap->eq[c / 2], bmap->eq[c / 2], 1 + bmap_total);
    if (oppose)
    ...
    c -= 2 * bmap->n_eq;
    if (oppose) {
      isl_seq_neg(bmap->ineq[c], bmap->ineq[c], 1 + bmap_total);
  ...
  for (i = 0; i < bmap->n_div; ++i) {
    ...
    for (j = 0; j < i; ++j)
      isl_int_add(vec->el[2 + dim + (*div_map)[j]],
                  vec->el[2 + dim + (*div_map)[j]], bmap->div[i][2 + dim + j]);
    for (j = 0; j < tab->bmap->n_div; ++j)
      if (isl_seq_eq(tab->bmap->div[j], vec->el, 2 + dim + tab->bmap->n_div))
        break;
    (*div_map)[i] = j;
    if (j == tab->bmap->n_div) {
 ...
 */
static int n_non_redundant(isl_ctx *ctx, struct isl_tab *tab, int offset,
                           int **index) {
  int i, n;
...
static isl_stat basic_map_collect_diff(__isl_take isl_basic_map *bmap,
  ...
  while (level >= 0) {
    ...
    if (init) {
      ...
      if (tab_add_divs(tab, map->p[level], &div_map[level]) < 0)
        goto error;
      offset = tab->n_con;
      snap[level] = isl_tab_snap(tab);
      if (tab_freeze_constraints(tab) < 0)
        goto error;
      if (tab_add_constraints(tab, map->p[level], div_map[level]) < 0)
        goto error;
      k[level] = 0;
      n[level] = 0;
      if (tab->empty) {
      ...
      modified = 1;
      n[level] = n_non_redundant(ctx, tab, offset, &index[level]);
      if (n[level] < 0)
        goto error;
      if (n[level] == 0) {
        level--;
      ...
      if (isl_tab_rollback(tab, snap[level]) < 0)
        goto error;
      if (tab_add_constraint(tab, map->p[level], div_map[level],
                             index[level][0], 1) < 0)
        goto error;
      level++;
      continue;
...
static isl_stat basic_map_subtract_add(struct isl_diff_collector *dc,
  ...
  sdc->diff = isl_map_union_disjoint(sdc->diff, isl_map_from_basic_map(bmap));
...
__isl_give isl_map *isl_map_subtract(__isl_take isl_map *map1,
  ...
  for (i = 0; i < map1->n; ++i) {
    struct isl_map *d;
    d = basic_map_subtract(isl_basic_map_copy(map1->p[i]), isl_map_copy(map2));
...
__isl_give isl_set *isl_set_subtract(__isl_take isl_set *set1,
  ...
  return set_from_map(isl_map_subtract(set_to_map(set1), set_to_map(set2)));
}
...
__isl_give isl_map *isl_map_subtract_domain(__isl_take isl_map *map,
  ...
  if (ok < 0)
    goto error;
  if (!ok)
    isl_die(isl_set_get_ctx(dom), isl_error_invalid, "incompatible spaces",
            goto error);
...
static isl_bool basic_map_diff_is_empty(__isl_keep isl_basic_map *bmap,
  ...
  r = basic_map_collect_diff(isl_basic_map_copy(bmap), isl_map_copy(map),
                             &edc.dc);
  if (!edc.empty)
 ...
 * as an isl_point.
 */
static __isl_give isl_point *
singleton_extract_point(__isl_keep isl_basic_map *bmap) {
  int j;
  isl_size dim;
  ...
  for (j = 0; j < bmap->n_eq; ++j) {
    int i = dim - 1 - j;
    isl_assert(bmap->ctx, isl_seq_first_non_zero(bmap->eq[j] + 1, i) == -1,
               goto error);
    isl_assert(bmap->ctx,
               ...
               goto error);
    isl_assert(bmap->ctx,
               isl_seq_first_non_zero(bmap->eq[j] + 1 + i + 1, dim - i - 1) ==
                   -1,
               goto error);
...
static isl_bool map_is_singleton_subset(__isl_keep isl_map *map1,
  ...
  if (map1->n != 1)
    isl_die(isl_map_get_ctx(map1), isl_error_invalid,
            "expecting single-disjunct input", return isl_bool_error);
...
isl_bool isl_map_is_subset(__isl_keep isl_map *map1, __isl_keep isl_map *map2) {
  return isl_map_align_params_map_map_and_test(map1, map2, &map_is_subset);
}
