...
HexagonEvaluator::composeWithSubRegIndex(const TargetRegisterClass &RC,
                                         unsigned Idx) const {
  if (Idx == 0)
    return RC;
...
#define im(i) MI.getOperand(i).getImm()
  ...
  auto rr0 = [this, Reg](const BT::RegisterCell &Val,
                         CellMapType &Outputs) -> bool {
  ...
  auto half = [this](const BT::RegisterCell &RC,
                     unsigned N) -> BT::RegisterCell {
  ...
  auto shuffle = [this](const BT::RegisterCell &Rs, const BT::RegisterCell &Rt,
    ...
    assert(Ws == Rt.width());
    RegisterCell RC =
        eXTR(Rt, I * BW, I * BW + BW).cat(eXTR(Rs, I * BW, I * BW + BW));
    I += 2;
    while (I * BW < Ws) {
  ...
  case S2_togglebit_i: {
    ...
    RC[BX] = RC[BX].is(0)
                 ? BT::BitValue::One
                 : RC[BX].is(1) ? BT::BitValue::Zero : BT::BitValue::self();
    return rr0(RC, Outputs);
  }
  ...
  case A4_bitspliti: {
    ...
    RegisterCell RZ =
        RegisterCell(W0).fill(BX, W1, Zero).fill(W1 + (W1 - BX), W0, Zero);
  ...
  case S2_extractup: {
    ...
    // with 0 bits.
    RegisterCell Pad =
        (Wd + Of > W0) ? rc(1).cat(eIMM(0, Wd + Of - W0)) : rc(1);
  ...
  case S2_packhl: {
    ...
    RegisterCell R2 = rc(2);
    RegisterCell RC =
        half(R2, 0).cat(half(R1, 0)).cat(half(R2, 1)).cat(half(R1, 1));
    return rr0(RC, Outputs);
  }
  case S2_shuffeb: {
