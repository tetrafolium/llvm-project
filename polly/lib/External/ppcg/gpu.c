...
remove_independences(struct gpu_prog *prog, struct gpu_array_info *array,
...
static int collect_array_info(struct gpu_prog *prog) {
  ...
  prog->array =
      isl_calloc_array(prog->ctx, struct gpu_array_info, prog->n_array);
  assert(prog->array);
  prog->n_array = 0;
 ...
 */
__isl_give isl_set *
gpu_array_positive_size_guard(struct gpu_array_info *array) {
  int i;
  isl_space *space;
  isl_set *guard;
...
static void read_grid_and_block_sizes(struct ppcg_kernel *kernel,
  ...
  set_used_sizes(gen, "block", kernel->id, kernel->block_dim, kernel->n_block);
  set_used_sizes(gen, "grid", kernel->id, kernel->grid_dim, kernel->n_grid);
}
 ...
 */
__isl_give isl_set *add_bounded_parameters(__isl_take isl_set *set, int *size,
  ...
  for (i = 0; i < len; ++i) {
    ...
    set = isl_set_upper_bound_si(set, isl_dim_param, nparam + i, size[i] - 1);
  }
...
add_bounded_parameters_dynamic(__isl_take isl_set *set,
 ...
 */
static __isl_give isl_union_map *
group_tagged_access_relation(struct gpu_array_ref_group *group) {
  int i;
...
static __isl_give isl_map *group_tile(struct gpu_array_ref_group *group) {
  ...
  local = isl_set_preimage_multi_aff(
      local, isl_multi_aff_copy(group->shared_tile->tiling));
...
compute_sched_to_copy(struct ppcg_kernel *kernel,
...
static void check_shared_memory_bound(struct ppcg_kernel *kernel) {
  ...
  for (i = 0; i < kernel->n_array; ++i) {
    ...
    for (j = 0; j < local->n_group; ++j) {
      ...
      group->shared_tile = gpu_array_tile_free(group->shared_tile);
    }
  }
...
extract_grid_size(struct ppcg_kernel *kernel,
  ...
  domain =
      isl_union_set_intersect(domain, isl_union_set_copy(kernel->block_filter));
...
static isl_stat extract_block_size(struct ppcg_kernel *kernel,
  ...
  for (i = 0; i < kernel->n_block; ++i) {
    ...
    block = isl_set_equate(block, isl_dim_param, pos, isl_dim_set, i);
  }
 ...
 */
static struct ppcg_kernel *
ppcg_kernel_create_local_arrays(struct ppcg_kernel *kernel,
                                struct gpu_prog *prog) {
  int i;
  isl_ctx *ctx;
  ...
  kernel->array =
      isl_calloc_array(ctx, struct gpu_local_array_info, prog->n_array);
  if (!kernel->array)
 ...
 */
static struct gpu_stmt *find_stmt(struct gpu_prog *prog,
 ...
 */
static struct gpu_array_ref_group *
find_ref_group(struct gpu_local_array_info *local,
               struct gpu_stmt_access *access) {
  int i, j;
 ...
 */
static __isl_give isl_multi_pw_aff *
tile_outer(__isl_take isl_multi_pw_aff *index,
           __isl_take isl_multi_pw_aff *tiling) {
 ...
 */
static __isl_give isl_multi_pw_aff *
transform_index(__isl_take isl_multi_pw_aff *index, __isl_keep isl_id *ref_id,
                void *user) {
...
gpu_local_array_info_linearize_index(struct gpu_local_array_info *array,
...
create_domain_leaf(struct ppcg_kernel *kernel, __isl_take isl_ast_node *node,
  ...
  stmt->u.d.ref2expr =
      gen->build_ast_expr(stmt->u.d.stmt->stmt, build, &transform_index, &data,
                          &transform_expr, &data);
...
static __isl_give isl_ast_node *create_access_leaf(
  ...
  if (group->array->linearize)
    expr = gpu_local_array_info_linearize_index(group->local_array, expr);
  stmt->u.c.index = expr;
 ...
 */
static __isl_give isl_ast_node *
create_sync_leaf(struct ppcg_kernel *kernel, __isl_take isl_ast_node *node,
 ...
 */
static __isl_give isl_ast_node *
build_array_bounds(__isl_take isl_ast_node *node, struct gpu_prog *prog,
                   __isl_keep isl_ast_build *build) {
  int i;
...
static __isl_give isl_ast_node *at_domain(__isl_take isl_ast_node *node,
  ...
  if (!is_sync)
    isl_die(data->prog->ctx, isl_error_internal, "unknown statement type",
            return isl_ast_node_free(node));
 ...
 */
static __isl_give isl_union_map *
wrapped_reference_to_access(__isl_take isl_union_set *ref,
                            __isl_take isl_union_map *tagged) {
 ...
 */
static __isl_give isl_union_map *
remove_local_accesses(struct gpu_prog *prog, __isl_take isl_union_map *tagged,
 ...
 */
static __isl_give isl_union_map *
remove_local_accesses_group(struct ppcg_kernel *kernel,
                            struct gpu_array_ref_group *group,
                            __isl_take isl_union_map *access,
                            __isl_keep isl_union_map *prefix, int read) {
 ...
 */
static isl_stat
build_grid_and_local_array_sizes(struct ppcg_kernel *kernel,
                                 __isl_keep isl_ast_build *build) {
...
group_tagged_writes(struct gpu_array_ref_group *group) {
  int i;
  isl_space *space;
 ...
 */
static __isl_give isl_union_set *
collect_non_private_tagged_writes(struct ppcg_kernel *kernel) {
  isl_union_set *writes;
  int i, j;
...
construct_band_tiles_sizes(__isl_keep isl_schedule_node *node, int *tile_size) {
  isl_space *space;
...
snap_band_to_sizes(__isl_take isl_schedule_node *node, int *factor,
                   struct ppcg_options *options) {
  isl_multi_val *mv;
  ...
  if (options->scale_tile_loops)
    node = isl_schedule_node_band_scale(node, isl_multi_val_copy(mv));
 ...
 */
static __isl_give isl_schedule_node *
tile_band(__isl_take isl_schedule_node *node, __isl_take isl_multi_val *sizes) {
...
static __isl_give isl_set *extract_context(__isl_keep isl_schedule_node *node,
  ...
  context = isl_set_intersect_params(context, isl_set_copy(prog->context));
 ...
 */
static __isl_give isl_union_set *
accessed_by_domain(__isl_take isl_union_set *domain, struct gpu_prog *prog) {
  isl_union_map *access;
  isl_union_set *arrays;
 ...
 */
static __isl_give isl_schedule_node *
split_band(__isl_take isl_schedule_node *node, int n) {
  int dim;
 ...
 */
static __isl_give isl_schedule_node *
scale_band(__isl_take isl_schedule_node *node,
...
parameter_vector(__isl_take isl_space *space, __isl_keep isl_id_list *names) {
  int i, n;
  isl_local_space *ls;
  isl_multi_aff *ma;
 ...
 */
static __isl_give isl_union_set *
set_schedule_modulo(__isl_keep isl_schedule_node *node,
                    __isl_keep isl_id_list *names, int *size) {
  int n, n_zero;
  isl_space *space;
  isl_multi_aff *ma;
 ...
 */
static __isl_give isl_schedule_node *
insert_context(struct ppcg_kernel *kernel, __isl_take isl_schedule_node *node) {
  isl_set *context;

  context = isl_set_universe(isl_set_get_space(kernel->context));

  context = add_bounded_parameters_dynamic(context, kernel->grid_size,
                                           kernel->block_ids);
  context =
      add_bounded_parameters(context, kernel->block_dim, kernel->thread_ids);
 ...
 */
static __isl_give isl_schedule_node *
insert_guard(__isl_take isl_schedule_node *node, __isl_keep isl_set *context,
 ...
 */
static __isl_give isl_schedule_node *
unroll(__isl_take isl_schedule_node *node) {
 ...
 */
static __isl_give isl_schedule_node *
add_sync(struct ppcg_kernel *kernel, __isl_take isl_schedule_node *node) {
  int depth;
  int need_sync;
 ...
 */
static __isl_give isl_union_map *
anchored_non_local_accesses(struct ppcg_kernel *kernel,
                            struct gpu_array_ref_group *group,
 ...
 */
static __isl_give isl_multi_aff *
create_from_access(isl_ctx *ctx, struct gpu_array_ref_group *group, int read) {
  struct gpu_array_tile *tile;
  isl_space *space;
  isl_id *id;
 ...
 */
static __isl_give isl_schedule_node *
add_group_write_sync(__isl_take isl_schedule_node *node,
                     struct ppcg_kernel *kernel,
 ...
 */
static __isl_give isl_schedule_node *
add_copies_group_private(struct ppcg_kernel *kernel,
                         struct gpu_array_ref_group *group,
                         __isl_take isl_schedule_node *node, int read) {
  struct gpu_array_tile *tile;
  isl_union_map *access;
 ...
 */
static __isl_give isl_schedule_node *
add_copies_group_shared(struct ppcg_kernel *kernel,
                        struct gpu_array_ref_group *group,
                        __isl_take isl_schedule_node *node, int read) {
  struct gpu_array_tile *tile;
  isl_union_map *access;
  ...
  if (tile->n < kernel->n_block)
    ...
    skip = 0;
  filter = set_schedule_modulo(graft, kernel->thread_ids, kernel->block_dim);
  if (!kernel->options->wrap)
    graft =
        snap_band_to_sizes(graft, kernel->block_dim + skip, kernel->options);
  if (tile->n > kernel->n_block && kernel->n_block > 0)
    graft = isl_schedule_node_parent(graft);
  graft = isl_schedule_node_insert_filter(graft, filter);
 ...
 */
static __isl_give isl_schedule_node *
add_copies_group(struct ppcg_kernel *kernel, struct gpu_array_ref_group *group,
                 __isl_take isl_schedule_node *node, int read) {
  enum ppcg_group_access_type type;
 ...
 */
static __isl_give isl_schedule_node *
add_copies(struct ppcg_kernel *kernel, __isl_take isl_schedule_node *node) {
  int i, j;
 ...
 */
static __isl_give isl_schedule_node *
atomic(__isl_take isl_schedule_node *node) {
 ...
 */
static __isl_give isl_schedule_node *
atomic_ancestors(__isl_take isl_schedule_node *node) {
  int pos;
 ...
 */
static __isl_give isl_union_set *
compute_sync_writes(struct ppcg_kernel *kernel,
  ...
  equal = isl_union_map_apply_range(
      kernel_prefix, isl_union_map_reverse(isl_union_map_copy(kernel_prefix)));
  wrap = isl_union_map_wrap(equal);
  local = isl_union_map_intersect_domain(local, wrap);
  equal = isl_union_map_apply_range(
      thread_prefix, isl_union_map_reverse(isl_union_map_copy(thread_prefix)));
  wrap = isl_union_map_wrap(equal);
 ...
 */
static __isl_give isl_schedule_node *
group_statements(__isl_take isl_schedule_node *node, int kernel_id) {
  char buffer[20];
  isl_id *id;
...
gpu_create_kernel(struct gpu_gen *gen, __isl_take isl_schedule_node *node,
                  int scale, __isl_keep isl_multi_val *sizes) {
  struct ppcg_kernel *kernel;
  isl_id *id;
  ...
  kernel->block_ids =
      ppcg_scop_generate_names(gen->prog->scop, kernel->n_grid, "b");
  kernel->block_filter =
      set_schedule_modulo(node, kernel->block_ids, kernel->grid_dim);
  kernel->grid_size = extract_grid_size(kernel, isl_union_set_copy(domain));
  if (!kernel->options->wrap)
    node = snap_band_to_sizes(node, kernel->grid_dim, kernel->options);
  if (scale)
  ...
  if (!single_statement)
    node = isl_schedule_node_parent(node);
  node =
      insert_guard(node, kernel->context, kernel->grid_size, gen->prog->scop);
  node = gpu_tree_move_down_to_thread(node, kernel->core);
  node = isl_schedule_node_child(node, 0);
  node = split_band(node, kernel->n_block);
  kernel->thread_ids =
      ppcg_scop_generate_names(gen->prog->scop, kernel->n_block, "t");
  kernel->thread_filter =
      set_schedule_modulo(node, kernel->thread_ids, kernel->block_dim);
  ...
  node = isl_schedule_node_child(node, 0);
  if (!kernel->options->wrap)
    node = snap_band_to_sizes(node, kernel->block_dim, kernel->options);
  node = isl_schedule_node_insert_filter(
  ...
  contraction = isl_union_pw_multi_aff_copy(kernel->contraction);
  kernel->copy_schedule = isl_union_pw_multi_aff_pullback_union_pw_multi_aff(
 ...
 */
static __isl_give isl_schedule_node *
insert_empty_permutable_band(__isl_take isl_schedule_node *node) {
  isl_space *space;
 ...
 */
static __isl_give isl_schedule_node *
try_hybrid_tile(struct gpu_gen *gen, __isl_take isl_schedule_node *node) {
  int tile_len;
  int *tile_size;
  isl_bool ok;
 ...
 */
static __isl_give isl_schedule_node *
mark_outer_permutable(__isl_take isl_schedule_node *node, void *user) {
 ...
 */
static __isl_give isl_union_set *
get_non_parallel_subtree_filters(__isl_keep isl_schedule_node *node,
                                 int only_initial) {
  isl_space *space;
  isl_union_set *filter;
  int i, n;
 ...
 */
static __isl_give isl_union_set *
get_all_non_parallel_subtree_filters(__isl_keep isl_schedule_node *node) {
 ...
 */
static __isl_give isl_union_set *
get_initial_non_parallel_subtree_filters(__isl_keep isl_schedule_node *node) {
 ...
 */
static __isl_give isl_schedule_node *
declare_accessed_local_variables(__isl_take isl_schedule_node *node,
                                 struct gpu_prog *prog,
 ...
 */
static __isl_give isl_schedule_node *
isolate_permutable_subtrees(__isl_take isl_schedule_node *node,
                            struct gpu_prog *prog) {
  isl_union_set *filter;
 ...
 */
static __isl_give isl_union_map *
approximate_copy_out(__isl_take isl_union_map *copy, struct gpu_prog *prog) {
  int i;
  isl_union_map *res;
 ...
 */
static __isl_give isl_schedule_node *
mark_kernels(struct gpu_gen *gen, __isl_take isl_schedule_node *node) {
  return isl_schedule_node_map_descendant_bottom_up(
      node, &mark_outer_permutable, gen);
}
...
construct_schedule_constraints(struct gpu_prog *prog) {
  isl_union_set *domain;
  ...
  if (prog->scop->options->live_range_reordering) {
    sc = isl_schedule_constraints_set_conditional_validity(
        sc, isl_union_map_copy(prog->scop->tagged_dep_flow),
    ...
    coincidence = isl_union_map_subtract(
        coincidence, isl_union_map_copy(prog->scop->independence));
    coincidence =
        isl_union_map_union(coincidence, isl_union_map_copy(prog->array_order));
  } else {
 ...
 */
static __isl_give isl_schedule_node *
band_set_permutable(__isl_take isl_schedule_node *node,
 ...
 */
static __isl_give isl_union_map *
get_local_coincidence(__isl_keep isl_schedule_node *node,
                      __isl_keep isl_schedule_constraints *sc) {
  ...
  if (isl_schedule_node_get_schedule_depth(node) == 0) {
    ...
    domain = isl_union_set_preimage_union_pw_multi_aff(domain, contraction);
    coincidence =
        isl_union_map_intersect_domain(coincidence, isl_union_set_copy(domain));
    coincidence = isl_union_map_intersect_range(coincidence, domain);
    return coincidence;
  }

  prefix = isl_schedule_node_get_prefix_schedule_multi_union_pw_aff(node);
  prefix =
      isl_multi_union_pw_aff_pullback_union_pw_multi_aff(prefix, contraction);
 ...
 */
static __isl_give isl_schedule_node *
band_set_coincident(__isl_take isl_schedule_node *node,
                    __isl_keep isl_schedule_constraints *sc) {
  isl_union_map *coincidence;
  isl_union_pw_multi_aff *contraction;
  ...
  for (i = 0; i < n; ++i) {
    ...
    coincidence_i =
        isl_union_map_eq_at_multi_union_pw_aff(coincidence_i, partial_i);
    subset = isl_union_map_is_subset(coincidence, coincidence_i);
    isl_union_map_free(coincidence_i);

    if (subset < 0)
      break;
    node = isl_schedule_node_band_member_set_coincident(node, i, subset);
  }
  if (i < n)
    node = isl_schedule_node_free(node);
 ...
 */
static __isl_give isl_schedule_node *
set_band_properties(__isl_take isl_schedule_node *node, void *user) {
...
determine_properties_original_schedule(struct gpu_gen *gen) {
  isl_schedule *schedule;
  isl_schedule_constraints *sc;
...
__isl_give isl_schedule *get_schedule(struct gpu_gen *gen) {
  return ppcg_get_schedule(gen->ctx, gen->options, &compute_or_set_properties,
                           gen);
}
...
create_copy_filters(struct gpu_prog *prog, const char *prefix,
 ...
 */
static __isl_give isl_schedule_node *
insert_positive_size_guards(__isl_take isl_schedule_node *graft,
 ...
 */
static __isl_give isl_schedule_node *
create_copy_device(struct gpu_prog *prog, __isl_keep isl_schedule_node *node,
 ...
 */
static __isl_give isl_union_set *
extract_local_accesses(struct gpu_prog *prog,
                       __isl_keep isl_union_set *domain) {
  int i;
  isl_union_set *local;
 ...
 */
static __isl_give isl_union_set *
expand_and_tag(__isl_take isl_union_set *domain,
 ...
 */
static __isl_give isl_union_set *
add_previous_filters(__isl_take isl_union_set *filters,
                     __isl_keep isl_schedule_node *node) {
 ...
 */
static __isl_give isl_union_set *
add_next_filters(__isl_take isl_union_set *filters,
                 __isl_keep isl_schedule_node *node) {
  isl_schedule_node *sibling;
 ...
 */
static void
remove_external_flow(struct ppcg_may_persist_data *data,
                     __isl_take isl_union_set *domain,
 ...
 */
static __isl_give isl_union_set *
node_may_persist(__isl_keep isl_schedule_node *node, struct gpu_prog *prog) {
  struct ppcg_may_persist_data data;
  isl_union_pw_multi_aff *contraction;
  isl_union_set *domain;
 ...
 */
static __isl_give isl_schedule_node *
add_to_from_device(__isl_take isl_schedule_node *node,
                   __isl_take isl_union_set *domain,
 ...
 */
static __isl_give isl_schedule_node *
add_init_clear_device(__isl_take isl_schedule_node *node) {
  isl_ctx *ctx;
  isl_space *space;
  isl_union_set *domain;
  isl_schedule_node *graft;
...
__isl_give isl_schedule *map_to_device(struct gpu_gen *gen,
  ...
  contraction = isl_schedule_node_get_subtree_contraction(node);
  domain = isl_union_set_preimage_union_pw_multi_aff(
      domain, isl_union_pw_multi_aff_copy(contraction));
  prefix = isl_schedule_node_get_prefix_schedule_union_map(node);
  prefix =
      isl_union_map_preimage_domain_union_pw_multi_aff(prefix, contraction);
...
extract_single_tagged_access(__isl_take isl_union_map *tagged,
...
static __isl_give isl_printer *generate(__isl_take isl_printer *p,
  ...
  if (any_permutable < 0 || !any_permutable) {
    ...
    p = gen->print(p, gen->prog, gen->tree, &gen->types, gen->print_user);
...
generate_wrap(__isl_take isl_printer *p, struct ppcg_scop *scop, void *user) {
  struct gpu_gen *gen = user;
