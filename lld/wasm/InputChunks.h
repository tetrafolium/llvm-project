...
public:
    ...
    virtual uint32_t getInputSize() const {
        return getSize();
    };

    virtual void writeTo(uint8_t *sectionStart) const;

    ArrayRef<WasmRelocation> getRelocations() const {
        return relocations;
    }
    void setRelocations(ArrayRef<WasmRelocation> rs) {
        relocations = rs;
    }
...
protected:
    ...
    virtual uint64_t getTombstone() const {
        return 0;
    }
...
public:
    ...
    uint32_t getComdat() const override {
        return segment.Data.Comdat;
    }
...
public:
    ...
    uint32_t getComdat() const override {
        return function->Comdat;
    }
    uint32_t getFunctionInputOffset() const {
        return getInputSectionOffset();
    }
    uint32_t getFunctionCodeOffset() const {
        return function->CodeOffset;
    }
    ...
    uint32_t getInputSize() const override {
...
public:
    ...
    void setBody(ArrayRef<uint8_t> body_) {
        body = body_;
    }
...
protected:
    ...
    uint64_t getTombstone() const override {
