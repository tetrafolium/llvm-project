...
static bool exception_spec_can_catch(int64_t specIndex,
  ...
  while (true) {
    ...
    const __shim_type_info* catchType = get_shim_type_info(
        ttypeIndex, classInfo, ttypeEncoding, true, unwind_exception);
    void* tempPtr = adjustedPtr;
    if (catchType->can_catch(excpType, tempPtr))
      return false;
  }
...
struct scan_results {
  int64_t
      ttypeIndex; // > 0 catch handler, < 0 exception spec handler, == 0 a cleanup
  const uint8_t*
      actionRecord; // Currently unused.  Retained to ease future maintenance.
...
static void scan_eh_tab(scan_results& results, _Unwind_Action actions,
  ...
  const uint8_t* lpStart =
      (const uint8_t*)readEncodedPointer(&lsda, lpStartEncoding);
  if (lpStart == 0)
...
#ifdef __USING_SJLJ_EXCEPTIONS__
  (void)
      callSiteEncoding; // When using SjLj exceptions, callSiteEncoding is never used
#endif
    ...
    if (results.reason == _URC_HANDLER_FOUND) {
      ...
      if (native_exception) {
        ...
        exception_header->handlerSwitchValue =
            static_cast<int>(results.ttypeIndex);
        exception_header->actionRecord = results.actionRecord;
        exception_header->languageSpecificData = results.languageSpecificData;
        exception_header->catchTemp =
            reinterpret_cast<void*>(results.landingPad);
...
static void save_results_to_barrier_cache(_Unwind_Exception* unwind_exception,
  ...
  unwind_exception->barrier_cache.bitpattern[1] =
      (uint32_t)results.actionRecord;
  unwind_exception->barrier_cache.bitpattern[2] =
      (uint32_t)results.languageSpecificData;
...
static void
load_results_from_barrier_cache(scan_results& results,
  ...
  results.actionRecord =
      (const uint8_t*)unwind_exception->barrier_cache.bitpattern[1];
  results.languageSpecificData =
      (const uint8_t*)unwind_exception->barrier_cache.bitpattern[2];
...
__gxx_personality_v0(_Unwind_State state, _Unwind_Exception* unwind_exception,
                     _Unwind_Context* context) {
  ...
  case _US_VIRTUAL_UNWIND_FRAME:
    ...
    scan_eh_tab(results, _UA_SEARCH_PHASE, native_exception, unwind_exception,
                context);
    if (results.reason == _URC_HANDLER_FOUND) {
  ...
  case _US_UNWIND_FRAME_STARTING:
    ...
    if (unwind_exception->barrier_cache.sp == _Unwind_GetGR(context, REG_SP)) {
      ...
      if (native_exception) {
        ...
        scan_eh_tab(
            results,
            static_cast<_Unwind_Action>(_UA_CLEANUP_PHASE | _UA_HANDLER_FRAME),
            native_exception, unwind_exception, context);
        if (results.reason !=
            _URC_HANDLER_FOUND) // phase1 search should guarantee to find one
          call_terminate(native_exception, unwind_exception);
      }
    ...
    //  phase 1 reported no catching-handlers.
    // Search for a (non-catching) cleanup
    scan_eh_tab(results, _UA_CLEANUP_PHASE, native_exception, unwind_exception,
                context);
    if (results.reason == _URC_HANDLER_FOUND) {
...
__attribute__((noreturn)) _LIBCXXABI_FUNC_VIS void
  ...
  try {
    ...
    if (native_old_exception) {
      ...
      if (native_new_exception &&
        ...
        adjustedPtr =
            __getExceptionClass(&new_exception_header->unwindHeader) ==
