...
public:
    ...
    uint64_t getID() const {
        return MI->getOperand(IDPos).getImm();
    }
...
public:
    ...
    bool isAnyReg() const {
        return (getCallingConv() == CallingConv::AnyReg);
    }
    bool hasDef() const {
        return HasDef;
    }
    ...
    uint64_t getID() const {
        return getMetaOper(IDPos).getImm();
    }
...
public:
    ...
    unsigned getCCIdx() const {
        return getVarIdx() + CCOffset;
    }

    /// Get index of Flags operand.
    unsigned getFlagsIdx() const {
        return getVarIdx() + FlagsOffset;
    }
    ...
    uint64_t getID() const {
        return MI->getOperand(NumDefs + IDPos).getImm();
    }
    ...
    uint64_t getFlags() const {
        return MI->getOperand(getFlagsIdx()).getImm();
    }
