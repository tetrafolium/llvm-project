...
struct SourceLocationFindingVisitor {
    ...
    void Post(const CharBlock &at) {
        source.ExtendToCover(at);
    }
...
struct SetExprHelper {
    ...
    void Set(const parser::Expr &x) {
        Set(x.typedExpr);
    }
    void Set(const parser::Variable &x) {
        Set(x.typedExpr);
    }
    void Set(const parser::DataStmtConstant &x) {
        Set(x.typedExpr);
    }
