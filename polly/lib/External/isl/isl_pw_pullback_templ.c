...
#include "isl_align_params_templ.c"
     ...
     */
    __isl_give PW *
    FN(PW, pullback_multi_aff)(__isl_take PW *pw,
  ...
  space = isl_space_join(isl_multi_aff_get_space(ma), FN(PW, get_space)(pw));

  for (i = 0; i < pw->n; ++i) {
    pw->p[i].set =
        isl_set_preimage_multi_aff(pw->p[i].set, isl_multi_aff_copy(ma));
    if (!pw->p[i].set)
      goto error;
    pw->p[i].FIELD =
        FN(EL, pullback_multi_aff)(pw->p[i].FIELD, isl_multi_aff_copy(ma));
    if (!pw->p[i].FIELD)
      goto error;
  }
...
FN(PW, pullback_pw_multi_aff_aligned)(__isl_take PW *pw,
  ...
  if (pma->n == 0) {
    isl_space *space;
    space =
        isl_space_join(isl_pw_multi_aff_get_space(pma), FN(PW, get_space)(pw));
  ...
  for (i = 1; i < pma->n; ++i) {
    ...
    res_i = FN(PW, intersect_domain)(res_i, isl_set_copy(pma->p[i].set));
