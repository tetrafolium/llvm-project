...
static cl::opt<unsigned> CountThreshold(
    "hexagon-cext-threshold", cl::init(3), cl::Hidden, cl::ZeroOrMore,
    cl::desc("Minimum number of extenders to trigger replacement"));

static cl::opt<unsigned>
    ReplaceLimit("hexagon-cext-limit", cl::init(0), cl::Hidden, cl::ZeroOrMore,
...
struct OffsetRange {
  ...
  bool operator!=(const OffsetRange &R) const { return !operator==(R); }
  bool operator<(const OffsetRange &R) const {
    if (Min != R.Min)
      return Min < R.Min;
    if (Max != R.Max)
...
struct RangeTree {
  ...
  void add(const OffsetRange &R) { Root = add(Root, R); }
  void erase(const Node *N) {
    Root = remove(Root, N);
    delete N;
  }
  void order(SmallVectorImpl<Node *> &Seq) const { order(Root, Seq); }
...
private:
  ...
  unsigned height(Node *N) { return N != nullptr ? N->Height : 0; }
  Node *update(Node *N) {
    assert(N != nullptr);
...
struct Loc {
  ...
  Loc(MachineBasicBlock *B, MachineBasicBlock::iterator It) : Block(B), At(It) {
    if (B->end() == It) {
      Pos = -1;
    } else {
...
private:
  ...
  struct ExtExpr {
    ...
    bool trivial() const { return Rs.Reg == 0; }
    bool operator==(const ExtExpr &Ex) const {
      return Rs == Ex.Rs && S == Ex.S && Neg == Ex.Neg;
    }
    bool operator!=(const ExtExpr &Ex) const { return !operator==(Ex); }
    bool operator<(const ExtExpr &Ex) const {
      if (Rs != Ex.Rs)
        return Rs < Ex.Rs;
      if (S != Ex.S)
  ...
  struct ExtDesc {
    ...
    MachineOperand &getOp() { return UseMI->getOperand(OpNum); }
    const MachineOperand &getOp() const { return UseMI->getOperand(OpNum); }
  };
  ...
  struct ExtRoot {
    ...
    bool operator!=(const ExtRoot &ER) const { return !operator==(ER); }
    bool operator<(const ExtRoot &ER) const;
  };
  ...
  struct ExtValue : public ExtRoot {
    ...
    bool operator!=(const ExtValue &EV) const { return !operator==(EV); }
...
raw_ostream &operator<<(raw_ostream &OS, const PrintInit &P) {
  OS << '[' << P.ExtI.first << ", " << PrintExpr(P.ExtI.second, P.HRI) << ']';
  return OS;
}
...
unsigned HCE::getRegOffOpcode(unsigned ExtOpc) const {
  ...
  if (D.mayLoad() || D.mayStore()) {
    ...
    case HexagonII::BaseLongOffset:
      ...
      case S4_pstorerbt_abs:
        return S2_pstorerbt_io;
      case S4_pstorerbf_abs:
        return S2_pstorerbf_io;
      case S4_pstorerht_abs:
        return S2_pstorerht_io;
      case S4_pstorerhf_abs:
        return S2_pstorerhf_io;
      case S4_pstorerft_abs:
        return S2_pstorerft_io;
      case S4_pstorerff_abs:
        return S2_pstorerff_io;
      case S4_pstorerit_abs:
        return S2_pstorerit_io;
      case S4_pstorerif_abs:
        return S2_pstorerif_io;
      case S4_pstorerdt_abs:
        return S2_pstorerdt_io;
      case S4_pstorerdf_abs:
...
unsigned HCE::getDirectRegReplacement(unsigned ExtOpc) const {
  ...
  case Hexagon::M2_accii:
    return Hexagon::M2_acci; // T -> T
  /* No M2_macsin */
  case Hexagon::M2_macsip:
    return Hexagon::M2_maci; // T -> T
  ...
  case Hexagon::M2_naccii:
    return Hexagon::M2_nacci; // T -> T
  case Hexagon::M4_mpyri_addi:
    return Hexagon::M4_mpyri_addr;
  case Hexagon::M4_mpyri_addr:
    return Hexagon::M4_mpyrr_addr; // _ -> T
  case Hexagon::M4_mpyrr_addi:
    return Hexagon::M4_mpyrr_addr; // _ -> T
  case Hexagon::S4_addaddi:
    return Hexagon::M2_acci; // _ -> T
  case Hexagon::S4_addi_asl_ri:
    return Hexagon::S2_asl_i_r_acc; // T -> T
  case Hexagon::S4_addi_lsr_ri:
    return Hexagon::S2_lsr_i_r_acc; // T -> T
  case Hexagon::S4_andi_asl_ri:
    return Hexagon::S2_asl_i_r_and; // T -> T
  case Hexagon::S4_andi_lsr_ri:
    return Hexagon::S2_lsr_i_r_and; // T -> T
  case Hexagon::S4_ori_asl_ri:
    return Hexagon::S2_asl_i_r_or; // T -> T
  case Hexagon::S4_ori_lsr_ri:
    return Hexagon::S2_lsr_i_r_or; // T -> T
  case Hexagon::S4_subaddi:
    return Hexagon::M2_subacc; // _ -> T
  case Hexagon::S4_subi_asl_ri:
    return Hexagon::S2_asl_i_r_nac; // T -> T
  case Hexagon::S4_subi_lsr_ri:
    return Hexagon::S2_lsr_i_r_nac; // T -> T
...
OffsetRange HCE::getOffsetRange(Register Rb, const MachineInstr &MI) const {
  ...
  if (!HII->getBaseAndOffsetPosition(MI, BaseP, OffP) ||
      Rb != Register(MI.getOperand(BaseP)) || !MI.getOperand(OffP).isImm())
    return OffsetRange::zero();

  uint64_t F =
      (D.TSFlags >> HexagonII::MemAccessSizePos) & HexagonII::MemAccesSizeMask;
...
void HCE::assignInits(const ExtRoot &ER, unsigned Begin, unsigned End,
  ...
  for (std::pair<const ExtenderInit, IndexList> &P : IMap) {
    ...
    auto SameValue = [&EV, this, IsStack](unsigned I) {
      const ExtDesc &ED = Extenders[I];
      return ED.Expr.Rs.isSlot() == IsStack && ExtValue(ED).Offset == EV.Offset;
    };
...
HCE::Register HCE::insertInitializer(Loc DefL, const ExtenderInit &ExtI) {
  ...
  if (Ex.Rs.isSlot()) {
    ...
    if (Ex.trivial()) {
      // DefR = ##EV
      InitI =
          BuildMI(MBB, At, dl, HII->get(Hexagon::A2_tfrsi), DefR).add(ExtOp);
    } else if (Ex.S == 0) {
      if (Ex.Neg) {
        // DefR = sub(##EV,Rb)
      ...
      if (HST->useCompound()) {
        unsigned NewOpc =
            Ex.Neg ? Hexagon::S4_subi_asl_ri : Hexagon::S4_addi_asl_ri;
...
bool HCE::replaceInstrExact(const ExtDesc &ED, Register ExtR) {
  ...
  if (ExtOpc == Hexagon::C2_cmpgei || ExtOpc == Hexagon::C2_cmpgeui) {
    unsigned NewOpc =
        ExtOpc == Hexagon::C2_cmpgei ? Hexagon::C2_cmplt : Hexagon::C2_cmpltu;
...
bool HCE::replaceInstrExpr(const ExtDesc &ED, const ExtenderInit &ExtI,
  ...
  if (ExtOpc == Hexagon::A2_tfrsi) {
    ...
    if (!Uses.contains(-Diff))
      dbgs() << "Diff: " << -Diff << " out of range " << Uses << " for " << MI;
...
#endif
    unsigned NewOpc =
        ExtOpc == Hexagon::M2_naccii ? Hexagon::A2_sub : Hexagon::A2_add;
    BuildMI(MBB, At, dl, HII->get(NewOpc))
        .add(MI.getOperand(0))
        .add(MI.getOperand(1))
        .add(MachineOperand(ExtR));
