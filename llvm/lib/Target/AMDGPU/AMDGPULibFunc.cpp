...
struct ManglingRule {
    ...
    int maxLeadIndex() const {
        return (std::max)(Lead[0], Lead[1]);
    }
    int getNumLeads() const {
...
public:
    ...
    static unsigned getNumArgs(ID Id) {
        return Table[toIndex(Id)].NumArgs;
    }
    static StringRef getName(ID Id) {
...
AMDGPULibFunc::Param ParamIterator::getNextParam() {
    ...
    default:
        ...
        default:
            llvm_unreachable("Unhandeled param rule");
        }
    }
    ++Index;
    return P;
...
bool ItaniumParamParser::parseItaniumParam(StringRef& param,
    ...
    if (isDigit(TC)) {
        ...
        case 'd':
            ...
            break;
        case 'D':
            if (!eatTerm(param, 'h')) return false;
...
static Type* getIntrinsicParamType(
    ...
    case AMDGPULibFunc::IMG2D:
    case AMDGPULibFunc::IMG3D:
        T = StructType::create(C,"ocl_image")->getPointerTo();
        break;
    case AMDGPULibFunc::SAMPLER:
        T = StructType::create(C,"ocl_sampler")->getPointerTo();
        break;
    case AMDGPULibFunc::EVENT:
        T = StructType::create(C,"ocl_event")->getPointerTo();
        break;
    default:
        llvm_unreachable("Unhandeled param type");
        return nullptr;
