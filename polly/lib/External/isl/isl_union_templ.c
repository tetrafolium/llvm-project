...
static __isl_give UNION *
FN(UNION, alloc)(__isl_take isl_space *space OPT_TYPE_PARAM, int size) {
  UNION *u;
...
__isl_give UNION *FN(FN(UNION, ZERO),
                     space)(__isl_take isl_space *space OPT_TYPE_PARAM) {
...
static __isl_give UNION *FN(UNION, add_part_generic)(__isl_take UNION *u,
  ...
  if (!entry->data)
    ...
    if (disjoint && FN(UNION, check_disjoint_domain)(entry->data, part) < 0)
      goto error;
    entry->data = FN(PART, union_add_)(entry->data, FN(PART, copy)(part));
    if (!entry->data)
      goto error;
    empty = FN(PART, IS_ZERO)(part);
    if (empty < 0)
 ...
 */
static __isl_give UNION *
FN(UNION, alloc_same_size_on_space)(__isl_keep UNION *u,
                                    __isl_take isl_space *space) {
  if (!u)
    goto error;
 ...
 */
static __isl_give UNION *
FN(UNION, transform_space)(__isl_take UNION *u, __isl_take isl_space *space,
                           S(UNION, transform_control) * control) {
  if (!space)
    return FN(UNION, free)(u);
  control->space = space;
 ...
 */
static __isl_give PART *FN(UNION, copy_part)(__isl_take PART *part,
                                             void *user) {
  return part;
}
...
__isl_null UNION *FN(UNION, free)(__isl_take UNION *u) {
  ...
  isl_hash_table_foreach(u->space->ctx, &u->table, &FN(UNION, free_u_entry),
                         NULL);
...
static __isl_give PART *FN(UNION, align_entry)(__isl_take PART *part,
                                               void *user) {
 ...
 */
static __isl_give UNION *
FN(UNION, realign_domain)(__isl_take UNION *u, __isl_take isl_reordering *r) {
  S(UNION, transform_control)
  control = {
...
static isl_stat FN(UNION, match_bin_entry)(__isl_take PART *part, void *user) {
  ...
  part2 = entry2->data;
  if (!isl_space_tuple_is_equal(part->dim, isl_dim_out, part2->dim,
                                isl_dim_out))
    isl_die(FN(UNION, get_ctx)(data->u2), isl_error_invalid,
            "entries should have the same range space", goto error);
 ...
 */
static __isl_give UNION *FN(UNION, match_bin_op)(
    __isl_take UNION *u1, __isl_take UNION *u2,
    __isl_give PART *(*fn)(__isl_take PART *, __isl_take PART *))
...
static __isl_give UNION *
FN(UNION,
   match_bin_op)(__isl_take UNION *u1, __isl_take UNION *u2,
                 __isl_give PART *(*fn)(__isl_take PART *, __isl_take PART *)) {
  ...
  if (FN(FN(UNION, foreach), BASE)(u1, &FN(UNION, match_bin_entry), &data) < 0)
    goto error;
...
static __isl_give UNION *
FN(UNION,
   any_set_op)(__isl_take UNION *u, __isl_take isl_set *set,
               __isl_give PW *(*fn)(__isl_take PW *, __isl_take isl_set *)) {
 ...
 */
static isl_stat FN(UNION, match_domain_entry)(__isl_take PART *part,
                                              void *user) {
  ...
  entry2 = isl_hash_table_find(data->uset->dim->ctx, &data->uset->table, hash,
                               &FN(UNION, set_has_space), space, 0);
  isl_space_free(space);
 ...
 */
static __isl_give UNION *
FN(UNION, match_domain_op)(__isl_take UNION *u, __isl_take isl_union_set *uset,
                           S(UNION, match_domain_control) * control) {
  ...
  if (FN(FN(UNION, foreach), BASE)(u, &FN(UNION, match_domain_entry), &data) <
      0)
    goto error;
 ...
 */
__isl_give UNION *
FN(UNION, intersect_domain_union_set)(__isl_take UNION *u,
  ...
  if (isl_union_set_is_params(uset))
    return FN(UNION, intersect_params)(u, isl_set_from_union_set(uset));
...
__isl_give UNION *FN(UNION,
                     intersect_domain_space)(__isl_take UNION *u,
                                             __isl_take isl_space *space) {
  S(UNION, transform_control)
  control = {
 ...
 */
__isl_give UNION *
FN(UNION, intersect_domain_wrapped_domain)(__isl_take UNION *u,
                                           __isl_take isl_union_set *uset) {
  S(UNION, match_domain_control)
  control = {
 ...
 */
__isl_give UNION *
FN(UNION, intersect_domain_wrapped_range)(__isl_take UNION *u,
                                          __isl_take isl_union_set *uset) {
  S(UNION, match_domain_control)
  control = {
      .filter = &FN(PART, domain_is_wrapping),
 ...
 */
__isl_give UNION *
FN(UNION, subtract_domain_union_set)(__isl_take UNION *u,
                                     __isl_take isl_union_set *uset) {
  S(UNION, transform_control)
  control = {
 ...
 */
__isl_give UNION *FN(UNION,
                     subtract_domain_space)(__isl_take UNION *u,
                                            __isl_take isl_space *space) {
  S(UNION, transform_control)
  control = {
...
S(UNION, plain_is_equal_data) { UNION *u2; };
...
isl_bool FN(UNION, plain_is_equal)(__isl_keep UNION *u1, __isl_keep UNION *u2) {
  ...
  data.u2 = u2;
  is_equal =
      FN(FN(UNION, every), BASE)(u1, &FN(UNION, plain_is_equal_el), &data);
...
__isl_give UNION *FN(UNION, drop_dims)(__isl_take UNION *u,
  ...
  if (type != isl_dim_param)
    isl_die(FN(UNION, get_ctx)(u), isl_error_invalid,
            "can only project out parameters", return FN(UNION, free)(u));
...
__isl_give UNION *FN(UNION, set_dim_name)(__isl_take UNION *u,
  ...
  if (type != isl_dim_param)
    isl_die(FN(UNION, get_ctx)(u), isl_error_invalid,
            "can only set parameter names", return FN(UNION, free)(u));
