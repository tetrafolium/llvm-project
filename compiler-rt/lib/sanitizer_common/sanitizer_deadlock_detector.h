...
public:
    ...
    uptr getNumLocks() const {
        return n_all_locks_;
    }
    uptr getLock(uptr idx) const {
        return all_locks_with_contexts_[idx].lock;
    }
...
public:
    ...
    uptr getData(uptr node) const {
        return data_[nodeToIndex(node)];
    }
...
private:
    ...
    uptr nodeToIndexUnchecked(uptr node) const {
        return node % size();
    }
    ...
    uptr nodeToEpoch(uptr node) const {
