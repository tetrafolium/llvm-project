...
struct AttributePosition
    ...
    ArrayRef<unsigned> getIndex() const final {
...
struct ResultPosition
    ...
    explicit ResultPosition(const KeyTy &key) : Base(key) {
        parent = key.first;
    }

    /// Returns the index of this position.
    ArrayRef<unsigned> getIndex() const final {
        return key.first->getIndex();
    }
...
public:
    ...
    Position *getRoot() {
        return OperationPosition::getRoot(uniquer);
    }
    ...
    Position *getType(Position *p) {
        return TypePosition::get(uniquer, p);
    }
