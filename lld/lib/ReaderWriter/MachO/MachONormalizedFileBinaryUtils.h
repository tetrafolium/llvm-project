...
public:
  ...
  void append_byte(uint8_t b) { _ostream << b; }
  void append_uleb128(uint64_t value) { llvm::encodeULEB128(value, _ostream); }
  ...
  void append_sleb128(int64_t value) { llvm::encodeSLEB128(value, _ostream); }
  ...
  size_t size() { return _ostream.tell(); }
  const uint8_t *bytes() {
...
template <typename T> static inline uint16_t read16(const T *loc, bool isBig) {
  assert((uint64_t)loc % alignof(T) == 0 && "invalid pointer alignment");
  return isBig ? read16be(loc) : read16le(loc);
}

template <typename T> static inline uint32_t read32(const T *loc, bool isBig) {
  assert((uint64_t)loc % alignof(T) == 0 && "invalid pointer alignment");
  return isBig ? read32be(loc) : read32le(loc);
}

template <typename T> static inline uint64_t read64(const T *loc, bool isBig) {
  assert((uint64_t)loc % alignof(T) == 0 && "invalid pointer alignment");
...
inline uint32_t bitFieldExtract(uint32_t value, bool isBigEndianBigField,
                                uint8_t firstBit, uint8_t bitCount) {
  const uint32_t mask = ((1 << bitCount) - 1);
  const uint8_t shift =
      isBigEndianBigField ? (32 - firstBit - bitCount) : firstBit;
  return (value >> shift) & mask;
}

inline void bitFieldSet(uint32_t &bits, bool isBigEndianBigField,
                        uint32_t newBits, uint8_t firstBit, uint8_t bitCount) {
  const uint32_t mask = ((1 << bitCount) - 1);
...
inline Relocation unpackRelocation(const llvm::MachO::any_relocation_info &r,
  ...
  if (r0 & llvm::MachO::R_SCATTERED) {
    ...
    result.type = (RelocationInfoType)bitFieldExtract(r0, true, 4, 4);
    ...
    result.type = (RelocationInfoType)bitFieldExtract(r1, isBigEndian, 28, 4);
