...
ExprResult Sema::DefaultArgumentPromotion(Expr *E) {
  ...
  if (BTy && (BTy->getKind() == BuiltinType::Half ||
    ...
    if (getLangOpts().OpenCL && !getOpenCLOptions().isEnabled("cl_khr_fp64")) {
...
static bool unsupportedTypeConversion(const Sema &S, QualType LHSType,
  ...
  */
  ...
  QualType LHSElemType = LHSComplex ? LHSComplex->getElementType() : LHSType;
  QualType RHSElemType = RHSComplex ? RHSComplex->getElementType() : RHSType;
...
ExprResult Sema::ActOnGenericSelectionExpr(SourceLocation KeyLoc,
  ...
  ExprResult ER = CreateGenericSelectionExpr(
      KeyLoc, DefaultLoc, RParenLoc, ControllingExpr,
      llvm::makeArrayRef(Types, NumAssocs), ArgExprs);
  delete[] Types;
  return ER;
}
...
ExprResult Sema::ActOnPredefinedExpr(SourceLocation Loc, tok::TokenKind Kind) {
  ...
  case tok::kw___func__:
    IK = PredefinedExpr::Func;
    break; // [C99 6.4.2.2]
  case tok::kw___FUNCTION__:
    IK = PredefinedExpr::Function;
    break;
  case tok::kw___FUNCDNAME__:
    IK = PredefinedExpr::FuncDName;
    break; // [MS]
  case tok::kw___FUNCSIG__:
    IK = PredefinedExpr::FuncSig;
    break; // [MS]
  case tok::kw_L__FUNCTION__:
    IK = PredefinedExpr::LFunction;
    break; // [MS]
  case tok::kw_L__FUNCSIG__:
    IK = PredefinedExpr::LFuncSig;
    break; // [MS]
  case tok::kw___PRETTY_FUNCTION__:
    IK = PredefinedExpr::PrettyFunction;
    break;
  }
...
static ExprResult convertVector(Expr *E, QualType ElementType, Sema &S) {
  ...
  QualType NewVecTy = S.Context.getVectorType(
      ElementType, VecTy->getNumElements(), VecTy->getVectorKind());
    ...
    if (Overflow) {
      if (RightSideIntValue < 64)
        ...
        S.Diag(Loc, diag::warn_xor_used_as_pow)
            << ExprStr << XorValue.toString(10, true);
      else
        return;
    } else {
...
QualType Sema::CheckAssignmentOperands(Expr *LHSExpr, ExprResult &RHS,
  ...
  // operand.
  return (getLangOpts().CPlusPlus ? LHSType : LHSType.getUnqualifiedType());
}
...
static bool captureInCapturedRegion(
    ...
    const bool RefersToCapturedVariable, Sema &S, bool Invalid) {
...
static bool captureInLambda(LambdaScopeInfo *LSI, VarDecl *Var,
                            SourceLocation Loc, const bool BuildAndDiagnose,
                            QualType &CaptureType, QualType &DeclRefType,
...
bool Sema::tryCaptureVariable(
  ...
  do {
    // Only block literals, captured statements, and lambda expressions can
    // capture; other scopes don't work.
    DeclContext *ParentDC = getParentOfCapturingContextOrNull(
...
static void MarkExprReferenced(Sema &SemaRef, SourceLocation Loc, Decl *D,
                               Expr *E, bool MightBeOdrUse) {
