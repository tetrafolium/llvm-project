...
#define CHECK_UNPOISONED_STRING_OF_LEN(x, len, n) \
  CHECK_UNPOISONED((x), common_flags()->strict_string_checks ? (len) + 1 : (n))
...
#define INTERCEPTOR_STRFTIME_BODY(char_type, ret_type, func, s, ...) \
  ...
  if (s)                                                             \
    __msan_unpoison(s, sizeof(char_type) * (res + 1));               \
  return res;
...
INTERCEPTOR(int, mbtowc, wchar_t *dest, const char *src, SIZE_T n) {
  ...
  if (res != -1 && dest)
    __msan_unpoison(dest, sizeof(wchar_t));
  return res;
}
...
INTERCEPTOR(int, prlimit, int pid, int resource, void *new_rlimit,
  ...
  if (!res)
    __msan_unpoison(old_rlimit, __sanitizer::struct_rlimit_sz);
  return res;
}
...
INTERCEPTOR(int, prlimit64, int pid, int resource, void *new_rlimit,
  ...
  if (!res)
    __msan_unpoison(old_rlimit, __sanitizer::struct_rlimit64_sz);
  return res;
}
...
// with MSan.
#if SANITIZER_LINUX
INTERCEPTOR(int, openpty, int *apparent, int *aworker, char *name,
            const void *termp, const void *winp) {
  ENSURE_MSAN_INITED();
  InterceptorScope interceptor_scope;
  int res = REAL(openpty)(apparent, aworker, name, termp, winp);
  if (!res) {
    __msan_unpoison(apparent, sizeof(*apparent));
...
// with MSan.
#if SANITIZER_LINUX
INTERCEPTOR(int, forkpty, int *apparent, char *name, const void *termp,
            const void *winp) {
  ENSURE_MSAN_INITED();
  InterceptorScope interceptor_scope;
  int res = REAL(forkpty)(apparent, name, termp, winp);
  if (res != -1)
    __msan_unpoison(apparent, sizeof(*apparent));
  return res;
}
...
#define COMMON_INTERCEPTOR_UNPOISON_PARAM(count) UnpoisonParam(count)
...
void *__msan_memmove(void *dest, const void *src, SIZE_T n) {
  ...
  if (msan_init_is_running)
    return REAL(memmove)(dest, src, n);
  ENSURE_MSAN_INITED();
  GET_STORE_STACK_TRACE;
