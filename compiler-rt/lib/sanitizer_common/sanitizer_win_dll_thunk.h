...
#define INTERCEPT_OR_DIE(main_function, dll_function)                       \
  static int intercept_##dll_function() {                                   \
    return __sanitizer::dllThunkIntercept(main_function,                    \
                                          (__sanitizer::uptr)dll_function); \
  }                                                                         \
  __pragma(section(".DLLTH$M", long, read)) __declspec(                     \
      allocate(".DLLTH$M")) int (*__dll_thunk_##dll_function)() =           \
...
#define INTERCEPT_WHEN_POSSIBLE(main_function, default_function, dll_function) \
  static int intercept_##dll_function() {                                      \
    return __sanitizer::dllThunkInterceptWhenPossible(                         \
        main_function, default_function, (__sanitizer::uptr)dll_function);     \
  }                                                                            \
  __pragma(section(".DLLTH$M", long, read)) __declspec(                        \
      allocate(".DLLTH$M")) int (*__dll_thunk_##dll_function)() =              \
      intercept_##dll_function;
