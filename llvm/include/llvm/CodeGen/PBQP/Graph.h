...
///
template <typename SolverT> class Graph : public GraphBase {
private:
...
public:
  ...
  template <typename OtherVectorT> NodeId addNode(OtherVectorT Costs) {
  ...
  const Vector &getNodeCosts(NodeId NId) const { return *getNodeCostsPtr(NId); }
