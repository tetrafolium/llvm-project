...
static const EnumEntry<unsigned> MachOSymbolTypes[] = {{"Undef", 0x0},
                                                       {"Abs", 0x2},
                                                       {"Indirect", 0xA},
                                                       {"PreboundUndef", 0xC},
                                                       {"Section", 0xE}};
...
void MachODumper::printStackMap() const {
  ...
  if (Obj->isLittleEndian())
    prettyPrintStackMap(W,
                        StackMapParser<support::little>(StackMapContentsArray));
  else
    prettyPrintStackMap(W, StackMapParser<support::big>(StackMapContentsArray));
}
