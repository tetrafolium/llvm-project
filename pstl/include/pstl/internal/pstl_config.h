...
// The version is XYYZ, where X is major, YY is minor, and Z is patch (i.e.
// X.YY.Z)
...
#define _PSTL_HIDE_FROM_ABI_PUSH                                               \
  _Pragma("clang attribute push(__attribute__((internal_linkage)), "           \
...
// note that when ICC or Clang is in use, _PSTL_GCC_VERSION might not fully
...
#define _PSTL_PRAGMA_SIMD_SCAN(PRM)                                            \
  _PSTL_PRAGMA(omp simd reduction(inscan, PRM))
#define _PSTL_PRAGMA_SIMD_INCLUSIVE_SCAN(PRM)                                  \
  _PSTL_PRAGMA(omp scan inclusive(PRM))
#define _PSTL_PRAGMA_SIMD_EXCLUSIVE_SCAN(PRM)                                  \
  _PSTL_PRAGMA(omp scan exclusive(PRM))
#else
#define _PSTL_PRAGMA_SIMD_SCAN(PRM)
#define _PSTL_PRAGMA_SIMD_INCLUSIVE_SCAN(PRM)
#define _PSTL_PRAGMA_SIMD_EXCLUSIVE_SCAN(PRM)
...
// Should be defined to 1 for environments with a vendor implementation of C++17
// execution policies
...
#define _PSTL_CPP14_MAKE_REVERSE_ITERATOR_PRESENT                              \
  (_MSC_VER >= 1900 || __cplusplus >= 201402L ||                               \
...
#define _PSTL_PRAGMA_SIMD_ORDERED_MONOTONIC(PRM)                               \
  _PSTL_PRAGMA(omp ordered simd monotonic(PRM))
#define _PSTL_PRAGMA_SIMD_ORDERED_MONOTONIC_2ARGS(PRM1, PRM2)                  \
  _PSTL_PRAGMA(omp ordered simd monotonic(PRM1, PRM2))
#else
#define _PSTL_PRAGMA_SIMD_ORDERED_MONOTONIC(PRM)
#define _PSTL_PRAGMA_SIMD_ORDERED_MONOTONIC_2ARGS(PRM1, PRM2)
#endif
...
#define _PSTL_PRAGMA_DECLARE_REDUCTION(NAME, OP)                               \
  _PSTL_PRAGMA(omp declare reduction(NAME:OP                                   \
                                     : omp_out(omp_in))                        \
...
#if _MSC_VER ||                                                                \
    __INTEL_COMPILER // the preprocessors don't type a message location
#define _PSTL_PRAGMA_LOCATION                                                  \
  __FILE__ ":" _PSTL_STRING(__LINE__) ": [Parallel STL message]: "
#else
