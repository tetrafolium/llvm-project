...
isl_schedule_node_from_domain(__isl_take isl_union_set *domain) {
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_from_extension(__isl_take isl_union_map *extension) {
  isl_ctx *ctx;
  isl_schedule *schedule;
...
isl_schedule_node_peek_leaf(__isl_keep isl_schedule_node *node) {
...
isl_schedule_node_get_leaf(__isl_keep isl_schedule_node *node) {
...
isl_schedule_node_get_type(__isl_keep isl_schedule_node *node) {
 ...
 */
enum isl_schedule_node_type
isl_schedule_node_get_parent_type(__isl_keep isl_schedule_node *node) {
  isl_size n;
  int pos;
  int has_parent;
...
isl_schedule_node_get_schedule(__isl_keep isl_schedule_node *node) {
  if (!node)
    return NULL;
...
isl_schedule_node_dup(__isl_keep isl_schedule_node *node) {
  if (!node)
    return NULL;

  return isl_schedule_node_alloc(
      isl_schedule_copy(node->schedule), isl_schedule_tree_copy(node->tree),
      isl_schedule_tree_list_copy(node->ancestors), node->child_pos);
}
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_cow(__isl_take isl_schedule_node *node) {
  if (!node)
    return NULL;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_copy(__isl_keep isl_schedule_node *node) {
  if (!node)
    return NULL;
 ...
 */
__isl_null isl_schedule_node *
isl_schedule_node_free(__isl_take isl_schedule_node *node) {
  if (!node)
    return NULL;
  if (--node->ref > 0)
    return NULL;
 ...
 */
isl_size
isl_schedule_node_get_schedule_depth(__isl_keep isl_schedule_node *node) {
  int i;
  isl_size n;
  int depth = 0;
  ...
  for (i = n - 1; i >= 0; --i) {
    ...
    tree = isl_schedule_tree_list_get_schedule_tree(node->ancestors, i);
    if (!tree)
      return isl_size_error;
    n = 0;
...
static isl_stat
collect_filter_prefix(__isl_keep isl_schedule_tree_list *list, int n,
...
static isl_stat collect_filter_prefix_expansion(
    __isl_take isl_schedule_tree *tree, __isl_keep isl_schedule_tree_list *list,
  ...
  if (data->collect_prefix) {
    ...
    prefix = isl_multi_union_pw_aff_pullback_union_pw_multi_aff(
        prefix, isl_union_pw_multi_aff_copy(c));
    data->prefix =
        isl_multi_union_pw_aff_flat_range_product(prefix, data->prefix);
  }
  filter = contracted.filter;
  if (data->universe_domain)
    filter = isl_union_set_preimage_union_pw_multi_aff(
        filter, isl_union_pw_multi_aff_copy(c));
  else
 ...
 */
static isl_stat collect_filter_prefix_init(
  ...
  case isl_schedule_node_band:
    ...
    if (data->collect_prefix) {
      ...
      mupa = isl_multi_union_pw_aff_reset_tuple_id(mupa, isl_dim_set);
 ...
 */
static isl_stat collect_filter_prefix_update(
    __isl_keep isl_schedule_tree *tree,
    struct isl_schedule_node_get_filter_prefix_data *data) {
  enum isl_schedule_node_type type;
  isl_multi_union_pw_aff *mupa;
  ...
  case isl_schedule_node_band:
    ...
    mupa = isl_schedule_tree_band_get_partial_schedule(tree);
    data->prefix =
        isl_multi_union_pw_aff_flat_range_product(mupa, data->prefix);
    if (!data->prefix)
      return isl_stat_error;
    break;
  case isl_schedule_node_filter:
...
collect_filter_prefix(__isl_keep isl_schedule_tree_list *list, int n,
  ...
  for (i = n - 1; i >= 0; --i) {
    ...
    if (type == isl_schedule_node_expansion)
      return collect_filter_prefix_expansion(tree, list, i, data);
    if (type == isl_schedule_node_extension && data->universe_domain &&
        !data->collect_prefix)
      return collect_universe_domain_extension(tree, list, i, data);
    if (!data->initialized)
...
isl_schedule_node_get_prefix_schedule_multi_union_pw_aff(
  ...
  data.prefix =
      isl_multi_union_pw_aff_intersect_domain(data.prefix, data.filter);
...
isl_schedule_node_get_prefix_schedule_union_pw_multi_aff(
  ...
  if (data.prefix && dim == 0) {
    ...
    prefix = isl_union_pw_multi_aff_from_multi_union_pw_aff(data.prefix);
    prefix = isl_union_pw_multi_aff_intersect_domain(prefix, data.filter);
  }
...
isl_schedule_node_get_domain(__isl_keep isl_schedule_node *node) {
  isl_size n;
  struct isl_schedule_node_get_filter_prefix_data data;
 ...
 */
__isl_give isl_union_set *
isl_schedule_node_get_universe_domain(__isl_keep isl_schedule_node *node) {
  isl_size n;
  struct isl_schedule_node_get_filter_prefix_data data;
 ...
 */
isl_size
isl_schedule_node_get_child_position(__isl_keep isl_schedule_node *node) {
  isl_size n;
  isl_bool has_parent;
 ...
 */
isl_bool
isl_schedule_node_has_previous_sibling(__isl_keep isl_schedule_node *node) {
  isl_size n;
  isl_bool has_parent;
 ...
 */
isl_bool
isl_schedule_node_has_next_sibling(__isl_keep isl_schedule_node *node) {
  isl_size n, n_child;
  isl_bool has_parent;
  isl_schedule_tree *tree;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_ancestor(__isl_take isl_schedule_node *node, int generation) {
  isl_size n;
  isl_schedule_tree *tree;
  ...
  if (generation < 0 || generation > n)
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
            "generation out of bounds", return isl_schedule_node_free(node));
  node = isl_schedule_node_cow(node);
  if (!node)
    return NULL;

  tree =
      isl_schedule_tree_list_get_schedule_tree(node->ancestors, n - generation);
  isl_schedule_tree_free(node->tree);
  node->tree = tree;
  node->ancestors =
      isl_schedule_tree_list_drop(node->ancestors, n - generation, generation);
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_parent(__isl_take isl_schedule_node *node) {
  if (!node)
    return NULL;
  if (!isl_schedule_node_has_parent(node))
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
            "node has no parent", return isl_schedule_node_free(node));
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_root(__isl_take isl_schedule_node *node) {
  isl_size n;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_child(__isl_take isl_schedule_node *node, int pos) {
  isl_size n;
  isl_ctx *ctx;
  isl_schedule_tree *tree;
  int *child_pos;
  ...
  if (!isl_schedule_node_has_children(node))
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
            "node has no children", return isl_schedule_node_free(node));
  ...
  node->ancestors = isl_schedule_tree_list_add(
      node->ancestors, isl_schedule_tree_copy(node->tree));
  tree = node->tree;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_first_child(__isl_take isl_schedule_node *node) {
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_previous_sibling(__isl_take isl_schedule_node *node) {
  isl_size n;
  ...
  if (n < 0)
    return isl_schedule_node_free(node);
  parent = isl_schedule_tree_list_get_schedule_tree(node->ancestors, n - 1);
  if (!parent)
    return isl_schedule_node_free(node);
  node->child_pos[n - 1]--;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_next_sibling(__isl_take isl_schedule_node *node) {
  isl_size n;
  isl_schedule_tree *parent, *tree;
  ...
  if (!isl_schedule_node_has_next_sibling(node))
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
            "node has no next sibling", return isl_schedule_node_free(node));
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_get_child(__isl_keep isl_schedule_node *node, int pos) {
...
static __isl_give isl_schedule_node *traverse(
    ...
    __isl_give isl_schedule_node *(*enter)(__isl_take isl_schedule_node *node,
                                           void *user),
    __isl_give isl_schedule_node *(*leave)(__isl_take isl_schedule_node *node,
                                           void *user),
    void *user) {
  isl_size depth;
  isl_size node_depth;
  ...
  do {
    ...
    while ((node_depth = isl_schedule_node_get_tree_depth(node)) > depth &&
...
preorder_enter(__isl_take isl_schedule_node *node, void *user) {
 ...
 */
static __isl_give isl_schedule_node *
preorder_leave(__isl_take isl_schedule_node *node, void *user) {
  return node;
}
...
isl_bool isl_schedule_node_every_descendant(
  ...
  r = isl_schedule_node_foreach_descendant_top_down(node, &call_every, &data);
  if (r >= 0)
    return isl_bool_true;
  if (data.failed)
    return isl_bool_false;
 ...
 */
static __isl_give isl_schedule_node *
postorder_enter(__isl_take isl_schedule_node *node, void *user) {
 ...
 */
static __isl_give isl_schedule_node *
postorder_leave(__isl_take isl_schedule_node *node, void *user) {
 ...
 */
isl_bool
isl_schedule_node_is_subtree_anchored(__isl_keep isl_schedule_node *node) {
  if (!node)
    return isl_bool_error;
 ...
 */
isl_bool
isl_schedule_node_band_member_get_coincident(__isl_keep isl_schedule_node *node,
                                             int pos) {
  if (!node)
    return isl_bool_error;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_band_member_set_coincident(__isl_take isl_schedule_node *node,
  ...
  tree = isl_schedule_tree_band_member_set_coincident(tree, pos, coincident);
 ...
 */
isl_bool
isl_schedule_node_band_get_permutable(__isl_keep isl_schedule_node *node) {
  if (!node)
    return isl_bool_error;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_band_set_permutable(__isl_take isl_schedule_node *node,
                                      int permutable) {
  isl_schedule_tree *tree;
...
isl_schedule_node_band_get_space(__isl_keep isl_schedule_node *node) {
  if (!node)
    return NULL;
...
__isl_give isl_schedule_node *isl_schedule_node_band_member_set_ast_loop_type(
    __isl_take isl_schedule_node *node, int pos, enum isl_ast_loop_type type) {
  isl_schedule_tree *tree;
...
enum isl_ast_loop_type isl_schedule_node_band_member_get_isolate_ast_loop_type(
  ...
  return isl_schedule_tree_band_member_get_isolate_ast_loop_type(node->tree,
                                                                 pos);
}
...
isl_schedule_node_band_member_set_isolate_ast_loop_type(
  ...
  tree = isl_schedule_tree_copy(node->tree);
  tree =
      isl_schedule_tree_band_member_set_isolate_ast_loop_type(tree, pos, type);
  return isl_schedule_node_graft_tree(node, tree);
}
...
static int check_space_multi_val(__isl_keep isl_schedule_node *node,
  ...
  equal =
      isl_space_tuple_is_equal(node_space, isl_dim_set, mv_space, isl_dim_set);
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_band_scale(__isl_take isl_schedule_node *node,
  ...
  if (anchored)
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
            "cannot scale band node with anchored subtree", goto error);
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_band_scale_down(__isl_take isl_schedule_node *node,
                                  __isl_take isl_multi_val *mv) {
  isl_schedule_tree *tree;
  int anchored;
  ...
  if (anchored)
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
            "cannot scale down band node with anchored subtree", goto error);
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_band_mod(__isl_take isl_schedule_node *node,
                           __isl_take isl_multi_val *mv) {
  isl_schedule_tree *tree;
  isl_bool anchored;
 ...
 */
static isl_stat
check_space_multi_union_pw_aff(__isl_keep isl_schedule_node *node,
  ...
  equal = isl_space_tuple_is_equal(node_space, isl_dim_set, mupa_space,
                                   isl_dim_set);
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_band_shift(__isl_take isl_schedule_node *node,
  ...
  if (anchored)
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
            "cannot shift band node with anchored subtree", goto error);
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_band_tile(__isl_take isl_schedule_node *node,
  ...
  if (anchored)
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
            "cannot tile band node with anchored subtree", goto error);
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_band_sink(__isl_take isl_schedule_node *node) {
  enum isl_schedule_node_type type;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_band_split(__isl_take isl_schedule_node *node, int pos) {
  isl_size depth;
  isl_schedule_tree *tree;
 ...
 */
__isl_give isl_set *
isl_schedule_node_context_get_context(__isl_keep isl_schedule_node *node) {
  if (!node)
    return NULL;
 ...
 */
__isl_give isl_union_set *
isl_schedule_node_domain_get_domain(__isl_keep isl_schedule_node *node) {
  if (!node)
    return NULL;
...
isl_schedule_node_expansion_get_expansion(__isl_keep isl_schedule_node *node) {
  if (!node)
    return NULL;
...
isl_schedule_node_expansion_set_contraction_and_expansion(
  ...
  tree = isl_schedule_tree_copy(node->tree);
  tree = isl_schedule_tree_expansion_set_contraction_and_expansion(
 ...
 */
__isl_give isl_union_map *
isl_schedule_node_extension_get_extension(__isl_keep isl_schedule_node *node) {
  if (!node)
    return NULL;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_extension_set_extension(__isl_take isl_schedule_node *node,
 ...
 */
__isl_give isl_union_set *
isl_schedule_node_filter_get_filter(__isl_keep isl_schedule_node *node) {
  if (!node)
    return NULL;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_filter_set_filter(__isl_take isl_schedule_node *node,
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_filter_intersect_filter(__isl_take isl_schedule_node *node,
                                          __isl_take isl_union_set *filter) {
 ...
 */
__isl_give isl_set *
isl_schedule_node_guard_get_guard(__isl_keep isl_schedule_node *node) {
  if (!node)
    return NULL;
 ...
 */
__isl_give isl_id *
isl_schedule_node_mark_get_id(__isl_keep isl_schedule_node *node) {
  if (!node)
    return NULL;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_sequence_splice(__isl_take isl_schedule_node *node, int pos,
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_sequence_splice_child(__isl_take isl_schedule_node *node,
                                        int pos) {
  int i;
  isl_size n;
  isl_union_set *filter;
  ...
  if (isl_schedule_node_get_type(node) != isl_schedule_node_sequence)
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
            "not a sequence node", return isl_schedule_node_free(node));
  ...
  for (i = 0; i < n; ++i) {
    ...
    child = isl_schedule_node_filter_intersect_filter(
        child, isl_union_set_copy(filter));
...
static __isl_give isl_schedule_node *update_ancestors(
  ...
  for (i = n - 1; i >= 0; --i) {
    ...
    parent = isl_schedule_tree_list_get_schedule_tree(node->ancestors, i);
    parent = isl_schedule_tree_replace_child(parent, node->child_pos[i], tree);
    if (fn) {
...
__isl_give isl_schedule_node *isl_schedule_node_insert_partial_schedule(
  ...
  if (anchored)
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
            "cannot insert band node in anchored subtree", goto error);
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_insert_context(__isl_take isl_schedule_node *node,
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_insert_filter(__isl_take isl_schedule_node *node,
                                __isl_take isl_union_set *filter) {
  isl_schedule_tree *tree;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_insert_guard(__isl_take isl_schedule_node *node,
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_insert_mark(__isl_take isl_schedule_node *node,
 ...
 */
static __isl_give isl_schedule_node *
isl_schedule_node_insert_children(__isl_take isl_schedule_node *node,
                                  enum isl_schedule_node_type type,
  ...
  for (i = 0; i < n; ++i) {
    ...
    node_i = isl_schedule_node_gist(node_i, isl_union_set_copy(filter));
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_insert_sequence(__isl_take isl_schedule_node *node,
                                  __isl_take isl_union_set_list *filters) {
  return isl_schedule_node_insert_children(node, isl_schedule_node_sequence,
                                           filters);
}
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_insert_set(__isl_take isl_schedule_node *node,
                             __isl_take isl_union_set_list *filters) {
  return isl_schedule_node_insert_children(node, isl_schedule_node_set,
                                           filters);
}
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_cut(__isl_take isl_schedule_node *node) {
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_delete(__isl_take isl_schedule_node *node) {
  isl_size n, depth;
  isl_schedule_tree *tree;
  enum isl_schedule_node_type type;
  ...
  if (depth == 0)
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
            "cannot delete root node", return isl_schedule_node_free(node));
  if (n != 1)
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
 ...
 */
static isl_bool
locally_covered_by_domain(__isl_keep isl_union_set *domain,
  ...
  test =
      isl_union_set_intersect(test, isl_union_set_copy(data->domain_universe));
...
group_band(__isl_take isl_schedule_tree *tree,
           __isl_keep isl_schedule_node *pos,
           struct isl_schedule_group_data *data) {
  isl_union_set *domain;
  isl_multi_aff *ma;
  ...
  if (has_id < 0) {
    ...
    mupa = isl_multi_union_pw_aff_set_tuple_id(mupa, isl_dim_set, id);
  }
...
union_set_drop_extra_params(__isl_take isl_union_set *uset,
                            __isl_keep isl_space *space, int n) {
  isl_size n2;
 ...
 */
static __isl_give isl_schedule_tree *
group_context(__isl_take isl_schedule_tree *tree,
              __isl_keep isl_schedule_node *pos,
              struct isl_schedule_group_data *data) {
  isl_space *space;
  isl_union_set *domain;
  ...
  involves =
      isl_union_map_involves_dims(data->expansion, isl_dim_param, n1, n2 - n1);
  if (involves < 0)
    return isl_schedule_tree_free(tree);
  if (involves)
  ...
  data->expansion =
      isl_union_map_project_out(data->expansion, isl_dim_param, n1, n2 - n1);
  ...
  if (n2 < 0)
    data->contraction = isl_union_pw_multi_aff_free(data->contraction);
  data->contraction = isl_union_pw_multi_aff_drop_dims(
      data->contraction, isl_dim_param, n1, n2 - n1);
  ...
  data->sched = isl_multi_aff_align_params(data->sched, isl_space_copy(space));
  n2 = isl_multi_aff_dim(data->sched, isl_dim_param);
  if (n2 < 0)
    data->sched = isl_multi_aff_free(data->sched);
  data->sched =
      isl_multi_aff_drop_dims(data->sched, isl_dim_param, n1, n2 - n1);
 ...
 */
static __isl_give isl_schedule_tree *
group_domain(__isl_take isl_schedule_tree *tree,
             __isl_keep isl_schedule_node *pos,
             struct isl_schedule_group_data *data) {
  isl_union_set *domain;
  isl_bool is_subset;
  ...
  if (!is_subset)
    isl_die(isl_schedule_tree_get_ctx(tree), isl_error_internal,
            ...
            return isl_schedule_tree_free(tree));
  domain = isl_schedule_tree_domain_get_domain(tree);
  domain = isl_union_set_subtract(domain, isl_union_set_copy(data->domain));
 ...
 */
static __isl_give isl_schedule_tree *
group_expansion(__isl_take isl_schedule_tree *tree,
                __isl_keep isl_schedule_node *pos,
                struct isl_schedule_group_data *data) {
  isl_union_set *domain;
  ...
  expansion = isl_schedule_tree_expansion_get_expansion(tree);
  expansion =
      isl_union_map_subtract_range(expansion, isl_union_set_copy(data->domain));
  ...
  umap = isl_union_map_apply_range(isl_union_map_copy(data->expansion), umap);
  upma = isl_union_pw_multi_aff_from_union_map(umap);
  contraction = isl_schedule_tree_expansion_get_contraction(tree);
  contraction = isl_union_pw_multi_aff_intersect_domain(contraction, domain);
  domain = isl_union_pw_multi_aff_domain(isl_union_pw_multi_aff_copy(upma));
 ...
 */
static __isl_give isl_schedule_tree *
group_ancestor(__isl_take isl_schedule_tree *tree,
               __isl_keep isl_schedule_node *pos, void *user) {
  ...
  case isl_schedule_node_filter:
    ...
    if (is_covered)
      domain = isl_union_set_subtract(
          domain, isl_union_set_copy(data->domain_universe));
    domain =
        isl_union_set_union(domain, isl_union_set_copy(data->group_universe));
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_group(__isl_take isl_schedule_node *node,
  ...
  if (data.dim == 0) {
    ...
    prefix = isl_multi_union_pw_aff_set_tuple_id(prefix, isl_dim_set, group_id);
    space = isl_multi_union_pw_aff_get_space(prefix);
    contraction = isl_union_pw_multi_aff_from_multi_union_pw_aff(prefix);
    univ = isl_union_set_universe(isl_union_set_copy(domain));
    contraction = isl_union_pw_multi_aff_intersect_domain(contraction, univ);
  ...
  disjoint =
      isl_union_set_is_disjoint(data.domain_universe, data.group_universe);
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_band_gist(__isl_take isl_schedule_node *node,
 ...
 */
static __isl_give isl_schedule_node *
gist_enter_expansion(__isl_take isl_schedule_node *node,
  ...
  contraction =
      isl_union_pw_multi_aff_gist(contraction, isl_union_set_copy(inner));
  ...
  node = isl_schedule_node_expansion_set_contraction_and_expansion(
      node, contraction, expansion);
 ...
 */
static __isl_give isl_schedule_node *
gist_leave_expansion(__isl_take isl_schedule_node *node,
                     struct isl_node_gist_data *data) {
  isl_size n;
  isl_bool identity;
  isl_union_map *expansion;
 ...
 */
static __isl_give isl_schedule_node *
gist_enter_extension(__isl_take isl_schedule_node *node,
                     struct isl_node_gist_data *data) {
  isl_size n;
 ...
 */
static __isl_give isl_schedule_node *
gist_enter(__isl_take isl_schedule_node *node, void *user) {
  ...
  do {
    ...
    node =
        isl_schedule_node_filter_set_filter(node, isl_union_set_copy(filter));
 ...
 */
static __isl_give isl_schedule_node *
gist_leave(__isl_take isl_schedule_node *node, void *user) {
  struct isl_node_gist_data *data = user;
  isl_schedule_tree *tree;
  int i;
  isl_size n;
  ...
  case isl_schedule_node_sequence:
    ...
    if (n == 1) {
      ...
      filter = isl_union_set_list_get_union_set(data->filters, 0);
...
isl_schedule_node_domain_intersect_domain(__isl_take isl_schedule_node *node,
  ...
  tree = isl_schedule_tree_domain_set_domain(tree, isl_union_set_copy(uset));
  node = isl_schedule_node_graft_tree(node, tree);
 ...
 */
static __isl_give isl_schedule_node *
subtree_expansion_enter(__isl_take isl_schedule_node *node, void *user) {
  ...
  do {
    ...
    case isl_schedule_node_filter:
      ...
      if (n < 0)
        data->expansions = isl_union_map_list_free(data->expansions);
      inner = isl_union_map_list_get_union_map(data->expansions, n - 1);
      inner = isl_union_map_intersect_range(inner, filter);
      data->expansions = isl_union_map_list_add(data->expansions, inner);
      break;
    case isl_schedule_node_expansion:
      n = isl_union_map_list_n_union_map(data->expansions);
      if (n < 0)
      ...
      data->expansions =
          isl_union_map_list_set_union_map(data->expansions, n - 1, inner);
      break;
    case isl_schedule_node_band:
    case isl_schedule_node_context:
    case isl_schedule_node_domain:
 ...
 */
static __isl_give isl_schedule_node *
subtree_expansion_leave(__isl_take isl_schedule_node *node, void *user) {
  struct isl_subtree_expansion_data *data = user;
  isl_size n;
  isl_union_map *inner;
  enum isl_schedule_node_type type;
  ...
  case isl_schedule_node_filter:
    ...
    if (n < 0)
      data->expansions = isl_union_map_list_free(data->expansions);
    data->expansions = isl_union_map_list_drop(data->expansions, n - 1, 1);
    break;
  case isl_schedule_node_leaf:
    n = isl_union_map_list_n_union_map(data->expansions);
    if (n < 0)
 ...
 */
__isl_give isl_union_map *
isl_schedule_node_get_subtree_expansion(__isl_keep isl_schedule_node *node) {
  ...
  node = isl_schedule_node_copy(node);
  node =
      traverse(node, &subtree_expansion_enter, &subtree_expansion_leave, &data);
  if (!node)
 ...
 */
static __isl_give isl_schedule_node *
subtree_contraction_enter(__isl_take isl_schedule_node *node, void *user) {
  ...
  do {
    ...
    case isl_schedule_node_filter:
      ...
      if (type != isl_schedule_node_set && type != isl_schedule_node_sequence)
        break;
      filter = isl_schedule_node_filter_get_filter(node);
      n = isl_union_pw_multi_aff_list_n_union_pw_multi_aff(data->contractions);
      if (n < 0)
        data->contractions =
      ...
      inner = isl_union_pw_multi_aff_intersect_domain(inner, filter);
      data->contractions =
          isl_union_pw_multi_aff_list_add(data->contractions, inner);
      break;
    case isl_schedule_node_expansion:
      n = isl_union_pw_multi_aff_list_n_union_pw_multi_aff(data->contractions);
      if (n < 0)
      ...
      inner = isl_union_pw_multi_aff_list_get_union_pw_multi_aff(
          data->contractions, n - 1);
      inner = isl_union_pw_multi_aff_pullback_union_pw_multi_aff(inner,
                                                                 contraction);
      data->contractions = isl_union_pw_multi_aff_list_set_union_pw_multi_aff(
 ...
 */
static __isl_give isl_schedule_node *
subtree_contraction_leave(__isl_take isl_schedule_node *node, void *user) {
  struct isl_subtree_contraction_data *data = user;
  isl_size n;
  ...
  case isl_schedule_node_filter:
    ...
    n = isl_union_pw_multi_aff_list_n_union_pw_multi_aff(data->contractions);
    if (n < 0)
      data->contractions = isl_union_pw_multi_aff_list_free(data->contractions);
    data->contractions =
        isl_union_pw_multi_aff_list_drop(data->contractions, n - 1, 1);
    break;
  case isl_schedule_node_leaf:
    n = isl_union_pw_multi_aff_list_n_union_pw_multi_aff(data->contractions);
    if (n < 0)
 ...
 */
__isl_give isl_union_pw_multi_aff *
isl_schedule_node_get_subtree_contraction(__isl_keep isl_schedule_node *node) {
  ...
  node = isl_schedule_node_copy(node);
  node = traverse(node, &subtree_contraction_enter, &subtree_contraction_leave,
                  &data);
  if (!node)
 ...
 */
static isl_bool has_ancestors(__isl_keep isl_schedule_node *node, int n,
 ...
 */
static __isl_give isl_schedule_node *
extend_extension(__isl_take isl_schedule_node *node,
                 __isl_take isl_union_map *extension) {
  isl_size pos;
  isl_bool disjoint;
 ...
 */
static __isl_give isl_union_set *
replace_by_universe_if_disjoint(__isl_take isl_union_set *uset,
 ...
 */
static __isl_give isl_schedule_node *
insert_extension(__isl_take isl_schedule_node *node,
                 __isl_take isl_union_map *extension) {
  enum isl_schedule_node_type ancestors[] = {isl_schedule_node_filter,
 ...
 */
static __isl_give isl_schedule_node *
graft_or_splice(__isl_take isl_schedule_node *node,
                __isl_take isl_schedule_tree *tree, int tree_pos) {
  isl_size pos;

  if (isl_schedule_node_get_parent_type(node) == isl_schedule_node_sequence) {
    pos = isl_schedule_node_get_child_position(node);
    if (pos < 0)
      node = isl_schedule_node_free(node);
    node = isl_schedule_node_parent(node);
 ...
 */
static __isl_give isl_schedule_node *
graft_extension(__isl_take isl_schedule_node *node,
                __isl_take isl_schedule_node *graft, int before) {
  isl_union_map *extension;
  ...
  graft_domain = replace_by_universe_if_disjoint(graft_domain, node_domain);
  ...
  if (!isl_schedule_node_has_children(graft)) {
    ...
    tree_graft = isl_schedule_tree_insert_filter(tree_graft, graft_domain);
  }
  if (before)
 ...
 */
static __isl_give isl_schedule_node *
extension_from_domain(__isl_take isl_schedule_node *node,
                      __isl_keep isl_schedule_node *pos) {
  isl_union_set *universe;
  isl_union_set *domain;
  isl_union_map *ext;
 ...
 */
static __isl_give isl_schedule_node *
isl_schedule_node_graft_before_or_after(__isl_take isl_schedule_node *node,
  ...
  if (isl_schedule_node_get_type(graft) != isl_schedule_node_extension)
    isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
            "expecting domain or extension as root of graft", goto error);
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_graft_before(__isl_take isl_schedule_node *node,
                               __isl_take isl_schedule_node *graft) {
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_graft_after(__isl_take isl_schedule_node *node,
                              __isl_take isl_schedule_node *graft) {
 ...
 */
static __isl_give isl_schedule_node *
isl_schedule_node_order_before_or_after(__isl_take isl_schedule_node *node,
                                        __isl_take isl_union_set *filter,
                                        int before) {
  enum isl_schedule_node_type ancestors[] = {isl_schedule_node_filter,
                                             isl_schedule_node_sequence};
  ...
  node_filter = isl_union_set_subtract(node_filter, isl_union_set_copy(filter));
  ...
  if (in_seq) {
    ...
    node_filter =
        isl_union_set_intersect(node_filter, isl_union_set_copy(parent_filter));
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_order_before(__isl_take isl_schedule_node *node,
                               __isl_take isl_union_set *filter) {
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_order_after(__isl_take isl_schedule_node *node,
                              __isl_take isl_union_set *filter) {
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_reset_user(__isl_take isl_schedule_node *node) {
  isl_schedule_tree *tree;
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_align_params(__isl_take isl_schedule_node *node,
 ...
 */
__isl_give isl_schedule_node *
isl_schedule_node_get_shared_ancestor(__isl_keep isl_schedule_node *node1,
                                      __isl_keep isl_schedule_node *node2) {
  int i;
  isl_size n1, n2;
...
isl_printer_print_schedule_node(__isl_take isl_printer *p,
                                __isl_keep isl_schedule_node *node) {
  isl_size n;
