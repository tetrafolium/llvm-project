...
isl_local_space_alloc_div(__isl_take isl_space *space,
...
__isl_give isl_local_space *
isl_local_space_from_space(__isl_take isl_space *space) {
  return isl_local_space_alloc(space, 0);
}

__isl_give isl_local_space *
isl_local_space_copy(__isl_keep isl_local_space *ls) {
  if (!ls)
    return NULL;
...
__isl_give isl_local_space *
isl_local_space_dup(__isl_keep isl_local_space *ls) {
  if (!ls)
    return NULL;
...
__isl_give isl_local_space *
isl_local_space_cow(__isl_take isl_local_space *ls) {
  if (!ls)
    return NULL;
...
__isl_null isl_local_space *
isl_local_space_free(__isl_take isl_local_space *ls) {
  if (!ls)
    return NULL;
...
drop_unknown_divs_and_extract_div(__isl_keep isl_local_space *ls, int pos) {
  int i;
  isl_size n;
  isl_bool unknown;
  isl_aff *aff;
...
isl_local_space_peek_space(__isl_keep isl_local_space *ls) {
  if (!ls)
    return NULL;
...
isl_local_space_get_space(__isl_keep isl_local_space *ls) {
 ...
 */
__isl_give isl_space *
isl_local_space_take_space(__isl_keep isl_local_space *ls) {
  isl_space *space;
 ...
 */
__isl_give isl_local_space *
isl_local_space_restore_space(__isl_take isl_local_space *ls,
...
isl_local_space_peek_local(__isl_keep isl_local_space *ls) {
 ...
 */
__isl_give isl_local_space *
isl_local_space_set_tuple_id(__isl_take isl_local_space *ls,
...
__isl_give isl_local_space *
isl_local_space_set_dim_name(__isl_take isl_local_space *ls,
                             enum isl_dim_type type, unsigned pos,
                             const char *s) {
  ls = isl_local_space_cow(ls);
  if (!ls)
...
__isl_give isl_local_space *
isl_local_space_set_dim_id(__isl_take isl_local_space *ls,
                           enum isl_dim_type type, unsigned pos,
 ...
 */
__isl_give isl_local_space *
isl_local_space_set_from_params(__isl_take isl_local_space *ls) {
  isl_space *space;
...
__isl_give isl_local_space *
isl_local_space_reset_space(__isl_take isl_local_space *ls,
                            __isl_take isl_space *space) {
  ls = isl_local_space_cow(ls);
  if (!ls || !space)
    goto error;
 ...
 */
__isl_give isl_local_space *
isl_local_space_realign(__isl_take isl_local_space *ls,
...
__isl_give isl_local_space *
isl_local_space_add_div(__isl_take isl_local_space *ls,
...
__isl_give isl_local_space *
isl_local_space_replace_divs(__isl_take isl_local_space *ls,
                             __isl_take isl_mat *div) {
  ls = isl_local_space_cow(ls);
 ...
 * defined by "exp".
 */
static void expand_row(__isl_keep isl_mat *dst, int d, __isl_keep isl_mat *src,
                       int s, int *exp) {
  int i;
...
__isl_give isl_mat *isl_merge_divs(__isl_keep isl_mat *div1,
                                   __isl_keep isl_mat *div2, int *exp1,
                                   int *exp2) {
  int i, j, k;
  isl_mat *div = NULL;
  unsigned d;
 ...
 */
__isl_give isl_local_space *
isl_local_space_swap_div(__isl_take isl_local_space *ls, int a, int b) {
  int offset;
 ...
 */
__isl_give isl_local_space *
isl_local_space_intersect(__isl_take isl_local_space *ls1,
  ...
  if (!isl_space_is_equal(ls1->dim, ls2->dim))
    isl_die(ctx, isl_error_invalid, "spaces should be identical", goto error);
...
__isl_give isl_local_space *
isl_local_space_domain(__isl_take isl_local_space *ls) {
  isl_size n_out;
...
__isl_give isl_local_space *
isl_local_space_range(__isl_take isl_local_space *ls) {
  isl_size n_in;
 ...
 */
__isl_give isl_local_space *
isl_local_space_from_domain(__isl_take isl_local_space *ls) {
  ls = isl_local_space_cow(ls);
  if (!ls)
    return NULL;
...
__isl_give isl_local_space *
isl_local_space_add_dims(__isl_take isl_local_space *ls, enum isl_dim_type type,
                         unsigned n) {
  isl_size pos;
...
isl_local_space_lift_basic_set(__isl_take isl_local_space *ls,
  ...
  if (n_local < 0 || isl_local_space_check_has_space(ls, space) < 0)
    goto error;
...
static __isl_give isl_local_space *normalize_div(__isl_take isl_local_space *ls,
  ...
  isl_int_gcd(ctx->normalize_gcd, ctx->normalize_gcd, ls->div->row[div][0]);
 ...
 */
__isl_give isl_local_space *
isl_local_space_substitute_equalities(__isl_take isl_local_space *ls,
 ...
 */
__isl_give isl_local_space *
isl_local_space_substitute_seq(__isl_take isl_local_space *ls,
                               enum isl_dim_type type, unsigned pos,
                               isl_int *subs, int subs_len, int first, int n) {
  int i;
  isl_int v;
  ...
  for (i = first; i < first + n; ++i) {
    if (isl_int_is_zero(ls->div->row[i][1 + pos]))
      continue;
    isl_seq_substitute(ls->div->row[i], pos, subs, ls->div->n_col, subs_len, v);
 ...
 */
__isl_give isl_local_space *
isl_local_space_substitute(__isl_take isl_local_space *ls,
                           enum isl_dim_type type, unsigned pos,
  ...
  if (n_div != 0)
    isl_die(isl_local_space_get_ctx(ls), isl_error_unsupported,
            "cannot handle divs yet", return isl_local_space_free(ls));
...
__isl_give isl_local_space *
isl_local_space_drop_dims(__isl_take isl_local_space *ls,
...
__isl_give isl_local_space *
isl_local_space_insert_dims(__isl_take isl_local_space *ls,
...
static isl_bool is_linear_div_constraint(__isl_keep isl_local_space *ls,
  ...
  if (isl_int_eq(constraint[pos], ls->div->row[div][0])) {
    *sign = -1;
    if (!isl_seq_is_neg(constraint + 1, ls->div->row[div] + 2, pos - 1))
      return isl_bool_false;
  ...
  if (isl_seq_first_non_zero(constraint + pos + 1, ls->div->n_row - div - 1) !=
      -1)
    return isl_bool_false;
...
isl_bool isl_local_space_is_div_constraint(__isl_keep isl_local_space *ls,
  ...
  if (sign < 0) {
    int neg;
    isl_int_sub(ls->div->row[div][1], ls->div->row[div][1],
                ls->div->row[div][0]);
    ...
    isl_int_add(ls->div->row[div][1], ls->div->row[div][1],
                ls->div->row[div][0]);
    if (!neg)
      return isl_bool_false;
  } else {
 ...
 */
__isl_give isl_local_space *
isl_local_space_lift(__isl_take isl_local_space *ls) {
  ls = isl_local_space_cow(ls);
  if (!ls)
    return NULL;
 ...
 */
__isl_give isl_basic_map *
isl_local_space_lifting(__isl_take isl_local_space *ls) {
 ...
 */
__isl_give isl_local_space *
isl_local_space_preimage_multi_aff(__isl_take isl_local_space *ls,
  ...
  for (i = 0; i < ls->div->n_row; ++i) {
    ...
    if (isl_seq_preimage(res->div->row[n_div_ma + i], ls->div->row[i], ma, 0, 0,
                         n_div_ma, n_div_ls, f, c1, c2, g, 1) < 0)
 ...
 */
__isl_give isl_local_space *
isl_local_space_move_dims(__isl_take isl_local_space *ls,
  ...
  if (!ls)
    return NULL;
  if (n == 0 && !isl_local_space_is_named_or_nested(ls, src_type) &&
  ...
  if (src_type == isl_dim_div)
    isl_die(isl_local_space_get_ctx(ls), isl_error_invalid, "cannot move divs",
            return isl_local_space_free(ls));
  ...
  if (!ls->div)
    return isl_local_space_free(ls);
  ls->dim =
      isl_space_move_dims(ls->dim, dst_type, dst_pos, src_type, src_pos, n);
  if (!ls->dim)
    return isl_local_space_free(ls);
 ...
 */
__isl_give isl_local_space *
isl_local_space_flatten_domain(__isl_take isl_local_space *ls) {
  if (!ls)
    return NULL;
 ...
 */
__isl_give isl_local_space *
isl_local_space_flatten_range(__isl_take isl_local_space *ls) {
  if (!ls)
    return NULL;
 ...
 */
__isl_give isl_local_space *
isl_local_space_wrap(__isl_take isl_local_space *ls) {
  ls = isl_local_space_cow(ls);
  if (!ls)
    return NULL;
