...
generate_next_level(__isl_take isl_union_map *executed,
                    __isl_take isl_ast_build *build);
static __isl_give isl_ast_graft_list *
generate_code(__isl_take isl_union_map *executed,
 ...
 */
static isl_stat
generate_non_single_valued(__isl_take isl_map *executed,
...
at_each_domain(__isl_take isl_ast_graft *graft, __isl_keep isl_map *executed,
  ...
  build = isl_ast_build_set_executed(
      build, isl_union_map_from_map(isl_map_copy(executed)));
  if (!build)
    return isl_ast_graft_free(graft);

  graft->node =
      build->at_each_domain(graft->node, build, build->at_each_domain_user);
 ...
 */
static __isl_give isl_ast_graft_list *
call_create_leaf(__isl_take isl_union_map *executed,
                 __isl_take isl_ast_build *build) {
  isl_set *guard;
  isl_ast_node *node;
  isl_ast_graft *graft;
  ...
  user_build =
      isl_ast_build_replace_pending_by_guard(user_build, isl_set_copy(guard));
...
static __isl_give isl_ast_graft_list *
build_ast_from_child(__isl_take isl_ast_build *build,
 ...
 */
static __isl_give isl_ast_graft_list *
generate_inner_level(__isl_take isl_union_map *executed,
                     __isl_take isl_ast_build *build) {
  isl_ctx *ctx;
...
before_each_for(__isl_take isl_ast_node *node,
                __isl_keep isl_ast_build *build) {
  isl_id *id;
 ...
 */
static __isl_give isl_ast_graft *
after_each_for(__isl_take isl_ast_graft *graft,
               __isl_keep isl_ast_build *build) {
  if (!graft || !build)
    return isl_ast_graft_free(graft);
  if (!build->after_each_for)
    return graft;
  graft->node =
      build->after_each_for(graft->node, build, build->after_each_for_user);
  if (!graft->node)
    return isl_ast_graft_free(graft);
  return graft;
}
...
plug_in_values(__isl_take isl_union_map *executed,
               __isl_keep isl_ast_build *build) {
  return isl_ast_build_substitute_values_union_map_domain(build, executed);
}
 ...
 *	f + s * ceil((l - f)/s)
 */
static __isl_give isl_aff *lower_bound(__isl_keep isl_constraint *c, int pos,
                                       __isl_keep isl_ast_build *build) {
  isl_aff *aff;
...
remove_redundant_lower_bounds(__isl_take isl_pw_aff_list *list,
                              __isl_keep isl_ast_build *build) {
  int i, j;
  isl_size n;
  isl_set *domain;
 ...
 */
static __isl_give isl_pw_aff_list *
lower_bounds(__isl_keep isl_constraint_list *constraints, int pos,
 ...
 */
static __isl_give isl_pw_aff_list *
upper_bounds(__isl_keep isl_constraint_list *constraints, int pos,
             __isl_keep isl_set *domain, __isl_keep isl_ast_build *build) {
  isl_ctx *ctx;
  isl_pw_aff_list *list;
  int i;
...
static __isl_give isl_ast_expr *reduce_list(enum isl_ast_expr_op_type type,
  ...
  if (n == 1)
    return isl_ast_build_expr_from_pw_aff_internal(
        build, isl_pw_aff_list_get_pw_aff(list, 0));
  ...
  for (i = 0; i < n; ++i) {
    ...
    expr_i = isl_ast_build_expr_from_pw_aff_internal(
        build, isl_pw_aff_list_get_pw_aff(list, i));
    if (!expr_i)
      goto error;
 ...
 */
static __isl_give isl_ast_graft *
refine_degenerate(__isl_take isl_ast_graft *graft,
                  __isl_keep isl_ast_build *build,
  ...
  graft->node->u.f.init = isl_ast_build_expr_from_pw_aff_internal(build, value);
...
intersect_constraints(__isl_keep isl_constraint_list *list) {
  int i;
  isl_size n;
  isl_basic_set *bset;
 ...
 */
static __isl_give isl_ast_graft *
set_enforced_from_set(__isl_take isl_ast_graft *graft,
                      __isl_keep isl_pw_aff_list *lower, int pos,
 ...
 */
static __isl_give isl_ast_graft *
set_enforced_from_list(__isl_take isl_ast_graft *graft,
 ...
 */
static __isl_give isl_pw_aff_list *
list_add_one(__isl_take isl_pw_aff_list *list,
             __isl_keep isl_ast_build *build) {
  int i;
  isl_size n;
  isl_space *space;
 ...
 */
static __isl_give isl_ast_graft *
set_for_cond_from_list(__isl_take isl_ast_graft *graft,
                       __isl_keep isl_pw_aff_list *list,
                       __isl_keep isl_ast_build *build) {
  int neg;
 ...
 */
static __isl_give isl_ast_graft *
set_for_cond_from_set(__isl_take isl_ast_graft *graft, __isl_keep isl_set *set,
                      __isl_keep isl_ast_build *build) {
  isl_ast_expr *cond;
...
static __isl_give isl_ast_graft *refine_generic_bounds(
    __isl_take isl_ast_graft *graft, __isl_take isl_constraint_list *c_lower,
    __isl_take isl_constraint_list *c_upper, __isl_keep isl_set *domain,
    __isl_keep isl_ast_build *build) {
  int depth;
  isl_ctx *ctx;
 ...
 */
static __isl_give isl_ast_graft *
refine_generic(__isl_take isl_ast_graft *graft,
...
extract_shared_enforced(__isl_keep isl_ast_graft_list *list,
                        __isl_keep isl_ast_build *build) {
  isl_ctx *ctx;
  isl_space *space;
...
static __isl_give isl_set *extract_pending(__isl_keep isl_ast_build *build,
  ...
  context = isl_set_intersect(context, isl_ast_build_get_generated(build));
 ...
 */
static __isl_give isl_ast_graft *
create_node_scaled(__isl_take isl_union_map *executed,
  ...
  sub_build =
      isl_ast_build_set_loop_bounds(sub_build, isl_basic_set_copy(bounds));
  ...
  children = generate_next_level(executed, isl_ast_build_copy(body_build));
  ...
  if (n > 1)
    children = isl_ast_graft_list_gist_guards(children, isl_set_copy(hoisted));
  guard = isl_set_intersect(guard, hoisted);
  if (!eliminated)
    guard = add_implied_guards(guard, degenerate, bounds, build);

  graft = isl_ast_graft_alloc_from_children(children, isl_set_copy(guard),
                                            enforced, build, sub_build);
  ...
  if (!eliminated) {
    ...
    for_build =
        isl_ast_build_replace_pending_by_guard(for_build, isl_set_copy(guard));
    if (degenerate)
      graft = refine_degenerate(graft, for_build, sub_build);
    else
      graft = refine_generic(graft, bounds, domain, for_build);
...
static isl_stat constraint_check_scaled(__isl_take isl_constraint *c,
  ...
  int i, j;
  isl_size n;
  enum isl_dim_type t[] = {isl_dim_param, isl_dim_in, isl_dim_out, isl_dim_div};
...
static isl_stat basic_map_check_scaled(__isl_take isl_basic_map *bmap,
  ...
  r = isl_basic_map_foreach_constraint(bmap, &constraint_check_scaled, user);
...
static __isl_give isl_ast_graft *create_node(__isl_take isl_union_map *executed,
  ...
  if (!isl_val_is_one(data.m)) {
    if (isl_union_map_foreach_map(executed, &map_check_scaled, &data) < 0 &&
  ...
  if (!isl_val_is_one(data.m)) {
    ...
    bounds = isl_basic_set_preimage_multi_aff(bounds, isl_multi_aff_copy(ma));
    domain = isl_set_preimage_multi_aff(domain, isl_multi_aff_copy(ma));
    map = isl_map_reverse(isl_map_from_multi_aff(ma));
    umap = isl_union_map_from_map(map);
    executed = isl_union_map_apply_domain(executed, isl_union_map_copy(umap));
    build = isl_ast_build_scale_down(build, isl_val_copy(data.m), umap);
  }
  isl_aff_free(offset);
  isl_val_free(data.m);
...
isl_basic_set_list_from_set(__isl_take isl_set *set) {
  isl_size n;
  isl_ctx *ctx;
 ...
 */
static __isl_give isl_ast_graft_list *
add_node(__isl_take isl_ast_graft_list *list,
         __isl_take isl_union_map *executed, __isl_take isl_basic_set *bounds,
         __isl_take isl_ast_build *build) {
  isl_ast_graft *graft;
  isl_set *domain = NULL;
  isl_union_set *uset;
  ...
  graft = create_node(executed, bounds, domain, isl_ast_build_copy(build));
...
static isl_bool domain_follows_at_depth(__isl_keep isl_basic_set *i,
  ...
  for (l = 0; l < depth; ++l)
    test = isl_basic_map_equate(test, isl_dim_in, l, isl_dim_out, l);
  test = isl_basic_map_order_ge(test, isl_dim_in, depth, isl_dim_out, depth);
 ...
 */
static __isl_give isl_basic_set_list *
add_split_on(__isl_take isl_basic_set_list *list,
             __isl_take isl_basic_set *bset, __isl_keep isl_basic_map *gt) {
  int i;
  isl_size n;
  ...
  for (i = 0; res && i < n; ++i) {
    ...
    res = isl_basic_set_list_concat(res, isl_basic_set_list_from_set(set1));
  }
...
static __isl_give isl_ast_graft_list *
generate_sorted_domains(__isl_keep isl_basic_set_list *domain_list,
...
static isl_stat add_nodes(__isl_take isl_basic_set_list *scc, void *user) {
  ...
  if (n == 1) {
    ...
    data->list = add_node(data->list, isl_union_map_copy(data->executed), bset,
  ...
  data->list = isl_ast_graft_list_concat(
      data->list, generate_sorted_domains(scc, data->executed, data->build));
  isl_basic_set_list_free(scc);
...
generate_sorted_domains(__isl_keep isl_basic_set_list *domain_list,
  ...
  data.build = build;
  if (isl_basic_set_list_foreach_scc(domain_list, &domain_follows_at_depth,
                                     &depth, &add_nodes, &data) < 0)
    data.list = isl_ast_graft_list_free(data.list);
 ...
 */
static __isl_give isl_ast_graft_list *
generate_parallel_domains(__isl_keep isl_basic_set_list *domain_list,
                          __isl_keep isl_union_map *executed,
                          __isl_keep isl_ast_build *build) {
  int depth;
  ...
  if (isl_basic_set_list_foreach_scc(domain_list, &shared_outer, &depth,
                                     &generate_sorted_domains_wrap, &data) < 0)
    data.list = isl_ast_graft_list_free(data.list);
...
static __isl_give isl_set *explicit_bounds(__isl_take isl_map *map,
  ...
  domain = isl_set_drop_constraints_involving_dims(
      domain, isl_dim_set, depth + 1, dim - (depth + 1));
  domain = isl_set_remove_divs_involving_dims(domain, isl_dim_set, depth, 1);
 ...
 */
static __isl_give isl_set *
separate_schedule_domains(__isl_take isl_space *space,
                          __isl_take isl_union_map *executed,
                          __isl_keep isl_ast_build *build) {
...
find_unroll_lower_bound(__isl_keep isl_ast_build *build,
                        __isl_keep isl_set *domain, int depth,
                        __isl_keep isl_basic_map *expansion, int *n) {
  struct isl_find_unroll_data data = {build, domain, depth, expansion,
                                      NULL,  n,      -1};
  isl_basic_set *hull;

  hull = isl_set_simple_hull(isl_set_copy(domain));

  if (isl_basic_set_foreach_constraint(hull, &constraint_find_unroll, &data) <
      0)
    goto error;
 ...
 */
static int
foreach_iteration(__isl_take isl_set *domain, __isl_keep isl_ast_build *build,
                  int (*init)(int n, void *user),
  ...
  stride_build =
      isl_ast_build_detect_strides(stride_build, isl_set_copy(domain));
  expansion = isl_ast_build_get_stride_expansion(stride_build);

  domain = isl_set_preimage_multi_aff(domain, isl_multi_aff_copy(expansion));
...
static int do_unroll_iteration(__isl_take isl_basic_set *bset, void *user) {
  ...
  data->unroll_domain = isl_set_union(data->unroll_domain, isl_set_copy(set));
  set = isl_set_intersect(set, isl_set_copy(data->class_domain));
  set = isl_set_make_disjoint(set);
  list = isl_basic_set_list_from_set(set);
  data->domains->list = isl_basic_set_list_concat(data->domains->list, list);
...
static __isl_give isl_set *do_unroll(struct isl_codegen_domains *domains,
  ...
  if (foreach_iteration(domain, domains->build, NULL, &do_unroll_iteration,
                        &data) < 0)
 ...
 */
static __isl_give isl_set *
compute_unroll_domains(struct isl_codegen_domains *domains,
                       __isl_take isl_set *class_domain) {
  isl_set *unroll_domain;
  ...
  for (i = 0; i < n; ++i) {
    ...
    unroll_domain =
        isl_set_intersect(unroll_domain, isl_set_copy(class_domain));
 ...
 */
static __isl_give isl_set *
compute_atomic_domain(struct isl_codegen_domains *domains,
                      __isl_take isl_set *class_domain) {
  isl_basic_set *bset;
  isl_basic_set_list *list;
  ...
  domain = isl_set_intersect(domain, isl_set_copy(domains->schedule_domain));
  empty = isl_set_is_empty(domain);
  if (empty < 0)
    class_domain = isl_set_free(class_domain);
  if (empty) {
...
static int compute_separate_domain(struct isl_codegen_domains *domains,
  ...
  executed =
      isl_union_map_intersect_domain(executed, isl_union_set_from_set(domain));
  empty = isl_union_map_is_empty(executed);
  if (empty < 0 || empty) {
    isl_union_map_free(executed);
    return empty < 0 ? -1 : 0;
...
static isl_stat compute_partial_domains(struct isl_codegen_domains *domains,
  ...
  class_domain = isl_set_subtract(class_domain, isl_set_copy(domains->done));
  domains->done = isl_set_union(domains->done, isl_set_copy(class_domain));
...
static isl_stat compute_class_domains(__isl_take isl_point *pnt, void *user) {
  ...
  domain = isl_map_domain(
      isl_map_intersect_range(isl_map_copy(domains->sep_class), class_set));
...
static void compute_domains_init_options(isl_set *option[4],
  ...
  for (type = isl_ast_loop_atomic; type <= isl_ast_loop_separate; ++type) {
    ...
    for (type2 = isl_ast_loop_atomic; type2 < type; ++type2)
      option[type] =
          isl_set_subtract(option[type], isl_set_copy(option[type2]));
  }
 ...
 */
static __isl_give isl_basic_set_list *
compute_domains(__isl_keep isl_union_map *executed,
                __isl_keep isl_ast_build *build) {
  struct isl_codegen_domains domains;
  isl_ctx *ctx;
  isl_set *domain;
 ...
 */
static __isl_give isl_ast_graft_list *
generate_shifted_component_flat(__isl_take isl_union_map *executed,
                                __isl_take isl_ast_build *build) {
 ...
 */
static __isl_give isl_ast_graft_list *
generate_shifted_component_tree_separate(__isl_take isl_union_map *executed,
                                         __isl_take isl_ast_build *build) {
  isl_space *space;
  isl_set *domain;
  isl_basic_set_list *domain_list;
  ...
  space = isl_ast_build_get_space(build, 1);
  domain =
      separate_schedule_domains(space, isl_union_map_copy(executed), build);
 ...
 */
static __isl_give isl_ast_graft_list *
generate_shifted_component_tree_unroll(__isl_take isl_union_map *executed,
                                       __isl_take isl_set *domain,
                                       __isl_take isl_ast_build *build) {
...
static isl_bool has_pure_outer_disjunction(__isl_keep isl_set *domain,
  ...
  depth = isl_ast_build_get_depth(build);
  inner = isl_set_drop_constraints_not_involving_dims(inner, isl_dim_set, depth,
                                                      dim - depth);
 ...
 */
static __isl_give isl_ast_graft_list *
generate_shifted_component_tree_base(__isl_take isl_union_map *executed,
                                     __isl_take isl_ast_build *build,
                                     int isolated) {
  ...
  if (type == isl_ast_loop_separate)
    return generate_shifted_component_tree_separate(executed, build);

  schedule_domain = isl_union_map_domain(isl_union_map_copy(executed));
  domain = isl_set_from_union_set(schedule_domain);

  if (type == isl_ast_loop_unroll)
    return generate_shifted_component_tree_unroll(executed, domain, build);
 ...
 */
static __isl_give isl_set *
extract_disjunction(__isl_take isl_set *domain,
                    __isl_keep isl_ast_build *build) {
  isl_set *hull;
  int depth;
  isl_size dim;
 ...
 */
static __isl_give isl_ast_graft_list *
list_add_guard(__isl_take isl_ast_graft_list *list, __isl_keep isl_set *guard,
               __isl_keep isl_ast_build *build,
               __isl_keep isl_ast_build *sub_build) {
  isl_ast_graft *graft;
  isl_size n;
...
static __isl_give isl_ast_graft_list *generate_shifted_component_tree_part(
  ...
  if (!isolated) {
    ...
    sub_build =
        isl_ast_build_restrict_generated(sub_build, isl_set_copy(domain));
  }
  list = generate_shifted_component_tree_base(
      executed, isl_ast_build_copy(sub_build), isolated);
  if (!isolated)
...
static __isl_give isl_ast_graft_list *generate_shifted_component_parts(
  ...
  list = generate_shifted_component_tree_part(executed, isolated, build, 1);
 ...
 */
static __isl_give isl_ast_graft_list *
generate_shifted_component_tree(__isl_take isl_union_map *executed,
                                __isl_take isl_ast_build *build) {
  int i, depth;
  ...
  if (pure < 0)
    ...
    return generate_shifted_component_only_after(executed, isolated, domain,
                                                 build, before, after);
  ...
  return generate_shifted_component_parts(executed, before, isolated, after,
                                          domain, build);
error:
  isl_set_free(domain);
  isl_set_free(isolated);
 ...
 */
static __isl_give isl_ast_graft_list *
generate_shifted_component(__isl_take isl_union_map *executed,
                           __isl_take isl_ast_build *build) {
  if (isl_ast_build_has_schedule_node(build))
 ...
 */
static __isl_give isl_union_map *
construct_component_executed(struct isl_set_map_pair *domain, int *order,
                             int n) {
  int i;
  isl_map *map;
  isl_union_map *executed;
 ...
 */
static __isl_give isl_ast_graft_list *
generate_shifted_component_from_list(struct isl_set_map_pair *domain,
                                     int *order, int n,
                                     __isl_take isl_ast_build *build) {
  isl_union_map *executed;
 ...
 * at position "depth"?
 */
static int at_most_one_non_fixed(struct isl_set_map_pair *domain, int *order,
                                 int n, int depth) {
  int i;
  int non_fixed = -1;
 ...
 */
static int eliminate_non_fixed(struct isl_set_map_pair *domain, int *order,
                               int n, int depth,
                               __isl_keep isl_ast_build *build) {
  int i;
  int base = -1;
  ...
  for (i = n - 1; i >= 0; --i) {
    ...
    if (f)
      continue;
    domain[order[i]].set =
        isl_ast_build_eliminate_inner(build, domain[order[i]].set);
    base = i;
  }
...
static int first_offset(struct isl_set_map_pair *domain, int *order, int n,
  ...
  min_first =
      isl_ast_build_map_to_iterator(build, isl_set_copy(domain[order[0]].set));
 ...
 */
static __isl_give isl_union_map *
construct_shifted_executed(struct isl_set_map_pair *domain, int *order, int n,
  ...
  for (i = 0; i < n; ++i) {
    ...
    map_i = isl_map_fix_val(map_i, isl_dim_out, depth + 1, isl_val_copy(v));
    v = isl_val_neg(v);
    c = isl_constraint_set_constant_val(c, v);
    map_i = isl_map_add_constraint(map_i, isl_constraint_copy(c));

    map_i = isl_map_apply_domain(isl_map_copy(domain[order[i]].map), map_i);
 ...
 */
static __isl_give isl_ast_graft_list *
generate_shift_component(struct isl_set_map_pair *domain, int *order, int n,
                         __isl_keep isl_val *stride,
                         __isl_keep isl_multi_val *offset,
                         __isl_take isl_ast_build *build) {
  isl_ast_graft_list *list;
  ...
  executed = construct_shifted_executed(domain, order, n, stride, mv, build);
  space = isl_ast_build_get_space(build, 1);
  space = isl_space_map_from_set(space);
 ...
 */
static __isl_give isl_ast_graft_list *
generate_component(struct isl_set_map_pair *domain, int *order, int n,
                   __isl_take isl_ast_build *build) {
  int i, d;
  int depth;
  isl_ctx *ctx;
  ...
  if (skip < 0)
    goto error;
  if (skip)
    return generate_shifted_component_from_list(domain, order, n, build);
  ...
  for (i = 0; i < n; ++i) {
    ...
    map = isl_map_from_domain_and_range(isl_set_copy(domain[order[base]].set),
                                        isl_set_copy(domain[order[i]].set));
    for (d = 0; d < depth; ++d)
  ...
  if (res < 0 || !gcd) {
    ...
    list = generate_shifted_component_from_list(domain, order, n, build);
  } else {
    list = generate_shift_component(domain, order, n, gcd, mv, build);
  }
...
static isl_bool after_in_context(__isl_keep isl_union_map *umap,
  ...
  umap1 = isl_union_map_intersect_range(umap1, isl_union_set_from_set(context));
...
child_filter(__isl_keep isl_schedule_node *node, int i) {
  isl_schedule_node *child;
  isl_union_set *filter;
...
static isl_bool after_in_sequence(__isl_keep isl_union_map *umap,
  ...
  for (i = 1; i < n; ++i) {
    ...
    for (j = 0; j < i; ++j) {
      ...
      umap_ij = isl_union_map_intersect_range(umap_ij, filter_j);
...
static isl_bool after_in_tree(__isl_keep isl_union_map *umap,
  ...
  case isl_schedule_node_domain:
    isl_die(isl_schedule_node_get_ctx(node), isl_error_internal,
            "unexpected internal domain node", return isl_bool_error);
...
static isl_bool any_scheduled_after(int i, int j, void *user) {
  ...
  for (pos = data->depth; pos < dim; ++pos) {
    ...
    follows = isl_set_follows_at(data->domain[i].set, data->domain[j].set, pos);
  ...
  if (isl_ast_build_has_schedule_node(data->build)) {
    ...
    after =
        after_in_subtree(data->build, data->domain[i].map, data->domain[j].map);
    if (after < 0 || after)
      return after;
  }
 ...
 */
static __isl_give isl_ast_graft_list *
generate_components(__isl_take isl_union_map *executed,
                    __isl_take isl_ast_build *build) {
  int i;
  ...
  while (list && n) {
    ...
    if (g->order[i] == -1)
      isl_die(ctx, isl_error_internal, "cannot happen", goto error);
    ++i;
    --n;
    while (g->order[i] != -1) {
      ++i;
    ...
    list_c = generate_component(data.domain, g->order + first, i - first,
                                isl_ast_build_copy(build));
 ...
 */
static __isl_give isl_union_map *
internal_executed(__isl_take isl_union_map *executed,
  ...
  executed = isl_union_map_apply_domain(executed, isl_union_map_from_map(proj));
  return executed;
}
...
static isl_stat generate_code_in_space(struct isl_generate_code_data *data,
  ...
  executed =
      isl_union_map_intersect_domain(executed, isl_union_set_from_set(set));
...
generate_code(__isl_take isl_union_map *executed,
  ...
  if (!build)
    goto error;
  space = isl_ast_build_get_space(build, 1);
  space = isl_space_align_params(space, isl_union_map_get_space(executed));
  space =
      isl_space_align_params(space, isl_union_map_get_space(build->options));
  ...
  if (isl_union_set_foreach_set(schedule_domain, &generate_code_set, &data) < 0)
    data.list = isl_ast_graft_list_free(data.list);
 ...
 */
static __isl_give isl_ast_graft_list *
build_ast_from_leaf(__isl_take isl_ast_build *build,
                    __isl_take isl_schedule_node *node,
                    __isl_take isl_union_map *executed) {
  isl_ast_graft_list *list;
 ...
 */
static __isl_give isl_ast_graft_list *
build_ast_from_band(__isl_take isl_ast_build *build,
                    __isl_take isl_schedule_node *node,
                    __isl_take isl_union_map *executed) {
  isl_space *space;
  ...
  extra = isl_multi_union_pw_aff_align_params(
      extra, isl_ast_build_get_space(build, 1));
 ...
 */
static __isl_give isl_ast_graft_list *
hoist_out_of_context(__isl_take isl_ast_graft_list *list,
                     __isl_keep isl_ast_build *build,
                     __isl_keep isl_ast_build *sub_build) {
  isl_ast_graft *graft;
  isl_basic_set *enforced;
  ...
  enforced =
      isl_basic_set_project_out(enforced, isl_dim_param, n_param, extra_param);
  enforced = isl_basic_set_remove_unknown_divs(enforced);
  guard = isl_ast_graft_list_extract_hoistable_guard(list, sub_build);
  guard = isl_set_remove_divs_involving_dims(guard, isl_dim_param, n_param,
                                             extra_param);
  guard = isl_set_project_out(guard, isl_dim_param, n_param, extra_param);
  guard = isl_set_compute_divs(guard);
  graft = isl_ast_graft_alloc_from_children(list, guard, enforced, build,
                                            sub_build);
  list = isl_ast_graft_list_from_ast_graft(graft);
 ...
 */
static __isl_give isl_ast_graft_list *
build_ast_from_context(__isl_take isl_ast_build *build,
                       __isl_take isl_schedule_node *node,
                       __isl_take isl_union_map *executed) {
  isl_set *context;
  isl_space *space;
  ...
  sub_build =
      isl_ast_build_restrict_generated(sub_build, isl_set_copy(context));
  context = isl_set_from_basic_set(isl_set_simple_hull(context));
  executed =
      isl_union_map_intersect_domain(executed, isl_union_set_from_set(context));

  list = build_ast_from_child(isl_ast_build_copy(sub_build), node, executed);
  n = isl_ast_graft_list_n_ast_graft(list);
  if (n < 0)
    list = isl_ast_graft_list_free(list);
  ...
  list = isl_ast_graft_list_fuse(list, sub_build);
  if (depth == 1)
    list = isl_ast_graft_list_insert_pending_guard_nodes(list, sub_build);
  if (n >= 1)
 ...
 */
static __isl_give isl_ast_graft_list *
build_ast_from_expansion(__isl_take isl_ast_build *build,
                         __isl_take isl_schedule_node *node,
                         __isl_take isl_union_map *executed) {
  isl_union_map *expansion;
  isl_size n1, n2;
  ...
  expansion = isl_schedule_node_expansion_get_expansion(node);
  expansion =
      isl_union_map_align_params(expansion, isl_union_map_get_space(executed));
 ...
 */
static __isl_give isl_ast_graft_list *
build_ast_from_extension(__isl_take isl_ast_build *build,
                         __isl_take isl_schedule_node *node,
                         __isl_take isl_union_map *executed) {
  isl_union_set *schedule_domain;
  ...
  extension = isl_union_map_preimage_domain_multi_aff(
      extension, isl_multi_aff_copy(build->internal2input));
  extension = isl_union_map_intersect_domain(extension, schedule_domain);
  extension =
      isl_ast_build_substitute_values_union_map_domain(build, extension);
 ...
 */
static __isl_give isl_ast_graft_list *
build_ast_from_filter(__isl_take isl_ast_build *build,
                      __isl_take isl_schedule_node *node,
                      __isl_take isl_union_map *executed) {
  isl_ctx *ctx;
  isl_union_set *filter;
  ...
  filter = isl_schedule_node_filter_get_filter(node);
  filter =
      isl_union_set_align_params(filter, isl_union_map_get_space(executed));
  n1 = isl_union_map_dim(executed, isl_dim_param);
 ...
 */
static __isl_give isl_ast_graft_list *
build_ast_from_guard(__isl_take isl_ast_build *build,
                     __isl_take isl_schedule_node *node,
                     __isl_take isl_union_map *executed) {
  isl_space *space;
  isl_set *guard, *hoisted;
  ...
  sub_build = isl_ast_build_copy(build);
  sub_build = isl_ast_build_restrict_generated(sub_build, isl_set_copy(guard));
  ...
  if (n < 0)
    list = isl_ast_graft_list_free(list);
  if (n > 1)
    list = isl_ast_graft_list_gist_guards(list, isl_set_copy(hoisted));
  guard = isl_set_intersect(guard, hoisted);
...
static isl_stat before_each_mark(__isl_keep isl_id *mark,
  ...
  if (!build->before_each_mark)
    return isl_stat_ok;
  return build->before_each_mark(mark, build, build->before_each_mark_user);
}
 ...
 */
static __isl_give isl_ast_graft *
after_each_mark(__isl_take isl_ast_graft *graft,
                __isl_keep isl_ast_build *build) {
  if (!graft || !build)
    return isl_ast_graft_free(graft);
  if (!build->after_each_mark)
    return graft;
  graft->node =
      build->after_each_mark(graft->node, build, build->after_each_mark_user);
  if (!graft->node)
    return isl_ast_graft_free(graft);
  return graft;
}
 ...
 */
static __isl_give isl_ast_graft_list *
build_ast_from_mark(__isl_take isl_ast_build *build,
                    __isl_take isl_schedule_node *node,
                    __isl_take isl_union_map *executed) {
  isl_id *mark;
  isl_ast_graft *graft;
...
static __isl_give isl_ast_graft_list *
build_ast_from_schedule_node(__isl_take isl_ast_build *build,
                             __isl_take isl_schedule_node *node,
                             __isl_take isl_union_map *executed);
 ...
 */
static __isl_give isl_ast_graft_list *
build_ast_from_sequence(__isl_take isl_ast_build *build,
                        __isl_take isl_schedule_node *node,
                        __isl_take isl_union_map *executed) {
  int i;
  isl_size n;
...
build_ast_from_schedule_node(__isl_take isl_ast_build *build,
  ...
  isl_die(isl_ast_build_get_ctx(build), isl_error_internal, "unhandled type",
          goto error);
error:
  isl_union_map_free(executed);
  isl_schedule_node_free(node);
 ...
 */
static __isl_give isl_ast_node *
build_ast_from_domain(__isl_take isl_ast_build *build,
                      __isl_take isl_schedule_node *node) {
  isl_ctx *ctx;
  ...
  if (is_params < 0)
    goto error;
  if (!is_params)
    isl_die(ctx, isl_error_unsupported, "expecting parametric initial context",
            goto error);
...
isl_ast_build_node_from_schedule(__isl_keep isl_ast_build *build,
  ...
  if (isl_schedule_node_get_type(node) != isl_schedule_node_domain)
    isl_die(ctx, isl_error_unsupported, "expecting root domain node",
            build = isl_ast_build_free(build));
