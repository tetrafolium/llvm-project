...
insert_shared(__isl_take isl_schedule_node *node) {
  isl_ctx *ctx;
  isl_id *id;
...
gpu_tree_insert_shared_before_thread(__isl_take isl_schedule_node *node) {
  int depth0, depth;
  int any_shared = 0;
  ...
  for (;;) {
    ...
    if (n == 0)
      isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
              "no thread marker found", return isl_schedule_node_free(node));
    if (n > 1)
      isl_die(isl_schedule_node_get_ctx(node), isl_error_invalid,
 ...
 */
static __isl_give isl_schedule_node *
core_child(__isl_take isl_schedule_node *node, __isl_keep isl_union_set *core) {
  int i, n;
 ...
 */
__isl_give isl_schedule_node *
gpu_tree_move_down_to_shared(__isl_take isl_schedule_node *node,
                             __isl_keep isl_union_set *core) {
  int is_shared;
 ...
 */
__isl_give isl_schedule_node *
gpu_tree_move_down_to_thread(__isl_take isl_schedule_node *node,
                             __isl_keep isl_union_set *core) {
  int is_thread;
 ...
 */
__isl_give isl_schedule_node *
gpu_tree_move_up_to_thread(__isl_take isl_schedule_node *node) {
  int is_thread;
 ...
 */
__isl_give isl_schedule_node *
gpu_tree_move_up_to_kernel(__isl_take isl_schedule_node *node) {
  int is_kernel;
 ...
 */
__isl_give isl_schedule_node *
gpu_tree_move_down_to_depth(__isl_take isl_schedule_node *node, int depth,
                            __isl_keep isl_union_set *core) {
  int is_shared;
  int is_thread = 0;
  ...
  while (node && isl_schedule_node_get_schedule_depth(node) < depth) {
    if (isl_schedule_node_get_type(node) == isl_schedule_node_band) {
      ...
      if (node_depth + node_dim > depth)
        node = isl_schedule_node_band_split(node, depth - node_depth);
    }
    node = core_child(node, core);
  }
  while ((is_shared = node_is_shared(node)) == 0 &&
...
create_sync_domain(struct ppcg_kernel *kernel) {
  isl_space *space;
  isl_id *id;
  char name[40];
 ...
 */
static __isl_give isl_schedule_node *
insert_sync_before(__isl_take isl_schedule_node *node,
                   struct ppcg_kernel *kernel) {
  isl_union_set *domain;
 ...
 */
static __isl_give isl_schedule_node *
insert_sync_after(__isl_take isl_schedule_node *node,
                  struct ppcg_kernel *kernel) {
  isl_union_set *domain;
  isl_schedule_node *graft;
 ...
 */
__isl_give isl_schedule_node *
gpu_tree_ensure_preceding_sync(__isl_take isl_schedule_node *node,
                               struct ppcg_kernel *kernel) {
  int has_sync;
 ...
 */
__isl_give isl_schedule_node *
gpu_tree_ensure_following_sync(__isl_take isl_schedule_node *node,
                               struct ppcg_kernel *kernel) {
  int has_sync;
 ...
 */
__isl_give isl_schedule_node *
gpu_tree_ensure_sync_after_core(__isl_take isl_schedule_node *node,
                                struct ppcg_kernel *kernel) {
  int has_sync;
 ...
 */
__isl_give isl_schedule_node *
gpu_tree_move_left_to_sync(__isl_take isl_schedule_node *node,
                           struct ppcg_kernel *kernel) {
  int has_sync;
  int is_sync;
 ...
 */
__isl_give isl_schedule_node *
gpu_tree_move_right_to_sync(__isl_take isl_schedule_node *node,
                            struct ppcg_kernel *kernel) {
  int has_sync;
  int is_sync;
