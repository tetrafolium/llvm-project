...
bool TGParser::AddSubClass(Record *CurRec, SubClassReference &SubClass) {
  ...
  for (unsigned i = 0, e = TArgs.size(); i != e; ++i) {
    if (i < SubClass.TemplateArgs.size()) {
      ...
      if (SetValue(CurRec, SubClass.RefRange.Start, TArgs[i], None,
...
Init *TGParser::ParseSimpleValue(Record *CurRec, RecTy *ItemType,
  ...
  case tgtok::Id: {
    ...
    if (ExpectedArgs.size() < Args.size()) {
      Error(NameLoc, "More template args specified than expected");
      return nullptr;
    }
    ...
    for (unsigned i = 0, e = ExpectedArgs.size(); i != e; ++i) {
      ...
      Error(NameLoc, "Value not specified for template argument #" + Twine(i) +
...
Init *TGParser::ParseDeclaration(Record *CurRec, bool ParsingTemplateArgs) {
  ...
  if (consume(tgtok::equal)) {
    ...
    if (!Val || SetValue(CurRec, ValLoc, DeclName, None, Val))
...
bool TGParser::ParseIf(MultiClass *CurMultiClass) {
  ...
  Init *ThenClauseList = TernOpInit::get(TernOpInit::IF, Condition,
                                         SingletonList, EmptyList, BitListTy)
                             ->Fold(nullptr);
  ...
  if (consume(tgtok::ElseKW)) {
    ...
    Init *ElseClauseList = TernOpInit::get(TernOpInit::IF, Condition, EmptyList,
                                           SingletonList, BitListTy)
                               ->Fold(nullptr);
    Loops.push_back(
...
bool TGParser::ParseClass() {
  ...
  if (CurRec) {
    ...
    if (!CurRec->getValues().empty() || !CurRec->getSuperClasses().empty() ||
...
bool TGParser::ParseMultiClass() {
  ...
  auto Result = MultiClasses.insert(std::make_pair(
