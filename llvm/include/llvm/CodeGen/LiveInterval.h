...
public:
  ...
  struct Segment {
    ...
    Segment(SlotIndex S, SlotIndex E, VNInfo *V) : start(S), end(E), valno(V) {
  ...
  ///
  inline VNInfo *getValNumInfo(unsigned ValNo) { return valnos[ValNo]; }
  ...
  VNInfo *getNextValue(SlotIndex def, VNInfo::Allocator &VNInfoAllocator) {
    VNInfo *VNI = new (VNInfoAllocator) VNInfo((unsigned)valnos.size(), def);
  ...
  VNInfo *createValueCopy(const VNInfo *orig,
    ...
    VNInfo *VNI = new (VNInfoAllocator) VNInfo((unsigned)valnos.size(), *orig);
    valnos.push_back(VNI);
    return VNI;
  }
  ...
  void MergeValueInAsValue(const LiveRange &RHS, const VNInfo *RHSValNo,
                           VNInfo *LHSValNo);
  ...
  void join(LiveRange &Other, const int *ValNoAssignments,
  ...
  iterator removeSegment(iterator I) { return segments.erase(I); }
