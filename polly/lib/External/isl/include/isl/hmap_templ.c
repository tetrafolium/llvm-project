...
__isl_give ISL_MAYBE(ISL_VAL)
    ISL_FN(ISL_HMAP, try_get)(__isl_keep ISL_HMAP *hmap,
  ...
  entry = isl_hash_table_find(hmap->ctx, &hmap->table, hash, &has_key, key, 0);
...
__isl_give ISL_HMAP *ISL_FN(ISL_HMAP, drop)(__isl_take ISL_HMAP *hmap,
  ...
  if (!entry)
    return ISL_FN(ISL_HMAP, free)(hmap);
  if (entry == isl_hash_table_entry_none)
    isl_die(hmap->ctx, isl_error_internal, "missing entry",
            return ISL_FN(ISL_HMAP, free)(hmap));
...
__isl_give ISL_HMAP *ISL_FN(ISL_HMAP, set)(__isl_take ISL_HMAP *hmap,
  ...
  entry = isl_hash_table_find(hmap->ctx, &hmap->table, hash, &has_key, key, 1);
...
isl_stat ISL_FN(ISL_HMAP,
                foreach)(__isl_keep ISL_HMAP *hmap,
                         isl_stat (*fn)(__isl_take ISL_KEY *key,
                                        __isl_take ISL_VAL *val, void *user),
                         void *user) {
  ...
  return isl_hash_table_foreach(hmap->ctx, &hmap->table, &call_on_copy, &data);
}
...
__isl_give isl_printer *ISL_FN(isl_printer_print,
                               ISL_HMAP_SUFFIX)(__isl_take isl_printer *p,
                                                __isl_keep ISL_HMAP *hmap) {
