...
static isl_stat scan_samples_add_sample(struct isl_scan_callback *cb,
  ...
  ss->samples =
      isl_mat_extend(ss->samples, ss->samples->n_row + 1, ss->samples->n_col);
  if (!ss->samples)
    goto error;

  isl_seq_cpy(ss->samples->row[ss->samples->n_row - 1], sample->el,
              sample->size);
...
isl_basic_set_scan_samples(__isl_take isl_basic_set *bset) {
  isl_ctx *ctx;
  isl_size dim;
...
static __isl_give isl_mat *
isl_basic_set_samples(__isl_take isl_basic_set *bset) {
  struct isl_mat *T;
  struct isl_mat *samples;
