...
__isl_give LIST(EL) * FN(LIST(EL), alloc)(isl_ctx *ctx, int n) {
  ...
  if (n < 0)
    isl_die(ctx, isl_error_invalid, "cannot create list of negative length",
            return NULL);
 ...
 */
static __isl_give LIST(EL) *
    FN(LIST(EL), grow)(__isl_take LIST(EL) * list, int n) {
  isl_ctx *ctx;
  int i, new_size;
  LIST(EL) * res;
 ...
 */
static isl_stat FN(LIST(EL), check_index)(__isl_keep LIST(EL) * list,
                                          int index) {
  if (!list)
...
__isl_give LIST(EL) *
    FN(LIST(EL), add)(__isl_take LIST(EL) * list, __isl_take struct EL *el) {
 ...
 */
__isl_give LIST(EL) *
    FN(LIST(EL), drop)(__isl_take LIST(EL) * list, unsigned first, unsigned n) {
  int i;

  if (!list)
    return NULL;
  if (first + n > list->n || first + n < first)
    isl_die(list->ctx, isl_error_invalid, "index out of bounds",
            return FN(LIST(EL), free)(list));
  if (n == 0)
    return list;
...
__isl_give LIST(EL) * FN(LIST(EL), insert)(__isl_take LIST(EL) * list,
  ...
  ctx = FN(LIST(EL), get_ctx)(list);
  if (pos > list->n)
    isl_die(ctx, isl_error_invalid, "index out of bounds", goto error);
 ...
 */
static __isl_keep EL *FN(LIST(EL), peek)(__isl_keep LIST(EL) * list,
                                         int index) {
...
static __isl_give EL *FN(FN(LIST(EL), take),
                         EL_BASE)(__isl_keep LIST(EL) * list, int index) {
  EL *el;
 ...
 */
static __isl_give LIST(EL) *
    FN(FN(LIST(EL), restore), EL_BASE)(__isl_take LIST(EL) * list, int index,
                                       __isl_take EL *el) {
...
isl_stat FN(LIST(EL), foreach)(__isl_keep LIST(EL) * list,
                               isl_stat (*fn)(__isl_take EL *el, void *user),
                               void *user) {
  int i;
...
isl_bool FN(LIST(EL), every)(__isl_keep LIST(EL) * list,
                             isl_bool (*test)(__isl_keep EL *el, void *user),
                             void *user) {
  int i;
 ...
 */
__isl_give LIST(EL) *
    FN(LIST(EL), map)(__isl_keep LIST(EL) * list,
                      __isl_give EL *(*fn)(__isl_take EL *el, void *user),
                      void *user) {
  int i, n;
...
__isl_give LIST(EL) *
  ...
  if (isl_sort(list->p, list->n, sizeof(list->p[0]), &FN(LIST(EL), cmp),
               &data) < 0)
    return FN(LIST(EL), free)(list);
...
static isl_bool FN(LIST(EL), follows)(int i, int j, void *user) {
  ...
  return data->follows(data->list->p[i], data->list->p[j], data->follows_user);
}
 ...
 */
static isl_stat
FN(LIST(EL), call_on_scc)(__isl_keep LIST(EL) * list, int *pos, int n,
                          isl_stat (*fn)(__isl_take LIST(EL) * scc, void *user),
                          void *user) {
  int i;
  isl_ctx *ctx;
  LIST(EL) * slice;
...
isl_stat FN(LIST(EL), foreach_scc)(
    ...
    isl_bool (*follows)(__isl_keep EL *a, __isl_keep EL *b, void *user),
    void *follows_user, isl_stat (*fn)(__isl_take LIST(EL) * scc, void *user),
    void *fn_user) {
  ...
  do {
    ...
    if (g->order[i] == -1)
      isl_die(ctx, isl_error_internal, "cannot happen", break);
    first = i;
    while (g->order[i] != -1) {
      ++i;
      --n;
    ...
    if (FN(LIST(EL), call_on_scc)(list, g->order + first, i - first, fn,
                                  fn_user) < 0)
      break;
    ++i;
  } while (n);
 ...
 */
static __isl_give LIST(EL) *
    FN(LIST(EL), concat_inplace)(__isl_take LIST(EL) * list1,
...
__isl_give isl_printer *CAT(isl_printer_print_,
                            LIST(EL_BASE))(__isl_take isl_printer *p,
                                           __isl_keep LIST(EL) * list) {
  int i;
