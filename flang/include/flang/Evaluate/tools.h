...
template <typename A> bool IsAssumedRank(const A &) {
    return false;
}
...
inline Expr<SomeType> AsGenericExpr(Expr<SomeType> &&x) {
    return std::move(x);
}
...
struct ExtractCoindexedObjectHelper {
    ...
    std::optional<CoarrayRef> operator()(const CoarrayRef &x) const {
        return x;
    }
    template <typename A>
...
template <TypeCategory C> Expr<SomeKind<C>> operator-(Expr<SomeKind<C>> &&x) {
    return std::visit(
    [](auto &xk) {
        return Expr<SomeKind<C>> {-std::move(xk)};
    }, x.u);
}
...
struct GetLastSymbolHelper
    ...
    using Base::operator();
    Result operator()(const Symbol &x) const {
        return &x;
    }
...
struct UnexpandabilityFindingVisitor
    ...
    bool operator()(const CoarrayRef &) {
        return true;
    }
};
