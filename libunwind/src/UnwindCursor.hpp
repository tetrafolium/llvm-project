...
private:
...
#if defined(_LIBUNWIND_TARGET_PPC64)
  bool compactSaysUseDwarf(Registers_ppc64 &, uint32_t *) const { return true; }
#endif
...
#if defined(_LIBUNWIND_TARGET_RISCV)
  bool compactSaysUseDwarf(Registers_riscv &, uint32_t *) const { return true; }
#endif
...
#if defined(_LIBUNWIND_TARGET_PPC64)
  compact_unwind_encoding_t dwarfEncoding(Registers_ppc64 &) const { return 0; }
#endif
  ...
  compact_unwind_encoding_t dwarfEncoding(Registers_or1k &) const { return 0; }
#endif
...
#if defined(_LIBUNWIND_TARGET_RISCV)
  compact_unwind_encoding_t dwarfEncoding(Registers_riscv &) const { return 0; }
#endif
  ...
  if (!foundFDE) {
    ...
    if (cachedFDE != 0) {
      foundFDE = CFI_Parser<A>::findFDE(_addressSpace, pc, sects.dwarf_section,
                                        sects.dwarf_section_length, cachedFDE,
                                        &fdeInfo, &cieInfo);
...
bool UnwindCursor<A, R>::getInfoFromCompactEncodingSection(
  ...
  const pint_t lsdaArrayEndAddr = sects.compact_unwind_section +
  ...
  if (pageKind == UNWIND_SECOND_LEVEL_REGULAR) {
    ...
    funcStart = pageIndex.functionOffset(low) + firstLevelFunctionOffset +
                sects.dso_base;
    if (low < last)
      funcEnd = pageIndex.functionOffset(low + 1) + firstLevelFunctionOffset +
                sects.dso_base;
    else
      funcEnd = firstLevelNextPageFunctionOffset + sects.dso_base;
    if (pc < funcStart) {
...
bool UnwindCursor<A, R>::getInfoFromSEH(pint_t pc) {
  ...
  if (pc != getLastPC()) {
    ...
    if (xdata->Flags & (UNW_FLAG_EHANDLER | UNW_FLAG_UHANDLER)) {
      ...
      // immediately follows the UNWIND_INFO. (This follows how both Clang and
      // MSVC emit these structures.) N.B. UNWIND_INFO structs are
      // DWORD-aligned.
