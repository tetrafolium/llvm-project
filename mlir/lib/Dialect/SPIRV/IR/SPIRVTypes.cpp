...
Type CompositeType::getElementType(unsigned index) const {
    ...
    .Case<MatrixType>([](MatrixType type) {
        return type.getColumnType();
    })
    .Case<StructType>(
    [index](StructType type) {
        return type.getElementType(index);
    })
    .Default(
...
void CompositeType::getExtensions(
    ...
    .Case<ArrayType, CooperativeMatrixNVType, MatrixType, RuntimeArrayType,
          StructType>(
    [&](auto type) {
        type.getExtensions(extensions, storage);
    })
...
void CompositeType::getCapabilities(
    ...
    .Case<ArrayType, CooperativeMatrixNVType, MatrixType, RuntimeArrayType,
          StructType>(
    [&](auto type) {
        type.getCapabilities(capabilities, storage);
    })
    .Case<VectorType>([&](VectorType type) {
...
unsigned CooperativeMatrixNVType::getRows() const {
    return getImpl()->rows;
}
