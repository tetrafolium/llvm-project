...
print_constraint_polylib(struct isl_basic_map *bmap, int ineq, int n,
...
static __isl_give isl_printer *
print_constraints_polylib(struct isl_basic_map *bmap,
                          __isl_take isl_printer *p) {
  int i;
...
static __isl_give isl_printer *
bset_print_constraints_polylib(struct isl_basic_set *bset,
                               __isl_take isl_printer *p) {
  return print_constraints_polylib(bset_to_bmap(bset), p);
}

static __isl_give isl_printer *
isl_basic_map_print_polylib(__isl_keep isl_basic_map *bmap,
...
static __isl_give isl_printer *
isl_basic_set_print_polylib(__isl_keep isl_basic_set *bset,
                            __isl_take isl_printer *p, int ext) {
...
static __isl_give isl_printer *print_name(__isl_keep isl_space *space,
  ...
  name = type == isl_dim_div ? NULL : isl_space_get_dim_name(space, type, pos);
  ...
  primes =
      count_same_name(space, name == buffer ? isl_dim_div : type, pos, name);
  if (primes < 0)
    return isl_printer_free(p);
...
static isl_stat pos2type(__isl_keep isl_space *space, enum isl_dim_type *type,
                         unsigned *pos) {
...
static __isl_give isl_printer *
print_term(__isl_keep isl_space *space, __isl_keep isl_mat *div, isl_int c,
           unsigned pos, __isl_take isl_printer *p, int latex) {
...
print_nested_var_list(__isl_take isl_printer *p,
  ...
  for (i = 0; i < dim; ++i) {
    ...
    if (data->print_dim)
      ...
      p = print_name(data->space, p, data->type, offset + i, data->latex);
  }
  return p;
}
...
static __isl_give isl_printer *
print_nested_map_dim(__isl_take isl_printer *p, __isl_keep isl_space *local_dim,
                     struct isl_print_space_data *data, int offset);

static __isl_give isl_printer *
print_nested_tuple(__isl_take isl_printer *p, __isl_keep isl_space *local_space,
                   enum isl_dim_type local_type,
                   struct isl_print_space_data *data, int offset) {
  ...
  if ((local_type == isl_dim_in || local_type == isl_dim_out) &&
    ...
    if (data->space != local_space && local_type == isl_dim_out)
      offset += local_space->n_in;
    p = print_nested_map_dim(p, local_space->nested[local_type - isl_dim_in],
                             data, offset);
  } else
    p = print_nested_var_list(p, local_space, local_type, data, offset);
  if (!data->latex || n != 1 || name)
...
static __isl_give isl_printer *
print_omega_parameters(__isl_keep isl_space *space, __isl_take isl_printer *p) {
  isl_size nparam = isl_space_dim(space, isl_dim_param);
...
static isl_bool next_is_opposite(__isl_keep isl_basic_map *bmap, int i,
  ...
  if (last >= o_div) {
    isl_bool is_div;
    is_div =
        isl_basic_map_is_div_constraint(bmap, bmap->ineq[i + 1], last - o_div);
    if (is_div < 0)
      return isl_bool_error;
    if (is_div)
      return isl_bool_false;
 ...
 */
static __isl_give isl_printer *
print_half_constraint(__isl_take isl_printer *p, __isl_keep isl_space *space,
                      __isl_keep isl_mat *div, isl_int *c, int last,
                      const char *op, int latex) {
...
static __isl_give isl_printer *print_mod(__isl_take isl_printer *p,
  ...
  p = isl_printer_print_str(p, "(");
  p = print_affine_of_len(space, div, p, div->row[pos] + 1, div->n_col - 1);
  p = isl_printer_print_str(p, ") ");
...
static isl_size print_as_modulo_pos(__isl_keep isl_printer *p,
                                    __isl_keep isl_space *space,
                                    __isl_keep isl_mat *div, unsigned pos,
                                    isl_int c) {
  isl_bool can_print;
  isl_size n_div;
  enum isl_dim_type type;
 ...
 */
static __isl_give isl_printer *
print_eq_mod_constraint(__isl_take isl_printer *p, __isl_keep isl_space *space,
                        __isl_keep isl_mat *div, unsigned div_pos, isl_int *c,
                        int last, int latex) {
  isl_ctx *ctx;
  int multiple;
  ...
  if (multiple)
    p = isl_printer_print_str(p, ")");
  p = isl_printer_print_str(p, " = ");
  isl_seq_combine(c, ctx->negone, c, c[last], div->row[div_pos] + 1, last);
  isl_int_set_si(c[last], 0);
  p = print_affine(p, space, div, c);
  return p;
}
 ...
 */
static __isl_give isl_printer *
print_eq_constraint(__isl_take isl_printer *p, __isl_keep isl_space *space,
                    __isl_keep isl_mat *div, isl_int *c, int last, int latex) {
  isl_size n_div;
  isl_size div_pos;
  ...
  if (n_div < 0 || div_pos < 0)
    return isl_printer_free(p);
  if (div_pos < n_div)
    return print_eq_mod_constraint(p, space, div, div_pos, c, last, latex);
...
static __isl_give isl_printer *print_constraints(__isl_keep isl_basic_map *bmap,
  ...
  for (i = 0; i < bmap->n_ineq; ++i) {
    ...
    if (l < 0)
      continue;
    if (!dump && l >= o_div && can_print_div_expr(p, div, l - o_div)) {
      isl_bool is_div;
      is_div = isl_basic_map_is_div_constraint(bmap, bmap->ineq[i], l - o_div);
      if (is_div < 0)
        goto error;
      if (is_div)
        continue;
    ...
    if (combine) {
      ...
      p = print_half_constraint(p, space, div, c->el, l, op, latex);
      first = 1;
    } else {
      op = constraint_op(s, strict, latex);
      p = print_constraint(p, space, div, c->el, l, op, latex);
      first = 0;
    }
  }
 ...
 */
static __isl_give isl_printer *
print_div_list(__isl_take isl_printer *p, __isl_keep isl_space *space,
               __isl_keep isl_mat *div, int latex, int print_defined_divs) {
  int i;
  int first = 1;
  isl_size n_div;
...
static __isl_give isl_printer *open_exists(__isl_take isl_printer *p,
                                           __isl_keep isl_space *space,
                                           __isl_keep isl_mat *div, int latex) {
  int dump;
 ...
 */
static __isl_give isl_printer *
print_optional_disjunct(__isl_keep isl_basic_map *bmap,
...
static __isl_give isl_printer *
basic_map_print_omega(__isl_keep isl_basic_map *bmap,
                      __isl_take isl_printer *p) {
  p = isl_printer_print_str(p, "{ [");
...
static __isl_give isl_printer *
basic_set_print_omega(__isl_keep isl_basic_set *bset,
                      __isl_take isl_printer *p) {
  p = isl_printer_print_str(p, "{ [");
 ...
 */
static __isl_give isl_printer *
print_param_tuple(__isl_take isl_printer *p, __isl_keep isl_space *space,
                  struct isl_print_space_data *data) {
  isl_size nparam;
...
static __isl_give isl_printer *
isl_basic_map_print_isl(__isl_keep isl_basic_map *bmap,
 ...
 */
static __isl_give isl_printer *
print_disjuncts_in_hull(__isl_keep isl_map *map, __isl_keep isl_space *space,
...
static __isl_give isl_printer *print_disjuncts(__isl_keep isl_map *map,
  ...
  if (!p->dump && map->n >= 2) {
    ...
    if (is_universe < 0)
      ...
      return print_disjuncts_in_hull(map, space, hull, p, latex);
...
static __isl_give isl_basic_map *drop_aff(__isl_take isl_basic_map *bmap,
  ...
  for (i = bmap->n_eq - 1; i >= 0; --i) {
    if (isl_seq_first_non_zero(bmap->eq[i] + 1 + v_div, bmap->n_div) != -1)
      continue;
...
static __isl_give struct isl_aff_split *split_aff(__isl_keep isl_map *map) {
  ...
  if (isl_sort(split, map->n, sizeof(struct isl_aff_split), &aff_split_cmp,
               NULL) < 0)
    goto error;

  n = map->n;
  for (i = n - 1; i >= 1; --i) {
    if (!isl_basic_map_plain_is_equal(split[i - 1].aff, split[i].aff))
      continue;
    isl_basic_map_free(split[i].aff);
    split[i - 1].map = isl_map_union(split[i - 1].map, split[i].map);
    if (i != n - 1)
      split[i] = split[n - 1];
    split[n - 1].aff = NULL;
    split[n - 1].map = NULL;
...
static __isl_give isl_printer *
isl_map_print_isl_body(__isl_keep isl_map *map, __isl_take isl_printer *p) {
  struct isl_print_space_data data = {0};
  ...
  if (split) {
    ...
    rational = map->n > 0 && ISL_F_ISSET(map->p[0], ISL_BASIC_MAP_RATIONAL);
...
isl_printer_print_basic_map(__isl_take isl_printer *p,
...
__isl_give isl_printer *
isl_printer_print_basic_set(__isl_take isl_printer *p,
 ...
 */
static __isl_give isl_printer *
isl_printer_print_union_map_isl_body(__isl_take isl_printer *p,
 ...
 */
static __isl_give isl_printer *
isl_printer_print_union_set_isl_body(__isl_take isl_printer *p,
 ...
 */
static __isl_give isl_printer *
isl_union_map_print_isl(__isl_keep isl_union_map *umap,
                        __isl_take isl_printer *p) {
...
static __isl_give isl_printer *
isl_union_map_print_latex(__isl_keep isl_union_map *umap,
                          __isl_take isl_printer *p) {
...
__isl_give isl_printer *
isl_printer_print_union_map(__isl_take isl_printer *p,
                            __isl_keep isl_union_map *umap) {
  if (!p || !umap)
    goto error;
  ...
  isl_die(p->ctx, isl_error_invalid, "invalid output format for isl_union_map",
          goto error);
error:
  isl_printer_free(p);
  return NULL;
...
__isl_give isl_printer *
isl_printer_print_union_set(__isl_take isl_printer *p,
                            __isl_keep isl_union_set *uset) {
  if (!p || !uset)
    goto error;
  ...
  isl_die(p->ctx, isl_error_invalid, "invalid output format for isl_union_set",
          goto error);
error:
  isl_printer_free(p);
  return NULL;
...
static __isl_give isl_printer *print_base(__isl_take isl_printer *p,
                                          __isl_keep isl_space *space,
                                          __isl_keep isl_mat *div, int var) {
  isl_size total;
...
static __isl_give isl_printer *
print_qpolynomial(__isl_take isl_printer *p, __isl_keep isl_qpolynomial *qp) {
  if (!p || !qp)
    goto error;
...
static __isl_give isl_printer *
print_qpolynomial_isl(__isl_take isl_printer *p,
 ...
 */
static __isl_give isl_printer *
print_qpolynomial_c(__isl_take isl_printer *p, __isl_keep isl_space *space,
                    __isl_keep isl_qpolynomial *qp) {
  isl_bool is_one;
  isl_val *den;
...
isl_printer_print_qpolynomial(__isl_take isl_printer *p,
  ...
  if (p->output_format == ISL_FORMAT_ISL)
    ...
    isl_die(qp->dim->ctx, isl_error_unsupported,
            "output format not supported for isl_qpolynomials", goto error);
error:
  isl_printer_free(p);
  return NULL;
}
...
static __isl_give isl_printer *
qpolynomial_fold_print(__isl_keep isl_qpolynomial_fold *fold,
                       __isl_take isl_printer *p) {
  int i;
...
static __isl_give isl_printer *
isl_pwqp_print_isl_body(__isl_take isl_printer *p,
...
static __isl_give isl_printer *
print_pw_qpolynomial_isl(__isl_take isl_printer *p,
                         __isl_keep isl_pw_qpolynomial *pwqp) {
  struct isl_print_space_data data = {0};
...
static __isl_give isl_printer *
isl_pwf_print_isl_body(__isl_take isl_printer *p,
...
static __isl_give isl_printer *
print_pw_qpolynomial_fold_isl(__isl_take isl_printer *p,
                              __isl_keep isl_pw_qpolynomial_fold *pwf) {
  struct isl_print_space_data data = {0};
 ...
 */
static __isl_give isl_printer *
print_constraint_c(__isl_take isl_printer *p, __isl_keep isl_local_space *ls,
                   isl_int *c, const char *op, int *first) {
  unsigned o_div;
  isl_size n_div;
  int div;
...
static __isl_give isl_printer *
print_ls_partial_affine_c(__isl_take isl_printer *p,
                          __isl_keep isl_local_space *ls, isl_int *c,
                          unsigned len);

static __isl_give isl_printer *
print_basic_set_c(__isl_take isl_printer *p, __isl_keep isl_space *space,
                  __isl_keep isl_basic_set *bset) {
  int i, j;
  int first = 1;
  ...
  for (i = 0; i < bset->n_eq; ++i) {
    ...
    if (j < 0)
      p = print_constraint_c(p, ls, bset->eq[i], "==", &first);
    else {
      if (i)
        p = isl_printer_print_str(p, " && ");
      p = isl_printer_print_str(p, "(");
      p = print_ls_partial_affine_c(p, ls, bset->eq[i], 1 + total + j);
      p = isl_printer_print_str(p, ") % ");
      p = isl_printer_print_isl_int(p, bset->eq[i][1 + total + j]);
      p = isl_printer_print_str(p, " == 0");
      first = 0;
    }
  }
 ...
 */
static __isl_give isl_printer *
print_pw_qpolynomial_c(__isl_take isl_printer *p,
                       __isl_keep isl_pw_qpolynomial *pwqp) {
  int i;
  isl_space *space;
...
static isl_stat print_pwqp_body(__isl_take isl_pw_qpolynomial *pwqp,
                                void *user) {
  struct isl_union_print_data *data;
  data = (struct isl_union_print_data *)user;
...
static __isl_give isl_printer *
print_union_pw_qpolynomial_isl(__isl_take isl_printer *p,
...
__isl_give isl_printer *isl_printer_print_union_pw_qpolynomial(
  ...
  isl_die(p->ctx, isl_error_invalid,
          "invalid output format for isl_union_pw_qpolynomial", goto error);
error:
  isl_printer_free(p);
  return NULL;
}
 ...
 */
static __isl_give isl_printer *
print_qpolynomial_fold_c(__isl_take isl_printer *p, __isl_keep isl_space *space,
 ...
 */
static __isl_give isl_printer *
print_pw_qpolynomial_fold_c(__isl_take isl_printer *p,
                            __isl_keep isl_pw_qpolynomial_fold *pwf) {
  int i;
  isl_space *space;
...
static __isl_give isl_printer *
isl_printer_print_space_isl(__isl_take isl_printer *p,
                            __isl_keep isl_space *space) {
  struct isl_print_space_data data = {0};
...
__isl_give isl_printer *isl_printer_print_space(__isl_take isl_printer *p,
  ...
  isl_die(isl_space_get_ctx(space), isl_error_unsupported,
          "output format not supported for space", return isl_printer_free(p));
}
...
static __isl_give isl_printer *print_aff_num_base(__isl_take isl_printer *p,
  ...
  if (total < 0)
    return isl_printer_free(p);
  p = print_affine_of_len(space, aff->ls->div, p, aff->v->el + 1, 1 + total);
...
static __isl_give isl_printer *print_mod_term(__isl_take isl_printer *p,
                                              __isl_keep isl_space *space,
                                              __isl_keep isl_aff *aff,
                                              int first, __isl_take isl_val *c,
 ...
 */
static __isl_give isl_printer *print_pw_aff_body(__isl_take isl_printer *p,
 ...
 */
static isl_stat print_pw_aff_body_wrap(__isl_take isl_pw_aff *pa, void *user) {
  struct isl_union_print_data *data;
  data = (struct isl_union_print_data *)user;
 ...
 */
static __isl_give isl_printer *
print_union_pw_aff_body(__isl_take isl_printer *p,
                        __isl_keep isl_union_pw_aff *upa) {
  struct isl_union_print_data data = {p, 1};

  p = isl_printer_print_str(p, s_open_set[0]);
  data.p = p;
  if (isl_union_pw_aff_foreach_pw_aff(upa, &print_pw_aff_body_wrap, &data) < 0)
 ...
 */
static __isl_give isl_printer *
print_union_pw_aff_isl(__isl_take isl_printer *p,
                       __isl_keep isl_union_pw_aff *upa) {
  struct isl_print_space_data data = {0};
  isl_space *space;
...
static __isl_give isl_printer *
print_multi_aff_isl(__isl_take isl_printer *p, __isl_keep isl_multi_aff *maff) {
  struct isl_print_space_data data = {0};
...
static __isl_give isl_printer *
print_pw_multi_aff_body(__isl_take isl_printer *p,
...
static __isl_give isl_printer *
print_pw_multi_aff_isl(__isl_take isl_printer *p,
                       __isl_keep isl_pw_multi_aff *pma) {
  struct isl_print_space_data data = {0};
 ...
 */
static __isl_give isl_printer *
print_unnamed_pw_multi_aff_c(__isl_take isl_printer *p,
                             __isl_keep isl_pw_multi_aff *pma) {
  int i;
  isl_space *space;
...
static __isl_give isl_printer *
print_pw_multi_aff_c(__isl_take isl_printer *p,
                     __isl_keep isl_pw_multi_aff *pma) {
  isl_size n;
  const char *name;
  ...
  if (pma->n < 1)
    isl_die(p->ctx, isl_error_unsupported,
            "cannot print empty isl_pw_multi_aff in C format", goto error);
  ...
  if (!name)
    isl_die(p->ctx, isl_error_unsupported,
            "cannot print unnamed isl_pw_multi_aff in C format", goto error);

  p = isl_printer_print_str(p, name);
  if (n != 0)
    isl_die(p->ctx, isl_error_unsupported, "not supported yet", goto error);
...
static __isl_give isl_printer *
print_union_pw_multi_aff_isl(__isl_take isl_printer *p,
  ...
  isl_union_pw_multi_aff_foreach_pw_multi_aff(
      upma, &print_pw_multi_aff_body_wrap, &data);
  p = data.p;
  p = isl_printer_print_str(p, s_close_set[0]);
  return p;
}
 ...
 */
static __isl_give isl_printer *
print_multi_pw_aff_isl(__isl_take isl_printer *p,
 ...
 */
static __isl_give isl_printer *
print_multi_val_isl(__isl_take isl_printer *p, __isl_keep isl_multi_val *mv) {
  struct isl_print_space_data data = {0};
 ...
 */
__isl_give isl_printer *
isl_printer_print_multi_val(__isl_take isl_printer *p,
 ...
 */
__isl_give isl_printer *
isl_printer_print_multi_id(__isl_take isl_printer *p,
                           __isl_keep isl_multi_id *mi) {
  if (!p || !mi)
    return isl_printer_free(p);
 ...
 */
static __isl_give isl_printer *
print_union_pw_aff_dim(__isl_take isl_printer *p,
 ...
 */
static __isl_give isl_printer *
print_multi_union_pw_aff_isl(__isl_take isl_printer *p,
