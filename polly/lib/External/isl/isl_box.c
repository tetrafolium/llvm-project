 ...
 */
static __isl_give isl_fixed_box *
isl_fixed_box_alloc(__isl_take isl_multi_aff *offset,
 ...
 */
static __isl_give isl_fixed_box *
isl_fixed_box_init(__isl_take isl_space *space) {
  isl_multi_aff *offset;
  isl_multi_val *size;
 ...
 */
static __isl_give isl_fixed_box *
isl_fixed_box_set_extent(__isl_take isl_fixed_box *box, int pos,
                         __isl_keep isl_aff *offset, __isl_keep isl_val *size) {
  if (!box)
    return NULL;
  box->offset = isl_multi_aff_set_aff(box->offset, pos, isl_aff_copy(offset));
 ...
 */
static __isl_give isl_fixed_box *
isl_fixed_box_set_valid_extent(__isl_take isl_fixed_box *box, int pos,
 ...
 */
static __isl_give isl_fixed_box *
isl_fixed_box_invalidate(__isl_take isl_fixed_box *box) {
  int i;
  isl_size n;
  isl_space *space;
  isl_val *infty;
 ...
 */
static __isl_give isl_fixed_box *
isl_fixed_box_project_domain_on_params(__isl_take isl_fixed_box *box) {
  isl_bool valid;
...
isl_fixed_box_get_offset(__isl_keep isl_fixed_box *box) {
  if (!box)
    return NULL;
...
isl_fixed_box_get_size(__isl_keep isl_fixed_box *box) {
  if (!box)
    return NULL;
...
static __isl_give isl_fixed_box *set_dim_extent(__isl_take isl_fixed_box *box,
  ...
  if (isl_basic_set_foreach_constraint(bset, &compute_size_in_direction,
                                       &info) < 0)
  ...
  if (valid < 0)
    ...
    box = isl_fixed_box_set_valid_extent(box, pos, info.offset, info.size);
  else
...
isl_printer_print_fixed_box(__isl_take isl_printer *p,
