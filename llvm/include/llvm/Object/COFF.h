...
struct coff_file_header {
    ...
    bool isImportLibrary() const {
        return NumberOfSections == 0xffff;
    }
};
...
struct coff_resource_dir_entry {
    union {
        ...
        // offset is set.
        void setNameOffset(uint32_t Offset) {
            NameOffset = Offset | (1 << 31);
        }
    } Identifier;
    union {
...
struct FpoData {
    ...
    frame_type getFP() const {
        return static_cast<frame_type>(Attributes >> 14);
    }
};
