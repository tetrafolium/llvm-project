...
public:
    ...
    size_t getSizeLowerBound() const {
        return Size;
    }
...
#undef GET_SVE_IMMEDIATE_CHECK
    ...
    for (auto &I : ImmChecks) {
        ...
        case SVETypeFlags::ImmCheckComplexRot90_270:
            if (CheckImmediateInSet([](int64_t V) {
            return V == 90 || V == 270;
        },
...
public:
    ...
    unsigned getLength() const {
        return FExpr->getLength() - Offset;
    }
    unsigned getCharByteWidth() const {
        return FExpr->getCharByteWidth();
    }

    StringLiteral::StringKind getKind() const {
        return FExpr->getKind();
    }
...
void Sema::CheckFloatComparison(SourceLocation Loc, Expr* LHS, Expr *RHS) {
    ...
    if (FloatingLiteral* FLL = dyn_cast<FloatingLiteral>(LeftExprSansParen)) {
        if (FLL->isExact())
            return;
    } else if (FloatingLiteral* FLR = dyn_cast<FloatingLiteral>(RightExprSansParen))
        if (FLR->isExact())
            return;
...
public:
    ...
    void VisitBinPtrMemD(const BinaryOperator *BO) {
        VisitBinPtrMem(BO);
    }
    void VisitBinPtrMemI(const BinaryOperator *BO) {
        VisitBinPtrMem(BO);
    }
    ...
    void VisitBinShl(const BinaryOperator *BO) {
        VisitBinShlShr(BO);
    }
    void VisitBinShr(const BinaryOperator *BO) {
        VisitBinShlShr(BO);
    }
