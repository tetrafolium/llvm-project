...
no_unroll_loops always_inline static inline uint64_t
...
static inline const void *align_ptr(const void *ptr, size_t bytes) {
  return (void *)((uintptr_t)ptr & -(uintptr_t)bytes);
}

always_inline static inline uint16_t fold_and_swap(uint64_t sum, bool swap) {
