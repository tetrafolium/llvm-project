...
public:
  ...
  inline bool immMskBitp(SDNode *inN) const {
    ...
    return (msksize >= 1 && msksize <= 8) || msksize == 16 || msksize == 24 ||
           msksize == 32;
  }
...
void XCoreDAGToDAGISel::Select(SDNode *N) {
  ...
  case ISD::Constant: {
    ...
    if (immMskBitp(N)) {
      ...
      SDNode *node =
          CurDAG->getMachineNode(XCore::LDWCP_lru6, dl, MVT::i32, MVT::Other,
...
static SDValue replaceInChain(SelectionDAG *CurDAG, SDValue Chain, SDValue Old,
                              SDValue New) {
  if (Chain == Old)
    return New;
...
bool XCoreDAGToDAGISel::tryBRIND(SDNode *N) {
  ...
  if (!CheckEventChainOut.use_empty()) {
    ...
    SDValue NewChain =
        replaceInChain(CurDAG, Chain, CheckEventChainOut, CheckEventChainIn);
    if (!NewChain.getNode())
      return false;
    Chain = NewChain;
  }
  ...
  SDValue Glue = SDValue(CurDAG->getMachineNode(XCore::SETSR_branch_u6, dl,
                                                MVT::Glue, constOne, Chain),
                         0);
  Glue = SDValue(CurDAG->getMachineNode(XCore::CLRSR_branch_u6, dl, MVT::Glue,
                                        constOne, Glue),
                 0);
