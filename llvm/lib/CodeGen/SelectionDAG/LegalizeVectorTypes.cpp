...
SDValue DAGTypeLegalizer::ScalarizeVecRes_FP_ROUND(SDNode *N) {
  ...
  return DAG.getNode(ISD::FP_ROUND, SDLoc(N), NewVT, Op, N->getOperand(1));
}

SDValue DAGTypeLegalizer::ScalarizeVecRes_FPOWI(SDNode *N) {
  SDValue Op = GetScalarizedVector(N->getOperand(0));
  return DAG.getNode(ISD::FPOWI, SDLoc(N), Op.getValueType(), Op,
                     N->getOperand(1));
}
...
void DAGTypeLegalizer::SplitVectorResult(SDNode *N, unsigned ResNo) {
  ...
  case ISD::INSERT_SUBVECTOR:
    ...
    break;
  case ISD::FPOWI:
    SplitVecRes_FPOWI(N, Lo, Hi);
    break;
  case ISD::FCOPYSIGN:
    SplitVecRes_FCOPYSIGN(N, Lo, Hi);
    break;
  case ISD::INSERT_VECTOR_ELT:
...
void DAGTypeLegalizer::SplitVecRes_FPOWI(SDNode *N, SDValue &Lo, SDValue &Hi) {
  SDLoc dl(N);
...
void DAGTypeLegalizer::SplitVecRes_MGATHER(MaskedGatherSDNode *MGT, SDValue &Lo,
                                           SDValue &Hi) {
  EVT LoVT, HiVT;
  SDLoc dl(MGT);
...
void DAGTypeLegalizer::SplitVecRes_VECTOR_SHUFFLE(ShuffleVectorSDNode *N,
  ...
  for (unsigned High = 0; High < 2; ++High) {
    ...
    if (useBuildVector) {
      ...
      SDValue Op1 =
          InputUsed[1] == -1U ? DAG.getUNDEF(NewVT) : Inputs[InputUsed[1]];
...
SDValue DAGTypeLegalizer::SplitVecOp_MGATHER(MaskedGatherSDNode *MGT,
  ...
  SDValue Res =
      DAG.getNode(ISD::CONCAT_VECTORS, dl, MGT->getValueType(0), Lo, Hi);
...
SDValue DAGTypeLegalizer::SplitVecOp_MSCATTER(MaskedScatterSDNode *N,
  ...
  Lo = DAG.getMaskedScatter(DAG.getVTList(MVT::Other), LoMemVT, DL, OpsLo, MMO,
                            N->getIndexType(), N->isTruncatingStore());
  ...
  return DAG.getMaskedScatter(DAG.getVTList(MVT::Other), HiMemVT, DL, OpsHi,
                              MMO, N->getIndexType(), N->isTruncatingStore());
}
...
SDValue DAGTypeLegalizer::WidenVecRes_SELECT(SDNode *N) {
  ...
  if (CondVT.isVector()) {
    if (SDValue WideCond = WidenVSELECTMask(N)) {
      ...
      return DAG.getNode(N->getOpcode(), SDLoc(N), WidenVT, WideCond, InOp1,
                         InOp2);
    }

    EVT CondEltVT = CondVT.getVectorElementType();
    EVT CondWidenVT =
        EVT::getVectorVT(*DAG.getContext(), CondEltVT, WidenNumElts);
...
SDValue DAGTypeLegalizer::WidenVecOp_EXTEND(SDNode *N) {
  ...
  if (InVT.getSizeInBits() != VT.getSizeInBits()) {
    EVT InEltVT = InVT.getVectorElementType();
    for (int i = MVT::FIRST_VECTOR_VALUETYPE, e = MVT::LAST_VECTOR_VALUETYPE;
         i < e; ++i) {
...
SDValue DAGTypeLegalizer::WidenVecOp_Convert(SDNode *N) {
  ...
  EVT WideVT =
      EVT::getVectorVT(*DAG.getContext(), EltVT, InVT.getVectorNumElements());
...
static EVT FindMemType(SelectionDAG &DAG, const TargetLowering &TLI,
                       unsigned Width, EVT WidenVT, unsigned Align = 0,
                       unsigned WidenEx = 0) {
  EVT WidenEltVT = WidenVT.getVectorElementType();
