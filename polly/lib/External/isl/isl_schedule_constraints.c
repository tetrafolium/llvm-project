...
isl_schedule_constraints_alloc(isl_ctx *ctx) {
 ...
 */
static __isl_give isl_schedule_constraints *
isl_schedule_constraints_init(__isl_take isl_schedule_constraints *sc) {
  isl_space *space;
  isl_union_map *empty;
  enum isl_edge_type i;
 ...
 */
static __isl_give isl_schedule_constraints *
isl_schedule_constraints_set_domain(__isl_take isl_schedule_constraints *sc,
 ...
 */
static __isl_give isl_schedule_constraints *
isl_schedule_constraints_set(__isl_take isl_schedule_constraints *sc,
...
__isl_give isl_schedule_constraints *isl_schedule_constraints_set_coincidence(
  ...
  return isl_schedule_constraints_set(sc, isl_edge_coincidence, coincidence);
}
...
isl_schedule_constraints_set_conditional_validity(
  ...
  sc =
      isl_schedule_constraints_set(sc, isl_edge_conditional_validity, validity);
  return sc;
}
...
isl_schedule_constraints_get(__isl_keep isl_schedule_constraints *sc,
                             enum isl_edge_type type) {
  if (!sc)
    return NULL;
 ...
 */
__isl_give isl_schedule_constraints *
isl_schedule_constraints_add(__isl_take isl_schedule_constraints *sc,
                             enum isl_edge_type type,
                             __isl_take isl_union_map *c) {
  if (!sc || !c)
    goto error;
...
apply_factor_domain(__isl_take isl_union_map *c,
 ...
 */
static __isl_give isl_union_map *
apply(__isl_take isl_union_map *c, __isl_keep isl_union_map *umap, int tag) {
  isl_union_map *t;
...
print_constraint(__isl_take isl_printer *p,
                 __isl_keep isl_schedule_constraints *sc,
                 enum isl_edge_type type) {
  isl_bool empty;
...
__isl_give isl_printer *isl_printer_print_schedule_constraints(
  ...
  p = print_yaml_field_union_set(p, key_str[isl_sc_key_domain], sc->domain);
  universe = isl_set_plain_is_universe(sc->context);
  if (universe < 0)
    return isl_printer_free(p);
  if (!universe)
    p = print_yaml_field_set(p, key_str[isl_sc_key_context], sc->context);
 ...
 */
__isl_give isl_schedule_constraints *
isl_stream_read_schedule_constraints(isl_stream *s) {
  isl_ctx *ctx;
  isl_schedule_constraints *sc;
  int more;
  int domain_set = 0;
...
isl_schedule_constraints_align_params(__isl_take isl_schedule_constraints *sc) {
  ...
  for (i = isl_edge_first; i <= isl_edge_last; ++i) {
    sc->constraint[i] =
        isl_union_map_align_params(sc->constraint[i], isl_space_copy(space));
    if (!sc->constraint[i])
...
int isl_schedule_constraints_n_basic_map(
  ...
  if (!sc)
    return -1;
  for (i = isl_edge_first; i <= isl_edge_last; ++i)
    if (isl_union_map_foreach_map(sc->constraint[i], &add_n_basic_map, &n) < 0)
      return -1;
 ...
 */
isl_size
isl_schedule_constraints_n_map(__isl_keep isl_schedule_constraints *sc) {
  enum isl_edge_type i;
  int n = 0;
