...
isl_schedule_tree_alloc(isl_ctx *ctx, enum isl_schedule_node_type type) {
...
isl_schedule_tree_dup(__isl_keep isl_schedule_tree *tree) {
  isl_ctx *ctx;
  ...
  case isl_schedule_node_error:
    isl_die(ctx, isl_error_internal, "allocation should have failed",
  ...
  case isl_schedule_node_expansion:
    dup->contraction = isl_union_pw_multi_aff_copy(tree->contraction);
...
isl_schedule_tree_cow(__isl_take isl_schedule_tree *tree) {
  if (!tree)
    return NULL;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_copy(__isl_keep isl_schedule_tree *tree) {
  if (!tree)
    return NULL;
 ...
 */
__isl_null isl_schedule_tree *
isl_schedule_tree_free(__isl_take isl_schedule_tree *tree) {
  if (!tree)
    return NULL;
  if (--tree->ref > 0)
    return NULL;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_from_band(__isl_take isl_schedule_band *band) {
  isl_ctx *ctx;
  isl_schedule_tree *tree;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_from_context(__isl_take isl_set *context) {
  isl_ctx *ctx;
  isl_schedule_tree *tree;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_from_domain(__isl_take isl_union_set *domain) {
  isl_ctx *ctx;
  isl_schedule_tree *tree;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_from_expansion(__isl_take isl_union_pw_multi_aff *contraction,
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_from_extension(__isl_take isl_union_map *extension) {
  isl_ctx *ctx;
  isl_schedule_tree *tree;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_from_filter(__isl_take isl_union_set *filter) {
  isl_ctx *ctx;
  isl_schedule_tree *tree;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_from_guard(__isl_take isl_set *guard) {
  isl_ctx *ctx;
  isl_schedule_tree *tree;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_from_mark(__isl_take isl_id *mark) {
  isl_ctx *ctx;
  isl_schedule_tree *tree;
 ...
 */
isl_bool
isl_schedule_tree_is_subtree_anchored(__isl_keep isl_schedule_tree *tree) {
...
int isl_schedule_tree_is_anchored(__isl_keep isl_schedule_tree *tree) {
  ...
  isl_die(isl_schedule_tree_get_ctx(tree), isl_error_internal, "unhandled case",
          return -1);
}
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_update_anchored(__isl_take isl_schedule_tree *tree) {
  int i;
  isl_size n;
  int anchored;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_from_children(enum isl_schedule_node_type type,
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_from_pair(enum isl_schedule_node_type type,
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_sequence_pair(__isl_take isl_schedule_tree *tree1,
                                __isl_take isl_schedule_tree *tree2) {
  return isl_schedule_tree_from_pair(isl_schedule_node_sequence, tree1, tree2);
}
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_set_pair(__isl_take isl_schedule_tree *tree1,
                           __isl_take isl_schedule_tree *tree2) {
...
isl_schedule_tree_get_type(__isl_keep isl_schedule_tree *tree) {
...
isl_bool isl_schedule_tree_plain_is_equal(__isl_keep isl_schedule_tree *tree1,
  ...
  switch (tree1->type) {
  case isl_schedule_node_band:
    equal = isl_schedule_band_plain_is_equal(tree1->band, tree2->band);
    break;
  case isl_schedule_node_context:
  ...
  case isl_schedule_node_domain:
    ...
    break;
  case isl_schedule_node_expansion:
    equal = isl_union_map_is_equal(tree1->expansion, tree2->expansion);
    if (equal >= 0 && equal)
      equal = isl_union_pw_multi_aff_plain_is_equal(tree1->contraction,
                                                    tree2->contraction);
    break;
  case isl_schedule_node_extension:
    equal = isl_union_map_is_equal(tree1->extension, tree2->extension);
    break;
  case isl_schedule_node_filter:
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_get_child(__isl_keep isl_schedule_tree *tree, int pos) {
  if (!tree)
    return NULL;
  if (!tree->children)
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_child(__isl_take isl_schedule_tree *tree, int pos) {
  isl_schedule_tree *child;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_reset_children(__isl_take isl_schedule_tree *tree) {
  tree = isl_schedule_tree_cow(tree);
  if (!tree)
    return NULL;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_drop_child(__isl_take isl_schedule_tree *tree, int pos) {
  isl_size n;
  ...
  if (pos < 0 || pos >= n)
    isl_die(isl_schedule_tree_get_ctx(tree), isl_error_invalid,
            "position out of bounds", return isl_schedule_tree_free(tree));
  if (n == 1)
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_replace_child(__isl_take isl_schedule_tree *tree, int pos,
  ...
  if (isl_schedule_tree_is_leaf(child)) {
    ...
    if (n != 1)
      isl_die(isl_schedule_tree_get_ctx(tree), isl_error_internal,
              "can only replace single child by leaf", goto error);
    return isl_schedule_tree_reset_children(tree);
  }

  if (!tree->children && pos == 0)
    tree->children = isl_schedule_tree_list_from_schedule_tree(child);
  else
    tree->children =
        isl_schedule_tree_list_set_schedule_tree(tree->children, pos, child);
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_set_children(__isl_take isl_schedule_tree *tree,
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_insert_band(__isl_take isl_schedule_tree *tree,
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_insert_context(__isl_take isl_schedule_tree *tree,
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_insert_domain(__isl_take isl_schedule_tree *tree,
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_insert_extension(__isl_take isl_schedule_tree *tree,
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_insert_filter(__isl_take isl_schedule_tree *tree,
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_children_insert_filter(__isl_take isl_schedule_tree *tree,
                                         __isl_take isl_union_set *filter) {
  int i;
  isl_size n;
  ...
  for (i = 0; i < n; ++i) {
    ...
    child = isl_schedule_tree_get_child(tree, i);
    child = isl_schedule_tree_insert_filter(child, isl_union_set_copy(filter));
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_insert_guard(__isl_take isl_schedule_tree *tree,
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_insert_mark(__isl_take isl_schedule_tree *tree,
 ...
 */
isl_bool
isl_schedule_tree_band_member_get_coincident(__isl_keep isl_schedule_tree *tree,
                                             int pos) {
  if (!tree)
    return isl_bool_error;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_band_member_set_coincident(__isl_take isl_schedule_tree *tree,
  ...
  if (isl_schedule_tree_band_member_get_coincident(tree, pos) == coincident)
    return tree;
  tree = isl_schedule_tree_cow(tree);
  if (!tree)
    return NULL;
  ...
  tree->band =
      isl_schedule_band_member_set_coincident(tree->band, pos, coincident);
  if (!tree->band)
    return isl_schedule_tree_free(tree);
  return tree;
}
 ...
 */
isl_bool
isl_schedule_tree_band_get_permutable(__isl_keep isl_schedule_tree *tree) {
  if (!tree)
    return isl_bool_error;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_band_set_permutable(__isl_take isl_schedule_tree *tree,
                                      int permutable) {
  if (!tree)
    return NULL;
  if (tree->type != isl_schedule_node_band)
...
isl_schedule_tree_band_get_space(__isl_keep isl_schedule_tree *tree) {
  if (!tree)
    return NULL;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_band_intersect_domain(__isl_take isl_schedule_tree *tree,
                                        __isl_take isl_union_set *domain) {
  if (!tree || !domain)
    goto error;
...
__isl_give isl_schedule_tree *isl_schedule_tree_band_set_partial_schedule(
  ...
  if (tree->type != isl_schedule_node_band)
    isl_die(isl_schedule_tree_get_ctx(tree), isl_error_invalid,
            "not a band node", return NULL);
  tree->band = isl_schedule_band_set_partial_schedule(tree->band, schedule);
...
__isl_give isl_schedule_tree *isl_schedule_tree_band_member_set_ast_loop_type(
    __isl_take isl_schedule_tree *tree, int pos, enum isl_ast_loop_type type) {
  tree = isl_schedule_tree_cow(tree);
  if (!tree)
    return NULL;
  ...
  tree->band =
      isl_schedule_band_member_set_ast_loop_type(tree->band, pos, type);
  if (!tree->band)
    return isl_schedule_tree_free(tree);
...
enum isl_ast_loop_type isl_schedule_tree_band_member_get_isolate_ast_loop_type(
  ...
  return isl_schedule_band_member_get_isolate_ast_loop_type(tree->band, pos);
}
...
isl_schedule_tree_band_member_set_isolate_ast_loop_type(
  ...
  tree->band =
      isl_schedule_band_member_set_isolate_ast_loop_type(tree->band, pos, type);
  if (!tree->band)
    return isl_schedule_tree_free(tree);
...
__isl_give isl_schedule_tree *isl_schedule_tree_band_set_ast_build_options(
  ...
  tree->band = isl_schedule_band_set_ast_build_options(tree->band, options);
  if (!tree->band)
    return isl_schedule_tree_free(tree);
 ...
 */
__isl_give isl_set *
isl_schedule_tree_context_get_context(__isl_keep isl_schedule_tree *tree) {
  if (!tree)
    return NULL;
...
isl_schedule_tree_domain_get_domain(__isl_keep isl_schedule_tree *tree) {
  if (!tree)
    return NULL;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_domain_set_domain(__isl_take isl_schedule_tree *tree,
                                    __isl_take isl_union_set *domain) {
  tree = isl_schedule_tree_cow(tree);
  if (!tree || !domain)
    goto error;
...
isl_schedule_tree_expansion_get_expansion(__isl_keep isl_schedule_tree *tree) {
  if (!tree)
    return NULL;
 ...
 */
__isl_give isl_union_map *
isl_schedule_tree_extension_get_extension(__isl_take isl_schedule_tree *tree) {
  if (!tree)
    return NULL;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_extension_set_extension(__isl_take isl_schedule_tree *tree,
                                          __isl_take isl_union_map *extension) {
  tree = isl_schedule_tree_cow(tree);
  if (!tree || !extension)
    goto error;
 ...
 */
__isl_give isl_union_set *
isl_schedule_tree_filter_get_filter(__isl_keep isl_schedule_tree *tree) {
  if (!tree)
    return NULL;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_filter_set_filter(__isl_take isl_schedule_tree *tree,
                                    __isl_take isl_union_set *filter) {
  tree = isl_schedule_tree_cow(tree);
  if (!tree || !filter)
    goto error;
 ...
 */
__isl_give isl_set *
isl_schedule_tree_guard_get_guard(__isl_take isl_schedule_tree *tree) {
  if (!tree)
    return NULL;
 ...
 */
__isl_give isl_id *
isl_schedule_tree_mark_get_id(__isl_keep isl_schedule_tree *tree) {
  if (!tree)
    return NULL;
...
subtree_schedule_extend(__isl_keep isl_schedule_tree *tree,
 ...
 */
static __isl_give isl_union_map *
subtree_schedule_extend_child(__isl_keep isl_schedule_tree *tree,
                              __isl_take isl_union_map *outer) {
  isl_schedule_tree *child;
  isl_union_map *res;
...
extract_space_from_filter_child(__isl_keep isl_schedule_tree *tree) {
  isl_space *space;
  isl_union_set *dom;
  isl_schedule_tree *child;
 ...
 */
static __isl_give isl_union_map *
subtree_schedule_extend_from_children(__isl_keep isl_schedule_tree *tree,
                                      __isl_take isl_union_map *outer) {
  int i;
  isl_size n;
  isl_size dim;
  ...
  for (i = 0; i < n; ++i) {
    ...
    child = isl_schedule_tree_list_get_schedule_tree(tree->children, i);
    dom = isl_schedule_tree_filter_get_filter(child);
    ...
    umap_i =
        isl_union_map_flat_range_product(isl_union_map_copy(outer), umap_i);
...
initial_domain(__isl_keep isl_schedule_tree *tree);
 ...
 */
static __isl_give isl_union_set *
initial_domain_from_children(__isl_keep isl_schedule_tree *tree) {
  int i;
  isl_size n;
  isl_space *space;
  isl_union_set *domain;
...
initial_domain(__isl_keep isl_schedule_tree *tree) {
  ...
  case isl_schedule_node_context:
    isl_die(isl_schedule_tree_get_ctx(tree), isl_error_internal,
            "context node should be handled by caller", return NULL);
  case isl_schedule_node_guard:
    isl_die(isl_schedule_tree_get_ctx(tree), isl_error_internal,
            "guard node should be handled by caller", return NULL);
  case isl_schedule_node_mark:
    isl_die(isl_schedule_tree_get_ctx(tree), isl_error_internal,
            "mark node should be handled by caller", return NULL);
  case isl_schedule_node_extension:
    isl_die(isl_schedule_tree_get_ctx(tree), isl_error_invalid,
            "cannot construct subtree schedule of tree "
            "with extension nodes",
  ...
  case isl_schedule_node_band:
    ...
    if (n == 0)
      isl_die(isl_schedule_tree_get_ctx(tree), isl_error_internal,
              "0D band should be handled by caller", return NULL);
    mupa = isl_schedule_band_get_partial_schedule(tree->band);
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_band_scale(__isl_take isl_schedule_tree *tree,
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_band_scale_down(__isl_take isl_schedule_tree *tree,
                                  __isl_take isl_multi_val *mv) {
  if (!tree || !mv)
    goto error;
  if (tree->type != isl_schedule_node_band)
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_band_mod(__isl_take isl_schedule_tree *tree,
                           __isl_take isl_multi_val *mv) {
  if (!tree || !mv)
    goto error;
  if (tree->type != isl_schedule_node_band)
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_band_shift(__isl_take isl_schedule_tree *tree,
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_sequence_splice(__isl_take isl_schedule_tree *tree, int pos,
                                  __isl_take isl_schedule_tree *child) {
  isl_size n;
  ...
  list1 = isl_schedule_tree_list_concat(
      list1, isl_schedule_tree_list_copy(child->children));
  list1 = isl_schedule_tree_list_concat(list1, list2);

  isl_schedule_tree_free(tree);
  isl_schedule_tree_free(child);
  return isl_schedule_tree_from_children(isl_schedule_node_sequence, list1);
error:
  isl_schedule_tree_free(tree);
  isl_schedule_tree_free(child);
  return NULL;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_band_tile(__isl_take isl_schedule_tree *tree,
  ...
  tree->band = isl_schedule_band_tile(tree->band, isl_multi_val_copy(sizes));
  if (!tree->band)
    goto error;
 ...
 */
static __isl_give isl_set *isolate_initial(__isl_keep isl_set *isolate, int pos,
                                           int n) {
  isl_id *id;
  isl_map *map;
 ...
 */
static __isl_give isl_set *isolate_final(__isl_keep isl_set *isolate, int pos,
                                         int n) {
  isl_id *id;
  isl_space *space;
  ...
  ma1 =
      isl_multi_aff_project_out_map(isl_space_copy(space), isl_dim_set, pos, n);
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_band_split(__isl_take isl_schedule_tree *tree, int pos,
                             int depth) {
  isl_size n;
  ...
  child->band = isl_schedule_band_replace_ast_build_option(
      child->band, isl_set_copy(isolate), child_isolate);
  tree->band = isl_schedule_band_drop(tree->band, pos, n - pos);
  tree->band = isl_schedule_band_replace_ast_build_option(
      tree->band, isl_set_copy(isolate), tree_isolate);
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_append_to_leaves(__isl_take isl_schedule_tree *tree1,
                                   __isl_take isl_schedule_tree *tree2) {
  int i;
  isl_size n;
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_reset_user(__isl_take isl_schedule_tree *tree) {
  ...
  case isl_schedule_node_domain:
    ...
    break;
  case isl_schedule_node_expansion:
    tree->contraction = isl_union_pw_multi_aff_reset_user(tree->contraction);
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_align_params(__isl_take isl_schedule_tree *tree,
  ...
  case isl_schedule_node_domain:
    ...
    break;
  case isl_schedule_node_expansion:
    tree->contraction = isl_union_pw_multi_aff_align_params(
        tree->contraction, isl_space_copy(space));
    tree->expansion = isl_union_map_align_params(tree->expansion, space);
    if (!tree->contraction || !tree->expansion)
      return isl_schedule_tree_free(tree);
    break;
  case isl_schedule_node_extension:
    tree->extension = isl_union_map_align_params(tree->extension, space);
    if (!tree->extension)
      return isl_schedule_tree_free(tree);
    break;
  case isl_schedule_node_filter:
...
__isl_give isl_schedule_tree *isl_schedule_tree_pullback_union_pw_multi_aff(
  ...
  if (tree->type == isl_schedule_node_band) {
    tree->band =
        isl_schedule_band_pullback_union_pw_multi_aff(tree->band, upma);
    if (!tree->band)
      return isl_schedule_tree_free(tree);
  } else if (tree->type == isl_schedule_node_domain) {
    tree->domain =
        isl_union_set_preimage_union_pw_multi_aff(tree->domain, upma);
    if (!tree->domain)
      return isl_schedule_tree_free(tree);
    ...
    tree->extension =
        isl_union_map_preimage_range_union_pw_multi_aff(tree->extension, upma);
    if (!tree->extension)
      return isl_schedule_tree_free(tree);
  } else if (tree->type == isl_schedule_node_filter) {
    tree->filter =
        isl_union_set_preimage_union_pw_multi_aff(tree->filter, upma);
    if (!tree->filter)
      return isl_schedule_tree_free(tree);
  }
 ...
 */
__isl_give isl_schedule_tree *
isl_schedule_tree_band_gist(__isl_take isl_schedule_tree *tree,
...
print_tree_band(__isl_take isl_printer *p, __isl_keep isl_schedule_band *band) {
  ...
  if (coincident) {
    ...
    if (n < 0)
      return isl_printer_free(p);
    for (i = 0; i < n; ++i) {
      p = isl_printer_print_int(
          p, isl_schedule_band_member_get_coincident(band, i));
      p = isl_printer_yaml_next(p);
    }
...
isl_printer_print_schedule_tree_mark(__isl_take isl_printer *p,
                                     __isl_keep isl_schedule_tree *tree,
  ...
  case isl_schedule_node_guard:
    ...
    break;
  case isl_schedule_node_mark:
    p = print_yaml_field_str(p, "mark", isl_id_get_name(tree->mark));
    break;
  case isl_schedule_node_band:
  ...
  if (n == 0) {
    if (n_ancestor > 0 && block) {
      ...
      p = isl_printer_print_schedule_tree_mark(p, leaf, 0, NULL);
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (n_ancestor > 0 && child_pos[0] == i)
      p = isl_printer_print_schedule_tree_mark(p, t, n_ancestor - 1,
                                               child_pos + 1);
    else
      p = isl_printer_print_schedule_tree_mark(p, t, -1, NULL);
 ...
 */
__isl_give isl_printer *
isl_printer_print_schedule_tree(__isl_take isl_printer *p,
                                __isl_keep isl_schedule_tree *tree) {
