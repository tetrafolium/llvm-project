...
bool fixupX86StructRetCalls(llvm::Module &module) {
  ...
  for (auto call_inst : rs_callsites) {
    ...
    llvm::AllocaInst *return_value_alloc =
        new llvm::AllocaInst(func->getReturnType(), DL.getAllocaAddrSpace(),
                             "var_vector_return_alloc", call_inst);
    ...
    llvm::AllocaInst *new_func_ptr = new llvm::AllocaInst(
        new_func_ptr_type, DL.getAllocaAddrSpace(), "new_func_ptr", call_inst);
