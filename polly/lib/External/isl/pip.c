...
struct isl_arg_choice pip_format[] = {
    {"set", FORMAT_SET}, {"affine", FORMAT_AFF}, {0}};

ISL_ARGS_START(struct options, options_args)
ISL_ARG_CHILD(struct options, isl, "isl", &isl_options_args, "isl options")
ISL_ARG_BOOL(struct options, verify, 'T', "verify", 0, NULL)
ISL_ARG_CHOICE(struct options, format, 0, "format", pip_format, FORMAT_SET,
               "output format")
ISL_ARGS_END
...
to_parameter_domain(__isl_take isl_basic_set *context) {
  isl_size dim;
  ...
  context =
      isl_basic_set_move_dims(context, isl_dim_param, 0, isl_dim_set, 0, dim);
 ...
 */
static __isl_give isl_basic_set *
move_parameters(__isl_take isl_basic_set *bset,
  ...
  if (dim < 0)
    return isl_basic_set_free(bset);
  bset = isl_basic_set_move_dims(bset, isl_dim_param, 0, isl_dim_set,
                                 dim - nparam, nparam);
  return bset;
}
 ...
 */
static __isl_give isl_basic_set *
plug_in_parameters(__isl_take isl_basic_set *bset, __isl_take isl_vec *params) {
  int i;
  isl_size n;
  ...
  for (i = 0; i < n; ++i)
    bset = isl_basic_set_fix(bset, isl_dim_param, i, params->el[1 + i]);
...
static __isl_give isl_vec *opt_at(__isl_take isl_basic_set *bset,
  ...
  for (i = 0; i < dim; ++i) {
    ...
    res = isl_basic_set_solve_ilp(bset, max, obj->el, &opt->el[1 + i], NULL);
    if (res == isl_lp_empty)
      goto empty;
...
int main(int argc, char **argv) {
  ...
  if (!urs_parms)
    context = isl_basic_set_intersect(
        context,
  ...
  if (!urs_unknowns)
    bset = isl_basic_set_intersect(
        bset, isl_basic_set_positive_orthant(isl_basic_set_get_space(bset)));
  ...
  if (options->format == FORMAT_AFF) {
    if (max)
      pma = isl_basic_set_partial_lexmax_pw_multi_aff(bset, context, &empty);
    else
      pma = isl_basic_set_partial_lexmin_pw_multi_aff(bset, context, &empty);
  } else {
    if (max)
