...
int IoStatementBase::EndIoStatement() {
    return GetIoStat();
}
...
InternalFormattedIoStatementState<DIR, CHAR>::InternalFormattedIoStatementState(
    ...
    : InternalIoStatementState<DIR, CHAR> {
    buffer, length, sourceFile,
            sourceLine
},
...
InternalListIoStatementState<DIR, CHAR>::InternalListIoStatementState(
    ...
    : InternalIoStatementState<DIR, CharType> {
    buffer, length, sourceFile,
            sourceLine
},
...
ConnectionState &ExternalIoStatementBase::GetConnectionState() {
    return unit_;
}
...
std::optional<DataEdit> IoStatementState::GetNextDataEdit(int n) {
    return std::visit(
    [&](auto &x) {
        return x.get().GetNextDataEdit(*this, n);
    }, u_);
}
...
void IoStatementState::BeginReadingRecord() {
    std::visit([](auto &x) {
        return x.get().BeginReadingRecord();
    }, u_);
}
...
ExternalFileUnit *IoStatementState::GetExternalFileUnit() const {
    return std::visit([](auto &x) {
        return x.get().GetExternalFileUnit();
    }, u_);
}
...
bool IoStatementState::Inquire(
    ...
    return std::visit(
    [&](auto &x) {
        return x.get().Inquire(inquiry, out, chars);
    }, u_);
}

bool IoStatementState::Inquire(InquiryKeywordHash inquiry, bool &out) {
    return std::visit([&](auto &x) {
        return x.get().Inquire(inquiry, out);
    }, u_);
}
