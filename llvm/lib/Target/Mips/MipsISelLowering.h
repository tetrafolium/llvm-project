...
protected:
  ...
  SDValue getAddrGlobalLargeGOT(NodeTy *N, const SDLoc &DL, EVT Ty,
                                ...
                                const MachinePointerInfo &PtrInfo) const {
    SDValue Hi =
        DAG.getNode(MipsISD::GotHi, DL, Ty, getTargetNode(N, Ty, DAG, HiFlag));
  ...
  SDValue getAddrNonPIC(NodeTy *N, const SDLoc &DL, EVT Ty,
    ...
    return DAG.getNode(ISD::ADD, DL, Ty, DAG.getNode(MipsISD::Hi, DL, Ty, Hi),
  ...
  // (add $gp, %gp_rel(sym))
  template <class NodeTy>
  SDValue getAddrGPRel(NodeTy *N, const SDLoc &DL, EVT Ty, SelectionDAG &DAG,
                       bool IsN64) const {
    SDValue GPRel = getTargetNode(N, Ty, DAG, MipsII::MO_GPREL);
    return DAG.getNode(
        ISD::ADD, DL, Ty, DAG.getRegister(IsN64 ? Mips::GP_64 : Mips::GP, Ty),
  ...
  virtual void getOpndList(SmallVectorImpl<SDValue> &Ops,
                           ...
                           bool InternalLinkage, bool IsCallReloc,
