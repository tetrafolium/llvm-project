...
void applyR1(GraphT &G, typename GraphT::NodeId NId) {
  ...
  assert(G.getNodeDegree(NId) == 1 && "R1 applied to node with degree != 1.");
...
void applyR2(GraphT &G, typename GraphT::NodeId NId) {
  ...
  assert(G.getNodeDegree(NId) == 2 && "R2 applied to node with degree != 2.");

  const Vector &XCosts = G.getNodeCosts(NId);

  typename GraphT::AdjEdgeItr AEItr = G.adjEdgeIds(NId).begin();
  EdgeId YXEId = *AEItr, ZXEId = *(++AEItr);
  ...
  unsigned XLen = XCosts.getLength(), YLen = YXECosts->getRows(),
...
template <typename VectorT> bool hasRegisterOptions(const VectorT &V) {
...
Solution backpropagate(GraphT &G, StackT stack) {
  ...
  while (!stack.empty()) {
    ...
    // register, spilling it may provide a lower cost solution. Assert here that
    // spilling is done by choice, not because there were no register available.
