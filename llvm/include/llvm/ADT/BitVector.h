...
public:
    ...
    int find_first_unset() const {
        return find_first_unset_in(0, Size);
    }
    ...
    int find_last_unset() const {
        return find_last_unset_in(0, Size);
    }
...
public:
    ...
    size_t getMemorySize() const {
        return Bits.size() * sizeof(BitWord);
    }
    size_t getBitCapacity() const {
        return Bits.size() * BITWORD_SIZE;
    }
};
...
template <> struct DenseMapInfo<BitVector> {
    static inline BitVector getEmptyKey() {
        return BitVector();
    }
