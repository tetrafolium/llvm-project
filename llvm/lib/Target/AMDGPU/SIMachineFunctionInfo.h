...
class SIMachineFunctionInfo final : public AMDGPUMachineFunction {
  ...
  DenseMap<const Value *, std::unique_ptr<const AMDGPUBufferPseudoSourceValue>>
      BufferPSVs;
  DenseMap<const Value *, std::unique_ptr<const AMDGPUImagePseudoSourceValue>>
      ImagePSVs;
...
public:
  ...
  ArrayRef<SpilledReg> getSGPRToVGPRSpills(int FrameIndex) const {
    ...
    return (I == SGPRToVGPRSpills.end()) ? ArrayRef<SpilledReg>()
                                         : makeArrayRef(I->second);
  }

  ArrayRef<SGPRSpillVGPRCSR> getSGPRSpillVGPRs() const { return SpillVGPRs; }
  ...
  unsigned getGDSSize() const { return GDSSize; }
  ...
  unsigned getLDSWaveSpillSize() const { return LDSWaveSpillSize; }
  ...
  const AMDGPUBufferPseudoSourceValue *getBufferPSV(const SIInstrInfo &TII,
    ...
    auto PSV = BufferPSVs.try_emplace(
        BufferRsrc, std::make_unique<AMDGPUBufferPseudoSourceValue>(TII));
  ...
  const AMDGPUImagePseudoSourceValue *getImagePSV(const SIInstrInfo &TII,
    ...
    auto PSV = ImagePSVs.try_emplace(
        ImgRsrc, std::make_unique<AMDGPUImagePseudoSourceValue>(TII));
    return PSV.first->second.get();
  }
