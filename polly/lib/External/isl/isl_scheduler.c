...
static int graph_init_table(isl_ctx *ctx, struct isl_sched_graph *graph) {
  ...
  for (i = 0; i < graph->n; ++i) {
    ...
    entry = isl_hash_table_find(ctx, graph->node_table, hash, &node_has_tuples,
 ...
 */
static int is_node(struct isl_sched_graph *graph, struct isl_sched_node *node) {
...
static int graph_init_edge_tables(isl_ctx *ctx, struct isl_sched_graph *graph) {
  ...
  for (i = 0; i <= isl_edge_last; ++i) {
    graph->edge_table[i] = isl_hash_table_alloc(ctx, graph->max_edge[i]);
...
graph_find_edge_entry(struct isl_sched_graph *graph, enum isl_edge_type type,
 ...
 * Return NULL on error.
 */
static struct isl_sched_edge *
graph_find_matching_edge(struct isl_sched_graph *graph,
...
static isl_stat graph_alloc(isl_ctx *ctx, struct isl_sched_graph *graph,
  ...
  graph->region = isl_alloc_array(ctx, struct isl_trivial_region, graph->n);
  graph->edge = isl_calloc_array(ctx, struct isl_sched_edge, graph->n_edge);
...
static isl_bool has_any_defining_equality(__isl_keep isl_basic_set *bset) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    has = isl_basic_set_has_defining_equality(bset, isl_dim_set, i, NULL);
    if (has < 0 || has)
      return has;
  }
...
static isl_stat compute_max_coefficient(isl_ctx *ctx,
  ...
  for (i = 0; i < node->nvar; ++i) {
    ...
    for (j = 0; j < node->nvar; ++j) {
      if (j == i)
        continue;
      if (isl_int_is_neg(v->el[j]) || isl_int_gt(v->el[j], size->n))
...
static isl_stat compress_node(struct isl_sched_node *node,
  ...
  if (!node->compressed) {
    ...
    hull = isl_set_preimage_multi_aff(hull, isl_multi_aff_copy(node->compress));
    node->hull = isl_set_intersect(node->hull, hull);
    node->compress = isl_multi_aff_pullback_multi_aff(compress, node->compress);
    node->decompress =
        isl_pw_multi_aff_pullback_pw_multi_aff(node->decompress, decompress);
  }
...
static isl_stat project_out_fixed(struct isl_sched_node *node,
  ...
  pma = isl_pw_multi_aff_pullback_multi_aff(isl_pw_multi_aff_copy(decompress),
...
static isl_stat compute_sizes(struct isl_sched_node *node,
  ...
  if (node->compressed)
    set = isl_set_preimage_pw_multi_aff(
        set, isl_pw_multi_aff_copy(node->decompress));
 ...
 * if needed.
 */
static isl_stat add_node(struct isl_sched_graph *graph, __isl_take isl_set *set,
  ...
  if (is_condition(edge2)) {
    if (!edge1->tagged_condition)
      ...
      edge1->tagged_condition =
          isl_union_map_union(edge1->tagged_condition, edge2->tagged_condition);
  }
  ...
  if (is_conditional_validity(edge2)) {
    if (!edge1->tagged_validity)
      ...
      edge1->tagged_validity =
          isl_union_map_union(edge1->tagged_validity, edge2->tagged_validity);
  }
...
static isl_stat extract_edge(__isl_take isl_map *map, void *user) {
  ...
  if (data->type == isl_edge_conditional_validity)
    graph->edge[graph->n_edge].tagged_validity = isl_union_map_from_map(tagged);
...
static isl_stat graph_init(struct isl_sched_graph *graph,
  ...
  graph->n = 0;
  domain = isl_schedule_constraints_get_domain(sc);
  domain = isl_union_set_intersect_params(
      domain, isl_schedule_constraints_get_context(sc));
...
static __isl_give isl_map *compress(__isl_take isl_map *map,
                                    ...
                                    struct isl_sched_node *dst) {
  if (src->compressed)
    map = isl_map_preimage_domain_pw_multi_aff(
        map, isl_pw_multi_aff_copy(src->decompress));
  if (dst->compressed)
    map = isl_map_preimage_range_pw_multi_aff(
        map, isl_pw_multi_aff_copy(dst->decompress));
  return map;
}
...
drop_coalescing_constraints(__isl_take isl_set *delta,
                            struct isl_sched_node *node) {
  isl_size nparam;
  isl_basic_set *bounds;
...
intra_coefficients(struct isl_sched_graph *graph, struct isl_sched_node *node,
 ...
 */
static __isl_give isl_basic_set *
inter_coefficients(struct isl_sched_graph *graph, struct isl_sched_edge *edge,
...
static __isl_give isl_dim_map *inter_dim_map(isl_ctx *ctx,
  ...
  isl_dim_map_range(dim_map, pos, -2, offset + src->nvar, 1, dst->nvar, -s);
  isl_dim_map_range(dim_map, pos + 1, -2, offset + src->nvar, 1, dst->nvar, s);
 ...
 */
static __isl_give isl_basic_set *
add_constraints_dim_map(__isl_take isl_basic_set *dst,
...
extract_linear_schedule(struct isl_sched_node *node) {
  isl_size n_row = isl_mat_rows(node->sched);

  if (n_row < 0)
    return NULL;
  return isl_mat_sub_alloc(node->sched, 0, n_row, 1 + node->nparam, node->nvar);
}
 ...
 */
static isl_stat update_count(__isl_take isl_basic_set *bset, int f, int *n_eq,
                             int *n_ineq) {
  isl_size eq, ineq;
...
static isl_stat count_map_constraints(struct isl_sched_graph *graph,
                                      struct isl_sched_edge *edge,
                                      __isl_take isl_map *map, int *n_eq,
                                      int *n_ineq, int use_coincidence) {
  isl_map *copy;
  isl_basic_set *coef;
 ...
 * Otherwise, we ignore them.
 */
static int count_constraints(struct isl_sched_graph *graph, int *n_eq,
                             int *n_ineq, int use_coincidence) {
  int i;
  ...
  *n_eq = *n_ineq = 0;
  for (i = 0; i < graph->n_edge; ++i) {
    ...
    if (count_map_constraints(graph, edge, map, n_eq, n_ineq, use_coincidence) <
        0)
      return -1;
  }
 ...
 */
static isl_stat
add_bound_coefficient_constraints(isl_ctx *ctx, struct isl_sched_graph *graph) {
  int i;
  int max;
 ...
 */
static isl_stat add_sum_constraint(struct isl_sched_graph *graph, int sum_pos,
                                   int first, int n) {
  int i, k;
  isl_size total;
...
static __isl_give isl_vec *solve_lp(isl_ctx *ctx,
  ...
  sol = isl_tab_basic_set_non_trivial_lexmin(lp, 2, graph->n, graph->region,
                                             &check_conflict, graph);
  for (i = 0; i < graph->n; ++i)
...
static __isl_give isl_vec *extract_var_coef(struct isl_sched_node *node,
  ...
  for (i = 0; i < node->nvar; ++i)
    isl_int_sub(csol->el[node->nvar - 1 - i], sol->el[pos + 2 * i + 1],
                sol->el[pos + 2 * i]);
...
static int update_schedule(struct isl_sched_graph *graph,
  ...
  if (!sol)
    goto error;
  if (sol->size == 0)
    isl_die(sol->ctx, isl_error_internal, "no solution found", goto error);
  if (graph->n_total_row >= graph->max_row)
    isl_die(sol->ctx, isl_error_internal, "too many schedule rows", goto error);
  ...
  for (i = 0; i < graph->n; ++i) {
    ...
    if (!node->sched)
      goto error;
    pos = node_cst_coef_offset(node);
    node->sched = isl_mat_set_element(node->sched, row, 0, sol->el[1 + pos]);
    pos = node_par_coef_offset(node);
    for (j = 0; j < node->nparam; ++j)
      node->sched =
          isl_mat_set_element(node->sched, row, 1 + j, sol->el[1 + pos + j]);
    for (j = 0; j < node->nvar; ++j)
      node->sched = isl_mat_set_element(node->sched, row, 1 + node->nparam + j,
                                        csol->el[j]);
...
static __isl_give isl_aff *extract_schedule_row(__isl_take isl_local_space *ls,
  ...
  for (j = 0; j < node->nvar; ++j) {
    if (isl_mat_get_element(node->sched, row, 1 + node->nparam + j, &v) < 0)
      goto error;
...
node_extract_partial_schedule_multi_aff(struct isl_sched_node *node, int first,
                                        int n) {
  int i;
  isl_space *space;
  isl_local_space *ls;
 ...
 */
static __isl_give isl_multi_aff *
node_extract_schedule_multi_aff(struct isl_sched_node *node) {
  isl_size nrow;
...
intersect_domains(__isl_take isl_union_map *umap, __isl_keep isl_map *map) {
  isl_union_set *uset;
...
static isl_stat update_edge(isl_ctx *ctx, struct isl_sched_graph *graph,
  ...
  if (edge->tagged_condition) {
    edge->tagged_condition = intersect_domains(edge->tagged_condition, id);
    if (!edge->tagged_condition)
      goto error;
  }
  if (edge->tagged_validity) {
    edge->tagged_validity = intersect_domains(edge->tagged_validity, id);
    if (!edge->tagged_validity)
      goto error;
  }
 ...
 */
static int
unconditionalize_adjacent_validity(struct isl_sched_graph *graph,
...
isl_sched_graph_domain(isl_ctx *ctx, struct isl_sched_graph *graph,
                       int (*pred)(struct isl_sched_node *node, int data),
                       int data) {
  int i;
  isl_set *set;
  isl_union_set *dom;
  ...
  if (i >= graph->n)
    isl_die(ctx, isl_error_internal, "empty component", return NULL);
...
extract_sccs(isl_ctx *ctx, struct isl_sched_graph *graph) {
  int i;
 ...
 */
static __isl_give isl_union_set_list *
extract_split(isl_ctx *ctx, struct isl_sched_graph *graph) {
  isl_union_set *dom;
  isl_union_set_list *filters;

  filters = isl_union_set_list_alloc(ctx, 2);
  dom = isl_sched_graph_domain(ctx, graph, &node_scc_at_most, graph->src_scc);
  filters = isl_union_set_list_add(filters, dom);
  dom = isl_sched_graph_domain(ctx, graph, &node_scc_at_least,
                               graph->src_scc + 1);
  filters = isl_union_set_list_add(filters, dom);
 ...
 */
static isl_stat
copy_nodes(struct isl_sched_graph *dst, struct isl_sched_graph *src,
  ...
  for (i = 0; i < src->n; ++i) {
    ...
    dst->node[j].compress = isl_multi_aff_copy(src->node[i].compress);
    dst->node[j].decompress = isl_pw_multi_aff_copy(src->node[i].decompress);
 ...
 */
static isl_stat
copy_edges(isl_ctx *ctx, struct isl_sched_graph *dst,
  ...
  for (i = 0; i < src->n_edge; ++i) {
    ...
    if (!is_node(dst, dst_src) || !is_node(dst, dst_dst)) {
      if (is_validity(edge) || is_conditional_validity(edge))
        isl_die(ctx, isl_error_internal, "backward (conditional) validity edge",
                return isl_stat_error);
      continue;
    }
    ...
    if (graph_edge_tables_add(ctx, dst, &dst->edge[dst->n_edge - 1]) < 0)
      return isl_stat_error;
  }
 ...
 */
static isl_stat
extract_sub_graph(isl_ctx *ctx, struct isl_sched_graph *graph,
...
compute_schedule(isl_schedule_node *node, struct isl_sched_graph *graph);
static __isl_give isl_schedule_node *
compute_schedule_wcc(isl_schedule_node *node, struct isl_sched_graph *graph);
 ...
 */
static __isl_give isl_schedule_node *
compute_sub_schedule(__isl_take isl_schedule_node *node, isl_ctx *ctx,
                     struct isl_sched_graph *graph,
                     int (*node_pred)(struct isl_sched_node *node, int data),
                     int (*edge_pred)(struct isl_sched_edge *edge, int data),
                     int data, int wcc) {
  ...
  if (extract_sub_graph(ctx, graph, node_pred, edge_pred, data, &split) < 0)
    goto error;
...
static isl_stat reset_band(struct isl_sched_graph *graph) {
  ...
  for (i = 0; i < graph->n; ++i) {
    ...
    node->sched = isl_mat_drop_rows(node->sched, graph->band_start, drop);
 ...
 */
static __isl_give isl_schedule_node *
compute_split_schedule(__isl_take isl_schedule_node *node,
                       struct isl_sched_graph *graph) {
  int is_seq;
  isl_ctx *ctx;
  isl_union_set_list *filters;
  ...
  node = compute_sub_schedule(node, ctx, graph, &node_scc_at_least,
                              &edge_src_scc_at_least, graph->src_scc + 1, 0);
  ...
  node = isl_schedule_node_child(node, 0);
  node = isl_schedule_node_child(node, 0);
  node = compute_sub_schedule(node, ctx, graph, &node_scc_at_most,
                              &edge_dst_scc_at_most, graph->src_scc, 0);
  is_seq = isl_schedule_node_get_type(node) == isl_schedule_node_sequence;
  node = isl_schedule_node_parent(node);
  node = isl_schedule_node_parent(node);
  if (is_seq)
 ...
 */
static __isl_give isl_schedule_node *
insert_current_band(__isl_take isl_schedule_node *node,
  ...
  for (i = 1; i < graph->n; ++i) {
    ...
    ma = node_extract_partial_schedule_multi_aff(&graph->node[i], start, n);
    mpa = isl_multi_pw_aff_from_multi_aff(ma);
  ...
  for (i = 0; i < n; ++i)
    node = isl_schedule_node_band_member_set_coincident(
        node, i, graph->node[0].coincident[start + i]);
 ...
 */
static __isl_give isl_schedule_node *
compute_next_band(__isl_take isl_schedule_node *node,
                  struct isl_sched_graph *graph, int permutable) {
  isl_ctx *ctx;
 ...
 */
static struct isl_sched_node *
graph_find_compressed_node(isl_ctx *ctx, struct isl_sched_graph *graph,
                           __isl_keep isl_space *space) {
  isl_id *id;
  struct isl_sched_node *node;
  ...
  if (!is_node(graph->root, node))
    isl_die(ctx, isl_error_internal, "space points to invalid node",
            return NULL);
  if (graph != graph->root)
    node = graph_find_node(ctx, graph, node->space);
  if (!is_node(graph, node))
    isl_die(ctx, isl_error_internal, "unable to find node", return NULL);
...
static __isl_give isl_schedule_node *
compute_component_schedule(__isl_take isl_schedule_node *node,
 ...
 */
static __isl_give isl_schedule_node *
split_scaled(__isl_take isl_schedule_node *node,
             struct isl_sched_graph *graph) {
  int i;
  int row;
  isl_ctx *ctx;
  ...
  for (i = 0; i < graph->n; ++i) {
    ...
    isl_int_fdiv_q(node->sched->row[row][0], node->sched->row[row][0], gcd);
    isl_int_mul(node->sched->row[row][0], node->sched->row[row][0], gcd);
...
static int is_trivial(struct isl_sched_node *node, __isl_keep isl_vec *sol) {
  ...
  trivial = isl_seq_first_non_zero(node_sol->el, node->nvar - node->rank) == -1;
 ...
 */
static __isl_give isl_tab_lexmin *
zero_out_node_coef(__isl_take isl_tab_lexmin *tl, struct isl_sched_node *node,
                   int pos) {
  int dim;
  isl_ctx *ctx;
  isl_vec *eq;
...
static __isl_give isl_vec *non_empty_solution(__isl_keep isl_tab_lexmin *tl) {
  ...
  if (sol->size == 0)
    isl_die(isl_vec_get_ctx(sol), isl_error_internal,
            "error in schedule construction", return isl_vec_free(sol));
  return sol;
}
 ...
 */
static __isl_give isl_union_set *
union_drop_coalescing_constraints(isl_ctx *ctx, struct isl_sched_graph *graph,
 ...
 */
static isl_stat
add_non_trivial_lineality(__isl_take isl_basic_set *lineality,
 ...
 */
static __isl_give isl_union_set *
exploit_intra_lineality(__isl_take isl_union_set *intra,
                        struct isl_exploit_lineality_data *data) {
 ...
 */
static __isl_give isl_union_map *
exploit_inter_lineality(__isl_take isl_union_map *inter,
                        struct isl_exploit_lineality_data *data) {
  isl_union_map *umap;
  ...
  inter = isl_union_map_subtract_range(inter, isl_union_set_copy(data->mask));
  umap = isl_union_map_apply_range(umap, isl_union_map_copy(data->equivalent));
  inter = isl_union_map_union(inter, umap);
  umap = isl_union_map_copy(inter);
  inter = isl_union_map_subtract_domain(inter, isl_union_set_copy(data->mask));
  umap = isl_union_map_apply_range(isl_union_map_copy(data->equivalent), umap);
  inter = isl_union_map_union(inter, umap);
 ...
 */
static __isl_give isl_union_map *collect_validity(
    struct isl_sched_graph *graph,
  ...
  for (i = 0; i < graph->n_edge; ++i) {
    ...
    if (!is_any_validity(edge) && (!coincidence || !is_coincidence(edge)))
      continue;
 ...
 */
static __isl_give isl_basic_set_list *
collect_intra_validity(isl_ctx *ctx, struct isl_sched_graph *graph,
                       int coincidence,
                       struct isl_exploit_lineality_data *data) {
  isl_union_map *intra;
  isl_union_set *delta;
 ...
 */
static __isl_give isl_basic_set_list *
collect_inter_validity(struct isl_sched_graph *graph, int coincidence,
                       struct isl_exploit_lineality_data *data) {
  isl_union_map *inter;
  isl_union_set *wrap;
  isl_basic_set_list *list;
...
compute_carrying_sol_coef(isl_ctx *ctx, struct isl_sched_graph *graph,
                          int n_edge, __isl_keep isl_basic_set_list *intra,
                          __isl_keep isl_basic_set_list *inter,
                          int want_integral, int carry_inter) {
  isl_basic_set *lp;
...
static __isl_give isl_vec *compute_carrying_sol(isl_ctx *ctx,
  ...
  carry.intra =
      collect_intra_validity(ctx, graph, coincidence, &carry.lineality);
  carry.inter = collect_inter_validity(graph, coincidence, &carry.lineality);
  ...
  if (fallback && n_intra > 0 &&
    ...
    sol = compute_carrying_sol_coef(ctx, graph, n_intra, carry.intra,
                                    carry.inter, fallback, 0);
  ...
  sol = compute_carrying_sol_coef(ctx, graph, n_edge, carry.intra, carry.inter,
                                  fallback, 1);
  isl_carry_clear(&carry);
  return sol;
error:
 ...
 */
static __isl_give isl_schedule_node *
carry_fallback(__isl_take isl_schedule_node *node,
 ...
 */
static __isl_give isl_schedule_node *
carry_feautrier(__isl_take isl_schedule_node *node,
                struct isl_sched_graph *graph) {
 ...
 */
static __isl_give isl_schedule_node *
carry_dependences(__isl_take isl_schedule_node *node,
                  struct isl_sched_graph *graph) {
 ...
 */
static __isl_give isl_schedule_node *
carry_coincidence(__isl_take isl_schedule_node *node,
                  struct isl_sched_graph *graph) {
 ...
 */
static __isl_give isl_schedule_node *
sort_statements(__isl_take isl_schedule_node *node,
                struct isl_sched_graph *graph, int initialized) {
  isl_ctx *ctx;
  isl_union_set_list *filters;
  ...
  ctx = isl_schedule_node_get_ctx(node);
  if (graph->n < 1)
    isl_die(ctx, isl_error_internal, "graph should have at least one node",
            return isl_schedule_node_free(node));
 ...
 */
static __isl_give isl_schedule_node *
compute_schedule_wcc_feautrier(isl_schedule_node *node,
                               struct isl_sched_graph *graph) {
 ...
 */
static int
has_adjacent_true_conditions(struct isl_sched_graph *graph,
  ...
  for (i = 0; i < graph->n_edge; ++i) {
    ...
    if (adjacent >= 0 && !adjacent)
      adjacent = range_intersects(condition, conditional_source);
    if (adjacent < 0)
      return -1;
    if (!adjacent)
      continue;
 ...
 */
static __isl_give isl_schedule_node *
compute_schedule_finish_band(__isl_take isl_schedule_node *node,
                             struct isl_sched_graph *graph, int initialized) {
  int empty;
 ...
 */
static __isl_give isl_schedule_node *
compute_schedule_wcc_whole(__isl_take isl_schedule_node *node,
                           struct isl_sched_graph *graph) {
  isl_ctx *ctx;
...
static isl_stat clustering_init(isl_ctx *ctx, struct isl_clustering *c,
  ...
  if (!c->scc || !c->cluster || !c->scc_cluster || !c->scc_node ||
      !c->scc_in_merge)
    return isl_stat_error;

  for (i = 0; i < c->n; ++i) {
    if (extract_sub_graph(ctx, graph, &node_scc_exactly, &edge_scc_exactly, i,
                          &c->scc[i]) < 0)
      return isl_stat_error;
    c->scc[i].scc = 1;
...
static int find_proximity(struct isl_sched_graph *graph,
  ...
  for (i = 0; i < graph->n_edge; ++i) {
    ...
    if (bad_cluster(&c->scc[edge->src->scc]) ||
      ...
      continue;
    dist = c->scc_cluster[edge->dst->scc] - c->scc_cluster[edge->src->scc];
    if (dist == 0)
      continue;
    weight = edge->weight;
...
static isl_stat mark_merge_sccs(isl_ctx *ctx, struct isl_sched_graph *graph,
  ...
  g = isl_tarjan_graph_component(ctx, graph->n, data.dst, &cluster_follows,
                                 &data);
  if (!g)
    goto error;
  ...
  if (i < 3)
    isl_die(ctx, isl_error_internal,
            "expecting at least two nodes in component", goto error);
  if (g->order[--i] != -1)
    isl_die(ctx, isl_error_internal, "expecting end of component marker",
            goto error);
 ...
 */
static __isl_give isl_union_map *
collect_cluster_map(isl_ctx *ctx, struct isl_sched_graph *graph,
                    struct isl_clustering *c) {
  int i, j;
  isl_space *space;
  ...
  for (i = 0; i < graph->scc; ++i) {
    ...
    for (j = 0; j < c->scc[i].n; ++j) {
      ...
      ma = node_extract_partial_schedule_multi_aff(node, start, n);
      ma = isl_multi_aff_set_tuple_id(ma, isl_dim_out, isl_id_copy(id));
...
add_non_conditional_constraints(struct isl_sched_edge *edge,
  ...
  for (t = isl_edge_first; t <= isl_edge_last; ++t) {
    if (t == isl_edge_condition || t == isl_edge_conditional_validity)
      continue;
    if (!is_type(edge, t))
      continue;
    sc = isl_schedule_constraints_add(sc, t, isl_union_map_copy(umap));
  }
 ...
 */
static __isl_give isl_schedule_constraints *
add_conditional_constraints(struct isl_sched_edge *edge,
                            __isl_keep isl_union_map *umap,
                            __isl_take isl_schedule_constraints *sc) {
  enum isl_edge_type t;
 ...
 */
static __isl_give isl_schedule_constraints *
collect_edge_constraints(struct isl_sched_edge *edge,
  ...
  umap = isl_union_map_apply_domain(umap, isl_union_map_copy(cluster_map));
  umap = isl_union_map_apply_range(umap, isl_union_map_copy(cluster_map));
 ...
 */
static __isl_give isl_schedule_constraints *
collect_constraints(struct isl_sched_graph *graph, int *scc_in_merge,
                    __isl_keep isl_union_map *cluster_map,
                    __isl_take isl_schedule_constraints *sc) {
  int i;
...
extract_node_transformation(isl_ctx *ctx, struct isl_sched_node *node,
                            struct isl_clustering *c,
                            struct isl_sched_graph *merge_graph) {
...
static isl_bool ok_to_merge_proximity(isl_ctx *ctx,
  ...
  for (i = 0; i < graph->n_edge; ++i) {
    ...
    if (!c->scc_in_merge[edge->dst->scc])
      continue;
    if (c->scc_cluster[edge->dst->scc] == c->scc_cluster[edge->src->scc])
      continue;
    bounded = has_bounded_distances(ctx, edge, graph, c, merge_graph);
    if (bounded < 0 || bounded)
      return bounded;
  }
...
static __isl_give isl_mat *node_transformation(isl_ctx *ctx,
  ...
  for (i = 0; i < n_row; ++i) {
    ...
    for (j = 0; j < n; ++j)
      isl_seq_addmul(t->row[i], t_node->sched->row[i][1 + n_param + j],
                     node->sched->row[first + j], 1 + n_param + n_var);
  }
  return t;
}
...
static isl_stat compute_weights(struct isl_sched_graph *graph,
  ...
  for (i = 0; i < graph->n_edge; ++i) {
    ...
    hull = isl_basic_map_project_out(hull, isl_dim_in, 0, src->rank);
    hull = isl_basic_map_project_out(hull, isl_dim_out, 0, dst->rank);
    ...
    hull = isl_basic_map_drop_constraints_not_involving_dims(hull, isl_dim_in,
                                                             0, n_in);
    hull = isl_basic_map_drop_constraints_not_involving_dims(hull, isl_dim_out,
                                                             0, n_out);
 ...
 */
static __isl_give isl_schedule_node *
finish_bands_clustering(__isl_take isl_schedule_node *node,
                        struct isl_sched_graph *graph,
                        struct isl_clustering *c) {
  int i;
  isl_ctx *ctx;
 ...
 */
static __isl_give isl_schedule_node *
compute_schedule_wcc_clustering(__isl_take isl_schedule_node *node,
                                struct isl_sched_graph *graph) {
  isl_ctx *ctx;
  struct isl_clustering c;
  int i;
 ...
 */
static __isl_give isl_schedule_node *
compute_schedule_wcc(__isl_take isl_schedule_node *node,
                     struct isl_sched_graph *graph) {
  isl_ctx *ctx;
...
compute_component_schedule(__isl_take isl_schedule_node *node,
  ...
  for (component = 0; component < graph->scc; ++component) {
    ...
    node = isl_schedule_node_child(node, 0);
    node = compute_sub_schedule(node, ctx, graph, &node_scc_exactly,
...
isl_union_set_compute_schedule(__isl_take isl_union_set *domain,
