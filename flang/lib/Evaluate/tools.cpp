...
ConvertRealOperandsResult ConvertRealOperands(
    ...
    common::visitors{
        [&](Expr<SomeInteger> &&ix,
            ...
            return {
                AsSameKindExprs<TypeCategory::Real>(
...
static std::optional<Expr<SomeType>> NoExpr() {
    return std::nullopt;
}
    ...
    common::visitors{
        ...
        [](Expr<SomeInteger> &&ix, Expr<SomeReal> &&ry) {
            ...
            [&](auto &&ryk) -> Expr<SomeReal> {
                ...
                return AsCategoryExpr(
                OPR<resultType>{
                    ConvertToType<resultType>(std::move(ix)),
                    std::move(ryk)
                });
            },
            std::move(ry.u)));
...
bool IsDummy(const Symbol &symbol) {
    return std::visit(
    common::visitors{[](const EntityDetails &x) {
        return x.isDummy();
    },
