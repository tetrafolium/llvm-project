...
bool MipsAsmPrinter::runOnMachineFunction(MachineFunction &MF) {
  ...
  if (Subtarget->inMips16Mode())
    for (std::map<const char *, const Mips16HardFloatInfo::FuncSignature
                                    *>::const_iterator it =
             MipsFI->StubsNeeded.begin();
...
static void emitDirectiveRelocJalr(const MachineInstr &MI,
                                   MCContext &OutContext, TargetMachine &TM,
  ...
  for (unsigned int I = MI.getDesc().getNumOperands(), E = MI.getNumOperands();
    ...
    if (MO.isMCSymbol() && (MO.getTargetFlags() & MipsII::MO_JALR)) {
      ...
      if (Callee && !Callee->getName().empty()) {
        ...
        const MCExpr *CaleeExpr = MCSymbolRefExpr::create(Callee, OutContext);
...
void MipsAsmPrinter::printSavedRegsBitmask() {
  ...
  FPUTopSavedRegOff =
      FPUBitmask ? (HasAFGR64Reg ? -AFGR64RegSize : -FGR32RegSize) : 0;
...
const char *MipsAsmPrinter::getCurrentABIString() const {
  ...
  case MipsABIInfo::ABI::O32:
    return "abi32";
  case MipsABIInfo::ABI::N32:
    return "abiN32";
...
void MipsAsmPrinter::printMemOperand(const MachineInstr *MI, int opNum,
                                     raw_ostream &O) {
...
void MipsAsmPrinter::printMemOperandEA(const MachineInstr *MI, int opNum,
                                       raw_ostream &O) {
...
void MipsAsmPrinter::printRegisterList(const MachineInstr *MI, int opNum,
                                       raw_ostream &O) {
...
void MipsAsmPrinter::emitEndOfAsmFile(Module &M) {
  ...
  for (std::map<const char *,
                const Mips16HardFloatInfo::FuncSignature *>::const_iterator it =
           StubsNeeded.begin();
