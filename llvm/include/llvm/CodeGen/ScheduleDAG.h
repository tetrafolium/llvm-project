...
public:
  ...
  SDep(SUnit *S, Kind kind, unsigned Reg) : Dep(S, kind), Contents() {
    switch (kind) {
    default:
  ...
  SDep(SUnit *S, OrderKind kind) : Dep(S, Order), Contents(), Latency(0) {
...
public:
  ...
  bool isTopReady() const { return NumPredsLeft == 0; }
  bool isBottomReady() const { return NumSuccsLeft == 0; }
...
class SUnitIterator
    : public std::iterator<std::forward_iterator_tag, SUnit, ptrdiff_t> {
  SUnit *Node;
  unsigned Operand;
...
public:
  bool operator==(const SUnitIterator &x) const { return Operand == x.Operand; }
  bool operator!=(const SUnitIterator &x) const { return !operator==(x); }

  pointer operator*() const { return Node->Preds[Operand].getSUnit(); }
  ...
  bool isCtrlDep() const { return getSDep().isCtrl(); }
  bool isArtificialDep() const { return getSDep().isArtificial(); }
