...
public:
    ...
    public:
        ...
        // Return bit i
        virtual bool is_set(int i) const {
            return false;
        }
        // Set bit i to 0
        ...
        virtual int next(int previous) const {
            return 0;
        }
#if KMP_OS_WINDOWS
        virtual int set_process_affinity(bool abort_on_error) const {
            return -1;
        }
#endif
        // Set the system's affinity to this affinity mask's value
        virtual int set_system_affinity(bool abort_on_error) const {
            return -1;
        }
        // Set this affinity mask to the current system affinity
        virtual int get_system_affinity(bool abort_on_error) {
            return -1;
        }
        // Only 1 DWORD in the mask should have any procs set.
        // Return the appropriate index, or -1 for an invalid mask.
        virtual int get_proc_group() const {
            return -1;
        }
    };
    void *operator new(size_t n);
    ...
    virtual Mask *allocate_mask() {
        return nullptr;
    }
    virtual void deallocate_mask(Mask *m) {}
    virtual Mask *allocate_mask_array(int num) {
        return nullptr;
    }
    virtual void deallocate_mask_array(Mask *m) {}
    virtual Mask *index_mask_array(Mask *m, int index) {
        return nullptr;
    }
...
static inline void __kmp_store_mxcsr(kmp_uint32 *p) {
    *p = _mm_getcsr();
}
#else
static inline void __kmp_load_mxcsr(const kmp_uint32 *p) {}
...
static inline void __kmp_x86_pause(void) {
    _mm_delay_32(300);
}
#else
static inline void __kmp_x86_pause(void) {
