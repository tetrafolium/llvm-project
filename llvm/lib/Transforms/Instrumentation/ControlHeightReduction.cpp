...
INITIALIZE_PASS_BEGIN(ControlHeightReductionLegacyPass, "chr",
                      "Reduce control height in the hot paths", false, false)
INITIALIZE_PASS_DEPENDENCY(BlockFrequencyInfoWrapperPass)
INITIALIZE_PASS_DEPENDENCY(DominatorTreeWrapperPass)
INITIALIZE_PASS_DEPENDENCY(ProfileSummaryInfoWrapperPass)
INITIALIZE_PASS_DEPENDENCY(RegionInfoPass)
INITIALIZE_PASS_END(ControlHeightReductionLegacyPass, "chr",
                    "Reduce control height in the hot paths", false, false)
...
struct CHRStats {
  CHRStats()
      : NumBranches(0), NumBranchesDelta(0), WeightedNumBranchesDelta(0) {}
...
static inline raw_ostream LLVM_ATTRIBUTE_UNUSED &
operator<<(raw_ostream &OS, const CHRStats &Stats) {
  Stats.print(OS);
  return OS;
}
...
static bool checkMDProf(MDNode *MD, BranchProbability &TrueProb,
  ...
  if (MDName->getString() != "branch_weights" || MD->getNumOperands() != 3)
    return false;
...
checkBiasedSelect(SelectInst *SI, Region *R,
  ...
  if (!checkMDProf(SI->getMetadata(LLVMContext::MD_prof), TrueProb, FalseProb))
    return false;
...
static bool shouldSplit(Instruction *InsertPoint,
  ...
  CHR_DEBUG(
      ...
      for (Value *V
           : PrevConditionValues) { dbgs() << *V << ", "; } dbgs()
      << " ConditionValues ";
      for (Value *V
...
SmallVector<CHRScope *, 8> CHR::splitScope(
  ...
  for (RegInfo &RI : RegInfos) {
    ...
    CHR_DEBUG(dbgs() << "ConditionValues "; for (Value *V
                                                 : ConditionValues) {
      dbgs() << *V << ", ";
    } dbgs() << "\n");
...
void CHR::classifyBiasedScopes(SmallVectorImpl<CHRScope *> &Scopes) {
  for (CHRScope *Scope : Scopes) {
    ...
    CHR_DEBUG(
        ...
        dbgs() << "TrueBiasedRegions "; for (Region *R
                                             : Scope->TrueBiasedRegions) {
          dbgs() << R->getNameStr() << ", ";
        } dbgs() << "\n";
        dbgs() << "FalseBiasedRegions "; for (Region *R
                                              : Scope->FalseBiasedRegions) {
          dbgs() << R->getNameStr() << ", ";
        } dbgs() << "\n";
        dbgs() << "TrueBiasedSelects "; for (SelectInst *SI
                                             : Scope->TrueBiasedSelects) {
          dbgs() << *SI << ", ";
        } dbgs() << "\n";
        dbgs() << "FalseBiasedSelects "; for (SelectInst *SI
...
void CHR::setCHRRegions(SmallVectorImpl<CHRScope *> &Input,
  ...
  for (CHRScope *Scope : Input) {
    ...
    CHR_DEBUG(
        ...
        << "\n";
        for (RegInfo &RI
             : Scope->CHRRegions) { dbgs() << RI.R->getNameStr() << "\n"; });
  }
}
...
void CHR::addToMergedCondition(bool IsTrueBiased, Value *Cond,
  ...
  if (IsTrueBiased) {
    ...
    if (!Done) {
      Value *Negate = IRB.CreateXor(ConstantInt::getTrue(F.getContext()), Cond);
