...
#include "MipsGenAsmMatcher.inc"
  ...
  bool expandRotation(MCInst &Inst, SMLoc IDLoc, MCStreamer &Out,
                      const MCSubtargetInfo *STI);
...
#undef GET_OPERAND_DIAGNOSTIC_TYPES
  ...
  bool hasDSP() const { return getSTI().getFeatureBits()[Mips::FeatureDSP]; }
  ...
  bool hasMSA() const { return getSTI().getFeatureBits()[Mips::FeatureMSA]; }
  ...
  bool inPicMode() { return IsPicEnabled; }
  ...
  bool hasMT() const { return getSTI().getFeatureBits()[Mips::FeatureMT]; }

  bool hasCRC() const { return getSTI().getFeatureBits()[Mips::FeatureCRC]; }

  bool hasVirt() const { return getSTI().getFeatureBits()[Mips::FeatureVirt]; }

  bool hasGINV() const { return getSTI().getFeatureBits()[Mips::FeatureGINV]; }
...
public:
  ...
  void addGPRMM16AsmRegMovePPairSecondOperands(MCInst &Inst, unsigned N) const {
    assert(N == 1 && "Invalid number of operands!");
    Inst.addOperand(MCOperand::createReg(getGPRMM16Reg()));
  }
  ...
  bool isMM16AsmRegZero() const {
    if (!(isRegIdx() && RegIdx.Kind))
      return false;
    return (RegIdx.Index == 0 || (RegIdx.Index >= 2 && RegIdx.Index <= 7) ||
            RegIdx.Index == 17);
  }
...
bool MipsAsmParser::processInstruction(MCInst &Inst, SMLoc IDLoc,
  ...
  if (hasMips32r6() && Opcode == Mips::SSNOP) {
    ...
    Warning(IDLoc, "ssnop is deprecated for " + ISA +
  ...
  if (hasCnMips()) {
    ...
    case Mips::BBIT132:
      ...
      Imm = Opnd.getImm();
      if (Imm < 0 ||
          Imm > (Opcode == Mips::BBIT0 || Opcode == Mips::BBIT1 ? 63 : 31))
        return Error(IDLoc, "immediate operand value out of range");
      if (Imm > 31) {
        Inst.setOpcode(Opcode == Mips::BBIT0 ? Mips::BBIT032 : Mips::BBIT132);
  ...
  if ((Opcode == Mips::JAL || Opcode == Mips::JAL_MM) && inPicMode()) {
    ...
    if (isJalrRelocAvailable(JalExpr)) {
      ...
      const MCExpr *RelocJalrExpr = MCSymbolRefExpr::create(
          JalSym, MCSymbolRefExpr::VK_None, getContext(), IDLoc);
  ...
  if (inMicroMipsMode()) {
        ...
        if ((OpInfo.OperandType == MCOI::OPERAND_MEMORY) ||
          ...
          if (Op.isImm()) {
            ...
            if (isInt<9>(MemOffset) && (MemOffset % 4 == 0) &&
                getContext()
                    .getRegisterInfo()
                    ->getRegClass(Mips::GPRMM16RegClassID)
                    .contains(DstReg.getReg()) &&
    ...
    case Mips::ADDIUSP_MM:
      ...
      if (!Opnd.isImm())
        return Error(IDLoc, "expected immediate operand kind");
      Imm = Opnd.getImm();
      if (Imm < -1032 || Imm > 1028 || (Imm < 8 && Imm > -12) || Imm % 4 != 0)
        return Error(IDLoc, "immediate operand value out of range");
      break;
    case Mips::SLL16_MM:
    case Mips::SRL16_MM:
    ...
    case Mips::ADDIUR2_MM:
      ...
      if (!Opnd.isImm())
        return Error(IDLoc, "expected immediate operand kind");
      Imm = Opnd.getImm();
      if (!(Imm == 1 || Imm == -1 || ((Imm % 4 == 0) && Imm < 28 && Imm > 0)))
        return Error(IDLoc, "immediate operand value out of range");
      break;
    case Mips::ANDI16_MM:
      Opnd = Inst.getOperand(2);
...
MipsAsmParser::tryExpandInstruction(MCInst &Inst, SMLoc IDLoc, MCStreamer &Out,
  ...
  case Mips::SDMacro:
    return expandLoadStoreDMacro(Inst, IDLoc, Out, STI,
               ...
               : MER_Success;
  case Mips::SDC1_M1:
    return expandStoreDM1Macro(Inst, IDLoc, Out, STI) ? MER_Fail : MER_Success;
  case Mips::SEQMacro:
...
bool MipsAsmParser::expandJalWithRegs(MCInst &Inst, SMLoc IDLoc,
  ...
  if (MCID.hasDelaySlot() && AssemblerOptions.back()->isReorder())
    TOut.emitEmptyDelaySlot(hasShortDelaySlot(JalrInst), IDLoc, STI);
...
bool MipsAsmParser::loadAndAddSymbolAddress(const MCExpr *SymExpr,
  ...
  if (inPicMode()) {
    ...
    unsigned TmpReg = DstReg;
    if (UseSrcReg && getContext().getRegisterInfo()->isSuperOrSubRegisterEq(
                         DstReg, SrcReg)) {
...
static unsigned nextReg(unsigned Reg) {
  ...
  switch (Reg) {
  default:
    llvm_unreachable("Unknown register in assembly macro expansion!");
  case Mips::ZERO:
    return Mips::AT;
  case Mips::AT:
    return Mips::V0;
...
bool MipsAsmParser::expandStoreDM1Macro(MCInst &Inst, SMLoc IDLoc,
  ...
  assert(Inst.getOperand(2).isImm() && "Offset for macro is not immediate!");
...
bool MipsAsmParser::expandSeqI(MCInst &Inst, SMLoc IDLoc, MCStreamer &Out,
  ...
  if (SrcReg == Mips::ZERO) {
    ...
    TOut.emitRRR(isGP64bit() ? Mips::DADDu : Mips::ADDu, DstReg, SrcReg, SrcReg,
                 IDLoc, STI);
    return false;
  }
...
static unsigned getRegisterForMxtrFP(MCInst &Inst, bool IsMFTC1) {
  ...
  case Mips::F31:
    return Mips::RA;
  default:
    llvm_unreachable("Unknown register for mttc1 alias!");
  }
}
...
static unsigned getRegisterForMxtrC0(MCInst &Inst, bool IsMFTC0) {
  ...
  case Mips::COP031:
    return Mips::RA;
  default:
    llvm_unreachable("Unknown register for mttc0 alias!");
  }
}
...
void MipsAsmParser::printWarningWithFixIt(const Twine &Msg, const Twine &FixMsg,
                                          SMRange Range, bool ShowColors) {
  getSourceManager().PrintMessage(Range.Start, SourceMgr::DK_Warning, Msg,
                                  Range, SMFixIt(Range, FixMsg), ShowColors);
}
...
OperandMatchResultTy MipsAsmParser::parseMemOperand(OperandVector &Operands) {
  MCAsmParser &Parser = getParser();
...
OperandMatchResultTy MipsAsmParser::matchAnyRegisterNameWithoutDollar(
...
OperandMatchResultTy MipsAsmParser::parseAnyRegister(OperandVector &Operands) {
  MCAsmParser &Parser = getParser();
...
OperandMatchResultTy MipsAsmParser::parseJumpTarget(OperandVector &Operands) {
  MCAsmParser &Parser = getParser();
...
OperandMatchResultTy MipsAsmParser::parseInvNum(OperandVector &Operands) {
  MCAsmParser &Parser = getParser();
  const MCExpr *IdVal;
...
OperandMatchResultTy MipsAsmParser::parseRegisterList(OperandVector &Operands) {
  MCAsmParser &Parser = getParser();
