...
__isl_give isl_space *isl_space_alloc(isl_ctx *ctx, unsigned nparam,
...
isl_stat isl_space_check_is_set(__isl_keep isl_space *space) {
  ...
  if (!is_set)
    isl_die(isl_space_get_ctx(space), isl_error_invalid, "space is not a set",
            return isl_stat_error);
  return isl_stat_ok;
}
...
isl_bool isl_space_is_map(__isl_keep isl_space *space) {
  ...
  if (!space)
    return isl_bool_error;
  r = space->tuple_id[0] != &isl_id_none && space->tuple_id[1] != &isl_id_none;
...
static isl_stat isl_space_check_is_map(__isl_keep isl_space *space) {
  ...
  if (is_space < 0)
    return isl_stat_error;
  if (!is_space)
    isl_die(isl_space_get_ctx(space), isl_error_invalid, "expecting map space",
            return isl_stat_error);
  return isl_stat_ok;
}
...
isl_stat isl_space_check_range_is_wrapping(__isl_keep isl_space *space) {
  ...
  if (wrapping < 0)
    return isl_stat_error;
  if (!wrapping)
    isl_die(isl_space_get_ctx(space), isl_error_invalid, "range not a product",
            return isl_stat_error);
  return isl_stat_ok;
}

__isl_give isl_space *isl_space_set_alloc(isl_ctx *ctx, unsigned nparam,
                                          unsigned dim) {
  isl_space *space;
...
isl_bool isl_space_is_params(__isl_keep isl_space *space) {
  if (!space)
    return isl_bool_error;
  if (space->n_in != 0 || space->nested[0] || space->n_out != 0 ||
      space->nested[1])
    return isl_bool_false;
  if (space->tuple_id[0] != &isl_id_none)
    return isl_bool_false;
...
static isl_size global_pos(__isl_keep isl_space *space, enum isl_dim_type type,
                           unsigned pos) {
...
static __isl_keep isl_space *isl_space_peek_nested(__isl_keep isl_space *space,
  ...
  if (!space)
    return NULL;
  if (!space->nested[pos])
    isl_die(isl_space_get_ctx(space), isl_error_invalid, "no nested space",
            return NULL);
...
copy_ids(__isl_take isl_space *dst, enum isl_dim_type dst_type, unsigned offset,
         __isl_keep isl_space *src, enum isl_dim_type src_type) {
  int i;
  isl_id *id;
...
__isl_take isl_space *isl_space_dup(__isl_keep isl_space *space) {
  ...
  if (!space)
    return NULL;
  dup = isl_space_alloc(space->ctx, space->nparam, space->n_in, space->n_out);
  if (!dup)
    return NULL;
  if (space->tuple_id[0] &&
  ...
  if (space->tuple_id[1] &&
    ...
    goto error;
  if (space->nested[0] && !(dup->nested[0] = isl_space_copy(space->nested[0])))
    goto error;
  if (space->nested[1] && !(dup->nested[1] = isl_space_copy(space->nested[1])))
    goto error;
  if (!space->ids)
    return dup;
...
static int name_ok(isl_ctx *ctx, const char *s) {
  ...
  dummy = strtol(s, &p, 0);
  if (p != s)
    isl_die(ctx, isl_error_invalid, "name looks like a number", return 0);
 ...
 */
static __isl_give isl_space *
isl_space_restore_nested(__isl_take isl_space *space, int pos,
...
static int space_can_have_id(__isl_keep isl_space *space,
  ...
  if (isl_space_is_set(space) && type != isl_dim_set)
    isl_die(space->ctx, isl_error_invalid, "set spaces can only have a set id",
            return 0);
  if (type != isl_dim_in && type != isl_dim_out)
    isl_die(space->ctx, isl_error_invalid,
            "only input, output and set tuples can have ids", return 0);
...
__isl_give isl_id *isl_space_get_tuple_id(__isl_keep isl_space *space,
  ...
  if (has_id < 0)
    return NULL;
  if (!has_id)
    isl_die(space->ctx, isl_error_invalid, "tuple has no id", return NULL);
...
__isl_give isl_space *isl_space_set_tuple_id(__isl_take isl_space *space,
  ...
  if (type != isl_dim_in && type != isl_dim_out)
    isl_die(space->ctx, isl_error_invalid,
            "only input, output and set tuples can have names", goto error);
  ...
  if (type == isl_dim_param) {
    ...
    for (i = 0; i < 2; ++i) {
      if (!space->nested[i])
        continue;
      space->nested[i] =
          isl_space_set_dim_id(space->nested[i], type, pos, isl_id_copy(id));
      if (!space->nested[i])
        goto error;
    }
  }
...
__isl_give isl_space *isl_space_reset_dim_id(__isl_take isl_space *space,
  ...
  if (type == isl_dim_param) {
    ...
    for (i = 0; i < 2; ++i) {
      if (!space->nested[i])
        continue;
      space->nested[i] = isl_space_reset_dim_id(space->nested[i], type, pos);
      if (!space->nested[i])
        goto error;
    }
  }
...
__isl_give isl_id *isl_space_get_dim_id(__isl_keep isl_space *space,
  ...
  if (!get_id(space, type, pos))
    isl_die(space->ctx, isl_error_invalid, "dim has no id", return NULL);
...
isl_stat isl_space_check_equal_tuples(__isl_keep isl_space *space1,
  ...
  if (is_equal < 0)
    return isl_stat_error;
  if (!is_equal)
    isl_die(isl_space_get_ctx(space1), isl_error_invalid, "incompatible spaces",
            return isl_stat_error);
...
static __isl_give isl_space *space_extend(__isl_take isl_space *space,
                                          unsigned nparam, unsigned n_in,
                                          unsigned n_out) {
  isl_id **ids = NULL;

  if (!space)
    return NULL;
  if (space->nparam == nparam && space->n_in == n_in && space->n_out == n_out)
    return space;
  ...
  if (space->ids) {
    ...
    if (n < nparam || n < n_in || n < n_out)
      isl_die(isl_space_get_ctx(space), isl_error_invalid,
              "overflow in total number of dimensions", goto error);
    ...
    get_ids(space, isl_dim_out, 0, space->n_out, ids + nparam + n_in);
    free(space->ids);
    space->ids = ids;
...
__isl_give isl_space *isl_space_add_dims(__isl_take isl_space *space,
  ...
  switch (type) {
  case isl_dim_param:
    space = space_extend(space, space->nparam + n, space->n_in, space->n_out);
    if (space && space->nested[0] &&
        !(space->nested[0] =
              isl_space_add_dims(space->nested[0], isl_dim_param, n)))
      goto error;
    if (space && space->nested[1] &&
        !(space->nested[1] =
              isl_space_add_dims(space->nested[1], isl_dim_param, n)))
      goto error;
    return space;
  case isl_dim_in:
    return space_extend(space, space->nparam, space->n_in + n, space->n_out);
  case isl_dim_out:
    return space_extend(space, space->nparam, space->n_in, space->n_out + n);
  default:
    isl_die(space->ctx, isl_error_invalid,
...
__isl_give isl_space *isl_space_insert_dims(__isl_take isl_space *space,
  ...
  if (!valid_dim_type(type))
    isl_die(ctx, isl_error_invalid,
            "cannot insert dimensions of specified type", goto error);
  ...
  if (space->ids) {
    ...
    for (t = isl_dim_param; t <= isl_dim_out; ++t) {
      if (t != type) {
        ...
        off += pos + n;
        get_ids(space, t, pos, s[t - o] - pos, ids + off);
        off += s[t - o] - pos;
      }
    }
    free(space->ids);
  ...
  if (type == isl_dim_param) {
    if (space && space->nested[0] &&
        !(space->nested[0] =
              isl_space_insert_dims(space->nested[0], isl_dim_param, pos, n)))
      goto error;
    if (space && space->nested[1] &&
        !(space->nested[1] =
              isl_space_insert_dims(space->nested[1], isl_dim_param, pos, n)))
      goto error;
  }
  ...
  if (space->ids) {
    ...
    for (t = isl_dim_param; t <= isl_dim_out; ++t) {
      if (t == dst_type) {
        ...
        off += n;
        get_ids(space, t, dst_pos, s[t - o] - dst_pos, ids + off);
        off += s[t - o] - dst_pos;
      } else if (t == src_type) {
        get_ids(space, t, 0, src_pos, ids + off);
        off += src_pos;
        get_ids(space, t, src_pos + n, s[t - o] - src_pos - n, ids + off);
        off += s[t - o] - src_pos - n;
      } else {
        get_ids(space, t, 0, s[t - o], ids + off);
        off += s[t - o];
...
__isl_give isl_space *isl_space_join(__isl_take isl_space *left,
  ...
  space = isl_space_alloc(left->ctx, left->nparam, left->n_in, right->n_out);
  if (!space)
    goto error;
...
__isl_give isl_space *isl_space_product(__isl_take isl_space *left,
  ...
  if (is_set != isl_space_is_set(right))
    isl_die(isl_space_get_ctx(left), isl_error_invalid,
            "expecting either two set spaces or two map spaces", goto error);
  if (is_set)
...
__isl_give isl_space *isl_space_domain_product(__isl_take isl_space *left,
  ...
  if (!isl_space_tuple_is_equal(left, isl_dim_out, right, isl_dim_out))
    isl_die(left->ctx, isl_error_invalid, "ranges need to match", goto error);
...
__isl_give isl_space *isl_space_range_product(__isl_take isl_space *left,
  ...
  if (!isl_space_tuple_is_equal(left, isl_dim_in, right, isl_dim_in))
    isl_die(left->ctx, isl_error_invalid, "domains need to match", goto error);
...
isl_space_domain_factor_domain(__isl_take isl_space *space) {
  isl_space *nested;
  isl_space *domain;
  ...
  if (!isl_space_domain_is_wrapping(space))
    isl_die(isl_space_get_ctx(space), isl_error_invalid, "domain not a product",
            return isl_space_free(space));

  nested = space->nested[0];
  domain = isl_space_copy(space);
  domain = isl_space_drop_dims(domain, isl_dim_in, nested->n_in, nested->n_out);
  if (!domain)
    return isl_space_free(space);
 ...
 */
__isl_give isl_space *
isl_space_domain_factor_range(__isl_take isl_space *space) {
  isl_space *nested;
  isl_space *range;
...
static __isl_give isl_space *range_factor_domain(__isl_take isl_space *space) {
  ...
  domain = isl_space_copy(space);
  domain =
      isl_space_drop_dims(domain, isl_dim_out, nested->n_in, nested->n_out);
  if (!domain)
    return isl_space_free(space);
  if (nested->tuple_id[0]) {
 ...
 */
__isl_give isl_space *
isl_space_range_factor_domain(__isl_take isl_space *space) {
...
static __isl_give isl_space *set_factor_domain(__isl_take isl_space *space) {
  ...
  if (!isl_space_is_wrapping(space))
    isl_die(isl_space_get_ctx(space), isl_error_invalid, "not a product",
            return isl_space_free(space));
 ...
 */
__isl_give isl_space *
isl_space_range_factor_range(__isl_take isl_space *space) {
  if (isl_space_check_range_is_wrapping(space) < 0)
    return isl_space_free(space);
...
__isl_give isl_space *isl_space_map_from_set(__isl_take isl_space *space) {
  ...
  if (n_id > 0 && space->ids) {
    ...
    if (!ids)
      goto error;
    get_ids(space, isl_dim_param, 0, space->nparam, ids);
    get_ids(space, isl_dim_out, 0, space->n_out, ids + space->nparam);
  }
...
__isl_give isl_space *
isl_space_map_from_domain_and_range(__isl_take isl_space *domain,
...
static __isl_give isl_space *set_ids(__isl_take isl_space *space,
                                     ...
                                     unsigned n, __isl_take isl_id **ids) {
  int i;
...
__isl_give isl_space *isl_space_reverse(__isl_take isl_space *space) {
  ...
  if (space->ids) {
    ...
    space = set_ids(space, isl_dim_in, 0, space->n_in, ids + space->n_out);
    free(ids);
  }
...
__isl_give isl_space *isl_space_range_reverse(__isl_take isl_space *space) {
  ...
  equal = isl_space_tuple_is_equal(nested, isl_dim_in, nested, isl_dim_out);
  if (equal < 0)
    return isl_space_free(space);
...
__isl_give isl_space *isl_space_drop_dims(__isl_take isl_space *space,
  ...
  if (type == isl_dim_param) {
    if (space && space->nested[0] &&
        !(space->nested[0] =
              isl_space_drop_dims(space->nested[0], isl_dim_param, first, num)))
      goto error;
    if (space && space->nested[1] &&
        !(space->nested[1] =
              isl_space_drop_dims(space->nested[1], isl_dim_param, first, num)))
      goto error;
  }
  return space;
error:
...
__isl_give isl_space *isl_space_from_domain(__isl_take isl_space *space) {
  if (!space)
    return NULL;
  if (!isl_space_is_set(space))
    isl_die(isl_space_get_ctx(space), isl_error_invalid, "not a set space",
            goto error);
  space = isl_space_reverse(space);
 ...
 */
__isl_give isl_space *
isl_space_add_unnamed_tuple_ui(__isl_take isl_space *space, unsigned dim) {
  ...
  if (!is_params && !is_set)
    isl_die(isl_space_get_ctx(space), isl_error_invalid,
            "cannot add tuple to map space", return isl_space_free(space));
  if (is_params)
 ...
 */
__isl_give isl_space *
isl_space_add_named_tuple_id_ui(__isl_take isl_space *space,
...
static __isl_give isl_space *add_bind_params(__isl_take isl_space *space,
  ...
  for (i = 0; i < n; ++i) {
    ...
    id = isl_multi_id_get_at(tuple, i);
    space = isl_space_set_dim_id(space, isl_dim_param, first + i, id);
  }
 ...
 */
__isl_give isl_space *
isl_space_bind_map_domain(__isl_take isl_space *space,
                          __isl_keep isl_multi_id *tuple) {
  isl_space *tuple_space;
 ...
 */
__isl_give isl_space *
isl_space_bind_domain_wrapped_domain(__isl_take isl_space *space,
                                     __isl_keep isl_multi_id *tuple) {
  isl_space *tuple_space;
  ...
  if (isl_space_check_is_map(space) < 0)
    return isl_space_free(space);
  tuple_space = isl_multi_id_peek_space(tuple);
  if (isl_space_check_domain_wrapped_domain_tuples(tuple_space, space) < 0)
    return isl_space_free(space);
  if (check_fresh_params(space, tuple) < 0)
    return isl_space_free(space);
  space = isl_space_domain_factor_range(space);
 ...
 */
static __isl_give isl_space *
isl_space_insert_domain(__isl_take isl_space *space,
 ...
 */
__isl_give isl_space *
isl_space_unbind_params_insert_domain(__isl_take isl_space *space,
                                      __isl_keep isl_multi_id *tuple) {
  int i;
  isl_size n;
  isl_space *tuple_space;
  ...
  for (i = 0; i < n; ++i) {
    ...
    id = isl_multi_id_get_id(tuple, i);
    tuple_space = isl_space_set_dim_id(tuple_space, isl_dim_set, i, id);
  }
...
__isl_give isl_space *isl_space_underlying(__isl_take isl_space *space,
  ...
  if (is_set < 0)
    return isl_space_free(space);
  if (n_div == 0 && is_set && space->nparam == 0 && space->n_in == 0 &&
      space->n_id == 0)
...
isl_bool isl_space_has_domain_tuples(__isl_keep isl_space *space1,
  ...
  if (is_set < 0 || !is_set)
    return is_set;
  return isl_space_tuple_is_equal(space1, isl_dim_set, space2, isl_dim_in);
}
...
isl_bool isl_space_has_range_tuples(__isl_keep isl_space *space1,
  ...
  if (is_set < 0 || !is_set)
    return is_set;
  return isl_space_tuple_is_equal(space1, isl_dim_set, space2, isl_dim_out);
}
 ...
 */
isl_stat
isl_space_check_domain_wrapped_domain_tuples(__isl_keep isl_space *space1,
                                             __isl_keep isl_space *space2) {
  isl_space *domain;
  isl_stat r;
...
__isl_give isl_space *isl_space_wrap(__isl_take isl_space *space) {
  ...
  wrap = isl_space_set_alloc(space->ctx, space->nparam,
                             space->n_in + space->n_out);
...
__isl_give isl_space *isl_space_unwrap(__isl_take isl_space *space) {
  ...
  if (!isl_space_is_wrapping(space))
    isl_die(space->ctx, isl_error_invalid, "not a wrapping space", goto error);
...
__isl_give isl_space *isl_space_lift(__isl_take isl_space *space,
  ...
  local_space = isl_space_drop_dims(local_space, isl_dim_set, 0, space->n_out);
  local_space = isl_space_add_dims(local_space, isl_dim_set, n_local);
  local_space = isl_space_set_tuple_name(local_space, isl_dim_set, "local");
...
__isl_give isl_space *isl_space_curry(__isl_take isl_space *space) {
  ...
  if (!isl_space_can_curry(space))
    isl_die(space->ctx, isl_error_invalid, "space cannot be curried",
            goto error);
  ...
  dom_ran = isl_space_range(dom);
  ran =
      isl_space_join(isl_space_from_domain(dom_ran), isl_space_from_range(ran));
...
__isl_give isl_space *isl_space_range_curry(__isl_take isl_space *space) {
  ...
  if (!isl_space_can_range_curry(space))
    isl_die(isl_space_get_ctx(space), isl_error_invalid,
            "space range cannot be curried", return isl_space_free(space));
...
__isl_give isl_space *isl_space_uncurry(__isl_take isl_space *space) {
  ...
  if (!isl_space_can_uncurry(space))
    isl_die(space->ctx, isl_error_invalid, "space cannot be uncurried",
            return isl_space_free(space));
  ...
  ran_ran = isl_space_range(ran);
  dom =
      isl_space_join(isl_space_from_domain(dom), isl_space_from_range(ran_dom));
  return isl_space_join(isl_space_from_domain(isl_space_wrap(dom)),
                        isl_space_from_range(ran_ran));
}
 ...
 */
__isl_give isl_space *
isl_space_extend_domain_with_range(__isl_take isl_space *space,
  ...
  if (model->nested[1]) {
    ...
    if (n_nested < 0 || n_space < 0)
      goto error;
    if (n_nested > n_space)
      nested = isl_space_drop_dims(nested, isl_dim_param, n_space,
                                   n_nested - n_space);
    if (!nested)
      goto error;
    space->nested[1] = nested;
