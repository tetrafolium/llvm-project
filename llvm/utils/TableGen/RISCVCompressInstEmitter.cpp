...
class RISCVCompressInstEmitter {
  ...
  struct CompressPat {
    ...
    IndexedMap<OpData> DestOperandMap; // Maps operands in the Dest Instruction
  ...
  for (auto &CompressPat : CompressPatterns) {
    ...
    for (auto &Set : AnyOfFeatureSets) {
      ...
      for (auto &Op : Set) {
        ...
        StringRef Not = Op.first ? "!" : "";
        CondStream << Not
                   << ("STI.getFeatureBits()[" + Namespace + "::" + Op.second +
                       "]")
                          .str();
        if (!isLast)
          CondStream << " || ";
          ...
          if (CompressOrUncompress) {
            unsigned Entry =
                getPredicates(MCOpPredicateMap, MCOpPredicates, DestOperand.Rec,
                              StringRef("MCOperandPredicate"));
            CondStream.indent(6) << Namespace + "ValidateMCOperand(" +
                                        "MI.getOperand(" +
                                        std::to_string(OpIdx) + "), STI, " +
                                        std::to_string(Entry) + ") &&\n";
          } else {
            unsigned Entry =
            ...
            CondStream.indent(6)
                << "MI.getOperand(" + std::to_string(OpIdx) + ").isImm() &&\n";
            CondStream.indent(6)
                << Namespace + "ValidateMachineOperand(" + "MI.getOperand(" +
                       std::to_string(OpIdx) + "), Subtarget, " +
                       std::to_string(Entry) + ") &&\n";
          }
          if (CompressOrUncompress)
            CodeStream.indent(6) << "OutInst.addOperand(MI.getOperand(" +
        ...
        if (CompressOrUncompress) {
          ...
          unsigned Entry =
              getPredicates(ImmLeafPredicateMap, ImmLeafPredicates,
                            DestOperand.Rec, StringRef("ImmediateCode"));
          CondStream.indent(6)
              << Namespace + "ValidateMachineOperand(" +
