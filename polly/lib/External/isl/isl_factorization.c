...
static __isl_give isl_factorizer *
isl_factorizer_alloc(__isl_keep isl_basic_set *bset,
...
isl_factorizer_identity(__isl_keep isl_basic_set *bset) {
...
static int update_group_i_with_row_j(struct isl_factor_groups *g, int i, int j,
  ...
  for (k = i + 1; k < H->n_col && j >= g->pos[k]; ++k) {
    update_group(g, k);
    update_group(g, i);
    if (g->group[k] != g->group[i] && !isl_int_is_zero(H->row[j][k])) {
 ...
 */
__isl_give isl_factorizer *
isl_basic_set_factorizer(__isl_keep isl_basic_set *bset) {
  int i, j, n, done;
  isl_mat *H, *U, *Q;
  isl_size nvar;
  ...
  if (!H)
    return NULL;
  isl_mat_sub_copy(bset->ctx, H->row, bset->eq, bset->n_eq, 0,
                   1 + isl_space_offset(bset->dim, isl_dim_set), nvar);
  isl_mat_sub_copy(bset->ctx, H->row + bset->n_eq, bset->ineq, bset->n_ineq, 0,
                   1 + isl_space_offset(bset->dim, isl_dim_set), nvar);
    ...
    for (i = 1; i < g.cnt[group]; ++i) {
      ...
      for (j = done + g.cnt[group]; j < nvar; ++j)
        if (g.group[j] == group)
          break;
      if (j == nvar)
        isl_die(bset->ctx, isl_error_internal, "internal error", goto error);
...
__isl_give isl_bool isl_factorizer_every_factor_basic_set(
  ...
  for (i = 0, n = 0; i < f->n_group; ++i) {
    ...
    factor = isl_basic_set_drop_constraints_involving(
        factor, nparam + n + f->len[i], nvar - n - f->len[i]);
    factor = isl_basic_set_drop_constraints_involving(factor, nparam, n);
    factor = isl_basic_set_drop(factor, isl_dim_set, n + f->len[i],
                                nvar - n - f->len[i]);
