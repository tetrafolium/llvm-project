...
static LogicalResult verifyOutputShape(
    ...
    if (expectedResultDims.size() == 0) {
        ...
        assert(llvm::all_of(extents, [](AffineExpr e) {
            return e;
        }) &&
...
static SmallVector<IntType, 4> extractVector(ArrayAttr arrayAttr) {
    ...
    [](IntegerAttr attr) {
        return static_cast<IntType>(attr.getInt());
    }));
}
...
isValidExtractOrInsertSlicesType(Operation *op, VectorType vectorType,
    ...
    if (llvm::any_of(strides, [](int64_t s) {
    return s != 1;
}))
...
AffineMap ExtractMapOp::map() {
    return calculateImplicitMap(*this);
}
...
public:
    ...
    LogicalResult matchAndRewrite(ExtractStridedSliceOp extractStridedSliceOp,
        ...
        if (llvm::any_of(sliceMaskDimSizes, [](int64_t sz) {
        return sz == 0;
    }))
...
static LogicalResult verify(ConstantMaskOp &op) {
    ...
    bool all_zeros = llvm::all_of(maskDimSizes, [](int64_t s) {
        return s == 0;
    });
