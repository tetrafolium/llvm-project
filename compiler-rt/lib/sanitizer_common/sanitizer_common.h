...
public:
    ...
    uptr capacity() const {
        return capacity_bytes_ / sizeof(T);
    }
...
public:
    InternalMmapVector() {
        InternalMmapVectorNoCtor<T>::Initialize(0);
    }
    ...
    ~InternalMmapVector() {
        InternalMmapVectorNoCtor<T>::Destroy();
    }
...
public:
    ...
    uptr base_address() const {
        return base_address_;
    }
    uptr max_executable_address() const {
        return max_executable_address_;
    }
    ModuleArch arch() const {
        return arch_;
    }
    const u8 *uuid() const {
        return uuid_;
    ...
    const IntrusiveList<AddressRange> &ranges() const {
        return ranges_;
    }
...
public:
    ...
    const LoadedModule *begin() const {
        return modules_.begin();
    }
    LoadedModule *begin() {
        return modules_.begin();
    }
    const LoadedModule *end() const {
        return modules_.end();
    }
    LoadedModule *end() {
        return modules_.end();
