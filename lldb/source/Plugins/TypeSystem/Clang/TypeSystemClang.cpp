...
lldb::BasicType TypeSystemClang::GetBasicTypeEnumeration(ConstString name) {
  if (name) {
    ...
    if (GetCompleteType(type)) {
      ...
      if (record_type) {
        ...
        if (record_decl) {
          ...
          if (base_type_ptr)
            *base_type_ptr =
                CompilerType(this, base_qual_type.getAsOpaquePtr());
          return num_fields;
        }
      }
    }
...
TypeSystemClang::GetTypeInfo(lldb::opaque_compiler_type_t type,
  ...
  switch (type_class) {
  case clang::Type::Attributed:
    return GetTypeInfo(qual_type->getAs<clang::AttributedType>()
                           ->getModifiedType()
                           .getAsOpaquePtr(),
...
clang::CXXMethodDecl *TypeSystemClang::AddMethodToCXXRecordType(
  ...
  if (name.startswith("~")) {
    ...
    cxx_ctor_decl =
        clang::CXXConstructorDecl::CreateDeserialized(getASTContext(), 0, 0);
