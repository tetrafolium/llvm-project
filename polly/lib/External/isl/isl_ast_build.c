...
isl_ast_build_init_derived(__isl_take isl_ast_build *build,
...
__isl_give isl_ast_build *isl_ast_build_dup(__isl_keep isl_ast_build *build) {
  ...
  if (build->loop_type) {
    ...
    dup->n = build->n;
    dup->loop_type = isl_alloc_array(ctx, enum isl_ast_loop_type, dup->n);
  ...
  if (!dup->iterators || !dup->domain || !dup->generated || !dup->pending ||
      ...
      (build->executed && !dup->executed) || (build->value && !dup->value) ||
      (build->node && !dup->node))
    return isl_ast_build_free(dup);
...
isl_ast_build_align_params(__isl_take isl_ast_build *build,
  ...
  build->domain = isl_set_align_params(build->domain, isl_space_copy(model));
  build->generated =
      isl_set_align_params(build->generated, isl_space_copy(model));
  build->pending = isl_set_align_params(build->pending, isl_space_copy(model));
  build->values =
      isl_multi_aff_align_params(build->values, isl_space_copy(model));
  build->offsets =
      isl_multi_aff_align_params(build->offsets, isl_space_copy(model));
  build->options =
      isl_union_map_align_params(build->options, isl_space_copy(model));
  if (build->internal2input) {
    build->internal2input =
        isl_multi_aff_align_params(build->internal2input, model);
    if (!build->internal2input)
      return isl_ast_build_free(build);
  } else {
  ...
  if (!build->domain || !build->values || !build->offsets || !build->options)
    return isl_ast_build_free(build);
 ...
 */
__isl_give isl_ast_build *
isl_ast_build_set_options(__isl_take isl_ast_build *build,
 ...
 */
__isl_give isl_ast_build *
isl_ast_build_set_iterators(__isl_take isl_ast_build *build,
  ...
  if (n_it > dim)
    build->iterators = isl_id_list_drop(build->iterators, dim, n_it - dim);
...
__isl_give isl_ast_build *isl_ast_build_set_before_each_for(
    __isl_take isl_ast_build *build,
    __isl_give isl_id *(*fn)(__isl_keep isl_ast_build *build, void *user),
    void *user) {
  build = isl_ast_build_cow(build);
...
__isl_give isl_ast_build *isl_ast_build_set_create_leaf(
    __isl_take isl_ast_build *build,
    __isl_give isl_ast_node *(*fn)(__isl_take isl_ast_build *build, void *user),
    void *user) {
  build = isl_ast_build_cow(build);
...
isl_ast_build_get_schedule_map_multi_aff(__isl_keep isl_ast_build *build) {
  ...
  if (needs_map) {
    ...
    ma = isl_multi_aff_drop_dims(ma, isl_dim_out, build->depth,
                                 dim - build->depth);
    for (i = build->depth - 1; i >= 0; --i)
      if (isl_ast_build_has_affine_value(build, i))
        ma = isl_multi_aff_drop_dims(ma, isl_dim_out, i, 1);
  }
 ...
 */
static __isl_give isl_ast_build *
isl_ast_build_init(__isl_take isl_ast_build *build,
                   __isl_take isl_space *space) {
  isl_set *set;
  ...
  build->domain = isl_set_intersect_params(isl_set_copy(set), build->domain);
  build->pending = isl_set_intersect_params(isl_set_copy(set), build->pending);
...
intersect_stride_constraint(__isl_take isl_set *set,
                            __isl_keep isl_ast_build *build) {
  isl_set *stride;
 ...
 */
static __isl_give isl_ast_build *
update_values(__isl_take isl_ast_build *build,
...
isl_ast_build_set_loop_bounds(__isl_take isl_ast_build *build,
  ...
  if (isl_ast_build_has_affine_value(build, build->depth)) {
    ...
    set = isl_set_compute_divs(set);
    build->pending = isl_set_intersect(build->pending, isl_set_copy(set));
...
isl_ast_build_set_pending_generated(__isl_take isl_ast_build *build,
  ...
  pending = isl_basic_set_drop_constraints_involving_dims(pending, isl_dim_set,
                                                          build->depth, 1);
  build->pending =
      isl_set_intersect(build->pending, isl_set_from_basic_set(pending));
  generated = bounds;
  generated = isl_basic_set_drop_constraints_not_involving_dims(
      generated, isl_dim_set, build->depth, 1);
  build->generated =
      isl_set_intersect(build->generated, isl_set_from_basic_set(generated));
 ...
 */
static __isl_give isl_ast_build *
isl_ast_build_restrict_internal(__isl_take isl_ast_build *build,
                                __isl_take isl_set *set) {
  build = isl_ast_build_cow(build);
  if (!build)
    goto error;
 ...
 */
__isl_give isl_ast_build *
isl_ast_build_restrict(__isl_take isl_ast_build *build,
                       __isl_take isl_set *set) {
  isl_bool needs_map;
 ...
 */
static __isl_give isl_ast_build *
extract_loop_types(__isl_take isl_ast_build *build) {
  int i;
  isl_size n;
  isl_ctx *ctx;
  ...
  free(build->loop_type);
  build->n = n;
  build->loop_type = isl_alloc_array(ctx, enum isl_ast_loop_type, build->n);
...
__isl_give isl_space *isl_ast_build_get_space(__isl_keep isl_ast_build *build,
  ...
  if (dim < 0)
    return isl_space_free(space);
  space =
      isl_space_drop_dims(space, isl_dim_set, build->depth, dim - build->depth);
  for (i = build->depth - 1; i >= 0; --i) {
...
isl_ast_build_get_schedule_space(__isl_keep isl_ast_build *build) {
  isl_space *space;
  int i, skip;
...
isl_ast_build_get_schedule(__isl_keep isl_ast_build *build) {
  isl_bool needs_map;
  ...
  if (needs_map) {
    ...
    executed =
        isl_union_map_apply_domain(executed, isl_union_map_from_map(proj));
  }
...
int isl_ast_build_options_involve_depth(__isl_keep isl_ast_build *build) {
  ...
  free = isl_union_map_every_map(build->options, &free_of_depth, &build->depth);
...
static const char *option_str[] = {[isl_ast_loop_atomic] = "atomic",
...
options_insert_dim(__isl_take isl_union_map *options,
                   __isl_take isl_space *space, int pos) {
  isl_map *map;
  ...
  options = isl_union_map_apply_domain(options, isl_union_map_from_map(map));
 ...
 */
static __isl_give isl_ast_build *
node_insert_dim(__isl_take isl_ast_build *build, int pos) {
  int i;
  int local_pos;
  ...
  loop_type = isl_realloc_array(ctx, build->loop_type, enum isl_ast_loop_type,
                                build->n + 1);
  if (!loop_type)
    return isl_ast_build_free(build);
...
isl_ast_build_insert_dim(__isl_take isl_ast_build *build, int pos) {
  ...
  build->domain = isl_set_insert_dims(build->domain, isl_dim_set, pos, 1);
  build->generated = isl_set_insert_dims(build->generated, isl_dim_set, pos, 1);
  build->pending = isl_set_insert_dims(build->pending, isl_dim_set, pos, 1);
  ...
  if (!build->iterators || !build->domain || !build->generated ||
      !build->pending || !build->values || !build->strides || !build->offsets ||
      !build->options)
    return isl_ast_build_free(build);
 ...
 */
static __isl_give isl_union_map *
embed_options(__isl_take isl_union_map *options, __isl_take isl_space *space) {
  isl_map *map;

  map = isl_map_universe(isl_space_unwrap(space));
  map = isl_map_range_map(map);

  options = isl_union_map_apply_range(isl_union_map_from_map(map), options);
...
__isl_give isl_ast_build *isl_ast_build_product(__isl_take isl_ast_build *build,
  ...
  space = isl_space_map_from_set(space);
  build->offsets =
      isl_multi_aff_align_params(build->offsets, isl_space_copy(space));
  build->offsets = isl_multi_aff_product(
      build->offsets, isl_multi_aff_zero(isl_space_copy(space)));
  build->values =
      isl_multi_aff_align_params(build->values, isl_space_copy(space));
  embedding = isl_multi_aff_identity(space);
  build->values =
      isl_multi_aff_product(build->values, isl_multi_aff_copy(embedding));
  if (build->internal2input) {
    build->internal2input =
        isl_multi_aff_product(build->internal2input, embedding);
    build->internal2input = isl_multi_aff_flatten_range(build->internal2input);
    if (!build->internal2input)
      return isl_ast_build_free(build);
  } else {
...
isl_ast_build_compute_gist_basic_set(__isl_keep isl_ast_build *build,
  ...
  bset =
      isl_basic_set_preimage_multi_aff(bset, isl_multi_aff_copy(build->values));
...
__isl_give isl_set *isl_ast_build_compute_gist(__isl_keep isl_ast_build *build,
  ...
  if (!isl_set_is_params(set))
    set = isl_set_preimage_multi_aff(set, isl_multi_aff_copy(build->values));
...
__isl_give isl_set *isl_ast_build_specialize(__isl_keep isl_ast_build *build,
  ...
  return isl_set_preimage_multi_aff(set, isl_multi_aff_copy(build->values));
}
...
isl_ast_build_compute_gist_pw_aff(__isl_keep isl_ast_build *build,
  ...
  if (!isl_set_is_params(build->domain))
    pa = isl_pw_aff_pullback_multi_aff(pa, isl_multi_aff_copy(build->values));
...
isl_ast_build_get_loop_type(__isl_keep isl_ast_build *build, int isolated) {
  ...
  return isl_schedule_node_band_member_get_isolate_ast_loop_type(build->node,
                                                                 local_pos);
}
...
isl_ast_build_extract_isolated(__isl_take isl_ast_build *build) {
  ...
  isolated = isl_set_preimage_multi_aff(
      isolated, isl_multi_aff_copy(build->internal2input));
 ...
 */
__isl_give isl_set *
isl_ast_build_get_isolated(__isl_keep isl_ast_build *build) {
  if (!build)
    return NULL;
  if (!build->internal2input)
    isl_die(isl_ast_build_get_ctx(build), isl_error_internal,
...
isl_ast_build_get_separation_class(__isl_keep isl_ast_build *build) {
  ...
  space_sep =
      isl_space_set_tuple_name(space_sep, isl_dim_set, "separation_class");
  space = isl_ast_build_get_space(build, 1);
