...
struct TrieNode {
  ...
  void addOrderedNodes(const Export &entry, std::vector<TrieNode *> &allNodes);
...
private:
  uint32_t loadCommandsSize(uint32_t &count, bool alwaysIncludeFunctionStarts);
  ...
  void appendSymbols(const std::vector<Symbol> &symbols, uint32_t &symOffset,
                     uint32_t &strOffset);
  ...
  template <typename T> llvm::Error writeSingleSegmentLoadCommand(uint8_t *&lc);
...
StringRef MachOFileLayout::dyldPath() { return "/usr/lib/dyld"; }
...
MachOFileLayout::MachOFileLayout(const NormalizedFile &file,
    ...
    : _file(file), _is64(MachOLinkingContext::is64Bit(file.arch)),
  ...
  if (file.fileType == llvm::MachO::MH_OBJECT) {
    ...
    _endOfLoadCommands = _startOfLoadCommands + segCommandBaseSize +
    ...
    _startOfSymbolStrings =
        _startOfIndirectSymbols +
        pointerAlign(_indirectSymbolTableCount * sizeof(uint32_t));
    _endOfSymbolStrings =
        _startOfSymbolStrings + pointerAlign(_symbolStringPoolSize);
    ...
    _endOfLoadCommands =
        _startOfLoadCommands +
        loadCommandsSize(_countOfLoadCommands, alwaysIncludeFunctionStarts);
...
void MachOFileLayout::buildFileOffsets() {
  ...
  DEBUG_WITH_TYPE("MachOFileLayout", llvm::dbgs() << "buildFileOffsets()\n");
  for (const Segment &sg : _file.segments) {
    ...
    DEBUG_WITH_TYPE("MachOFileLayout",
                    llvm::dbgs() << "  segment=" << sg.name << ", fileOffset="
                                 << _segInfo[&sg].fileOffset << "\n");
...
size_t MachOFileLayout::size() const { return _endOfSymbolStrings; }
...
llvm::Error MachOFileLayout::writeSingleSegmentLoadCommand(uint8_t *&lc) {
  ...
  if (seg->nsects) {
    seg->vmsize =
        _file.sections.back().address + _file.sections.back().content.size();
  ...
  if (_swap)
    swapStruct(*seg);
  typename T::section *sout =
      reinterpret_cast<typename T::section *>(lc + sizeof(typename T::command));
...
static void writeVersionMinLoadCommand(const NormalizedFile &_file, bool _swap,
                                       uint8_t *&lc) {
  if (!_file.hasMinVersionLoadCommand)
    return;
...
llvm::Error MachOFileLayout::writeLoadCommands() {
  ...
  if (_file.fileType == llvm::MachO::MH_OBJECT) {
    ...
    if (_file.fileType == llvm::MachO::MH_EXECUTE) {
      ...
      uint32_t size =
          pointerAlign(sizeof(dylinker_command) + dyldPath().size() + 1);
      ...
      if (_swap)
        swapStruct(*dl);
      memcpy(lc + sizeof(dylinker_command), dyldPath().data(),
             dyldPath().size());
...
void MachOFileLayout::writeRelocations() {
  ...
  for (Section sect : _file.sections) {
    for (Relocation r : sect.relocations) {
      any_relocation_info *rb =
          reinterpret_cast<any_relocation_info *>(&_buffer[relOffset]);
...
void MachOFileLayout::writeDataInCodeInfo() {
  ...
  for (const DataInCode &entry : _file.dataInCode) {
    data_in_code_entry *dst =
        reinterpret_cast<data_in_code_entry *>(&_buffer[offset]);
...
void MachOFileLayout::writeSymbolTable() {
  ...
  uint32_t *indirects =
      reinterpret_cast<uint32_t *>(&_buffer[_startOfIndirectSymbols]);
  if (_file.fileType == llvm::MachO::MH_OBJECT) {
...
void MachOFileLayout::writeBindingInfo() {
  memcpy(&_buffer[_startOfBindingInfo], _bindingInfo.bytes(),
         _bindingInfo.size());
}

void MachOFileLayout::writeLazyBindingInfo() {
  memcpy(&_buffer[_startOfLazyBindingInfo], _lazyBindingInfo.bytes(),
         _lazyBindingInfo.size());
}
...
void TrieNode::addSymbol(const Export &entry, BumpPtrAllocator &allocator,
                         std::vector<TrieNode *> &allNodes) {
  ...
  for (TrieEdge &edge : _children) {
    ...
    for (int n = edgeStr.size() - 1; n > 0; --n) {
      if (partialStr.substr(0, n).equals(edgeStr.substr(0, n))) {
        ...
        DEBUG_WITH_TYPE("trie-builder",
                        llvm::dbgs() << "splice in TrieNode('" << bNodeStr
                                     << "') between edge '" << abEdgeStr
                                     << "' and edge='" << bcEdgeStr << "'\n");
  ...
  DEBUG_WITH_TYPE("trie-builder", llvm::dbgs() << "new TrieNode('" << entry.name
                                               << "') with edge '" << partialStr
                                               << "' from node='"
                                               << _cummulativeString << "'\n");
  ...
  if (_hasExportInfo) {
    if (_flags & EXPORT_SYMBOL_FLAGS_REEXPORT) {
      if (!_importedName.empty()) {
        ...
        uint32_t nodeSize =
            llvm::getULEB128Size(_flags) + llvm::getULEB128Size(_other) + 1;
        assert(nodeSize < 256);
        out.append_byte(nodeSize);
        out.append_uleb128(_flags);
        out.append_uleb128(_other);
      ...
      uint32_t nodeSize =
          llvm::getULEB128Size(_flags) + llvm::getULEB128Size(_address);
      assert(nodeSize < 256);
      out.append_byte(nodeSize);
      out.append_uleb128(_flags);
      out.append_uleb128(_address);
...
void MachOFileLayout::computeSymbolTableSizes() {
  ...
  _symbolTableSize =
      nlistSize * (_file.stabsSymbols.size() + _file.localSymbols.size() +
                   _file.globalSymbols.size() + _file.undefinedSymbols.size());
  ...
  _symbolTableGlobalsStartIndex =
      _file.stabsSymbols.size() + _file.localSymbols.size();
  _symbolTableUndefinesStartIndex =
      _symbolTableGlobalsStartIndex + _file.globalSymbols.size();
