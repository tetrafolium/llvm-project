...
isl_schedule_band_dup(__isl_keep isl_schedule_band *band) {
  int i;
  isl_ctx *ctx;
  ...
  if (band->loop_type) {
    dup->loop_type = isl_alloc_array(ctx, enum isl_ast_loop_type, band->n);
  ...
  if (band->isolate_loop_type) {
    dup->isolate_loop_type =
        isl_alloc_array(ctx, enum isl_ast_loop_type, band->n);
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_cow(__isl_take isl_schedule_band *band) {
  if (!band)
    return NULL;
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_copy(__isl_keep isl_schedule_band *band) {
  if (!band)
    return NULL;
 ...
 */
__isl_null isl_schedule_band *
isl_schedule_band_free(__isl_take isl_schedule_band *band) {
  if (!band)
    return NULL;
...
isl_bool isl_schedule_band_plain_is_equal(__isl_keep isl_schedule_band *band1,
  ...
  if (band1->isolate_loop_type)
    for (i = 0; i < band1->n; ++i)
      if (band1->isolate_loop_type[i] != band2->isolate_loop_type[i])
        return isl_bool_false;
 ...
 */
isl_bool
isl_schedule_band_member_get_coincident(__isl_keep isl_schedule_band *band,
                                        int pos) {
  if (!band)
    return isl_bool_error;
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_set_permutable(__isl_take isl_schedule_band *band,
                                 int permutable) {
  if (!band)
    return NULL;
...
isl_schedule_band_get_space(__isl_keep isl_schedule_band *band) {
  if (!band)
    return NULL;
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_intersect_domain(__isl_take isl_schedule_band *band,
                                   __isl_take isl_union_set *domain) {
  band = isl_schedule_band_cow(band);
  if (!band || !domain)
    goto error;

  band->mupa = isl_multi_union_pw_aff_intersect_domain(band->mupa, domain);
  if (!band->mupa)
    return isl_schedule_band_free(band);
...
isl_schedule_band_get_partial_schedule(__isl_keep isl_schedule_band *band) {
...
isl_schedule_band_member_get_ast_loop_type(__isl_keep isl_schedule_band *band,
                                           int pos) {
  if (!band)
...
__isl_give isl_schedule_band *isl_schedule_band_member_set_ast_loop_type(
    __isl_take isl_schedule_band *band, int pos, enum isl_ast_loop_type type) {
  if (!band)
    return NULL;
  ...
  if (!band->loop_type) {
    ...
    ctx = isl_schedule_band_get_ctx(band);
    band->loop_type = isl_calloc_array(ctx, enum isl_ast_loop_type, band->n);
...
isl_schedule_band_member_set_isolate_ast_loop_type(
  ...
  if (!band)
    return NULL;
  if (isl_schedule_band_member_get_isolate_ast_loop_type(band, pos) == type)
    return band;
  ...
  if (!band->isolate_loop_type) {
    ...
    band->isolate_loop_type =
        isl_calloc_array(ctx, enum isl_ast_loop_type, band->n);
...
add_loop_types(__isl_take isl_union_set *options, int n,
...
isl_schedule_band_get_ast_build_options(__isl_keep isl_schedule_band *band) {
 ...
 */
static isl_bool
has_isolate_loop_type_options(__isl_keep isl_union_set *options) {
 ...
 */
static enum isl_ast_loop_type
extract_loop_type(__isl_keep isl_union_set *options, int pos, int isolate) {
  isl_ctx *ctx;
  ...
  for (type = isl_ast_loop_atomic; type <= isl_ast_loop_separate; ++type) {
    ...
    if (res != isl_ast_loop_default)
      isl_die(ctx, isl_error_invalid, "conflicting loop type options",
              return isl_ast_loop_error);
    res = type;
  }
...
static int extract_loop_types(__isl_keep isl_schedule_band *band,
  ...
  if (!band->loop_type) {
    ...
    band->loop_type = isl_alloc_array(ctx, enum isl_ast_loop_type, band->n);
    if (band->n && !band->loop_type)
      return -1;
  }
  for (i = 0; i < band->n; ++i) {
 ...
 */
static __isl_give isl_union_set *
clear_loop_types(__isl_take isl_union_set *options) {
  isl_union_set *types;
 ...
 */
static __isl_give isl_union_set *
clear_isolate_loop_types(__isl_take isl_union_set *options) {
  isl_union_set *types;
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_set_ast_build_options(__isl_take isl_schedule_band *band,
 ...
 */
__isl_give isl_set *
isl_schedule_band_get_ast_isolate_option(__isl_keep isl_schedule_band *band,
                                         int depth) {
  isl_space *space;
  isl_set *isolate;
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_replace_ast_build_option(__isl_take isl_schedule_band *band,
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_scale(__isl_take isl_schedule_band *band,
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_scale_down(__isl_take isl_schedule_band *band,
                             __isl_take isl_multi_val *mv) {
  band = isl_schedule_band_cow(band);
  if (!band || !mv)
    goto error;
  band->mupa = isl_multi_union_pw_aff_scale_down_multi_val(band->mupa, mv);
  band->mupa = isl_multi_union_pw_aff_floor(band->mupa);
  if (!band->mupa)
    return isl_schedule_band_free(band);
  return band;
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_mod(__isl_take isl_schedule_band *band,
                      __isl_take isl_multi_val *mv) {
  band = isl_schedule_band_cow(band);
  if (!band || !mv)
    goto error;
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_shift(__isl_take isl_schedule_band *band,
  ...
  if (!band || !shift)
    goto error;
  dom1 = isl_multi_union_pw_aff_domain(isl_multi_union_pw_aff_copy(band->mupa));
  dom2 = isl_multi_union_pw_aff_domain(isl_multi_union_pw_aff_copy(shift));
...
isl_multi_union_pw_aff_tile(__isl_take isl_multi_union_pw_aff *sched,
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_tile(__isl_take isl_schedule_band *band,
                       __isl_take isl_multi_val *sizes) {
  band = isl_schedule_band_cow(band);
  if (!band || !sizes)
    goto error;
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_point(__isl_take isl_schedule_band *band,
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_drop(__isl_take isl_schedule_band *band, int pos, int n) {
  int i;

  if (pos < 0 || n < 0 || pos + n > band->n)
    isl_die(isl_schedule_band_get_ctx(band), isl_error_internal,
            "range out of bounds", return isl_schedule_band_free(band));
  ...
  band->mupa =
      isl_multi_union_pw_aff_drop_dims(band->mupa, isl_dim_set, pos, n);
  if (!band->mupa)
    return isl_schedule_band_free(band);
  ...
  if (band->isolate_loop_type)
    for (i = pos + n; i < band->n; ++i)
      band->isolate_loop_type[i - n] = band->isolate_loop_type[i];
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_reset_user(__isl_take isl_schedule_band *band) {
  band = isl_schedule_band_cow(band);
  if (!band)
    return NULL;

  band->mupa = isl_multi_union_pw_aff_reset_user(band->mupa);
  band->ast_build_options = isl_union_set_reset_user(band->ast_build_options);
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_align_params(__isl_take isl_schedule_band *band,
  ...
  band->mupa =
      isl_multi_union_pw_aff_align_params(band->mupa, isl_space_copy(space));
...
__isl_give isl_schedule_band *isl_schedule_band_pullback_union_pw_multi_aff(
  ...
  band->mupa =
      isl_multi_union_pw_aff_pullback_union_pw_multi_aff(band->mupa, upma);
  if (!band->mupa)
    return isl_schedule_band_free(band);
 ...
 */
__isl_give isl_schedule_band *
isl_schedule_band_gist(__isl_take isl_schedule_band *band,
