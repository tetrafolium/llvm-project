 ...
 */
inline int isl_sioimath_is_small(isl_sioimath val) { return val & 0x00000001; }
 ...
 */
inline int32_t isl_sioimath_get_small(isl_sioimath val) { return val >> 32; }
...
typedef struct {
  mpz_t big;
  mp_digit
      digits[(sizeof(uintmax_t) + sizeof(mp_digit) - 1) / sizeof(mp_digit)];
...
#define ISL_SIOIMATH_TO_DIGITS(num, digits, used)                              \
  do {                                                                         \
    int i = 0;                                                                 \
    do {                                                                       \
      (digits)[i] = ((num) >> (sizeof(mp_digit) * CHAR_BIT * i));              \
      i += 1;                                                                  \
      if (i >= (sizeof(num) + sizeof(mp_digit) - 1) / sizeof(mp_digit))        \
        break;                                                                 \
...
inline double isl_sioimath_get_d(isl_sioimath_src val) {
  ...
  for (i = 0; i < big->used; ++i)
    result =
        result * (double)((uintmax_t)MP_DIGIT_MAX + 1) + (double)big->digits[i];
...
inline void isl_sioimath_add(isl_sioimath_ptr dst, isl_sioimath_src lhs,
  ...
  if (isl_sioimath_decode_small(lhs, &smalllhs) &&
    ...
    isl_sioimath_set_int64(dst, (int64_t)smalllhs + (int64_t)smallrhs);
    return;
  }
  ...
  if (isl_sioimath_decode_small(lhs, &smalllhs)) {
    ...
    default:
      if ((MP_SMALL_MIN <= rhs) && (rhs <= MP_SMALL_MAX)) {
        mp_int_expt_value(smalllhs, rhs, isl_sioimath_reinit_big(dst));
        isl_sioimath_try_demote(dst);
        return;
      }
    }
...
inline void isl_sioimath_cdiv_q(isl_sioimath_ptr dst, isl_sioimath_src lhs,
  ...
  if (isl_sioimath_decode_small(lhs, &lhssmall) &&
    ...
    if ((lhssmall >= 0) && (rhssmall >= 0))
      q = ((int64_t)lhssmall + (int64_t)rhssmall - 1) / rhssmall;
...
inline void isl_sioimath_cdiv_q_ui(isl_sioimath_ptr dst, isl_sioimath_src lhs,
  ...
  if (isl_sioimath_decode_small(lhs, &lhssmall) && (rhs <= INT32_MAX)) {
    if (lhssmall >= 0)
      q = ((int64_t)lhssmall + ((int64_t)rhs - 1)) / (int64_t)rhs;
    else
...
inline int isl_sioimath_cmp(isl_sioimath_src lhs, isl_sioimath_src rhs) {
  ...
  if (isl_sioimath_decode_small(rhs, &rhssmall))
    return mp_int_compare_value(isl_sioimath_bigarg_src(lhs, &lhsscratch),
                                rhssmall);

  if (isl_sioimath_decode_small(lhs, &lhssmall))
    return -mp_int_compare_value(isl_sioimath_bigarg_src(rhs, &rhsscratch),
                                 lhssmall);

  return mp_int_compare(isl_sioimath_get_big(lhs), isl_sioimath_get_big(rhs));
}
...
inline int isl_sioimath_abs_cmp(isl_sioimath_src lhs, isl_sioimath_src rhs) {
  ...
  return mp_int_compare_unsigned(isl_sioimath_bigarg_src(lhs, &lhsscratch),
                                 isl_sioimath_bigarg_src(rhs, &rhsscratch));
}
...
inline int isl_sioimath_is_divisible_by(isl_sioimath_src lhs,
  ...
  if (isl_sioimath_decode_small(rhs, &rhssmall))
    return mp_int_divisible_value(isl_sioimath_bigarg_src(lhs, &lhsscratch),
                                  rhssmall);
...
inline uint32_t isl_sioimath_hash(isl_sioimath_src arg, uint32_t hash) {
  ...
  int i;
  uint32_t num;
  mp_digit digits[(sizeof(uint32_t) + sizeof(mp_digit) - 1) / sizeof(mp_digit)];
  mp_size used;
