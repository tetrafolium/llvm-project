...
public:
    ...
    bool needsRelocations() {
        return internalGotSymbols.size();
    }
...
public:
    ...
    bool isNeeded() const override {
        return indirectFunctions.size() > 0;
    };
    void writeBody() override;
    void addEntry(FunctionSymbol *sym);
    uint32_t numEntries() const {
        return indirectFunctions.size();
    }
...
public:
    ...
    unsigned numNames() const {
        return numNamedGlobals() + numNamedFunctions();
    }
...
public:
    ...
    void writeBody() override;
    bool isNeeded() const override {
        return sec->getNumRelocations() > 0;
    };
