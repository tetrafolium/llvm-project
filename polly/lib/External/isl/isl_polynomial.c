...
isl_bool isl_poly_is_equal(__isl_keep isl_poly *poly1,
  ...
  if (is_cst1) {
    ...
    if (!cst1 || !cst2)
      return isl_bool_error;
    r = isl_int_eq(cst1->n, cst2->n) && isl_int_eq(cst1->d, cst2->d);
...
isl_qpolynomial_reset_domain_space(__isl_take isl_qpolynomial *qp,
 ...
 */
__isl_give isl_qpolynomial *
isl_qpolynomial_reset_space_and_domain(__isl_take isl_qpolynomial *qp,
                                       __isl_take isl_space *space,
...
isl_qpolynomial_peek_domain_space(__isl_keep isl_qpolynomial *qp) {
...
#undef PEEK_SPACE
     ...
     */
    static __isl_give isl_local_space *
    isl_qpolynomial_get_domain_local_space(__isl_keep isl_qpolynomial *qp) {
  isl_space *space;
...
__isl_give isl_space *
isl_qpolynomial_get_space(__isl_keep isl_qpolynomial *qp) {
  isl_space *space;
  if (!qp)
    return NULL;
 ...
 */
static isl_size
isl_qpolynomial_domain_var_offset(__isl_keep isl_qpolynomial *qp,
                                  enum isl_dim_type type) {
  isl_space *space;
...
static __isl_give isl_poly *
replace_by_constant_term(__isl_take isl_poly *poly) {
  isl_poly_rec *rec;
  isl_poly *cst;
...
__isl_give isl_poly *isl_poly_sum(__isl_take isl_poly *poly1,
  ...
  for (i = rec2->n - 1; i >= 0; --i) {
    ...
    rec1->p[i] = isl_poly_sum(rec1->p[i], isl_poly_copy(rec2->p[i]));
    if (!rec1->p[i])
      goto error;
    if (i != rec1->n - 1)
      continue;
...
__isl_give isl_poly *isl_poly_mul_rec(__isl_take isl_poly *poly1,
  ...
  for (i = 0; i < rec1->n; ++i) {
    res->p[i] =
        isl_poly_mul(isl_poly_copy(rec2->p[0]), isl_poly_copy(rec1->p[i]));
    if (!res->p[i])
      goto error;
    res->n++;
  }
  ...
  for (i = 0; i < rec1->n; ++i) {
    for (j = 1; j < rec2->n; ++j) {
      isl_poly *poly;
      poly = isl_poly_mul(isl_poly_copy(rec2->p[j]), isl_poly_copy(rec1->p[i]));
...
__isl_give isl_poly *isl_poly_mul(__isl_take isl_poly *poly1,
  ...
  if (poly2->var < poly1->var) {
    ...
    for (i = 0; i < rec->n; ++i) {
      rec->p[i] = isl_poly_mul(rec->p[i], isl_poly_copy(poly2));
      if (!rec->p[i])
        goto error;
    }
    isl_poly_free(poly2);
...
__isl_give isl_qpolynomial *
isl_qpolynomial_dup(__isl_keep isl_qpolynomial *qp) {
...
__isl_give isl_qpolynomial *
isl_qpolynomial_cow(__isl_take isl_qpolynomial *qp) {
  if (!qp)
    return NULL;
...
static isl_bool compatible_divs(__isl_keep isl_mat *div1,
  ...
  isl_assert(div1->ctx,
             div1->n_row >= div2->n_row && div1->n_col >= div2->n_col,
             return isl_bool_error);
...
static __isl_give isl_qpolynomial *sort_divs(__isl_take isl_qpolynomial *qp) {
  ...
  array =
      isl_alloc_array(qp->div->ctx, struct isl_div_sort_info, qp->div->n_row);
  ...
  qsort(array, qp->div->n_row, sizeof(struct isl_div_sort_info), div_sort_cmp);
  ...
  for (i = 0; i < len - div_pos; ++i) {
    if (i > 0 && isl_seq_eq(qp->div->row[i - skip - 1], qp->div->row[i - skip],
                            qp->div->n_col)) {
      qp->div = isl_mat_drop_rows(qp->div, i - skip, 1);
      isl_mat_col_add(qp->div, 2 + div_pos + i - skip - 1,
                      2 + div_pos + i - skip);
      qp->div = isl_mat_drop_cols(qp->div, 2 + div_pos + i - skip, 1);
      skip++;
    }
...
static __isl_give isl_qpolynomial *with_merged_divs(
  ...
  isl_assert(qp1->div->ctx,
             qp1->div->n_row >= qp2->div->n_row &&
                 qp1->div->n_col >= qp2->div->n_col,
             goto error);
...
__isl_give isl_qpolynomial *
isl_qpolynomial_add_on_domain(__isl_keep isl_set *dom,
...
__isl_give isl_qpolynomial *
isl_qpolynomial_add_isl_int(__isl_take isl_qpolynomial *qp, isl_int v) {
...
__isl_give isl_qpolynomial *
isl_qpolynomial_neg(__isl_take isl_qpolynomial *qp) {
  if (!qp)
    return NULL;
...
__isl_give isl_qpolynomial *
isl_qpolynomial_mul_isl_int(__isl_take isl_qpolynomial *qp, isl_int v) {
  if (isl_int_is_one(v))
    return qp;
...
__isl_give isl_qpolynomial *
isl_qpolynomial_scale(__isl_take isl_qpolynomial *qp, isl_int v) {
...
isl_qpolynomial_scale_down_val(__isl_take isl_qpolynomial *qp,
  ...
  if (!isl_val_is_rat(v))
    isl_die(isl_qpolynomial_get_ctx(qp), isl_error_invalid,
            "expecting rational factor", goto error);
  if (isl_val_is_zero(v))
    isl_die(isl_val_get_ctx(v), isl_error_invalid, "cannot scale down by zero",
            goto error);
...
isl_pw_qpolynomial_pow(__isl_take isl_pw_qpolynomial *pwqp, unsigned power) {
  int i;
...
__isl_give isl_qpolynomial *
isl_qpolynomial_cst_on_domain(__isl_take isl_space *domain, isl_int v) {
...
isl_bool isl_qpolynomial_is_cst(__isl_keep isl_qpolynomial *qp, isl_int *n,
                                isl_int *d) {
  isl_bool is_cst;
  isl_poly_cst *cst;
...
isl_poly_get_constant_val(__isl_keep isl_poly *poly) {
  isl_bool is_cst;
  isl_poly_cst *cst;
...
static void update_coeff(__isl_keep isl_vec *aff, __isl_keep isl_poly_cst *cst,
                         int pos) {
  isl_int gcd;
  isl_int f;
...
__isl_give isl_vec *
isl_qpolynomial_extract_affine(__isl_keep isl_qpolynomial *qp) {
  isl_vec *aff;
  isl_size d;
...
__isl_give isl_qpolynomial *
isl_qpolynomial_var_pow_on_domain(__isl_take isl_space *domain, int pos,
                                  int power) {
  struct isl_ctx *ctx;
  ...
  return isl_qpolynomial_alloc(domain, 0, isl_poly_var_pow(ctx, pos, power));
}
...
__isl_give isl_poly *isl_poly_subs(__isl_take isl_poly *poly, unsigned first,
                                   unsigned n, __isl_keep isl_poly **subs) {
  int i;
  isl_bool is_cst;
  isl_poly_rec *rec;
  isl_poly *base, *res;
...
static void normalize_div(__isl_keep isl_qpolynomial *qp, int div) {
  ...
  isl_int_gcd(ctx->normalize_gcd, ctx->normalize_gcd, qp->div->row[div][0]);
...
substitute_div(__isl_take isl_qpolynomial *qp, int div,
 ...
 */
static __isl_give isl_qpolynomial *
substitute_non_divs(__isl_take isl_qpolynomial *qp) {
  int i, j;
  isl_size div_pos;
  isl_poly *s;
  ...
  for (i = 0; qp && i < qp->div->n_row; ++i) {
    ...
    for (j = i + 1; j < qp->div->n_row; ++j) {
      if (isl_int_is_zero(qp->div->row[j][2 + div_pos + i]))
        continue;
      isl_seq_combine(qp->div->row[j] + 1, qp->div->ctx->one,
                      qp->div->row[j] + 1, qp->div->row[j][2 + div_pos + i],
...
static void invert_div(__isl_keep isl_qpolynomial *qp, int div,
  ...
  isl_seq_neg(qp->div->row[div] + 1, qp->div->row[div] + 1, qp->div->n_col - 1);
  isl_int_sub_ui(qp->div->row[div][1], qp->div->row[div][1], 1);
  isl_int_add(qp->div->row[div][1], qp->div->row[div][1], qp->div->row[div][0]);
  *mat = isl_mat_col_neg(*mat, 1 + total + div);
  isl_mat_col_mul(qp->div, 2 + total + div, qp->div->ctx->negone,
                  2 + total + div);
}
...
static __isl_give isl_qpolynomial *reduce_divs(__isl_take isl_qpolynomial *qp) {
  ...
  if (n_div && !s)
    goto error;
  for (i = 0; i < n_div; ++i)
    s[i] = isl_poly_from_affine(ctx, mat->row[i], ctx->one, 1 + total);
...
__isl_give isl_qpolynomial *
isl_qpolynomial_rat_cst_on_domain(__isl_take isl_space *domain, const isl_int n,
                                  const isl_int d) {
  struct isl_qpolynomial *qp;
  isl_poly_cst *cst;
 ...
 */
__isl_give isl_qpolynomial *
isl_qpolynomial_val_on_domain(__isl_take isl_space *domain,
...
#include "check_type_range_templ.c"
    ...
    isl_bool
    isl_qpolynomial_involves_dims(__isl_keep isl_qpolynomial *qp,
  ...
  if (isl_qpolynomial_check_range(qp, type, first, n) < 0)
    return isl_bool_error;
  isl_assert(qp->dim->ctx, type == isl_dim_param || type == isl_dim_in,
             return isl_bool_error);
 ...
 */
static __isl_give isl_qpolynomial *
remove_redundant_divs(__isl_take isl_qpolynomial *qp) {
  int i, j;
  isl_size div_pos;
  int len;
  int skip;
...
__isl_give isl_poly *isl_poly_drop(__isl_take isl_poly *poly, unsigned first,
                                   unsigned n) {
  int i;
  isl_poly_rec *rec;
...
isl_qpolynomial_drop_dims(__isl_take isl_qpolynomial *qp,
  ...
  if (!qp)
    return NULL;
  if (type == isl_dim_out)
    isl_die(qp->dim->ctx, isl_error_invalid, "cannot drop output/set dimension",
            goto error);
  if (isl_qpolynomial_check_range(qp, type, first, n) < 0)
    return isl_qpolynomial_free(qp);
  type = domain_type(type);
  ...
  isl_assert(qp->dim->ctx, type == isl_dim_param || type == isl_dim_set,
             goto error);
...
static __isl_give isl_qpolynomial *
isl_qpolynomial_substitute_equalities_lifted(__isl_take isl_qpolynomial *qp,
  ...
  for (i = 0; i < eq->n_eq; ++i) {
    ...
    poly = isl_poly_from_affine(qp->dim->ctx, eq->eq[i], denom, total);
 ...
 */
__isl_give isl_qpolynomial *
isl_qpolynomial_substitute_equalities(__isl_take isl_qpolynomial *qp,
                                      __isl_take isl_basic_set *eq) {
  if (!qp || !eq)
    goto error;
  if (qp->div->n_row > 0)
 ...
 */
static __isl_give isl_qpolynomial *
isl_qpolynomial_zero_in_space(__isl_take isl_space *space) {
...
isl_pw_qpolynomial_mul(__isl_take isl_pw_qpolynomial *pwqp1,
  ...
  for (i = 0; i < pwqp1->n; ++i) {
    for (j = 0; j < pwqp2->n; ++j) {
      ...
      prod = isl_qpolynomial_mul(isl_qpolynomial_copy(pwqp1->p[i].qp),
...
__isl_give isl_val *isl_poly_eval(__isl_take isl_poly *poly,
  ...
  base =
      isl_val_rat_from_isl_int(poly->ctx, vec->el[1 + poly->var], vec->el[0]);

  res = isl_poly_eval(isl_poly_copy(rec->p[rec->n - 1]), isl_vec_copy(vec));

  for (i = rec->n - 2; i >= 0; --i) {
    res = isl_val_mul(res, isl_val_copy(base));
    res = isl_val_add(
        res, isl_poly_eval(isl_poly_copy(rec->p[i]), isl_vec_copy(vec)));
  }
...
isl_qpolynomial_insert_dims(__isl_take isl_qpolynomial *qp,
  ...
  if (type == isl_dim_out)
    isl_die(qp->div->ctx, isl_error_invalid,
            "cannot insert output/set dimensions", goto error);
...
static int *reordering_move(isl_ctx *ctx, unsigned len, unsigned dst,
...
__isl_give isl_qpolynomial *
isl_qpolynomial_move_dims(__isl_take isl_qpolynomial *qp,
  ...
  if (dst_type == isl_dim_out || src_type == isl_dim_out)
    isl_die(qp->dim->ctx, isl_error_invalid, "cannot move output/set dimension",
            goto error);
  ...
  if (n == 0 && !isl_space_is_named_or_nested(qp->dim, src_type) &&
  ...
  reordering = reordering_move(qp->dim->ctx, qp->div->n_col - 2, g_dst_pos,
                               g_src_pos, n);
  if (!reordering)
    goto error;
  ...
  qp->dim =
      isl_space_move_dims(qp->dim, dst_type, dst_pos, src_type, src_pos, n);
  if (!qp->dim)
    goto error;
...
__isl_give isl_qpolynomial *
isl_qpolynomial_from_affine(__isl_take isl_space *space, isl_int *f,
                            isl_int denom) {
  isl_size d;
  isl_poly *poly;
...
__isl_give isl_qpolynomial *isl_qpolynomial_from_aff(__isl_take isl_aff *aff) {
  ...
  qp = isl_qpolynomial_alloc(isl_aff_get_domain_space(aff), aff->ls->div->n_row,
                             poly);
  if (!qp)
    goto error;
...
isl_pw_qpolynomial_from_pw_aff(__isl_take isl_pw_aff *pwaff) {
  ...
  pwqp = isl_pw_qpolynomial_alloc_size(isl_pw_aff_get_space(pwaff), pwaff->n);
...
isl_qpolynomial_substitute(__isl_take isl_qpolynomial *qp,
  ...
  if (type == isl_dim_out)
    isl_die(qp->dim->ctx, isl_error_invalid,
            "cannot substitute output/set dimension", goto error);
  if (isl_qpolynomial_check_range(qp, type, first, n) < 0)
    return isl_qpolynomial_free(qp);
  type = domain_type(type);
...
isl_stat isl_qpolynomial_as_polynomial_on_domain(
  ...
  if (!qp || !bset)
    return isl_stat_error;
  if (qp->div->n_row == 0)
    return fn(isl_basic_set_copy(bset), isl_qpolynomial_copy(qp), user);
...
__isl_give isl_poly *isl_poly_coeff(__isl_keep isl_poly *poly, unsigned pos,
                                    int deg) {
  int i;
  isl_bool is_cst;
  isl_poly_rec *rec;
 ...
 */
__isl_give isl_qpolynomial *
isl_qpolynomial_coeff(__isl_keep isl_qpolynomial *qp, enum isl_dim_type type,
                      unsigned t_pos, int deg) {
  unsigned g_pos;
  isl_poly *poly;
  isl_qpolynomial *c;
  ...
  c = isl_qpolynomial_alloc(isl_space_copy(qp->dim), qp->div->n_row, poly);
  if (!c)
    return NULL;
  isl_mat_free(c->div);
...
__isl_give isl_poly *isl_poly_homogenize(__isl_take isl_poly *poly, int deg,
  ...
  for (i = 0; i < rec->n; ++i) {
    ...
    if (is_zero)
      continue;
    rec->p[i] = isl_poly_homogenize(rec->p[i], poly->var < last ? deg + i : i,
                                    target, first, last);
    if (!rec->p[i])
      goto error;
  }
...
isl_size isl_term_dim(__isl_keep isl_term *term, enum isl_dim_type type) {
  ...
  case isl_dim_in:
  case isl_dim_out:
    return isl_space_dim(term->dim, type);
  case isl_dim_div:
    return term->div->n_row;
  case isl_dim_all:
    dim = isl_space_dim(term->dim, isl_dim_all);
    if (dim < 0)
      return isl_size_error;
...
static isl_size isl_term_offset(__isl_keep isl_term *term,
  ...
  case isl_dim_div:
    return isl_space_dim(space, isl_dim_all);
  default:
    isl_die(isl_term_get_ctx(term), isl_error_invalid, "invalid dimension type",
            return isl_size_error);
  }
}
...
__isl_give isl_val *isl_term_get_coefficient_val(__isl_keep isl_term *term) {
  ...
  return isl_val_rat_from_isl_int(isl_term_get_ctx(term), term->n, term->d);
}
...
#include "check_type_range_templ.c"
    ...
    isl_size
    isl_term_get_exp(__isl_keep isl_term *term, enum isl_dim_type type,
                     unsigned pos) {
  isl_size offset;
...
__isl_give isl_term *
isl_poly_foreach_term(__isl_keep isl_poly *poly,
  ...
  if (is_bad)
    isl_die(isl_term_get_ctx(term), isl_error_invalid,
            "cannot handle NaN/infty polynomial", return isl_term_free(term));
...
isl_qpolynomial_from_term(__isl_take isl_term *term) {
  ...
  for (i = 0; i < n; ++i) {
    if (!term->pow[i])
      continue;
    poly =
        isl_poly_mul(poly, isl_poly_var_pow(term->dim->ctx, i, term->pow[i]));
  }

  qp = isl_qpolynomial_alloc(isl_space_copy(term->dim), term->div->n_row, poly);
  if (!qp)
    goto error;
  isl_mat_free(qp->div);
...
__isl_give isl_val *
isl_qpolynomial_opt_on_domain(__isl_take isl_qpolynomial *qp,
                              __isl_take isl_set *set, int max) {
...
__isl_give isl_qpolynomial *
isl_qpolynomial_morph_domain(__isl_take isl_qpolynomial *qp,
 ...
 */
__isl_give isl_qpolynomial *
isl_qpolynomial_realign_domain(__isl_take isl_qpolynomial *qp,
...
isl_qpolynomial_align_params(__isl_take isl_qpolynomial *qp,
  ...
  if (!equal_params) {
    ...
    exp =
        isl_reordering_extend_space(exp, isl_qpolynomial_get_domain_space(qp));
 ...
 */
static isl_stat set_div(__isl_take isl_set *set, __isl_take isl_qpolynomial *qp,
                        int div, isl_int v,
  ...
  for (i = div + 1; i < qp->div->n_row; ++i) {
    if (isl_int_is_zero(qp->div->row[i][2 + div_pos + div]))
      continue;
    isl_int_addmul(qp->div->row[i][1], qp->div->row[i][2 + div_pos + div], v);
...
static isl_stat split_div(__isl_take isl_set *set,
                          __isl_take isl_qpolynomial *qp, int div, isl_int min,
                          isl_int max, struct isl_split_periods_data *data) {
...
static isl_stat split_periods(__isl_take isl_set *set,
  ...
  for (i = 0; i < qp->div->n_row; ++i) {
    ...
    if (isl_seq_first_non_zero(qp->div->row[i] + 2 + div_pos, qp->div->n_row) !=
        -1)
      continue;

    lp_res = isl_set_solve_lp(set, 0, qp->div->row[i] + 1, set->ctx->one, &min,
                              NULL, NULL);
    ...
    lp_res = isl_set_solve_lp(set, 1, qp->div->row[i] + 1, set->ctx->one, &max,
                              NULL, NULL);
    if (lp_res == isl_lp_error)
      goto error2;
    if (lp_res == isl_lp_unbounded || lp_res == isl_lp_empty)
 ...
 */
static __isl_give isl_pw_qpolynomial *
constant_on_domain(__isl_take isl_basic_set *bset, int cst) {
  isl_space *space;
  isl_qpolynomial *qp;
 ...
 */
static isl_bool
multiplicative_call_factor_pw_qpolynomial(__isl_keep isl_basic_set *bset,
                                          void *user) {
...
static __isl_give isl_pw_qpolynomial *compressed_multiplicative_call(
  ...
  every = isl_factorizer_every_factor_basic_set(
      f, &multiplicative_call_factor_pw_qpolynomial, &data);
  if (every < 0)
 ...
 */
static __isl_give isl_qpolynomial *
qp_drop_floors(__isl_take isl_qpolynomial *qp, int down) {
  int i;
  isl_poly *s;
  ...
  for (i = qp->div->n_row - 1; i >= 0; --i) {
    if (down) {
      isl_int_sub(qp->div->row[i][1], qp->div->row[i][1], qp->div->row[i][0]);
      isl_int_add_ui(qp->div->row[i][1], qp->div->row[i][1], 1);
    }
    s = isl_poly_from_affine(qp->dim->ctx, qp->div->row[i] + 1,
                             qp->div->row[i][0], qp->div->n_col - 1);
    qp = substitute_div(qp, i, s);
 ...
 */
static __isl_give isl_pw_qpolynomial *
pwqp_drop_floors(__isl_take isl_pw_qpolynomial *pwqp) {
  int i;
  ...
  for (i = 0; i < qp->div->n_row; ++i) {
    ...
    for (j = 0; j < i; ++j) {
      if (isl_int_sgn(row[2 + div_pos + j]) >= 0)
        continue;
      isl_int_fdiv_q(v->el[1 + div_pos + j], row[2 + div_pos + j], row[0]);
      isl_int_submul(row[2 + div_pos + j], row[0], v->el[1 + div_pos + j]);
    }
    for (j = i + 1; j < qp->div->n_row; ++j) {
      if (isl_int_is_zero(qp->div->row[j][2 + div_pos + i]))
        continue;
    ...
    s = isl_poly_from_affine(qp->dim->ctx, v->el, qp->div->ctx->one, v->size);
 ...
 */
__isl_give isl_pw_qpolynomial *
isl_pw_qpolynomial_to_polynomial(__isl_take isl_pw_qpolynomial *pwqp,
                                 int sign) {
  int i;
  ...
  for (i = 0; i < pwqp->n; ++i) {
    if (pwqp->p[i].qp->div->n_row == 0) {
      isl_pw_qpolynomial *t;
      t = isl_pw_qpolynomial_alloc(isl_set_copy(pwqp->p[i].set),
                                   isl_qpolynomial_copy(pwqp->p[i].qp));
      data.res = isl_pw_qpolynomial_add_disjoint(data.res, t);
      continue;
    }
    data.qp = pwqp->p[i].qp;
    if (isl_set_foreach_orthant(pwqp->p[i].set, &to_polynomial_on_orthant,
                                &data) < 0)
      goto error;
  }
...
static __isl_give isl_pw_qpolynomial *
poly_entry(__isl_take isl_pw_qpolynomial *pwqp, void *user) {
  int *sign = user;
...
__isl_give isl_union_pw_qpolynomial *isl_union_pw_qpolynomial_to_polynomial(
  ...
  return isl_union_pw_qpolynomial_transform_inplace(upwqp, &poly_entry, &sign);
}
