...
public:
    typename Ty::iterator begin() {
        return _atoms.begin();
    }
    typename Ty::iterator end() {
        return _atoms.end();
    }
    Ty _atoms;
};
...
template <typename T> struct SequenceTraits<File::AtomRange<T> > {
    static size_t size(IO &io, File::AtomRange<T> &seq) {
        return seq.size();
    }
...
template <> struct MappingTraits<const lld::Reference *> {
    ...
    public:
        ...
        const lld::Atom *target() const override {
            return _target;
        }
        ...
        void setTarget(const lld::Atom *a) override {
            _target = a;
        }
...
template <> struct MappingTraits<const lld::DefinedAtom *> {
    ...
    public:
        ...
        uint64_t sectionSize() const override {
            return _sectionSize;
        }
        ...
        DynamicExport dynamicExport() const override {
            return _dynamicExport;
        }
        CodeModel codeModel() const override {
            return _codeModel;
        }
