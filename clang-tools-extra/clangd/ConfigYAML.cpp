...
public:
    ...
    bool parse(Fragment &F, Node &N) {
        ...
        Dict.handle("CompileFlags", [&](Node &N) {
            parse(F.CompileFlags, N);
        });
        Dict.handle("Index", [&](Node &N) {
            parse(F.Index, N);
        });
        Dict.handle("Style", [&](Node &N) {
            parse(F.Style, N);
        });
        Dict.handle("ClangTidy", [&](Node &N) {
            parse(F.ClangTidy, N);
        });
        Dict.parse(N);
...
private:
    ...
    void parse(Fragment::IndexBlock &F, Node &N) {
        ...
        [&](Node &N) {
            F.Background = scalarValue(N, "Background");
        });
    ...
    void parse(Fragment::IndexBlock::ExternalBlock &F, Node &N) {
        ...
        Dict.handle("File", [&](Node &N) {
            F.File = scalarValue(N, "File");
        });
        Dict.handle("Server",
        [&](Node &N) {
        ...
        [&](Node &N) {
            F.MountPoint = scalarValue(N, "MountPoint");
        });
        Dict.parse(N);
    }
