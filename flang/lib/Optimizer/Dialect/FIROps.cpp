...
static void printCmpfOp(OpAsmPrinter &p, CmpfOp op) {
    printCmpOp(p, op);
}
...
static void printCmpcOp(OpAsmPrinter &p, fir::CmpcOp op) {
    printCmpOp(p, op);
}
...
mlir::Region &fir::IterWhileOp::getLoopBody() {
    return region();
}
...
mlir::Region &fir::LoopOp::getLoopBody() {
    return region();
}
