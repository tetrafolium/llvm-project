...
struct OMPInformationCache : public InformationCache {
    ...
    struct RuntimeFunctionInfo {
        ...
        size_t getNumFunctionsWithUses() const {
            return UsesMap.size();
        }
...
private:
    ...
    bool isKernel(Function &F) {
        return OMPInfoCache.Kernels.count(&F);
    }
...
struct AAICVTracker : public StateWrapper<BooleanState, AbstractAttribute> {
    ...
    bool isAssumedTracked() const {
        return getAssumed();
    }

    /// Returns true if value is known to be tracked.
    bool isKnownTracked() const {
        return getAssumed();
    }
...
Pass *llvm::createOpenMPOptLegacyPass() {
    return new OpenMPOptLegacyPass();
}
