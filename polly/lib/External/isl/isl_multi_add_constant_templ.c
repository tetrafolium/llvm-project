...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), add_constant_val)(__isl_take MULTI(BASE) * multi,
  ...
  for (i = 0; i < n; ++i) {
    multi->u.p[i] = FN(EL, add_constant_val)(multi->u.p[i], isl_val_copy(v));
    if (!multi->u.p[i])
      goto error;
  }
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), add_constant_multi_val)(__isl_take MULTI(BASE) * multi,
  ...
  equal =
      isl_space_tuple_is_equal(multi_space, isl_dim_out, mv_space, isl_dim_out);
