...
protected:
    ...
    operator AbstractAttribute *() {
        return cast<AbstractAttribute>(this);
    }
...
public:
    ...
    aaiterator begin() {
        return aaiterator(Deps.begin(), &DepGetValAA);
    }
    aaiterator end() {
        return aaiterator(Deps.end(), &DepGetValAA);
    }
    iterator child_begin() {
        return iterator(Deps.begin(), &DepGetVal);
    }
    iterator child_end() {
        return iterator(Deps.end(), &DepGetVal);
    }
...
struct AADepGraph {
    ...
    iterator begin() {
        return SyntheticRoot.child_begin();
    }
    iterator end() {
        return SyntheticRoot.child_end();
    }
...
struct IRPosition {
    ...
    IRPosition() : Enc(nullptr, ENC_VALUE) {
        verify();
    }
...
template <> struct DenseMapInfo<IRPosition> : DenseMapInfo<void *> {
    static inline IRPosition getEmptyKey() {
        return IRPosition::EmptyKey;
    }
    ...
    InformationCache(const Module &M, AnalysisGetter &AG,
        ...
        : DL(M.getDataLayout()), Allocator(Allocator),
          Explorer(
              ...
              /* LIGetter */
              [&](const Function &F) {
        return AG.getAnalysis<LoopAnalysis>(F);
    },
    /* DTGetter */
    [&](const Function &F) {
...
struct DerefState : AbstractState {
    ...
    /// See AbstractState::isValidState()
    bool isValidState() const override {
        return DerefBytesState.isValidState();
    }
...
struct PotentialValuesState : AbstractState {
    ...
    /// See AbstractState::isValidState(...)
    bool isValidState() const override {
        return IsValidState.isValidState();
    }

    /// See AbstractState::isAtFixpoint(...)
    bool isAtFixpoint() const override {
        return IsValidState.isAtFixpoint();
    }
...
private:
    ...
    void reduceUndefValue() {
        UndefIsContained = UndefIsContained & Set.empty();
    }
