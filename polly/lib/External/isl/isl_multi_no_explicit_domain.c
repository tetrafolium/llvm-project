 ...
 * No.
 */
static int FN(MULTI(BASE),
              has_explicit_domain)(__isl_keep MULTI(BASE) * multi) {
  return 0;
}
...
static __isl_give MULTI(BASE) *
    FN(MULTI(BASE), init_explicit_domain)(__isl_take MULTI(BASE) * multi) {
  return multi;
}
 ...
 */
static __isl_give MULTI(BASE) *
    FN(MULTI(BASE), intersect_explicit_domain)(__isl_take MULTI(BASE) * dst,
                                               __isl_keep MULTI(BASE) * src) {
  return dst;
}
 ...
 */
static __isl_give MULTI(BASE) *
    FN(MULTI(BASE), copy_explicit_domain)(__isl_take MULTI(BASE) * dst,
                                          __isl_keep MULTI(BASE) * src) {
  return dst;
}
...
static __isl_give MULTI(BASE) *
    FN(MULTI(BASE),
       intersect_explicit_domain_product)(__isl_take MULTI(BASE) * dst,
                                          __isl_keep MULTI(BASE) * src1,
...
static __isl_give MULTI(BASE) *
    FN(MULTI(BASE),
       align_explicit_domain_params)(__isl_take MULTI(BASE) * multi,
 ...
 */
static __isl_give MULTI(BASE) *
    FN(MULTI(BASE), reset_explicit_domain_space)(__isl_take MULTI(BASE) * multi,
                                                 __isl_take isl_space *space) {
  isl_space_free(space);
  return multi;
}
...
isl_bool FN(MULTI(BASE),
            involves_explicit_domain_dims)(__isl_keep MULTI(BASE) * multi,
...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), insert_explicit_domain_dims)(__isl_take MULTI(BASE) * multi,
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), drop_explicit_domain_dims)(__isl_take MULTI(BASE) * multi,
                                               enum isl_dim_type type,
                                               unsigned pos, unsigned n) {
  return multi;
}
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), move_explicit_domain_dims)(__isl_take MULTI(BASE) * multi,
 ...
 * "multi" cannot have an explicit domain, so this function is never called.
 */
static void FN(MULTI(BASE),
               free_explicit_domain)(__isl_keep MULTI(BASE) * multi) {}
...
static isl_bool FN(MULTI(BASE),
                   equal_explicit_domain)(__isl_keep MULTI(BASE) * multi1,
                                          __isl_keep MULTI(BASE) * multi2) {
  return isl_bool_true;
}

static isl_stat FN(MULTI(BASE),
                   check_explicit_domain)(__isl_keep MULTI(BASE) * multi)
    __attribute__((unused));
