...
void ShapeDialect::printType(Type type, DialectAsmPrinter &os) const {
    ...
    .Case<ValueShapeType>([&](Type) {
        os << "value_shape";
    })
    .Case<WitnessType>([&](Type) {
        os << "witness";
    })
    .Default([](Type) {
...
OpFoldResult ConstShapeOp::fold(ArrayRef<Attribute>) {
    return shapeAttr();
}
...
OpFoldResult ConstSizeOp::fold(ArrayRef<Attribute>) {
    return valueAttr();
}
...
OpFoldResult FromExtentsOp::fold(ArrayRef<Attribute> operands) {
    if (llvm::any_of(operands, [](Attribute a) {
    return !a;
}))
    return nullptr;
