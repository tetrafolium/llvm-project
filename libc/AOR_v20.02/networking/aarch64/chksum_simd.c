...
always_inline static inline uint64_t slurp_head64(const void **pptr,
                                                  uint32_t *nbytes) {
  Assert(*nbytes >= 8);
  uint64_t sum = 0;
...
always_inline static inline uint64_t slurp_tail64(uint64_t sum, const void *ptr,
                                                  uint32_t nbytes) {
  Assert(nbytes < 8);
  if (likely(nbytes != 0)) {
...
unsigned short __chksum_aarch64_simd(const void *ptr, unsigned int nbytes) {
