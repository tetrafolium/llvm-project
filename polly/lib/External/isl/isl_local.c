...
#include "check_type_range_templ.c"
     ...
     */
    static isl_stat
    isl_local_check_pos(__isl_keep isl_local *local, int pos) {
...
__isl_give isl_local *isl_local_reorder(__isl_take isl_local *local,
  ...
  for (i = 0; i < div->n_row; ++i) {
    ...
    for (j = 0; j < r->len; ++j)
      isl_int_set(mat->row[i][2 + r->pos[j]], div->row[i][2 + j]);
  }
...
__isl_give isl_vec *isl_local_extend_point_vec(__isl_keep isl_local *local,
  ...
  if (dim < 0 || n_div < 0 || size < 0)
    return isl_vec_free(v);
  if (size != 1 + dim)
    isl_die(isl_local_get_ctx(local), isl_error_invalid, "incorrect size",
            return isl_vec_free(v));
  if (n_div == 0)
    return v;
            ...
            "expecting integer point", return isl_vec_free(v));
  {
    ...
    for (i = 0; i < n_div; ++i) {
      isl_seq_inner_product(mat->row[i] + 1, v->el, 1 + dim + i,
                            &v->el[1 + dim + i]);
      isl_int_fdiv_q(v->el[1 + dim + i], v->el[1 + dim + i], mat->row[i][0]);
    }
  }
