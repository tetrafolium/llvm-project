...
const char *simple_types[] = {"bool",   "char",   "short",       "int",
                              "float",  "double", "long double", "wchar_t",
                              "void *", "char *"};

void gl(const char *c) { printf("%s\n", c); }

void g(const char *c) { printf("%s", c); }

void g(int i) { printf("%d", i); }
...
void gs(int s) {
  ...
  while (i_bases) {
    ...
    if (!base_present[s][base]) {
      ...
      // PARAM: 1/4 are public, 1/8 are privare, 1/8 are protected, the reset,
      // default
  ...
  for (int i = 0; i < n_funcs; ++i) {
    ...
    if (ret_type) {
      ...
      g("  virtual void fun");
    g(fn);
    g("(char *t) { mix(\"vfn this offset\", (char *)this - t); mix(\"vfn "
      "uuid\", ");
    g(++uuid);
    if (ret_type)
      gl("); return 0; }");
  ...
  if (polymorphic) {
    ...
    for (int i = 0; i < n_bases; ++i) {
      g("    if ((char *)dynamic_cast<s");
      g(bases[i]);
      gl("*>(this))");
      g("      mix(\"base dyn cast\", t - (char *)dynamic_cast<s");
      g(bases[i]);
      gl("*>(this));");
      g("    else mix(\"no dyncast\", ");
      g(++uuid);
      gl(");");
    }
  }
