...
struct __libcpp_unique_locale {
  __libcpp_unique_locale(const char* nm)
...
inline T& make(A0 a0, A1 a1, A2 a2) {
  static typename aligned_storage<sizeof(T)>::type buf;
...
public:
  ...
  bool has_facet(long id) const {
    return static_cast<size_t>(id) < facets_.size() &&
...
locale::__imp::__imp(const string& name, size_t refs)
    : facet(refs), facets_(N), name_(name) {
#ifndef _LIBCPP_NO_EXCEPTIONS
  try {
...
const locale& locale::__imp::make_classic() {
...
locale& locale::__imp::make_global() {
  // only one thread can get in here and it only gets in once
  static aligned_storage<sizeof(locale)>::type buf;
...
locale::locale() _NOEXCEPT : __locale_(__global().__locale_) {
  __locale_->__add_shared();
}

locale::locale(const locale& l) _NOEXCEPT : __locale_(l.__locale_) {
  __locale_->__add_shared();
}

locale::~locale() { __locale_->__release_shared(); }

const locale& locale::operator=(const locale& other) _NOEXCEPT {
...
locale::locale(const string& name) : __locale_(new __imp(name)) {
  __locale_->__add_shared();
}
...
const locale::facet* locale::use_facet(id& x) const {
...
void locale::facet::__on_zero_shared() _NOEXCEPT { delete this; }
...
public:
  __fake_bind(void (locale::id::*pmf)(), locale::id* id) : id_(id), pmf_(pmf) {}

  void operator()() const { (id_->*pmf_)(); }
};
...
void locale::id::__init() { __id_ = __libcpp_atomic_add(&__next_id, 1); }

// template <> class collate_byname<char>

collate_byname<char>::collate_byname(const char* n, size_t refs)
    : collate<char>(refs), __l(newlocale(LC_ALL_MASK, n, 0)) {
  if (__l == 0)
...
collate_byname<char>::collate_byname(const string& name, size_t refs)
    : collate<char>(refs), __l(newlocale(LC_ALL_MASK, name.c_str(), 0)) {
  if (__l == 0)
    __throw_runtime_error("collate_byname<char>::collate_byname"
                          " failed to construct for " +
                          name);
...
collate_byname<char>::~collate_byname() { freelocale(__l); }

int collate_byname<char>::do_compare(const char_type* __lo1,
                                     const char_type* __hi1,
...
collate_byname<char>::do_transform(const char_type* lo,
...
collate_byname<wchar_t>::collate_byname(const char* n, size_t refs)
    : collate<wchar_t>(refs), __l(newlocale(LC_ALL_MASK, n, 0)) {
  if (__l == 0)
...
collate_byname<wchar_t>::collate_byname(const string& name, size_t refs)
    : collate<wchar_t>(refs), __l(newlocale(LC_ALL_MASK, name.c_str(), 0)) {
  if (__l == 0)
    __throw_runtime_error("collate_byname<wchar_t>::collate_byname(size_t refs)"
                          " failed to construct for " +
                          name);
...
collate_byname<wchar_t>::~collate_byname() { freelocale(__l); }

int collate_byname<wchar_t>::do_compare(const char_type* __lo1,
                                        const char_type* __hi1,
                                        const char_type* __lo2,
                                        const char_type* __hi2) const {
  string_type lhs(__lo1, __hi1);
...
collate_byname<wchar_t>::do_transform(const char_type* lo,
                                      const char_type* hi) const {
  const string_type in(lo, hi);
...
const wchar_t* ctype<wchar_t>::do_is(const char_type* low,
  ...
  for (; low != high; ++low, ++vec)
    *vec = static_cast<mask>(isascii(*low) ? ctype<char>::classic_table()[*low]
                                           : 0);
  return low;
}

const wchar_t* ctype<wchar_t>::do_scan_is(mask m, const char_type* low,
...
const wchar_t* ctype<wchar_t>::do_scan_not(mask m, const char_type* low,
                                           const char_type* high) const {
  for (; low != high; ++low)
    if (!(isascii(*low) && (ctype<char>::classic_table()[*low] & m)))
      break;
...
const wchar_t* ctype<wchar_t>::do_toupper(char_type* low,
                                          const char_type* high) const {
  for (; low != high; ++low)
#ifdef _LIBCPP_HAS_DEFAULTRUNELOCALE
...
#else
    *low = (isascii(*low) && islower_l(*low, _LIBCPP_GET_C_LOCALE))
               ? (*low - L'a' + L'A')
               : *low;
#endif
  return low;
...
#else
  return (isascii(c) && isupper_l(c, _LIBCPP_GET_C_LOCALE)) ? c - L'A' + 'a'
                                                            : c;
#endif
}

const wchar_t* ctype<wchar_t>::do_tolower(char_type* low,
                                          const char_type* high) const {
  for (; low != high; ++low)
#ifdef _LIBCPP_HAS_DEFAULTRUNELOCALE
...
#else
    *low = (isascii(*low) && isupper_l(*low, _LIBCPP_GET_C_LOCALE))
               ? *low - L'A' + L'a'
               : *low;
#endif
  return low;
...
const char* ctype<wchar_t>::do_widen(const char* low, const char* high,
...
const wchar_t* ctype<wchar_t>::do_narrow(const char_type* low,
...
ctype<char>::ctype(const mask* tab, bool del, size_t refs)
    : locale::facet(refs), __tab_(tab), __del_(del) {
  if (__tab_ == 0)
...
char ctype<char>::do_toupper(char_type c) const {
  ...
  return isascii(c)
             ? static_cast<char>(
                   _DefaultRuneLocale.__mapupper[static_cast<ptrdiff_t>(c)])
             : c;
#elif defined(__NetBSD__)
  return static_cast<char>(
      __classic_upper_table()[static_cast<unsigned char>(c)]);
...
const char* ctype<char>::do_toupper(char_type* low,
  ...
  for (; low != high; ++low)
    ...
    *low =
        isascii(*low)
            ? static_cast<char>(
                  _DefaultRuneLocale.__mapupper[static_cast<ptrdiff_t>(*low)])
            : *low;
#elif defined(__NetBSD__)
    *low = static_cast<char>(
        __classic_upper_table()[static_cast<unsigned char>(*low)]);
#elif defined(__GLIBC__) || defined(__EMSCRIPTEN__)
    *low = isascii(*low)
               ? static_cast<char>(
                     __classic_upper_table()[static_cast<size_t>(*low)])
               : *low;
#else
    *low = (isascii(*low) && islower_l(*low, _LIBCPP_GET_C_LOCALE))
               ? *low - 'a' + 'A'
...
char ctype<char>::do_tolower(char_type c) const {
  ...
  return isascii(c)
             ? static_cast<char>(
                   _DefaultRuneLocale.__maplower[static_cast<ptrdiff_t>(c)])
             : c;
#elif defined(__NetBSD__)
  return static_cast<char>(
      __classic_lower_table()[static_cast<unsigned char>(c)]);
#elif defined(__GLIBC__) || defined(__EMSCRIPTEN__)
  return isascii(c) ? static_cast<char>(
                          __classic_lower_table()[static_cast<size_t>(c)])
                    : c;
#else
...
const char* ctype<char>::do_tolower(char_type* low,
                                    const char_type* high) const {
  for (; low != high; ++low)
#ifdef _LIBCPP_HAS_DEFAULTRUNELOCALE
    *low =
        isascii(*low)
            ? static_cast<char>(
                  _DefaultRuneLocale.__maplower[static_cast<ptrdiff_t>(*low)])
            : *low;
#elif defined(__NetBSD__)
    *low = static_cast<char>(
        __classic_lower_table()[static_cast<unsigned char>(*low)]);
#elif defined(__GLIBC__) || defined(__EMSCRIPTEN__)
    *low = isascii(*low)
               ? static_cast<char>(
                     __classic_lower_table()[static_cast<size_t>(*low)])
               : *low;
#else
    *low = (isascii(*low) && isupper_l(*low, _LIBCPP_GET_C_LOCALE))
               ? *low - 'A' + 'a'
...
const char* ctype<char>::do_widen(const char* low, const char* high,
                                  char_type* dest) const {
  for (; low != high; ++low, ++dest)
    *dest = *low;
  return low;
...
const char* ctype<char>::do_narrow(const char_type* low, const char_type* high,
...
const ctype<char>::mask* ctype<char>::classic_table() _NOEXCEPT {
...
const int* ctype<char>::__classic_upper_table() _NOEXCEPT {
  return _LIBCPP_GET_C_LOCALE->__ctype_toupper;
}
#elif defined(__NetBSD__)
const short* ctype<char>::__classic_lower_table() _NOEXCEPT {
  return _C_tolower_tab_ + 1;
}

const short* ctype<char>::__classic_upper_table() _NOEXCEPT {
...
ctype_byname<char>::ctype_byname(const char* name, size_t refs)
    : ctype<char>(0, false, refs), __l(newlocale(LC_ALL_MASK, name, 0)) {
  if (__l == 0)
...
const char* ctype_byname<char>::do_tolower(char_type* low,
                                           const char_type* high) const {
  for (; low != high; ++low)
...
ctype_byname<wchar_t>::ctype_byname(const char* name, size_t refs)
    : ctype<wchar_t>(refs), __l(newlocale(LC_ALL_MASK, name, 0)) {
  if (__l == 0)
...
ctype_byname<wchar_t>::ctype_byname(const string& name, size_t refs)
    : ctype<wchar_t>(refs), __l(newlocale(LC_ALL_MASK, name.c_str(), 0)) {
  if (__l == 0)
    __throw_runtime_error("ctype_byname<wchar_t>::ctype_byname"
                          " failed to construct for " +
                          name);
...
ctype_byname<wchar_t>::~ctype_byname() { freelocale(__l); }
...
bool ctype_byname<wchar_t>::do_is(mask m, char_type c) const {
  ...
  if ((m & space) == space)
    result |= (iswspace_l(ch, __l) != 0);
  if ((m & print) == print)
    result |= (iswprint_l(ch, __l) != 0);
  if ((m & cntrl) == cntrl)
    result |= (iswcntrl_l(ch, __l) != 0);
  if ((m & upper) == upper)
    result |= (iswupper_l(ch, __l) != 0);
  if ((m & lower) == lower)
    result |= (iswlower_l(ch, __l) != 0);
  if ((m & alpha) == alpha)
    result |= (iswalpha_l(ch, __l) != 0);
  if ((m & digit) == digit)
    result |= (iswdigit_l(ch, __l) != 0);
  if ((m & punct) == punct)
    result |= (iswpunct_l(ch, __l) != 0);
  if ((m & xdigit) == xdigit)
    result |= (iswxdigit_l(ch, __l) != 0);
  if ((m & blank) == blank)
    result |= (iswblank_l(ch, __l) != 0);
  return result;
#endif
}

const wchar_t* ctype_byname<wchar_t>::do_is(const char_type* low,
...
const wchar_t* ctype_byname<wchar_t>::do_scan_is(mask m, const char_type* low,
                                                 const char_type* high) const {
  for (; low != high; ++low) {
#ifdef _LIBCPP_WCTYPE_IS_MASK
...
#else
    wint_t ch = static_cast<wint_t>(*low);
    if ((m & space) == space && iswspace_l(ch, __l))
      break;
    if ((m & print) == print && iswprint_l(ch, __l))
      break;
    if ((m & cntrl) == cntrl && iswcntrl_l(ch, __l))
      break;
    if ((m & upper) == upper && iswupper_l(ch, __l))
      break;
    if ((m & lower) == lower && iswlower_l(ch, __l))
      break;
    if ((m & alpha) == alpha && iswalpha_l(ch, __l))
      break;
    if ((m & digit) == digit && iswdigit_l(ch, __l))
      break;
    if ((m & punct) == punct && iswpunct_l(ch, __l))
      break;
    if ((m & xdigit) == xdigit && iswxdigit_l(ch, __l))
      break;
    if ((m & blank) == blank && iswblank_l(ch, __l))
      break;
#endif
  }
  return low;
...
const wchar_t* ctype_byname<wchar_t>::do_scan_not(mask m, const char_type* low,
                                                  const char_type* high) const {
  for (; low != high; ++low) {
#ifdef _LIBCPP_WCTYPE_IS_MASK
    if (!iswctype_l(*low, m, __l))
...
wchar_t ctype_byname<wchar_t>::do_toupper(char_type c) const {
...
wchar_t ctype_byname<wchar_t>::do_tolower(char_type c) const {
  return towlower_l(c, __l);
}

const wchar_t* ctype_byname<wchar_t>::do_tolower(char_type* low,
                                                 const char_type* high) const {
  for (; low != high; ++low)
...
const char* ctype_byname<wchar_t>::do_widen(const char* low, const char* high,
                                            char_type* dest) const {
  for (; low != high; ++low, ++dest)
...
const wchar_t* ctype_byname<wchar_t>::do_narrow(const char_type* low,
                                                const char_type* high,
                                                char dfault, char* dest) const {
  for (; low != high; ++low, ++dest) {
...
codecvt<char, char, mbstate_t>::result codecvt<char, char, mbstate_t>::do_out(
    state_type&, const intern_type* frm, const intern_type*,
...
codecvt<char, char, mbstate_t>::result codecvt<char, char, mbstate_t>::do_in(
    state_type&, const extern_type* frm, const extern_type*,
...
codecvt<char, char, mbstate_t>::do_unshift(state_type&, extern_type* to,
                                           extern_type*,
                                           extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int codecvt<char, char, mbstate_t>::do_encoding() const _NOEXCEPT { return 1; }

bool codecvt<char, char, mbstate_t>::do_always_noconv() const _NOEXCEPT {
  return true;
}
...
int codecvt<char, char, mbstate_t>::do_max_length() const _NOEXCEPT {
  return 1;
}
...
codecvt<wchar_t, char, mbstate_t>::codecvt(size_t refs)
    : locale::facet(refs), __l(_LIBCPP_GET_C_LOCALE) {}

codecvt<wchar_t, char, mbstate_t>::codecvt(const char* nm, size_t refs)
    : locale::facet(refs), __l(newlocale(LC_ALL_MASK, nm, 0)) {
  if (__l == 0)
...
codecvt<wchar_t, char, mbstate_t>::do_out(state_type& st,
  ...
  to_nxt = to;
  for (frm_nxt = frm; frm != frm_end && to != to_end;
       frm = frm_nxt, to = to_nxt) {
    // save state in case it is needed to recover to_nxt on error
    mbstate_t save_state = st;
    size_t n =
        __libcpp_wcsnrtombs_l(to, &frm_nxt, static_cast<size_t>(fend - frm),
...
codecvt<wchar_t, char, mbstate_t>::result
codecvt<wchar_t, char, mbstate_t>::do_in(state_type& st, const extern_type* frm,
  ...
  for (frm_nxt = frm; frm != frm_end && to != to_end;
    ...
    mbstate_t save_state = st;
    size_t n =
        __libcpp_mbsnrtowcs_l(to, &frm_nxt, static_cast<size_t>(fend - frm),
                              static_cast<size_t>(to_end - to), &st, __l);
    if (n == size_t(-1)) {
      // need to recover to_nxt
...
codecvt<wchar_t, char, mbstate_t>::result
codecvt<wchar_t, char, mbstate_t>::do_unshift(state_type& st, extern_type* to,
                                              extern_type* to_end,
                                              extern_type*& to_nxt) const {
  to_nxt = to;
  extern_type tmp[MB_LEN_MAX];
...
int codecvt<wchar_t, char, mbstate_t>::do_encoding() const _NOEXCEPT {
  if (__libcpp_mbtowc_l(nullptr, nullptr, MB_LEN_MAX, __l) != 0)
    return -1;

  // stateless encoding
  if (__l == 0 || __libcpp_mb_cur_max_l(__l) ==
...
int codecvt<wchar_t, char, mbstate_t>::do_length(state_type& st,
                                                 const extern_type* frm,
                                                 const extern_type* frm_end,
                                                 size_t mx) const {
  int nbytes = 0;
  for (size_t nwchar_t = 0; nwchar_t < mx && frm != frm_end; ++nwchar_t) {
    size_t n =
        __libcpp_mbrlen_l(frm, static_cast<size_t>(frm_end - frm), &st, __l);
    switch (n) {
    case 0:
      ++nbytes;
      ++frm;
...
int codecvt<wchar_t, char, mbstate_t>::do_max_length() const _NOEXCEPT {
...
static codecvt_base::result utf16_to_utf8(const uint16_t* frm,
                                          const uint16_t* frm_end,
                                          const uint16_t*& frm_nxt, uint8_t* to,
                                          uint8_t* to_end, uint8_t*& to_nxt,
  ...
  for (; frm_nxt < frm_end; ++frm_nxt) {
    ...
    if (wc1 < 0x0080) {
      ...
      *to_nxt++ = static_cast<uint8_t>(0x80 | (wc1 & 0x003F));
      ...
      if (to_end - to_nxt < 4)
        return codecvt_base::partial;
      if (((((wc1 & 0x03C0UL) >> 6) + 1) << 16) + ((wc1 & 0x003FUL) << 10) +
              (wc2 & 0x03FF) >
          Maxcode)
        return codecvt_base::error;
      ++frm_nxt;
      ...
      *to_nxt++ = static_cast<uint8_t>(0x80 | ((z & 0x03) << 4) |
                                       ((wc1 & 0x003C) >> 2));
      *to_nxt++ = static_cast<uint8_t>(0x80 | ((wc1 & 0x0003) << 4) |
                                       ((wc2 & 0x03C0) >> 6));
...
static codecvt_base::result utf16_to_utf8(const uint32_t* frm,
                                          const uint32_t* frm_end,
                                          const uint32_t*& frm_nxt, uint8_t* to,
                                          uint8_t* to_end, uint8_t*& to_nxt,
                                          unsigned long Maxcode = 0x10FFFF,
                                          codecvt_mode mode = codecvt_mode(0)) {
  frm_nxt = frm;
...
static codecvt_base::result utf8_to_utf16(const uint8_t* frm,
                                          const uint8_t* frm_end,
                                          const uint8_t*& frm_nxt, uint16_t* to,
                                          uint16_t* to_end, uint16_t*& to_nxt,
                                          unsigned long Maxcode = 0x10FFFF,
                                          codecvt_mode mode = codecvt_mode(0)) {
  frm_nxt = frm;
  to_nxt = to;
  ...
  for (; frm_nxt < frm_end && to_nxt < to_end; ++to_nxt) {
    ...
    if (c1 < 0x80) {
      ...
      *to_nxt = t;
      ...
      if (to_end - to_nxt < 2)
        return codecvt_base::partial;
      if ((((c1 & 7UL) << 18) + ((c2 & 0x3FUL) << 12) + ((c3 & 0x3FUL) << 6) +
           (c4 & 0x3F)) > Maxcode)
        return codecvt_base::error;
      *to_nxt = static_cast<uint16_t>(
          ...
          ((c2 & 0x0F) << 2) | ((c3 & 0x30) >> 4));
      *++to_nxt =
          static_cast<uint16_t>(0xDC00 | ((c3 & 0x0F) << 6) | (c4 & 0x3F));
      frm_nxt += 4;
    } else {
      return codecvt_base::error;
    }
...
static codecvt_base::result utf8_to_utf16(const uint8_t* frm,
                                          const uint8_t* frm_end,
                                          const uint8_t*& frm_nxt, uint32_t* to,
                                          uint32_t* to_end, uint32_t*& to_nxt,
                                          unsigned long Maxcode = 0x10FFFF,
                                          codecvt_mode mode = codecvt_mode(0)) {
  frm_nxt = frm;
  to_nxt = to;
  ...
  for (; frm_nxt < frm_end && to_nxt < to_end; ++to_nxt) {
    ...
    if (c1 < 0x80) {
      ...
      *to_nxt = static_cast<uint32_t>(
          ...
          ((c2 & 0x0F) << 2) | ((c3 & 0x30) >> 4));
      *++to_nxt =
          static_cast<uint32_t>(0xDC00 | ((c3 & 0x0F) << 6) | (c4 & 0x3F));
      frm_nxt += 4;
    } else {
      return codecvt_base::error;
    }
...
static int utf8_to_utf16_length(const uint8_t* frm, const uint8_t* frm_end,
  ...
  for (size_t nchar16_t = 0; frm_nxt < frm_end && nchar16_t < mx; ++nchar16_t) {
    ...
    if (c1 < 0x80) {
      ...
      if ((frm_end - frm_nxt < 2) || (frm_nxt[1] & 0xC0) != 0x80)
        break;
      uint16_t t =
          static_cast<uint16_t>(((c1 & 0x1F) << 6) | (frm_nxt[1] & 0x3F));
      if (t > Maxcode)
        break;
      frm_nxt += 2;
    } else if (c1 < 0xF0) {
...
static codecvt_base::result ucs4_to_utf8(const uint32_t* frm,
                                         const uint32_t* frm_end,
                                         const uint32_t*& frm_nxt, uint8_t* to,
                                         uint8_t* to_end, uint8_t*& to_nxt,
                                         unsigned long Maxcode = 0x10FFFF,
...
static codecvt_base::result utf8_to_ucs4(const uint8_t* frm,
                                         const uint8_t* frm_end,
                                         const uint8_t*& frm_nxt, uint32_t* to,
                                         uint32_t* to_end, uint32_t*& to_nxt,
                                         unsigned long Maxcode = 0x10FFFF,
    ...
    if (c1 < 0x80) {
      ...
      *to_nxt = t;
      ...
      if ((c3 & 0xC0) != 0x80 || (c4 & 0xC0) != 0x80)
        return codecvt_base::error;
      uint32_t t =
          static_cast<uint32_t>(((c1 & 0x07) << 18) | ((c2 & 0x3F) << 12) |
                                ((c3 & 0x3F) << 6) | (c4 & 0x3F));
      if (t > Maxcode)
        return codecvt_base::error;
      *to_nxt = t;
...
static int utf8_to_ucs4_length(const uint8_t* frm, const uint8_t* frm_end,
                               size_t mx, unsigned long Maxcode = 0x10FFFF,
                               codecvt_mode mode = codecvt_mode(0)) {
  const uint8_t* frm_nxt = frm;
  if (mode & consume_header) {
  ...
  for (size_t nchar32_t = 0; frm_nxt < frm_end && nchar32_t < mx; ++nchar32_t) {
    ...
    if (c1 < 0x80) {
      ...
      if ((c3 & 0xC0) != 0x80 || (c4 & 0xC0) != 0x80)
        break;
      if ((((c1 & 0x07u) << 18) | ((c2 & 0x3Fu) << 12) | ((c3 & 0x3Fu) << 6) |
           (c4 & 0x3Fu)) > Maxcode)
        break;
      frm_nxt += 4;
    } else {
...
static codecvt_base::result ucs2_to_utf8(const uint16_t* frm,
                                         const uint16_t* frm_end,
                                         const uint16_t*& frm_nxt, uint8_t* to,
                                         uint8_t* to_end, uint8_t*& to_nxt,
                                         unsigned long Maxcode = 0x10FFFF,
...
static codecvt_base::result utf8_to_ucs2(const uint8_t* frm,
                                         const uint8_t* frm_end,
                                         const uint8_t*& frm_nxt, uint16_t* to,
                                         uint16_t* to_end, uint16_t*& to_nxt,
                                         unsigned long Maxcode = 0x10FFFF,
...
static int utf8_to_ucs2_length(const uint8_t* frm, const uint8_t* frm_end,
                               size_t mx, unsigned long Maxcode = 0x10FFFF,
                               codecvt_mode mode = codecvt_mode(0)) {
  const uint8_t* frm_nxt = frm;
  if (mode & consume_header) {
...
ucs4_to_utf16be(const uint32_t* frm, const uint32_t* frm_end,
    ...
    if (wc < 0x010000) {
      ...
      *to_nxt++ = static_cast<uint8_t>(wc);
      ...
      if (to_end - to_nxt < 4)
        return codecvt_base::partial;
      uint16_t t =
          static_cast<uint16_t>(0xD800 | ((((wc & 0x1F0000) >> 16) - 1) << 6) |
                                ((wc & 0x00FC00) >> 10));
...
static codecvt_base::result
utf16be_to_ucs4(const uint8_t* frm, const uint8_t* frm_end,
  ...
  for (; frm_nxt < frm_end - 1 && to_nxt < to_end; ++to_nxt) {
    ...
    if ((c1 & 0xFC00) != 0xD800) {
      ...
      *to_nxt = static_cast<uint32_t>(c1);
      ...
      if ((c2 & 0xFC00) != 0xDC00)
        return codecvt_base::error;
      uint32_t t = static_cast<uint32_t>(((((c1 & 0x03C0) >> 6) + 1) << 16) |
                                         ((c1 & 0x003F) << 10) | (c2 & 0x03FF));
      if (t > Maxcode)
        return codecvt_base::error;
      *to_nxt = t;
      frm_nxt += 4;
...
static int utf16be_to_ucs4_length(const uint8_t* frm, const uint8_t* frm_end,
                                  size_t mx, unsigned long Maxcode = 0x10FFFF,
                                  codecvt_mode mode = codecvt_mode(0)) {
  const uint8_t* frm_nxt = frm;
  if (mode & consume_header) {
  ...
  for (size_t nchar32_t = 0; frm_nxt < frm_end - 1 && nchar32_t < mx;
       ++nchar32_t) {
    uint16_t c1 = static_cast<uint16_t>(frm_nxt[0] << 8 | frm_nxt[1]);
    if ((c1 & 0xFC00) == 0xDC00)
      break;
...
static codecvt_base::result
ucs4_to_utf16le(const uint32_t* frm, const uint32_t* frm_end,
                const uint32_t*& frm_nxt, uint8_t* to, uint8_t* to_end,
                uint8_t*& to_nxt, unsigned long Maxcode = 0x10FFFF,
                codecvt_mode mode = codecvt_mode(0)) {
  frm_nxt = frm;
...
static codecvt_base::result
utf16le_to_ucs4(const uint8_t* frm, const uint8_t* frm_end,
                const uint8_t*& frm_nxt, uint32_t* to, uint32_t* to_end,
                uint32_t*& to_nxt, unsigned long Maxcode = 0x10FFFF,
                codecvt_mode mode = codecvt_mode(0)) {
  frm_nxt = frm;
...
static int utf16le_to_ucs4_length(const uint8_t* frm, const uint8_t* frm_end,
                                  size_t mx, unsigned long Maxcode = 0x10FFFF,
                                  codecvt_mode mode = codecvt_mode(0)) {
  const uint8_t* frm_nxt = frm;
  if (mode & consume_header) {
...
static codecvt_base::result
ucs2_to_utf16be(const uint16_t* frm, const uint16_t* frm_end,
...
static codecvt_base::result
utf16be_to_ucs2(const uint8_t* frm, const uint8_t* frm_end,
...
static int utf16be_to_ucs2_length(const uint8_t* frm, const uint8_t* frm_end,
                                  size_t mx, unsigned long Maxcode = 0x10FFFF,
                                  codecvt_mode mode = codecvt_mode(0)) {
  const uint8_t* frm_nxt = frm;
  if (mode & consume_header) {
  ...
  for (size_t nchar16_t = 0; frm_nxt < frm_end - 1 && nchar16_t < mx;
       ++nchar16_t) {
    uint16_t c1 = static_cast<uint16_t>(frm_nxt[0] << 8 | frm_nxt[1]);
    if ((c1 & 0xF800) == 0xD800 || c1 > Maxcode)
      break;
...
static codecvt_base::result
ucs2_to_utf16le(const uint16_t* frm, const uint16_t* frm_end,
                const uint16_t*& frm_nxt, uint8_t* to, uint8_t* to_end,
                uint8_t*& to_nxt, unsigned long Maxcode = 0x10FFFF,
                codecvt_mode mode = codecvt_mode(0)) {
  frm_nxt = frm;
...
static codecvt_base::result
utf16le_to_ucs2(const uint8_t* frm, const uint8_t* frm_end,
                const uint8_t*& frm_nxt, uint16_t* to, uint16_t* to_end,
                uint16_t*& to_nxt, unsigned long Maxcode = 0x10FFFF,
                codecvt_mode mode = codecvt_mode(0)) {
  frm_nxt = frm;
...
static int utf16le_to_ucs2_length(const uint8_t* frm, const uint8_t* frm_end,
                                  size_t mx, unsigned long Maxcode = 0x10FFFF,
                                  codecvt_mode mode = codecvt_mode(0)) {
  const uint8_t* frm_nxt = frm;
  frm_nxt = frm;
...
codecvt<char16_t, char, mbstate_t>::do_out(state_type&, const intern_type* frm,
                                           const intern_type* frm_end,
                                           const intern_type*& frm_nxt,
                                           extern_type* to, extern_type* to_end,
                                           extern_type*& to_nxt) const {
...
codecvt<char16_t, char, mbstate_t>::result
codecvt<char16_t, char, mbstate_t>::do_in(state_type&, const extern_type* frm,
                                          const extern_type* frm_end,
                                          const extern_type*& frm_nxt,
                                          intern_type* to, intern_type* to_end,
                                          intern_type*& to_nxt) const {
...
codecvt<char16_t, char, mbstate_t>::result
codecvt<char16_t, char, mbstate_t>::do_unshift(state_type&, extern_type* to,
                                               extern_type*,
                                               extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int codecvt<char16_t, char, mbstate_t>::do_length(state_type&,
                                                  const extern_type* frm,
                                                  const extern_type* frm_end,
                                                  size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
codecvt<char16_t, char8_t, mbstate_t>::do_out(
    state_type&, const intern_type* frm, const intern_type* frm_end,
...
codecvt<char16_t, char8_t, mbstate_t>::result
codecvt<char16_t, char8_t, mbstate_t>::do_in(
    state_type&, const extern_type* frm, const extern_type* frm_end,
...
codecvt<char16_t, char8_t, mbstate_t>::result
codecvt<char16_t, char8_t, mbstate_t>::do_unshift(state_type&, extern_type* to,
                                                  extern_type*,
                                                  extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int codecvt<char16_t, char8_t, mbstate_t>::do_encoding() const _NOEXCEPT {
  return 0;
}

bool codecvt<char16_t, char8_t, mbstate_t>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int codecvt<char16_t, char8_t, mbstate_t>::do_length(state_type&,
                                                     const extern_type* frm,
                                                     const extern_type* frm_end,
                                                     size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
int codecvt<char16_t, char8_t, mbstate_t>::do_max_length() const _NOEXCEPT {
  return 4;
}
...
codecvt<char32_t, char, mbstate_t>::do_out(state_type&, const intern_type* frm,
                                           const intern_type* frm_end,
                                           const intern_type*& frm_nxt,
                                           extern_type* to, extern_type* to_end,
                                           extern_type*& to_nxt) const {
...
codecvt<char32_t, char, mbstate_t>::result
codecvt<char32_t, char, mbstate_t>::do_in(state_type&, const extern_type* frm,
                                          const extern_type* frm_end,
                                          const extern_type*& frm_nxt,
                                          intern_type* to, intern_type* to_end,
                                          intern_type*& to_nxt) const {
...
codecvt<char32_t, char, mbstate_t>::result
codecvt<char32_t, char, mbstate_t>::do_unshift(state_type&, extern_type* to,
                                               extern_type*,
                                               extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int codecvt<char32_t, char, mbstate_t>::do_length(state_type&,
                                                  const extern_type* frm,
                                                  const extern_type* frm_end,
                                                  size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
codecvt<char32_t, char8_t, mbstate_t>::do_out(
    state_type&, const intern_type* frm, const intern_type* frm_end,
    const intern_type*& frm_nxt, extern_type* to, extern_type* to_end,
    extern_type*& to_nxt) const {
  const uint32_t* _frm = reinterpret_cast<const uint32_t*>(frm);
...
codecvt<char32_t, char8_t, mbstate_t>::result
codecvt<char32_t, char8_t, mbstate_t>::do_in(
    state_type&, const extern_type* frm, const extern_type* frm_end,
    const extern_type*& frm_nxt, intern_type* to, intern_type* to_end,
    intern_type*& to_nxt) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
codecvt<char32_t, char8_t, mbstate_t>::result
codecvt<char32_t, char8_t, mbstate_t>::do_unshift(state_type&, extern_type* to,
                                                  extern_type*,
                                                  extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int codecvt<char32_t, char8_t, mbstate_t>::do_encoding() const _NOEXCEPT {
  return 0;
}

bool codecvt<char32_t, char8_t, mbstate_t>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int codecvt<char32_t, char8_t, mbstate_t>::do_length(state_type&,
                                                     const extern_type* frm,
                                                     const extern_type* frm_end,
                                                     size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
int codecvt<char32_t, char8_t, mbstate_t>::do_max_length() const _NOEXCEPT {
  return 4;
}
...
__codecvt_utf8<wchar_t>::do_unshift(state_type&, extern_type* to, extern_type*,
                                    extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
}

int __codecvt_utf8<wchar_t>::do_encoding() const _NOEXCEPT { return 0; }

bool __codecvt_utf8<wchar_t>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int __codecvt_utf8<wchar_t>::do_length(state_type&, const extern_type* frm,
                                       const extern_type* frm_end,
                                       size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
  const uint8_t* _frm_end = reinterpret_cast<const uint8_t*>(frm_end);
...
int __codecvt_utf8<wchar_t>::do_max_length() const _NOEXCEPT {
...
__codecvt_utf8<char16_t>::do_unshift(state_type&, extern_type* to, extern_type*,
                                     extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
}

int __codecvt_utf8<char16_t>::do_encoding() const _NOEXCEPT { return 0; }

bool __codecvt_utf8<char16_t>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int __codecvt_utf8<char16_t>::do_length(state_type&, const extern_type* frm,
                                        const extern_type* frm_end,
                                        size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
  const uint8_t* _frm_end = reinterpret_cast<const uint8_t*>(frm_end);
...
int __codecvt_utf8<char16_t>::do_max_length() const _NOEXCEPT {
  if (_Mode_ & consume_header)
    return 6;
  return 3;
}
...
__codecvt_utf8<char32_t>::do_unshift(state_type&, extern_type* to, extern_type*,
                                     extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
}

int __codecvt_utf8<char32_t>::do_encoding() const _NOEXCEPT { return 0; }

bool __codecvt_utf8<char32_t>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int __codecvt_utf8<char32_t>::do_length(state_type&, const extern_type* frm,
                                        const extern_type* frm_end,
                                        size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
  const uint8_t* _frm_end = reinterpret_cast<const uint8_t*>(frm_end);
...
int __codecvt_utf8<char32_t>::do_max_length() const _NOEXCEPT {
  if (_Mode_ & consume_header)
    return 7;
  return 4;
}
...
__codecvt_utf16<wchar_t, false>::do_unshift(state_type&, extern_type* to,
                                            extern_type*,
                                            extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int __codecvt_utf16<wchar_t, false>::do_encoding() const _NOEXCEPT { return 0; }

bool __codecvt_utf16<wchar_t, false>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int __codecvt_utf16<wchar_t, false>::do_length(state_type&,
                                               const extern_type* frm,
                                               const extern_type* frm_end,
                                               size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
int __codecvt_utf16<wchar_t, false>::do_max_length() const _NOEXCEPT {
  if (_Mode_ & consume_header)
    return 6;
  return 4;
}
...
__codecvt_utf16<wchar_t, true>::do_unshift(state_type&, extern_type* to,
                                           extern_type*,
                                           extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int __codecvt_utf16<wchar_t, true>::do_encoding() const _NOEXCEPT { return 0; }

bool __codecvt_utf16<wchar_t, true>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int __codecvt_utf16<wchar_t, true>::do_length(state_type&,
                                              const extern_type* frm,
                                              const extern_type* frm_end,
                                              size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
int __codecvt_utf16<wchar_t, true>::do_max_length() const _NOEXCEPT {
  if (_Mode_ & consume_header)
    return 6;
  return 4;
}
...
__codecvt_utf16<char16_t, false>::do_out(state_type&, const intern_type* frm,
                                         const intern_type* frm_end,
                                         const intern_type*& frm_nxt,
                                         extern_type* to, extern_type* to_end,
                                         extern_type*& to_nxt) const {
...
__codecvt_utf16<char16_t, false>::result
__codecvt_utf16<char16_t, false>::do_in(state_type&, const extern_type* frm,
                                        const extern_type* frm_end,
                                        const extern_type*& frm_nxt,
                                        intern_type* to, intern_type* to_end,
                                        intern_type*& to_nxt) const {
...
__codecvt_utf16<char16_t, false>::result
__codecvt_utf16<char16_t, false>::do_unshift(state_type&, extern_type* to,
                                             extern_type*,
                                             extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int __codecvt_utf16<char16_t, false>::do_encoding() const _NOEXCEPT {
  return 0;
}

bool __codecvt_utf16<char16_t, false>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int __codecvt_utf16<char16_t, false>::do_length(state_type&,
                                                const extern_type* frm,
                                                const extern_type* frm_end,
                                                size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
int __codecvt_utf16<char16_t, false>::do_max_length() const _NOEXCEPT {
  if (_Mode_ & consume_header)
    return 4;
  return 2;
}
...
__codecvt_utf16<char16_t, true>::do_unshift(state_type&, extern_type* to,
                                            extern_type*,
                                            extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int __codecvt_utf16<char16_t, true>::do_encoding() const _NOEXCEPT { return 0; }

bool __codecvt_utf16<char16_t, true>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int __codecvt_utf16<char16_t, true>::do_length(state_type&,
                                               const extern_type* frm,
                                               const extern_type* frm_end,
                                               size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
int __codecvt_utf16<char16_t, true>::do_max_length() const _NOEXCEPT {
  if (_Mode_ & consume_header)
    return 4;
  return 2;
}
...
__codecvt_utf16<char32_t, false>::do_out(state_type&, const intern_type* frm,
                                         const intern_type* frm_end,
                                         const intern_type*& frm_nxt,
                                         extern_type* to, extern_type* to_end,
                                         extern_type*& to_nxt) const {
...
__codecvt_utf16<char32_t, false>::result
__codecvt_utf16<char32_t, false>::do_in(state_type&, const extern_type* frm,
                                        const extern_type* frm_end,
                                        const extern_type*& frm_nxt,
                                        intern_type* to, intern_type* to_end,
                                        intern_type*& to_nxt) const {
...
__codecvt_utf16<char32_t, false>::result
__codecvt_utf16<char32_t, false>::do_unshift(state_type&, extern_type* to,
                                             extern_type*,
                                             extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int __codecvt_utf16<char32_t, false>::do_encoding() const _NOEXCEPT {
  return 0;
}

bool __codecvt_utf16<char32_t, false>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int __codecvt_utf16<char32_t, false>::do_length(state_type&,
                                                const extern_type* frm,
                                                const extern_type* frm_end,
                                                size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
int __codecvt_utf16<char32_t, false>::do_max_length() const _NOEXCEPT {
  if (_Mode_ & consume_header)
    return 6;
  return 4;
}
...
__codecvt_utf16<char32_t, true>::do_unshift(state_type&, extern_type* to,
                                            extern_type*,
                                            extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int __codecvt_utf16<char32_t, true>::do_encoding() const _NOEXCEPT { return 0; }

bool __codecvt_utf16<char32_t, true>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int __codecvt_utf16<char32_t, true>::do_length(state_type&,
                                               const extern_type* frm,
                                               const extern_type* frm_end,
                                               size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
int __codecvt_utf16<char32_t, true>::do_max_length() const _NOEXCEPT {
  if (_Mode_ & consume_header)
    return 6;
  return 4;
}
...
__codecvt_utf8_utf16<wchar_t>::result __codecvt_utf8_utf16<wchar_t>::do_unshift(
    state_type&, extern_type* to, extern_type*, extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
}

int __codecvt_utf8_utf16<wchar_t>::do_encoding() const _NOEXCEPT { return 0; }

bool __codecvt_utf8_utf16<wchar_t>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int __codecvt_utf8_utf16<wchar_t>::do_length(state_type&,
                                             const extern_type* frm,
                                             const extern_type* frm_end,
                                             size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
int __codecvt_utf8_utf16<wchar_t>::do_max_length() const _NOEXCEPT {
  if (_Mode_ & consume_header)
    return 7;
  return 4;
}
...
__codecvt_utf8_utf16<char16_t>::do_unshift(state_type&, extern_type* to,
                                           extern_type*,
                                           extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int __codecvt_utf8_utf16<char16_t>::do_encoding() const _NOEXCEPT { return 0; }

bool __codecvt_utf8_utf16<char16_t>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int __codecvt_utf8_utf16<char16_t>::do_length(state_type&,
                                              const extern_type* frm,
                                              const extern_type* frm_end,
                                              size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
__codecvt_utf8_utf16<char32_t>::do_unshift(state_type&, extern_type* to,
                                           extern_type*,
                                           extern_type*& to_nxt) const {
  to_nxt = to;
  return noconv;
...
int __codecvt_utf8_utf16<char32_t>::do_encoding() const _NOEXCEPT { return 0; }

bool __codecvt_utf8_utf16<char32_t>::do_always_noconv() const _NOEXCEPT {
  return false;
}

int __codecvt_utf8_utf16<char32_t>::do_length(state_type&,
                                              const extern_type* frm,
                                              const extern_type* frm_end,
                                              size_t mx) const {
  const uint8_t* _frm = reinterpret_cast<const uint8_t*>(frm);
...
static bool checked_string_to_wchar_convert(wchar_t& dest, const char* ptr,
                                            locale_t loc) {
  if (*ptr == '\0')
    return false;
  mbstate_t mb = {};
...
static bool checked_string_to_char_convert(char& dest, const char* ptr,
                                           locale_t __loc) {
  if (*ptr == '\0')
    return false;
  if (!ptr[1]) {
...
numpunct<char>::numpunct(size_t refs)
    : locale::facet(refs), __decimal_point_('.'), __thousands_sep_(',') {}

numpunct<wchar_t>::numpunct(size_t refs)
    : locale::facet(refs), __decimal_point_(L'.'), __thousands_sep_(L',') {}
...
void __num_put_base::__format_int(char* __fmtp, const char* __len, bool __signd,
...
bool __num_put_base::__format_float(char* __fmtp, const char* __len,
                                    ios_base::fmtflags __flags) {
...
char* __num_put_base::__identify_padding(char* __nb, char* __ne,
...
string __time_get_storage<char>::__analyze(char fmt, const ctype<char>& ct) {
  ...
  while (bb != be) {
    ...
    ptrdiff_t i = __scan_keyword(w, be, this->__weeks_, this->__weeks_ + 14, ct,
                                 err, false) -
                  this->__weeks_;
    if (i < 14) {
      result.push_back('%');
    ...
    w = bb;
    i = __scan_keyword(w, be, this->__months_, this->__months_ + 24, ct, err,
                       false) -
        this->__months_;
    if (i < 24) {
      result.push_back('%');
    ...
    if (this->__am_pm_[0].size() + this->__am_pm_[1].size() > 0) {
      w = bb;
      i = __scan_keyword(w, be, this->__am_pm_, this->__am_pm_ + 2, ct, err,
                         false) -
          this->__am_pm_;
      if (i < 2) {
        result.push_back('%');
...
template <>
wstring __time_get_storage<wchar_t>::__analyze(char fmt,
  ...
  while (wbb != wbe) {
    ...
    w = wbb;
    i = __scan_keyword(w, wbe, this->__months_, this->__months_ + 24, ct, err,
                       false) -
        this->__months_;
    if (i < 24) {
      result.push_back(L'%');
    ...
    if (this->__am_pm_[0].size() + this->__am_pm_[1].size() > 0) {
      w = wbb;
      i = __scan_keyword(w, wbe, this->__am_pm_, this->__am_pm_ + 2, ct, err,
                         false) -
          this->__am_pm_;
      if (i < 2) {
        result.push_back(L'%');
...
void __time_put::__do_put(char* __nb, char*& __ne, const tm* __tm, char __fmt,
                          char __mod) const {
...
void __time_put::__do_put(wchar_t* __wb, wchar_t*& __we, const tm* __tm,
  ...
  size_t j =
      __libcpp_mbsrtowcs_l(__wb, &__nb, countof(__wb, __we), &mb, __loc_);
  if (j == size_t(-1))
    __throw_runtime_error("locale not supported");
  __we = __wb + j;
}
...
template <class charT>
static void __init_pat(money_base::pattern& pat,
                       basic_string<charT>& __curr_symbol_, bool intl,
                       char cs_precedes, char sep_by_space, char sign_posn,
                       charT space_char) {
...
void moneypunct_byname<char, false>::init(const char* nm) {
  ...
  lconv* lc = __libcpp_localeconv_l(loc.get());
  if (!checked_string_to_char_convert(__decimal_point_, lc->mon_decimal_point,
                                      loc.get()))
    __decimal_point_ = base::do_decimal_point();
  if (!checked_string_to_char_convert(__thousands_sep_, lc->mon_thousands_sep,
                                      loc.get()))
  ...
  __init_pat(__pos_format_, __dummy_curr_symbol, false, lc->p_cs_precedes,
             lc->p_sep_by_space, lc->p_sign_posn, ' ');
  __init_pat(__neg_format_, __curr_symbol_, false, lc->n_cs_precedes,
             lc->n_sep_by_space, lc->n_sign_posn, ' ');
}
...
void moneypunct_byname<char, true>::init(const char* nm) {
  ...
  string_type __dummy_curr_symbol = __curr_symbol_;
#if defined(_LIBCPP_MSVCRT) || defined(__MINGW32__)
  __init_pat(__pos_format_, __dummy_curr_symbol, true, lc->p_cs_precedes,
             lc->p_sep_by_space, lc->p_sign_posn, ' ');
  __init_pat(__neg_format_, __curr_symbol_, true, lc->n_cs_precedes,
             lc->n_sep_by_space, lc->n_sign_posn, ' ');
#else  // _LIBCPP_MSVCRT
  __init_pat(__pos_format_, __dummy_curr_symbol, true, lc->int_p_cs_precedes,
             lc->int_p_sep_by_space, lc->int_p_sign_posn, ' ');
  __init_pat(__neg_format_, __curr_symbol_, true, lc->int_n_cs_precedes,
             lc->int_n_sep_by_space, lc->int_n_sign_posn, ' ');
#endif // !_LIBCPP_MSVCRT
}
...
void moneypunct_byname<wchar_t, false>::init(const char* nm) {
  ...
  if (!loc)
    __throw_runtime_error("moneypunct_byname"
                          ...
                          string(nm));
  lconv* lc = __libcpp_localeconv_l(loc.get());
  if (!checked_string_to_wchar_convert(__decimal_point_, lc->mon_decimal_point,
                                       loc.get()))
    __decimal_point_ = base::do_decimal_point();
  ...
  string_type __dummy_curr_symbol = __curr_symbol_;
  __init_pat(__pos_format_, __dummy_curr_symbol, false, lc->p_cs_precedes,
             lc->p_sep_by_space, lc->p_sign_posn, L' ');
  __init_pat(__neg_format_, __curr_symbol_, false, lc->n_cs_precedes,
             lc->n_sep_by_space, lc->n_sign_posn, L' ');
}
...
#else  // _LIBCPP_MSVCRT
  __init_pat(__pos_format_, __dummy_curr_symbol, true, lc->int_p_cs_precedes,
             lc->int_p_sep_by_space, lc->int_p_sign_posn, L' ');
  __init_pat(__neg_format_, __curr_symbol_, true, lc->int_n_cs_precedes,
             lc->int_n_sep_by_space, lc->int_n_sign_posn, L' ');
#endif // !_LIBCPP_MSVCRT
}
...
template class _LIBCPP_CLASS_TEMPLATE_INSTANTIATION_VIS
    _LIBCPP_DEPRECATED_IN_CXX20 codecvt_byname<char16_t, char, mbstate_t>;
template class _LIBCPP_CLASS_TEMPLATE_INSTANTIATION_VIS
    _LIBCPP_DEPRECATED_IN_CXX20 codecvt_byname<char32_t, char, mbstate_t>;
#ifndef _LIBCPP_NO_HAS_CHAR8_T
template class _LIBCPP_CLASS_TEMPLATE_INSTANTIATION_VIS
