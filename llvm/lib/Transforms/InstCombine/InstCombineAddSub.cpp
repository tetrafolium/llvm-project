...
private:
    bool insaneIntVal(int V) {
        return V > 4 || V < -4;
    }

    APFloat *getFpValPtr() {
        return reinterpret_cast<APFloat *>(&FpValBuf);
    }
...
public:
    ...
    Value *getSymVal() const {
        return Val;
    }
    const FAddendCoef &getCoef() const {
        return Coeff;
    }
...
private:
    void Scale(const FAddendCoef& ScaleAmt) {
        Coeff *= ScaleAmt;
    }
