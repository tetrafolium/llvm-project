...
__attribute__((noreturn)) static void demangling_terminate_handler() {
      ...
      if (__isOurExceptionClass(unwind_exception)) {
        void* thrown_object =
            __getExceptionClass(unwind_exception) == kOurDependentExceptionClass
                ? ((__cxa_dependent_exception*)exception_header)
                      ->primaryException
                : exception_header + 1;
        ...
        const char* name =
            __cxa_demangle(thrown_type->name(), buf, &len, &status);
        if (status != 0)
        ...
        if (catch_type->can_catch(thrown_type, thrown_object)) {
          ...
          const std::exception* e =
              static_cast<const std::exception*>(thrown_object);
          abort_message("terminating with %s exception of type %s: %s", cause,
                        name, e->what());
        } else
          // Else just note that we're terminating with an exception
          abort_message("terminating with %s exception of type %s", cause,
                        name);
      } else
...
__attribute__((noreturn)) static void demangling_unexpected_handler() {
  cause = "unexpected";
  std::terminate();
}
...
//
_LIBCXXABI_DATA_VIS
_LIBCPP_SAFE_STATIC std::terminate_handler __cxa_terminate_handler =
    default_terminate_handler;

_LIBCXXABI_DATA_VIS
_LIBCPP_SAFE_STATIC std::unexpected_handler __cxa_unexpected_handler =
...
unexpected_handler set_unexpected(unexpected_handler func) _NOEXCEPT {
  ...
  return __libcpp_atomic_exchange(&__cxa_unexpected_handler, func, _AO_Acq_Rel);
}

terminate_handler set_terminate(terminate_handler func) _NOEXCEPT {
  if (func == 0)
    func = default_terminate_handler;
  return __libcpp_atomic_exchange(&__cxa_terminate_handler, func, _AO_Acq_Rel);
}
