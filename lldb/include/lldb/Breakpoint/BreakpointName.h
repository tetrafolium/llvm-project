...
public:
  ...
  public:
    ...
    bool AnySet() { return m_set_mask.AnySet(permissions_mask[allPerms]); }
  ...
  BreakpointName(ConstString name, BreakpointOptions &options,
                 const Permissions &permissions = Permissions(),
                 const char *help = nullptr)
      : m_name(name), m_options(options), m_permissions(permissions) {
    SetHelp(help);
  };
  ...
  void SetOptions(const BreakpointOptions &options) { m_options = options; }
  ...
  const char *GetHelp() { return m_help.c_str(); }
