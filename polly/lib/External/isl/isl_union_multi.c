 ...
 */
static __isl_null S(UNION, group) *
    FN(UNION, group_free)(__isl_take S(UNION, group) * group) {
  isl_ctx *ctx;
  ...
  isl_hash_table_foreach(ctx, &group->part_table, &FN(UNION, free_group_entry),
                         NULL);
 ...
 */
static __isl_give S(UNION, group) *
    FN(UNION, group_alloc)(__isl_take isl_space *domain_space, int size) {
  isl_ctx *ctx;
  S(UNION, group) * group;
 ...
 */
static __isl_give S(UNION, group) *
    FN(UNION, group_cow)(__isl_take S(UNION, group) * group) {
  return group;
}
 ...
 */
static isl_stat
FN(UNION, group_call_on_copy)(__isl_keep S(UNION, group) * group, void *user) {
  isl_ctx *ctx;
...
FN(UNION, find_part_entry)(__isl_keep UNION *u, __isl_keep isl_space *space,
                           int reserve) {
  isl_ctx *ctx;
  uint32_t hash;
  ...
  group_entry = isl_hash_table_find(ctx, &u->table, hash,
                                    &FN(UNION, group_has_same_domain_space),
                                    space, reserve);
...
FN(UNION, remove_part_entry)(__isl_take UNION *u,
  ...
  hash = isl_space_get_domain_hash(space);
  group_entry = isl_hash_table_find(
      ctx, &u->table, hash, &FN(UNION, group_has_same_domain_space), space, 0);
  if (!group_entry)
    return FN(UNION, free)(u);
 ...
 */
static isl_stat FN(UNION, check_disjoint_domain_entry)(void **entry,
                                                       void *user) {
  PART *part = user;
  PART *other = *entry;
  isl_bool equal;
  ...
  if (!disjoint)
    isl_die(FN(PART, get_ctx)(part), isl_error_invalid,
            "overlapping domain with other part", return isl_stat_error);
  return isl_stat_ok;
}
 ...
 */
static isl_stat
FN(UNION, group_call_inplace)(__isl_keep S(UNION, group) * group, void *user) {
  isl_ctx *ctx;
  ...
  ctx = isl_space_get_ctx(group->domain_space);
  return isl_hash_table_foreach(ctx, &group->part_table, data->fn, data->user);
}

/* Call "fn" on each part entry of "u".
 */
static isl_stat FN(UNION,
                   foreach_inplace)(__isl_keep UNION *u,
                                    isl_stat (*fn)(void **part, void *user),
                                    void *user) {
 ...
 */
static isl_stat
FN(UNION, group_single_space)(__isl_keep S(UNION, group) * group, void *user) {
...
isl_bool FN(FN(UNION, isa), BASE)(__isl_take UNION *u) {
  ...
  if (FN(UNION, foreach_group)(u, &FN(UNION, group_single_space), &single) < 0)
    return isl_bool_error;
  return single;
}
...
static isl_stat FN(UNION, extract_part)(void **entry, void *user) {
  ...
  if (*part_p)
    isl_die(FN(PART, get_ctx)(part), isl_error_internal, "more than one part",
            return isl_stat_error);
...
__isl_give PART *FN(FN(UNION, as), BASE)(__isl_take UNION *u) {
  ...
  if (!has_single_space)
    isl_die(FN(UNION, get_ctx)(u), isl_error_invalid,
            "expecting elements in exactly one space", goto error);
