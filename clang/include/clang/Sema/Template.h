...
public:
  ...
  bool isRewrite() const { return Kind == TemplateSubstitutionKind::Rewrite; }
  ...
  unsigned getNumRetainedOuterLevels() const { return NumRetainedOuterLevels; }
  ...
  void setArgument(unsigned Depth, unsigned Index, TemplateArgument Arg) {
    ...
    const_cast<TemplateArgument &>(
        TemplateArgumentLists[getNumLevels() - Depth - 1][Index]) = Arg;
  }
  ...
  void addOuterTemplateArguments(const TemplateArgumentList *TemplateArgs) {
    addOuterTemplateArguments(
        ArgList(TemplateArgs->data(), TemplateArgs->size()));
  }
  ...
  void addOuterRetainedLevel() { ++NumRetainedOuterLevels; }
  void addOuterRetainedLevels(unsigned Num) { NumRetainedOuterLevels += Num; }

  /// Retrieve the innermost template argument list.
  const ArgList &getInnermost() const { return TemplateArgumentLists.front(); }
};
...
public:
  ...
  DeducedTemplateArgument(ASTContext &Ctx, const llvm::APSInt &Value,
                          QualType ValueType, bool DeducedFromArrayBound)
...
class TemplateDeclInstantiator
  ...
  SmallVector<
      std::pair<ClassTemplateDecl *, ClassTemplatePartialSpecializationDecl *>,
      4>
...
#include "clang/AST/DeclNodes.inc"
  ...
  void adjustForRewrite(RewriteKind RK, FunctionDecl *Orig, QualType &T,
                        TypeSourceInfo *&TInfo, DeclarationNameInfo &NameInfo);
