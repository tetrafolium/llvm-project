...
namespace LLVM.ClangFormat {
  ...
  public class OptionPageGrid : DialogPage {
    ...
    public class StyleConverter : TypeConverter {
      ...
      public StyleConverter() {
        ...
        values = new ArrayList(new string[]{"file", "Chromium", "Google",
                                            "LLVM", "Mozilla", "WebKit"});
      }
    ...
    public sealed class FilenameConverter : TypeConverter {
      ...
      public override
        ...
        // Check if string contains quotes. On Windows, file names cannot
        // contain quotes. We do not accept them however to avoid hard-to-debug
        // problems. A quote in user input would end the parameter quote and so
    ...
    [Description(
        "The name of the predefined style used as a fallback in case clang-format " +
    ...
    [DisplayName("Enable")]
    [Description(
        "Enable running clang-format when modified files are saved. " +
    ...
    [Description(
        "When formatting on save, clang-format will be applied only to " +
  ...
  [ProvideAutoLoad(
      UIContextGuids80.SolutionExists)] // Load package on solution load
...
#region Package Members
    ...
    protected override void Initialize() {
      ...
      if (commandService != null) {
        {
          var menuCommandID =
              new CommandID(GuidList.guidClangFormatCmdSet,
    ...
    private static bool FileHasExtension(string filePath,
                                         string fileExtensions) {
      var extensions = fileExtensions.ToLower().Split(
    ...
    private void OnBeforeSave(object sender, Document document) {
      ...
      if (!FileHasExtension(document.FullName,
                            options.FormatOnSaveFileExtensions))
        return;
    ...
    private void FormatSelection(OptionPageGrid options) {
      ...
      RunClangFormatAndApplyReplacements(text, start, end, path, filePath,
                                         options, view);
    }
    ...
    private void FormatView(IWpfTextView view, OptionPageGrid options) {
      ...
      RunClangFormatAndApplyReplacements(text, 0, text.Length, path, filePath,
                                         options, view);
    }

    private void RunClangFormatAndApplyReplacements(string text, int start,
                                                    int end, string path,
                                                    string filePath,
      ...
      try {
        string replacements =
            RunClangFormat(text, start, end, path, filePath, options);
        ApplyClangFormatReplacements(replacements, view);
      } catch (Exception e) {
        var uiShell = (IVsUIShell) GetService(typeof(SVsUIShell));
        var id = Guid.Empty;
        int result;
        uiShell.ShowMessageBox(0, ref id,
                               "Error while running clang-format:", e.Message,
    ...
    /// <summary>
    /// Runs the given text through clang-format and returns the replacements as
    /// XML.
    ///
    ...
    private static string RunClangFormat(string text, int start, int end,
                                         ...
                                         OptionPageGrid options) {
      string vsixPath =
          Path.GetDirectoryName(typeof(ClangFormatPackage).Assembly.Location);
      ...
      // Poor man's escaping - this will not work when quotes are already
      // escaped in the input (but we don't need more).
      string style = options.Style.Replace("\"", "\\\"");
      string fallbackStyle = options.FallbackStyle.Replace("\"", "\\\"");
      process.StartInfo.Arguments =
          " -offset " + offset + " -length " + length +
          " -output-replacements-xml " + " -style \"" + style + "\"" +
      ...
      // as writes to the buffers will block until they are read from the other
      // side. Thus, we:
      // 1. Start the process - clang-format.exe will start to read the input
      // from the
      //    standard input.
      try {
        process.Start();
      } catch (Exception e) {
        throw new Exception("Cannot execute " + process.StartInfo.FileName +
                            ".\n\"" + e.Message +
                            "\".\nPlease make sure it is on the PATH.");
      }
      // 2. We write everything to the standard output - this cannot block, as
      // clang-format
      //    reads the full standard input before analyzing it without writing
      //    anything to the standard output.
      StreamWriter utf8Writer = new StreamWriter(
          process.StandardInput.BaseStream, new UTF8Encoding(false));
      utf8Writer.Write(text);
      // 3. We notify clang-format that the input is done - after this point
      // clang-format
      ...
      if (process.ExitCode != 0) {
        // FIXME: If clang-format writes enough to the standard error stream to
        // block, we will never reach this point; instead, read the standard
    ...
    /// </summary>
    private static void ApplyClangFormatReplacements(string replacements,
                                                     IWpfTextView view) {
      ...
      foreach (XElement replacement in root.Descendants("replacement")) {
        ...
        var span = new Span(Encoding.UTF8.GetCharCount(bytes, 0, offset),
