...
isl_morph_get_var_multi_aff(__isl_keep isl_morph *morph) {
 ...
 */
static __isl_give isl_morph *
isl_morph_set_ran_tuple_id(__isl_take isl_morph *morph, __isl_keep isl_id *id) {
  morph = isl_morph_cow(morph);
  if (!morph)
    return NULL;
...
__isl_give isl_morph *isl_morph_identity(__isl_keep isl_basic_set *bset) {
  ...
  return isl_morph_alloc(universe, isl_basic_set_copy(universe), id,
                         isl_mat_copy(id));
}
...
__isl_give isl_morph *isl_morph_empty(__isl_keep isl_basic_set *bset) {
  ...
  return isl_morph_alloc(empty, isl_basic_set_copy(empty), id,
                         isl_mat_copy(id));
}
 ...
 * compressed space.
 */
__isl_give isl_morph *
isl_basic_set_variable_compression(__isl_keep isl_basic_set *bset,
                                   enum isl_dim_type type) {
  unsigned otype;
  isl_size ntype;
  unsigned orest;
 ...
 */
__isl_give isl_morph *
isl_basic_set_variable_compression_with_id(__isl_keep isl_basic_set *bset,
 ...
 */
__isl_give isl_morph *
isl_basic_set_parameter_compression(__isl_keep isl_basic_set *bset) {
  isl_size nparam;
  isl_size nvar;
  isl_size n_div;
  int n_eq;
  ...
  if (isl_seq_first_non_zero(bset->eq[bset->n_eq - 1] + 1 + nparam,
    ...
    isl_die(isl_basic_set_get_ctx(bset), isl_error_invalid,
            "input not allowed to have parameter equalities", return NULL);
  if (n_eq > nvar + n_div)
    isl_die(isl_basic_set_get_ctx(bset), isl_error_invalid, "input not gaussed",
            return NULL);

  B = isl_mat_sub_alloc6(bset->ctx, bset->eq, 0, n_eq, 0, 1 + nparam);
  H = isl_mat_sub_alloc6(bset->ctx, bset->eq, 0, n_eq, 1 + nparam,
                         nvar + n_div);
...
static __isl_give isl_basic_set *add_strides(__isl_take isl_basic_set *bset,
  ...
  for (i = 0; 1 + i < morph->inv->n_row; ++i) {
    ...
    if (k < 0)
      goto error;
    isl_seq_cpy(bset->eq[k], morph->inv->row[1 + i], morph->inv->n_col);
    isl_seq_clr(bset->eq[k] + morph->inv->n_col, bset->n_div);
    isl_int_set(bset->eq[k][morph->inv->n_col + div], morph->inv->row[0][0]);
  }
...
__isl_give isl_basic_set *isl_morph_basic_set(__isl_take isl_morph *morph,
  ...
  res = isl_basic_set_alloc_space(isl_space_copy(morph->ran->dim),
                                  bset->n_div + max_stride,
                                  bset->n_eq + max_stride, bset->n_ineq);

  for (i = 0; i < bset->n_div; ++i)
    if (isl_basic_set_alloc_div(res) < 0)
      goto error;

  mat = isl_mat_sub_alloc6(bset->ctx, bset->eq, 0, bset->n_eq, 0,
                           morph->inv->n_row);
  ...
  mat = isl_mat_sub_alloc6(bset->ctx, bset->ineq, 0, bset->n_ineq, 0,
                           morph->inv->n_row);
  mat = isl_mat_product(mat, isl_mat_copy(morph->inv));
  if (!mat)
    goto error;
  for (i = 0; i < bset->n_ineq; ++i) {
    ...
    isl_seq_scale(res->ineq[k] + mat->n_col, bset->ineq[i] + mat->n_col,
                  morph->inv->row[0][0], bset->n_div);
  }
  isl_mat_free(mat);

  mat = isl_mat_sub_alloc6(bset->ctx, bset->div, 0, bset->n_div, 1,
                           morph->inv->n_row);
  mat = isl_mat_product(mat, isl_mat_copy(morph->inv));
  if (!mat)
    goto error;
  for (i = 0; i < bset->n_div; ++i) {
    isl_int_mul(res->div[i][0], morph->inv->row[0][0], bset->div[i][0]);
    isl_seq_cpy(res->div[i] + 1, mat->row[i], mat->n_col);
    isl_seq_scale(res->div[i] + 1 + mat->n_col, bset->div[i] + 1 + mat->n_col,
                  morph->inv->row[0][0], bset->n_div);
  }
  isl_mat_free(mat);
 ...
 */
__isl_give isl_morph *
isl_basic_set_full_compression(__isl_keep isl_basic_set *bset) {
