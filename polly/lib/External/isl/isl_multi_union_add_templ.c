...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), union_add)(__isl_take MULTI(BASE) * multi1,
  ...
  if (!multi1)
    goto error;
  if (multi1->n > 0)
    return FN(MULTI(BASE), bin_op)(multi1, multi2, &FN(EL, union_add));
  ...
  if (!multi1)
    goto error;
  multi1->u.dom = FN(DOM, union)(multi1->u.dom, FN(DOM, copy)(multi2->u.dom));
  if (!multi1->u.dom)
    goto error;
  FN(MULTI(BASE), free)(multi2);
  return multi1;
