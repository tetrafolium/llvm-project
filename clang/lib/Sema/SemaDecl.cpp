  ...
  case LookupResult::Ambiguous:
    ...
    for (LookupResult::iterator Res = Result.begin(), ResEnd = Result.end();
      ...
      if (isa<TypeDecl>(*Res) || isa<ObjCInterfaceDecl>(*Res) ||
        ...
        if (!IIDecl || (*Res)->getLocation().getRawEncoding() <
...
static ParsedType buildNestedType(Sema &S, CXXScopeSpec &SS, QualType T,
                                  SourceLocation NameLoc) {
  ASTContext &Context = S.Context;
  ...
  if (getLangOpts().CPlusPlus) {
    ...
    if (OldReturnType != NewReturnType) {
      ...
      if (OldAT && OldAT->isDeduced()) {
        New->setType(
            SubstAutoType(New->getType(), OldAT->isDependentType()
                                              ? Context.DependentTy
                                              : OldAT->getDeducedType()));
        NewQType = Context.getCanonicalType(SubstAutoType(
            NewQType, OldAT->isDependentType() ? Context.DependentTy
                                               : OldAT->getDeducedType()));
      }
    }
...
Decl *Sema::ParsedFreeStandingDeclSpec(Scope *S, AccessSpecifier AS,
                                       DeclSpec &DS, RecordDecl *&AnonRecord) {
...
static FunctionDecl *CreateNewFunctionDecl(Sema &SemaRef, Declarator &D,
  ...
  if (Name.getNameKind() == DeclarationName::CXXConstructorName) {
    ...
    return CXXConversionDecl::Create(SemaRef.Context, cast<CXXRecordDecl>(DC),
                                     ...
                                     isInline, ExplicitSpecifier, ConstexprKind,
...
Sema::ActOnFunctionDeclarator(Scope *S, Declarator &D, DeclContext *DC,
  ...
  if (!getLangOpts().CPlusPlus) {
    ...
    if (!NewFD->isInvalidDecl())
      D.setRedeclaration(
          CheckFunctionDeclaration(S, NewFD, Previous, isMemberSpecialization));
...
static bool CheckTargetCausesMultiVersioning(
  ...
  for (const auto *FD : OldFD->redecls()) {
    ...
    if (PreviousDeclsHaveMultiVersionAttribute(FD) &&
      ...
      S.Diag(FD->getLocation(), diag::err_multiversion_required_in_redecl) << 0;
      S.Diag(NewFD->getLocation(), diag::note_multiversioning_caused_here);
      NewFD->setInvalidDecl();
      return true;
    }
...
bool Sema::CheckFunctionDeclaration(Scope *S, FunctionDecl *NewFD,
  ...
  if (getLangOpts().CPlusPlus) {
    ...
    if (unsigned BuiltinID = NewFD->getBuiltinID()) {
      ...
      if (!T.isNull() && !Context.hasSameFunctionTypeIgnoringExceptionSpec(
                             T, NewFD->getType()))
...
Decl *Sema::ActOnObjCContainerStartDefinition(Decl *IDecl) {
  ...
  assert(
      OCD->getLexicalParent() == CurContext &&
      "The next DeclContext should be lexically contained in the current one.");
  CurContext = OCD;
  return IDecl;
}
  ...
  if (!FieldTy->isDependentType()) {
    ...
    if (CStdConstraintViolation || MSBitfieldViolation) {
      ...
      if (FieldName)
        return Diag(FieldLoc, diag::err_bitfield_width_exceeds_type_width)
               << FieldName << Value.toString(10) << !CStdConstraintViolation
               << DiagWidth;

      return Diag(FieldLoc, diag::err_anon_bitfield_width_exceeds_type_width)
             << Value.toString(10) << !CStdConstraintViolation << DiagWidth;
    }
    ...
    if (BitfieldIsOverwide && !FieldTy->isBooleanType() && FieldName) {
      Diag(FieldLoc, diag::warn_bitfield_width_exceeds_type_width)
          << FieldName << Value.toString(10) << (unsigned)TypeWidth;
    }
  }
...
Decl *Sema::ActOnField(Scope *S, Decl *TagD, SourceLocation DeclStart,
  ...
  FieldDecl *Res = HandleField(S, cast_or_null<RecordDecl>(TagD), DeclStart, D,
...
Decl *Sema::ActOnIvar(Scope *S, SourceLocation DeclStart, Declarator &D,
  ...
  // Construct the decl.
  ObjCIvarDecl *NewID =
      ObjCIvarDecl::Create(Context, EnclosingContext, DeclStart, Loc, II, T,
  ...
  if (!Val) {
    if (Enum->isDependentType())
      ...
      if (EnumVal < LastEnumConst->getInitVal()) {
        ...
        Diag(IdLoc, diag::ext_enum_value_not_int) << EnumVal.toString(10) << 1;
      }
    }
  }
  ...
  for (const auto &Vec : DupVector) {
    ...
    for (auto *ECD : llvm::make_range(Vec->begin() + 1, Vec->end()))
      S.Diag(ECD->getLocation(), diag::note_duplicate_element)
          << ECD << ECD->getInitVal().toString(10) << ECD->getSourceRange();
  }
}
...
void Sema::ActOnEnumBody(SourceLocation EnumLoc, SourceRange BraceRange,
  ...
  for (unsigned i = 0, e = Elements.size(); i != e; ++i) {
    ...
    if (InitVal.isUnsigned() || InitVal.isNonNegative())
      NumPositiveBits =
          std::max(NumPositiveBits, (unsigned)InitVal.getActiveBits());
    else
      NumNegativeBits =
