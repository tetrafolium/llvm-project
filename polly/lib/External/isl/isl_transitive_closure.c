 ...
 */
static __isl_give isl_map *set_path_length(__isl_take isl_map *map, int exactly,
                                           int length) {
  isl_space *space;
...
static __isl_give isl_map *path_along_steps(__isl_take isl_space *space,
  ...
  for (i = 0; i < d; ++i) {
    ...
    if (i == d - 1)
      ...
      for (j = 0; j < n; ++j)
        isl_int_set(path->eq[k][1 + nparam + 2 * d + j], steps->row[j][i]);
  }
 ...
 */
static isl_bool
parametric_constant_never_positive(__isl_keep isl_basic_set *bset, isl_int *c,
                                   int *div_purity) {
  isl_size d;
  isl_size n_div;
  isl_size nparam;
  ...
  for (i = 0; i < n_div; ++i) {
    if (div_purity[i] != PURE_PARAM)
      continue;
    isl_int_set(bset->ineq[k][1 + nparam + d + i], c[1 + nparam + d + i]);
  }
...
add_delta_constraints(__isl_take isl_basic_map *path,
                      __isl_keep isl_basic_set *delta, unsigned off,
                      unsigned nparam, unsigned d, int *div_purity, int eq,
                      int *impurity) {
  int i, k;
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (p == PURE_VAR) {
      isl_seq_cpy(path_c + off, delta_c[i] + 1 + nparam, d);
    ...
    isl_seq_cpy(path_c + off - n_div, delta_c[i] + 1 + nparam + d, n_div);
  }
...
static __isl_give isl_map *path_along_delta(__isl_take isl_space *space,
  ...
  if (impurity) {
    ...
    delta = isl_basic_set_project_out(delta, isl_dim_param, 0, nparam);
    delta = isl_basic_set_add_dims(delta, isl_dim_param, nparam);
    delta = isl_basic_set_reset_space(delta, space);
    if (!delta)
      goto error;
    path =
        isl_basic_map_extend_constraints(path, delta->n_eq, delta->n_ineq + 1);
    path = add_delta_constraints(path, delta, off, nparam, d, NULL, 1, NULL);
    path = add_delta_constraints(path, delta, off, nparam, d, NULL, 0, NULL);
...
equate_parameter_to_length(__isl_take isl_space *space, unsigned param) {
  struct isl_basic_map *bmap;
  isl_size d;
  isl_size nparam;
  isl_size total;
...
static __isl_give isl_map *construct_extended_path(__isl_take isl_space *space,
  ...
  for (i = 0; i < map->n; ++i) {
    ...
    for (j = 0; j < d; ++j) {
      ...
      fixed = isl_basic_set_plain_dim_is_fixed(delta, j, &steps->row[n][j]);
      if (fixed < 0) {
        isl_basic_set_free(delta);
        goto error;
      }
    ...
    if (j < d) {
      path = isl_map_apply_range(
          path, path_along_delta(isl_space_copy(space), delta));
...
static __isl_give isl_map *construct_component(__isl_take isl_space *space,
  ...
  if (check && (*exact = check_exactness(isl_map_copy(map), isl_map_copy(app),
                                         project)) < 0)
    goto error;
 ...
 */
static __isl_give isl_map *
construct_projected_component(__isl_take isl_space *space,
                              __isl_keep isl_map *map, isl_bool *exact,
                              int project) {
  isl_map *app;
  unsigned d;
 ...
 */
static int composability(__isl_keep isl_set *C, int i, isl_set **dom,
                         isl_set **ran, int *left, int *right,
                         __isl_keep isl_map *map) {
  int j;
  int ok;
    ...
    if (ok & RIGHT) {
      if (!dom[j])
        dom[j] = isl_set_from_basic_set(
            isl_basic_map_domain(isl_basic_map_copy(map->p[j])));
      if (!dom[j])
        return -1;
    ...
    if (ok & LEFT) {
      if (!ran[j])
        ran[j] = isl_set_from_basic_set(
            isl_basic_map_range(isl_basic_map_copy(map->p[j])));
      if (!ran[j])
        return -1;
...
static __isl_give isl_map *compose(__isl_keep isl_map *map, int i,
                                   __isl_take isl_map *qc, int *left,
                                   int *right) {
  int j;
  isl_map *comp;
...
static __isl_give isl_map *compute_incremental(__isl_take isl_space *space,
  ...
  tc = construct_projected_component(isl_space_copy(space), map_i, exact, 1);
  isl_map_free(map_i);
  ...
  *res = NULL;
  ...
  for (i = 0; i < map->n; ++i) {
    ...
    ran[i] = isl_set_from_basic_set(
        isl_basic_map_range(isl_basic_map_copy(map->p[i])));
    qc = q_closure(isl_space_copy(space), isl_set_copy(C), map->p[i], &exact_i);
    if (!qc)
      goto error;
    if (!exact_i) {
      isl_map_free(qc);
    ...
    *res = compute_incremental(isl_space_copy(space), map, i, qc, left, right,
                               &exact_i);
    if (!*res)
      goto error;
    if (exact_i)
...
static __isl_give isl_map *incremental_closure(__isl_take isl_space *space,
  ...
  if (incremental_on_entire_domain(space, map, dom, ran, left, right, &res) < 0)
    goto error;
  ...
  for (i = 0; !res && i < map->n; ++i) {
    ...
    if (!ran[i])
      goto error;
    C = isl_set_union(isl_set_copy(dom[i]), isl_set_copy(ran[i]));
    C = isl_set_from_basic_set(isl_set_simple_hull(C));
    if (!C)
      goto error;
    if (C->n != 1) {
...
static isl_stat add_length(__isl_keep isl_map *map, isl_map ***grid, int n) {
  ...
  for (i = 0; i < n; ++i)
    for (j = 0; j < n; ++j)
      grid[i][j] = isl_map_product(grid[i][j], isl_map_copy(step));
...
static void floyd_warshall_iterate(isl_map ***grid, int n, isl_bool *exact) {
  ...
  for (r = 0; r < n; ++r) {
    ...
    grid[r][r] =
        isl_map_transitive_closure(grid[r][r], check ? &r_exact : NULL);
    if (check && !r_exact)
    ...
    for (p = 0; p < n; ++p)
      for (q = 0; q < n; ++q) {
        ...
        if (p == r && q == r)
          continue;
        loop = isl_map_apply_range(isl_map_copy(grid[p][r]),
                                   isl_map_copy(grid[r][q]));
        grid[p][q] = isl_map_union(grid[p][q], loop);
        loop =
            isl_map_apply_range(isl_map_copy(grid[p][r]),
                                isl_map_apply_range(isl_map_copy(grid[r][r]),
                                                    isl_map_copy(grid[r][q])));
        grid[p][q] = isl_map_union(grid[p][q], loop);
 ...
 */
static __isl_give isl_map *
floyd_warshall_with_groups(__isl_take isl_space *space, __isl_keep isl_map *map,
  ...
  for (k = 0; k < map->n; ++k) {
    i = group[2 * k];
    j = group[2 * k + 1];
    grid[i][j] = isl_map_union(
        grid[i][j], isl_map_from_basic_map(isl_basic_map_copy(map->p[k])));
  }
...
static int *setup_groups(isl_ctx *ctx, __isl_keep isl_basic_map **list, int n,
  ...
  *set = isl_calloc_array(ctx, isl_set *, 2 * n);
  ...
  for (i = 0; i < n; ++i) {
    isl_set *dom;
    dom = isl_set_from_basic_set(
        isl_basic_map_domain(isl_basic_map_copy(list[i])));
    if (merge(*set, group, dom, 2 * i) < 0)
      goto error;
    dom = isl_set_from_basic_set(
        isl_basic_map_range(isl_basic_map_copy(list[i])));
...
static isl_bool basic_map_follows(int i, int j, void *user) {
  ...
  map21 = isl_map_from_basic_map(isl_basic_map_apply_range(
      isl_basic_map_copy(data->list[j]), isl_basic_map_copy(data->list[i])));
  ...
  map12 = isl_map_from_basic_map(isl_basic_map_apply_range(
      isl_basic_map_copy(data->list[i]), isl_basic_map_copy(data->list[j])));
 ...
 */
static __isl_give isl_map *
construct_power_components(__isl_take isl_space *space, __isl_keep isl_map *map,
                           isl_bool *exact, int project) {
  int i, n, c;
  struct isl_map *path = NULL;
  ...
  while (n) {
    ...
    while (g->order[i] != -1) {
      comp =
          isl_map_add_basic_map(comp, isl_basic_map_copy(map->p[g->order[i]]));
      --n;
      ++i;
    }
    path_comp = floyd_warshall(isl_space_copy(space), comp, exact, project);
    path_comp = anonymize(path_comp);
    path_comb =
        isl_map_apply_range(isl_map_copy(path), isl_map_copy(path_comp));
...
static __isl_give isl_map *construct_power(__isl_keep isl_map *map,
  ...
  app = construct_power_components(isl_space_copy(space), map, exact, project);
 ...
 */
static __isl_give isl_map *map_power(__isl_take isl_map *map, isl_bool *exact,
                                     int project) {
  struct isl_map *app = NULL;
...
static __isl_give isl_map *box_closure_on_domain(__isl_take isl_map *map,
  ...
  total = isl_space_dim(space, isl_dim_all);
  bmap =
      isl_basic_map_alloc_space(space, aff->n_div + 1, aff->n_div, 2 * d + 1);
  ...
  for (i = 0; i < aff->n_eq; ++i) {
    ...
    isl_seq_cpy(bmap->eq[k] + 1 + nparam + d, aff->eq[i] + 1 + nparam, d);
    isl_seq_neg(bmap->eq[k] + 1 + nparam, aff->eq[i] + 1 + nparam, d);
    isl_seq_cpy(bmap->eq[k] + 1 + nparam + 2 * d, aff->eq[i] + 1 + nparam + d,
                aff->n_div);
  ...
  for (i = 0; i < d; ++i) {
    ...
    res = isl_set_solve_lp(delta, 0, obj->el, map->ctx->one, &opt, NULL, NULL);
    if (res == isl_lp_error)
      goto error;
    if (res == isl_lp_ok) {
      k = isl_basic_map_alloc_inequality(bmap);
      if (k < 0)
        goto error;
      isl_seq_clr(bmap->ineq[k], 1 + nparam + 2 * d + bmap->n_div);
    ...
    res = isl_set_solve_lp(delta, 1, obj->el, map->ctx->one, &opt, NULL, NULL);
    if (res == isl_lp_error)
      goto error;
    if (res == isl_lp_ok) {
      k = isl_basic_map_alloc_inequality(bmap);
  ...
  *qc = box_closure_with_identity(map_i, C);
  ...
  for (j = 0; j < map->n; ++j) {
    ...
    if (i == j)
      continue;
    map_j = isl_map_from_basic_map(isl_basic_map_copy(map->p[j]));
    test = isl_map_apply_range(isl_map_copy(id), isl_map_copy(map_j));
...
static __isl_give isl_map *transitive_closure_omega(__isl_take isl_map *map,
  ...
  for (i = 0; i < map->n; ++i) {
    ...
    for (j = 0; j < map->n; ++j) {
      if (j == i)
        continue;
      app = isl_map_add_basic_map(app, isl_basic_map_copy(map->p[j]));
    }
...
union_floyd_warshall_on_list(isl_ctx *ctx, __isl_keep isl_basic_map **list,
                             int n, isl_bool *exact) {
  int i, j, k;
  int n_group;
  int *group = NULL;
  isl_set **set = NULL;
  ...
  for (k = 0; k < n; ++k) {
    ...
    j = group[2 * k + 1];
    grid[i][j] = isl_map_union(
        grid[i][j], isl_map_from_basic_map(isl_basic_map_copy(list[k])));
  }
 ...
 */
static __isl_give isl_union_map *
union_floyd_warshall(__isl_take isl_union_map *umap, isl_bool *exact) {
  int i, n;
  isl_ctx *ctx;
 ...
 */
static __isl_give isl_union_map *
union_components(__isl_take isl_union_map *umap, isl_bool *exact) {
  int i;
  int n;
  isl_ctx *ctx;
  isl_basic_map **list = NULL;
  ...
  while (l) {
    ...
    while (g->order[i] != -1) {
      comp = isl_union_map_add_map(
          comp, isl_map_from_basic_map(isl_basic_map_copy(list[g->order[i]])));
      --l;
      ++i;
    }
