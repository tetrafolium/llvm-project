...
static __isl_give isl_space *isl_space_prefix(__isl_take isl_space *space,
  ...
  for (i = 0; i < nvar; ++i) {
    ...
    prefix_name = isl_alloc_array(ctx, char, prefix_len + strlen(name) + 1);
    if (!prefix_name)
      goto error;
    memcpy(prefix_name, prefix, prefix_len);
    strcpy(prefix_name + prefix_len, name);

    space = isl_space_set_dim_name(space, isl_dim_set, i, prefix_name);
    free(prefix_name);
  }
...
isl_space_coefficients(__isl_take isl_space *space) {
  ...
  space_param = isl_space_move_dims(space_param, isl_dim_set, 0, isl_dim_param,
                                    0, nparam);
  space_param = isl_space_prefix(space_param, "c_");
  space_param = isl_space_insert_dims(space_param, isl_dim_set, 0, 1);
  space_param = isl_space_set_dim_name(space_param, isl_dim_set, 0, "c_cst");
...
static __isl_give isl_space *isl_space_unprefix(__isl_take isl_space *space,
  ...
  for (i = 0; i < n; ++i) {
    ...
    space = isl_space_set_dim_name(space, type, i, name + prefix_len);
  }
...
static __isl_give isl_space *isl_space_solutions(__isl_take isl_space *space) {
  ...
  if (nparam < 0)
    return isl_space_free(space);
  space = isl_space_move_dims(space, isl_dim_param, 0, isl_dim_in, 0, nparam);
...
rational_universe(__isl_take isl_space *space) {
  isl_basic_set *bset;
...
static __isl_give isl_basic_set *farkas(__isl_take isl_basic_set *bset,
  ...
  dual = isl_basic_set_alloc_space(space, bset->n_eq + bset->n_ineq, total,
                                   bset->n_ineq + (shift > 0));
  ...
  for (i = 0; i < total; ++i) {
    ...
    for (j = 0; j < bset->n_eq; ++j)
      isl_int_set(dual->eq[k][1 + shift + total + j], bset->eq[j][1 + i]);
  ...
  for (i = 0; i < bset->n_ineq; ++i) {
    ...
    if (k < 0)
      goto error;
    isl_seq_clr(dual->ineq[k], 1 + shift + total + bset->n_eq + bset->n_ineq);
  ...
  if (shift > 0) {
    ...
    for (j = 0; j < bset->n_eq; ++j)
      isl_int_neg(dual->ineq[k][2 + total + j], bset->eq[j][0]);
    for (j = 0; j < bset->n_ineq; ++j)
      isl_int_neg(dual->ineq[k][2 + total + bset->n_eq + j], bset->ineq[j][0]);
  }
 ...
 */
static __isl_give isl_basic_set *
isl_basic_set_coefficients_base(__isl_take isl_basic_set *bset) {
 ...
 */
static isl_stat isl_coefficients_product_data_init(
    isl_ctx *ctx, struct isl_coefficients_product_data *data, int n) {
  data->n = n;
  data->pos = 0;
  data->start_next = 0;
  data->factors = isl_calloc_array(ctx, struct isl_coefficients_factor_data, n);
  if (!data->factors)
 ...
 */
static isl_bool
isl_basic_set_coefficients_factor(__isl_keep isl_basic_set *bset, void *user) {
 ...
 */
static __isl_give isl_basic_set *
add_lines(__isl_take isl_basic_set *bset,
 ...
 */
static __isl_give isl_basic_set *
add_rays(__isl_take isl_basic_set *bset,
 ...
 */
static __isl_give isl_basic_set *
add_vertex(__isl_take isl_basic_set *bset,
           struct isl_coefficients_product_data *data) {
  int i;
  int k;
  isl_int lcm, f;
 ...
 */
static __isl_give isl_basic_set *
construct_product(isl_ctx *ctx, struct isl_coefficients_product_data *data) {
  int i;
 ...
 */
static __isl_give isl_basic_set *
isl_basic_set_coefficients_product(__isl_take isl_factorizer *f) {
  struct isl_coefficients_product_data data;
  isl_ctx *ctx;
  isl_basic_set *coeff;
  isl_bool every;
  ...
  every = isl_factorizer_every_factor_basic_set(
      f, &isl_basic_set_coefficients_factor, &data);
 ...
 */
static __isl_give isl_basic_set *
isl_basic_set_coefficients_morphed_product(__isl_take isl_factorizer *f) {
 ...
 */
static __isl_give isl_basic_set *
basic_set_coefficients(__isl_take isl_basic_set *bset) {
  isl_factorizer *f;
  isl_size nparam;
  ...
  if (nparam < 0)
    return isl_basic_set_free(bset);
  bset =
      isl_basic_set_move_dims(bset, isl_dim_set, 0, isl_dim_param, 0, nparam);
...
isl_basic_set_coefficients(__isl_take isl_basic_set *bset) {
  ...
  if (bset->n_div)
    isl_die(bset->ctx, isl_error_invalid,
            "input set not allowed to have local variables", goto error);
 ...
 */
static __isl_give isl_basic_set *
coefficients_wrap(__isl_take isl_basic_set *bset, void *user) {
