 ...
 */
__isl_give isl_val *isl_val_rat_from_isl_int(isl_ctx *ctx, isl_int n,
                                             isl_int d) {
  isl_val *v;
...
long isl_val_get_num_si(__isl_keep isl_val *v) {
  ...
  if (!isl_int_fits_slong(v->n))
    isl_die(isl_val_get_ctx(v), isl_error_invalid, "numerator too large",
            return 0);
...
long isl_val_get_den_si(__isl_keep isl_val *v) {
  ...
  if (!isl_int_fits_slong(v->d))
    isl_die(isl_val_get_ctx(v), isl_error_invalid, "denominator too large",
            return 0);
...
__isl_give isl_val *isl_val_pow2(__isl_take isl_val *v) {
  ...
  if (!isl_val_is_int(v))
    isl_die(isl_val_get_ctx(v), isl_error_invalid,
            "can only compute integer powers", return isl_val_free(v));
  neg = isl_val_is_neg(v);
  if (neg)
    isl_int_neg(v->n, v->n);
  if (!isl_int_fits_ulong(v->n))
    isl_die(isl_val_get_ctx(v), isl_error_invalid, "exponent too large",
...
__isl_give isl_val *isl_val_div(__isl_take isl_val *v1,
  ...
  if (isl_val_is_zero(v2) || (!isl_val_is_rat(v1) && !isl_val_is_rat(v2))) {
    isl_val_free(v2);
    return isl_val_set_nan(v1);
  }
  if (isl_val_is_zero(v1)) {
...
isl_bool isl_val_is_divisible_by(__isl_keep isl_val *v1,
  ...
  if (!isl_val_is_int(v1) || !isl_val_is_int(v2))
    isl_die(isl_val_get_ctx(v1), isl_error_invalid, "expecting two integers",
            return isl_bool_error);
 ...
 */
static void isl_int_gcdext(isl_int *g, isl_int *x, isl_int *y, isl_int a,
                           isl_int b) {
  isl_int d, tmp;
...
__isl_give isl_val *isl_val_gcdext(__isl_take isl_val *v1,
  ...
  if (!isl_val_is_int(v1) || !isl_val_is_int(v2))
    isl_die(ctx, isl_error_invalid, "expecting two integers", goto error);
...
isl_bool isl_val_eq(__isl_keep isl_val *v1, __isl_keep isl_val *v2) {
  ...
  return isl_bool_ok(isl_int_eq(v1->n, v2->n) && isl_int_eq(v1->d, v2->d));
}
...
isl_bool isl_val_abs_eq(__isl_keep isl_val *v1, __isl_keep isl_val *v2) {
  ...
  return isl_bool_ok(isl_int_abs_eq(v1->n, v2->n) && isl_int_eq(v1->d, v2->d));
}
...
isl_bool isl_val_ne(__isl_keep isl_val *v1, __isl_keep isl_val *v2) {
  ...
  return isl_bool_ok(isl_int_ne(v1->n, v2->n) || isl_int_ne(v1->d, v2->d));
}
