...
isl_multi_pw_aff_init_explicit_domain(__isl_take isl_multi_pw_aff *mpa) {
 ...
 */
isl_bool
isl_multi_pw_aff_involves_explicit_domain_dims(__isl_keep isl_multi_pw_aff *mpa,
...
static __isl_give isl_multi_pw_aff *isl_multi_pw_aff_move_explicit_domain_dims(
    ...
    unsigned dst_pos, enum isl_dim_type src_type, unsigned src_pos,
    unsigned n) {
  if (isl_multi_pw_aff_check_has_explicit_domain(mpa) < 0)
    return isl_multi_pw_aff_free(mpa);
  mpa = isl_multi_pw_aff_cow(mpa);
  ...
  mpa->u.dom =
      isl_set_move_dims(mpa->u.dom, dst_type, dst_pos, src_type, src_pos, n);
  if (!mpa->u.dom)
    return isl_multi_pw_aff_free(mpa);
  return mpa;
}
