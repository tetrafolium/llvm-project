...
public:
    ArrayRef<Node> nodes() const {
        return makeArrayRef(Nodes.get(), NodesSize);
    }
    ...
    ArrayRef<Edge> edges() const {
        return makeArrayRef(Edges.get(), EdgesSize);
    }
    const Edge *edges_begin() const {
        return edges().begin();
    }
    ...
    public:
        ...
        index_iterator index_begin() const {
            return V.set_bits_begin();
        }
        index_iterator index_end() const {
            return V.set_bits_end();
        }
        ...
        iterator begin() const {
            return iterator{*this, V.find_first()};
        }
        iterator end() const {
...
struct GraphTraits<ImmutableGraph<NodeValueT, EdgeValueT> *> {
    ...
    static typename GraphT::size_type size(GraphT *G) {
        return G->nodes_size();
    }
};
