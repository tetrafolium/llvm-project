...
__isl_keep isl_space *FN(MULTI(BASE),
                         peek_space)(__isl_keep MULTI(BASE) * multi) {
  return multi ? multi->space : NULL;
}

__isl_give isl_space *FN(MULTI(BASE),
                         get_space)(__isl_keep MULTI(BASE) * multi) {
  return isl_space_copy(FN(MULTI(BASE), peek_space)(multi));
}

__isl_give isl_space *FN(MULTI(BASE),
                         get_domain_space)(__isl_keep MULTI(BASE) * multi) {
...
__isl_give MULTI(BASE) * FN(MULTI(BASE), dup)(__isl_keep MULTI(BASE) * multi) {
  ...
  for (i = 0; i < multi->n; ++i)
    dup = FN(FN(MULTI(BASE), set), BASE)(dup, i, FN(EL, copy)(multi->u.p[i]));
  if (FN(MULTI(BASE), has_explicit_domain)(multi))
...
#include "check_type_range_templ.c"
     ...
     */
    __isl_give EL *
    FN(MULTI(BASE), get_at)(__isl_keep MULTI(BASE) * multi, int pos) {
  isl_ctx *ctx;
...
static __isl_give MULTI(BASE) *
    FN(MULTI(BASE), restore)(__isl_take MULTI(BASE) * multi, int pos,
                             __isl_take EL *el) {
 ...
 */
static __isl_give MULTI(BASE) *
    FN(MULTI(BASE), restore_check_space)(__isl_take MULTI(BASE) * multi,
                                         int pos, __isl_take EL *el) {
  isl_space *space;
 ...
 */
__isl_give MULTI(BASE) * FN(MULTI(BASE), set_at)(__isl_take MULTI(BASE) * multi,
                                                 int pos, __isl_take EL *el) {
  ...
  if (match < 0)
    goto error;
  if (!match) {
    multi = FN(MULTI(BASE), align_params)(multi, FN(EL, get_space)(el));
...
__isl_give MULTI(BASE) *
    FN(FN(MULTI(BASE), set), BASE)(__isl_take MULTI(BASE) * multi, int pos,
                                   __isl_take EL *el) {
 ...
 */
__isl_give LIST(EL) *
    FN(MULTI(BASE), get_list)(__isl_keep MULTI(BASE) * multi) {
  isl_size n;
  int i;
  LIST(EL) * list;
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), reset_space_and_domain)(__isl_take MULTI(BASE) * multi,
  ...
  for (i = 0; i < multi->n; ++i) {
    multi->u.p[i] =
        FN(EL, reset_domain_space)(multi->u.p[i], isl_space_copy(domain));
    if (!multi->u.p[i])
      goto error;
  }
  if (FN(MULTI(BASE), has_explicit_domain)(multi)) {
    multi = FN(MULTI(BASE),
               reset_explicit_domain_space)(multi, isl_space_copy(domain));
    if (!multi)
      goto error;
  }
...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), reset_domain_space)(__isl_take MULTI(BASE) * multi,
                                        __isl_take isl_space *domain) {
  isl_space *space;
...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), reset_space)(__isl_take MULTI(BASE) * multi,
                                 __isl_take isl_space *space) {
  isl_space *domain;
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), reset_user)(__isl_take MULTI(BASE) * multi) {
  isl_space *space;
...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), realign_domain)(__isl_take MULTI(BASE) * multi,
  ...
  for (i = 0; i < multi->n; ++i) {
    multi->u.p[i] =
        FN(EL, realign_domain)(multi->u.p[i], isl_reordering_copy(exp));
    if (!multi->u.p[i])
      goto error;
  }
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), align_params)(__isl_take MULTI(BASE) * multi,
  ...
  if (!isl_space_has_named_params(multi->space))
    isl_die(ctx, isl_error_invalid, "input has unnamed parameters", goto error);

  if (FN(MULTI(BASE), has_explicit_domain)(multi)) {
    multi = FN(MULTI(BASE),
               align_explicit_domain_params)(multi, isl_space_copy(model));
    if (!multi)
      goto error;
  }
 ...
 */
__isl_give MULTI(BASE) *
    FN(FN(MULTI(BASE), from), LIST(BASE))(__isl_take isl_space *space,
  ...
  ctx = isl_space_get_ctx(space);
  if (n != dim)
    isl_die(ctx, isl_error_invalid, "invalid number of elements in list",
            goto error);
...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), drop_dims)(__isl_take MULTI(BASE) * multi,
  ...
  if (FN(MULTI(BASE), has_explicit_domain)(multi))
    multi = FN(MULTI(BASE), drop_explicit_domain_dims)(multi, type, first, n);
  if (!multi)
    return NULL;
...
#include "isl_align_params_bin_templ.c"
    ...
    __isl_give
    MULTI(BASE) *
    FN(MULTI(BASE), range_product)(__isl_take MULTI(BASE) * multi1,
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), range_factor_domain)(__isl_take MULTI(BASE) * multi) {
  isl_space *space;
  isl_size total, keep;
  ...
  if (!isl_space_range_is_wrapping(multi->space))
    isl_die(FN(MULTI(BASE), get_ctx)(multi), isl_error_invalid,
            "range is not a product", return FN(MULTI(BASE), free)(multi));
  ...
  if (keep < 0)
    multi = FN(MULTI(BASE), free)(multi);
  multi = FN(MULTI(BASE), drop_dims)(multi, isl_dim_out, keep, total - keep);
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), range_factor_range)(__isl_take MULTI(BASE) * multi) {
  isl_space *space;
  isl_size total, keep;
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), factor_range)(__isl_take MULTI(BASE) * multi) {
  isl_space *space;
  isl_size total, keep;
  ...
  if (!isl_space_is_wrapping(multi->space))
    isl_die(FN(MULTI(BASE), get_ctx)(multi), isl_error_invalid, "not a product",
            return FN(MULTI(BASE), free)(multi));
...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), flatten_range)(__isl_take MULTI(BASE) * multi) {
  if (!multi)
    return NULL;
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), flat_range_product)(__isl_take MULTI(BASE) * multi1,
                                        __isl_take MULTI(BASE) * multi2) {
  MULTI(BASE) * multi;
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), range_splice)(__isl_take MULTI(BASE) * multi1, unsigned pos,
                                  __isl_take MULTI(BASE) * multi2) {
  MULTI(BASE) * res;
  isl_size dim;
...
#include "isl_type_check_equal_space_templ.c"
    ...
    static __isl_give MULTI(BASE) *
    FN(MULTI(BASE),
       bin_op)(__isl_take MULTI(BASE) * multi1, __isl_take MULTI(BASE) * multi2,
...
static __isl_give MULTI(BASE) *
  ...
  for (i = 0; i < multi1->n; ++i) {
    multi1->u.p[i] = fn(multi1->u.p[i], FN(EL, copy)(multi2->u.p[i]));
    if (!multi1->u.p[i])
      goto error;
  }

  if (FN(MULTI(BASE), has_explicit_domain)(multi2))
    multi1 = FN(MULTI(BASE), intersect_explicit_domain)(multi1, multi2);
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), from_range)(__isl_take MULTI(BASE) * multi) {
  isl_space *space;
  ...
  if (!isl_space_is_set(multi->space))
    isl_die(FN(MULTI(BASE), get_ctx)(multi), isl_error_invalid,
            "not living in a set space", return FN(MULTI(BASE), free)(multi));
