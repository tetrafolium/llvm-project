...
struct BaseOffset {
    ...
    bool isEmpty() const {
        return NonVirtualOffset.isZero() && !VirtualBase;
    }
};
...
public:
    ...
    const_iterator components_begin() const {
        return Components.rbegin();
    }
    const_iterator components_end() const {
        return Components.rend();
    }

    const VCallOffsetMap &getVCallOffsets() const {
        return VCallOffsets;
    }
