 ...
 */
static __isl_give isl_set *parametrize(__isl_take isl_set *set, int first,
                                       __isl_keep isl_id_list *ids) {
  int i, n;
  unsigned nparam;
  ...
  for (i = 0; i < n; ++i)
    set =
        isl_set_equate(set, isl_dim_param, nparam + i, isl_dim_set, first + i);
 ...
 * freshly created parameters with identifiers "ids".
 */
__isl_give isl_set *parametrization(__isl_take isl_space *space, int len,
...
ppcg_get_schedule(isl_ctx *ctx, struct ppcg_options *options,
...
ppcg_set_schedule_node_type(__isl_take isl_schedule_node *node,
