...
FN(PW,
   better)(__isl_keep EL *el1, __isl_keep EL *el2,
...
static __isl_give PW *FN(PW, merge)(__isl_take PW *pw1, __isl_take PW *pw2,
  ...
  while (i < pw1->n || j < pw2->n) {
    ...
    if (i < pw1->n && j < pw2->n)
      cmp = FN(EL, plain_cmp)(pw1->p[i].FIELD, pw2->p[j].FIELD);
    else
      cmp = i < pw1->n ? -1 : 1;
...
static __isl_give PW *FN(PW, union_opt_cmp)(
  ...
  if (!isl_space_is_equal(pw1->dim, pw2->dim))
    isl_die(ctx, isl_error_invalid, "arguments should live in the same space",
            goto error);
  ...
  for (i = 0; i < pw1->n; ++i) {
    for (j = 0; j < pw2->n; ++j) {
      ...
      disjoint = isl_set_is_disjoint(pw1->p[i].set, pw2->p[j].set);
      if (disjoint < 0)
        goto error;
      if (disjoint)
        continue;
      better = FN(PW, better)(pw2->p[j].FIELD, pw1->p[i].FIELD, cmp);
