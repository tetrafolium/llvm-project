 ...
 */
__isl_null isl_stride_info *
isl_stride_info_free(__isl_take isl_stride_info *si) {
  if (!si)
    return NULL;
...
isl_stride_info_copy(__isl_keep isl_stride_info *si) {
  if (!si)
    return NULL;
...
static isl_stat set_stride(struct isl_detect_stride_data *data,
  ...
  if (data->found) {
    ...
    stride2 = data->stride;
    g = isl_val_gcdext(isl_val_copy(stride), isl_val_copy(stride2), &a, &b);
...
static isl_stat detect_stride(__isl_take isl_constraint *c, void *user) {
  ...
  if (has_stride >= 0 && has_stride) {
    ...
    for (i = 0; i < n_div; ++i)
      aff = isl_aff_set_coefficient_si(aff, isl_dim_div, i, 0);
