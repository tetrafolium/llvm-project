...
public:
    ...
    CGOpenMPRegionKind getRegionKind() const {
        return RegionKind;
    }
...
public:
    ...
    public:
        ...
        unsigned getNumberOfParts() const {
...
public:
    ...
    CodeGenFunction::CGCapturedStmtInfo *getOldCSI() const {
        return OldCSI;
    }
...
void CGOpenMPRuntime::createOffloadEntriesAndInfoMetadata() {
    ...
    auto &&GetMDString = [&C](StringRef V) {
...
static void EmitOMPAggregateReduction(
    ...
    // Emit copy.
    CodeGenFunction::OMPPrivateScope Scope(CGF);
    Scope.addPrivate(LHSVar, [=]() {
        return LHSElementCurrent;
    });
    Scope.addPrivate(RHSVar, [=]() {
...
public:
    ...
    public:
        ...
        const ValueDecl *getMapDecl() const {
            return MapDecl;
        }
        const Expr *getMapExpr() const {
            return MapExpr;
        }
    };
    ...
    public:
        ...
        const ValueDecl *getDevicePtrDecl() const {
            return DevPtrDecl;
        }
        void setDevicePtrDecl(const ValueDecl *D) {
            DevPtrDecl = D;
        }
    };
...
bool CGOpenMPRuntime::isNontemporalDecl(const ValueDecl *VD) const {
    ...
    [VD](const NontemporalDeclsSet &Set) {
        return Set.count(VD) > 0;
    });
}
...
void CGOpenMPRuntime::checkAndEmitSharedLastprivateConditional(
    ...
    Range, [](const LastprivateConditionalData &D) {
        return !D.Disabled;
    });
