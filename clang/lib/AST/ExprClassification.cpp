...
static Cl::Kinds ClassifyConditional(ASTContext &Ctx, const Expr *trueExpr,
...
static Cl::Kinds ClassifyExprValueKind(const LangOptions &Lang, const Expr *E,
                                       ExprValueKind Kind) {
  switch (Kind) {
  case VK_RValue:
...
#include "clang/AST/StmtNodes.inc"
  ...
  case Expr::SubstNonTypeTemplateParmExprClass:
    return ClassifyInternal(
        Ctx, cast<SubstNonTypeTemplateParmExpr>(E)->getReplacement());
  ...
  case Expr::DeclRefExprClass:
    if (E->getType() == Ctx.UnknownAnyTy)
      return isa<FunctionDecl>(cast<DeclRefExpr>(E)->getDecl()) ? Cl::CL_PRValue
                                                                : Cl::CL_LValue;
  ...
  case Expr::CXXUnresolvedConstructExprClass:
    return ClassifyUnnamed(
        Ctx, cast<CXXUnresolvedConstructExpr>(E)->getTypeAsWritten());
...
static Cl::Kinds ClassifyDecl(ASTContext &Ctx, const Decl *D) {
  ...
  if (const auto *NTTParm = dyn_cast<NonTypeTemplateParmDecl>(D))
    ...
    islvalue = isa<VarDecl>(D) || isa<FieldDecl>(D) ||
               ...
               isa<MSGuidDecl>(D) || isa<TemplateParamObjectDecl>(D) ||
...
static Cl::Kinds ClassifyMemberExpr(ASTContext &Ctx, const MemberExpr *E) {
  if (E->getType() == Ctx.UnknownAnyTy)
    return (isa<FunctionDecl>(E->getMemberDecl()) ? Cl::CL_PRValue
                                                  : Cl::CL_LValue);
...
static Cl::Kinds ClassifyBinaryOp(ASTContext &Ctx, const BinaryOperator *E) {
  ...
  if (E->isAssignmentOp())
    return (E->getLHS()->getObjectKind() == OK_ObjCProperty ? Cl::CL_PRValue
                                                            : Cl::CL_LValue);
