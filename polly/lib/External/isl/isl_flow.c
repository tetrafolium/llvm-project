 ...
 */
static __isl_give isl_restriction *
isl_restriction_alloc(__isl_take isl_map *source_map,
                      enum isl_restriction_type type) {
  isl_ctx *ctx;
...
isl_restriction_none(__isl_take isl_map *source_map) {
 ...
 */
__isl_give isl_restriction *
isl_restriction_empty(__isl_take isl_map *source_map) {
 ...
 */
__isl_give isl_restriction *
isl_restriction_input(__isl_take isl_set *source_restr,
 ...
 */
__isl_give isl_restriction *
isl_restriction_output(__isl_take isl_set *source_restr) {
  isl_ctx *ctx;
  isl_restriction *restr;
...
isl_restriction_free(__isl_take isl_restriction *restr) {
  if (!restr)
    return NULL;
...
isl_access_info_set_restrict(__isl_take isl_access_info *acc,
 ...
 */
__isl_give isl_access_info *
isl_access_info_add_source(__isl_take isl_access_info *acc,
  ...
  if (must) {
    if (acc->n_may)
      acc->source[acc->n_must + acc->n_may] = acc->source[acc->n_must];
 ...
 */
static __isl_give isl_access_info *
isl_access_info_sort_sources(__isl_take isl_access_info *acc) {
...
isl_stat isl_flow_foreach(__isl_keep isl_flow *deps,
  ...
  for (i = 0; i < deps->n_source; ++i) {
    if (isl_map_plain_is_empty(deps->dep[i].map))
      continue;
    if (fn(isl_map_copy(deps->dep[i].map), deps->dep[i].must, deps->dep[i].data,
           user) < 0)
      return isl_stat_error;
  }
...
static __isl_give isl_map *restricted_partial_lexmax(
    __isl_keep isl_access_info *acc, __isl_take isl_map *dep, int source,
    __isl_take isl_set *sink, __isl_give isl_set **empty) {
  isl_map *source_map;
  isl_restriction *restr;
  isl_set *sink_domain;
  isl_set *sink_restr;
  ...
  source_map = isl_map_apply_domain(source_map, isl_map_copy(acc->domain_map));
  sink_domain = isl_set_copy(sink);
  sink_domain = isl_set_apply(sink_domain, isl_map_copy(acc->domain_map));
  restr = acc->restrict_fn(source_map, sink_domain, acc->source[source].data,
                           acc->restrict_user);
...
static struct isl_map *last_source(struct isl_access_info *acc,
                                   struct isl_set *set_C, int j, int level,
...
static struct isl_map *last_later_source(struct isl_access_info *acc,
                                         struct isl_map *old_map, int j,
                                         int before_level, int k,
                                         int after_level,
                                         struct isl_set **empty) {
  isl_space *space;
  ...
  dep_map = isl_map_apply_range(read_map, write_map);
  space = space_align_and_join(
      isl_map_get_space(acc->source[k].map),
...
static isl_stat intermediate_sources(__isl_keep isl_access_info *acc,
  ...
  for (k = j - 1; k >= 0; --k) {
    ...
    plevel2 = acc->level_before(acc->source[j].data, acc->source[k].data);
    if (plevel2 < 0)
      return isl_stat_error;
    ...
    for (level = sink_level; level <= depth; ++level) {
      ...
      T = last_later_source(acc, copy, j, sink_level, k, level, &trest);
...
static __isl_give isl_map *all_sources(__isl_keep isl_access_info *acc,
                                       __isl_take isl_set *set_C, int j,
                                       int level) {
  isl_map *read_map;
  isl_map *write_map;
  isl_map *dep_map;
...
static __isl_give isl_map *all_later_sources(__isl_keep isl_access_info *acc,
                                             __isl_take isl_map *old_map, int j,
                                             int before_level, int k,
                                             int after_level) {
  isl_space *space;
  isl_set *set_C;
  ...
  dep_map = isl_map_apply_range(read_map, write_map);
  space =
      isl_space_join(isl_map_get_space(acc->source[acc->n_must + j].map),
...
all_intermediate_sources(__isl_keep isl_access_info *acc,
                         __isl_take isl_map *map, struct isl_map **must_rel,
                         struct isl_map **may_rel, int j, int sink_level) {
  int k, level;
  isl_size n_in = isl_map_dim(acc->source[acc->n_must + j].map, isl_dim_in);
  int depth = 2 * n_in + 1;
  ...
  for (k = 0; k < acc->n_must; ++k) {
    ...
    for (level = sink_level; level <= depth; ++level) {
      ...
      map = isl_map_union(map, T);
      may_rel[k] = isl_map_union_disjoint(
          may_rel[k], isl_map_intersect_range(isl_map_copy(must_rel[k]),
                                              isl_set_copy(ran)));
      T = isl_map_from_domain_and_range(
          isl_set_universe(isl_space_domain(isl_map_get_space(must_rel[k]))),
          ran);
...
static __isl_give isl_flow *handle_coscheduled(__isl_keep isl_access_info *acc,
  ...
  for (i = acc->n_must - 1; i >= 0; --i) {
    ...
    for (j = i - 1; j >= 0; --j) {
      ...
      coscheduled = acc->coscheduled(acc->source[i].data, acc->source[j].data);
      if (coscheduled < 0) {
        isl_map_free(move);
      ...
      if (!coscheduled)
        continue;
      depth = acc->level_before(acc->source[i].data, acc->source[j].data) / 2;
    ...
    for (j = 0; j < acc->n_may; ++j) {
      ...
      pos = acc->n_must + j;
      coscheduled =
          acc->coscheduled(acc->source[i].data, acc->source[pos].data);
      if (coscheduled < 0) {
        isl_map_free(move);
        return isl_flow_free(flow);
      }
      if (!coscheduled)
        continue;
      depth = acc->level_before(acc->source[i].data, acc->source[pos].data) / 2;
      map = coscheduled_source(acc, must_rel[i], pos, depth);
      factor = isl_map_domain_factor_range(isl_map_copy(map));
      pos = 2 * acc->n_must + j;
      flow->dep[pos].map = isl_map_union(flow->dep[pos].map, factor);
      map = isl_map_domain_factor_domain(map);
      move = isl_map_union(move, map);
    }
 ...
 */
static __isl_give isl_flow *
compute_mem_based_dependences(__isl_keep isl_access_info *acc) {
  int i;
  isl_set *mustdo;
  isl_set *maydo;
  isl_flow *res;
  ...
  for (i = 0; i < acc->n_may; ++i) {
    ...
    mustdo = isl_set_subtract(mustdo, isl_map_range(isl_map_copy(dep)));
 ...
 *	    compute the last iteration of the source that precedes the sink
 *access at that level add result to possible last accesses at level l of source
 *w for all sources w2 that we haven't considered yet at this level that may
 ...
 *								of dep
 *			add result to possible last accesses at level l of write
 *w2 and replace possible last accesses dep by the remainder
 *
 *
 ...
 */
static __isl_give isl_flow *
compute_val_based_dependences(__isl_keep isl_access_info *acc) {
  isl_ctx *ctx;
  isl_flow *res;
  isl_set *mustdo = NULL;
  isl_set *maydo = NULL;
  ...
  for (level = depth; level >= 1; --level) {
    ...
    for (j = acc->n_must - 1; j >= 0; --j) {
      ...
      plevel = acc->level_before(acc->source[j].data, acc->sink.data);
      if (plevel < 0)
        goto error;
      if (!can_precede_at_level(plevel, level))
        continue;
      ...
      if (isl_set_plain_is_empty(mustdo) && isl_set_plain_is_empty(maydo))
        break;
    }
    for (j = j - 1; j >= 0; --j) {
      int plevel;
    ...
    for (j = 0; j < acc->n_may; ++j) {
      ...
      plevel =
          acc->level_before(acc->source[acc->n_must + j].data, acc->sink.data);
      if (plevel < 0)
        goto error;
      if (!can_precede_at_level(plevel, level))
        continue;
      ...
      T = all_intermediate_sources(acc, T, must_rel, may_rel, j, level);
      res->dep[2 * acc->n_must + j].map = T;
    }

    for (j = acc->n_must - 1; j >= 0; --j) {
      res->dep[2 * j].map =
          isl_map_union_disjoint(res->dep[2 * j].map, must_rel[j]);
      res->dep[2 * j + 1].map =
          isl_map_union_disjoint(res->dep[2 * j + 1].map, may_rel[j]);
    }
 ...
 */
static __isl_give isl_flow *
access_info_compute_flow_core(__isl_take isl_access_info *acc) {
 ...
 */
__isl_give isl_flow *
isl_access_info_compute_flow(__isl_take isl_access_info *acc) {
  int j;
  struct isl_flow *res;
 ...
 */
static __isl_give struct isl_sched_info *
sched_info_alloc(__isl_keep isl_map *map) {
  isl_ctx *ctx;
  isl_space *space;
...
isl_union_access_info_free(__isl_take isl_union_access_info *access) {
 ...
 */
isl_ctx *
isl_union_access_info_get_ctx(__isl_keep isl_union_access_info *access) {
  if (!access)
    return NULL;
...
isl_union_access_info_alloc(isl_ctx *ctx) {
 ...
 */
static __isl_give isl_union_access_info *
isl_union_access_info_init(__isl_take isl_union_access_info *info) {
  isl_space *space;
  isl_union_map *empty;
  enum isl_access_type i;
...
isl_union_access_info_from_sink(__isl_take isl_union_map *sink) {
  isl_ctx *ctx;
 ...
 */
static __isl_give isl_union_access_info *
isl_union_access_info_set(__isl_take isl_union_access_info *info,
 ...
 */
__isl_give isl_union_access_info *
isl_union_access_info_set_must_source(__isl_take isl_union_access_info *access,
                                      __isl_take isl_union_map *must_source) {
  return isl_union_access_info_set(access, isl_access_must_source, must_source);
}

/* Replace the possible source accesses of "access" by "may_source".
 */
__isl_give isl_union_access_info *
isl_union_access_info_set_may_source(__isl_take isl_union_access_info *access,
                                     __isl_take isl_union_map *may_source) {
  return isl_union_access_info_set(access, isl_access_may_source, may_source);
}

/* Replace the kills of "info" by "kill".
 */
__isl_give isl_union_access_info *
isl_union_access_info_set_kill(__isl_take isl_union_access_info *info,
...
isl_union_access_info_get(__isl_keep isl_union_access_info *info,
                          enum isl_access_type type) {
  if (!info)
    return NULL;
...
isl_union_access_info_get_must_source(__isl_keep isl_union_access_info *info) {
 ...
 */
__isl_give isl_union_map *
isl_union_access_info_get_may_source(__isl_keep isl_union_access_info *info) {
 ...
 */
__isl_give isl_union_map *
isl_union_access_info_get_kill(__isl_keep isl_union_access_info *info) {
 ...
 */
static isl_bool
isl_union_access_has_kill(__isl_keep isl_union_access_info *info) {
  isl_bool empty;
 ...
 */
__isl_give isl_union_access_info *
isl_union_access_info_set_schedule(__isl_take isl_union_access_info *access,
 ...
 */
__isl_give isl_union_access_info *
isl_union_access_info_set_schedule_map(__isl_take isl_union_access_info *access,
...
__isl_give isl_union_access_info *
isl_union_access_info_copy(__isl_keep isl_union_access_info *access) {
  ...
  if (access->schedule)
    copy = isl_union_access_info_set_schedule(
        copy, isl_schedule_copy(access->schedule));
  else
    copy = isl_union_access_info_set_schedule_map(
        copy, isl_union_map_copy(access->schedule_map));
...
print_access_field(__isl_take isl_printer *p,
...
isl_printer_print_union_access_info(__isl_take isl_printer *p,
  ...
  if (access->schedule) {
    ...
    p = print_yaml_field_union_map(p, key_str[isl_ai_key_schedule_map],
                                   access->schedule_map);
  }
 ...
 */
__isl_give char *
isl_union_access_info_to_str(__isl_keep isl_union_access_info *access) {
  isl_printer *p;
  char *s;
 ...
 */
__isl_give isl_union_access_info *
isl_stream_read_union_access_info(isl_stream *s) {
  isl_ctx *ctx;
  ...
  while ((more = isl_stream_yaml_next(s)) > 0) {
    ...
    case isl_ai_key_schedule_map:
      ...
      info = isl_union_access_info_set_schedule_map(info, schedule_map);
      if (!info)
        return NULL;
      break;
 ...
 */
__isl_give isl_union_access_info *
isl_union_access_info_read_from_file(isl_ctx *ctx, FILE *input) {
  isl_stream *s;
  isl_union_access_info *access;
 ...
 */
static __isl_give isl_union_access_info *
isl_union_access_info_align_params(__isl_take isl_union_access_info *access) {
  isl_space *space;
  enum isl_access_type i;
  ...
  if (access->schedule_map)
    space = isl_space_align_params(
        space, isl_union_map_get_space(access->schedule_map));
  for (i = isl_access_sink; i < isl_access_end; ++i)
    access->access[i] =
        isl_union_map_align_params(access->access[i], isl_space_copy(space));
  if (!access->schedule_map) {
    isl_space_free(space);
  } else {
    access->schedule_map =
...
isl_union_access_info_introduce_schedule(
  ...
  for (i = isl_access_sink; i < isl_access_end; ++i)
    access->access[i] =
        isl_union_map_apply_range(isl_union_map_copy(sm), access->access[i]);
 ...
 */
__isl_null isl_union_flow *
isl_union_flow_free(__isl_take isl_union_flow *flow) {
  if (!flow)
    return NULL;
 ...
 */
__isl_give isl_union_map *
isl_union_flow_get_full_must_dependence(__isl_keep isl_union_flow *flow) {
  if (!flow)
    return NULL;
 ...
 */
__isl_give isl_union_map *
isl_union_flow_get_full_may_dependence(__isl_keep isl_union_flow *flow) {
  if (!flow)
    return NULL;
 ...
 */
__isl_give isl_union_map *
isl_union_flow_get_must_dependence(__isl_keep isl_union_flow *flow) {
  isl_union_map *dep;
 ...
 */
__isl_give isl_union_map *
isl_union_flow_get_may_dependence(__isl_keep isl_union_flow *flow) {
  isl_union_map *dep;
 ...
 */
static __isl_give isl_union_map *
isl_union_flow_get_non_must_dependence(__isl_keep isl_union_flow *flow) {
  if (!flow)
    return NULL;
 ...
 */
__isl_give isl_union_map *
isl_union_flow_get_must_no_source(__isl_keep isl_union_flow *flow) {
  if (!flow)
    return NULL;
 ...
 */
__isl_give isl_union_map *
isl_union_flow_get_may_no_source(__isl_keep isl_union_flow *flow) {
  if (!flow)
    return NULL;
 ...
 */
static __isl_give isl_union_map *
isl_union_flow_get_non_must_no_source(__isl_keep isl_union_flow *flow) {
  if (!flow)
    return NULL;
...
isl_union_flow_alloc(__isl_take isl_space *space) {
  isl_ctx *ctx;
  isl_union_map *empty;
  isl_union_flow *flow;
  ...
  if (!flow->must_dep || !flow->may_dep || !flow->must_no_source ||
      !flow->may_no_source)
...
isl_union_flow_copy(__isl_keep isl_union_flow *flow) {
  isl_union_flow *copy;
  ...
  copy->must_dep =
      isl_union_map_union(copy->must_dep, isl_union_map_copy(flow->must_dep));
  copy->may_dep =
      isl_union_map_union(copy->may_dep, isl_union_map_copy(flow->may_dep));
  copy->must_no_source = isl_union_map_union(
      copy->must_no_source, isl_union_map_copy(flow->must_no_source));
  copy->may_no_source = isl_union_map_union(
      copy->may_no_source, isl_union_map_copy(flow->may_no_source));

  if (!copy->must_dep || !copy->may_dep || !copy->must_no_source ||
      !copy->may_no_source)
 ...
 */
static __isl_give isl_union_flow *
isl_union_flow_drop_schedule(__isl_take isl_union_flow *flow) {
  if (!flow)
    return NULL;
  ...
  flow->may_no_source = isl_union_map_domain_factor_range(flow->may_no_source);
...
static isl_stat collect_matching_array(__isl_take isl_map *map, void *user) {
  ...
  data->accesses =
      isl_access_info_add_source(data->accesses, map, data->must, info);
...
static isl_stat compute_flow(__isl_take isl_map *map, void *user) {
  ...
  if (isl_union_map_foreach_map(data->must_source, &count_matching_array,
                                data) < 0)
    goto error;
  if (isl_union_map_foreach_map(data->may_source, &count_matching_array, data) <
      0)
    goto error;

  data->sink_info = sched_info_alloc(map);
  data->source_info =
      isl_calloc_array(ctx, struct isl_sched_info *, data->count);

  data->accesses = isl_access_info_alloc(isl_map_copy(map), data->sink_info,
                                         &before, data->count);
  ...
  data->count = 0;
  data->must = 1;
  if (isl_union_map_foreach_map(data->must_source, &collect_matching_array,
                                data) < 0)
    goto error;
  data->must = 0;
  if (isl_union_map_foreach_map(data->may_source, &collect_matching_array,
                                data) < 0)
    goto error;
  ...
  df->must_no_source = isl_union_map_union(
      df->must_no_source,
      isl_union_map_from_map(isl_flow_get_no_source(flow, 1)));
  df->may_no_source = isl_union_map_union(
      df->may_no_source,
 ...
 */
static __isl_give isl_union_flow *
isl_union_flow_drop_kill_source(__isl_take isl_union_flow *flow,
  ...
  move =
      isl_union_map_intersect_range_factor_range(move, isl_union_map_copy(may));
  may = isl_union_map_union(may, isl_union_map_copy(must));
  flow->may_dep =
      isl_union_map_intersect_range_factor_range(flow->may_dep, may);
  flow->must_dep =
      isl_union_map_intersect_range_factor_range(flow->must_dep, must);
 ...
 */
static __isl_give isl_union_access_info *
isl_union_access_info_normalize(__isl_take isl_union_access_info *access) {
  if (!access)
    return NULL;
  access->access[isl_access_may_source] = isl_union_map_subtract(
 ...
 */
static __isl_give isl_union_flow *
compute_flow_union_map(__isl_take isl_union_access_info *access) {
  struct isl_compute_flow_data data;
  isl_union_map *sink;
...
static void isl_compute_flow_schedule_data_align_params(
  ...
  for (i = 1; i < data->n_sink; ++i)
    space =
        isl_space_align_params(space, isl_map_get_space(data->sink[i].access));
  ...
  for (i = 0; i < data->n_sink; ++i)
    data->sink[i].access =
        isl_map_align_params(data->sink[i].access, isl_space_copy(space));
  for (i = 0; i < data->n_source; ++i)
    data->source[i].access =
        isl_map_align_params(data->source[i].access, isl_space_copy(space));
...
static int before_node(void *first, void *second) {
  ...
  if (isl_schedule_node_get_type(shared) == isl_schedule_node_sequence) {
    ...
    pos1 = isl_schedule_node_get_ancestor_child_position(node1, shared);
    pos2 = isl_schedule_node_get_ancestor_child_position(node2, shared);
    if (pos1 < 0 || pos2 < 0) {
      isl_schedule_node_free(shared);
      return -1;
    }
 ...
 */
static __isl_give isl_access_info *
add_matching_sources(__isl_take isl_access_info *access,
                     struct isl_scheduled_access *sink,
                     struct isl_compute_flow_schedule_data *data) {
  int i;
  isl_space *space;
  ...
  for (i = 0; i < data->n_source; ++i) {
    ...
    access = isl_access_info_add_source(access, isl_map_copy(source->access),
                                        source->must, source->node);
  }
 ...
 */
static __isl_give isl_union_flow *
compute_single_flow(__isl_take isl_union_flow *uf,
                    struct isl_scheduled_access *sink,
                    struct isl_compute_flow_schedule_data *data) {
  int i;
  ...
  uf->must_no_source =
      isl_union_map_union(uf->must_no_source, isl_union_map_from_map(map));
  map = isl_map_domain_factor_range(isl_flow_get_no_source(flow, 0));
  uf->may_no_source =
      isl_union_map_union(uf->may_no_source, isl_union_map_from_map(map));
 ...
 */
static __isl_give isl_union_flow *
compute_flow_schedule(__isl_take isl_union_access_info *access) {
  ...
  data.n_sink = 0;
  data.n_source = 0;
  if (isl_schedule_foreach_schedule_node_top_down(
          access->schedule, &count_sink_source, &data) < 0)
    goto error;
  ...
  data.n_source = 0;
  if (isl_schedule_foreach_schedule_node_top_down(
          access->schedule, &collect_sink_source, &data) < 0)
    goto error;
 ...
 */
__isl_give isl_union_flow *
isl_union_access_info_compute_flow(__isl_take isl_union_access_info *access) {
  isl_bool has_kill;
 ...
 */
__isl_give isl_printer *
isl_printer_print_union_flow(__isl_take isl_printer *p,
  ...
  isl_union_map_free(umap);
  p = print_yaml_field_union_map(p, "must_no_source", flow->must_no_source);
