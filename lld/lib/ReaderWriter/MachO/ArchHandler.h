...
public:
  ...
  typedef std::function<llvm::Error(uint32_t symbolIndex, const lld::Atom **)>
  ...
  virtual llvm::Error getReferenceInfo(
      const normalized::Relocation &reloc, const DefinedAtom *inAtom,
      uint32_t offsetInAtom, uint64_t fixupAddress, bool isBigEndian,
      FindAtomBySectionAndAddress atomFromAddress,
      FindAtomBySymbolIndex atomFromSymbolIndex, Reference::KindValue *kind,
      const lld::Atom **target, Reference::Addend *addend) = 0;

  /// Analyzes a pair of relocations from a .o file and returns the info
  /// (kind, target, addend) needed to instantiate a Reference.
  /// Two helper functions are passed as parameters to find the target atom
  /// given a symbol index or address.
  virtual llvm::Error getPairReferenceInfo(
  ...
  virtual void generateAtomContent(
  ...
  virtual void
  appendSectionRelocations(const DefinedAtom &atom, uint64_t atomSectionOffset,
  ...
  virtual Reference::KindValue
  dataInCodeTransitionStart(const MachODefinedAtom &atom) {
    return 0;
  }
  ...
  /// a data-in-code range.
  virtual Reference::KindValue
  dataInCodeTransitionEnd(const MachODefinedAtom &atom) {
    return 0;
  }
