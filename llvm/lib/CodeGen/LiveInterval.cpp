...
private:
    ...
    CollectionT &segments() {
        return impl().segmentsColl();
    }

    Segment *segmentAt(iterator I) {
        return const_cast<Segment *>(&(*I));
    }
};
...
private:
    ...
    LiveRange::Segments &segmentsColl() {
        return LR->segments;
    }

    void insertAtEnd(const Segment &S) {
        LR->segments.push_back(S);
    }
    ...
    iterator findInsertPos(Segment S) {
        return llvm::upper_bound(*LR, S.start);
    }
};
...
private:
    ...
    LiveRange::SegmentSet &segmentsColl() {
