...
static bool canSinkInstructions(
  ...
  if (HasUse) {
    ...
    if (!all_of(Insts, [&PNUse, &Succ](const Instruction *I) -> bool {
          ...
          return (PNUse && PNUse->getParent() == Succ &&
...
bool SimplifyCFGOpt::SpeculativelyExecuteBB(BranchInst *BI, BasicBlock *ThenBB,
  ...
  Convert |= validateAndCostRequiredSelects(
      BB, ThenBB, EndBB, SpeculatedInstructions, BudgetRemaining, TTI);
  ...
  if (SpeculatedStoreValue) {
    ...
    Value *S =
        Builder.CreateSelect(BrCond, TrueV, FalseV, "spec.store.select", BI);
      ...
      for (unsigned i = 0, e = PHIs.size(); i != e; ++i) {
        ...
        if (PBI->getSuccessor(0) == UniqueSucc) {
          ...
          MergedCond = cast<Instruction>(Builder.CreateBinOp(
              Instruction::And, NotCond, CondInPred, "and.cond"));
          if (PBI_C->isOne())
            MergedCond = cast<Instruction>(Builder.CreateBinOp(
