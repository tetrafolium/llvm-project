...
template <> struct FPUIntType<float> {
    using Type = uint32_t;
};
template <> struct FPUIntType<double> {
    using Type = uint64_t;
};
...
UIntType mantissa :
    MantissaWidth<T>::value;
uint16_t exponent :
    ExponentWidth<T>::value;
    uint8_t sign : 1;
    ...
    bool isInf() const {
        return mantissa == 0 && exponent == maxExponent;
    }

    bool isNaN() const {
        return exponent == maxExponent && mantissa != 0;
    }
