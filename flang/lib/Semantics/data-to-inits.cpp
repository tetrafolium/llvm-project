...
public:
    ...
    const SomeExpr *operator*() const {
        return GetExpr(GetConstant());
    }
    parser::CharBlock LocateSource() const {
...
public:
    ...
    const DataInitializations &inits() const {
        return inits_;
    }
    bool HasSurplusValues() const {
        return !values_.IsAtEnd();
    }
...
bool DataInitializationCompiler::Scan(const parser::DataStmtObject &object) {
    ...
    common::visitors{
        ...
        [&](const parser::DataImpliedDo &ido) {
            return Scan(ido);
        },
    },
    object.u);
