...
protected:
  ...
  /// HasAcquireRelease - True if the subtarget supports v8 atomics (LDA/LDAEX
  /// etc) instructions
  ...
  /// HasRetAddrStack - Some processors perform return stack prediction. CodeGen
  /// should avoid issue "normal" call instructions to callees which do not
  /// return.
  ...
  /// HasFPAO - if true, processor  does positive address offset computation
  /// faster
  bool HasFPAO = false;
