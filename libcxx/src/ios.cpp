...
const char* __iostream_category::name() const _NOEXCEPT { return "iostream"; }
...
static size_t __ios_new_cap(
    size_t __req_size,
    size_t __current_cap) { // Precondition: __req_size > __current_cap
...
long& ios_base::iword(int index) {
  ...
  if (req_size > __iarray_cap_) {
    ...
    long* iarray =
        static_cast<long*>(realloc(__iarray_, newcap * sizeof(long)));
    if (iarray == 0) {
      setstate(badbit);
      static long error;
      error = 0;
...
void*& ios_base::pword(int index) {
  ...
  if (req_size > __parray_cap_) {
    ...
    void** parray =
        static_cast<void**>(realloc(__parray_, newcap * sizeof(void*)));
    if (parray == 0) {
      setstate(badbit);
      static void* error;
      error = 0;
...
void ios_base::register_callback(event_callback fn, int index) {
  ...
  if (req_size > __event_cap_) {
    ...
    event_callback* fns = static_cast<event_callback*>(
        realloc(__fn_, newcap * sizeof(event_callback)));
    if (fns == 0)
      setstate(badbit);
    __fn_ = fns;
...
void ios_base::swap(ios_base& rhs) _NOEXCEPT {
