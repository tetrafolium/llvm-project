...
static SDValue narrowIfNeeded(SelectionDAG *CurDAG, SDValue N) {
  ...
  MachineSDNode *Node = CurDAG->getMachineNode(TargetOpcode::EXTRACT_SUBREG, dl,
                                               MVT::i32, N, SubReg);
...
static SDValue Widen(SelectionDAG *CurDAG, SDValue N) {
  ...
  MachineSDNode *Node = CurDAG->getMachineNode(TargetOpcode::INSERT_SUBREG, dl,
...
bool AArch64DAGToDAGISel::tryIndexedLoad(SDNode *N) {
  ...
  if (InsertTo64) {
    SDValue SubReg = CurDAG->getTargetConstant(AArch64::sub_32, dl, MVT::i32);
    LoadedVal = SDValue(
        CurDAG->getMachineNode(AArch64::SUBREG_TO_REG, dl, MVT::i64,
...
static bool isBitfieldExtractOpFromAnd(SelectionDAG *CurDAG, SDNode *N,
  ...
  LSB = SrlImm;
  MSB = SrlImm +
        (VT == MVT::i32 ? countTrailingOnes<uint32_t>(AndImm)
