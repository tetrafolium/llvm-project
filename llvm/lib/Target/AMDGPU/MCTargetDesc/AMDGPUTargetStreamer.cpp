...
StringRef AMDGPUTargetStreamer::getArchNameFromElfMach(unsigned ElfMach) {
  ...
  switch (ElfMach) {
  default:
    llvm_unreachable("Unhandled ELF::EF_AMDGPU type");
...
void AMDGPUTargetAsmStreamer::EmitDirectiveHSACodeObjectVersion(
  ...
  OS << "\t.hsa_code_object_version " << Twine(Major) << "," << Twine(Minor)
     << '\n';
}

void AMDGPUTargetAsmStreamer::EmitDirectiveHSACodeObjectISA(
    uint32_t Major, uint32_t Minor, uint32_t Stepping, StringRef VendorName,
    StringRef ArchName) {
  OS << "\t.hsa_code_object_isa " << Twine(Major) << "," << Twine(Minor) << ","
     << Twine(Stepping) << ",\"" << VendorName << "\",\"" << ArchName << "\"\n";
}

void AMDGPUTargetAsmStreamer::EmitAMDKernelCodeT(
    const amd_kernel_code_t &Header) {
...
#define PRINT_FIELD(STREAM, DIRECTIVE, KERNEL_DESC, MEMBER_NAME, FIELD_NAME)   \
  ...
  PRINT_FIELD(OS, ".amdhsa_system_sgpr_private_segment_wavefront_offset", KD,
              compute_pgm_rsrc2,
...
void AMDGPUTargetELFStreamer::EmitDirectiveHSACodeObjectISA(
    uint32_t Major, uint32_t Minor, uint32_t Stepping, StringRef VendorName,
    StringRef ArchName) {
...
void AMDGPUTargetELFStreamer::EmitAMDKernelCodeT(
    const amd_kernel_code_t &Header) {
...
void AMDGPUTargetELFStreamer::EmitAmdhsaKernelDescriptor(
  ...
  MCSymbolELF *KernelCodeSymbol =
      cast<MCSymbolELF>(Context.getOrCreateSymbol(Twine(KernelName)));
