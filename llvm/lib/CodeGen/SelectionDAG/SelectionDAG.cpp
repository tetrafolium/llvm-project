...
SDValue SelectionDAG::FoldSetCC(EVT VT, SDValue N1, SDValue N2,
  ...
  if (ConstantSDNode *N2C = dyn_cast<ConstantSDNode>(N2)) {
    ...
    if (ConstantSDNode *N1C = dyn_cast<ConstantSDNode>(N1)) {
      ...
      case ISD::SETEQ:
        return getBoolConstant(C1 == C2, dl, VT, OpVT);
      case ISD::SETNE:
        return getBoolConstant(C1 != C2, dl, VT, OpVT);
      case ISD::SETULT:
        return getBoolConstant(C1.ult(C2), dl, VT, OpVT);
      case ISD::SETUGT:
        return getBoolConstant(C1.ugt(C2), dl, VT, OpVT);
      case ISD::SETULE:
        return getBoolConstant(C1.ule(C2), dl, VT, OpVT);
      case ISD::SETUGE:
        return getBoolConstant(C1.uge(C2), dl, VT, OpVT);
      case ISD::SETLT:
        return getBoolConstant(C1.slt(C2), dl, VT, OpVT);
      case ISD::SETGT:
        return getBoolConstant(C1.sgt(C2), dl, VT, OpVT);
      case ISD::SETLE:
        return getBoolConstant(C1.sle(C2), dl, VT, OpVT);
      case ISD::SETGE:
        return getBoolConstant(C1.sge(C2), dl, VT, OpVT);
      }
    }
  }
...
unsigned SelectionDAG::ComputeNumSignBits(SDValue Op, const APInt &DemandedElts,
  ...
  case ISD::SIGN_EXTEND_VECTOR_INREG: {
    ...
    APInt DemandedSrcElts =
        DemandedElts.zextOrSelf(SrcVT.getVectorNumElements());
...
SDValue SelectionDAG::getNode(unsigned Opcode, const SDLoc &DL, EVT VT,
  ...
  if (ConstantSDNode *C = dyn_cast<ConstantSDNode>(Operand)) {
    ...
    case ISD::BITCAST:
      ...
      break;
    case ISD::ABS:
      return getConstant(Val.abs(), DL, VT, C->isTargetOpcode(), C->isOpaque());
    case ISD::BITREVERSE:
    ...
    case ISD::FP16_TO_FP: {
      ...
      (void)FPV.convert(EVTToAPFloatSemantics(VT), APFloat::rmNearestTiesToEven,
                        &Ignored);
  ...
  if (ConstantFPSDNode *C = dyn_cast<ConstantFPSDNode>(Operand)) {
    ...
    case ISD::FP_TO_FP16: {
      ...
      // This can return overflow, underflow, or inexact; we don't care.
      // FIXME need to be more flexible about rounding mode.
      (void)V.convert(APFloat::IEEEhalf(), APFloat::rmNearestTiesToEven,
                      &Ignored);
      return getConstant(V.bitcastToAPInt().getZExtValue(), DL, VT);
    }
    }
  ...
  case ISD::FP_EXTEND:
    assert(VT.isFloatingPoint() && Operand.getValueType().isFloatingPoint() &&
           "Invalid FP cast!");
    if (Operand.getValueType() == VT)
      return Operand; // noop conversion.
    assert((!VT.isVector() ||
            VT.getVectorElementCount() ==
  ...
  case ISD::TRUNCATE:
    ...
    if (OpOpcode == ISD::ZERO_EXTEND || OpOpcode == ISD::SIGN_EXTEND ||
      ...
      if (Operand.getOperand(0).getValueType().getScalarType().bitsLT(
              VT.getScalarType()))
        return getNode(OpOpcode, DL, VT, Operand.getOperand(0));
  ...
  case ISD::SIGN_EXTEND_VECTOR_INREG:
    ...
    break;
  case ISD::ABS:
    assert(VT.isInteger() && VT == Operand.getValueType() && "Invalid ABS!");
    if (OpOpcode == ISD::UNDEF)
      return getUNDEF(VT);
    break;
  case ISD::BSWAP:
    assert(VT.isInteger() && VT == Operand.getValueType() && "Invalid BSWAP!");
...
SDValue SelectionDAG::getAtomic(unsigned Opcode, const SDLoc &dl, EVT MemVT,
  ...
  assert((Opcode == ISD::ATOMIC_LOAD_ADD || Opcode == ISD::ATOMIC_LOAD_SUB ||
          ...
          Opcode == ISD::ATOMIC_LOAD_FSUB || Opcode == ISD::ATOMIC_SWAP ||
