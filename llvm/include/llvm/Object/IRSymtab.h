...
struct Symbol {
    ...
    bool isFormatSpecific() const {
        return (Flags >> S::FB_format_specific) & 1;
    }
    bool isUnnamedAddr() const {
        return (Flags >> S::FB_unnamed_addr) & 1;
    }
    bool isExecutable() const {
        return (Flags >> S::FB_executable) & 1;
    }
...
class Reader {
    ...
    StringRef str(storage::Str S) const {
        return S.get(Strtab);
    }
...
public:
    ...
    StringRef getTargetTriple() const {
        return str(header().TargetTriple);
    }

    /// Returns the source file path specified at compile time.
    StringRef getSourceFileName() const {
        return str(header().SourceFileName);
    }
    ...
    StringRef getCOFFLinkerOpts() const {
        return str(header().COFFLinkerOpts);
    }
