...
protected:
  virtual void* do_allocate(size_t, size_t) { __throw_bad_alloc(); }
...
__default_memory_resource(bool set = false,
                          memory_resource* new_res = nullptr) _NOEXCEPT {
#ifndef _LIBCPP_HAS_NO_ATOMIC_HEADER
  ...
  if (set) {
    ...
    return _VSTD::atomic_exchange_explicit(&__res, new_res,
                                           memory_order_acq_rel);
  } else {
    return _VSTD::atomic_load_explicit(&__res, memory_order_acquire);
  }
#elif !defined(_LIBCPP_HAS_NO_THREADS)
