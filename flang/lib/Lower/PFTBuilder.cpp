...
public:
    ...
    /// Get the result
    std::unique_ptr<lower::pft::Program> result() {
        return std::move(pgm);
    }
    ...
    // Module like
    bool Pre(const parser::Module &node) {
        return enterModule(node);
    }
    bool Pre(const parser::Submodule &node) {
        return enterModule(node);
    }
    ...
    void analyzeBranches(lower::pft::Evaluation *parentConstruct,
        ...
        for (auto &eval : evaluationList) {
            eval.visit(common::visitors{
                ...
                [&](const parser::GotoStmt &s) {
                    markBranchTarget(eval, s.v);
                },
                [&](const parser::IfStmt &) {
                    lastIfStmtEvaluation = &eval;
                },
...
public:
    ...
    std::size_t getNodeIndex(const lower::pft::Program &) {
        return 0;
    }
...
Fortran::lower::pft::Evaluation::getOwningProcedure() const {
    return parentVariant.visit(common::visitors{
        [](lower::pft::FunctionLikeUnit &c) {
            return &c;
        },
        [&](lower::pft::Evaluation &c) {
...
void Fortran::lower::pft::Program::dump() {
