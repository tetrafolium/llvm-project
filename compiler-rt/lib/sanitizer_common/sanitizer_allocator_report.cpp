...
class ScopedAllocatorErrorReport {
 public:
  ScopedAllocatorErrorReport(const char *error_summary_,
      ...
      : error_summary(error_summary_), stack(stack_) {
    Printf("%s", d.Error());
  }
...
void NORETURN ReportCallocOverflow(uptr count, uptr size,
                                   const StackTrace *stack) {
  {
    ...
    Report(
        "ERROR: %s: calloc parameters overflow: count * size (%zd * %zd) "
...
void NORETURN ReportPvallocOverflow(uptr size, const StackTrace *stack) {
  {
    ...
    Report(
        "ERROR: %s: pvalloc parameters overflow: size 0x%zx rounded up to "
...
void NORETURN ReportInvalidAllocationAlignment(uptr alignment,
                                               const StackTrace *stack) {
  {
    ...
    Report(
        "ERROR: %s: invalid allocation alignment: %zd, alignment must be a "
        "power of two\n",
...
#else
    Report(
        "ERROR: %s: invalid alignment requested in aligned_alloc: %zd, "
...
void NORETURN ReportAllocationSizeTooBig(uptr user_size, uptr max_size,
                                         const StackTrace *stack) {
  {
    ...
    Report(
        "ERROR: %s: requested allocation size 0x%zx exceeds maximum "
...
void NORETURN ReportOutOfMemory(uptr requested_size, const StackTrace *stack) {
  {
    ...
    Report(
        "ERROR: %s: allocator is out of memory trying to allocate 0x%zx "
        "bytes\n",
