...
#include "isl_type_check_equal_space_templ.c"
    ...
    isl_point_alloc(__isl_take isl_space *space, __isl_take isl_vec *vec) {
...
#include "check_type_range_templ.c"
     ...
     */
    __isl_give isl_val *
    isl_point_get_coordinate_val(__isl_keep isl_point *pnt,
  ...
  if (isl_point_is_void(pnt))
    isl_die(ctx, isl_error_invalid, "void point does not have coordinates",
            return NULL);
  ...
  v = isl_val_rat_from_isl_int(ctx, pnt->vec->el[1 + pos], pnt->vec->el[0]);
...
__isl_give isl_point *isl_point_set_coordinate_val(__isl_take isl_point *pnt,
  ...
  if (isl_int_eq(pnt->vec->el[0], v->d)) {
    ...
    isl_seq_scale(pnt->vec->el + 1, pnt->vec->el + 1, v->d, pnt->vec->size - 1);
    isl_int_mul(pnt->vec->el[1 + pos], pnt->vec->el[0], v->n);
...
isl_stat isl_set_foreach_point(__isl_keep isl_set *set,
  ...
  for (i = 0; i < set->n; ++i)
    if (isl_basic_set_scan(isl_basic_set_copy(set->p[i]), &fp.callback) < 0)
      goto error;
...
isl_basic_set_box_from_points(__isl_take isl_point *pnt1,
  ...
  isl_assert(pnt1->dim->ctx, isl_space_is_equal(pnt1->dim, pnt2->dim),
             goto error);
...
print_coordinate(__isl_take isl_printer *p, struct isl_print_space_data *data,
                 unsigned pos) {
