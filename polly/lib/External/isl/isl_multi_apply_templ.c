...
__isl_give MULTI(BASE) *
    FN(FN(MULTI(BASE), apply_aligned),
       APPLY_DOMBASE)(__isl_take MULTI(BASE) * multi, __isl_take APPLY_DOM *set,
...
static __isl_give MULTI(BASE) *
  ...
  if (!isl_space_has_named_params(multi->space) ||
    ...
    isl_die(ctx, isl_error_invalid, "unaligned unnamed parameters", goto error);
  multi = FN(MULTI(BASE), align_params)(multi, FN(APPLY_DOM, get_space)(set));
