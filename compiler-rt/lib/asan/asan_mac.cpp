...
void AsanApplyToGlobals(globals_op_fptr op, const void *needle) {
  ...
  __asan_global *globals =
      (__asan_global *)getsectiondata(mh, "__DATA", "__asan_globals", &size);

  if (!globals)
    return;
  if (size % sizeof(__asan_global) != 0)
    return;
...
// alloc_asan_context().
extern "C" void asan_dispatch_call_block_and_release(void *block) {
...
extern "C" asan_block_context_t *alloc_asan_context(void *ctxt,
                                                    dispatch_function_t func,
...
INTERCEPTOR(void, dispatch_group_async_f, dispatch_group_t group,
  ...
  (group, dq, (void *)asan_ctxt, asan_dispatch_call_block_and_release);
}
