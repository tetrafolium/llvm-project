...
static __isl_give isl_ast_graft *
isl_ast_graft_copy(__isl_keep isl_ast_graft *graft);
...
isl_ast_graft_get_node(__isl_keep isl_ast_graft *graft) {
 ...
 */
__isl_give isl_ast_graft *
isl_ast_graft_alloc_domain(__isl_take isl_map *executed,
                           __isl_keep isl_ast_build *build) {
  isl_ast_node *node;
...
static __isl_give isl_set *hoist_guard(__isl_take isl_set *guard,
  ...
  if (depth < dim) {
    guard = isl_set_remove_divs_involving_dims(guard, isl_dim_set, depth, 1);
 ...
 */
__isl_give isl_set *
isl_ast_graft_list_extract_hoistable_guard(__isl_keep isl_ast_graft_list *list,
                                           __isl_keep isl_ast_build *build) {
  int i;
  isl_size n;
  isl_bool equal;
  ...
  for (i = 0; i < n; ++i) {
    ...
    guard_i = isl_set_intersect(guard_i, isl_set_from_basic_set(enforced));
    guard_i = isl_set_intersect(guard_i, isl_ast_build_get_domain(build));
...
ast_node_insert_if(__isl_take isl_ast_node *node, __isl_take isl_set *guard,
                   __isl_keep isl_ast_build *build) {
 ...
 */
static __isl_give isl_ast_graft *
insert_if_node(__isl_take isl_ast_graft *graft, __isl_take isl_set *guard,
               __isl_keep isl_ast_build *build) {
  int univ;
 ...
 */
static __isl_give isl_ast_graft *
insert_pending_guard_node(__isl_take isl_ast_graft *graft,
                          __isl_keep isl_ast_build *build) {
  if (!graft)
    return NULL;
 ...
 */
__isl_give isl_ast_graft *
isl_ast_graft_set_enforced(__isl_take isl_ast_graft *graft,
...
update_enforced(__isl_take isl_basic_set *enforced,
                __isl_keep isl_ast_graft *graft, int depth) {
  isl_size dim;
  ...
  if (depth < dim)
    enforced_g = isl_basic_set_eliminate(enforced_g, isl_dim_set, depth, 1);
  enforced_g = isl_basic_set_remove_unknown_divs(enforced_g);
  enforced_g =
      isl_basic_set_align_params(enforced_g, isl_basic_set_get_space(enforced));
  enforced =
      isl_basic_set_align_params(enforced, isl_basic_set_get_space(enforced_g));
  enforced = isl_set_simple_hull(isl_basic_set_union(enforced, enforced_g));
...
static __isl_give isl_ast_graft_list *graft_extend_body(
    __isl_take isl_ast_graft_list *list, __isl_keep isl_ast_node **body,
...
extend_then(__isl_take isl_ast_graft_list *list,
 ...
 */
static __isl_give isl_ast_graft_list *
extend_else(__isl_take isl_ast_graft_list *list,
            __isl_keep isl_ast_node *last_if, __isl_take isl_ast_graft *graft,
            __isl_keep isl_ast_build *build) {
 ...
 */
static __isl_give isl_ast_graft_list *
insert_pending_guard_nodes(__isl_take isl_ast_graft_list *list,
                           __isl_keep isl_ast_build *build) {
  int i, j, n_if;
  isl_size n;
  int allow_else;
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (n_if > 0) {
      ...
      for (j = n_if - 1; j >= 0; --j) {
        subset = isl_set_is_subset(test, if_node[j].guard);
        if (subset < 0 || subset) {
          found_then = j;
          break;
        }
        if (!allow_else)
          continue;
        subset = isl_set_is_subset(test, if_node[j].complement);
        if (subset < 0 || subset) {
          found_else = j;
          break;
        }
    ...
    if (found_then >= 0)
      graft->guard =
          isl_set_gist(graft->guard, isl_set_copy(if_node[found_then].guard));
    ...
    if (found_then >= 0)
      res = extend_then(res, if_node[found_then].node, graft, build);
    else if (found_else >= 0)
      res = extend_else(res, if_node[found_else].node, graft, build);
    else
 ...
 */
static __isl_give isl_ast_node_list *
extract_node_list(__isl_keep isl_ast_graft_list *list) {
  int i;
  isl_size n;
  isl_ctx *ctx;
...
isl_ast_graft_list_extract_shared_enforced(__isl_keep isl_ast_graft_list *list,
                                           __isl_keep isl_ast_build *build) {
  int i;
  isl_size n;
  int depth;
...
static __isl_give isl_ast_graft *store_guard(__isl_take isl_ast_graft *graft,
  ...
  graft->guard = isl_set_gist(graft->guard, isl_ast_build_get_generated(build));
 ...
 */
static __isl_give isl_ast_graft_list *
gist_guards(__isl_take isl_ast_graft_list *list, __isl_keep isl_set *context) {
  int i;
  isl_size n;
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (!graft)
      break;
    graft->guard = isl_set_gist(graft->guard, isl_set_copy(context));
    if (!graft->guard)
      graft = isl_ast_graft_free(graft);
    list = isl_ast_graft_list_set_ast_graft(list, i, graft);
  }
 ...
 * respect to "context".
 */
__isl_give isl_ast_graft_list *
isl_ast_graft_list_gist_guards(__isl_take isl_ast_graft_list *list,
...
__isl_give isl_ast_graft *isl_ast_graft_alloc_from_children(
  ...
  guard_build =
      isl_ast_build_replace_pending_by_guard(guard_build, isl_set_copy(guard));
 ...
 */
static __isl_give isl_ast_graft *
ast_graft_list_fuse(__isl_take isl_ast_graft_list *list,
                    __isl_keep isl_ast_build *build) {
  isl_ast_graft *graft;
  isl_basic_set *enforced;
  isl_set *guard;
  ...
  graft =
      isl_ast_graft_alloc_from_children(list, guard, enforced, build, build);
 ...
 */
__isl_give isl_ast_graft_list *
isl_ast_graft_list_fuse(__isl_take isl_ast_graft_list *list,
                        __isl_keep isl_ast_build *build) {
  isl_size n;
  isl_ast_graft *graft;
 ...
 */
static __isl_give isl_ast_graft *
isl_ast_graft_fuse(__isl_take isl_ast_graft *graft1,
 ...
 */
__isl_give isl_ast_graft *
isl_ast_graft_insert_for(__isl_take isl_ast_graft *graft,
                         __isl_take isl_ast_node *node) {
  if (!graft)
    goto error;
 ...
 */
__isl_give isl_ast_graft *
isl_ast_graft_insert_mark(__isl_take isl_ast_graft *graft,
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_from_graft_list(__isl_take isl_ast_graft_list *list,
                             __isl_keep isl_ast_build *build) {
  isl_ast_node_list *node_list;
 ...
 */
__isl_give isl_ast_graft *
isl_ast_graft_enforce(__isl_take isl_ast_graft *graft,
                      __isl_take isl_basic_set *enforced) {
  if (!graft || !enforced)
    goto error;

  enforced = isl_basic_set_align_params(
      enforced, isl_basic_set_get_space(graft->enforced));
  graft->enforced = isl_basic_set_align_params(
      graft->enforced, isl_basic_set_get_space(enforced));
...
__isl_give isl_basic_set *
isl_ast_graft_get_enforced(__isl_keep isl_ast_graft *graft) {
 ...
 */
__isl_give isl_ast_graft *
isl_ast_graft_add_guard(__isl_take isl_ast_graft *graft,
                        __isl_take isl_set *guard,
                        __isl_keep isl_ast_build *build) {
 ...
 */
__isl_give isl_ast_graft_list *
isl_ast_graft_list_unembed(__isl_take isl_ast_graft_list *list, int product) {
  int i;
  isl_size n;
 ...
 */
__isl_give isl_ast_graft *
isl_ast_graft_preimage_multi_aff(__isl_take isl_ast_graft *graft,
  ...
  enforced = graft->enforced;
  graft->enforced =
      isl_basic_set_preimage_multi_aff(enforced, isl_multi_aff_copy(ma));
 ...
 */
__isl_give isl_ast_graft_list *
isl_ast_graft_list_preimage_multi_aff(__isl_take isl_ast_graft_list *list,
                                      __isl_take isl_multi_aff *ma) {
  int i;
  isl_size n;
  ...
  for (i = 0; i < n; ++i) {
    ...
    graft = isl_ast_graft_list_get_ast_graft(list, i);
    graft = isl_ast_graft_preimage_multi_aff(graft, isl_multi_aff_copy(ma));
    list = isl_ast_graft_list_set_ast_graft(list, i, graft);
  }
 ...
 */
__isl_give isl_ast_graft_list *
isl_ast_graft_list_sort_guard(__isl_take isl_ast_graft_list *list) {
 ...
 */
__isl_give isl_ast_graft_list *
isl_ast_graft_list_merge(__isl_take isl_ast_graft_list *list1,
  ...
  for (i = 0; i < list2->n; ++i) {
    ...
    for (j = list1->n; j >= 0; --j) {
      ...
      if (j == first)
        ...
        cmp = isl_set_plain_cmp(list1->p[j - 1]->guard, graft->guard);
      if (cmp > 0) {
        disjoint = isl_set_is_disjoint(graft->guard, list1->p[j - 1]->guard);
        if (disjoint < 0) {
          isl_ast_graft_free(graft);
      ...
      if (cmp > 0)
        continue;
      if (cmp < 0) {
        list1 = isl_ast_graft_list_insert(list1, j, graft);
        break;
      }
      ...
      list1 = isl_ast_graft_list_set_ast_graft(list1, j, graft_j);
      break;
    }
 ...
 */
static __isl_give isl_set_to_ast_graft_list *
split_on_guard(__isl_keep isl_ast_graft_list *list, int *any_match) {
 ...
 */
static __isl_give isl_ast_graft_list *
reconstruct(__isl_take isl_ast_graft_list *list,
            __isl_keep isl_set_to_ast_graft_list *guard2list,
            __isl_keep isl_ast_build *build) {
  list = isl_ast_graft_list_clear(list);
  if (isl_set_to_ast_graft_list_foreach(guard2list, &add_same_guard, &list) < 0)
    list = isl_ast_graft_list_free(list);
 ...
 */
__isl_give isl_ast_graft_list *
isl_ast_graft_list_group_on_guard(__isl_take isl_ast_graft_list *list,
                                  __isl_keep isl_ast_build *build) {
  int any_match = 0;
  isl_size n;
  isl_set_to_ast_graft_list *guard2list;
...
isl_printer_print_ast_graft(__isl_take isl_printer *p,
