...
public:
    ...
    void Unparse(const Initialization &x) { // R743 & R805
        std::visit(common::visitors{
            [&](const ConstantExpr &y) {
                Put(" = "), Walk(y);
            },
            [&](const NullInit &y) {
                Put(" => "), Walk(y);
            },
            [&](const InitialDataTarget &y) {
                Put(" => "), Walk(y);
            },
    ...
    void Unparse(const CoarraySpec &x) { // R809
        std::visit(common::visitors{
            [&](const DeferredCoshapeSpecList &y) {
                Walk(y);
            },
            [&](const ExplicitCoshapeSpec &y) {
                Walk(y);
            },
        },
        x.u);
...
#undef FMT
    ...
    void Unparse(const UseStmt &x) { // R1409
        ...
        std::visit(common::visitors{
            [&](const std::list<Rename> &y) {
                Walk(", ", y, ", ");
            },
            [&](const std::list<Only> &y) {
                Walk(", ONLY: ", y, ", ");
            },
        },
        x.u);
...
#include "llvm/Frontend/OpenACC/ACC.cpp.inc"
    ...
    void Unparse(const OmpMapType::Always &) {
        Word("ALWAYS,");
    }
...
private:
    ...
    void BeginOpenMP() {
        openmpDirective_ = true;
    }
    void EndOpenMP() {
        openmpDirective_ = false;
    }
    void BeginOpenACC() {
        openaccDirective_ = true;
    }
    void EndOpenACC() {
        openaccDirective_ = false;
    }
