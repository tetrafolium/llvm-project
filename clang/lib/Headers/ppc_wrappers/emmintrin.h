...
extern __inline __m128i
  ...
  __asm__("xvcvdpsxws %x0,%x1" : "=wa"(temp) : "wa"(rounded) :);

#ifdef _ARCH_PWR8
  temp = vec_mergeo(temp, temp);
  result =
      (__v4si)vec_vpkudum((__vector long long)temp, (__vector long long)vzero);
#else
  {
...
extern __inline __m128
  ...
  __asm__("xvcvdpsp %x0,%x1" : "=wa"(temp) : "wa"(__A) :);

#ifdef _ARCH_PWR8
  temp = vec_mergeo(temp, temp);
  result =
      (__v4sf)vec_vpkudum((__vector long long)temp, (__vector long long)vzero);
#else
  {
    const __v16qu pkperm = {0x00, 0x01, 0x02, 0x03, 0x08, 0x09, 0x0a, 0x0b,
                            0x14, 0x15, 0x16, 0x17, 0x1c, 0x1d, 0x1e, 0x1f};
...
extern __inline __m128i
  ...
  /* VSX Vector truncate Double-Precision to integer and Convert to
   Signed Integer Word format with Saturate.  */
  __asm__("xvcvdpsxws %x0,%x1" : "=wa"(temp) : "wa"(__A) :);
...
extern __inline __m128d
  ...
  temp = vec_vmrghw(a, a);
#endif
  __asm__(" xvcvspdp %x0,%x1" : "=wa"(result) : "wa"(temp) :);
  return (__m128d)result;
#endif
}
...
extern __inline __m128
  ...
  __asm__("xscvdpsp %x0,%x1" : "=wa"(temp_s) : "wa"(temp_b) :);
...
extern __inline __m128d
  ...
  __asm__("xscvspdp %x0,%x1" : "=wa"(res) : "wa"(temp) :);
...
extern __inline int
  ...
  result = ((__vector unsigned long long)vec_vbpermq(
      (__vector unsigned char)__A, (__vector unsigned char)perm_mask));
...
extern __inline __m128i
  ...
  __asm__("vmulouw %0,%1,%2" : "=v"(result) : "v"(__A), "v"(__B) :);
#else
