...
public:
    ...
    auto getArguments() const {
        return llvm::make_pointee_range(arguments);
    }
...
public:
    ...
    Element *getInputs() const {
        return inputs.get();
    }
    Element *getResults() const {
        return results.get();
    }
...
public:
    ...
    auto getElements() const {
        return llvm::make_pointee_range(elements);
    }

    /// Return the anchor of this optional group.
    Element *getAnchor() const {
        return elements[anchor].get();
    }

    /// Return the index of the first element that needs to be parsed.
    unsigned getParseStart() const {
        return parseStart;
    }
...
struct OperationFormat {
    ...
    public:
        ...
        Optional<int> getBuilderIdx() const {
            return builderIdx;
        }
        void setBuilderIdx(int idx) {
            builderIdx = idx;
        }
...
void OperationFormat::genParserRegionResolution(Operator &op,
    ...
    elements, [](auto &elt) {
        return isa<RegionsDirective>(elt.get());
    });
    if (hasAllRegions) {
...
void OperationFormat::genParserSuccessorResolution(Operator &op,
    ...
    elements, [](auto &elt) {
        return isa<SuccessorsDirective>(elt.get());
    });
    if (hasAllSuccessors) {
...
static auto findArg(RangeT &&range, StringRef name) {
    auto it = llvm::find_if(range, [=](auto &arg) {
