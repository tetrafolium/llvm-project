...
void AssocEntityDetails::set_rank(int rank) {
    rank_ = rank;
}
...
void Symbol::SetType(const DeclTypeSpec &type) {
    std::visit(common::visitors{
        ...
        [&](AssocEntityDetails &x) {
            x.set_type(type);
        },
        [&](ProcEntityDetails &x) {
            x.interface().set_type(type);
        },
        [&](TypeParamDetails &x) {
            x.set_type(type);
        },
