...
private:
  ...
  bool X86FastEmitStore(EVT VT, unsigned ValReg, bool ValIsKill,
                        X86AddressMode &AM, MachineMemOperand *MMO = nullptr,
                        bool Aligned = false);
  ...
  bool TryEmitSmallMemcpy(X86AddressMode DestAM, X86AddressMode SrcAM,
                          uint64_t Len);
...
X86FastISel::addFullAddress(const MachineInstrBuilder &MIB,
  ...
  AM.IndexReg = constrainOperandRegClass(
      MIB->getDesc(), AM.IndexReg, MIB->getNumOperands() + X86::AddrIndexReg);
...
bool X86FastISel::foldX86XALUIntrinsic(X86::CondCode &CC, const Instruction *I,
  ...
  Type *RetTy = cast<StructType>(Callee->getReturnType())->getTypeAtIndex(0U);
...
bool X86FastISel::X86FastEmitLoad(MVT VT, X86AddressMode &AM,
  ...
  case MVT::v2f64:
    if (IsNonTemporal && Alignment >= 16 && HasSSE41)
      ...
      Opc =
          HasVLX ? X86::VMOVAPDZ128rm : HasAVX ? X86::VMOVAPDrm : X86::MOVAPDrm;
    else
      Opc =
          HasVLX ? X86::VMOVUPDZ128rm : HasAVX ? X86::VMOVUPDrm : X86::MOVUPDrm;
    break;
  case MVT::v4i32:
  case MVT::v2i64:
  case MVT::v8i16:
...
bool X86FastISel::X86FastEmitStore(EVT VT, unsigned ValReg, bool ValIsKill,
                                   X86AddressMode &AM, MachineMemOperand *MMO,
                                   bool Aligned) {
  ...
  case MVT::i1: {
    ...
    BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(X86::AND8ri),
            AndResult)
  ...
  case MVT::v2f64:
    if (Aligned) {
      ...
      Opc =
          HasVLX ? X86::VMOVUPDZ128mr : HasAVX ? X86::VMOVUPDmr : X86::MOVUPDmr;
    break;
  case MVT::v4i32:
  case MVT::v2i64:
  case MVT::v8i16:
...
bool X86FastISel::X86FastEmitStore(EVT VT, const Value *Val, X86AddressMode &AM,
...
/// ISD::SIGN_EXTEND).
bool X86FastISel::X86FastEmitExtend(ISD::NodeType Opc, EVT DstVT, unsigned Src,
  ...
  if (const GlobalValue *GV = dyn_cast<GlobalValue>(V)) {
    ...
    if (!Subtarget->isPICStyleRIPRel() ||
      ...
      if (I != LocalValueMap.end() && I->second) {
        ...
        MachineInstrBuilder LoadMI = BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt,
                                             DbgLoc, TII.get(Opc), LoadReg);
  ...
  case Instruction::GetElementPtr: {
    ...
    for (User::const_op_iterator i = U->op_begin() + 1, e = U->op_end(); i != e;
      ...
      for (;;) {
        ...
        if (IndexReg == 0 && (!AM.GV || !Subtarget->isPICStyleRIPRel()) &&
            (S == 1 || S == 2 || S == 4 || S == 8)) {
...
bool X86FastISel::X86SelectCallAddress(const Value *V, X86AddressMode &AM) {
  ...
  case Instruction::IntToPtr:
    ...
    if (InMBB && TLI.getValueType(DL, U->getOperand(0)->getType()) ==
                     TLI.getPointerTy(DL))
      return X86SelectCallAddress(U->getOperand(0), AM);
    break;
  ...
  if (const GlobalValue *GV = dyn_cast<GlobalValue>(V)) {
    ...
    if (Subtarget->isPICStyleRIPRel() && (AM.Base.Reg != 0 || AM.IndexReg != 0))
      return false;
...
bool X86FastISel::X86SelectRet(const Instruction *I) {
  ...
  if (CC != CallingConv::C && CC != CallingConv::Fast &&
      CC != CallingConv::Tail && CC != CallingConv::X86_FastCall &&
      CC != CallingConv::X86_StdCall && CC != CallingConv::X86_ThisCall &&
      CC != CallingConv::X86_64_SysV && CC != CallingConv::Win64)
    return false;
...
bool X86FastISel::X86SelectZExt(const Instruction *I) {
  ...
  if (DstVT == MVT::i64) {
    ...
    case MVT::i8:
      MovInst = X86::MOVZX32rr8;
      break;
    case MVT::i16:
      MovInst = X86::MOVZX32rr16;
      break;
    case MVT::i32:
      MovInst = X86::MOV32rr;
      break;
    ...
    BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(MovInst),
            Result32)
        .addReg(ResultReg);
  ...
  if (const CmpInst *CI = dyn_cast<CmpInst>(BI->getCondition())) {
    ...
    if (TI->hasOneUse() && TI->getParent() == I->getParent() &&
      ...
      case MVT::i8:
        ...
        break;
      case MVT::i16:
        TestOpc = X86::TEST16ri;
        break;
      case MVT::i32:
        TestOpc = X86::TEST32ri;
        break;
      case MVT::i64:
        TestOpc = X86::TEST64ri32;
        break;
      }
      if (TestOpc) {
...
bool X86FastISel::X86SelectDivRem(const Instruction *I) {
  ...
  BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(OpEntry.OpCopy),
          TypeEntry.LowInReg)
      .addReg(Op0Reg);
  ...
  if (OpEntry.OpSignExtend) {
    if (OpEntry.IsOpSigned)
      ...
      if (VT == MVT::i16) {
        BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(Copy),
                TypeEntry.HighInReg)
  ...
  BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(OpEntry.OpDivRem))
      .addReg(Op1Reg);
...
bool X86FastISel::X86FastEmitSSESelect(MVT RetVT, const Instruction *I) {
  ...
  if (Subtarget->hasAVX512()) {
    ...
    unsigned CmpOpcode = (RetVT == MVT::f32) ? X86::VCMPSSZrr : X86::VCMPSDZrr;
    ...
    unsigned MovReg =
        fastEmitInst_rrrr(MovOpcode, VR128X, RHSReg, RHSIsKill, CmpReg, true,
                          ImplicitDefReg, true, LHSReg, LHSIsKill);
    ...
    // don't bother.
    unsigned CmpOpcode = (RetVT == MVT::f32) ? X86::VCMPSSrr : X86::VCMPSDrr;
...
bool X86FastISel::X86FastEmitPseudoSelect(MVT RetVT, const Instruction *I) {
  ...
  case MVT::i32:
    ...
    break;
  case MVT::f32:
    Opc = Subtarget->hasAVX512() ? X86::CMOV_FR32X : X86::CMOV_FR32;
    break;
  case MVT::f64:
    Opc = Subtarget->hasAVX512() ? X86::CMOV_FR64X : X86::CMOV_FR64;
    break;
  }
...
bool X86FastISel::X86SelectTrunc(const Instruction *I) {
  ...
  Register ResultReg =
      fastEmitInst_extractsubreg(MVT::i8, InputReg, false, X86::sub_8bit);
  if (!ResultReg)
    return false;
...
bool X86FastISel::fastLowerIntrinsicCall(const IntrinsicInst *II) {
  ...
  case Intrinsic::convert_to_fp16: {
    ...
    if (IsFloatToHalf) {
      ...
      unsigned Opc =
          Subtarget->hasVLX() ? X86::VCVTPH2PSZ128rr : X86::VCVTPH2PSrr;
  ...
  case Intrinsic::frameaddress: {
    ...
    switch (VT.SimpleTy) {
    default:
      llvm_unreachable("Invalid result type for frameaddress.");
    case MVT::i32:
      Opc = X86::MOV32rm;
      RC = &X86::GR32RegClass;
      break;
  ...
  case Intrinsic::stackprotector: {
    ...
    X86AddressMode AM;
    if (!X86SelectAddress(Slot, AM))
      return false;
    if (!X86FastEmitStore(PtrTy, Op1, AM))
      return false;
    return true;
  }
  ...
  case Intrinsic::sqrt: {
    ...
    unsigned AVXLevel =
        Subtarget->hasAVX512() ? 2 : Subtarget->hasAVX() ? 1 : 0;
    unsigned Opc;
    switch (VT.SimpleTy) {
    default:
      return false;
  ...
  case Intrinsic::umul_with_overflow: {
    ...
    if (const auto *CI = dyn_cast<ConstantInt>(RHS)) {
      ...
      if (CI->isOne() && (BaseOpc == ISD::ADD || BaseOpc == ISD::SUB) &&
        ...
        ResultReg =
            fastEmit_ri(VT, VT, BaseOpc, LHSReg, LHSIsKill, CI->getZExtValue());
    }
    ...
    if (!ResultReg) {
      ...
      ResultReg =
          fastEmit_rr(VT, VT, BaseOpc, LHSReg, LHSIsKill, RHSReg, RHSIsKill);
    }

    // FastISel doesn't have a pattern for all X86::MUL*r and X86::IMUL*r. Emit
    // it manually.
    if (BaseOpc == X86ISD::UMUL && !ResultReg) {
      static const uint16_t MULOpc[] = {X86::MUL8r, X86::MUL16r, X86::MUL32r,
                                        X86::MUL64r};
...
bool X86FastISel::fastLowerArguments() {
  ...
  static const MCPhysReg GPR32ArgRegs[] = {X86::EDI, X86::ESI, X86::EDX,
                                           X86::ECX, X86::R8D, X86::R9D};
  static const MCPhysReg GPR64ArgRegs[] = {X86::RDI, X86::RSI, X86::RDX,
  ...
  for (auto const &Arg : F->args()) {
    ...
    case MVT::i32:
      SrcReg = GPR32ArgRegs[GPRIdx++];
      break;
    case MVT::i64:
      SrcReg = GPR64ArgRegs[GPRIdx++];
      break;
    case MVT::f32:
      LLVM_FALLTHROUGH;
    case MVT::f64:
      SrcReg = XMMArgRegs[FPRIdx++];
      break;
    }
...
bool X86FastISel::fastLowerCall(CallLoweringInfo &CLI) {
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc,
              TII.get(TargetOpcode::COPY), VA.getLocReg())
          .addReg(ArgReg);
  ...
  for (unsigned i = 0; i != RVLocs.size(); ++i) {
    ...
    // Copy out the result.
    BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc,
            TII.get(TargetOpcode::COPY), CopyReg)
        .addReg(SrcReg);
    ...
    if (CopyVT != VA.getValVT()) {
      ...
      addFrameReference(
          BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(Opc)), FI)
          .addReg(CopyReg);
      Opc = ResVT == MVT::f32 ? X86::MOVSSrm_alt : X86::MOVSDrm_alt;
      addFrameReference(BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc,
                                TII.get(Opc), ResultReg + i),
                        FI);
    }
  }
...
bool X86FastISel::fastSelectInstruction(const Instruction *I) {
  switch (I->getOpcode()) {
  default:
    break;
...
unsigned X86FastISel::X86MaterializeGV(const GlobalValue *GV, MVT VT) {
  ...
  if (X86SelectAddress(GV, AM)) {
    ...
    // to emit an LEA.
    if (AM.BaseType == X86AddressMode::RegBase && AM.IndexReg == 0 &&
        AM.Disp == 0 && AM.GV == nullptr)
      return AM.Base.Reg;
...
unsigned X86FastISel::fastMaterializeAlloca(const AllocaInst *C) {
  ...
  Register ResultReg = createResultReg(RC);
  addFullAddress(BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(Opc),
                         ResultReg),
                 AM);
  return ResultReg;
}
...
bool X86FastISel::tryToFoldLoadIntoMI(MachineInstr *MI, unsigned OpNo,
  ...
  for (MachineInstr::mop_iterator I = Result->operands_begin(),
    ...
    Register IndexReg =
        constrainOperandRegClass(Result->getDesc(), MO.getReg(), OperandNo);
