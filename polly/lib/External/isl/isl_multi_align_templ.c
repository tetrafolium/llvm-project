...
static __isl_give MULTI(BASE) *
    FN(FN(MULTI(BASE), align_params), ALIGN_DOMBASE)(
        __isl_take MULTI(BASE) * multi, __isl_take ALIGN_DOM *domain,
        __isl_give MULTI(BASE) *
            fn(__isl_take MULTI(BASE) * multi, __isl_take ALIGN_DOM *domain)) {
  isl_bool aligned;
  isl_bool named;
  isl_space *dom_space;
  ...
  multi =
      FN(MULTI(BASE), align_params)(multi, FN(ALIGN_DOM, get_space)(domain));
  domain =
      FN(ALIGN_DOM, align_params)(domain, FN(MULTI(BASE), get_space)(multi));
  return fn(multi, domain);
error:
