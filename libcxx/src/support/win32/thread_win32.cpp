...
int __libcpp_condvar_wait(__libcpp_condvar_t* __cv, __libcpp_mutex_t* __m) {
  SleepConditionVariableSRW((PCONDITION_VARIABLE)__cv, (PSRWLOCK)__m, INFINITE,
                            0);
  return 0;
}
...
int __libcpp_condvar_timedwait(__libcpp_condvar_t* __cv, __libcpp_mutex_t* __m,
  ...
  if (!SleepConditionVariableSRW(
          (PCONDITION_VARIABLE)__cv, (PSRWLOCK)__m,
          timeout_ms.count() > 0 ? timeout_ms.count() : 0, 0)) {
...
int __libcpp_execute_once(__libcpp_exec_once_flag* __flag,
  ...
  if (!InitOnceExecuteOnce((PINIT_ONCE)__flag,
...
bool __libcpp_thread_id_less(__libcpp_thread_id __lhs,
                             __libcpp_thread_id __rhs) {
  return __lhs < __rhs;
}
...
int __libcpp_thread_create(__libcpp_thread_t* __t, void* (*__func)(void*),
  ...
  *__t = reinterpret_cast<HANDLE>(_beginthreadex(
      nullptr, 0, __libcpp_beginthreadex_thunk, __data, 0, nullptr));
