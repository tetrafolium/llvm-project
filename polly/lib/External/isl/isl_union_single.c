...
static isl_bool FN(UNION, has_same_domain_space)(const void *entry,
  ...
  return isl_space_tuple_is_equal(part->dim, isl_dim_in, space, isl_dim_in);
}
...
FN(UNION, find_part_entry)(__isl_keep UNION *u, __isl_keep isl_space *space,
  ...
  entry = isl_hash_table_find(
      ctx, &u->table, hash, &FN(UNION, has_same_domain_space), space, reserve);
  if (!entry || entry == isl_hash_table_entry_none)
    return entry;
  if (reserve && !entry->data)
    return entry;
  part = entry->data;
  equal = isl_space_tuple_is_equal(part->dim, isl_dim_out, space, isl_dim_out);
  if (equal < 0)
    return NULL;
  if (equal)
    return entry;
  ...
  isl_die(FN(UNION, get_ctx)(u), isl_error_invalid,
          ...
          "expression over a given domain",
          return NULL);
}
