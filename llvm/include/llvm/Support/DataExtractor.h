...
public:
    ...
    uint64_t getAddress(Cursor &C) const {
        return getUnsigned(C, AddressSize);
    }
    ...
    int64_t getSLEB128(Cursor &C) const {
        return getSLEB128(&C.Offset, &C.Err);
    }
    ...
    uint64_t getULEB128(Cursor &C) const {
        return getULEB128(&C.Offset, &C.Err);
    }
