...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), scale_val)(__isl_take MULTI(BASE) * multi,
  ...
  for (i = 0; i < multi->n; ++i) {
    multi->u.p[i] = FN(EL, scale_val)(multi->u.p[i], isl_val_copy(v));
    if (!multi->u.p[i])
      goto error;
  }
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), scale_down_val)(__isl_take MULTI(BASE) * multi,
                                    __isl_take isl_val *v) {
  int i;
  ...
  for (i = 0; i < multi->n; ++i) {
    multi->u.p[i] = FN(EL, scale_down_val)(multi->u.p[i], isl_val_copy(v));
    if (!multi->u.p[i])
      goto error;
  }
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), scale_multi_val)(__isl_take MULTI(BASE) * multi,
  ...
  if (!isl_space_tuple_is_equal(multi->space, isl_dim_out, mv->space,
                                isl_dim_set))
    isl_die(isl_multi_val_get_ctx(mv), isl_error_invalid, "spaces don't match",
            goto error);
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), scale_down_multi_val)(__isl_take MULTI(BASE) * multi,
                                          __isl_take isl_multi_val *mv) {
  int i;
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), mod_multi_val)(__isl_take MULTI(BASE) * multi,
                                   __isl_take isl_multi_val *mv) {
  int i;
