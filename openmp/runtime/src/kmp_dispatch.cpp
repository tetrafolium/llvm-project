  ...
  case kmp_sch_guided_analytical_chunked: {
    ...
    while (1) {
      ...
      if (chunkIdx >= (UT)pr->u.p.parm2) {
        ...
        /* The following check is to workaround the lack of long double
           precision on Windows* OS. This check works around the possible effect
           that init != 0 for chunkIdx == 0.
         */
#if KMP_USE_X87CONTROL
