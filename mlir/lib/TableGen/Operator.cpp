...
auto Operator::result_begin() -> value_iterator {
    return results.begin();
}

auto Operator::result_end() -> value_iterator {
    return results.end();
}
...
auto Operator::operand_begin() -> value_iterator {
    return operands.begin();
}
auto Operator::operand_end() -> value_iterator {
    return operands.end();
}
...
bool Operator::isVariadic() const {
    ...
    [](const NamedTypeConstraint &op) {
        return op.isVariadic();
    });
}
...
ArrayRef<llvm::SMLoc> Operator::getLoc() const {
    return def.getLoc();
}
