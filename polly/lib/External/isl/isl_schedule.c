...
isl_schedule_from_schedule_tree(isl_ctx *ctx,
  ...
  if (type != isl_schedule_node_domain && type != isl_schedule_node_extension)
 ...
 */
__isl_give isl_schedule *
isl_schedule_set_root(__isl_take isl_schedule *schedule,
                      __isl_take isl_schedule_tree *tree) {
  if (!schedule || !tree)
    goto error;
...
isl_schedule_peek_leaf(__isl_keep isl_schedule *schedule) {
...
isl_bool isl_schedule_plain_is_equal(__isl_keep isl_schedule *schedule1,
  ...
  return isl_schedule_tree_plain_is_equal(schedule1->root, schedule2->root);
}
...
isl_schedule_get_space(__isl_keep isl_schedule *schedule) {
  enum isl_schedule_node_type type;
  isl_space *space;
...
reset_user(__isl_take isl_schedule_node *node, void *user) {
...
isl_schedule_reset_user(__isl_take isl_schedule *schedule) {
  return isl_schedule_map_schedule_node_bottom_up(schedule, &reset_user, NULL);
}
 ...
 */
static __isl_give isl_schedule_node *
align_params(__isl_take isl_schedule_node *node, void *user) {
...
isl_schedule_align_params(__isl_take isl_schedule *schedule,
  ...
  schedule =
      isl_schedule_map_schedule_node_bottom_up(schedule, &align_params, space);
 ...
 */
static __isl_give isl_schedule_node *
pullback_upma(__isl_take isl_schedule_node *node, void *user) {
  isl_union_pw_multi_aff *upma = user;

  return isl_schedule_node_pullback_union_pw_multi_aff(
      node, isl_union_pw_multi_aff_copy(upma));
}
...
__isl_give isl_schedule *isl_schedule_pullback_union_pw_multi_aff(
  ...
  schedule =
      isl_schedule_map_schedule_node_bottom_up(schedule, &pullback_upma, upma);
...
insert_filter_in_child_of_type(__isl_take isl_schedule_tree *tree,
...
__isl_give isl_schedule *isl_schedule_pair(enum isl_schedule_node_type type,
  ...
  if (!disjoint)
    isl_die(ctx, isl_error_invalid, "schedule domains not disjoint",
            filter1 = isl_union_set_free(filter1));
...
isl_schedule_sequence(__isl_take isl_schedule *schedule1,
                      __isl_take isl_schedule *schedule2) {
  return isl_schedule_pair(isl_schedule_node_sequence, schedule1, schedule2);
}
