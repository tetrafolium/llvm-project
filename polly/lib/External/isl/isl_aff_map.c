...
isl_basic_map_from_aff2(__isl_take isl_aff *aff, int rational) {
  int k;
  int pos;
  isl_bool is_nan;
  isl_local_space *ls;
  ...
  if (is_nan < 0)
    goto error;
  if (is_nan)
    isl_die(isl_aff_get_ctx(aff), isl_error_invalid, "cannot convert NaN",
            goto error);
...
isl_basic_map_from_multi_aff2(__isl_take isl_multi_aff *maff, int rational) {
  ...
  if (dim != maff->n)
    isl_die(isl_multi_aff_get_ctx(maff), isl_error_internal, "invalid space",
            goto error);
 ...
 */
static __isl_give isl_basic_map *
basic_map_from_multi_aff(__isl_take isl_multi_aff *ma) {
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_from_multi_aff(__isl_take isl_multi_aff *ma) {
 ...
 */
__isl_give isl_map *
isl_map_from_multi_aff_internal(__isl_take isl_multi_aff *maff) {
  isl_basic_map *bmap;
...
__isl_give isl_map *isl_map_from_pw_aff_internal(__isl_take isl_pw_aff *pwaff) {
  ...
  for (i = 0; i < pwaff->n; ++i) {
    ...
    map_i = isl_map_intersect_domain(map_i, isl_set_copy(pwaff->p[i].set));
...
isl_map_from_pw_multi_aff_internal(__isl_take isl_pw_multi_aff *pma) {
  ...
  for (i = 0; i < pma->n; ++i) {
    ...
    map_i = isl_map_from_basic_map(bmap);
    map_i = isl_map_intersect_domain(map_i, isl_set_copy(pma->p[i].set));
    map = isl_map_union_disjoint(map, map_i);
  }
 ...
 */
__isl_give isl_map *
isl_map_from_pw_multi_aff(__isl_take isl_pw_multi_aff *pma) {
...
__isl_give isl_set *
isl_set_from_pw_multi_aff(__isl_take isl_pw_multi_aff *pma) {
...
map_from_multi_pw_aff(__isl_take isl_multi_pw_aff *mpa) {
  int i;
  isl_size dim;
  isl_space *space;
  isl_map *map;
  ...
  if (isl_space_dim(mpa->space, isl_dim_out) != mpa->n)
    isl_die(isl_multi_pw_aff_get_ctx(mpa), isl_error_internal, "invalid space",
            goto error);
 ...
 */
__isl_give isl_map *
isl_map_from_multi_pw_aff(__isl_take isl_multi_pw_aff *mpa) {
 ...
 */
__isl_give isl_set *
isl_set_from_multi_pw_aff(__isl_take isl_multi_pw_aff *mpa) {
...
isl_union_map_from_union_pw_aff(__isl_take isl_union_pw_aff *upa) {
  ...
  if (isl_union_pw_aff_foreach_pw_aff(upa, &map_from_pw_aff_entry, &umap) < 0)
...
isl_union_map_from_union_pw_multi_aff(__isl_take isl_union_pw_multi_aff *upma) {
  ...
  if (isl_union_pw_multi_aff_foreach_pw_multi_aff(upma, &map_from_pw_multi_aff,
                                                  &umap) < 0)
    goto error;
