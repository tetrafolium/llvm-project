...
static int parse_remapped_files_with_opt(const char *opt_name, int argc,
                                         const char **argv, int start_arg,
  ...
  *unsaved_files = (struct CXUnsavedFile *)malloc(sizeof(struct CXUnsavedFile) *
                                                  *num_unsaved_files);
  ...
  for (i = 0; i != *num_unsaved_files; ++i) {
    ...
    if (!sep) {
      fprintf(stderr, "error: %sfrom:to argument is missing comma\n", opt_name);
...
static int parse_remapped_files_with_try(int try_idx, int argc,
                                         const char **argv, int start_arg,
                                         struct CXUnsavedFile **unsaved_files,
                                         int *num_unsaved_files) {
  ...
  ret = parse_remapped_files(argc, argv, start_arg, &unsaved_files_no_try_idx,
  ...
  *unsaved_files = (struct CXUnsavedFile *)realloc(
      unsaved_files_no_try_idx,
      sizeof(struct CXUnsavedFile) * *num_unsaved_files);
  assert(*unsaved_files);
  memcpy(*unsaved_files + num_unsaved_files_no_try_idx, unsaved_files_try_idx,
         sizeof(struct CXUnsavedFile) * num_unsaved_files_try_idx);
...
static void PrintRange(CXSourceRange R, const char *str) {
  ...
  clang_getSpellingLocation(clang_getRangeStart(R), &begin_file, &begin_line,
                            &begin_column, 0);
  clang_getSpellingLocation(clang_getRangeEnd(R), &end_file, &end_line,
                            &end_column, 0);
...
static void DumpCXCommentInternal(struct CommentASTDumpingContext *Ctx,
  ...
  case CXComment_InlineCommand:
    ...
    PrintCXStringWithPrefixAndDispose(
        "CommandName", clang_InlineCommandComment_getCommandName(Comment));
    ...
    for (i = 0, e = clang_InlineCommandComment_getNumArgs(Comment); i != e;
         ++i) {
      printf(" Arg[%u]=", i);
  ...
  case CXComment_HTMLStartTag: {
    ...
    PrintCXStringWithPrefixAndDispose("Name",
  ...
  case CXComment_BlockCommand:
    ...
    PrintCXStringWithPrefixAndDispose(
        "CommandName", clang_BlockCommandComment_getCommandName(Comment));
    for (i = 0, e = clang_BlockCommandComment_getNumArgs(Comment); i != e;
         ++i) {
      printf(" Arg[%u]=", i);
      PrintCXStringAndDispose(clang_BlockCommandComment_getArgText(Comment, i));
    }
    break;
  case CXComment_ParamCommand:
    ...
    if (clang_ParamCommandComment_isDirectionExplicit(Comment))
      ...
      printf(" implicitly");
    PrintCXStringWithPrefixAndDispose(
        "ParamName", clang_ParamCommandComment_getParamName(Comment));
  ...
  case CXComment_TParamCommand:
    ...
    PrintCXStringWithPrefixAndDispose(
        "ParamName", clang_TParamCommandComment_getParamName(Comment));
    if (clang_TParamCommandComment_isParamPositionValid(Comment)) {
      printf(" ParamPosition={");
      for (i = 0, e = clang_TParamCommandComment_getDepth(Comment); i != e;
           ++i) {
  ...
  case CXComment_VerbatimBlockLine:
    ...
    PrintCXStringWithPrefixAndDispose(
        "Text", clang_VerbatimBlockLineComment_getText(Comment));
    break;
  case CXComment_VerbatimLine:
    printf("CXComment_VerbatimLine");
    PrintCXStringWithPrefixAndDispose(
        "Text", clang_VerbatimLineComment_getText(Comment));
    break;
...
static void PrintCursor(CXCursor Cursor, const char *CommentSchemaFile) {
  ...
  if (clang_isInvalid(Cursor.kind)) {
    ...
    printf("%s=%s", clang_getCString(ks), clang_getCString(string));
    clang_disposeString(ks);
    ...
    NumPlatformAvailability = clang_getCursorPlatformAvailability(
        Cursor, &AlwaysDeprecated, &DeprecatedMessage, &AlwaysUnavailable,
        &UnavailableMessage, PlatformAvailability, 2);
    ...
    if (!clang_equalCursors(SpecializationOf, clang_getNullCursor())) {
      ...
      clang_getSpellingLocation(Loc, 0, &line, &column, 0);
      printf(" [Specialization of %s:%d:%d]", clang_getCString(Name), line,
             column);
      ...
      if (Cursor.kind == CXCursor_FunctionDecl) {
        ...
        for (I = 0; I < NumTemplateArgs; I++) {
          ...
          case CXTemplateArgumentKind_Type: {
            ...
            printf(" [Template arg %d: kind: %d, type: %s]", I, TAK,
                   clang_getCString(S));
            clang_disposeString(S);
          } break;
          case CXTemplateArgumentKind_Integral:
            printf(" [Template arg %d: kind: %d, intval: %lld]", I, TAK,
    ...
    RefNameRange = clang_getCursorReferenceNameRange(
        Cursor,
        CXNameRange_WantQualifier | CXNameRange_WantSinglePiece |
    ...
    for (RefNameRangeNr = 0; 1; RefNameRangeNr++) {
      RefNameRange = clang_getCursorReferenceNameRange(
          Cursor, CXNameRange_WantQualifier | CXNameRange_WantTemplateArgs,
          RefNameRangeNr);
...
void PrintDiagnostic(CXDiagnostic Diagnostic) {
  ...
  clang_getSpellingLocation(clang_getDiagnosticLocation(Diagnostic), &file, 0,
                            0, 0);
  if (!file)
    return;
  ...
  for (i = 0; i != num_fixits; ++i) {
    ...
    clang_getSpellingLocation(start, &start_file, &start_line, &start_column,
                              0);
...
enum CXChildVisitResult FilteredPrintingVisitor(CXCursor Cursor,
  ...
  if (!Data->Filter || (Cursor.kind == *(enum CXCursorKind *)Data->Filter)) {
    ...
    unsigned line, column;
    clang_getSpellingLocation(Loc, 0, &line, &column, 0);
    printf("// %s: %s:%d:%d: ", FileCheckPrefix, GetCursorSource(Cursor), line,
           column);
...
FunctionScanVisitor(CXCursor Cursor, CXCursor Parent, CXClientData ClientData) {
  const char *startBuf, *endBuf;
  unsigned startLine, startColumn, endLine, endColumn, curLine, curColumn;
  CXCursor Ref;
  VisitorData *Data = (VisitorData *)ClientData;

  if (Cursor.kind != CXCursor_FunctionDecl || !clang_isCursorDefinition(Cursor))
    return CXChildVisit_Continue;

  clang_getDefinitionSpellingAndExtent(Cursor, &startBuf, &endBuf, &startLine,
...
void InclusionVisitor(CXFile includedFile, CXSourceLocation *includeStack,
  ...
  for (i = 0; i < includeStackLen; ++i) {
    ...
    unsigned line, column;
    clang_getSpellingLocation(includeStack[i], &includingFile, &line, &column,
                              0);
...
static enum CXVisitorResult FieldVisitor(CXCursor C, CXClientData client_data) {
  (*(int *)client_data) += 1;
...
int perform_test_load_source(int argc, const char **argv, const char *filter,
  ...
  result =
      perform_test_load(Idx, TU, filter, NULL, Visitor, PV, CommentSchemaFile);
  free_remapped_files(unsaved_files, num_unsaved_files);
  clang_disposeIndex(Idx);
  return result;
}
...
int perform_test_reparse_source(int argc, const char **argv, int trials,
  ...
  Err = clang_parseTranslationUnit2(Idx, 0, argv + compiler_arg_idx,
                                    argc - compiler_arg_idx, 0, 0,
                                    getDefaultParsingOptions(), &TU);
  if (Err != CXError_Success) {
    fprintf(stderr, "Unable to load translation unit!\n");
    describeLibclangFailure(Err);
  ...
  for (trial = 0; trial < trials; ++trial) {
    free_remapped_files(unsaved_files, num_unsaved_files);
    if (parse_remapped_files_with_try(trial, argc, argv, 0, &unsaved_files,
                                      &num_unsaved_files)) {
      clang_disposeTranslationUnit(TU);
      clang_disposeIndex(Idx);
      return -1;
    ...
    Err = clang_reparseTranslationUnit(
        TU, trial >= remap_after_trial ? num_unsaved_files : 0,
...
static int perform_file_scan(const char *ast_file, const char *source_file,
  ...
  for (;;) {
    ...
    if ((c == EOF || !clang_equalCursors(cursor, prevCursor)) &&
      ...
      print_cursor_file_scan(TU, prevCursor, start_line, start_col, line, col,
                             prefix);
      start_line = line;
      start_col = col;
    }
  ...
  for (I = 0; I != N; ++I) {
    ...
    if (Kind == CXCompletionChunk_Optional) {
      ...
      print_completion_string(
          clang_getCompletionChunkCompletionString(completion_string, I), file);
      fprintf(file, "}");
      continue;
    }
...
static void print_completion_result(CXTranslationUnit translation_unit,
  ...
  switch (
      clang_getCompletionAvailability(completion_result->CompletionString)) {
  case CXAvailability_Available:
    break;
  ...
  annotationCount =
      clang_getCompletionNumAnnotations(completion_result->CompletionString);
  if (annotationCount) {
    unsigned i;
    fprintf(file, " (");
  ...
  BriefComment =
      clang_getCompletionBriefComment(completion_result->CompletionString);
  BriefCommentCString = clang_getCString(BriefComment);
...
int perform_code_completion(int argc, const char **argv, int timing_only) {
  ...
  if ((errorCode =
           parse_file_line_column(input, &filename, &line, &column, 0, 0)))
    return errorCode;
  ...
  Err = clang_parseTranslationUnit2(CIdx, 0, argv + num_unsaved_files + 2,
                                    argc - num_unsaved_files - 2, 0, 0,
                                    getDefaultParsingOptions(), &TU);
  if (Err != CXError_Success) {
    fprintf(stderr, "Unable to load translation unit!\n");
    describeLibclangFailure(Err);
  ...
  Err = clang_reparseTranslationUnit(TU, 0, 0, clang_defaultReparseOptions(TU));
  ...
  for (I = 0; I != Repeats; ++I) {
    results = clang_codeCompleteAt(TU, filename, line, column, unsaved_files,
                                   num_unsaved_files, completionOptions);
    if (!results) {
  ...
  if (results) {
    ...
    containerKind =
        clang_codeCompleteGetContainerKind(results, &containerIsIncomplete);
...
static int inspect_cursor_at(int argc, const char **argv,
  ...
  while (strstr(argv[NumLocations + 1], locations_flag) ==
         argv[NumLocations + 1])
    ++NumLocations;

  /* Parse the locations. */
  assert(NumLocations > 0 && "Unable to count locations?");
  Locations = (CursorSourceLocation *)malloc(NumLocations *
  ...
  Err = clang_parseTranslationUnit2(
      CIdx, argv[argc - 1], argv + num_unsaved_files + 1 + NumLocations,
      argc - num_unsaved_files - 2 - NumLocations, unsaved_files,
  ...
  for (I = 0; I != Repeats; ++I) {
    ...
    for (Loc = 0; Loc < NumLocations; ++Loc) {
      ...
      Cursor =
          clang_getCursor(TU, clang_getLocation(TU, file, Locations[Loc].line,
...
static enum CXVisitorResult findFileRefsVisit(void *context, CXCursor cursor,
                                              CXSourceRange range) {
  if (clang_Range_isNull(range))
    return CXVisit_Continue;
...
static int find_file_refs_at(int argc, const char **argv) {
  ...
  /* Count the number of locations. */
  while (strstr(argv[NumLocations + 1], "-file-refs-at=") ==
         argv[NumLocations + 1])
    ++NumLocations;
...
findFileIncludesVisit(void *context, CXCursor cursor, CXSourceRange range) {
  PrintCursor(cursor, NULL);
  PrintRange(range, "");
  printf("\n");
  return CXVisit_Continue;
...
static int find_file_includes_in(int argc, const char **argv) {
  ...
  /* Count the number of locations. */
  while (strstr(argv[NumFilenames + 1], "-file-includes-in=") ==
         argv[NumFilenames + 1])
    ++NumFilenames;
  ...
  CIdx = clang_createIndex(1, 1);
  Err = clang_parseTranslationUnit2(
      CIdx, argv[argc - 1], argv + num_unsaved_files + 1 + NumFilenames,
      argc - num_unsaved_files - 2 - NumFilenames, unsaved_files,
      Repeats > 1 ? 0 : num_unsaved_files, getDefaultParsingOptions(), &TU);
...
static const char *
getEntityTemplateKindString(CXIdxEntityCXXTemplateKind kind) {
  switch (kind) {
...
static const char *getEntityLanguageString(CXIdxEntityLanguage kind) {
  switch (kind) {
  case CXIdxEntityLang_None:
    return "<none>";
  case CXIdxEntityLang_C:
    return "C";
  case CXIdxEntityLang_ObjC:
    return "ObjC";
  case CXIdxEntityLang_CXX:
    return "C++";
  case CXIdxEntityLang_Swift:
    return "Swift";
  }
...
static void printEntityInfo(const char *cb, CXClientData client_data,
                            const CXIdxEntityInfo *info) {
  const char *name;
  IndexData *index_data;
  unsigned i;
...
static void index_diagnostic(CXClientData client_data, CXDiagnosticSet diagSet,
                             void *reserved) {
  CXString str;
  const char *cstr;
  unsigned numDiags, i;
...
static CXIdxClientFile index_ppIncludedFile(CXClientData client_data,
  ...
  printf(" | isImport: %d | isAngled: %d | isModule: %d", info->isImport,
         info->isAngled, info->isModuleImport);
...
static void index_indexEntityReference(CXClientData client_data,
  ...
  printf(" | refkind: ");
  switch (info->kind) {
  case CXIdxEntityRef_Direct:
    printf("direct");
    break;
  case CXIdxEntityRef_Implicit:
    printf("implicit");
    break;
  }
  printf(" | role:");
...
static IndexerCallbacks IndexCB = {
    index_abortQuery,       index_diagnostic,
    index_enteredMainFile,  index_ppIncludedFile,
    index_importedASTFile,  index_startedTranslationUnit,
    index_indexDeclaration, index_indexEntityReference};
...
static int index_compile_args(int num_args, const char **args,
  ...
  result = clang_indexSourceFile(idxAction, &index_data, &IndexCB,
                                 sizeof(IndexCB), index_opts, 0, args, num_args,
                                 0, 0, 0, getDefaultParsingOptions());
...
static int index_ast_file(const char *ast_file, CXIndex Idx,
                          CXIndexAction idxAction,
                          ImportedASTFilesData *importedASTs,
                          const char *check_prefix) {
  CXTranslationUnit TU;
  ...
  index_opts = getIndexOptions();
  result = clang_indexTranslationUnit(idxAction, &index_data, &IndexCB,
                                      sizeof(IndexCB), index_opts, TU);
  if (index_data.fail_for_error)
    result = -1;
...
static int index_file(int argc, const char **argv, int full) {
  ...
  result =
      index_compile_args(argc, argv, idxAction, importedASTs, check_prefix);
  if (result != 0)
    goto finished;
...
int perform_token_annotation(int argc, const char **argv) {
  ...
  Err = clang_parseTranslationUnit2(
      CIdx, argv[argc - 1], argv + num_unsaved_files + 2,
      argc - num_unsaved_files - 3, unsaved_files, num_unsaved_files,
      getDefaultParsingOptions(), &TU);
  if (Err != CXError_Success) {
    fprintf(stderr, "unable to parse input\n");
    describeLibclangFailure(Err);
  ...
  for (i = 0; i != skipped_ranges->count; ++i) {
    unsigned start_line, start_column, end_line, end_column;
    clang_getSpellingLocation(clang_getRangeStart(skipped_ranges->ranges[i]), 0,
                              &start_line, &start_column, 0);
    clang_getSpellingLocation(clang_getRangeEnd(skipped_ranges->ranges[i]), 0,
  ...
  for (i = 0; i != num_tokens; ++i) {
    ...
    clang_getSpellingLocation(clang_getRangeStart(extent), 0, &start_line,
                              &start_column, 0);
    clang_getSpellingLocation(clang_getRangeEnd(extent), 0, &end_line,
                              &end_column, 0);
...
static int perform_test_compilation_db(const char *database, int argc,
                                       const char **argv) {
...
static unsigned isUSR(const char *s) { return s[0] == 'c' && s[1] == ':'; }
...
int write_pch_file(const char *filename, int argc, const char *argv[]) {
  ...
  switch (
      clang_saveTranslationUnit(TU, filename, clang_defaultSaveOptions(TU))) {
...
static const char *getDiagnosticCodeStr(enum CXLoadDiag_Error error) {
  switch (error) {
  case CXLoadDiag_CannotLoad:
    return "Cannot Load File";
  case CXLoadDiag_None:
    break;
  case CXLoadDiag_Unknown:
    return "Unknown";
  case CXLoadDiag_InvalidFile:
    return "Invalid File";
  }
  return "None";
}
...
static void printDiagnosticSet(CXDiagnosticSet Diags, unsigned indent) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    fprintf(stderr, "%s:%d:%d: %s: %s", FileNameStr ? FileNameStr : "(null)",
            line, column, getSeverityString(clang_getDiagnosticSeverity(D)),
...
static void print_usage(void) {
  fprintf(
      ...
      "       c-index-test -get-macro-info-cursor-at=<site> <compiler "
      "arguments>\n"
      "       c-index-test -file-refs-at=<site> <compiler arguments>\n"
      "       c-index-test -file-includes-in=<filename> <compiler "
      "arguments>\n");
  fprintf(stderr,
          "       c-index-test -index-file [-check-prefix=<FileCheck prefix>] "
          "<compiler arguments>\n"
          "       c-index-test -index-file-full [-check-prefix=<FileCheck "
          "prefix>] <compiler arguments>\n"
          "       c-index-test -index-tu [-check-prefix=<FileCheck prefix>] "
          "<AST file>\n"
          "       c-index-test -index-compile-db [-check-prefix=<FileCheck "
          "prefix>] <compilation database>\n"
...
int cindextest_main(int argc, const char **argv) {
  ...
  if (argc > 2 && strstr(argv[1], "-evaluate-cursor-at=") == argv[1])
    return inspect_cursor_at(argc, argv,
                             "-evaluate-cursor-at=", inspect_evaluate_cursor);
  if (argc > 2 && strstr(argv[1], "-get-macro-info-cursor-at=") == argv[1])
    return inspect_cursor_at(
        argc, argv, "-get-macro-info-cursor-at=", inspect_macroinfo_cursor);
  ...
  if (argc > 2 && strcmp(argv[1], "-index-compile-db") == 0)
    ...
    if (I)
      return perform_test_load_tu(argv[2], argv[3], argc >= 5 ? argv[4] : 0, I,
             ...
             strncmp(argv[1], "-test-load-source-reparse", 25) == 0) {
    ...
    return perform_single_file_parse(argv[2]);
           ...
           strcmp(argv[1], "-retain-excluded-conditional-blocks") == 0)
    return perform_file_retain_excluded_cb(argv[2]);
  else if (argc >= 4 && strcmp(argv[1], "-test-file-scan") == 0)
    return perform_file_scan(argv[2], argv[3], argc >= 5 ? argv[4] : 0);
    ...
    return perform_test_load_source(argc - 2, argv + 2, "all", PrintType, 0);
  else if (argc > 2 && strcmp(argv[1], "-test-print-type-size") == 0)
    return perform_test_load_source(argc - 2, argv + 2, "all", PrintTypeSize,
                                    0);
    ...
    return perform_test_load_source(argc - 2, argv + 2, "all", PrintBitWidth,
                                    0);
...
void thread_runner(void *client_data_v) {
  ...
  client_data->result =
      client_data->main_func(client_data->argc, client_data->argv);
}
