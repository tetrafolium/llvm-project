...
static __isl_give isl_vec *interval_sample(__isl_take isl_basic_set *bset) {
  ...
  if (bset->n_eq > 0) {
    ...
    if (isl_int_is_one(bset->eq[0][1]))
      ...
      isl_assert(bset->ctx, isl_int_is_negone(bset->eq[0][1]), goto error);
  ...
  for (i = 1; i < bset->n_ineq; ++i) {
    isl_seq_inner_product(sample->block.data, bset->ineq[i], 2, &t);
    if (isl_int_is_neg(t))
      break;
  }
  isl_int_clear(t);
...
sample_eq(__isl_take isl_basic_set *bset,
...
static struct isl_mat *tab_equalities(struct isl_tab *tab) {
  ...
  for (i = 0, j = 0; i < tab->n_con; ++i) {
    ...
    if (i < bset->n_eq)
      ...
      isl_seq_cpy(eq->row[j], bset->ineq[i - bset->n_eq] + 1, tab->n_var);
    ++j;
  }
...
static enum isl_lp_result compute_min(isl_ctx *ctx, struct isl_tab *tab,
  ...
  return isl_tab_min(tab, tab->basis->row[1 + level], ctx->one, &min->el[level],
                     NULL, 0);
}
...
static enum isl_lp_result compute_max(isl_ctx *ctx, struct isl_tab *tab,
  ...
  isl_seq_neg(tab->basis->row[1 + level] + 1, tab->basis->row[1 + level] + 1,
              dim);
  res = isl_tab_min(tab, tab->basis->row[1 + level], ctx->one, &max->el[level],
                    NULL, 0);
  isl_seq_neg(tab->basis->row[1 + level] + 1, tab->basis->row[1 + level] + 1,
              dim);
...
static int greedy_search(isl_ctx *ctx, struct isl_tab *tab,
                         __isl_keep isl_vec *min, __isl_keep isl_vec *max,
                         int level) {
  ...
  do {
    isl_int_add(tab->basis->row[1 + level][0], min->el[level], max->el[level]);
    isl_int_fdiv_q_ui(tab->basis->row[1 + level][0],
                      tab->basis->row[1 + level][0], 2);
    isl_int_neg(tab->basis->row[1 + level][0], tab->basis->row[1 + level][0]);
    ...
    if (res == isl_lp_error)
      return -1;
    if (res != isl_lp_ok)
      isl_die(ctx, isl_error_internal, "expecting bounded rational solution",
              return -1);
...
__isl_give isl_vec *isl_tab_sample(struct isl_tab *tab) {
  ...
  if (!tab->basis)
    return NULL;
  isl_assert(tab->mat->ctx, tab->basis->n_row == tab->n_var + 1, return NULL);
  isl_assert(tab->mat->ctx, tab->basis->n_col == tab->n_var + 1, return NULL);
  ...
  if (tab->n_unbounded == tab->n_var) {
    ...
    sample = isl_mat_vec_inverse_product(isl_mat_copy(tab->basis), sample);
    return sample;
  }
...
static __isl_give isl_basic_set *shift_cone(__isl_take isl_basic_set *cone,
  ...
  shift = isl_basic_set_alloc_space(isl_basic_set_get_space(cone), 0, 0,
                                    cone->n_ineq);
  ...
  for (i = 0; i < cone->n_ineq; ++i) {
    ...
    isl_seq_inner_product(shift->ineq[k] + 1, vec->el + 1, total,
                          &shift->ineq[k][0]);
    isl_int_cdiv_q(shift->ineq[k][0], shift->ineq[k][0], vec->el[0]);
    isl_int_neg(shift->ineq[k][0], shift->ineq[k][0]);
    for (j = 0; j < total; ++j) {
      if (isl_int_is_nonneg(shift->ineq[k][1 + j]))
        continue;
      isl_int_add(shift->ineq[k][0], shift->ineq[k][0], shift->ineq[k][1 + j]);
    }
  }
...
static __isl_give isl_vec *round_up_in_cone(__isl_take isl_vec *vec,
  ...
  cone =
      isl_basic_set_remove_dims(cone, isl_dim_set, 0, total - (vec->size - 1));
 ...
 */
__isl_give isl_vec *
isl_basic_set_sample_with_cone(__isl_take isl_basic_set *bset,
                               __isl_take isl_basic_set *cone) {
  isl_size total;
  unsigned cone_dim;
  struct isl_mat *M, *U;
  ...
  bounded = isl_basic_set_drop_constraints_involving(bounded, total - cone_dim,
                                                     cone_dim);
 ...
 */
static int tab_shift_cone(struct isl_tab *tab, struct isl_tab *tab_cone,
                          struct isl_mat *U) {
  int i;
  isl_int v;
...
static __isl_give isl_vec *basic_set_sample(__isl_take isl_basic_set *bset,
  ...
  if (dim < 0 || isl_basic_set_check_no_params(bset) < 0 ||
 ...
 */
__isl_give isl_vec *
isl_basic_set_sample_bounded(__isl_take isl_basic_set *bset) {
