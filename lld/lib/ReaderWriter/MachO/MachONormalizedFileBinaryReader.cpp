...
static std::error_code appendRelocations(Relocations &relocs, StringRef buffer,
                                         bool bigEndian, uint32_t reloff,
                                         uint32_t nreloc) {
...
bool isThinObjectFile(StringRef path, MachOLinkingContext::Arch &arch) {
  ...
  const mach_header *mh =
      reinterpret_cast<const mach_header *>(fileBuffer.begin());
  bool is64, isBig;
  ...
  arch = MachOLinkingContext::archFromCpuType(read32(&mh->cputype, isBig),
...
bool sliceFromFatFile(MemoryBufferRef mb, MachOLinkingContext::Arch arch,
  ...
  const fat_arch *fend = reinterpret_cast<const fat_arch *>(
      start + sizeof(fat_header) + sizeof(fat_arch) * nfat_arch);
...
readBinary(std::unique_ptr<MemoryBuffer> &mb,
  ...
  if (f->arch != arch) {
    return llvm::make_error<GenericError>(
        Twine("file is wrong architecture. Expected "
              "(" +
              MachOLinkingContext::nameFromArch(arch) + ") found (" +
  ...
  auto ec = forEachLoadCommand(
      lcRange, lcCount, isBig, is64,
      [&](uint32_t cmd, uint32_t size, const char *lc) -> bool {
        if (cmd == LC_DYSYMTAB) {
          const dysymtab_command *d =
              reinterpret_cast<const dysymtab_command *>(lc);
      ...
      [&](uint32_t cmd, uint32_t size, const char *lc) -> bool {
        ...
        case LC_SEGMENT_64:
          if (is64) {
            ...
            for (unsigned i = 0; i < sectionCount; ++i) {
              ...
              section.type =
                  (SectionType)(read32(&sect->flags, isBig) & SECTION_TYPE);
              section.attributes =
                  read32(&sect->flags, isBig) & SECTION_ATTRIBUTES;
        ...
        case LC_SEGMENT:
          if (!is64) {
            ...
            const unsigned sectionCount = read32(&seg->nsects, isBig);
            const section *sects =
                reinterpret_cast<const section *>(lc + sizeof(segment_command));
            const unsigned lcSize =
                sizeof(segment_command) + sectionCount * sizeof(section);
            // Verify sections don't extend beyond end of segment load command.
            if (lcSize > size)
              return true;
            for (unsigned i = 0; i < sectionCount; ++i) {
              ...
              section.type =
                  (SectionType)(read32(&sect->flags, isBig) & SECTION_TYPE);
              section.attributes =
                  read32((const uint8_t *)&sect->flags, isBig) &
                  SECTION_ATTRIBUTES;
              section.alignment = 1 << read32(&sect->align, isBig);
          ...
          break;
        case LC_SYMTAB: {
          const symtab_command *st =
              reinterpret_cast<const symtab_command *>(lc);
          ...
          if (is64) {
            ...
            for (uint32_t i = 0; i < symCount; ++i) {
              ...
              sout.type =
                  static_cast<NListType>(sin->n_type & (N_STAB | N_TYPE));
        ...
        case LC_RPATH: {
          const rpath_command *rpc =
              reinterpret_cast<const rpath_command *>(lc);
        ...
        case LC_VERSION_MIN_TVOS:
          // If we are emitting an object file, then we may take the load
          // command kind from these commands and pass it on to the output file.
  ...
  if (dyldInfo) {
    ...
    if (dyldInfo->export_size) {
      ...
      for (const ExportEntry &trieExport :
           MachOObjectFile::exports(Err, trie)) {
        Export normExport;
        normExport.name = trieExport.name().copy(f->ownedAllocations);
        normExport.offset = trieExport.address();
        normExport.kind = ExportSymbolKind(trieExport.flags() &
        ...
        if (!trieExport.otherName().empty())
          normExport.otherName =
              trieExport.otherName().copy(f->ownedAllocations);
