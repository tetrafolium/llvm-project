...
public:
  ...
  virtual bool
  enableNonTemporal(const MachineBasicBlock::iterator &MI) const = 0;
...
SIMemOpAccess::toSIAtomicScope(SyncScope::ID SSID,
  ...
  if (SSID == SyncScope::System)
    return std::make_tuple(SIAtomicScope::SYSTEM, SIAtomicAddrSpace::ATOMIC,
                           true);
  if (SSID == MMI->getAgentSSID())
    return std::make_tuple(SIAtomicScope::AGENT, SIAtomicAddrSpace::ATOMIC,
                           true);
  if (SSID == MMI->getWorkgroupSSID())
    return std::make_tuple(SIAtomicScope::WORKGROUP, SIAtomicAddrSpace::ATOMIC,
                           true);
  if (SSID == MMI->getWavefrontSSID())
    return std::make_tuple(SIAtomicScope::WAVEFRONT, SIAtomicAddrSpace::ATOMIC,
                           true);
  if (SSID == SyncScope::SingleThread)
    return std::make_tuple(SIAtomicScope::SINGLETHREAD,
                           SIAtomicAddrSpace::ATOMIC, true);
  if (SSID == MMI->getSystemOneAddressSpaceSSID())
    return std::make_tuple(SIAtomicScope::SYSTEM,
                           SIAtomicAddrSpace::ATOMIC & InstrScope, false);
...
Optional<SIMemOpInfo> SIMemOpAccess::constructFromMIWithMMO(
  ...
  for (const auto &MMO : MI->memoperands()) {
    ...
    if (OpOrdering != AtomicOrdering::NotAtomic) {
      ...
      Ordering =
          isStrongerThan(Ordering, OpOrdering) ? Ordering : MMO->getOrdering();
  ...
  if (MOI.isAtomic()) {
    ...
    if (MOI.getOrdering() == AtomicOrdering::Acquire ||
      ...
      Changed |= CC->insertAcquire(MI, MOI.getScope(),
                                   MOI.getOrderingAddrSpace(), Position::AFTER);
    }
  ...
  if (MOI.isAtomic()) {
    ...
    if (MOI.getOrdering() == AtomicOrdering::Acquire ||
      ...
      Changed |= CC->insertAcquire(
          MI, MOI.getScope(), MOI.getOrderingAddrSpace(), Position::BEFORE);
...
bool SIMemoryLegalizer::expandAtomicCmpxchgOrRmw(
  ...
  if (MOI.isAtomic()) {
    ...
    if (MOI.getOrdering() == AtomicOrdering::Acquire ||
        ...
        MOI.getFailureOrdering() == AtomicOrdering::SequentiallyConsistent) {
      Changed |=
          CC->insertWait(MI, MOI.getScope(), MOI.getOrderingAddrSpace(),
