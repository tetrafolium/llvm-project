...
private:
  ...
  bool evaluateScalarMathFunc(FuncInfo &FInfo, double &Res0, double &Res1,
...
INITIALIZE_PASS(AMDGPUUseNativeCalls, "amdgpu-usenative",
                "Replace builtin math calls with that native versions.", false,
                false)
...
bool AMDGPULibCalls::replaceWithNative(CallInst *CI, const FuncInfo &FInfo) {
  ...
  if (getArgType(FInfo) != AMDGPULibFunc::F32 ||
      FInfo.getPrefix() != AMDGPULibFunc::NOPFX || !HasNative(FInfo.getId()))
    return false;
...
bool AMDGPULibCalls::fold_recip(CallInst *CI, IRBuilder<> &B,
  ...
  if (ConstantFP *CF = dyn_cast<ConstantFP>(opr0)) {
    ...
    Value *nval =
        B.CreateFDiv(ConstantFP::get(CF->getType(), 1.0), opr0, "recip2div");
    LLVM_DEBUG(errs() << "AMDIC: " << *CI << " ---> " << *nval << "\n");
    replaceCall(nval);
    return true;
  }
...
bool AMDGPULibCalls::fold_divide(CallInst *CI, IRBuilder<> &B,
  ...
  {
    Value *nval1 = B.CreateFDiv(ConstantFP::get(opr1->getType(), 1.0), opr1,
                                "__div2recip");
...
bool AMDGPULibCalls::fold_pow(CallInst *CI, IRBuilder<> &B,
  ...
  if (getVecSize(FInfo) == 1) {
    ...
    if (CF) {
      ...
      needlog = true;
      needcopysign = needabs =
          FInfo.getId() != AMDGPULibFunc::EI_POWR && (!CF || CF->isNegative());
    }
  } else {
...
bool AMDGPULibCalls::fold_wavefrontsize(CallInst *CI, IRBuilder<> &B) {
  ...
  LLVM_DEBUG(errs() << "AMDIC: fold_wavefrontsize (" << *CI << ") with " << N
                    << "\n");
...
AllocaInst *AMDGPULibCalls::insertAlloca(CallInst *UI, IRBuilder<> &B,
  ...
  AllocaInst *Alloc =
      B.CreateAlloca(RetType, 0, std::string(prefix) + UI->getName());
  Alloc->setAlignment(
      Align(UCallee->getParent()->getDataLayout().getTypeAllocSize(RetType)));
  return Alloc;
}

bool AMDGPULibCalls::evaluateScalarMathFunc(FuncInfo &FInfo, double &Res0,
                                            double &Res1, Constant *copr0,
                                            Constant *copr1, Constant *copr2) {
