...
#define PPC64_OPD2                                                             \
  SEPARATOR.p2align 3 SEPARATOR.quad.Lfunc_begin0 SEPARATOR.quad.TOC           \
      .@tocbase SEPARATOR.quad 0 SEPARATOR.text SEPARATOR.Lfunc_begin0:
#else
#define PPC64_OPD1
#define PPC64_OPD2
#endif
...
.pushsection ".note.gnu.property",
    "a" SEPARATOR.balign 8 SEPARATOR.long 4 SEPARATOR.long 0x10 SEPARATOR
        ...
        .long 4 SEPARATOR
        .long 3 SEPARATOR /* GNU_PROPERTY_AARCH64_FEATURE_1_BTI AND */ /* GNU_PROPERTY_AARCH64_FEATURE_1_PAC
                                                                        */
        .long 0 SEPARATOR.popsection SEPARATOR
...
#define WEAK_ALIAS(name, aliasname)                                            \
  .globl SYMBOL_NAME(aliasname) SEPARATOR WEAK_SYMBOL(aliasname)               \
...
#else
#define WEAK_ALIAS(name, aliasname)                                            \
  WEAK_SYMBOL(aliasname) SEPARATOR SYMBOL_NAME(aliasname) = SYMBOL_NAME(name)
#endif
...
#define EXPORT_SYMBOL2(name)                                                   \
  .section.drectve, "yn" SEPARATOR.ascii "-export:", #name, "\0" SEPARATOR.text
...
#define WEAK_ALIAS(name, aliasname)                                            \
  .globl SYMBOL_NAME(aliasname) SEPARATOR EXPORT_SYMBOL(aliasname)             \
      SEPARATOR SYMBOL_NAME(aliasname) = SYMBOL_NAME(name)
#else
...
#define DEFINE_LIBUNWIND_FUNCTION(name)                                        \
  .globl SYMBOL_NAME(name) SEPARATOR HIDDEN_SYMBOL(SYMBOL_NAME(name))          \
      SEPARATOR SYMBOL_IS_FUNC(SYMBOL_NAME(name))                              \
          SEPARATOR PPC64_OPD1 SYMBOL_NAME(name)                               \
      : PPC64_OPD2 AARCH64_BTI
