...
bool ContainsAnyImpliedDoIndex(const ExtentExpr &expr) {
    struct MyVisitor : public AnyTraverse<MyVisitor> {
        ...
        MyVisitor() : Base{*this} {}
        using Base::operator();
        bool operator()(const ImpliedDoIndex &) {
            return true;
        }
    };
...
public:
    ...
    static ExtentExpr Default() {
        return ExtentExpr{1};
    }
    static ExtentExpr Combine(Result &&, Result &&) {
        return Default();
    }
            ...
            for (const auto &shapeSpec : details->shape()) {
                if (j++ == dimension_) {
                    if (const auto &bound{shapeSpec.lbound().GetExplicit()}) {
                        ...
                        return ExtentExpr{
                            DescriptorInquiry{
                                NamedEntity{common::Clone(component)},
                                DescriptorInquiry::Field::LowerBound, dimension_}};
                    } else {
                        break;
                    }
...
auto GetShapeHelper::operator()(const Symbol &symbol) const -> Result {
    ...
    common::visitors{
        ...
        [](const semantics::TypeParamDetails &) {
            return Scalar();
        },
        [](const auto &) {
            return Result{};
