...
vertex_coordinate(__isl_keep isl_basic_set *vertex, int i,
...
static isl_stat add_fold(__isl_take isl_qpolynomial *b, __isl_keep isl_set *dom,
  ...
  if (tight)
    data->fold_tight =
        isl_qpolynomial_fold_fold_on_domain(dom, data->fold_tight, fold);
  else
    data->fold = isl_qpolynomial_fold_fold_on_domain(dom, data->fold, fold);
  return isl_stat_ok;
}
    ...
    while (i > 0) {
      if (i == n - 1) {
        ...
        for (j = 2; j <= left[i - 1]; ++j)
          isl_int_divexact_ui(multinom->el[i], multinom->el[i], j);
        b = isl_qpolynomial_coeff(c[i - 1], isl_dim_in, n - 1 - i, left[i - 1]);
        b = isl_qpolynomial_project_domain_on_params(b);
        space = isl_qpolynomial_get_domain_space(b);
        f = isl_qpolynomial_rat_cst_on_domain(space, ctx->one, multinom->el[i]);
      ...
      ++k[i];
      if (k[i])
        isl_int_divexact_ui(multinom->el[i], multinom->el[i], k[i]);
      isl_qpolynomial_free(c[i]);
      c[i] = isl_qpolynomial_coeff(c[i - 1], isl_dim_in, n - 1 - i, k[i]);
      left[i] = left[i - 1] - k[i];
      k[i + 1] = -1;
...
static isl_stat bernstein_coefficients_cell(__isl_take isl_cell *cell,
  ...
  if (n_vertices > nvar + 1 && ctx->opt->bernstein_triangulate)
    return isl_cell_foreach_simplex(cell, &bernstein_coefficients_cell, user);
  ...
  for (i = 0; i < 1 + nvar; ++i)
    subs[i] = isl_qpolynomial_zero_on_domain(isl_space_copy(space_dst));

  for (i = 0; i < n_vertices; ++i) {
    isl_qpolynomial *c;
    c = isl_qpolynomial_var_on_domain(isl_space_copy(space_dst), isl_dim_set,
                                      1 + nvar + i);
    for (j = 0; j < nvar; ++j) {
      int k = cell->ids[i];
      isl_qpolynomial *v;
      ...
      v = isl_qpolynomial_add_dims(v, isl_dim_in, 1 + nvar + n_vertices);
  ...
  data->fold =
      isl_qpolynomial_fold_empty(data->type, isl_space_copy(space_param));
  ...
  pwf =
      isl_pw_qpolynomial_fold_alloc(data->type, isl_set_copy(dom), data->fold);
 ...
 */
static __isl_give isl_pw_qpolynomial_fold *
bernstein_coefficients_base(__isl_take isl_basic_set *bset,
                            __isl_take isl_qpolynomial *poly,
                            struct bernstein_data *data, isl_bool *tight) {
  isl_size nvar;
  isl_space *space;
  isl_pw_qpolynomial_fold *pwf;
  ...
  data->pwf = isl_pw_qpolynomial_fold_zero(isl_space_copy(space), data->type);
  ...
  if (isl_vertices_foreach_disjoint_cell(vertices, &bernstein_coefficients_cell,
                                         data) < 0)
 ...
 */
static __isl_give isl_pw_qpolynomial_fold *
bernstein_coefficients_recursive(__isl_take isl_pw_qpolynomial *pwqp,
                                 int n_group, int *len,
                                 struct bernstein_data *data, isl_bool *tight) {
  int i;
  isl_size nparam;
  ...
  pwqp = isl_pw_qpolynomial_move_dims(pwqp, isl_dim_param, nparam, isl_dim_in,
                                      0, nvar - len[n_group - 1]);
  ...
  for (i = n_group - 2; i >= 0; --i) {
    ...
    pwf = isl_pw_qpolynomial_fold_move_dims(pwf, isl_dim_in, 0, isl_dim_param,
                                            nparam - len[i], len[i]);
    if (tight && !*tight)
      tight = NULL;
...
static __isl_give isl_pw_qpolynomial_fold *
bernstein_coefficients_factors(__isl_take isl_basic_set *bset,
                               __isl_take isl_qpolynomial *poly,
                               struct bernstein_data *data, isl_bool *tight) {
  isl_factorizer *f;
  isl_set *set;
  ...
  pwf = bernstein_coefficients_recursive(pwqp, f->n_group, f->len, data, tight);
...
static __isl_give isl_pw_qpolynomial_fold *
bernstein_coefficients_full_recursive(__isl_take isl_basic_set *bset,
                                      __isl_take isl_qpolynomial *poly,
                                      struct bernstein_data *data,
                                      isl_bool *tight) {
  int i;
 ...
 */
isl_stat
isl_qpolynomial_bound_on_domain_bernstein(__isl_take isl_basic_set *bset,
                                          __isl_take isl_qpolynomial *poly,
