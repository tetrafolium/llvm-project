...
__isl_give isl_aff *isl_aff_alloc(__isl_take isl_local_space *ls) {
  ...
  if (!isl_local_space_divs_known(ls))
    isl_die(ctx, isl_error_invalid, "local space has unknown divs", goto error);
  if (!isl_local_space_is_set(ls))
    isl_die(ctx, isl_error_invalid,
            "domain of affine expression should be a set", goto error);
...
__isl_give isl_aff *isl_aff_dup(__isl_keep isl_aff *aff) {
  ...
  return isl_aff_alloc_vec(isl_local_space_copy(aff->ls), isl_vec_copy(aff->v));
}
...
isl_pw_aff_zero_on_domain(__isl_take isl_local_space *ls) {
 ...
 */
__isl_give isl_pw_aff *
isl_pw_aff_nan_on_domain(__isl_take isl_local_space *ls) {
...
__isl_give isl_aff *isl_aff_val_on_domain(__isl_take isl_local_space *ls,
  ...
  if (!isl_val_is_rat(val))
    isl_die(isl_val_get_ctx(val), isl_error_invalid, "expecting rational value",
            goto error);
 ...
 */
__isl_give isl_aff *
isl_aff_param_on_domain_space_id(__isl_take isl_space *space,
 ...
 */
static __isl_keep isl_local_space *
isl_aff_peek_domain_local_space(__isl_keep isl_aff *aff) {
 ...
 */
static __isl_keep isl_space *
isl_aff_peek_domain_space(__isl_keep isl_aff *aff) {
...
isl_aff_get_domain_local_space(__isl_keep isl_aff *aff) {
 ...
 */
__isl_give isl_local_space *
isl_aff_take_domain_local_space(__isl_keep isl_aff *aff) {
  isl_local_space *ls;
 ...
 */
__isl_give isl_aff *
isl_aff_restore_domain_local_space(__isl_keep isl_aff *aff,
 ...
 */
__isl_give isl_aff *
isl_aff_reset_space_and_domain(__isl_take isl_aff *aff,
                               __isl_take isl_space *space,
 ...
 * variables.
 */
static __isl_give isl_vec *
vec_reorder(__isl_take isl_vec *vec, __isl_take isl_reordering *r, int n_div) {
  isl_space *space;
  isl_vec *res;
  isl_size dim;
  int i;
...
__isl_give isl_aff *isl_aff_align_params(__isl_take isl_aff *aff,
  ...
  if (!equal_params) {
    ...
    exp = isl_reordering_extend_space(exp, isl_aff_get_domain_space(aff));
...
__isl_give isl_val *isl_aff_get_coefficient_val(__isl_keep isl_aff *aff,
  ...
  if (type == isl_dim_out)
    isl_die(ctx, isl_error_invalid,
            "output/set dimension does not have a coefficient", return NULL);
  if (type == isl_dim_in)
    type = isl_dim_set;
...
int isl_aff_coefficient_sgn(__isl_keep isl_aff *aff, enum isl_dim_type type,
  ...
  if (type == isl_dim_out)
    isl_die(ctx, isl_error_invalid,
            "output/set dimension does not have a coefficient", return 0);
  if (type == isl_dim_in)
    type = isl_dim_set;
...
__isl_give isl_aff *isl_aff_set_constant_val(__isl_take isl_aff *aff,
  ...
  if (!isl_val_is_rat(v))
    isl_die(isl_aff_get_ctx(aff), isl_error_invalid, "expecting rational value",
            goto error);

  if (isl_int_eq(aff->v->el[1], v->n) && isl_int_eq(aff->v->el[0], v->d)) {
    isl_val_free(v);
    return aff;
  }
  ...
  if (isl_int_eq(aff->v->el[0], v->d)) {
    ...
    isl_seq_scale(aff->v->el + 1, aff->v->el + 1, v->d, aff->v->size - 1);
    isl_int_mul(aff->v->el[1], aff->v->el[0], v->n);
...
__isl_give isl_aff *isl_aff_set_coefficient_val(__isl_take isl_aff *aff,
  ...
  if (type == isl_dim_out)
    isl_die(aff->v->ctx, isl_error_invalid,
            "output/set dimension does not have a coefficient", goto error);
  if (type == isl_dim_in)
    type = isl_dim_set;
...
static __isl_give isl_aff *plug_in_integral_divs(__isl_take isl_aff *aff) {
  ...
  for (i = 0; i < n; ++i) {
    if (!isl_int_is_one(aff->ls->div->row[i][0]))
      continue;
    ls = isl_local_space_copy(aff->ls);
    ls = isl_local_space_substitute_seq(
        ls, isl_dim_div, i, aff->ls->div->row[i], len, i + 1, n - (i + 1));
    ...
    isl_seq_substitute(vec->el, off + i, aff->ls->div->row[i], len, len, v);
  ...
  for (i = 1; i < n; ++i) {
    for (j = 0; j < i; ++j) {
      if (!isl_int_is_one(aff->ls->div->row[i][1 + off + j]))
        continue;
      aff->ls = isl_local_space_substitute_seq(
          aff->ls, isl_dim_div, j, aff->ls->div->row[j], aff->v->size, i, 1);
      if (!aff->ls)
        return isl_aff_free(aff);
    }
  }
...
static __isl_give isl_aff *merge_divs(__isl_take isl_aff *aff, int a, int b) {
  ...
  isl_int_add(aff->v->el[1 + off + a], aff->v->el[1 + off + a],
              aff->v->el[1 + off + b]);
...
__isl_give isl_aff *isl_aff_mod_val(__isl_take isl_aff *aff,
  ...
  if (!isl_val_is_int(m))
    isl_die(isl_val_get_ctx(m), isl_error_invalid, "expecting integer modulo",
            goto error);
...
__isl_give isl_aff *isl_aff_set_dim_id(__isl_take isl_aff *aff,
  ...
  if (type == isl_dim_out)
    isl_die(aff->v->ctx, isl_error_invalid,
            "cannot set name of output/set dimension", goto error);
  if (type == isl_dim_in)
    type = isl_dim_set;
...
__isl_give isl_aff *isl_aff_set_tuple_id(__isl_take isl_aff *aff,
  ...
  if (!aff)
    goto error;
  if (type != isl_dim_in)
    isl_die(aff->v->ctx, isl_error_invalid, "cannot only set id of input tuple",
            goto error);
...
isl_aff_substitute_equalities_lifted(__isl_take isl_aff *aff,
  ...
  aff->ls =
      isl_local_space_substitute_equalities(aff->ls, isl_basic_set_copy(eq));
  aff->v = isl_vec_cow(aff->v);
  if (!aff->ls || !aff->v)
    goto error;
  ...
  for (i = 0; i < eq->n_eq; ++i) {
    ...
    isl_seq_elim(aff->v->el + 1, eq->eq[i], j, o_div, &aff->v->el[0]);
  }
 ...
 */
__isl_give isl_aff *
isl_aff_substitute_equalities(__isl_take isl_aff *aff,
                              __isl_take isl_basic_set *eq) {
  isl_size n_div;
...
aff_nonneg_basic_set(__isl_take isl_aff *aff, int rational, void *user) {
  isl_constraint *ineq;
  isl_basic_set *bset;
...
__isl_give isl_set *isl_aff_ne_set(__isl_take isl_aff *aff1,
  ...
  set_lt = isl_aff_lt_set(isl_aff_copy(aff1), isl_aff_copy(aff2));
...
#include "check_type_range_templ.c"
     ...
     */
    isl_bool
    isl_aff_involves_dims(__isl_keep isl_aff *aff, enum isl_dim_type type,
...
__isl_give isl_aff *isl_aff_drop_dims(__isl_take isl_aff *aff,
  ...
  if (!aff)
    return NULL;
  if (type == isl_dim_out)
    isl_die(aff->v->ctx, isl_error_invalid, "cannot drop output/set dimension",
            return isl_aff_free(aff));
  if (type == isl_dim_in)
    type = isl_dim_set;
...
__isl_give isl_aff *isl_aff_insert_dims(__isl_take isl_aff *aff,
  ...
  if (type == isl_dim_out)
    isl_die(aff->v->ctx, isl_error_invalid,
            "cannot insert output/set dimensions", return isl_aff_free(aff));
  if (type == isl_dim_in)
    type = isl_dim_set;
  if (n == 0 && !isl_local_space_is_named_or_nested(aff->ls, type))
    return aff;
...
__isl_give isl_aff *isl_aff_move_dims(__isl_take isl_aff *aff,
  ...
  if (!aff)
    return NULL;
  if (n == 0 && !isl_local_space_is_named_or_nested(aff->ls, src_type) &&
      !isl_local_space_is_named_or_nested(aff->ls, dst_type))
    return aff;

  if (dst_type == isl_dim_out || src_type == isl_dim_out)
    isl_die(isl_aff_get_ctx(aff), isl_error_invalid,
            "cannot move output/set dimension", return isl_aff_free(aff));
  if (dst_type == isl_dim_div || src_type == isl_dim_div)
    isl_die(isl_aff_get_ctx(aff), isl_error_invalid, "cannot move divs",
            return isl_aff_free(aff));
  ...
  aff->ls = isl_local_space_move_dims(aff->ls, dst_type, dst_pos, src_type,
                                      src_pos, n);
  if (!aff->v || !aff->ls)
    return isl_aff_free(aff);
...
pw_aff_locus(__isl_take isl_pw_aff *pwaff,
             __isl_give isl_basic_set *(*fn)(__isl_take isl_aff *aff,
                                             int rational, void *user),
...
isl_pw_aff_order_map(__isl_take isl_pw_aff *pa1, __isl_take isl_pw_aff *pa2,
 ...
 */
static __isl_give isl_set *
pw_aff_list_set(__isl_take isl_pw_aff_list *list1,
  ...
  if (list1->n < 1 || list2->n < 1)
    isl_die(ctx, isl_error_invalid, "list should contain at least one element",
            goto error);
  ...
  for (i = 0; i < list1->n; ++i)
    for (j = 0; j < list2->n; ++j) {
      ...
      set_ij = fn(isl_pw_aff_copy(list1->p[i]), isl_pw_aff_copy(list2->p[j]));
...
__isl_give isl_set *isl_pw_aff_ne_set(__isl_take isl_pw_aff *pwaff1,
  ...
  isl_pw_aff_align_params_bin(&pwaff1, &pwaff2);
  set_lt = isl_pw_aff_lt_set(isl_pw_aff_copy(pwaff1), isl_pw_aff_copy(pwaff2));
...
__isl_give isl_pw_aff *isl_pw_aff_scale_down(__isl_take isl_pw_aff *pwaff,
  ...
  if (!isl_int_is_pos(v))
    isl_die(isl_pw_aff_get_ctx(pwaff), isl_error_invalid,
            "factor needs to be positive", return isl_pw_aff_free(pwaff));
...
__isl_give isl_pw_aff *isl_pw_aff_cond(__isl_take isl_pw_aff *cond,
  ...
  pwaff_true =
      isl_pw_aff_align_params(pwaff_true, isl_pw_aff_get_space(pwaff_false));
  pwaff_false =
      isl_pw_aff_align_params(pwaff_false, isl_pw_aff_get_space(pwaff_true));
  ...
  return isl_pw_aff_select(cond_true, pwaff_true, cond_false, pwaff_false);
error:
  isl_pw_aff_free(cond);
  isl_pw_aff_free(pwaff_true);
  isl_pw_aff_free(pwaff_false);
...
__isl_give isl_aff *isl_aff_mul(__isl_take isl_aff *aff1,
  ...
  if (!isl_aff_is_cst(aff2))
    isl_die(isl_aff_get_ctx(aff1), isl_error_invalid,
            "at least one affine expression should be constant", goto error);
...
__isl_give isl_pw_aff *isl_pw_aff_div(__isl_take isl_pw_aff *pa1,
  ...
  if (!is_cst)
    isl_die(isl_pw_aff_get_ctx(pa2), isl_error_invalid,
            "second argument should be a piecewise constant", goto error);
  isl_pw_aff_align_params_bin(&pa1, &pa2);
...
static __isl_give isl_pw_aff *pw_aff_min(__isl_take isl_pw_aff *pwaff1,
  ...
  le = isl_pw_aff_le_set(isl_pw_aff_copy(pwaff1), isl_pw_aff_copy(pwaff2));
...
static __isl_give isl_pw_aff *pw_aff_max(__isl_take isl_pw_aff *pwaff1,
  ...
  dom = isl_set_intersect(isl_pw_aff_domain(isl_pw_aff_copy(pwaff1)),
                          isl_pw_aff_domain(isl_pw_aff_copy(pwaff2)));
  ge = isl_pw_aff_ge_set(isl_pw_aff_copy(pwaff1), isl_pw_aff_copy(pwaff2));
...
isl_pw_aff_list_set_rational(__isl_take isl_pw_aff_list *list) {
  int i, n;
...
isl_stat isl_aff_check_match_domain_space(__isl_keep isl_aff *aff,
  ...
  if (match < 0)
    goto error;
  if (!match)
    isl_die(isl_aff_get_ctx(aff), isl_error_invalid, "parameters don't match",
            goto error);
  match = isl_space_tuple_is_equal(space, isl_dim_in, aff_space, isl_dim_set);
  if (match < 0)
    goto error;
  if (!match)
    isl_die(isl_aff_get_ctx(aff), isl_error_invalid, "domains don't match",
            goto error);
  isl_space_free(aff_space);
  return isl_stat_ok;
error:
 ...
 */
static __isl_give isl_basic_set *
isl_multi_aff_domain(__isl_take isl_multi_aff *ma) {
  isl_space *space;
...
isl_multi_aff_from_aff_mat(__isl_take isl_space *space,
  ...
  if (n_row < 0 || n_col < 0 || n_out < 0 || total < 0)
    goto error;
  if (n_row < 1)
    isl_die(ctx, isl_error_invalid, "insufficient number of rows", goto error);
  if (n_col < 1)
    isl_die(ctx, isl_error_invalid, "insufficient number of columns",
            goto error);
  if (1 + n_out != n_row || 2 + total != n_row + n_col)
    isl_die(ctx, isl_error_invalid, "dimension mismatch", goto error);
...
isl_multi_aff_get_constant_multi_val(__isl_keep isl_multi_aff *ma) {
  int i;
  isl_size n;
  isl_space *space;
  isl_multi_val *mv;
 ...
 */
__isl_give isl_multi_aff *
isl_multi_aff_flatten_domain(__isl_take isl_multi_aff *ma) {
  isl_space *space;
 ...
 */
__isl_give isl_multi_aff *
isl_multi_aff_domain_map(__isl_take isl_space *space) {
  int i;
  isl_size n_in;
  isl_local_space *ls;
  isl_multi_aff *ma;
  ...
  if (!space)
    return NULL;
  if (!isl_space_is_map(space))
    isl_die(isl_space_get_ctx(space), isl_error_invalid, "not a map space",
            goto error);
...
__isl_give isl_multi_aff *isl_multi_aff_range_map(__isl_take isl_space *space) {
  ...
  for (i = 0; i < n_out; ++i) {
    ...
    aff =
        isl_aff_var_on_domain(isl_local_space_copy(ls), isl_dim_set, n_in + i);
    ma = isl_multi_aff_set_aff(ma, i, aff);
  }
  isl_local_space_free(ls);
  return ma;
...
isl_pw_multi_aff_domain_map(__isl_take isl_space *space) {
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_range_map(__isl_take isl_space *space) {
 ...
 */
__isl_give isl_multi_aff *
isl_multi_aff_project_out_map(__isl_take isl_space *space,
                              enum isl_dim_type type, unsigned first,
                              unsigned n) {
  int i;
  isl_size dim;
  ...
  for (i = 0; i < dim - (first + n); ++i) {
    ...
    aff = isl_aff_var_on_domain(isl_local_space_copy(ls), isl_dim_set,
                                first + n + i);
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_project_out_map(__isl_take isl_space *space,
                                 enum isl_dim_type type, unsigned first,
                                 unsigned n) {
  isl_multi_aff *ma;
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_identity(__isl_take isl_space *space) {
...
isl_multi_aff_substitute_equalities(__isl_take isl_multi_aff *maff,
                                    __isl_take isl_basic_set *eq) {
  int i;
  ...
  for (i = 0; i < maff->n; ++i) {
    maff->u.p[i] =
        isl_aff_substitute_equalities(maff->u.p[i], isl_basic_set_copy(eq));
    if (!maff->u.p[i])
      goto error;
  }
...
__isl_give isl_multi_aff *
isl_multi_aff_add_on_domain(__isl_keep isl_set *dom,
 ...
 */
static __isl_give isl_set *
isl_multi_aff_lex_gte_set_0d(__isl_take isl_multi_aff *ma1,
                             __isl_take isl_multi_aff *ma2, int equal) {
  isl_space *space;
 ...
 */
static __isl_give isl_set *
isl_multi_aff_order_at(__isl_keep isl_multi_aff *ma1,
 ...
 */
static __isl_give isl_set *
isl_multi_aff_lex_gte_set(__isl_take isl_multi_aff *ma1,
                          __isl_take isl_multi_aff *ma2, int equal) {
  int i;
  isl_size n;
  isl_space *space;
 ...
 */
static isl_bool
isl_pw_multi_aff_range_is_wrapping(__isl_keep isl_pw_multi_aff *pma) {
 ...
 */
static isl_stat
pw_multi_aff_check_range_product(__isl_keep isl_pw_multi_aff *pma) {
  isl_bool wraps;
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_range_factor_domain(__isl_take isl_pw_multi_aff *pma) {
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_range_factor_range(__isl_take isl_pw_multi_aff *pma) {
  return pw_multi_aff_factor(pma, &pw_multi_aff_check_range_product,
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_union_lexmax(__isl_take isl_pw_multi_aff *pma1,
                              __isl_take isl_pw_multi_aff *pma2) {
  isl_pw_multi_aff_align_params_bin(&pma1, &pma2);
  return isl_pw_multi_aff_union_opt_cmp(pma1, pma2, &isl_multi_aff_lex_ge_set);
}
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_union_lexmin(__isl_take isl_pw_multi_aff *pma1,
                              __isl_take isl_pw_multi_aff *pma2) {
  isl_pw_multi_aff_align_params_bin(&pma1, &pma2);
  return isl_pw_multi_aff_union_opt_cmp(pma1, pma2, &isl_multi_aff_lex_le_set);
}

__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_add(__isl_take isl_pw_multi_aff *pma1,
                     __isl_take isl_pw_multi_aff *pma2) {
  isl_pw_multi_aff_align_params_bin(&pma1, &pma2);
  return isl_pw_multi_aff_on_shared_domain(pma1, pma2, &isl_multi_aff_add);
}

/* Subtract "pma2" from "pma1" and return the result.
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_sub(__isl_take isl_pw_multi_aff *pma1,
                     __isl_take isl_pw_multi_aff *pma2) {
  isl_pw_multi_aff_align_params_bin(&pma1, &pma2);
  return isl_pw_multi_aff_on_shared_domain(pma1, pma2, &isl_multi_aff_sub);
}

__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_union_add(__isl_take isl_pw_multi_aff *pma1,
                           __isl_take isl_pw_multi_aff *pma2) {
...
isl_union_pw_aff_union_add(__isl_take isl_union_pw_aff *upa1,
...
isl_union_pw_multi_aff_union_add(__isl_take isl_union_pw_multi_aff *upma1,
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_product(__isl_take isl_pw_multi_aff *pma1,
                         __isl_take isl_pw_multi_aff *pma2) {
  int i, j, n;
  isl_space *space;
  ...
  n = pma1->n * pma2->n;
  space =
      isl_space_product(isl_space_copy(pma1->dim), isl_space_copy(pma2->dim));
  ...
  for (i = 0; i < pma1->n; ++i) {
    for (j = 0; j < pma2->n; ++j) {
      ...
      ma = isl_multi_aff_product(isl_multi_aff_copy(pma1->p[i].maff),
...
static __isl_give isl_aff *subtract_initial(__isl_take isl_aff *aff,
                                            __isl_keep isl_multi_aff *ma, int n,
                                            isl_int *c, isl_int denom) {
  int i, first;
  int sign;
  isl_int d;
 ...
 */
static __isl_give isl_aff *
extract_aff_from_equality(__isl_keep isl_basic_map *bmap, int pos, int eq,
                          int div, int ineq, __isl_keep isl_multi_aff *ma) {
  unsigned o_out;
  ...
  if (isl_int_is_neg(bmap->eq[eq][o_out + pos])) {
    ...
    isl_seq_cpy(aff->v->el + 1 + o_out, bmap->eq[eq] + o_out + n_out, n_div);
  } else {
    isl_seq_neg(aff->v->el + 1, bmap->eq[eq], o_out);
    isl_seq_neg(aff->v->el + 1 + o_out, bmap->eq[eq] + o_out + n_out, n_div);
  }
  if (div < n_div)
  ...
  if (div < n_div) {
    ...
    isl_seq_cpy(shift->v->el + 1 + o_out, bmap->ineq[ineq] + o_out + n_out,
                n_div);
    isl_int_set_si(shift->v->el[0], 1);
    shift =
        subtract_initial(shift, ma, pos, bmap->ineq[ineq] + o_out, ctx->negone);
    aff = isl_aff_add(aff, isl_aff_copy(shift));
    mod = isl_val_int_from_isl_int(ctx, bmap->eq[eq][o_out + n_out + div]);
    mod = isl_val_abs(mod);
 ...
 */
static __isl_give isl_aff *
extract_isl_aff_from_basic_map(__isl_keep isl_basic_map *bmap, int pos,
 ...
 */
static __isl_give isl_multi_aff *
extract_isl_multi_aff_from_basic_map(__isl_take isl_basic_map *bmap) {
  int i;
  isl_size n_out;
  isl_multi_aff *ma;
 ...
 */
__isl_give isl_multi_aff *
isl_multi_aff_from_basic_set_equalities(__isl_take isl_basic_set *bset) {
...
plain_pw_multi_aff_from_map(__isl_take isl_set *domain,
 ...
 */
static __isl_give isl_pw_multi_aff *
pw_multi_aff_from_map_base(__isl_take isl_map *map) {
  int i;
  int sv;
  ...
  if (sv < 0)
    goto error;
  if (!sv)
    isl_die(isl_map_get_ctx(map), isl_error_invalid, "map is not single-valued",
            goto error);
 ...
 */
static __isl_give isl_pw_multi_aff *
pw_multi_aff_from_map_div(__isl_take isl_map *map,
                          __isl_take isl_basic_map *hull, int d, int i) {
  isl_ctx *ctx;
  ...
  if (is_set) {
    ...
    ma = isl_multi_aff_range_product(ma, isl_multi_aff_from_aff(aff));
  }
 ...
 */
static int is_potential_div_constraint(isl_int *c, int offset, int d,
                                       int total) {
  ...
  if (isl_seq_first_non_zero(c + offset, d) != -1)
    return 0;
  if (isl_seq_first_non_zero(c + offset + d + 1, total - (offset + d + 1)) !=
      -1)
    return 0;
  return 1;
}
 ...
 */
static __isl_give isl_pw_multi_aff *
pw_multi_aff_from_map_check_div(__isl_take isl_map *map) {
  int d;
  isl_size dim;
  int i, j, n;
  int offset;
  ...
  for (d = 0; d < dim; ++d) {
    for (i = 0; i < n; ++i) {
      if (!is_potential_div_constraint(hull->ineq[i], offset, d, 1 + total))
        continue;
      for (j = i + 1; j < n; ++j) {
        if (!isl_seq_is_neg(hull->ineq[i] + 1, hull->ineq[j] + 1, total))
          continue;
        isl_int_add(sum, hull->ineq[i][0], hull->ineq[j][0]);
        if (isl_int_abs_lt(sum, hull->ineq[i][offset + d]))
          break;
      }
      if (j >= n)
        continue;
 ...
 */
static __isl_give isl_pw_multi_aff *
pw_multi_aff_from_map_stride(__isl_take isl_map *map,
                             __isl_take isl_basic_map *hull, int d, int i,
                             isl_int gcd) {
  isl_set *set;
  isl_space *space;
  ...
  if (aff) {
    ...
    if (isl_int_is_one(hull->eq[i][o_out + d]))
      isl_seq_neg(aff->v->el + 1, hull->eq[i], aff->v->size - 1);
    else
      isl_seq_cpy(aff->v->el + 1, hull->eq[i], aff->v->size - 1);
 ...
 */
static __isl_give isl_pw_multi_aff *
pw_multi_aff_from_map_check_strides(__isl_take isl_map *map,
  ...
  for (i = 0; i < n_out; ++i) {
    for (j = 0; j < hull->n_eq; ++j) {
      ...
      if (!isl_int_is_one(eq[o_out + i]) && !isl_int_is_negone(eq[o_out + i]))
        continue;
      if (isl_seq_first_non_zero(eq + o_out, i) != -1)
        continue;
      if (isl_seq_first_non_zero(eq + o_out + i + 1, n_out - (i + 1)) != -1)
        continue;
      ...
      res = pw_multi_aff_from_map_stride(map, hull, i, j, gcd);
      isl_int_clear(gcd);
      return res;
    }
  }
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_from_map(__isl_take isl_map *map) {
  isl_bool sv;
  isl_size n;
  isl_basic_map *hull;
...
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_from_set(__isl_take isl_set *set) {
 ...
 */
__isl_give isl_union_pw_multi_aff *
isl_union_pw_multi_aff_from_aff(__isl_take isl_aff *aff) {
  isl_multi_aff *ma;
  isl_pw_multi_aff *pma;
 ...
 */
__isl_give isl_union_pw_multi_aff *
isl_union_pw_multi_aff_from_union_map(__isl_take isl_union_map *umap) {
  isl_space *space;
 ...
 */
__isl_give isl_union_pw_multi_aff *
isl_union_pw_multi_aff_from_union_set(__isl_take isl_union_set *uset) {
...
__isl_give isl_aff *isl_aff_substitute(__isl_take isl_aff *aff,
  ...
  if (!isl_space_is_equal(aff->ls->dim, subs->ls->dim))
    isl_die(ctx, isl_error_invalid, "spaces don't match",
            return isl_aff_free(aff));
  n_div = isl_aff_domain_dim(subs, isl_dim_div);
  if (n_div < 0)
    return isl_aff_free(aff);
  if (n_div != 0)
    isl_die(ctx, isl_error_unsupported, "cannot handle divs yet",
            return isl_aff_free(aff));
  ...
  isl_int_init(v);
  isl_seq_substitute(aff->v->el, pos, subs->v->el, aff->v->size, subs->v->size,
                     v);
  isl_int_clear(v);
 ...
 */
__isl_give isl_multi_aff *
isl_multi_aff_substitute(__isl_take isl_multi_aff *maff, enum isl_dim_type type,
  ...
  for (i = 0; i < maff->n; ++i) {
    maff->u.p[i] = isl_aff_substitute(maff->u.p[i], type, pos, subs);
    if (!maff->u.p[i])
      return isl_multi_aff_free(maff);
  }
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_substitute(__isl_take isl_pw_multi_aff *pma,
                            enum isl_dim_type type, unsigned pos,
  ...
  for (i = 0; i < pma->n; ++i) {
    for (j = 0; j < subs->n; ++j) {
      ...
      common = isl_set_substitute(common, type, pos, subs->p[j].aff);
      ...
      res_ij = isl_multi_aff_substitute(isl_multi_aff_copy(pma->p[i].maff),
...
isl_stat isl_seq_preimage(isl_int *dst, isl_int *src,
                          __isl_keep isl_multi_aff *ma, int n_before,
                          int n_after, int n_div_ma, int n_div_bmap, isl_int f,
                          isl_int c1, isl_int c2, isl_int g, int has_denom) {
  int i;
  ...
  for (i = 0; i < n_out; ++i) {
    ...
    o_dst = has_denom;
    o_src = 1;
    isl_seq_combine(dst + o_dst, c1, dst + o_dst, c2, ma->u.p[i]->v->el + o_src,
                    1 + n_param);
    o_dst += 1 + n_param;
    o_src += 1 + n_param;
...
__isl_give isl_aff *isl_aff_pullback_multi_aff(__isl_take isl_aff *aff,
  ...
  if (isl_seq_preimage(res->v->el, aff->v->el, ma, 0, 0, n_div_ma, n_div_aff, f,
                       c1, c2, g, 1) < 0)
 ...
 */
__isl_give isl_multi_aff *
isl_multi_aff_pullback_multi_aff(__isl_take isl_multi_aff *ma1,
                                 __isl_take isl_multi_aff *ma2) {
  int i;
  isl_space *space = NULL;
  ...
  for (i = 0; i < ma1->n; ++i) {
    ma1->u.p[i] =
        isl_aff_pullback_multi_aff(ma1->u.p[i], isl_multi_aff_copy(ma2));
    if (!ma1->u.p[i])
      goto error;
  }
 ...
 */
__isl_give isl_multi_aff *
isl_multi_aff_align_divs(__isl_take isl_multi_aff *maff) {
  int i;
...
__isl_give isl_multi_aff *isl_multi_aff_lift(__isl_take isl_multi_aff *maff,
  ...
  space =
      isl_space_extend_domain_with_range(space, isl_multi_aff_get_space(maff));
  if (!space)
    return isl_multi_aff_free(maff);
...
#include "check_type_range_templ.c"
     ...
     */
    __isl_give isl_pw_aff *
    isl_pw_multi_aff_get_pw_aff(__isl_keep isl_pw_multi_aff *pma, int pos) {
  int i;
  isl_size n_out;
  isl_space *space;
  isl_pw_aff *pa;
  ...
  space = isl_space_drop_dims(space, isl_dim_out, pos + 1, n_out - pos - 1);
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_from_domain(__isl_take isl_set *set) {
  isl_multi_aff *ma;
  isl_space *space;
 ...
 */
__isl_give isl_union_pw_multi_aff *
isl_union_pw_multi_aff_from_domain(__isl_take isl_union_set *uset) {
  isl_space *space;
  isl_union_pw_multi_aff *upma;
  ...
  if (isl_union_set_foreach_set(uset, &add_pw_multi_aff_from_domain, &upma) < 0)
    goto error;
...
static isl_stat bin_entry(__isl_take isl_pw_multi_aff *pma, void *user) {
  ...
  data->pma = pma;
  r = isl_union_pw_multi_aff_foreach_pw_multi_aff(data->upma2, data->fn, data);
  isl_pw_multi_aff_free(pma);
...
bin_op(__isl_take isl_union_pw_multi_aff *upma1,
       __isl_take isl_union_pw_multi_aff *upma2,
       isl_stat (*fn)(__isl_take isl_pw_multi_aff *pma, void *user)) {
  isl_space *space;
  ...
  if (isl_union_pw_multi_aff_foreach_pw_multi_aff(upma1, &bin_entry, &data) < 0)
    goto error;
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_range_product(__isl_take isl_pw_multi_aff *pma1,
                               __isl_take isl_pw_multi_aff *pma2) {
  isl_space *space;
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_flat_range_product(__isl_take isl_pw_multi_aff *pma1,
                                    __isl_take isl_pw_multi_aff *pma2) {
  isl_space *space;
  ...
  return isl_pw_multi_aff_on_shared_domain_in(
      pma1, pma2, space, &isl_multi_aff_flat_range_product);
}
 ...
 */
static isl_stat gen_range_product_entry(
    __isl_take isl_pw_multi_aff *pma2,
  ...
  match = isl_space_tuple_is_equal(space1, isl_dim_in, space2, isl_dim_in);
  if (match < 0 || !match) {
    isl_pw_multi_aff_free(pma2);
...
static isl_stat flat_range_product_entry(__isl_take isl_pw_multi_aff *pma2,
                                         void *user) {
  return gen_range_product_entry(pma2, &isl_pw_multi_aff_flat_range_product,
                                 user);
}
...
static isl_stat range_product_entry(__isl_take isl_pw_multi_aff *pma2,
                                    void *user) {
  return gen_range_product_entry(pma2, &isl_pw_multi_aff_range_product, user);
}
 ...
 */
__isl_give isl_union_pw_multi_aff *
isl_union_pw_multi_aff_range_product(__isl_take isl_union_pw_multi_aff *upma1,
                                     __isl_take isl_union_pw_multi_aff *upma2) {
 ...
 */
static __isl_give isl_pw_multi_aff *
pw_multi_aff_set_pw_aff(__isl_take isl_pw_multi_aff *pma, unsigned pos,
  ...
  if (!isl_space_tuple_is_equal(pma->dim, isl_dim_in, pa->dim, isl_dim_in))
    isl_die(isl_pw_multi_aff_get_ctx(pma), isl_error_invalid,
  ...
  for (i = 0; i < pma->n; ++i) {
    for (j = 0; j < pa->n; ++j) {
      ...
      res_ij = isl_multi_aff_set_aff(isl_multi_aff_copy(pma->p[i].maff), pos,
                                     isl_aff_copy(pa->p[j].aff));
      res_ij = isl_multi_aff_gist(res_ij, isl_set_copy(common));
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_set_pw_aff(__isl_take isl_pw_multi_aff *pma, unsigned pos,
                            __isl_take isl_pw_aff *pa) {
  isl_bool equal_params;
...
isl_stat isl_pw_aff_check_match_domain_space(__isl_keep isl_pw_aff *pa,
  ...
  if (match < 0)
    goto error;
  if (!match)
    isl_die(isl_pw_aff_get_ctx(pa), isl_error_invalid, "parameters don't match",
            goto error);
  match = isl_space_tuple_is_equal(space, isl_dim_in, pa_space, isl_dim_in);
  if (match < 0)
    goto error;
  if (!match)
    isl_die(isl_pw_aff_get_ctx(pa), isl_error_invalid, "domains don't match",
            goto error);
  isl_space_free(pa_space);
  return isl_stat_ok;
error:
 ...
 */
isl_bool
isl_multi_pw_aff_has_non_trivial_domain(__isl_keep isl_multi_pw_aff *mpa) {
  if (!mpa)
    return isl_bool_error;
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_scale_multi_val(__isl_take isl_pw_multi_aff *pma,
  ...
  if (!pma || !mv)
    goto error;
  if (!isl_space_tuple_is_equal(pma->dim, isl_dim_out, mv->space, isl_dim_set))
    isl_die(isl_pw_multi_aff_get_ctx(pma), isl_error_invalid,
  ...
  if (equal_params < 0)
    goto error;
  if (!equal_params) {
    pma = isl_pw_multi_aff_align_params(pma, isl_multi_val_get_space(mv));
    mv = isl_multi_val_align_params(mv, isl_pw_multi_aff_get_space(pma));
    if (!pma || !mv)
      goto error;
  }
  ...
  for (i = 0; i < pma->n; ++i) {
    pma->p[i].maff =
        isl_multi_aff_scale_multi_val(pma->p[i].maff, isl_multi_val_copy(mv));
    if (!pma->p[i].maff)
      goto error;
  }
 ...
 */
static __isl_give isl_pw_multi_aff *
union_pw_multi_aff_scale_multi_val_entry(__isl_take isl_pw_multi_aff *pma,
                                         void *user) {
  isl_multi_val *mv = user;

  if (!pma)
    return NULL;
  if (!isl_space_tuple_is_equal(pma->dim, isl_dim_out, mv->space,
                                isl_dim_set)) {
 ...
 */
__isl_give isl_union_pw_multi_aff *
isl_union_pw_multi_aff_scale_multi_val(__isl_take isl_union_pw_multi_aff *upma,
                                       __isl_take isl_multi_val *mv) {
  ...
  upma = isl_union_pw_multi_aff_align_params(upma, isl_multi_val_get_space(mv));
  mv = isl_multi_val_align_params(mv, isl_union_pw_multi_aff_get_space(upma));
  if (!upma || !mv)
    goto error;
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_zero(__isl_take isl_space *space) {
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_from_pw_aff(__isl_take isl_pw_aff *pa) {
  int i;
  isl_space *space;
  isl_pw_multi_aff *pma;
 ...
 */
static __isl_give isl_pw_multi_aff *
isl_pw_multi_aff_from_multi_pw_aff_0D(__isl_take isl_multi_pw_aff *mpa) {
  isl_space *space;
  isl_set *dom;
  isl_multi_aff *ma;
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_from_multi_pw_aff(__isl_take isl_multi_pw_aff *mpa) {
  int i;
  isl_space *space;
  isl_pw_aff *pa;
  isl_pw_multi_aff *pma;
...
isl_multi_pw_aff_from_aff(__isl_take isl_aff *aff) {
 ...
 */
__isl_give isl_multi_pw_aff *
isl_multi_pw_aff_from_multi_aff(__isl_take isl_multi_aff *ma) {
  int i;
  isl_size n;
 ...
 */
__isl_give isl_multi_pw_aff *
isl_multi_pw_aff_from_pw_multi_aff(__isl_take isl_pw_multi_aff *pma) {
  int i;
  isl_size n;
  isl_space *space;
  isl_multi_pw_aff *mpa;
...
isl_bool isl_multi_pw_aff_is_equal(__isl_keep isl_multi_pw_aff *mpa1,
  ...
  if (!equal_params) {
    ...
    mpa1 =
        isl_multi_pw_aff_align_params(mpa1, isl_multi_pw_aff_get_space(mpa2));
    mpa2 =
        isl_multi_pw_aff_align_params(mpa2, isl_multi_pw_aff_get_space(mpa1));
...
isl_multi_pw_aff_pullback_multi_aff_aligned(__isl_take isl_multi_pw_aff *mpa,
                                            __isl_take isl_multi_aff *ma) {
  int i;
  isl_space *space = NULL;
  ...
  for (i = 0; i < mpa->n; ++i) {
    mpa->u.p[i] =
        isl_pw_aff_pullback_multi_aff(mpa->u.p[i], isl_multi_aff_copy(ma));
    if (!mpa->u.p[i])
      goto error;
  }
  if (isl_multi_pw_aff_has_explicit_domain(mpa)) {
    mpa->u.dom = isl_set_preimage_multi_aff(mpa->u.dom, isl_multi_aff_copy(ma));
    if (!mpa->u.dom)
      goto error;
  }
 ...
 */
__isl_give isl_multi_pw_aff *
isl_multi_pw_aff_pullback_multi_aff(__isl_take isl_multi_pw_aff *mpa,
                                    __isl_take isl_multi_aff *ma) {
  isl_bool equal_params;
...
isl_multi_pw_aff_pullback_pw_multi_aff_aligned(
  ...
  if (isl_multi_pw_aff_has_explicit_domain(mpa)) {
    mpa->u.dom =
        isl_set_preimage_pw_multi_aff(mpa->u.dom, isl_pw_multi_aff_copy(pma));
    if (!mpa->u.dom)
      goto error;
  }
 ...
 */
__isl_give isl_multi_pw_aff *
isl_multi_pw_aff_pullback_pw_multi_aff(__isl_take isl_multi_pw_aff *mpa,
                                       __isl_take isl_pw_multi_aff *pma) {
  isl_bool equal_params;
  ...
  mpa = isl_multi_pw_aff_align_params(mpa, isl_pw_multi_aff_get_space(pma));
  pma = isl_pw_multi_aff_align_params(pma, isl_multi_pw_aff_get_space(mpa));
  return isl_multi_pw_aff_pullback_pw_multi_aff_aligned(mpa, pma);
error:
  isl_multi_pw_aff_free(mpa);
  isl_pw_multi_aff_free(pma);
...
isl_multi_pw_aff_apply_aff_aligned(__isl_take isl_multi_pw_aff *mpa,
  ...
  for (i = 0; i < n_div; ++i) {
    ...
    pa_i = isl_multi_pw_aff_apply_aff_aligned(isl_multi_pw_aff_copy(mpa), div);
 ...
 */
__isl_give isl_pw_aff *
isl_multi_pw_aff_apply_aff(__isl_take isl_multi_pw_aff *mpa,
                           __isl_take isl_aff *aff) {
  isl_bool equal_params;
 ...
 */
static __isl_give isl_pw_aff *
isl_multi_pw_aff_apply_pw_aff_aligned(__isl_take isl_multi_pw_aff *mpa,
                                      __isl_take isl_pw_aff *pa) {
  isl_space *space;
  isl_pw_aff *res;
  int i;
  ...
  space =
      isl_space_join(isl_multi_pw_aff_get_space(mpa), isl_pw_aff_get_space(pa));
  ...
  for (i = 0; i < pa->n; ++i) {
    ...
    pa_i = isl_multi_pw_aff_apply_aff_aligned(isl_multi_pw_aff_copy(mpa),
                                              isl_aff_copy(pa->p[i].aff));
    domain = isl_set_copy(pa->p[i].set);
    domain = isl_set_preimage_multi_pw_aff(domain, isl_multi_pw_aff_copy(mpa));
 ...
 */
__isl_give isl_pw_aff *
isl_multi_pw_aff_apply_pw_aff(__isl_take isl_multi_pw_aff *mpa,
                              __isl_take isl_pw_aff *pa) {
  isl_bool equal_params;
 ...
 */
static __isl_give isl_pw_aff *
isl_pw_aff_pullback_multi_pw_aff_aligned(__isl_take isl_pw_aff *pa,
 ...
 */
__isl_give isl_pw_aff *
isl_pw_aff_pullback_multi_pw_aff(__isl_take isl_pw_aff *pa,
                                 __isl_take isl_multi_pw_aff *mpa) {
 ...
 */
__isl_give isl_multi_pw_aff *
isl_multi_pw_aff_pullback_multi_pw_aff(__isl_take isl_multi_pw_aff *mpa1,
  ...
  if (isl_multi_pw_aff_has_explicit_domain(mpa1)) {
    mpa1->u.dom =
        isl_set_preimage_multi_pw_aff(mpa1->u.dom, isl_multi_pw_aff_copy(mpa2));
    if (!mpa1->u.dom)
      goto error;
  }
...
static __isl_give isl_map *isl_multi_pw_aff_order_map(
  ...
  mpa1 = isl_multi_pw_aff_align_params(mpa1, isl_multi_pw_aff_get_space(mpa2));
  mpa2 = isl_multi_pw_aff_align_params(mpa2, isl_multi_pw_aff_get_space(mpa1));
  if (!mpa1 || !mpa2)
    goto error;
  match = isl_space_tuple_is_equal(mpa1->space, isl_dim_out, mpa2->space,
                                   isl_dim_out);
  if (match < 0)
    goto error;
  if (!match)
 ...
 */
static __isl_give isl_map *
isl_multi_pw_aff_eq_map_on_space(__isl_keep isl_multi_pw_aff *mpa1,
                                 __isl_keep isl_multi_pw_aff *mpa2,
                                 __isl_take isl_space *space) {
  int i;
  isl_size n;
 ...
 */
__isl_give isl_map *
isl_map_eq_at_multi_pw_aff(__isl_take isl_map *map,
                           __isl_take isl_multi_pw_aff *mpa) {
  return isl_map_order_at_multi_pw_aff(map, mpa, &isl_multi_pw_aff_eq_map);
}
 ...
 */
__isl_give isl_multi_aff *
isl_multi_aff_multi_val_on_space(__isl_take isl_space *space,
                                 __isl_take isl_multi_val *mv) {
  int i;
  isl_size n;
  isl_space *space2;
 ...
 */
__isl_give isl_pw_multi_aff *
isl_pw_multi_aff_multi_val_on_domain(__isl_take isl_set *domain,
                                     __isl_take isl_multi_val *mv) {
  isl_space *space;
  isl_multi_aff *ma;
 ...
 */
__isl_give isl_union_pw_multi_aff *
isl_union_pw_multi_aff_multi_val_on_domain(__isl_take isl_union_set *domain,
                                           __isl_take isl_multi_val *mv) {
  ...
  data.mv = mv;
  if (isl_union_set_foreach_set(domain, &pw_multi_aff_multi_val_on_domain,
                                &data) < 0)
...
static isl_stat pullback_entry(__isl_take isl_pw_multi_aff *pma2, void *user) {
  ...
  if (!isl_space_tuple_is_equal(data->pma->dim, isl_dim_in, pma2->dim,
                                isl_dim_out)) {
    isl_pw_multi_aff_free(pma2);
    return isl_stat_ok;
  }
 ...
 */
static isl_stat
isl_union_pw_aff_check_match_domain_space(__isl_keep isl_union_pw_aff *upa,
                                          __isl_keep isl_space *space) {
  isl_space *upa_space;
  isl_bool match;
  ...
  if (match < 0)
    return isl_stat_error;
  if (!match)
    isl_die(isl_space_get_ctx(space), isl_error_invalid, "expecting set space",
            return isl_stat_error);
 ...
 */
static isl_bool
isl_union_pw_aff_matching_params(__isl_keep isl_union_pw_aff *upa,
                                 __isl_keep isl_space *space) {
  isl_space *upa_space;
  isl_bool match;
 ...
 */
static __isl_give isl_union_pw_aff *
isl_union_pw_aff_reset_domain_space(__isl_take isl_union_pw_aff *upa,
                                    __isl_take isl_space *space) {
 ...
 */
__isl_give isl_union_pw_aff *
isl_union_pw_aff_floor(__isl_take isl_union_pw_aff *upa) {
 ...
 */
__isl_give isl_union_pw_aff *
isl_union_pw_aff_mod_val(__isl_take isl_union_pw_aff *upa,
                         __isl_take isl_val *m) {
  isl_union_pw_aff *res;
  ...
  if (!isl_val_is_int(m))
    isl_die(isl_val_get_ctx(m), isl_error_invalid, "expecting integer modulo",
            goto error);
  if (!isl_val_is_pos(m))
    isl_die(isl_val_get_ctx(m), isl_error_invalid, "expecting positive modulo",
            goto error);
 ...
 */
__isl_give isl_union_pw_aff *
isl_union_pw_multi_aff_get_union_pw_aff(__isl_keep isl_union_pw_multi_aff *upma,
                                        int pos) {
  ...
  data.pos = pos;
  if (isl_union_pw_multi_aff_foreach_pw_multi_aff(upma, &get_union_pw_aff,
                                                  &data) < 0)
    data.res = isl_union_pw_aff_free(data.res);
 ...
 */
__isl_give isl_union_pw_aff *
isl_union_pw_aff_aff_on_domain(__isl_take isl_union_set *domain,
                               __isl_take isl_aff *aff) {
  isl_pw_aff *pa;
 ...
 */
__isl_give isl_union_pw_aff *
isl_union_pw_aff_param_on_domain_id(__isl_take isl_union_set *domain,
                                    __isl_take isl_id *id) {
  isl_space *space;
  isl_aff *aff;
 ...
 */
static __isl_give isl_union_pw_aff *
isl_union_pw_aff_pw_aff_on_domain_aligned(__isl_take isl_union_set *domain,
                                          __isl_take isl_pw_aff *pa) {
 ...
 */
__isl_give isl_union_pw_aff *
isl_union_pw_aff_pw_aff_on_domain(__isl_take isl_union_set *domain,
                                  __isl_take isl_pw_aff *pa) {
  isl_bool is_set;
  isl_bool equal_params;
 ...
 */
__isl_give isl_union_pw_aff *
isl_union_pw_aff_val_on_domain(__isl_take isl_union_set *domain,
                               __isl_take isl_val *v) {
 ...
 */
__isl_give isl_union_pw_multi_aff *
isl_union_pw_multi_aff_from_union_pw_aff(__isl_take isl_union_pw_aff *upa) {
  isl_space *space;
  isl_union_pw_multi_aff *upma;
  ...
  if (isl_union_pw_aff_foreach_pw_aff(upa, &pw_multi_aff_from_pw_aff_entry,
                                      &upma) < 0)
    upma = isl_union_pw_multi_aff_free(upma);
...
isl_union_pw_aff_zero_union_set(__isl_take isl_union_pw_aff *upa) {
  isl_union_set *zero;
 ...
 */
__isl_give isl_union_set *
isl_union_pw_aff_bind_id(__isl_take isl_union_pw_aff *upa,
                         __isl_take isl_id *id) {
...
static isl_stat pa_pb_pma(__isl_take isl_pw_multi_aff *pma, void *user) {
  ...
  if (!isl_space_tuple_is_equal(data->pa->dim, isl_dim_in, pma->dim,
                                isl_dim_out)) {
    isl_pw_multi_aff_free(pma);
    return isl_stat_ok;
  }
...
static isl_stat upa_pb_upma(__isl_take isl_pw_aff *pa, void *user) {
  ...
  data->pa = pa;
  r = isl_union_pw_multi_aff_foreach_pw_multi_aff(data->upma, &pa_pb_pma, data);
  isl_pw_aff_free(pa);
...
__isl_give isl_union_pw_aff *isl_union_pw_aff_pullback_union_pw_multi_aff(
    __isl_take isl_union_pw_aff *upa, __isl_take isl_union_pw_multi_aff *upma) {
...
isl_multi_union_pw_aff_zero(__isl_take isl_space *space) {
  isl_bool params;
  isl_size dim;
  ...
  if (dim < 0)
    goto error;
  if (dim != 0)
    isl_die(isl_space_get_ctx(space), isl_error_invalid, "expecting 0D space",
            goto error);
 ...
 */
__isl_give isl_multi_union_pw_aff *
isl_multi_union_pw_aff_from_multi_aff(__isl_take isl_multi_aff *ma) {
  isl_multi_pw_aff *mpa;
 ...
 */
__isl_give isl_multi_union_pw_aff *
isl_multi_union_pw_aff_from_multi_pw_aff(__isl_take isl_multi_pw_aff *mpa) {
  int i;
  isl_size n;
  isl_space *space;
...
isl_multi_union_pw_aff_from_union_pw_multi_aff(
  ...
  if (n == 0)
    isl_die(isl_union_pw_multi_aff_get_ctx(upma), isl_error_invalid,
            "cannot extract range space from empty input", goto error);
 ...
 */
__isl_give isl_multi_union_pw_aff *
isl_multi_union_pw_aff_from_union_map(__isl_take isl_union_map *umap) {
  isl_union_pw_multi_aff *upma;
...
isl_multi_union_pw_aff_multi_val_on_domain_aligned(
  ...
  for (i = 0; i < n; ++i) {
    ...
    v = isl_multi_val_get_val(mv, i);
    upa = isl_union_pw_aff_val_on_domain(isl_union_set_copy(domain), v);
    mupa = isl_multi_union_pw_aff_set_union_pw_aff(mupa, i, upa);
  }
  if (isl_multi_union_pw_aff_has_explicit_domain(mupa))
 ...
 */
__isl_give isl_multi_union_pw_aff *
isl_multi_union_pw_aff_multi_val_on_domain(__isl_take isl_union_set *domain,
                                           __isl_take isl_multi_val *mv) {
  isl_bool equal_params;
  ...
  domain = isl_union_set_align_params(domain, isl_multi_val_get_space(mv));
 ...
 */
__isl_give isl_multi_union_pw_aff *
isl_multi_union_pw_aff_multi_aff_on_domain(__isl_take isl_union_set *domain,
                                           __isl_take isl_multi_aff *ma) {
  isl_pw_multi_aff *pma;
...
isl_multi_union_pw_aff_pw_multi_aff_on_domain_aligned(
  ...
  for (i = 0; i < n; ++i) {
    ...
    pa = isl_pw_multi_aff_get_pw_aff(pma, i);
    upa = isl_union_pw_aff_pw_aff_on_domain(isl_union_set_copy(domain), pa);
    mupa = isl_multi_union_pw_aff_set_union_pw_aff(mupa, i, upa);
  }
  if (isl_multi_union_pw_aff_has_explicit_domain(mupa))
    mupa = isl_multi_union_pw_aff_intersect_domain(mupa,
...
isl_multi_union_pw_aff_pw_multi_aff_on_domain(
  ...
  if (equal_params < 0)
    goto error;
  if (equal_params)
    return isl_multi_union_pw_aff_pw_multi_aff_on_domain_aligned(domain, pma);
  domain = isl_union_set_align_params(domain, isl_pw_multi_aff_get_space(pma));
  pma = isl_pw_multi_aff_align_params(pma, isl_union_set_get_space(domain));
  return isl_multi_union_pw_aff_pw_multi_aff_on_domain_aligned(domain, pma);
error:
  isl_union_set_free(domain);
  isl_pw_multi_aff_free(pma);
 ...
 */
__isl_give isl_union_set *
isl_multi_union_pw_aff_zero_union_set(__isl_take isl_multi_union_pw_aff *mupa) {
  int i;
  isl_size n;
  isl_union_pw_aff *upa;
  isl_union_set *zero;
...
isl_union_map_from_multi_union_pw_aff(__isl_take isl_multi_union_pw_aff *mupa) {
  int i;
  isl_size n;
  isl_space *space;
  isl_union_map *umap;
...
static isl_stat reset_range_space(__isl_take isl_pw_multi_aff *pma,
  ...
  space = isl_space_domain(space);
  space =
      isl_space_extend_domain_with_range(space, isl_space_copy(data->range));
  pma = isl_pw_multi_aff_reset_space(pma, space);
  data->res = isl_union_pw_multi_aff_add_pw_multi_aff(data->res, pma);
 ...
 */
static __isl_give isl_multi_union_pw_aff *
mupa_intersect_range_0D(__isl_take isl_multi_union_pw_aff *mupa,
 ...
 */
__isl_give isl_multi_union_pw_aff *
isl_multi_union_pw_aff_intersect_range(__isl_take isl_multi_union_pw_aff *mupa,
                                       __isl_take isl_set *range) {
  isl_union_pw_multi_aff *upma;
  isl_union_set *domain;
  isl_space *space;
  ...
  match =
      isl_space_tuple_is_equal(mupa->space, isl_dim_set, space, isl_dim_set);
  isl_space_free(space);
  if (match < 0)
    goto error;
  if (!match)
 ...
 */
__isl_give isl_union_set *
isl_multi_union_pw_aff_domain_0D(__isl_take isl_multi_union_pw_aff *mupa) {
  isl_union_set *dom;
 ...
 */
__isl_give isl_union_set *
isl_multi_union_pw_aff_domain(__isl_take isl_multi_union_pw_aff *mupa) {
  int i;
  isl_size n;
  isl_union_pw_aff *upa;
  isl_union_set *dom;
 ...
 */
static __isl_give isl_union_pw_aff *
multi_union_pw_aff_apply_aff(__isl_take isl_multi_union_pw_aff *mupa,
                             __isl_take isl_aff *aff) {
  int i;
  isl_size n_in, n_div;
  isl_union_pw_aff *upa;
  ...
  for (i = 0; i < n_div; ++i) {
    ...
    div = isl_aff_get_div(aff, i);
    upa_i =
        multi_union_pw_aff_apply_aff(isl_multi_union_pw_aff_copy(mupa), div);
 ...
 */
__isl_give isl_union_pw_aff *
isl_multi_union_pw_aff_apply_aff(__isl_take isl_multi_union_pw_aff *mupa,
                                 __isl_take isl_aff *aff) {
  isl_size dim;
  isl_space *space1, *space2;
  isl_bool equal;

  mupa = isl_multi_union_pw_aff_align_params(mupa, isl_aff_get_space(aff));
  ...
  if (equal < 0)
    goto error;
  if (!equal)
    isl_die(isl_aff_get_ctx(aff), isl_error_invalid, "spaces don't match",
            goto error);
  dim = isl_aff_dim(aff, isl_dim_in);
  if (dim < 0)
    goto error;
  if (dim == 0)
    isl_die(isl_aff_get_ctx(aff), isl_error_invalid, "cannot determine domains",
            goto error);
 ...
 */
static __isl_give isl_multi_union_pw_aff *
mupa_apply_multi_aff_0D(__isl_take isl_multi_union_pw_aff *mupa,
                        __isl_take isl_multi_aff *ma) {
  isl_union_set *dom;
 ...
 */
__isl_give isl_multi_union_pw_aff *
isl_multi_union_pw_aff_apply_multi_aff(__isl_take isl_multi_union_pw_aff *mupa,
                                       __isl_take isl_multi_aff *ma) {
  isl_space *space1, *space2;
  ...
  mupa = isl_multi_union_pw_aff_align_params(mupa, isl_multi_aff_get_space(ma));
  ma = isl_multi_aff_align_params(ma, isl_multi_union_pw_aff_get_space(mupa));
  n_in = isl_multi_aff_dim(ma, isl_dim_in);
  n_out = isl_multi_aff_dim(ma, isl_dim_out);
  ...
  if (equal < 0)
    goto error;
  if (!equal)
    isl_die(isl_multi_aff_get_ctx(ma), isl_error_invalid, "spaces don't match",
            goto error);
  if (n_in == 0)
  ...
  for (i = 0; i < n_out; ++i) {
    ...
    upa = multi_union_pw_aff_apply_aff(isl_multi_union_pw_aff_copy(mupa), aff);
 ...
 */
static __isl_give isl_union_pw_aff *
isl_multi_union_pw_aff_apply_pw_aff_0D(__isl_take isl_multi_union_pw_aff *mupa,
                                       __isl_take isl_pw_aff *pa) {
  isl_union_set *dom;
 ...
 */
__isl_give isl_union_pw_aff *
isl_multi_union_pw_aff_apply_pw_aff(__isl_take isl_multi_union_pw_aff *mupa,
                                    __isl_take isl_pw_aff *pa) {
  int i;
  isl_bool equal;
  isl_size n_in;
  ...
  mupa = isl_multi_union_pw_aff_align_params(mupa, isl_pw_aff_get_space(pa));
  pa = isl_pw_aff_align_params(pa, isl_multi_union_pw_aff_get_space(mupa));
  if (!mupa || !pa)
    goto error;
  ...
  if (equal < 0)
    goto error;
  if (!equal)
    isl_die(isl_pw_aff_get_ctx(pa), isl_error_invalid, "spaces don't match",
            goto error);
 ...
 */
static __isl_give isl_multi_union_pw_aff *
mupa_apply_pw_multi_aff_0D(__isl_take isl_multi_union_pw_aff *mupa,
                           __isl_take isl_pw_multi_aff *pma) {
  isl_union_set *dom;
...
__isl_give isl_multi_union_pw_aff *isl_multi_union_pw_aff_apply_pw_multi_aff(
    __isl_take isl_multi_union_pw_aff *mupa, __isl_take isl_pw_multi_aff *pma) {
  isl_space *space1, *space2;
  isl_multi_union_pw_aff *res;
  isl_bool equal;
  int i;
  ...
  for (i = 0; i < n_out; ++i) {
    ...
    pa = isl_pw_multi_aff_get_pw_aff(pma, i);
    upa = isl_multi_union_pw_aff_apply_pw_aff(isl_multi_union_pw_aff_copy(mupa),
                                              pa);
    res = isl_multi_union_pw_aff_set_union_pw_aff(res, i, upa);
  }
 ...
 */
static __isl_give isl_multi_union_pw_aff *
preimage_explicit_domain(__isl_take isl_multi_union_pw_aff *mupa,
  ...
  if (is_params)
    mupa->u.dom = isl_union_set_intersect_params(
        mupa->u.dom, isl_union_set_params(isl_union_pw_multi_aff_domain(upma)));
  else
    mupa->u.dom = isl_union_set_preimage_union_pw_multi_aff(mupa->u.dom, upma);
  if (!mupa->u.dom)
    return isl_multi_union_pw_aff_free(mupa);
  return mupa;
}
...
isl_multi_union_pw_aff_pullback_union_pw_multi_aff(
  ...
  mupa = isl_multi_union_pw_aff_align_params(
      mupa, isl_union_pw_multi_aff_get_space(upma));
  upma = isl_union_pw_multi_aff_align_params(
      upma, isl_multi_union_pw_aff_get_space(mupa));
  mupa = isl_multi_union_pw_aff_cow(mupa);
  n = isl_multi_union_pw_aff_dim(mupa, isl_dim_set);
  if (n < 0 || !upma)
    goto error;
  ...
  for (i = 0; i < n; ++i) {
    ...
    upa = isl_multi_union_pw_aff_get_union_pw_aff(mupa, i);
    upa = isl_union_pw_aff_pullback_union_pw_multi_aff(
        upa, isl_union_pw_multi_aff_copy(upma));
    mupa = isl_multi_union_pw_aff_set_union_pw_aff(mupa, i, upa);
  }
...
__isl_give isl_multi_pw_aff *isl_multi_union_pw_aff_extract_multi_pw_aff(
  ...
  space_mpa =
      isl_space_map_from_domain_and_range(isl_space_copy(space), space_mpa);
  ...
  for (i = 0; i < n; ++i) {
    ...
    upa = isl_multi_union_pw_aff_get_union_pw_aff(mupa, i);
    pa = isl_union_pw_aff_extract_pw_aff(upa, isl_space_copy(space));
    mpa = isl_multi_pw_aff_set_pw_aff(mpa, i, pa);
    isl_union_pw_aff_free(upa);
  }
 ...
 */
static isl_bool
isl_union_pw_multi_aff_un_op_filter_drop_user(__isl_take isl_pw_multi_aff *pma,
                                              void *user) {
 ...
 */
static __isl_give isl_pw_multi_aff *
isl_union_pw_multi_aff_un_op_drop_user(__isl_take isl_pw_multi_aff *pma,
                                       void *user) {
  struct isl_union_pw_multi_aff_un_op_control *control = user;
...
static isl_stat isl_aff_check_has_domain_space(__isl_keep isl_aff *aff,
  ...
  if (ok < 0)
    return isl_stat_error;
  if (!ok)
    isl_die(isl_aff_get_ctx(aff), isl_error_invalid, "incompatible spaces",
            return isl_stat_error);
  return isl_stat_ok;
}
