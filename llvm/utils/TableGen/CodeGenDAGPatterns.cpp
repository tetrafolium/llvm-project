...
std::string TreePredicateFn::getPredCode() const {
  ...
  if (isAtomic()) {
    if (getMemoryVT() == nullptr && !isAtomicOrderingMonotonic() &&
        getAddressSpaces() == nullptr && !isAtomicOrderingAcquire() &&
...
bool SDTypeConstraint::ApplyTypeConstraint(TreePatternNode *N,
  ...
  case SDTCisSameAs: {
    ...
    return NodeToApply->UpdateNodeType(ResNo, OtherNode->getExtType(OResNo),
                                       TP) |
           OtherNode->UpdateNodeType(OResNo, NodeToApply->getExtType(ResNo),
                                     TP);
  }
...
TreePatternNodePtr TreePattern::ParseTreePattern(Init *TheInit,
  ...
  TreePatternNodePtr Result = std::make_shared<TreePatternNode>(
...
void CodeGenDAGPatterns::ParseInstructions() {
  ...
  for (Record *Instr : Instrs) {
    ...
    if (!LI || LI->empty() || hasNullFragReference(LI)) {
      ...
      Instructions.insert(
          std::make_pair(Instr, DAGInstruction(Results, Operands, ImpResults)));
      continue; // no pattern.
    }
...
void CodeGenDAGPatterns::ExpandHwModeBasedTypes() {
  ...
  auto AppendPattern = [this, &ModeChecks](PatternToMatch &P, unsigned Mode) {
    ...
    PatternsToMatch.emplace_back(
        P.getSrcRecord(), Preds, std::move(NewSrc), std::move(NewDst),
        P.getDstRegs(), P.getAddedComplexity(), Record::getNewUID(), Mode);
  };
