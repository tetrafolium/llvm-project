...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), identity)(__isl_take isl_space *space) {
  int i;
  isl_size n_in, n_out;
  isl_local_space *ls;
  MULTI(BASE) * multi;
  ...
  for (i = 0; i < n_out; ++i) {
    EL *el;
    el = FN(EL, var_on_domain)(isl_local_space_copy(ls), isl_dim_set, i);
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), identity_on_domain_space)(__isl_take isl_space *space) {
 ...
 */
__isl_give MULTI(BASE) *
    FN(FN(MULTI(BASE), identity_multi), BASE)(__isl_take MULTI(BASE) * multi) {
  isl_space *space;
