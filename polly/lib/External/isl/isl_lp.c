...
enum isl_lp_result isl_tab_solve_lp(__isl_keep isl_basic_map *bmap,
                                    int maximize, isl_int *f, isl_int denom,
...
enum isl_lp_result isl_basic_set_solve_lp(__isl_keep isl_basic_set *bset,
                                          ...
                                          isl_int *opt, isl_int *opt_denom,
                                          __isl_give isl_vec **sol) {
  return isl_basic_map_solve_lp(bset_to_bmap(bset), max, f, d, opt, opt_denom,
                                sol);
}
...
enum isl_lp_result isl_map_solve_lp(__isl_keep isl_map *map, int max,
  ...
  res = isl_basic_map_solve_lp(map->p[0], max, f, d, opt, opt_denom, sol);
  ...
  for (i = 1; i < map->n; ++i) {
    ...
    res_i = isl_basic_map_solve_lp(map->p[i], max, f, d, &opt_i,
...
enum isl_lp_result isl_set_solve_lp(__isl_keep isl_set *set, int max,
                                    ...
                                    isl_int *opt_denom,
                                    __isl_give isl_vec **sol) {
  return isl_map_solve_lp(set_to_map(set), max, f, d, opt, opt_denom, sol);
}
 ...
 */
static __isl_give isl_val *basic_set_opt_lp(__isl_keep isl_basic_set *bset,
  ...
  if (!res)
    return NULL;
  lp_res = isl_basic_set_solve_lp(bset, max, obj->v->el + 1, obj->v->el[0],
                                  &res->n, &res->d, NULL);
  if (lp_res == isl_lp_ok)
...
isl_basic_set_opt_lp_val_aligned(__isl_keep isl_basic_set *bset, int max,
  ...
  if (!isl_space_is_equal(bset->dim, obj->ls->dim))
    isl_die(ctx, isl_error_invalid, "spaces don't match", return NULL);
 ...
 */
static __isl_give isl_val *
isl_basic_set_opt_lp_val(__isl_keep isl_basic_set *bset, int max,
                         __isl_keep isl_aff *obj) {
  isl_bool equal;
  isl_val *res;
