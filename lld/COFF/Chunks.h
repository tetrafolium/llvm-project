...
public:
    ...
    void setOutputSectionIdx(uint16_t o) {
        osidx = o;
    }
    uint16_t getOutputSectionIdx() const {
        return osidx;
    }
...
public:
    ...
    virtual uint32_t getOutputCharacteristics() const {
        return 0;
    }
    ...
    // Returns a human-readable name of this chunk. Chunks are unnamed chunks of
    // bytes, so this is used only for logging or debugging.
    virtual StringRef getDebugName() const {
        return "";
    }
...
public:
    ...
    public:
        ...
        bool operator==(const AssociatedIterator &r) const {
            return cur == r.cur;
        }
        // FIXME: Wrong const-ness, but it makes filter ranges work.
        SectionChunk &operator*() const {
            return *cur;
        }
        SectionChunk &operator*() {
            return *cur;
        }
