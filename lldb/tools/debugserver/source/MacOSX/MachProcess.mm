...
#if defined(WITH_BKS) || defined(WITH_FBS)
static void SplitEventData(const char *data,
...
static void SetBKSError(NSInteger error_code, std::string error_description,
                        DNBError &error) {
  ...
  for (std::string value : values) {
    if (value.compare("BackgroundContentFetching") == 0) {
      ...
      [options setObject:@YES
                  forKey:BKSOpenApplicationOptionKeyActivateSuspended];
      found_one = true;
    } else {
...
static void SetFBSError(NSInteger error_code, std::string error_description,
                        DNBError &error) {
...
static bool FBSAddEventDataToOptions(NSMutableDictionary *options,
  ...
  for (std::string value : values) {
    if (value.compare("BackgroundContentFetching") == 0) {
      ...
      DNBLog("Setting ActivateSuspended key in options dictionary.");
      [options setObject:@YES
                  forKey:FBSOpenApplicationOptionKeyActivateSuspended];
      found_one = true;
#if WITH_CAROUSEL
...
nub_bool_t MachProcess::SyncThreadState(nub_thread_t tid) {
  ...
  DNBLogThreadedIf(
      LOG_THREAD,
      "thread = 0x%8.8" PRIx32 " calling thread_abort_safely (tid) => %u "
      "(GetGPRState() for stop_count = %u)",
      thread_sp->MachPortNumber(), kret, thread_sp->Process()->StopCount());
...
void MachProcess::SetState(nub_state_t new_state) {
  ...
  // Scope for mutex locker
  {
    ...
    if (old_state == eStateExited) {
      DNBLogThreadedIf(LOG_PROCESS,
                       "MachProcess::SetState(%s) ignoring new "
      ...
      DNBLogThreadedIf(LOG_PROCESS,
                       "MachProcess::SetState(%s) upating state (previous "
                       "state was %s), event_mask = 0x%8.8x",
                       DNBStateAsString(new_state), DNBStateAsString(old_state),
                       event_mask);
  ...
  if (IsRunning(state)) {
    if (m_sent_interrupt_signo == 0) {
      ...
      if (Signal(m_sent_interrupt_signo)) {
        ...
        DNBLogThreadedIf(LOG_PROCESS,
                         "MachProcess::Interrupt() - failed to "
                         "send %i signal to interrupt process",
                         m_sent_interrupt_signo);
      }
    } else {
      DNBLogThreadedIf(LOG_PROCESS,
                       "MachProcess::Interrupt() - previously "
...
void MachProcess::PrivateResume() {
  ...
  if (m_auto_resume_signo)
    DNBLogThreadedIf(LOG_PROCESS,
                     "MachProcess::PrivateResume() - task 0x%x "
...
pid_t MachProcess::AttachForDebug(pid_t pid, bool unmask_signals, char *err_str,
  ...
  if (pid != 0) {
    ...
    if (err.Success()) {
      ...
      DNBLogError(
          "MachProcess::AttachForDebug error: failed to attach to pid %d", pid);

      struct kinfo_proc kinfo;
      int mib[] = {CTL_KERN, KERN_PROC, KERN_PROC_PID, pid};
      size_t len = sizeof(struct kinfo_proc);
      if (sysctl(mib, sizeof(mib) / sizeof(mib[0]), &kinfo, &len, NULL, 0) ==
              0 &&
          len > 0) {
...
const void *MachProcess::PrepareForAttach(const char *path,
  ...
  if (launch_flavor == eLaunchFlavorFBS) {
    ...
    DNBLogThreadedIf(LOG_PROCESS,
                     "Calling BKSSystemService openApplication: "
  ...
  if (launch_flavor == eLaunchFlavorSpringBoard) {
    ...
    DNBLogThreadedIf(LOG_PROCESS,
                     "SBSLaunchApplicationForDebugging ( @\"%s\" "
                     ", NULL, NULL, NULL, @\"%s\", @\"%s\", "
  ...
  if (m_pid == INVALID_NUB_PROCESS) {
    ...
    if (launch_flavor == eLaunchFlavorPosixSpawn) {
      ...
      if (err == 0) {
        ...
        DNBLogThreadedIf(LOG_PROCESS,
                         "error: failed to attach to spawned pid "
                         "%d (err = %i, errno = %i (%s))",
