...
public:
    ...
    bool isStatic() const {
        return properties & MP_Static;
    }

    // Returns true if this is a private method.
    bool isPrivate() const {
        return properties & MP_Private;
    }
...
protected:
    ...
    void forAllMethods(llvm::function_ref<void(const OpMethod &)> func) const {
        ...
        llvm::for_each(constructors, [&](ConsRef ptr) {
            func(*ptr);
        });
        llvm::for_each(methods, [&](MethodRef ptr) {
            func(*ptr);
        });
    }
