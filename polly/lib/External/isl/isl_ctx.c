...
int isl_ctx_next_operation(isl_ctx *ctx) {
  ...
  if (ctx->max_operations && ctx->operations >= ctx->max_operations)
    isl_die(ctx, isl_error_quota, "maximal number of operations exceeded",
            return -1);
  ctx->operations++;
  return 0;
}
...
static struct isl_options *find_nested_options(struct isl_args *args, void *opt,
  ...
  for (i = 0; args->args[i].type != isl_arg_end; ++i) {
    ...
    options = find_nested_options(arg->u.child.child, child, wanted);
    if (options)
      return options;
  }
...
void isl_ctx_ref(struct isl_ctx *ctx) { ctx->ref++; }
...
void isl_ctx_free(struct isl_ctx *ctx) {
  ...
  if (ctx->ref != 0)
    isl_die(ctx, isl_error_invalid,
            "isl_ctx freed, but some objects still reference it", return );
 ...
 */
int isl_ctx_last_error_line(isl_ctx *ctx) { return ctx ? ctx->error_line : -1; }
...
int isl_ctx_aborted(isl_ctx *ctx) { return ctx ? ctx->abort : -1; }
