...
isl_bool FN(MULTI(BASE), involves_dims)(__isl_keep MULTI(BASE) * multi,
  ...
  if (FN(MULTI(BASE), has_explicit_domain)(multi))
    return FN(MULTI(BASE), involves_explicit_domain_dims)(multi, type, first,
                                                          n);
...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), insert_dims)(__isl_take MULTI(BASE) * multi,
                                 enum isl_dim_type type, unsigned first,
                                 unsigned n) {
  int i;
  ...
  if (!multi->space)
    return FN(MULTI(BASE), free)(multi);
  if (FN(MULTI(BASE), has_explicit_domain)(multi))
    multi = FN(MULTI(BASE), insert_explicit_domain_dims)(multi, type, first, n);
  if (!multi)
    return NULL;

  for (i = 0; i < multi->n; ++i) {
    multi->u.p[i] = FN(EL, insert_dims)(multi->u.p[i], type, first, n);
    if (!multi->u.p[i])
      return FN(MULTI(BASE), free)(multi);
  }
...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), add_dims)(__isl_take MULTI(BASE) * multi,
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), project_domain_on_params)(__isl_take MULTI(BASE) * multi) {
  isl_size n;
  isl_bool involves;
  isl_space *space;
