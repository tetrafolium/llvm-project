...
InstructionContext RecognizableInstr::insnContext() const {
  ...
  if (Encoding == X86Local::EVEX) {
    ...
    if (HasREX_WPrefix &&
        (OpSize == X86Local::OpSize16 || OpPrefix == X86Local::PD))
...
#define MAP(from, to) case X86Local::MRM_##from:
  ...
  if (Form == X86Local::AddRegFrm || Form == X86Local::MRMSrcRegCC ||
    ...
    for (currentOpcode = opcodeToSet; currentOpcode < opcodeToSet + Count;
         ++currentOpcode)
      tables.setTableFields(
          *opcodeType, insnContext(), currentOpcode, *filter, UID, Is32Bit,
          OpPrefix == 0, IgnoresVEX_L || EncodeRC, IgnoresVEX_W, AddressSize);
  } else {
