...
get_prefix(__isl_keep isl_schedule_node *node) {
...
static isl_stat merge_pair(int n, struct ppcg_grouping_leaf leaves[], int pos) {
  ...
  leaves[pos].domain =
      isl_union_set_union(leaves[pos].domain, leaves[pos + 1].domain);
  leaves[pos].list =
      isl_union_set_list_concat(leaves[pos].list, leaves[pos + 1].list);
  leaves[pos].prefix = isl_multi_union_pw_aff_union_add(leaves[pos].prefix,
                                                        leaves[pos + 1].prefix);
  for (i = pos + 1; i + 1 < n; ++i)
    leaves[i] = leaves[i + 1];
...
static int merge_leaves(int n, struct ppcg_grouping_leaf leaves[],
  ...
  for (i = n - 1; i >= 0; --i) {
    ...
    dep_i = isl_union_map_intersect_domain(
        dep_i, isl_union_set_copy(leaves[i].domain));
    dep_i = isl_union_map_intersect_range(
        dep_i, isl_union_set_copy(leaves[i + 1].domain));
    data.merge = 0;
    data.src = &leaves[i];
    data.dst = &leaves[i + 1];
...
schedule_from_domain_and_list(__isl_keep isl_union_set *domain,
                              __isl_keep isl_union_set_list *list) {
  isl_schedule *schedule;
...
static isl_union_pw_multi_aff *group_contraction_from_prefix_and_domain(
    struct ppcg_grouping *grouping, __isl_keep isl_multi_union_pw_aff *prefix,
 ...
 */
static isl_stat add_groups(struct ppcg_grouping *grouping, int n,
                           struct ppcg_grouping_leaf leaves[]) {
  int i;
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (n_leaf <= 1)
      continue;
    schedule = schedule_from_domain_and_list(leaves[i].domain, leaves[i].list);
    upma = group_contraction_from_prefix_and_domain(grouping, leaves[i].prefix,
                                                    leaves[i].domain);
    ...
    if (!grouping->domain || !grouping->contraction || !grouping->schedule)
      return isl_stat_error;
  }
 ...
 */
static isl_stat group_subsequence(__isl_keep isl_schedule_node *node, int first,
                                  int n, struct ppcg_grouping *grouping) {
  int n_merge;
  struct ppcg_grouping_leaf *leaves;
  ...
  if (n_merge >= 0 && n_merge < n && add_groups(grouping, n_merge, leaves) < 0)
    return isl_stat_error;
...
static isl_bool detect_groups(__isl_keep isl_schedule_node *node, void *user) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (first >= 0 && type != isl_schedule_node_leaf) {
      if (group_subsequence(node, first, i - first, grouping) < 0)
        return isl_bool_error;
      first = -1;
    }
    if (first < 0 && type == isl_schedule_node_leaf)
...
ppcg_compute_schedule(__isl_take isl_schedule_constraints *sc,
  ...
  grouping.group_id = 0;
  if (isl_schedule_foreach_schedule_node_top_down(schedule, &detect_groups,
                                                  &grouping) < 0)
    goto error;
