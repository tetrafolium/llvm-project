...
AffineExpr AffineExprMatcher::matched() {
    return storage->matched;
}
...
void SDBMExpr::print(raw_ostream &os) const {
    struct Printer : public SDBMVisitor<Printer> {
        ...
        void visitDim(SDBMDimExpr expr) {
            prn << 'd' << expr.getPosition();
        }
        void visitSymbol(SDBMSymbolExpr expr) {
            prn << 's' << expr.getPosition();
        }
...
struct SDBMNegator : public SDBMVisitor<SDBMNegator, SDBMExpr> {
    ...
    //  -(x) = -x
    SDBMExpr visitDirect(SDBMDirectExpr expr) {
        return SDBMNegExpr::get(expr);
    }
    // A negation expression is unwrapped.
    //  -(-x) = x
    SDBMExpr visitNeg(SDBMNegExpr expr) {
        return expr.getVar();
    }
...
Optional<SDBMExpr> SDBMExpr::tryConvertAffineExpr(AffineExpr affine) {
    struct Converter : public AffineExprVisitor<Converter, SDBMExpr> {
        ...
        SDBMExpr visitFloorDivExpr(AffineBinaryOpExpr expr) {
            return {};
        }
        SDBMExpr visitCeilDivExpr(AffineBinaryOpExpr expr) {
            return {};
        }
