...
public:
    ...
    ArrayRef<const char *> llvmOptions() const {
        return _llvmOptions;
    }

    /// \name Methods used by Drivers to configure TargetInfo
    /// @{
    void setOutputPath(StringRef str) {
        _outputPath = str;
    }
    ...
    void setDeadStripping(bool enable) {
        _deadStrip = enable;
    }
    void setGlobalsAreDeadStripRoots(bool v) {
        _globalsAreDeadStripRoots = v;
    }
    ...
    virtual uint64_t getNextOrdinalAndIncrement() const {
        return _nextOrdinal++;
    }
