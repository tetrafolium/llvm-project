...
public:
    ...
    void SetUseValueProfileMask(uint32_t VPMask) {
        UseValueProfileMask = VPMask;
    }
    void SetPrintNewPCs(bool P) {
        DoPrintNewPCs = P;
    }
    void SetPrintNewFuncs(size_t P) {
        NumPrintNewFuncs = P;
    }
    ...
    bool PcIsFuncEntry(const PCTableEntry *TE) {
        return TE->PCFlags & 1;
    }
...
private:
    ...
    struct Module {
        ...
        uint8_t *Stop()  {
            return Regions[NumRegions - 1].Stop;
        }
        size_t Size()   {
            return Stop() - Start();
        }
