...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), move_dims)(__isl_take MULTI(BASE) * multi,
  ...
  if (n == 0 && !isl_space_is_named_or_nested(multi->space, src_type) &&
  ...
  if (dst_type == isl_dim_div || src_type == isl_dim_div)
    isl_die(FN(MULTI(BASE), get_ctx)(multi), isl_error_invalid,
            "cannot move divs", return FN(MULTI(BASE), free)(multi));
  ...
  multi->space = isl_space_move_dims(multi->space, dst_type, dst_pos, src_type,
                                     src_pos, n);
  if (!multi->space)
    return FN(MULTI(BASE), free)(multi);
  if (FN(MULTI(BASE), has_explicit_domain)(multi))
    multi = FN(MULTI(BASE), move_explicit_domain_dims)(multi, dst_type, dst_pos,
                                                       src_type, src_pos, n);
  if (!multi)
    return NULL;

  for (i = 0; i < multi->n; ++i) {
    multi->u.p[i] = FN(EL, move_dims)(multi->u.p[i], dst_type, dst_pos,
                                      src_type, src_pos, n);
    if (!multi->u.p[i])
      return FN(MULTI(BASE), free)(multi);
  }
