...
void ARMTargetLowering::addTypeForNEON(MVT VT, MVT PromotedLdStVT,
    ...
    if (!VT.isFloatingPoint())
        for (auto Opcode : {
                    ISD::SADDSAT, ISD::UADDSAT, ISD::SSUBSAT, ISD::USUBSAT
                })
            setOperationAction(Opcode, VT, Legal);
}
...
SDValue ARMTargetLowering::ReconstructShuffle(SDValue Op,
    ...
    struct ShuffleSourceInfo {
        ...
        bool operator ==(SDValue OtherVec) {
            return Vec == OtherVec;
        }
    };
...
static SDValue PerformVECREDUCE_ADDCombine(SDNode *N, SelectionDAG &DAG,
    ...
    auto IsVADDV = [&](MVT RetTy, unsigned ExtendCode, ArrayRef<MVT> ExtTypes) {
        ...
        if (llvm::any_of(ExtTypes, [&A](MVT Ty) {
        return A.getValueType() == Ty;
        }))
        return A;
        return SDValue();
    };
