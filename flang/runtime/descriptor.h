...
class Dimension {
public:
    SubscriptValue LowerBound() const {
        return raw_.lower_bound;
    }
    SubscriptValue Extent() const {
        return raw_.extent;
    }
    SubscriptValue UpperBound() const {
        return LowerBound() + Extent() - 1;
    }
    SubscriptValue ByteStride() const {
        return raw_.sm;
    }
...
public:
    ...
    const typeInfo::DerivedType *derivedType() const {
        return derivedType_;
    }
    ...
    const std::uint64_t &flags() const {
        return flags_;
    }
...
public:
    ...
    const ISO::CFI_cdesc_t &raw() const {
        return raw_;
    }
    std::size_t ElementBytes() const {
