...
static __isl_give isl_vertices *
compute_chambers(__isl_take isl_basic_set *bset,
...
static __isl_give isl_vertices *
vertices_from_list(__isl_keep isl_basic_set *bset, int n_vertices,
...
static isl_bool can_select(__isl_keep isl_basic_set *bset, int level,
                           struct isl_tab *tab, __isl_keep isl_mat *facets,
 ...
 */
static __isl_give isl_vertices *
lower_dim_vertices(__isl_keep isl_basic_set *bset) {
  isl_morph *morph;
  isl_vertices *vertices;
 ...
 */
__isl_give isl_vertices *
isl_basic_set_compute_vertices(__isl_keep isl_basic_set *bset) {
  struct isl_tab *tab;
  int level;
  int init;
  isl_size nvar;
  ...
  while (level >= 0) {
    if (level >= bset->n_ineq || (!init && selection[level] != SELECTED)) {
      --level;
      init = 0;
      continue;
    }
    if (init) {
      ...
      ok = can_select(bset, level, tab, facets, selected, selection);
      if (ok < 0)
        goto error;
      if (ok) {
...
static __isl_give isl_vertices *
vertices_add_chambers(__isl_take isl_vertices *vertices, int n_chambers,
...
static isl_bool can_intersect(struct isl_tab *tab,
  ...
  if (bset->n_eq > 0)
    isl_die(isl_basic_set_get_ctx(bset), isl_error_internal,
            "expecting full-dimensional input", return isl_bool_error);
...
static int add_chamber(struct isl_chamber_list **list,
                       __isl_keep isl_vertices *vertices, struct isl_tab *tab,
                       int *selection) {
  int n_frozen;
  int i, j;
  int n_vertices = 0;
...
compute_chambers(__isl_take isl_basic_set *bset,
  ...
  while (todo) {
    ...
    for (i = 0; i < vertices->n_vertices; ++i) {
      selection[i] = bset_covers_tab(vertices->v[i].dom, todo->tab);
      if (selection[i] < 0)
        goto error;
      if (!selection[i])
        continue;
...
static __isl_give isl_vertex *
isl_vertex_alloc(__isl_take isl_vertices *vertices, int id) {
  isl_ctx *ctx;
  isl_vertex *vertex;
...
static struct isl_tab *tab_for_shifted_cone(__isl_keep isl_basic_set *bset) {
  ...
  if (total < 0)
    return NULL;
  tab = isl_tab_alloc(bset->ctx, bset->n_eq + bset->n_ineq + 1, 1 + total, 0);
  if (!tab)
    return NULL;
...
isl_basic_set_interior_point(__isl_keep isl_basic_set *bset) {
  isl_vec *vec;
  struct isl_tab *tab;
 ...
 */
isl_stat isl_vertices_foreach_disjoint_cell(
...
isl_stat isl_cell_foreach_vertex(__isl_keep isl_cell *cell,
  ...
  for (i = 0; i < cell->n_vertices; ++i) {
    ...
    vertex = isl_vertex_alloc(isl_vertices_copy(cell->vertices), cell->ids[i]);
    if (!vertex)
      return isl_stat_error;
...
__isl_give isl_vertices *isl_morph_vertices(__isl_take isl_morph *morph,
  ...
  for (i = 0; i < vertices->n_vertices; ++i) {
    vertices->v[i].dom =
        isl_morph_basic_set(isl_morph_copy(param_morph), vertices->v[i].dom);
    vertices->v[i].vertex =
        isl_morph_basic_set(isl_morph_copy(morph), vertices->v[i].vertex);
  ...
  for (i = 0; i < vertices->n_chambers; ++i) {
    vertices->c[i].dom =
        isl_morph_basic_set(isl_morph_copy(param_morph), vertices->c[i].dom);
    if (!vertices->c[i].dom)
      goto error;
  }
 ...
 */
static isl_stat call_on_simplex(__isl_keep isl_cell *cell, int *simplex_ids,
                                int n_simplex, int *other_ids, int n_other,
                                isl_stat (*fn)(__isl_take isl_cell *simplex,
                                               void *user),
                                void *user) {
  int i;
  isl_ctx *ctx;
...
static int vertex_on_facet(__isl_keep isl_basic_set *vertex,
                           __isl_keep isl_basic_set *bset, int facet,
 ...
 */
static isl_stat
triangulate(__isl_keep isl_cell *cell, __isl_keep isl_vec *v, int *simplex_ids,
            int n_simplex, int *other_ids, int n_other,
  ...
  if (n_simplex + n_other == d + 1)
    return call_on_simplex(cell, simplex_ids, n_simplex, other_ids, n_other, fn,
                           user);
  ...
  for (i = 0; i < bset->n_ineq; ++i) {
    ...
    if (triangulate(cell, v, simplex_ids, n_simplex + 1, ids, k, fn, user) < 0)
      goto error;
  }
  free(ids);
...
isl_stat isl_cell_foreach_simplex(__isl_take isl_cell *cell,
  ...
  r = triangulate(cell, v, simplex_ids, 0, cell->ids, cell->n_vertices, fn,
                  user);
