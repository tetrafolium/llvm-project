...
static isl_stat FN(MULTI(BASE),
                   check_has_explicit_domain)(__isl_keep MULTI(BASE) * multi) {
  if (!multi)
 ...
 */
static __isl_keep DOM *
FN(MULTI(BASE), peek_explicit_domain)(__isl_keep MULTI(BASE) * multi) {
 ...
 */
static __isl_give DOM *FN(MULTI(BASE),
                          get_explicit_domain)(__isl_keep MULTI(BASE) * multi) {
...
static __isl_give MULTI(BASE) *
    FN(MULTI(BASE), set_explicit_domain)(__isl_take MULTI(BASE) * multi,
                                         __isl_take DOM *dom) {
  if (FN(MULTI(BASE), check_has_explicit_domain)(multi) < 0)
    goto error;
...
isl_stat FN(MULTI(BASE),
  ...
  if (equal < 0)
    return isl_stat_error;
  if (!equal)
    isl_die(FN(MULTI(BASE), get_ctx)(multi), isl_error_internal, "check failed",
            return isl_stat_error);
  return isl_stat_ok;
}
