...
static bool isDimOpValidSymbol(DimOp dimOp, Region *region) {
    ...
    [&](auto op) {
        return isMemRefSizeValidSymbol(op, i, region);
    })
    .Default([](Operation *) {
        return false;
    });
...
Region &AffineForOp::getLoopBody() {
    return region();
}
...
Region &AffineParallelOp::getLoopBody() {
    return region();
}
...
static void print(OpAsmPrinter &p, AffineParallelOp op) {
    ...
    bool elideSteps = llvm::all_of(steps, [](int64_t step) {
        return step == 1;
    });
    if (!elideSteps) {
        p << " step (";
