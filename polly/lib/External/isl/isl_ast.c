...
isl_ctx *
isl_ast_print_options_get_ctx(__isl_keep isl_ast_print_options *options) {
...
isl_ast_print_options_dup(__isl_keep isl_ast_print_options *options) {
  isl_ctx *ctx;
...
__isl_give isl_ast_print_options *
isl_ast_print_options_cow(__isl_take isl_ast_print_options *options) {
  if (!options)
    return NULL;
...
__isl_give isl_ast_print_options *
isl_ast_print_options_copy(__isl_keep isl_ast_print_options *options) {
  if (!options)
    return NULL;
...
isl_ast_print_options_free(__isl_take isl_ast_print_options *options) {
  if (!options)
    return NULL;
...
__isl_give isl_ast_expr *isl_ast_expr_dup(__isl_keep isl_ast_expr *expr) {
  ...
  case isl_ast_expr_op:
    dup = isl_ast_expr_alloc_op(ctx, expr->u.op.op, expr->u.op.n_arg);
    if (!dup)
      return NULL;
    for (i = 0; i < expr->u.op.n_arg; ++i)
      dup->u.op.args[i] = isl_ast_expr_copy(expr->u.op.args[i]);
    break;
...
isl_ast_expr_op_get_type(__isl_keep isl_ast_expr *expr) {
  if (!expr)
    return isl_ast_expr_op_error;
  if (expr->type != isl_ast_expr_op)
    isl_die(isl_ast_expr_get_ctx(expr), isl_error_invalid,
            "expression not an operation", return isl_ast_expr_op_error);
  return expr->u.op.op;
}
 ...
 */
enum isl_ast_expr_op_type
isl_ast_expr_get_op_type(__isl_keep isl_ast_expr *expr) {
...
isl_bool isl_ast_expr_is_equal(__isl_keep isl_ast_expr *expr1,
  ...
  case isl_ast_expr_op:
    ...
    for (i = 0; i < expr1->u.op.n_arg; ++i) {
      isl_bool equal;
      equal = isl_ast_expr_is_equal(expr1->u.op.args[i], expr2->u.op.args[i]);
      if (equal < 0 || !equal)
        return equal;
    }
    return isl_bool_true;
  ...
  isl_die(isl_ast_expr_get_ctx(expr1), isl_error_internal, "unhandled case",
          return isl_bool_error);
}
...
isl_ast_expr_alloc_op(isl_ctx *ctx, enum isl_ast_expr_op_type op, int n_arg) {
  isl_ast_expr *expr;
...
__isl_give isl_ast_expr *isl_ast_expr_from_val(__isl_take isl_val *v) {
  ...
  if (!v)
    return NULL;
  if (!isl_val_is_int(v))
    isl_die(isl_val_get_ctx(v), isl_error_invalid, "expecting integer value",
            goto error);
 ...
 */
__isl_give isl_ast_expr *
isl_ast_expr_alloc_unary(enum isl_ast_expr_op_type type,
                         __isl_take isl_ast_expr *arg) {
  isl_ctx *ctx;
 ...
 */
__isl_give isl_ast_expr *
isl_ast_expr_address_of(__isl_take isl_ast_expr *expr) {
  if (!expr)
    return NULL;
 ...
 */
__isl_give isl_ast_expr *
isl_ast_expr_alloc_binary(enum isl_ast_expr_op_type type,
...
ast_expr_with_arguments(enum isl_ast_expr_op_type type,
 ...
 */
__isl_give isl_ast_expr *
isl_ast_expr_access(__isl_take isl_ast_expr *array,
 ...
 */
__isl_give isl_ast_expr *
isl_ast_expr_call(__isl_take isl_ast_expr *function,
                  __isl_take isl_ast_expr_list *arguments) {
 ...
 */
__isl_give isl_ast_expr *
isl_ast_expr_substitute_ids(__isl_take isl_ast_expr *expr,
  ...
  case isl_ast_expr_op:
    for (i = 0; i < expr->u.op.n_arg; ++i) {
      ...
      arg = isl_ast_expr_substitute_ids(arg, isl_id_to_ast_expr_copy(id2expr));
...
isl_ast_node_alloc_mark(__isl_take isl_id *id, __isl_take isl_ast_node *node) {
  isl_ctx *ctx;
  isl_ast_node *mark;
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_alloc_user(__isl_take isl_ast_expr *expr) {
  isl_ctx *ctx;
  isl_ast_node *node;
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_alloc_block(__isl_take isl_ast_node_list *list) {
  isl_ast_node *node;
  isl_ctx *ctx;
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_from_ast_node_list(__isl_take isl_ast_node_list *list) {
  isl_size n;
  isl_ast_node *node;
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_for_set_body(__isl_take isl_ast_node *node,
                          __isl_take isl_ast_node *body) {
  node = isl_ast_node_cow(node);
  if (!node || !body)
    goto error;
  if (node->type != isl_ast_node_for)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not a for node",
            goto error);
...
__isl_give isl_ast_node *
isl_ast_node_for_get_body(__isl_keep isl_ast_node *node) {
  if (!node)
    return NULL;
  if (node->type != isl_ast_node_for)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not a for node",
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_for_mark_degenerate(__isl_take isl_ast_node *node) {
  node = isl_ast_node_cow(node);
  if (!node)
    return NULL;
...
__isl_give isl_ast_expr *
isl_ast_node_for_get_iterator(__isl_keep isl_ast_node *node) {
  if (!node)
    return NULL;
  if (node->type != isl_ast_node_for)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not a for node",
...
__isl_give isl_ast_expr *
isl_ast_node_for_get_init(__isl_keep isl_ast_node *node) {
  if (!node)
    return NULL;
  if (node->type != isl_ast_node_for)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not a for node",
 ...
 */
__isl_give isl_ast_expr *
isl_ast_node_for_get_cond(__isl_keep isl_ast_node *node) {
  if (!node)
    return NULL;
  if (node->type != isl_ast_node_for)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not a for node",
 ...
 */
__isl_give isl_ast_expr *
isl_ast_node_for_get_inc(__isl_keep isl_ast_node *node) {
  if (!node)
    return NULL;
  if (node->type != isl_ast_node_for)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not a for node",
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_if_set_then(__isl_take isl_ast_node *node,
                         __isl_take isl_ast_node *child) {
  node = isl_ast_node_cow(node);
  if (!node || !child)
    goto error;
  if (node->type != isl_ast_node_if)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not an if node",
            goto error);
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_if_get_then_node(__isl_keep isl_ast_node *node) {
  if (!node)
    return NULL;
  if (node->type != isl_ast_node_if)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not an if node",
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_if_get_then(__isl_keep isl_ast_node *node) {
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_if_get_else_node(__isl_keep isl_ast_node *node) {
  if (!node)
    return NULL;
  if (node->type != isl_ast_node_if)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not an if node",
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_if_get_else(__isl_keep isl_ast_node *node) {
  return isl_ast_node_if_get_else_node(node);
}

__isl_give isl_ast_expr *
isl_ast_node_if_get_cond(__isl_keep isl_ast_node *node) {
  if (!node)
    return NULL;
  if (node->type != isl_ast_node_if)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not a guard node",
            return NULL);
  return isl_ast_expr_copy(node->u.i.guard);
}

__isl_give isl_ast_node_list *
isl_ast_node_block_get_children(__isl_keep isl_ast_node *node) {
  if (!node)
    return NULL;
  if (node->type != isl_ast_node_block)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not a block node",
            return NULL);
  return isl_ast_node_list_copy(node->u.b.children);
}

__isl_give isl_ast_expr *
isl_ast_node_user_get_expr(__isl_keep isl_ast_node *node) {
  if (!node)
    return NULL;
  if (node->type != isl_ast_node_user)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not a user node",
            return NULL);
...
__isl_give isl_id *isl_ast_node_mark_get_id(__isl_keep isl_ast_node *node) {
  ...
  if (node->type != isl_ast_node_mark)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not a mark node",
            return NULL);
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_mark_get_node(__isl_keep isl_ast_node *node) {
  if (!node)
    return NULL;
  if (node->type != isl_ast_node_mark)
    isl_die(isl_ast_node_get_ctx(node), isl_error_invalid, "not a mark node",
 ...
 */
__isl_give isl_ast_node *
isl_ast_node_set_annotation(__isl_take isl_ast_node *node,
...
isl_stat isl_ast_node_foreach_descendant_top_down(
  ...
  case isl_ast_node_if:
    ok = isl_ast_node_foreach_descendant_top_down(node->u.i.then, fn, user);
    if (ok < 0)
      return isl_stat_error;
...
static char *op_str_c[] = {
    [isl_ast_expr_op_and] = "&&",        [isl_ast_expr_op_and_then] = "&&",
    [isl_ast_expr_op_or] = "||",         [isl_ast_expr_op_or_else] = "||",
    [isl_ast_expr_op_max] = "max",       [isl_ast_expr_op_min] = "min",
    [isl_ast_expr_op_minus] = "-",       [isl_ast_expr_op_add] = "+",
    [isl_ast_expr_op_sub] = "-",         [isl_ast_expr_op_mul] = "*",
    [isl_ast_expr_op_fdiv_q] = "floord", [isl_ast_expr_op_pdiv_q] = "/",
    [isl_ast_expr_op_pdiv_r] = "%",      [isl_ast_expr_op_zdiv_r] = "%",
    [isl_ast_expr_op_div] = "/",         [isl_ast_expr_op_eq] = "==",
    [isl_ast_expr_op_le] = "<=",         [isl_ast_expr_op_ge] = ">=",
    [isl_ast_expr_op_lt] = "<",          [isl_ast_expr_op_gt] = ">",
    [isl_ast_expr_op_member] = ".",      [isl_ast_expr_op_address_of] = "&"};
 ...
 */
static int op_prec[] = {
    [isl_ast_expr_op_and] = 13,    [isl_ast_expr_op_and_then] = 13,
    [isl_ast_expr_op_or] = 14,     [isl_ast_expr_op_or_else] = 14,
    [isl_ast_expr_op_max] = 2,     [isl_ast_expr_op_min] = 2,
    [isl_ast_expr_op_minus] = 3,   [isl_ast_expr_op_add] = 6,
    [isl_ast_expr_op_sub] = 6,     [isl_ast_expr_op_mul] = 5,
    [isl_ast_expr_op_div] = 5,     [isl_ast_expr_op_fdiv_q] = 2,
    [isl_ast_expr_op_pdiv_q] = 5,  [isl_ast_expr_op_pdiv_r] = 5,
    [isl_ast_expr_op_zdiv_r] = 5,  [isl_ast_expr_op_cond] = 15,
    [isl_ast_expr_op_select] = 15, [isl_ast_expr_op_eq] = 9,
    [isl_ast_expr_op_le] = 8,      [isl_ast_expr_op_ge] = 8,
    [isl_ast_expr_op_lt] = 8,      [isl_ast_expr_op_gt] = 8,
    [isl_ast_expr_op_call] = 2,    [isl_ast_expr_op_access] = 2,
    [isl_ast_expr_op_member] = 2,  [isl_ast_expr_op_address_of] = 3};

/* Is the operator left-to-right associative?
 */
static int op_left[] = {
    [isl_ast_expr_op_and] = 1,    [isl_ast_expr_op_and_then] = 1,
    [isl_ast_expr_op_or] = 1,     [isl_ast_expr_op_or_else] = 1,
    [isl_ast_expr_op_max] = 1,    [isl_ast_expr_op_min] = 1,
    [isl_ast_expr_op_minus] = 0,  [isl_ast_expr_op_add] = 1,
    [isl_ast_expr_op_sub] = 1,    [isl_ast_expr_op_mul] = 1,
    [isl_ast_expr_op_div] = 1,    [isl_ast_expr_op_fdiv_q] = 1,
    [isl_ast_expr_op_pdiv_q] = 1, [isl_ast_expr_op_pdiv_r] = 1,
    [isl_ast_expr_op_zdiv_r] = 1, [isl_ast_expr_op_cond] = 0,
    [isl_ast_expr_op_select] = 0, [isl_ast_expr_op_eq] = 1,
    [isl_ast_expr_op_le] = 1,     [isl_ast_expr_op_ge] = 1,
    [isl_ast_expr_op_lt] = 1,     [isl_ast_expr_op_gt] = 1,
    [isl_ast_expr_op_call] = 1,   [isl_ast_expr_op_access] = 1,
    [isl_ast_expr_op_member] = 1, [isl_ast_expr_op_address_of] = 0};
...
static int is_div_mod(enum isl_ast_expr_op_type op) {
  return op == isl_ast_expr_op_div || op == isl_ast_expr_op_pdiv_r ||
...
isl_ast_expr_op_type_set_print_name(__isl_take isl_printer *p,
  ...
  if (type > isl_ast_expr_op_last)
    isl_die(isl_printer_get_ctx(p), isl_error_invalid, "invalid type",
            return isl_printer_free(p));
 ...
 */
__isl_give isl_printer *
isl_ast_op_type_set_print_name(__isl_take isl_printer *p,
                               enum isl_ast_expr_op_type type,
                               __isl_keep const char *name) {
...
static __isl_give isl_printer *print_ast_expr_c(__isl_take isl_printer *p,
  ...
  case isl_ast_expr_op:
    ...
    if (expr->u.op.n_arg == 1) {
      p = isl_printer_print_str(p, get_op_str_c(p, expr->u.op.op));
      p = print_sub_expr_c(p, expr->u.op.op, expr->u.op.args[0], 0);
      break;
    }
 ...
 */
static char *op_str[] = {[isl_ast_expr_op_and] = "and",
...
print_ast_expr_isl(__isl_take isl_printer *p, __isl_keep isl_ast_expr *expr);
 ...
 */
__isl_give isl_printer *
isl_printer_print_ast_expr(__isl_take isl_printer *p,
                           __isl_keep isl_ast_expr *expr) {
  int format;
...
static __isl_give isl_printer *
print_ast_node_isl(__isl_take isl_printer *p, __isl_keep isl_ast_node *node);

/* Print a YAML sequence containing the entries in "list" to "p".
 */
static __isl_give isl_printer *
print_ast_node_list(__isl_take isl_printer *p,
...
static __isl_give isl_printer *
print_ast_node_c(__isl_take isl_printer *p, __isl_keep isl_ast_node *node,
                 __isl_keep isl_ast_print_options *options, int in_block,
                 int in_list);
static __isl_give isl_printer *
print_if_c(__isl_take isl_printer *p, __isl_keep isl_ast_node *node,
 ...
 */
static __isl_give isl_printer *
print_body_c(__isl_take isl_printer *p, __isl_keep isl_ast_node *node,
  ...
  if (!force_block && !else_node && !need_block(node)) {
    ...
    p = isl_ast_node_print(node, p, isl_ast_print_options_copy(options));
    p = isl_printer_indent(p, -2);
    return p;
  }
 ...
 */
static __isl_give isl_printer *
print_for_c(__isl_take isl_printer *p, __isl_keep isl_ast_node *node,
            __isl_keep isl_ast_print_options *options, int in_block,
            int in_list) {
  isl_id *id;
  const char *name;
...
print_ast_node_c(__isl_take isl_printer *p, __isl_keep isl_ast_node *node,
  ...
  case isl_ast_node_for:
    if (options->print_for)
      return options->print_for(p, isl_ast_print_options_copy(options), node,
  ...
  case isl_ast_node_user:
    if (options->print_user)
      return options->print_user(p, isl_ast_print_options_copy(options), node,
 ...
 */
__isl_give isl_printer *
isl_ast_node_for_print(__isl_keep isl_ast_node *node, __isl_take isl_printer *p,
                       __isl_take isl_ast_print_options *options) {
  if (!node || !options)
    goto error;
  if (node->type != isl_ast_node_for)
 ...
 */
__isl_give isl_printer *
isl_ast_node_if_print(__isl_keep isl_ast_node *node, __isl_take isl_printer *p,
                      __isl_take isl_ast_print_options *options) {
  if (!node || !options)
    goto error;
  if (node->type != isl_ast_node_if)
 ...
 */
__isl_give isl_printer *
isl_ast_node_print(__isl_keep isl_ast_node *node, __isl_take isl_printer *p,
                   __isl_take isl_ast_print_options *options) {
  int in_block = 0;
 ...
 */
__isl_give isl_printer *
isl_printer_print_ast_node(__isl_take isl_printer *p,
                           __isl_keep isl_ast_node *node) {
  int format;
  isl_ast_print_options *options;
 ...
 */
__isl_give isl_printer *
isl_ast_node_list_print(__isl_keep isl_ast_node_list *list,
...
#define ISL_AST_MACRO_ALL                                                      \
  (ISL_AST_MACRO_FDIV_Q | ISL_AST_MACRO_MIN | ISL_AST_MACRO_MAX)
...
static int ast_node_required_macros(__isl_keep isl_ast_node *node, int macros) {
  ...
  case isl_ast_node_for:
    ...
    if (!node->u.f.degenerate) {
      macros = ast_expr_required_macros(node->u.f.cond, macros);
      macros = ast_expr_required_macros(node->u.f.inc, macros);
    }
  ...
  case isl_ast_node_if:
    ...
    if (node->u.i.else_node)
      macros = ast_node_required_macros(node->u.i.else_node, macros);
    break;
  case isl_ast_node_block:
    macros = ast_node_list_required_macros(node->u.b.children, macros);
    break;
  case isl_ast_node_mark:
 ...
 */
static void free_printed(void *user) { free(user); }
 ...
 */
__isl_give isl_printer *
isl_ast_expr_op_type_print_macro(enum isl_ast_expr_op_type type,
                                 __isl_take isl_printer *p) {
  isl_bool skip;
  ...
  case isl_ast_expr_op_min:
    ...
    p = isl_printer_print_str(p, "(x,y)    ((x) < (y) ? (x) : (y))");
    p = isl_printer_end_line(p);
    break;
 ...
 */
__isl_give isl_printer *
isl_ast_op_type_print_macro(enum isl_ast_expr_op_type type,
                            __isl_take isl_printer *p) {
 ...
 * bit vector.
 */
static isl_stat foreach_ast_expr_op_type(
    int macros, isl_stat (*fn)(enum isl_ast_expr_op_type type, void *user),
    void *user) {
  if (macros & ISL_AST_MACRO_MIN && fn(isl_ast_expr_op_min, user) < 0)
    return isl_stat_error;
  if (macros & ISL_AST_MACRO_MAX && fn(isl_ast_expr_op_max, user) < 0)
    return isl_stat_error;
  if (macros & ISL_AST_MACRO_FDIV_Q && fn(isl_ast_expr_op_fdiv_q, user) < 0)
    return isl_stat_error;
...
__isl_give isl_printer *isl_ast_expr_print_macros(__isl_keep isl_ast_expr *expr,
                                                  __isl_take isl_printer *p) {
  if (isl_ast_expr_foreach_ast_expr_op_type(expr, &ast_op_type_print_macro,
                                            &p) < 0)
    return isl_printer_free(p);
  return p;
}
...
__isl_give isl_printer *isl_ast_node_print_macros(__isl_keep isl_ast_node *node,
                                                  __isl_take isl_printer *p) {
  if (isl_ast_node_foreach_ast_expr_op_type(node, &ast_op_type_print_macro,
                                            &p) < 0)
    return isl_printer_free(p);
  return p;
}
