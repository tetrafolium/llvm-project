...
struct isl_tab *isl_tab_alloc(struct isl_ctx *ctx, unsigned n_row,
                              unsigned n_var, unsigned M) {
  int i;
  struct isl_tab *tab;
  unsigned off = 2 + M;
...
int isl_tab_extend_cons(struct isl_tab *tab, unsigned n_new) {
  ...
  if (tab->max_con < tab->n_con + n_new) {
    ...
    con = isl_realloc_array(tab->mat->ctx, tab->con, struct isl_tab_var,
                            tab->max_con + n_new);
    if (!con)
      return -1;
    tab->con = con;
  ...
  if (tab->mat->n_row < tab->n_row + n_new) {
    ...
    tab->mat = isl_mat_extend(tab->mat, tab->n_row + n_new, off + tab->n_col);
    if (!tab->mat)
      return -1;
    row_var =
        isl_realloc_array(tab->mat->ctx, tab->row_var, int, tab->mat->n_row);
    if (!row_var)
      return -1;
    tab->row_var = row_var;
    if (tab->row_sign) {
      enum isl_tab_row_sign *s;
      s = isl_realloc_array(tab->mat->ctx, tab->row_sign, enum isl_tab_row_sign,
                            tab->mat->n_row);
      if (!s)
        return -1;
      tab->row_sign = s;
...
int isl_tab_extend_vars(struct isl_tab *tab, unsigned n_new) {
  ...
  if (tab->max_var < tab->n_var + n_new) {
    var = isl_realloc_array(tab->mat->ctx, tab->var, struct isl_tab_var,
                            tab->n_var + n_new);
    if (!var)
      return -1;
    tab->var = var;
  ...
  if (tab->mat->n_col < off + tab->n_col + n_new) {
    ...
    tab->mat =
        isl_mat_extend(tab->mat, tab->mat->n_row, off + tab->n_col + n_new);
    if (!tab->mat)
      return -1;
    p = isl_realloc_array(tab->mat->ctx, tab->col_var, int, tab->n_col + n_new);
    if (!p)
      return -1;
    tab->col_var = p;
  }
...
struct isl_tab *isl_tab_dup(struct isl_tab *tab) {
  ...
  if (tab->row_sign) {
    dup->row_sign =
        isl_alloc_array(tab->mat->ctx, enum isl_tab_row_sign, tab->mat->n_row);
  ...
  if (tab->samples) {
    ...
    if (!dup->samples)
      goto error;
    dup->sample_index =
        isl_alloc_array(tab->mat->ctx, int, tab->samples->n_row);
...
tab_mat_product(__isl_keep isl_mat *mat1, __isl_keep isl_mat *mat2,
                unsigned row1, unsigned row2, unsigned col1, unsigned col2,
                unsigned off, unsigned r1, unsigned r2, unsigned d1,
                unsigned d2) {
  int i;
  struct isl_mat *prod;
  unsigned n;

  prod = isl_mat_alloc(mat1->ctx, mat1->n_row + mat2->n_row, off + col1 + col2);
  if (!prod)
    return NULL;
  ...
  for (i = 0; i < r1; ++i) {
    ...
    isl_seq_cpy(prod->row[n + i] + off + d1 + d2, mat1->row[i] + off + d1,
                col1 - d1);
  ...
  for (i = 0; i < r2; ++i) {
    ...
    isl_seq_cpy(prod->row[n + i] + off + d1, mat2->row[i] + off, d2);
    isl_seq_clr(prod->row[n + i] + off + d1 + d2, col1 - d1);
    isl_seq_cpy(prod->row[n + i] + off + col1 + d1, mat2->row[i] + off + d2,
                col2 - d2);
  }
  ...
  for (i = 0; i < row1 - r1; ++i) {
    ...
    isl_seq_clr(prod->row[n + i] + off + d1, d2);
    isl_seq_cpy(prod->row[n + i] + off + d1 + d2, mat1->row[r1 + i] + off + d1,
                col1 - d1);
    isl_seq_clr(prod->row[n + i] + off + col1 + d1, col2 - d2);
  }
  ...
  for (i = 0; i < row2 - r2; ++i) {
    ...
    isl_seq_clr(prod->row[n + i] + off, d1);
    isl_seq_cpy(prod->row[n + i] + off + d1, mat2->row[r2 + i] + off, d2);
    isl_seq_clr(prod->row[n + i] + off + d1 + d2, col1 - d1);
 ...
 */
static void update_index1(struct isl_tab_var *var, unsigned r1, unsigned r2,
 ...
 */
static void update_index2(struct isl_tab_var *var, unsigned row1, unsigned col1,
...
struct isl_tab *isl_tab_product(struct isl_tab *tab1, struct isl_tab *tab2) {
  ...
  if (!prod)
    return NULL;
  prod->mat = tab_mat_product(tab1->mat, tab2->mat, tab1->n_row, tab2->n_row,
  ...
  for (i = 0; i < tab2->n_var; ++i) {
    ...
    update_index2(&prod->var[tab1->n_var + i], tab1->n_row, tab1->n_col, r1, r2,
                  d1, d2);
  }
  ...
  for (i = 0; i < tab2->n_con; ++i) {
    ...
    update_index2(&prod->con[tab1->n_con + i], tab1->n_row, tab1->n_col, r1, r2,
                  d1, d2);
  }
  prod->col_var =
      isl_alloc_array(tab1->mat->ctx, int, tab1->n_col + tab2->n_col);
  ...
  prod->row_var =
      isl_alloc_array(tab1->mat->ctx, int, tab1->mat->n_row + tab2->mat->n_row);
 ...
 */
static int pivot_row(struct isl_tab *tab, struct isl_tab_var *var, int sgn,
                     int c) {
  int j, r, tsgn;
  isl_int t;
  unsigned off = 2 + tab->M;
  ...
  for (j = tab->n_redundant; j < tab->n_row; ++j) {
    ...
    if (tsgn < 0 || (tsgn == 0 && tab->row_var[j] < tab->row_var[r]))
      r = j;
  }
  isl_int_clear(t);
  return r;
 ...
 * opposite direction.
 */
static void find_pivot(struct isl_tab *tab, struct isl_tab_var *var,
                       struct isl_tab_var *skip_var, int sgn, int *row,
                       int *col) {
  int j, r, c;
  isl_int *tr;
  ...
  *row = *col = -1;
  ...
  for (j = tab->n_dead; j < tab->n_col; ++j) {
    if (isl_int_is_zero(tr[j]))
      continue;
    if (isl_int_sgn(tr[j]) != sgn && var_from_col(tab, j)->is_nonneg)
      continue;
...
static isl_stat push_union(struct isl_tab *tab, enum isl_tab_undo_type type,
                           union isl_tab_undo_val u) WARN_UNUSED;
...
isl_stat isl_tab_push_var(struct isl_tab *tab, enum isl_tab_undo_type type,
                          struct isl_tab_var *var) {
  union isl_tab_undo_val u;
  if (var->is_row)
...
int isl_tab_add_sample(struct isl_tab *tab, __isl_take isl_vec *sample) {
  ...
  if (tab->n_sample + 1 > tab->samples->n_row) {
    int *t = isl_realloc_array(tab->mat->ctx, tab->sample_index, int,
                               tab->n_sample + 1);
    if (!t)
      goto error;
    tab->sample_index = t;
  ...
  tab->samples =
      isl_mat_extend(tab->samples, tab->n_sample + 1, tab->samples->n_col);
  if (!tab->samples)
    goto error;
 ...
 */
static void update_row_sign(struct isl_tab *tab, int row, int col,
                            int row_sgn) {
  int i;
...
int isl_tab_pivot(struct isl_tab *tab, int row, int col) {
  ...
  for (i = 0; i < tab->n_row; ++i) {
    ...
    for (j = 0; j < off - 1 + tab->n_col; ++j) {
      if (j == off - 1 + col)
        continue;
      isl_int_mul(mat->row[i][1 + j], mat->row[i][1 + j], mat->row[row][0]);
      isl_int_addmul(mat->row[i][1 + j], mat->row[i][off + col],
                     mat->row[row][1 + j]);
    }
    isl_int_mul(mat->row[i][off + col], mat->row[i][off + col],
                mat->row[row][off + col]);
 ...
 */
static int to_row(struct isl_tab *tab, struct isl_tab_var *var,
                  int sign) WARN_UNUSED;
...
static void check_table(struct isl_tab *tab) {
  ...
  for (i = tab->n_redundant; i < tab->n_row; ++i) {
    ...
    if (!var->is_nonneg)
      continue;
    if (tab->M) {
      isl_assert(tab->mat->ctx, !isl_int_is_neg(tab->mat->row[i][2]), abort());
      if (isl_int_is_pos(tab->mat->row[i][2]))
        continue;
    }
    isl_assert(tab->mat->ctx, !isl_int_is_neg(tab->mat->row[i][1]), abort());
  }
}
...
static int row_at_most_neg_one(struct isl_tab *tab, int row) {
  ...
  return isl_int_is_neg(tab->mat->row[row][1]) &&
         isl_int_abs_ge(tab->mat->row[row][1], tab->mat->row[row][0]);
}
...
int isl_tab_kill_col(struct isl_tab *tab, int col) {
  ...
  if (tab->need_undo) {
    if (isl_tab_push_var(tab, isl_tab_undo_zero, var_from_col(tab, col)) < 0)
      return -1;
    if (col != tab->n_dead)
...
static int row_is_manifestly_non_integral(struct isl_tab *tab, int row) {
  ...
  if (tab->M && !isl_int_eq(tab->mat->row[row][2], tab->mat->row[row][0]))
    return 0;
...
static isl_stat close_row(struct isl_tab *tab, struct isl_tab_var *var,
  ...
  if (!var->is_nonneg)
    isl_die(isl_tab_get_ctx(tab), isl_error_internal,
            "expecting non-negative variable", return isl_stat_error);
  var->is_zero = 1;
  ...
  for (j = tab->n_dead; j < tab->n_col; ++j) {
    ...
    if (isl_int_is_pos(mat->row[var->index][off + j]))
      isl_die(isl_tab_get_ctx(tab), isl_error_internal,
              "row cannot have positive coefficients", return isl_stat_error);
...
int isl_tab_add_row(struct isl_tab *tab, isl_int *line) {
  ...
  for (i = 0; i < tab->n_var; ++i) {
    if (tab->var[i].is_zero)
      continue;
    if (tab->var[i].is_row) {
      isl_int_lcm(a, row[0], tab->mat->row[tab->var[i].index][0]);
      isl_int_swap(a, row[0]);
      isl_int_divexact(a, row[0], a);
      isl_int_divexact(b, row[0], tab->mat->row[tab->var[i].index][0]);
      isl_int_mul(b, b, line[1 + i]);
      isl_seq_combine(row + 1, a, row + 1, b,
                      tab->mat->row[tab->var[i].index] + 1,
                      1 + tab->M + tab->n_col);
    } else
      isl_int_addmul(row[off + tab->var[i].index], line[1 + i], row[0]);
...
isl_stat isl_tab_add_ineq(struct isl_tab *tab, isl_int *ineq) {
  ...
  if (tab->bmap) {
    ...
    isl_assert(tab->mat->ctx, tab->n_eq == bmap->n_eq, return isl_stat_error);
    isl_assert(tab->mat->ctx, tab->n_con == bmap->n_eq + bmap->n_ineq,
               return isl_stat_error);
...
int isl_tab_add_valid_eq(struct isl_tab *tab, isl_int *eq) {
  ...
  if (isl_int_is_neg(tab->mat->row[r][1])) {
    isl_seq_neg(tab->mat->row[r] + 1, tab->mat->row[r] + 1, 1 + tab->n_col);
    var->negated = 1;
  }
  var->is_nonneg = 1;
  if (to_col(tab, var) < 0)
...
isl_stat isl_tab_add_eq(struct isl_tab *tab, isl_int *eq) {
  ...
  if (sgn > 0) {
    isl_seq_neg(tab->mat->row[row] + 1, tab->mat->row[row] + 1, 1 + tab->n_col);
    var->negated = 1;
    sgn = -1;
  }
...
int isl_tab_insert_div(struct isl_tab *tab, int pos, __isl_keep isl_vec *div,
  ...
  if (div->size != 1 + 1 + tab->n_var)
    isl_die(isl_tab_get_ctx(tab), isl_error_invalid, "unexpected size",
            return -1);
  ...
  if (pos < o_div || pos > tab->n_var)
    isl_die(isl_tab_get_ctx(tab), isl_error_invalid, "invalid position",
            return -1);
 ...
 */
__isl_give struct isl_tab *
isl_tab_from_basic_map(__isl_keep isl_basic_map *bmap, int track) {
  int i;
  struct isl_tab *tab;
  isl_size total;
...
__isl_give struct isl_tab *
isl_tab_from_basic_set(__isl_keep isl_basic_set *bset, int track) {
...
struct isl_tab *isl_tab_from_recession_cone(__isl_keep isl_basic_set *bset,
  ...
  if (total < 0 || offset < 0)
    return NULL;
  tab = isl_tab_alloc(bset->ctx, bset->n_eq + bset->n_ineq, total - offset, 0);
  if (!tab)
    return NULL;
...
int isl_tab_sample_is_integer(struct isl_tab *tab) {
  ...
  for (i = 0; i < tab->n_var; ++i) {
    ...
    if (!tab->var[i].is_row)
      continue;
    row = tab->var[i].index;
    if (!isl_int_is_divisible_by(tab->mat->row[row][1], tab->mat->row[row][0]))
      return 0;
  }
  return 1;
}
...
static struct isl_vec *extract_integer_sample(struct isl_tab *tab) {
  ...
  for (i = 0; i < tab->n_var; ++i) {
    if (!tab->var[i].is_row)
      ...
      isl_int_divexact(vec->block.data[1 + i], tab->mat->row[row][1],
                       tab->mat->row[row][0]);
    }
  }
...
static void get_rounded_sample_value(struct isl_tab *tab,
                                     struct isl_tab_var *var, int sgn,
                                     isl_int *v) {
  if (!var->is_row)
    isl_int_set_si(*v, 0);
  else if (sgn > 0)
...
isl_basic_map_update_from_tab(__isl_take isl_basic_map *bmap,
                              struct isl_tab *tab) {
  int i;
  unsigned n_eq;
  ...
  if (!tab->rational && bmap && !bmap->sample && isl_tab_sample_is_integer(tab))
    bmap->sample = extract_integer_sample(tab);
  return bmap;
}

__isl_give isl_basic_set *
isl_basic_set_update_from_tab(__isl_take isl_basic_set *bset,
                              struct isl_tab *tab) {
  return bset_from_bmap(isl_basic_map_update_from_tab(bset_to_bmap(bset), tab));
}
...
static isl_stat drop_last_con_in_row(struct isl_tab *tab, int r) {
  if (!tab->con[r].is_row)
    isl_die(isl_tab_get_ctx(tab), isl_error_internal,
            "row unexpectedly moved to column", return isl_stat_error);
  if (r + 1 != tab->n_con)
    isl_die(isl_tab_get_ctx(tab), isl_error_internal,
...
static isl_stat cut_to_hyperplane(struct isl_tab *tab,
  ...
  if (var->is_row) {
    ...
    isl_seq_neg(row + 1, tab->mat->row[var->index] + 1, 1 + tab->n_col);
  } else {
    isl_int_set_si(row[0], 1);
...
static isl_stat isl_tab_check_con(struct isl_tab *tab, int con) {
  ...
  if (con < 0 || con >= tab->n_con)
    isl_die(isl_tab_get_ctx(tab), isl_error_invalid, "position out of bounds",
            return isl_stat_error);
  return isl_stat_ok;
}
...
int isl_tab_relax(struct isl_tab *tab, int con) {
  ...
  if (!var->is_row && (var->index < 0 || var->index < tab->n_dead))
    isl_die(tab->mat->ctx, isl_error_invalid, "cannot relax dead constraint",
            return -1);
  ...
  if (var->is_row) {
    isl_int_add(tab->mat->row[var->index][1], tab->mat->row[var->index][1],
                tab->mat->row[var->index][0]);
...
int isl_tab_shift_var(struct isl_tab *tab, int pos, isl_int shift) {
  ...
  if (var->is_row) {
    isl_int_addmul(tab->mat->row[var->index][1], shift,
                   tab->mat->row[var->index][0]);
  } else {
    int i;
    unsigned off = 2 + tab->M;
    ...
    for (i = 0; i < tab->n_row; ++i) {
      if (isl_int_is_zero(tab->mat->row[i][off + var->index]))
        continue;
      isl_int_submul(tab->mat->row[i][1], shift,
                     tab->mat->row[i][off + var->index]);
    }
  }
...
static int may_be_equality(struct isl_tab *tab, int row) {
  return tab->rational
             ? isl_int_is_zero(tab->mat->row[row][1])
             : isl_int_lt(tab->mat->row[row][1], tab->mat->row[row][0]);
}
...
int isl_tab_detect_implicit_equalities(struct isl_tab *tab) {
  ...
  for (i = tab->n_redundant; i < tab->n_row; ++i) {
    ...
    var->marked = !var->frozen && var->is_nonneg && may_be_equality(tab, i);
    if (var->marked)
      n_marked++;
  }
  for (i = tab->n_dead; i < tab->n_col; ++i) {
...
static int update_con_after_move(struct isl_tab *tab, int i, int old) {
  ...
  if (p[index] != ~old)
    isl_die(tab->mat->ctx, isl_error_internal, "broken internal state",
            return -1);
  p[index] = ~i;
...
isl_stat isl_tab_swap_constraints(struct isl_tab *tab, int con1, int con2) {
  ...
  if (isl_tab_check_con(tab, con1) < 0 || isl_tab_check_con(tab, con2) < 0)
    return isl_stat_error;
 ...
 * indices into this array, they have to be updated accordingly.
 */
static isl_stat con_drop_entries(struct isl_tab *tab, unsigned first,
                                 unsigned n) {
  int i;

  if (first + n > tab->n_con || first + n < first)
    isl_die(isl_tab_get_ctx(tab), isl_error_internal, "invalid range",
            return isl_stat_error);
 ...
 */
__isl_give isl_basic_map *
isl_tab_make_equalities_explicit(struct isl_tab *tab,
  ...
  for (i = bmap->n_ineq - 1; i >= 0; --i) {
    ...
    if (rotate_constraints(tab, 0, tab->n_eq + i + 1) < 0)
      return isl_basic_map_free(bmap);
    if (rotate_constraints(tab, tab->n_eq + i + 1, bmap->n_ineq - i) < 0)
      return isl_basic_map_free(bmap);
    tab->n_eq++;
  }
...
int isl_tab_is_equality(struct isl_tab *tab, int con) {
  ...
  off = 2 + tab->M;
  return isl_int_is_zero(tab->mat->row[row][1]) && !row_is_big(tab, row) &&
 ...
 * one more element in the constraint array are available in the tableau.
 */
enum isl_lp_result isl_tab_min(struct isl_tab *tab, isl_int *f, isl_int denom,
                               isl_int *opt, isl_int *opt_denom,
                               unsigned flags) {
  int r;
  ...
  isl_int_mul(tab->mat->row[var->index][0], tab->mat->row[var->index][0],
              denom);
  if (ISL_FL_ISSET(flags, ISL_TAB_SAVE_DUAL)) {
    ...
    for (i = 0; i < tab->n_con; ++i) {
      ...
      if (tab->con[i].negated)
        isl_int_neg(tab->dual->el[1 + i], tab->mat->row[var->index][pos]);
      else
        isl_int_set(tab->dual->el[1 + i], tab->mat->row[var->index][pos]);
    }
  }
...
static isl_bool get_constant(struct isl_tab *tab, struct isl_tab_var *var,
  ...
  is_cst = detect_constant_with_tmp(tab, var, value ? value : &target, &tmp);
...
static isl_stat unrelax(struct isl_tab *tab,
                        struct isl_tab_var *var) WARN_UNUSED;
static isl_stat unrelax(struct isl_tab *tab, struct isl_tab_var *var) {
  ...
  if (var->is_row) {
    isl_int_sub(tab->mat->row[var->index][1], tab->mat->row[var->index][1],
                tab->mat->row[var->index][0]);
    if (var->is_nonneg) {
      int sgn = restore_row(tab, var);
      isl_assert(tab->mat->ctx, sgn >= 0, return isl_stat_error);
    }
  } else {
    int i;
...
static isl_stat restore_last_redundant(struct isl_tab *tab) {
  ...
  if (tab->n_redundant < 1)
    isl_die(isl_tab_get_ctx(tab), isl_error_internal, "no redundant rows",
            return isl_stat_error);
...
static isl_stat perform_undo_var(struct isl_tab *tab,
                                 struct isl_tab_undo *undo) WARN_UNUSED;
static isl_stat perform_undo_var(struct isl_tab *tab,
                                 struct isl_tab_undo *undo) {
  ...
  case isl_tab_undo_redundant:
    if (!var->is_row || var->index != tab->n_redundant - 1)
      isl_die(isl_tab_get_ctx(tab), isl_error_internal,
              "not undoing last redundant row", return isl_stat_error);
  ...
  case isl_tab_undo_allocate:
    if (undo->u.var_index >= 0) {
      isl_assert(tab->mat->ctx, !var->is_row, return isl_stat_error);
...
static enum isl_ineq_type separation_type(struct isl_tab *tab, unsigned row) {
  ...
  pos = isl_seq_first_non_zero(tab->mat->row[row] + off + tab->n_dead + pos + 1,
...
static void isl_tab_print_internal(__isl_keep struct isl_tab *tab, FILE *out,
                                   int indent) {
  unsigned r, c;
  int i;
  ...
  fprintf(out, "%*sn_redundant: %d, n_dead: %d", indent, "", tab->n_redundant,
          tab->n_dead);
  if (tab->rational)
  ...
  for (i = 0; i < tab->n_var; ++i) {
    if (i)
      fprintf(out, (i == tab->n_param || i == tab->n_var - tab->n_div) ? "; "
                                                                       : ", ");
    fprintf(out, "%c%d%s", tab->var[i].is_row ? 'r' : 'c', tab->var[i].index,
            tab->var[i].is_zero ? " [=0]"
  ...
  for (i = 0; i < tab->n_con; ++i) {
    if (i)
      fprintf(out, ", ");
    fprintf(out, "%c%d%s", tab->con[i].is_row ? 'r' : 'c', tab->con[i].index,
            tab->con[i].is_zero ? " [=0]"
