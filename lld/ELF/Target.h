...
class TargetInfo {
public:
    virtual uint32_t calcEFlags() const {
        return 0;
    }
    virtual RelExpr getRelExpr(RelType type, const Symbol &s,
                               const uint8_t *loc) const = 0;
    virtual RelType getDynRel(RelType type) const {
        return 0;
    }
    ...
    virtual int getTlsGdRelaxSkip(RelType type) const {
        return 1;
    }
    ...
    virtual uint32_t getThunkSectionSpacing() const {
        return 0;
    }
