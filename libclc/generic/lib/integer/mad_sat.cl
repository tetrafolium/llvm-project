...
_CLC_OVERLOAD _CLC_DEF char mad_sat(char x, char y, char z) {
  return clamp((short)mad24((short)x, (short)y, (short)z), (short)CHAR_MIN,
               (short)CHAR_MAX);
}

_CLC_OVERLOAD _CLC_DEF uchar mad_sat(uchar x, uchar y, uchar z) {
  return clamp((ushort)mad24((ushort)x, (ushort)y, (ushort)z), (ushort)0,
               (ushort)UCHAR_MAX);
}

_CLC_OVERLOAD _CLC_DEF short mad_sat(short x, short y, short z) {
  return clamp((int)mad24((int)x, (int)y, (int)z), (int)SHRT_MIN,
               (int)SHRT_MAX);
}

_CLC_OVERLOAD _CLC_DEF ushort mad_sat(ushort x, ushort y, ushort z) {
  return clamp((uint)mad24((uint)x, (uint)y, (uint)z), (uint)0,
               (uint)USHRT_MAX);
}
...
_CLC_TERNARY_VECTORIZE(_CLC_OVERLOAD _CLC_DEF, uchar, mad_sat, uchar, uchar,
                       uchar)
_CLC_TERNARY_VECTORIZE(_CLC_OVERLOAD _CLC_DEF, short, mad_sat, short, short,
                       short)
_CLC_TERNARY_VECTORIZE(_CLC_OVERLOAD _CLC_DEF, ushort, mad_sat, ushort, ushort,
                       ushort)
...
_CLC_TERNARY_VECTORIZE(_CLC_OVERLOAD _CLC_DEF, ulong, mad_sat, ulong, ulong,
                       ulong)
