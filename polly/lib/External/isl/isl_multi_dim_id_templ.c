...
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), set_dim_name)(__isl_take MULTI(BASE) * multi,
  ...
  if (type == isl_dim_out)
    return multi;
  for (i = 0; i < multi->n; ++i) {
    multi->u.p[i] = FN(EL, set_dim_name)(multi->u.p[i], type, pos, s);
    if (!multi->u.p[i])
      return FN(MULTI(BASE), free)(multi);
  }
 ...
 */
__isl_give MULTI(BASE) *
    FN(MULTI(BASE), set_dim_id)(__isl_take MULTI(BASE) * multi,
                                enum isl_dim_type type, unsigned pos,
