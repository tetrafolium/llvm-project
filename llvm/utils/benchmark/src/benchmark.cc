...
State::State(size_t max_iters, const std::vector<int64_t> &ranges, int thread_i,
             ...
             internal::ThreadManager *manager)
    : total_iterations_(0), batch_leftover_(0), max_iterations(max_iters),
      started_(false), finished_(false), error_occurred_(false), range_(ranges),
      bytes_processed_(0), items_processed_(0), complexity_n_(0), counters(),
      thread_index(thread_i), threads(n_threads), timer_(timer),
      manager_(manager) {
...
CreateReporter(std::string const &name,
