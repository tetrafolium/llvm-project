...
isl_basic_map_normalize_constraints(__isl_take isl_basic_map *bmap) {
  int i;
  isl_int gcd;
...
isl_basic_set_normalize_constraints(__isl_take isl_basic_set *bset) {
...
reduce_coefficient_in_div(__isl_take isl_basic_map *bmap, int div, int pos) {
  isl_int shift;
  int add_one;
...
static isl_bool needs_reduction(__isl_keep isl_basic_map *bmap, int div,
  ...
  isl_int_divexact_ui(bmap->div[div][1 + pos], bmap->div[div][1 + pos], 2);
 ...
 */
static __isl_give isl_basic_map *
reduce_div_coefficients_of_div(__isl_take isl_basic_map *bmap, int div) {
  int i;
  isl_size total;
 ...
 */
static __isl_give isl_basic_map *
reduce_div_coefficients(__isl_take isl_basic_map *bmap) {
  int i;
 ...
 */
static __isl_give isl_basic_map *
normalize_div_expression(__isl_take isl_basic_map *bmap, int div) {
  isl_size total = isl_basic_map_dim(bmap, isl_dim_all);
  ...
  if (isl_int_is_one(ctx->normalize_gcd))
    return bmap;
  isl_int_fdiv_q(bmap->div[div][1], bmap->div[div][1], ctx->normalize_gcd);
  isl_int_divexact(bmap->div[div][0], bmap->div[div][0], ctx->normalize_gcd);
  isl_seq_scale_down(bmap->div[div] + 2, bmap->div[div] + 2, ctx->normalize_gcd,
                     total);
 ...
 */
static __isl_give isl_basic_map *
normalize_div_expressions(__isl_take isl_basic_map *bmap) {
  int i;
 ...
 */
static __isl_give isl_basic_map *
eliminate_var_using_equality(__isl_take isl_basic_map *bmap, unsigned pos,
                             isl_int *eq, int keep_divs, int *progress) {
  isl_size total;
  isl_size v_div;
  int k;
  int last_div;
  ...
  for (k = 0; k < bmap->n_div; ++k) {
    ...
    if (last_div == -1 || (keep_divs && last_div < k)) {
      isl_seq_elim(bmap->div[k] + 1, eq, 1 + pos, 1 + total, &bmap->div[k][0]);
 ...
 */
static __isl_give isl_basic_map *
eliminate_divs_eq(__isl_take isl_basic_map *bmap, int *progress) {
  int d;
  int i;
  int modified = 0;
  unsigned off;
 ...
 */
static __isl_give isl_basic_map *
eliminate_divs_ineq(__isl_take isl_basic_map *bmap, int *progress) {
  int d;
  int i;
  unsigned off;
  struct isl_ctx *ctx;
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_gauss5(__isl_take isl_basic_map *bmap, int *progress,
  ...
  for (done = 0; done < bmap->n_eq; ++done) {
    ...
    bmap = eliminate_var_using_equality(bmap, last_var, bmap->eq[done], 1,
                                        progress);

    if (last_var >= total_var)
      bmap = set_div_from_eq(bmap, last_var - total_var, done, progress);
    if (!bmap)
      return NULL;
  }
  if (done == bmap->n_eq)
...
__isl_give isl_basic_set *isl_basic_set_gauss(__isl_take isl_basic_set *bset,
                                              int *progress) {
  return bset_from_bmap(isl_basic_map_gauss(bset_to_bmap(bset), progress));
}
 ...
 */
static __isl_give isl_basic_map *
remove_duplicate_divs(__isl_take isl_basic_map *bmap, int *progress) {
  unsigned int size;
  int *index;
  int *elim_for;
  int k, l, h;
  ...
  for (--k; k >= 0; --k) {
    ...
    for (h = hash; index[h]; h = (h + 1) % size)
      if (isl_seq_eq(bmap->div[k], bmap->div[index[h] - 1], 2 + total))
        break;
    if (index[h]) {
      *progress = 1;
      l = index[h] - 1;
...
static __isl_give isl_basic_map *normalize_divs(__isl_take isl_basic_map *bmap,
  ...
  if (div_eq < bmap->n_eq) {
    B = isl_mat_sub_alloc6(bmap->ctx, bmap->eq, div_eq, bmap->n_eq - div_eq, 0,
                           1 + v_div);
  ...
  for (i = 1; i < T->n_row; ++i) {
    ...
    for (j = 0; j < i; ++j) {
      if (isl_int_is_zero(T->row[i][j]))
        continue;
      if (pos[j] < T->n_row && C2)
        isl_seq_submul(bmap->div[k] + 1, T->row[i][j], C2->row[pos[j]],
                       1 + v_div);
      else
        isl_int_neg(bmap->div[k][1 + pos[j]], T->row[i][j]);
    }
...
static __isl_give isl_basic_map *
set_div_from_lower_bound(__isl_take isl_basic_map *bmap, int div, int ineq) {
 ...
 */
static isl_bool better_div_constraint(__isl_keep isl_basic_map *bmap, int div,
                                      int ineq) {
  unsigned total = isl_basic_map_offset(bmap, isl_dim_div);
  int last_div;
  int last_ineq;
  ...
  last_div = isl_seq_last_non_zero(bmap->div[div] + 1, total + bmap->n_div);
 ...
 */
static __isl_give isl_basic_map *
check_for_div_constraints(__isl_take isl_basic_map *bmap, int k, int l,
                          isl_int sum, int *progress) {
  int i;
  unsigned total = isl_basic_map_offset(bmap, isl_dim_div);
...
__isl_give isl_basic_map *
isl_basic_map_remove_duplicate_constraints(__isl_take isl_basic_map *bmap,
  ...
  for (k = 0; bmap && k < bmap->n_ineq - 1; ++k) {
    ...
    if (isl_int_is_pos(sum)) {
      if (detect_divs)
        bmap = check_for_div_constraints(bmap, k, l, sum, progress);
      continue;
    }
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_detect_inequality_pairs(__isl_take isl_basic_map *bmap,
                                      int *progress) {
  int duplicate;

  do {
    duplicate = 0;
    bmap = isl_basic_map_remove_duplicate_constraints(bmap, &duplicate, 0);
 ...
 */
static __isl_give isl_basic_map *
eliminate_unit_div(__isl_take isl_basic_map *bmap, int div, int *progress) {
  int j;
  isl_size v_div, dim;
  isl_ctx *ctx;
  ...
  for (j = 0; j < bmap->n_ineq; ++j) {
    ...
    if (s < 0)
      isl_seq_combine(bmap->ineq[j], ctx->negone, bmap->div[div] + 1,
                      bmap->div[div][0], bmap->ineq[j], 1 + dim);
    else
      isl_seq_combine(bmap->ineq[j], ctx->one, bmap->div[div] + 1,
                      bmap->div[div][0], bmap->ineq[j], 1 + dim);
    if (s < 0) {
      isl_int_add(bmap->ineq[j][0], bmap->ineq[j][0], bmap->div[div][0]);
 ...
 */
static __isl_give isl_basic_map *
eliminate_unit_divs(__isl_take isl_basic_map *bmap, int *progress) {
...
static isl_bool is_pure_unit_div(__isl_keep isl_basic_map *bmap, int div) {
  ...
  for (i = 0; i < n_ineq; ++i) {
    ...
    if (isl_int_is_zero(bmap->ineq[i][1 + v_div + div]))
      continue;
    skip = isl_basic_map_is_div_constraint(bmap, bmap->ineq[i], div);
    if (skip < 0)
      return isl_bool_error;
    if (skip)
      continue;
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_eliminate_pure_unit_divs(__isl_take isl_basic_map *bmap) {
  return eliminate_selected_unit_divs(bmap, &is_pure_unit_div, NULL);
}

__isl_give isl_basic_map *
isl_basic_map_simplify(__isl_take isl_basic_map *bmap) {
  int progress = 1;
  if (!bmap)
    return NULL;
  while (progress) {
    ...
    bmap = isl_basic_map_remove_duplicate_constraints(bmap, &progress, 1);
    if (bmap && progress)
...
__isl_give isl_basic_set *
isl_basic_set_simplify(__isl_take isl_basic_set *bset) {
...
isl_bool isl_basic_map_is_div_constraint(__isl_keep isl_basic_map *bmap,
  ...
  if (isl_int_eq(constraint[pos], bmap->div[div][0])) {
    int neg;
    isl_int_sub(bmap->div[div][1], bmap->div[div][1], bmap->div[div][0]);
    ...
    if (!neg)
      return isl_bool_false;
    if (isl_seq_first_non_zero(constraint + pos + 1, bmap->n_div - div - 1) !=
        -1)
      return isl_bool_false;
 ...
 */
static __isl_give isl_basic_map *
remove_redundant_divs(__isl_take isl_basic_map *bmap) {
  int i;
  isl_size v_div;
  ...
  for (i = bmap->n_div - 1; i >= 0; --i) {
    ...
    if (!redundant)
      continue;
    bmap = isl_basic_map_drop_constraints_involving(bmap, v_div + i, 1);
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_mark_final(__isl_take isl_basic_map *bmap) {
  if (!bmap)
    return NULL;
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_finalize(__isl_take isl_basic_map *bmap) {
...
__isl_give isl_basic_set *
isl_basic_set_finalize(__isl_take isl_basic_set *bset) {
 ...
 */
static __isl_give isl_basic_map *
remove_dependent_vars(__isl_take isl_basic_map *bmap, int pos) {
  int i;
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_eliminate_vars(__isl_take isl_basic_map *bmap, unsigned pos,
                             unsigned n) {
  int d;
  int i, j, k;
  isl_size total;
  ...
  for (d = pos + n - 1; d >= 0 && d >= total - bmap->n_div && d >= pos; --d)
  ...
  for (d = pos + n - 1; d >= 0 && d >= pos; --d) {
    ...
    for (i = 0; i < bmap->n_eq; ++i) {
      if (isl_int_is_zero(bmap->eq[i][1 + d]))
        continue;
      bmap = eliminate_var_using_equality(bmap, d, bmap->eq[i], 0, NULL);
      if (isl_basic_map_drop_equality(bmap, i) < 0)
        return isl_basic_map_free(bmap);
      need_gauss = 1;
      break;
    ...
    bmap = isl_basic_map_extend_constraints(bmap, 0, n_lower * n_upper);
    if (!bmap)
      goto error;
    for (i = bmap->n_ineq - 1; i >= 0; --i) {
      int last;
      ...
      for (j = 0; j < i; ++j) {
        ...
        if (k < 0)
          goto error;
        isl_seq_cpy(bmap->ineq[k], bmap->ineq[i], 1 + total);
        isl_seq_elim(bmap->ineq[k], bmap->ineq[j], 1 + d, 1 + total, NULL);
      }
...
__isl_give isl_basic_set *
isl_basic_set_eliminate_vars(__isl_take isl_basic_set *bset, unsigned pos,
                             unsigned n) {
  return bset_from_bmap(
      isl_basic_map_eliminate_vars(bset_to_bmap(bset), pos, n));
}
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_eliminate(__isl_take isl_basic_map *bmap, enum isl_dim_type type,
...
__isl_give isl_basic_set *
isl_basic_set_eliminate(__isl_take isl_basic_set *bset, enum isl_dim_type type,
                        unsigned first, unsigned n) {
...
__isl_give isl_basic_map *isl_basic_map_drop_constraints_involving_unknown_divs(
  ...
  for (i = 0; i < n_div; ++i) {
    ...
    bmap =
        isl_basic_map_drop_constraints_involving_dims(bmap, isl_dim_div, i, 1);
    n_div = isl_basic_map_dim(bmap, isl_dim_div);
    if (n_div < 0)
      return isl_basic_map_free(bmap);
    i = -1;
 ...
 */
__isl_give isl_map *
isl_map_drop_constraints_involving_unknown_divs(__isl_take isl_map *map) {
  int i;
  isl_bool known;
  ...
  for (i = 0; i < map->n; ++i) {
    map->p[i] =
        isl_basic_map_drop_constraints_involving_unknown_divs(map->p[i]);
    if (!map->p[i])
      return isl_map_free(map);
  }
...
static int set_reduced_using_equalities(isl_int *dst, isl_int *src,
                                        __isl_keep isl_basic_set *bset,
                                        int *elim, unsigned total) {
  return reduced_using_equalities(dst, src, bset_to_bmap(bset), elim, total);
}

static __isl_give isl_basic_set *
isl_basic_set_reduce_using_equalities(__isl_take isl_basic_set *bset,
  ...
  for (i = 0; i < bset->n_eq; ++i)
    set_reduced_using_equalities(bset->eq[i], bset->eq[i], context, elim, dim);
  for (i = 0; i < bset->n_ineq; ++i)
    set_reduced_using_equalities(bset->ineq[i], bset->ineq[i], context, elim,
                                 dim);
  isl_basic_set_free(context);
  free(elim);
...
static __isl_give isl_basic_set *
remove_shifted_constraints(__isl_take isl_basic_set *bset,
                           __isl_keep isl_basic_set *context) {
  struct isl_constraint_index ci;
  int k;
 ...
 */
static __isl_give isl_basic_map *
isl_basic_map_remove_shifted_constraints(__isl_take isl_basic_map *bmap,
 ...
 */
static __isl_give isl_basic_map *
drop_unrelated_constraints(__isl_take isl_basic_map *bmap, int *relevant) {
  int i;
  isl_size dim;
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_drop_unrelated_constraints(__isl_take isl_basic_map *bmap,
 ...
 */
static __isl_give isl_basic_set *
drop_irrelevant_constraints(__isl_take isl_basic_set *context,
                            __isl_keep isl_basic_set *bset) {
  int *group;
  isl_size dim;
  int i, j;
 ...
 */
static __isl_give isl_basic_set *
drop_irrelevant_constraints_marked(__isl_take isl_basic_set *context,
                                   __isl_keep isl_mat *ineq, int *row) {
  int *group;
  isl_size dim;
  int i, j;
  isl_size n;
 ...
 */
static __isl_give isl_basic_set *
update_ineq_free(__isl_take isl_basic_set *bset, __isl_take isl_mat *ineq,
 ...
 */
static __isl_give isl_basic_set *
uset_gist_full(__isl_take isl_basic_set *bset, __isl_take isl_mat *ineq,
               __isl_take isl_basic_set *context) {
  int i, r;
  int *row = NULL;
  isl_ctx *ctx;
...
static __isl_give isl_mat *extract_ineq(__isl_keep isl_basic_set *bset) {
  ...
  ineq = isl_mat_sub_alloc6(ctx, bset->ineq, 0, bset->n_ineq, 0, 1 + total);
 ...
 */
static __isl_give isl_basic_set *
uset_gist_uncompressed(__isl_take isl_basic_set *bset,
                       __isl_take isl_basic_set *context) {
  isl_mat *ineq;
 ...
 */
static __isl_give isl_basic_set *
replace_by_empty(__isl_take isl_basic_set *bset) {
  isl_space *space;
 ...
 */
static __isl_give isl_basic_set *
uset_gist_compressed(__isl_take isl_basic_set *bset,
                     __isl_take isl_basic_set *context, __isl_take isl_mat *T) {
  isl_ctx *ctx;
  isl_mat *ineq;
  int i;
  ...
  for (i = 0; i < n_row; ++i) {
    ...
    if (isl_int_is_one(ctx->normalize_gcd))
      continue;
    isl_seq_scale_down(ineq->row[i] + 1, ineq->row[i] + 1, ctx->normalize_gcd,
                       n_col - 1);
    isl_int_fdiv_r(rem, ineq->row[i][0], ctx->normalize_gcd);
    isl_int_fdiv_q(ineq->row[i][0], ineq->row[i][0], ctx->normalize_gcd);
 ...
 */
static __isl_give isl_basic_set *
project_onto_involved(__isl_take isl_basic_set *bset,
  ...
  for (i = 0; i < n; ++i) {
    ...
    involved = isl_basic_set_involves_dims(template, isl_dim_set, i, 1);
    if (involved < 0)
      return isl_basic_set_free(bset);
    if (involved)
      continue;
...
static int n_div_eq(__isl_keep isl_basic_map *bmap) {
  ...
  for (i = 0; i < bmap->n_eq; ++i)
    if (isl_seq_first_non_zero(bmap->eq[i] + 1 + total, n_div) == -1)
      return i;
 ...
 */
static __isl_give isl_basic_map *
basic_map_from_equalities(__isl_take isl_space *space, __isl_take isl_mat *eq) {
  int i, k;
  isl_size total;
  ...
  bmap = isl_basic_map_alloc_space(isl_space_copy(space), 0, eq->n_row, 0);
...
combined_variable_compression(__isl_keep isl_basic_map *bmap1, int n1,
                              __isl_keep isl_basic_map *bmap2, int n2,
                              int total) {
  isl_ctx *ctx;
  isl_mat *E1, *E2, *V;
  isl_basic_map *bmap;
  ...
  if (bmap1->n_eq == n1) {
    E2 = isl_mat_sub_alloc6(ctx, bmap2->eq, n2, bmap2->n_eq - n2, 0, 1 + total);
    return isl_mat_variable_compression(E2, NULL);
  }
  if (bmap2->n_eq == n2) {
    E1 = isl_mat_sub_alloc6(ctx, bmap1->eq, n1, bmap1->n_eq - n1, 0, 1 + total);
...
static __isl_give isl_mat *extract_compressed_stride_constraints(
  ...
  V = combined_variable_compression(bmap, bmap_n_eq, context, context_n_eq,
                                    total);

  A = isl_mat_sub_alloc6(ctx, context->eq, 0, context_n_eq, 0, 1 + total);
  B = isl_mat_sub_alloc6(ctx, context->eq, 0, context_n_eq, 1 + total, n_div);
  ...
  A = isl_mat_sub_alloc6(ctx, bmap->eq, 0, bmap_n_eq, 0, 1 + total + n_div);
 ...
 */
static __isl_give isl_basic_map *
reduce_stride_constraints(__isl_take isl_basic_map *bmap, int n,
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (div < 0)
      isl_die(isl_basic_map_get_ctx(bmap), isl_error_internal,
              "equality constraints modified unexpectedly", goto error);
    ...
    if (isl_int_is_one(gcd))
      continue;
    isl_int_divexact(bmap->eq[i][1 + total + div], bmap->eq[i][1 + total + div],
                     gcd);
 ...
 */
static __isl_give isl_basic_map *
gist_strides(__isl_take isl_basic_map *bmap,
  ...
  A = extract_compressed_stride_constraints(bmap, bmap_n_eq, context,
                                            context_n_eq);
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_gist(__isl_take isl_basic_map *bmap,
                   __isl_take isl_basic_map *context) {
...
__isl_give isl_map *isl_map_gist_basic_map(__isl_take isl_map *map,
  ...
  if (!map)
    goto error;
  for (i = map->n - 1; i >= 0; --i) {
    map->p[i] = isl_basic_map_gist(map->p[i], isl_basic_map_copy(context));
    if (!map->p[i])
      goto error;
 ...
 */
static __isl_give isl_basic_map *
drop_inequalities(__isl_take isl_basic_map *bmap,
                  __isl_keep isl_basic_map *context) {
  int i1, i2;
  ...
  while (bmap && i1 >= 0 && i2 >= 0) {
    ...
    if (isl_seq_first_non_zero(bmap->ineq[i1] + 1 + total, extra) != -1) {
      --i1;
      continue;
    }
 ...
 */
static __isl_give isl_basic_map *
drop_equalities(__isl_take isl_basic_map *bmap,
                __isl_keep isl_basic_map *context) {
  int i1, i2;
  isl_size total, bmap_total;
  unsigned extra;
  ...
  while (bmap && i1 >= 0 && i2 >= 0) {
    ...
    if (isl_seq_first_non_zero(bmap->eq[i1] + 1 + total, extra) != -1)
      break;
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_plain_gist(__isl_take isl_basic_map *bmap,
                         __isl_take isl_basic_map *context) {
  isl_bool done, known;
...
replace_by_disjunct(__isl_take isl_map *map, int pos,
                    __isl_take isl_basic_map *context) {
  isl_basic_map *bmap;
 ...
 */
__isl_give isl_map *
isl_map_plain_gist_basic_map(__isl_take isl_map *map,
                             __isl_take isl_basic_map *context) {
  int i;
  isl_bool univ, known;
  ...
  if (known < 0)
    goto error;
  if (!known)
    isl_die(isl_map_get_ctx(map), isl_error_invalid, "context has unknown divs",
            goto error);
  ...
  for (i = 0; i < map->n; ++i) {
    map->p[i] =
        isl_basic_map_plain_gist(map->p[i], isl_basic_map_copy(context));
 ...
 */
__isl_give isl_set *
isl_set_plain_gist_basic_set(__isl_take isl_set *set,
                             __isl_take isl_basic_set *context) {
  return set_from_map(
      isl_map_plain_gist_basic_map(set_to_map(set), bset_to_bmap(context)));
}
...
__isl_give isl_map *isl_map_gist(__isl_take isl_map *map,
  ...
  if (n_disjunct_context == 1) {
    ...
    hull = isl_map_unshifted_simple_hull_from_map_list(context, list);
  }
...
__isl_give isl_basic_set *
isl_basic_set_gist(__isl_take isl_basic_set *bset,
                   __isl_take isl_basic_set *context) {
  return bset_from_bmap(
      isl_basic_map_gist(bset_to_bmap(bset), bset_to_bmap(context)));
}

__isl_give isl_set *isl_set_gist_basic_set(__isl_take isl_set *set,
                                           __isl_take isl_basic_set *context) {
  return set_from_map(
      isl_map_gist_basic_map(set_to_map(set), bset_to_bmap(context)));
}

__isl_give isl_set *
isl_set_gist_params_basic_set(__isl_take isl_set *set,
                              __isl_take isl_basic_set *context) {
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_gist_domain(__isl_take isl_basic_map *bmap,
                          __isl_take isl_basic_set *context) {
  isl_space *space = isl_basic_map_get_space(bmap);
...
isl_bool isl_basic_map_plain_is_disjoint(__isl_keep isl_basic_map *bmap1,
  ...
  for (i = 0; i < bmap2->n_eq; ++i) {
    int reduced;
    reduced = reduced_using_equalities(v->block.data, bmap2->eq[i], bmap1, elim,
                                       total);
  ...
  for (i = 0; i < bmap2->n_ineq; ++i) {
    int reduced;
    reduced = reduced_using_equalities(v->block.data, bmap2->ineq[i], bmap1,
                                       elim, total);
  ...
  for (i = 0; i < bmap1->n_ineq; ++i) {
    int reduced;
    reduced = reduced_using_equalities(v->block.data, bmap1->ineq[i], bmap2,
                                       elim, total);
    if (reduced && isl_int_is_neg(v->block.data[0]) &&
        isl_seq_first_non_zero(v->block.data + 1, total) == -1)
      goto disjoint;
  ...
  for (i = 0; i < n_div; ++i) {
    ...
    for (j = 0; j < bmap->n_ineq; ++j) {
      ...
      isl_int_mul(bmap->ineq[j][1 + v_div + div],
                  bmap->ineq[j][1 + v_div + div], bmap->ineq[l][0]);
 ...
 */
static isl_bool int_between_bounds(__isl_keep isl_basic_map *bmap, int i, int l,
                                   int u, struct test_ineq_data *data) {
  unsigned offset;
  isl_size n_div;
  ...
  isl_int_gcd(data->g, bmap->ineq[l][offset + i], bmap->ineq[u][offset + i]);
  ...
  isl_seq_combine(data->v->el, data->fl, bmap->ineq[u], data->fu, bmap->ineq[l],
                  offset + n_div);
 ...
 */
static __isl_give isl_basic_map *
drop_more_redundant_divs(__isl_take isl_basic_map *bmap, __isl_take int *pairs,
                         int n) {
  isl_ctx *ctx;
    ...
    for (l = 0; l < bmap->n_ineq; ++l) {
      ...
      for (u = 0; u < bmap->n_ineq; ++u) {
        ...
        if (isl_int_is_negone(bmap->ineq[u][off + i]))
          continue;
        has_int = int_between_bounds(bmap, i, l, u, &data);
        if (has_int < 0)
          goto error;
  ...
  for (i = 0; i < bmap->n_ineq; ++i) {
    ...
    if (isl_int_is_zero(bmap->ineq[i][1 + v_div + div1])) {
      if (isl_int_is_pos(bmap->ineq[i][1 + v_div + div2]))
        isl_seq_combine(bmap->ineq[i], m, bmap->ineq[i], ctx->one,
                        bmap->ineq[l], total);
      else
        isl_seq_combine(bmap->ineq[i], m, bmap->ineq[i], ctx->one,
                        bmap->ineq[u], total);
 ...
 */
static __isl_give isl_basic_map *
coalesce_or_drop_more_redundant_divs(__isl_take isl_basic_map *bmap, int *pairs,
                                     int n) {
  int i, l, u;
  isl_size v_div;
  isl_size n_div;
 ...
 */
static __isl_give isl_basic_map *
drop_redundant_divs_again(__isl_take isl_basic_map *bmap, __isl_take int *pairs,
                          int simplify) {
  if (simplify)
 ...
 * "ineq" of "map"?
 */
static isl_bool has_coef_one(__isl_keep isl_basic_map *bmap, int div,
                             int ineq) {
  unsigned o_div;
 ...
 */
static __isl_give isl_basic_map *
set_eq_and_try_again(__isl_take isl_basic_map *bmap, int ineq,
 ...
 */
static __isl_give isl_basic_map *
drop_div_and_try_again(__isl_take isl_basic_map *bmap, int div, int ineq1,
                       int ineq2, __isl_take int *pairs) {
  if (ineq1 > ineq2) {
 ...
 * in *l.
 */
static void lower_bound_from_parallel(__isl_keep isl_basic_map *bmap, int ineq,
                                      int lower, int pos, isl_int *l) {
 ...
 * in *u.
 */
static void lower_bound_from_opposite(__isl_keep isl_basic_map *bmap, int ineq,
                                      int upper, int pos, isl_int *u) {
 ...
 * Return -1 on error.
 */
static int lower_bound_is_cst(__isl_keep isl_basic_map *bmap, int div,
                              int ineq) {
  int i;
  int lower = -1, upper = -1;
  unsigned o_div;
 ...
 */
static __isl_give isl_basic_map *
isl_basic_map_drop_redundant_divs_ineq(__isl_take isl_basic_map *bmap) {
  int i, j;
  isl_size off;
  int *pairs = NULL;
  int n = 0;
  ...
  for (i = 0; i < bmap->n_div; ++i) {
    ...
    if (!opp) {
      ...
      if (one < 0)
        goto error;
      if (one)
        return set_eq_and_try_again(bmap, last_pos, pairs);
      lower = lower_bound_is_cst(bmap, i, last_pos);
      if (lower < 0)
        goto error;
      if (lower < n_ineq)
        return fix_cst_lower(bmap, i, last_pos, lower, pairs);
      continue;
    }

    isl_int_add(bmap->ineq[last_pos][0], bmap->ineq[last_pos][0],
                bmap->ineq[last_neg][0]);
    isl_int_add_ui(bmap->ineq[last_pos][0], bmap->ineq[last_pos][0], 1);
    redundant =
        isl_int_ge(bmap->ineq[last_pos][0], bmap->ineq[last_pos][1 + off + i]);
    isl_int_sub_ui(bmap->ineq[last_pos][0], bmap->ineq[last_pos][0], 1);
    isl_int_sub(bmap->ineq[last_pos][0], bmap->ineq[last_pos][0],
                bmap->ineq[last_neg][0]);
    if (redundant)
      return drop_div_and_try_again(bmap, i, last_pos, last_neg, pairs);
    if (defined)
 ...
 */
static __isl_give isl_basic_map *
isl_basic_map_preimage_vars(__isl_take isl_basic_map *bmap, unsigned pos,
  ...
  if (n_col != n_row)
    isl_die(isl_mat_get_ctx(T), isl_error_invalid, "expecting square matrix",
            goto error);
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_drop_redundant_divs(__isl_take isl_basic_map *bmap) {
  int first;
  int i;
  unsigned o_div;
  isl_size n_div;
  ...
  for (i = 0; i < bmap->n_eq; ++i) {
    l = isl_seq_first_non_zero(bmap->eq[i] + o_div + first, n_div - (first));
    if (l < 0)
      continue;
    l += first;
    if (isl_seq_first_non_zero(bmap->eq[i] + o_div + l + 1, n_div - (l + 1)) ==
        -1)
      continue;
    break;
  }
...
static __isl_give isl_vec *normalize_constraint(__isl_take isl_vec *v,
  ...
  isl_seq_scale_down(v->el + 1, v->el + 1, ctx->normalize_gcd, v->size - 1);
  return v;
}
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_reduce_coefficients(__isl_take isl_basic_map *bmap) {
  isl_size total;
  isl_bool multi;
  isl_ctx *ctx;
  isl_vec *v;
 ...
 */
__isl_give isl_basic_map *
isl_basic_map_shift_div(__isl_take isl_basic_map *bmap, int div, int pos,
                        isl_int shift) {
  int i;
  isl_size total, n_div;
  ...
  for (i = 0; i < bmap->n_eq; ++i) {
    if (isl_int_is_zero(bmap->eq[i][1 + total + div]))
      continue;
    isl_int_submul(bmap->eq[i][pos], shift, bmap->eq[i][1 + total + div]);
  }
  for (i = 0; i < bmap->n_ineq; ++i) {
    if (isl_int_is_zero(bmap->ineq[i][1 + total + div]))
      continue;
    isl_int_submul(bmap->ineq[i][pos], shift, bmap->ineq[i][1 + total + div]);
  }
  for (i = 0; i < bmap->n_div; ++i) {
    if (isl_int_is_zero(bmap->div[i][0]))
      continue;
    if (isl_int_is_zero(bmap->div[i][1 + 1 + total + div]))
      continue;
    isl_int_submul(bmap->div[i][1 + pos], shift,
