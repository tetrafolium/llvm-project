...
static SVal conjureOffsetSymbolOnLocation(SVal Symbol, SVal Other,
  ...
  if (Other.getAs<Loc>() && Ty->isIntegralOrEnumerationType() &&
      Symbol.isUnknown()) {
...
ProgramStateRef ExprEngine::handleLVectorSplat(ProgramStateRef state,
  ...
  SVal result = svalBuilder.conjureSymbolVal(nullptr, CastE, LCtx, resultType,
                                             currBldrCtx->blockCount());
...
void ExprEngine::handleUOExtension(ExplodedNodeSet::iterator I,
  ...
  ProgramStateRef state = (*I)->getState();
  const LocationContext *LCtx = (*I)->getLocationContext();
  Bldr.generateNode(U, *I, state->BindExpr(U, LCtx, state->getSVal(Ex, LCtx)));
}
