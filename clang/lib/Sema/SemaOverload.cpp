...
ImplicitConversionRank clang::GetConversionRank(ImplicitConversionKind Kind) {
  static const ImplicitConversionRank Rank[(int)ICK_Num_Conversion_Kinds] = {
      ...
      ICR_Conversion, ICR_Conversion, ICR_Conversion, ICR_Conversion,
      ICR_OCL_Scalar_Widening, ICR_Complex_Real_Conversion, ICR_Conversion,
      ICR_Conversion, ICR_Writeback_Conversion,
  ...
  FloatingIntegralConversion:
    if (FromType->isRealFloatingType() && ToType->isIntegralType(Ctx)) {
      ...
      if (Optional<llvm::APSInt> IntConstantValue =
        ...
        bool ignored;
        Result.convertToInteger(ConvertedValue, llvm::APFloat::rmTowardZero,
                                &ignored);
    ...
    if (FromType->isRealFloatingType() && ToType->isRealFloatingType() &&
      ...
      if (Initializer->isCXX11ConstantExpr(Ctx, &ConstantValue)) {
        ...
        llvm::APFloat::opStatus ConvertStatus =
            FloatVal.convert(Ctx.getFloatTypeSemantics(ToType),
...
/// signature.
Sema::OverloadKind Sema::CheckOverload(Scope *S, FunctionDecl *New,
  ...
  if (OldMethod && NewMethod && !OldMethod->isStatic() &&
    ...
    if (OldMethod->getRefQualifier() != NewMethod->getRefQualifier()) {
      if (!UseMemberUsingDeclRules &&
        ...
        //     and the same template parameter lists cannot be overloaded if any
        //     of them, but not all, have a ref-qualifier (8.3.5).
  ...
  if (FromType == S.Context.OverloadTy) {
    ...
    if (FunctionDecl *Fn = S.ResolveAddressOfOverloadedFunction(
      ...
      if (!S.Context.hasSameUnqualifiedType(
        ...
        if (!S.IsFunctionConversion(
                FromType, S.ExtractUnqualifiedFunctionType(ToType), resultTy))
...
bool Sema::IsPointerConversion(Expr *From, QualType FromType, QualType ToType,
  ...
  if (FromType->isObjCObjectPointerType() && ToPointeeType->isVoidType() &&
    ...
    ConvertedType = BuildSimilarlyQualifiedPointerType(
        FromType->getAs<ObjCObjectPointerType>(), ToPointeeType, ToType,
        Context);
    return true;
  }
...
bool Sema::CheckMemberPointerConversion(Expr *From, QualType ToType,
  ...
  if (Paths.isAmbiguous(
          Context.getCanonicalType(FromClass).getUnqualifiedType())) {
  ...
  if (const RecordType *VBase = Paths.getDetectedVirtual()) {
    Diag(From->getExprLoc(), diag::err_memptr_conv_via_virtual)
        << FromClass << ToClass << QualType(VBase, 0) << From->getSourceRange();
    return true;
  }

  if (!IgnoreBaseAccess)
    CheckBaseClassAccess(From->getExprLoc(), FromClass, ToClass, Paths.front(),
...
Sema::CompareReferenceRelationship(SourceLocation Loc, QualType OrigT1,
  ...
  if (UnqualT1 == UnqualT2) {
    // Nothing to do.
  } else if (isCompleteType(Loc, OrigT2) && isTypeValid(UnqualT1) &&
             isTypeValid(UnqualT2) && IsDerivedFrom(Loc, UnqualT2, UnqualT1))
  ...
  if (!X.isNull()) {
    for (unsigned i = 0, e = From->getNumInits(); i < e; ++i) {
      ...
      ImplicitConversionSequence ICS = TryCopyInitialization(
          S, Init, X, SuppressUserConversions, InOverloadResolution,
          AllowObjCWritebackConversion);
...
/// expression.
ExprResult Sema::PerformObjectArgumentInitialization(
...
/// operators.
void Sema::AddMethodCandidate(CXXMethodDecl *Method, DeclAccessPair FoundDecl,
  ...
  for (unsigned I = 0, N = std::min(ParamTypes.size(), Args.size()); I != N;
    ...
    if (!ParamType->isDependentType()) {
      ...
      Conversions[ConvIdx] = TryCopyInitialization(
          *this, Args[I], ParamType, SuppressUserConversions,
          /*InOverloadResolution=*/true,
          /*AllowObjCWritebackConversion=*/
...
void Sema::AddTemplateConversionCandidate(
  ...
  if (TemplateDeductionResult Result = DeduceTemplateArguments(
          FunctionTemplate, ToType, Specialization, Info)) {
    OverloadCandidate &Candidate = CandidateSet.addCandidate();
    Candidate.FoundDecl = FoundDecl;
    Candidate.Function = FunctionTemplate->getTemplatedDecl();
    Candidate.Viable = false;
...
public:
  BuiltinOperatorOverloadBuilder(
      Sema &S, ArrayRef<Expr *> Args, Qualifiers VisibleTypeConversionsQuals,
      bool HasArithmeticOrEnumeralCandidateType,
      SmallVectorImpl<BuiltinCandidateTypeSet> &CandidateTypes,
      OverloadCandidateSet &CandidateSet)
      : S(S), Args(Args),
...
static void DiagnoseOpenCLExtensionDisabled(Sema &S, OverloadCandidate *Cand) {
  ...
  S.Diag(Callee->getLocation(), diag::note_ovl_candidate_disabled_by_extension)
...
Sema::resolveAddressOfSingleOverloadCandidate(Expr *E, DeclAccessPair &Pair) {
  ...
  for (auto I = Ovl->decls_begin(), E = Ovl->decls_end(); I != E; ++I) {
    ...
    // previous one.
    if (Result) {
      Optional<bool> MoreConstrainedThanPrevious =
...
bool Sema::ResolveAndFixSingleFunctionTemplateSpecialization(
    ExprResult &SrcExpr, bool doFunctionPointerConverion, bool complain,
  ...
  case OR_Success: {
    ...
    if (FnDecl) {
      ...
      if (CXXMethodDecl *Method = dyn_cast<CXXMethodDecl>(FnDecl)) {
        ...
        ExprResult InputRes = PerformObjectArgumentInitialization(
  ...
  case OR_Success: {
    ...
    if (FnDecl) {
      ...
      ExprResult ArgsRes0 = PerformImplicitConversion(
          Args[0], Best->BuiltinParamTypes[0], Best->Conversions[0], AA_Passing,
          CCK_ForBuiltinOverloadedOp);
      ...
      ExprResult ArgsRes1 = PerformImplicitConversion(
          Args[1], Best->BuiltinParamTypes[1], Best->Conversions[1], AA_Passing,
          CCK_ForBuiltinOverloadedOp);
...
ExprResult Sema::BuildOverloadedArrowExpr(Scope *S, Expr *Base,
  ...
  CXXMethodDecl *Method = cast<CXXMethodDecl>(Best->Function);
  ExprResult BaseResult = PerformObjectArgumentInitialization(
...
Expr *Sema::FixOverloadedFunctionReference(Expr *E, DeclAccessPair Found,
  ...
  if (ParenExpr *PE = dyn_cast<ParenExpr>(E)) {
    Expr *SubExpr = FixOverloadedFunctionReference(PE->getSubExpr(), Found, Fn);
  ...
  if (ImplicitCastExpr *ICE = dyn_cast<ImplicitCastExpr>(E)) {
    ...
    assert(
        Context.hasSameType(ICE->getSubExpr()->getType(), SubExpr->getType()) &&
