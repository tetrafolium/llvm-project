...
protected:
  ...
  bool m_fake : 1, // If true, then this section only can contain the address if
                   // one of its
                   // children contains an address. This allows for gaps between
                   // the children that are contained in the address range for
                   // this section, but do not produce hits unless the children
                   // contain the address.
