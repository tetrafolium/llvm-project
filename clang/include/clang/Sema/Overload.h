...
struct AmbiguousConversionSequence {
  ...
  QualType getToType() const { return QualType::getFromOpaquePtr(ToTypePtr); }
...
private:
  enum { Uninitialized = BadConversion + 1 };
...
public:
  ...
  void setBad(BadConversionSequence::FailureKind Failure, Expr *FromExpr,
              QualType ToType) {
  ...
  void setBad(BadConversionSequence::FailureKind Failure, QualType FromType,
              QualType ToType) {
    setKind(BadConversion);
  ...
  /// Whether the target is really a std::initializer_list, and the
  /// sequence only represents the worst element conversion.
  bool isStdInitializerListElement() const { return StdInitializerListElement; }
  ...
  void DiagnoseAmbiguousConversion(Sema &S, SourceLocation CaretLoc,
...
struct OverloadCandidate {
  ...
  bool TryToFixBadConversion(unsigned Idx, Sema &S) {
    bool CanFix = Fix.tryToFixConversion(Conversions[Idx].Bad.FromExpr,
...
public:
  ...
  void NoteCandidates(Sema &S, ArrayRef<Expr *> Args,
                      ArrayRef<OverloadCandidate *> Cands, StringRef Opc = "",
                      SourceLocation OpLoc = SourceLocation());
