...
bool ensureInitTF() {
    return TFLibInitializer->IsInitialized;
}
...
public:
    ...
    std::vector<TF_Tensor *> &getOutput() {
        return Output;
    }
...
public:
    ...
    const std::vector<TF_Tensor *> &getInput() const {
        return Input;
    }
...
TFModelEvaluator::TFModelEvaluator(StringRef SavedModelPath,
                                   ...
                                   const char *Tags)
    : TFModelEvaluator(
          SavedModelPath, InputSpecs, [&](size_t I) {
    return OutputSpecs[I];
},
