...
__isl_give isl_val *isl_stream_read_val(__isl_keep isl_stream *s) {
  ...
  if (tok->type == '-' && isl_stream_eat_if_available(s, ISL_TOKEN_INFTY)) {
    isl_token_free(tok);
...
accept_affine_list(__isl_keep isl_stream *s, __isl_take isl_space *space,
                   struct vars *v);
...
static __isl_give isl_pw_aff *accept_affine(__isl_keep isl_stream *s,
  ...
  for (;;) {
    ...
    if (tok->type == '(' || is_start_of_div(tok) ||
        ...
        tok->type == ISL_TOKEN_IDENT || tok->type == ISL_TOKEN_AFF) {
      isl_pw_aff *term;
      isl_stream_push_token(s, tok);
      tok = NULL;
      term = accept_affine_factor(s, isl_space_copy(space), v);
      if (sign < 0)
...
accept_extended_affine(__isl_keep isl_stream *s, __isl_take isl_space *space,
 ...
 */
static void set_current_line_col(__isl_keep isl_stream *s, int *line,
                                 int *col) {
  struct isl_token *tok;
...
static __isl_give isl_map *read_var_def(__isl_keep isl_stream *s,
                                        ...
                                        enum isl_dim_type type, struct vars *v,
                                        int rational) {
  isl_pw_aff *def;
  isl_size pos;
  isl_map *def_map;
...
accept_affine_list(__isl_keep isl_stream *s, __isl_take isl_space *space,
  ...
  for (;;) {
    ...
    pwaff = accept_affine(s, isl_space_copy(space), v);
    list = isl_pw_aff_list_concat(list, isl_pw_aff_list_from_pw_aff(pwaff));
    if (!list)
      goto error;
  }
...
static int next_is_end_tuple_element(__isl_keep isl_stream *s) {
  return isl_stream_next_token_is(s, ',') || isl_stream_next_token_is(s, ']');
}
 ...
 */
static __isl_give isl_pw_aff *
identity_tuple_el_on_space(__isl_take isl_space *space, struct vars *v) {
  isl_aff *aff;
...
static int resolve_paren_expr(__isl_keep isl_stream *s, struct vars *v,
                              __isl_take isl_map *map, int rational);
 ...
 */
static __isl_give isl_pw_aff *
update_piecewise_affine_colon(__isl_take isl_pw_aff *pa,
                              __isl_keep isl_stream *s, struct vars *v,
                              int rational) {
  isl_space *dom_space;
  isl_map *map;
 ...
 */
static __isl_give isl_pw_aff *
accept_piecewise_affine(__isl_keep isl_stream *s, __isl_take isl_space *space,
                        struct vars *v, int rational) {
  isl_pw_aff *res;
  isl_space *res_space;
  ...
  do {
    ...
    if (seen_paren)
      pa = accept_piecewise_affine(s, isl_space_copy(space), v, rational);
    else
      pa = accept_extended_affine(s, isl_space_copy(space), v, rational);
 ...
 */
static __isl_give isl_space *read_tuple_list(
    __isl_keep isl_stream *s, struct vars *v, __isl_take isl_space *space,
    int rational, int comma,
    __isl_give isl_space *(*read_el)(__isl_keep isl_stream *s, struct vars *v,
                                     __isl_take isl_space *space, int rational,
                                     void *user),
    void *user) {
  if (!space)
    return NULL;
  ...
  for (;;) {
    ...
    tok = isl_stream_next_token(s);
    if (!comma && tok && tok->type == ']' && isl_stream_next_token_is(s, '[')) {
      isl_token_free(tok);
      tok = isl_stream_next_token(s);
    } else if (!tok || tok->type != ',') {
      if (tok)
 ...
 */
static __isl_give isl_space *read_tuple_space(
    __isl_keep isl_stream *s, struct vars *v, __isl_take isl_space *space,
    int rational, int comma,
    __isl_give isl_space *(*read_el)(__isl_keep isl_stream *s, struct vars *v,
                                     __isl_take isl_space *space, int rational,
  ...
  if (next_is_tuple(s)) {
    isl_space *out;
    res = read_tuple_space(s, v, isl_space_copy(space), rational, comma,
                           read_el, user);
    if (isl_stream_eat(s, ISL_TOKEN_TO))
      goto error;
    out = read_tuple_space(s, v, isl_space_copy(space), rational, comma,
                           read_el, user);
    res = isl_space_product(res, out);
  } else
    res = read_tuple_list(s, v, isl_space_copy(space), rational, comma, read_el,
                          user);
  if (isl_stream_eat(s, ']'))
    goto error;
...
read_tuple(__isl_keep isl_stream *s, struct vars *v, int rational, int comma) {
  int i;
  isl_size n;
  isl_space *space;
  isl_pw_aff_list *list;
  ...
  space = read_tuple_space(s, v, space, rational, comma, &read_tuple_pw_aff_el,
                           &list);
...
static __isl_give isl_map *map_from_tuple(__isl_take isl_multi_pw_aff *tuple,
  ...
  if (type == isl_dim_param) {
    if (isl_space_has_tuple_name(space, isl_dim_set) ||
      ...
      isl_die(ctx, isl_error_invalid,
              "parameter tuples cannot be named or nested", goto error);
    }
    map = isl_map_add_dims(map, type, n);
    for (i = 0; i < n; ++i) {
      isl_id *id;
      if (!isl_space_has_dim_name(space, isl_dim_set, i))
        isl_die(ctx, isl_error_invalid, "parameters must be named", goto error);
  ...
  for (i = 0; i < n; ++i) {
    ...
    space = isl_pw_aff_get_domain_space(pa);
    aff = isl_aff_zero_on_domain(isl_local_space_from_space(space));
    aff = isl_aff_add_coefficient_si(aff, isl_dim_in, v->n - n + i, -1);
...
construct_constraints(__isl_take isl_set *set, int type,
                      __isl_keep isl_pw_aff_list *left,
                      __isl_keep isl_pw_aff_list *right, int rational) {
  isl_set *cond;
...
static __isl_give isl_map *add_constraint(__isl_keep isl_stream *s,
  ...
  for (;;) {
    ...
    if (is_list_comparator_type(type) && n1 != n2) {
      isl_stream_error(s, NULL, "list arguments not of same size");
      goto error;
    }
...
static __isl_give isl_map *read_exists(__isl_keep isl_stream *s, struct vars *v,
                                       __isl_take isl_map *map, int rational) {
  int n = v->n;
...
basic_map_read_polylib_constraint(__isl_keep isl_stream *s,
  ...
  for (j = 0; j < 1 + total; ++j) {
    ...
    if (tok->on_new_line) {
      isl_stream_error(s, tok, "coefficient should not appear on new line");
      isl_stream_push_token(s, tok);
      goto error;
    }
...
static __isl_give isl_basic_map *
basic_map_read_polylib(__isl_keep isl_stream *s) {
  int i;
  struct isl_token *tok;
  ...
  if (tok->type != ISL_TOKEN_VALUE || tok2->type != ISL_TOKEN_VALUE) {
    isl_stream_push_token(s, tok2);
    isl_stream_push_token(s, tok);
    isl_stream_error(s, NULL, "expecting constraint matrix dimensions");
    return NULL;
  }
  ...
  if (tok) {
    if (tok->type != ISL_TOKEN_VALUE) {
      isl_stream_error(s, tok, "expecting number of output dimensions");
      isl_stream_push_token(s, tok);
      goto error;
    }
    ...
    tok = isl_stream_next_token_on_same_line(s);
    if (!tok || tok->type != ISL_TOKEN_VALUE) {
      isl_stream_error(s, tok, "expecting number of input dimensions");
      if (tok)
        isl_stream_push_token(s, tok);
      goto error;
    }
    ...
    tok = isl_stream_next_token_on_same_line(s);
    if (!tok || tok->type != ISL_TOKEN_VALUE) {
      isl_stream_error(s, tok, "expecting number of existentials");
      if (tok)
        isl_stream_push_token(s, tok);
      goto error;
    }
    ...
    tok = isl_stream_next_token_on_same_line(s);
    if (!tok || tok->type != ISL_TOKEN_VALUE) {
      isl_stream_error(s, tok, "expecting number of parameters");
      if (tok)
        isl_stream_push_token(s, tok);
      goto error;
    }
    ...
    if (n_col != 1 + out + in + local + nparam + 1) {
      isl_stream_error(s, NULL, "dimensions don't match");
      goto error;
    }
  } else
    out = n_col - 2 - nparam;
...
static __isl_give isl_map *map_read_polylib(__isl_keep isl_stream *s) {
  ...
  for (i = 1; map && i < n; ++i)
    map = isl_map_union(map, isl_map_from_basic_map(basic_map_read_polylib(s)));
...
read_term(__isl_keep isl_stream *s, __isl_keep isl_map *map, struct vars *v);

static __isl_give isl_pw_qpolynomial *
read_factor(__isl_keep isl_stream *s, __isl_keep isl_map *map, struct vars *v) {
  ...
  if (tok->type == '(') {
    ...
    if (tok2 && tok2->type == '/') {
      ...
      qp = isl_qpolynomial_rat_cst_on_domain(isl_map_get_space(map), tok->u.v,
                                             tok2->u.v);
      isl_token_free(tok2);
    } else {
      isl_stream_push_token(s, tok2);
      qp = isl_qpolynomial_cst_on_domain(isl_map_get_space(map), tok->u.v);
    }
    isl_token_free(tok);
...
read_term(__isl_keep isl_stream *s, __isl_keep isl_map *map, struct vars *v) {
  ...
  for (;;) {
    ...
    } else if (tok->type == ISL_TOKEN_VALUE && isl_int_is_neg(tok->u.v)) {
      isl_pw_qpolynomial *pwqp2;
...
static struct isl_obj obj_read_poly_or_fold(__isl_keep isl_stream *s,
  ...
  while (isl_stream_eat_if_available(s, ',')) {
    ...
    pwqp = read_term(s, set, v);
    pwf_i = isl_pw_qpolynomial_fold_from_pw_qpolynomial(isl_fold_max, pwqp);
...
static struct isl_obj obj_add(__isl_keep isl_stream *s, struct isl_obj obj1,
                              struct isl_obj obj2) {
  ...
  if (obj1.type != obj2.type) {
    isl_stream_error(s, NULL, "attempt to combine incompatible objects");
    goto error;
  }
  if (!obj1.type->add)
...
static struct isl_obj obj_read(__isl_keep isl_stream *s) {
  ...
  if (tok->type == ISL_TOKEN_VALUE) {
    ...
    tok2 = isl_stream_next_token(s);
    if (!tok2 || tok2->type != ISL_TOKEN_VALUE || isl_int_is_neg(tok2->u.v)) {
      if (tok2)
        isl_stream_push_token(s, tok2);
...
__isl_give isl_map *isl_stream_read_map(__isl_keep isl_stream *s) {
  ...
  obj = obj_read(s);
  if (obj.v)
    isl_assert(s->ctx, obj.type == isl_obj_map || obj.type == isl_obj_set,
               goto error);
...
__isl_give isl_union_map *isl_stream_read_union_map(__isl_keep isl_stream *s) {
  ...
  if (obj.v && obj.type == isl_obj_union_set && isl_union_set_is_empty(obj.v))
    obj.type = isl_obj_union_map;
...
static __isl_give isl_basic_map *basic_map_read(__isl_keep isl_stream *s) {
  ...
  if (obj.v && (obj.type != isl_obj_map && obj.type != isl_obj_set))
    isl_die(s->ctx, isl_error_invalid, "not a (basic) set or map", goto error);
  map = obj.v;
  if (!map)
    return NULL;
...
static __isl_give isl_basic_set *basic_set_read(__isl_keep isl_stream *s) {
  ...
  if (!isl_basic_map_may_be_set(bmap))
    isl_die(s->ctx, isl_error_invalid, "input is not a set", goto error);
...
__isl_give isl_map *isl_map_read_from_file(struct isl_ctx *ctx, FILE *input) {
  struct isl_map *map;
  isl_stream *s = isl_stream_new_file(ctx, input);
  if (!s)
    return NULL;
...
__isl_give isl_set *isl_set_read_from_file(struct isl_ctx *ctx, FILE *input) {
  isl_set *set;
  isl_stream *s = isl_stream_new_file(ctx, input);
  if (!s)
    return NULL;
...
isl_stream_read_pw_qpolynomial(__isl_keep isl_stream *s) {
  struct isl_obj obj;

  obj = obj_read(s);
  if (obj.v)
    isl_assert(s->ctx, obj.type == isl_obj_pw_qpolynomial, goto error);
...
__isl_give isl_aff *isl_stream_read_aff(__isl_keep isl_stream *s) {
  ...
  if (dim < 0)
    goto error;
  if (dim != 1)
    isl_die(s->ctx, isl_error_invalid, "expecting single affine expression",
            goto error);
 ...
 */
static __isl_give isl_multi_pw_aff *
extract_mpa_from_tuple(__isl_take isl_space *dom_space,
                       __isl_keep isl_multi_pw_aff *tuple) {
  int i;
  isl_size dim, n;
  isl_space *space;
  ...
  if (!isl_space_is_params(dom_space))
    space =
        isl_space_map_from_domain_and_range(isl_space_copy(dom_space), space);
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (isl_pw_aff_involves_dims(pa, isl_dim_in, dim, i + 1)) {
      ...
      isl_pw_aff_free(pa);
      isl_die(ctx, isl_error_invalid, "not an affine expression",
              return isl_multi_pw_aff_free(mpa));
    }
...
read_conditional_multi_aff(__isl_keep isl_stream *s, __isl_take isl_set *dom,
                           struct vars *v) {
  isl_multi_pw_aff *tuple;
  isl_multi_pw_aff *mpa;
...
isl_stream_read_union_pw_multi_aff(__isl_keep isl_stream *s) {
  struct vars *v;
  isl_set *dom;
...
isl_stream_read_pw_multi_aff(__isl_keep isl_stream *s) {
  ...
  if (single_space < 0)
    ...
    isl_die(s->ctx, isl_error_invalid, "expecting expression in single space",
            upma = isl_union_pw_multi_aff_free(upma));
...
__isl_give isl_multi_aff *isl_stream_read_multi_aff(__isl_keep isl_stream *s) {
  ...
  if (!isl_set_plain_is_universe(dom))
    isl_die(s->ctx, isl_error_invalid, "expecting universe parameter domain",
            goto error);
  if (isl_stream_eat(s, '{'))
    goto error;
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (isl_aff_involves_dims(aff, isl_dim_in, dim, i + 1)) {
      isl_aff_free(aff);
      isl_die(s->ctx, isl_error_invalid, "not an affine expression",
              goto error);
    }
...
isl_stream_read_multi_pw_aff(__isl_keep isl_stream *s) {
  struct vars *v;
  isl_set *dom = NULL;
  isl_multi_pw_aff *tuple = NULL;
...
read_union_pw_aff_with_dom(__isl_keep isl_stream *s, __isl_take isl_set *dom,
                           struct vars *v) {
  isl_pw_aff *pa;
...
isl_stream_read_union_pw_aff(__isl_keep isl_stream *s) {
  struct vars *v;
  isl_set *dom;
  isl_union_pw_aff *upa = NULL;
...
read_multi_union_pw_aff_body_core(__isl_keep isl_stream *s, struct vars *v,
                                  __isl_take isl_space *space) {
  isl_union_pw_aff_list *list;
  isl_multi_union_pw_aff *mupa;

  list = isl_union_pw_aff_list_alloc(s->ctx, 0);
  space = read_tuple_space(s, v, space, 1, 0, &read_union_pw_aff_el, &list);
...
read_union_set_body(__isl_keep isl_stream *s, struct vars *v,
                    __isl_take isl_space *space) {
  struct isl_obj obj = {isl_obj_set, NULL};
  isl_map *map;
 ...
 */
static __isl_give isl_multi_union_pw_aff *
read_multi_union_pw_aff_body(__isl_keep isl_stream *s, struct vars *v,
                             __isl_take isl_space *space) {
  isl_multi_union_pw_aff *mupa;
 ...
 */
static __isl_give isl_multi_union_pw_aff *
read_multi_union_pw_aff_core(__isl_keep isl_stream *s) {
  struct vars *v;
  isl_set *dom = NULL;
  isl_space *space;
...
isl_stream_read_multi_union_pw_aff(__isl_keep isl_stream *s) {
  isl_multi_union_pw_aff *mupa;
...
isl_stream_read_union_pw_qpolynomial(__isl_keep isl_stream *s) {
  struct isl_obj obj;
  ...
  if (obj.v)
    isl_assert(s->ctx, obj.type == isl_obj_union_pw_qpolynomial, goto error);
