...
struct AnnotationsVerifier {
    static LogicalResult verify(GenericOpType op) {
        return success();
    }
};
...
static SmallVector<AffineMap, 4> getAffineMaps(ArrayAttr attrs) {
    ...
    attrs, [](Attribute a) {
        return a.cast<AffineMapAttr>().getValue();
    }));
}
...
Value mlir::linalg::ReshapeOp::getViewSource() {
    return src();
}
...
std::string mlir::linalg::generateLibraryCallName(Operation *op) {
    ...
    llvm::interleave(
    types.begin(), types.end(), [&](Type t) {
        appendMangledType(ss, t);
    },
    [&]() {
        ss << "_";
    });
