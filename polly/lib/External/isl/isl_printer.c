...
struct isl_printer_ops {
  ...
  __isl_give isl_printer *(*print_double)(__isl_take isl_printer *p, double d);
...
static struct isl_printer_ops file_ops = {
    file_start_line,    file_end_line,  file_print_double, file_print_int,
    file_print_isl_int, file_print_str, file_flush};

static struct isl_printer_ops str_ops = {
    str_start_line,    str_end_line,  str_print_double, str_print_int,
    str_print_isl_int, str_print_str, str_flush};
...
__isl_give isl_id *isl_printer_get_note(__isl_keep isl_printer *p,
  ...
  if (has_note < 0)
    goto error;
  if (!has_note)
    isl_die(isl_printer_get_ctx(p), isl_error_invalid, "no such note",
            goto error);
...
static __isl_give isl_printer *push_state(__isl_take isl_printer *p,
  ...
  if (p->yaml_size < p->yaml_depth + 1) {
    enum isl_yaml_state *state;
    state = isl_realloc_array(p->ctx, p->yaml_state, enum isl_yaml_state,
                              p->yaml_depth + 1);
    if (!state)
      return isl_printer_free(p);
    p->yaml_state = state;
...
static __isl_give isl_printer *update_state(__isl_take isl_printer *p,
  ...
  if (!p)
    return NULL;
  if (p->yaml_depth < 1)
    isl_die(isl_printer_get_ctx(p), isl_error_invalid, "not in YAML construct",
            return isl_printer_free(p));
...
isl_printer_yaml_start_mapping(__isl_take isl_printer *p) {
  enum isl_yaml_state state;
 ...
 */
__isl_give isl_printer *
isl_printer_yaml_end_mapping(__isl_take isl_printer *p) {
  enum isl_yaml_state state;
 ...
 */
__isl_give isl_printer *
isl_printer_yaml_start_sequence(__isl_take isl_printer *p) {
  if (!p)
    return NULL;
  p = enter_state(p, p->yaml_style == ISL_YAML_STYLE_BLOCK);
 ...
 */
__isl_give isl_printer *
isl_printer_yaml_end_sequence(__isl_take isl_printer *p) {
