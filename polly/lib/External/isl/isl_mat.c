...
__isl_give isl_mat *isl_mat_alloc(isl_ctx *ctx, unsigned n_row,
                                  unsigned n_col) {
  int i;
  struct isl_mat *mat;
...
__isl_give isl_mat *isl_mat_extend(__isl_take isl_mat *mat, unsigned n_row,
                                   unsigned n_col) {
  int i;
  isl_int *old;
  isl_int **row;
...
__isl_give isl_mat *isl_mat_sub_alloc6(isl_ctx *ctx, isl_int **row,
                                       unsigned first_row, unsigned n_row,
                                       unsigned first_col, unsigned n_col) {
  int i;
  struct isl_mat *mat;
...
__isl_give isl_mat *isl_mat_sub_alloc(__isl_keep isl_mat *mat,
  ...
  if (!mat)
    return NULL;
  return isl_mat_sub_alloc6(mat->ctx, mat->row, first_row, n_row, first_col,
                            n_col);
}

void isl_mat_sub_copy(struct isl_ctx *ctx, isl_int **dst, isl_int **src,
                      unsigned n_row, unsigned dst_col, unsigned src_col,
                      unsigned n_col) {
  int i;
...
static isl_stat check_col(__isl_keep isl_mat *mat, int col) {
  ...
  if (col < 0 || col >= mat->n_col)
    isl_die(isl_mat_get_ctx(mat), isl_error_invalid, "column out of range",
            return isl_stat_error);
  return isl_stat_ok;
}
...
static isl_stat check_row(__isl_keep isl_mat *mat, int row) {
  ...
  if (row < 0 || row >= mat->n_row)
    isl_die(isl_mat_get_ctx(mat), isl_error_invalid, "row out of range",
            return isl_stat_error);
  return isl_stat_ok;
}
...
static isl_stat check_col_range(__isl_keep isl_mat *mat, unsigned first,
  ...
  if (first + n > mat->n_col || first + n < first)
    isl_die(isl_mat_get_ctx(mat), isl_error_invalid,
            "column position or range out of bounds", return isl_stat_error);
  return isl_stat_ok;
}
...
static isl_stat check_row_range(__isl_keep isl_mat *mat, unsigned first,
  ...
  if (first + n > mat->n_row || first + n < first)
    isl_die(isl_mat_get_ctx(mat), isl_error_invalid,
            "row position or range out of bounds", return isl_stat_error);
  return isl_stat_ok;
}
 ...
 */
__isl_give isl_val *isl_mat_get_element_val(__isl_keep isl_mat *mat, int row,
                                            int col) {
  isl_ctx *ctx;
...
__isl_give isl_mat *isl_mat_set_element(__isl_take isl_mat *mat, int row,
                                        int col, isl_int v) {
  mat = isl_mat_cow(mat);
  if (check_row(mat, row) < 0)
...
__isl_give isl_mat *isl_mat_set_element_si(__isl_take isl_mat *mat, int row,
                                           int col, int v) {
  mat = isl_mat_cow(mat);
  if (check_row(mat, row) < 0)
    return isl_mat_free(mat);
 ...
 */
__isl_give isl_mat *isl_mat_set_element_val(__isl_take isl_mat *mat, int row,
                                            int col, __isl_take isl_val *v) {
  if (!v)
    return isl_mat_free(mat);
  if (!isl_val_is_int(v))
...
isl_bool isl_mat_is_scaled_identity(__isl_keep isl_mat *mat) {
  ...
  for (i = 0; i < mat->n_row; ++i) {
    ...
    if (isl_int_ne(mat->row[0][0], mat->row[i][i]))
      return isl_bool_false;
    if (isl_seq_first_non_zero(mat->row[i] + i + 1, mat->n_col - (i + 1)) != -1)
      return isl_bool_false;
  }
...
__isl_give isl_mat *isl_mat_aff_direct_sum(__isl_take isl_mat *left,
  ...
  for (i = 1; i < sum->n_row; ++i) {
    ...
    isl_int_addmul(sum->row[i][0], right->row[0][0], right->row[i][0]);
    isl_seq_scale(sum->row[i] + 1, left->row[i] + 1, left->row[0][0],
                  left->n_col - 1);
    isl_seq_scale(sum->row[i] + left->n_col, right->row[i] + 1,
                  right->row[0][0], right->n_col - 1);
  }
...
static void exchange(__isl_keep isl_mat *M, __isl_keep isl_mat **U,
                     __isl_keep isl_mat **Q, unsigned row, unsigned i,
                     unsigned j) {
  int r;
 ...
 */
static __isl_give isl_mat *eliminate(__isl_take isl_mat *mat, int row,
                                     int col) {
  int k;
  isl_size nr, nc;
  isl_ctx *ctx;
...
__isl_give isl_mat *isl_mat_diagonal(__isl_take isl_mat *mat1,
  ...
  for (i = 0; i < mat2->n_row; ++i) {
    ...
    isl_seq_cpy(mat->row[mat1->n_row + i] + mat1->n_col, mat2->row[i],
                mat2->n_col);
  }
  isl_mat_free(mat1);
  isl_mat_free(mat2);
...
static isl_stat inv_exchange(__isl_keep isl_mat **left,
                             __isl_keep isl_mat **right, unsigned i,
                             unsigned j) {
...
static void inv_oppose(__isl_keep isl_mat *left, __isl_keep isl_mat *right,
                       unsigned row) {
...
static void inv_subtract(__isl_keep isl_mat *left, __isl_keep isl_mat *right,
  ...
  isl_int_neg(m, m);
  isl_seq_combine(left->row[i] + row, left->ctx->one, left->row[i] + row, m,
                  left->row[row] + row, left->n_col - row);
  isl_seq_combine(right->row[i], right->ctx->one, right->row[i], m,
                  right->row[row], right->n_col);
}
...
__isl_give isl_mat *isl_mat_inverse_product(__isl_take isl_mat *left,
  ...
  for (row = 0; row < left->n_row; ++row) {
    ...
    first = row + 1;
    while ((off = row_first_non_zero(left->row + first, left->n_row - first,
                                     row)) != -1) {
      first += off;
      isl_int_fdiv_q(a, left->row[first][row], left->row[row][row]);
    ...
    for (i = 0; i < row; ++i) {
      ...
      isl_int_neg(b, b);
      isl_seq_combine(left->row[i] + i, a, left->row[i] + i, b,
                      left->row[row] + i, left->n_col - i);
      isl_seq_combine(right->row[i], a, right->row[i], b, right->row[row],
                      right->n_col);
    }
  }
  isl_int_clear(b);
  ...
  for (row = 0; row < left->n_row; ++row) {
    ...
    if (isl_int_is_one(left->row[row][row]))
      continue;
    isl_seq_scale(right->row[row], right->row[row], left->row[row][row],
                  right->n_col);
  }
  isl_int_clear(a);
...
void isl_mat_col_combine(__isl_keep isl_mat *mat, unsigned dst, isl_int m1,
                         unsigned src1, isl_int m2, unsigned src2) {
  int i;
  isl_int tmp;
...
__isl_give isl_mat *isl_mat_right_inverse(__isl_take isl_mat *mat) {
  ...
  for (row = 0; row < mat->n_row; ++row) {
    ...
    while ((off = isl_seq_first_non_zero(mat->row[row] + first,
                                         mat->n_col - first)) != -1) {
      first += off;
      isl_int_fdiv_q(a, mat->row[row][first], mat->row[row][row]);
...
__isl_give isl_mat *isl_mat_swap_cols(__isl_take isl_mat *mat, unsigned i,
                                      unsigned j) {
  int r;

  mat = isl_mat_cow(mat);
  if (check_col_range(mat, i, 1) < 0 || check_col_range(mat, j, 1) < 0)
    return isl_mat_free(mat);
...
__isl_give isl_mat *isl_mat_swap_rows(__isl_take isl_mat *mat, unsigned i,
                                      unsigned j) {
  isl_int *t;
  ...
  if (!mat)
    return NULL;
  mat = isl_mat_cow(mat);
  if (check_row_range(mat, i, 1) < 0 || check_row_range(mat, j, 1) < 0)
    return isl_mat_free(mat);
...
__isl_give isl_mat *isl_mat_product(__isl_take isl_mat *left,
  ...
  for (i = 0; i < prod->n_row; ++i) {
    for (j = 0; j < prod->n_col; ++j)
      isl_int_mul(prod->row[i][j], left->row[i][0], right->row[0][j]);
    for (k = 1; k < left->n_col; ++k) {
      if (isl_int_is_zero(left->row[i][k]))
        continue;
      for (j = 0; j < prod->n_col; ++j)
        isl_int_addmul(prod->row[i][j], left->row[i][k], right->row[k][j]);
    }
  }
  isl_mat_free(left);
  isl_mat_free(right);
...
static int preimage(struct isl_ctx *ctx, isl_int **q, unsigned n,
  ...
  for (i = 0; i < n; ++i) {
    ...
    isl_seq_cpy(q[i] + has_div + t->n_col, q[i] + has_div + t->n_col + e,
                n_div);
...
__isl_give isl_basic_set *isl_basic_set_preimage(__isl_take isl_basic_set *bset,
  ...
  if (mat->n_col > mat->n_row) {
    bset = isl_basic_set_add_dims(bset, isl_dim_set, mat->n_col - mat->n_row);
    if (!bset)
      goto error;
  ...
  if (preimage(ctx, bset->eq, bset->n_eq, bset->n_div, 0, isl_mat_copy(mat)) <
      0)
    goto error;
...
__isl_give isl_set *isl_set_preimage(__isl_take isl_set *set,
  ...
  for (i = 0; i < set->n; ++i) {
    set->p[i] = isl_basic_set_preimage(set->p[i], isl_mat_copy(mat));
    if (!set->p[i])
      goto error;
  }
  if (mat->n_col != mat->n_row) {
...
__isl_give isl_mat *isl_mat_drop_cols(__isl_take isl_mat *mat, unsigned col,
                                      unsigned n) {
  int r;
...
__isl_give isl_mat *isl_mat_drop_rows(__isl_take isl_mat *mat, unsigned row,
                                      unsigned n) {
  int r;
...
__isl_give isl_mat *isl_mat_insert_cols(__isl_take isl_mat *mat, unsigned col,
                                        unsigned n) {
  isl_mat *ext;
  ...
  isl_mat_sub_copy(mat->ctx, ext->row, mat->row, mat->n_row, col + n, col,
                   mat->n_col - col);
...
__isl_give isl_mat *isl_mat_insert_rows(__isl_take isl_mat *mat, unsigned row,
                                        unsigned n) {
  isl_mat *ext;
...
void isl_mat_col_submul(__isl_keep isl_mat *mat, int dst_col, isl_int f,
                        int src_col) {
  int i;
...
void isl_mat_col_add(__isl_keep isl_mat *mat, int dst_col, int src_col) {
  ...
  for (i = 0; i < mat->n_row; ++i)
    isl_int_add(mat->row[i][dst_col], mat->row[i][dst_col],
                mat->row[i][src_col]);
}
...
__isl_give isl_mat *isl_mat_concat(__isl_take isl_mat *top,
  ...
  if (!mat)
    goto error;
  isl_mat_sub_copy(mat->ctx, mat->row, top->row, top->n_row, 0, 0, mat->n_col);
  isl_mat_sub_copy(mat->ctx, mat->row + top->n_row, bot->row, bot->n_row, 0, 0,
                   mat->n_col);
  isl_mat_free(top);
  isl_mat_free(bot);
  return mat;
...
__isl_give isl_vec *isl_mat_get_row(__isl_keep isl_mat *mat, unsigned row) {
  ...
  if (!mat)
    return NULL;
  if (row >= mat->n_row)
    isl_die(mat->ctx, isl_error_invalid, "row out of range", return NULL);
...
__isl_give isl_mat *isl_mat_move_cols(__isl_take isl_mat *mat, unsigned dst_col,
                                      unsigned src_col, unsigned n) {
  isl_mat *res;
  ...
  if (dst_col < src_col) {
    isl_mat_sub_copy(res->ctx, res->row, mat->row, mat->n_row, 0, 0, dst_col);
    isl_mat_sub_copy(res->ctx, res->row, mat->row, mat->n_row, dst_col, src_col,
                     n);
    isl_mat_sub_copy(res->ctx, res->row, mat->row, mat->n_row, dst_col + n,
                     dst_col, src_col - dst_col);
    isl_mat_sub_copy(res->ctx, res->row, mat->row, mat->n_row, src_col + n,
                     src_col + n, res->n_col - src_col - n);
  } else {
    isl_mat_sub_copy(res->ctx, res->row, mat->row, mat->n_row, 0, 0, src_col);
    isl_mat_sub_copy(res->ctx, res->row, mat->row, mat->n_row, src_col,
                     src_col + n, dst_col - src_col);
    isl_mat_sub_copy(res->ctx, res->row, mat->row, mat->n_row, dst_col, src_col,
                     n);
    isl_mat_sub_copy(res->ctx, res->row, mat->row, mat->n_row, dst_col + n,
                     dst_col + n, res->n_col - dst_col - n);
  }
  isl_mat_free(mat);
