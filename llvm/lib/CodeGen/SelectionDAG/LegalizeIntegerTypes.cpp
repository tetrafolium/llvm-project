...
void DAGTypeLegalizer::PromoteIntegerResult(SDNode *N, unsigned ResNo) {
  ...
  case ISD::SCALAR_TO_VECTOR:
    Res = PromoteIntRes_SCALAR_TO_VECTOR(N);
    break;
  case ISD::SPLAT_VECTOR:
    Res = PromoteIntRes_SPLAT_VECTOR(N);
    break;
  ...
  case ISD::FLT_ROUNDS_:
    Res = PromoteIntRes_FLT_ROUNDS(N);
    break;
  ...
  case ISD::ADDCARRY:
  case ISD::SUBCARRY:
    Res = PromoteIntRes_ADDSUBCARRY(N, ResNo);
    break;
...
SDValue DAGTypeLegalizer::PromoteIntRes_VSELECT(SDNode *N) {
  ...
  SDValue LHS = GetPromotedInteger(N->getOperand(1));
  SDValue RHS = GetPromotedInteger(N->getOperand(2));
  return DAG.getNode(ISD::VSELECT, SDLoc(N), LHS.getValueType(), Mask, LHS,
                     RHS);
}
...
SDValue DAGTypeLegalizer::PromoteIntRes_TRUNCATE(SDNode *N) {
  ...
  case TargetLowering::TypeSplitVector: {
    ...
    EVT HalfNVT =
        EVT::getVectorVT(*DAG.getContext(), NVT.getScalarType(), NumElts / 2);
...
bool DAGTypeLegalizer::PromoteIntegerOperand(SDNode *N, unsigned OpNo) {
  ...
  case ISD::INSERT_VECTOR_ELT:
    ...
    break;
  case ISD::SCALAR_TO_VECTOR:
    Res = PromoteIntOp_SCALAR_TO_VECTOR(N);
    break;
  case ISD::SPLAT_VECTOR:
    Res = PromoteIntOp_SPLAT_VECTOR(N);
    break;
  case ISD::VSELECT:
  case ISD::SELECT:
  ...
  case ISD::ADDCARRY:
  case ISD::SUBCARRY:
    Res = PromoteIntOp_ADDSUBCARRY(N, OpNo);
    break;
...
void DAGTypeLegalizer::ExpandIntegerResult(SDNode *N, unsigned ResNo) {
  ...
  case ISD::CTTZ:
    ...
    break;
  case ISD::FLT_ROUNDS_:
    ExpandIntRes_FLT_ROUNDS(N, Lo, Hi);
    break;
  case ISD::STRICT_FP_TO_SINT:
  case ISD::FP_TO_SINT:
    ExpandIntRes_FP_TO_SINT(N, Lo, Hi);
    break;
  case ISD::STRICT_FP_TO_UINT:
  case ISD::FP_TO_UINT:
    ExpandIntRes_FP_TO_UINT(N, Lo, Hi);
    break;
  case ISD::FP_TO_SINT_SAT:
  case ISD::FP_TO_UINT_SAT:
  ...
  case ISD::LLROUND:
  case ISD::LLRINT:
    ExpandIntRes_LLROUND_LLRINT(N, Lo, Hi);
    break;
  case ISD::LOAD:
  ...
  case ISD::ADDCARRY:
  case ISD::SUBCARRY:
    ExpandIntRes_ADDSUBCARRY(N, Lo, Hi);
    break;
...
void DAGTypeLegalizer::ExpandIntRes_ADDSUB(SDNode *N, SDValue &Lo,
  ...
  if (N->getOpcode() == ISD::ADD) {
    ...
    SDValue Carry1 = DAG.getSelect(dl, NVT, Cmp1, DAG.getConstant(1, dl, NVT),
                                   DAG.getConstant(0, dl, NVT));
    SDValue Cmp2 =
        DAG.getSetCC(dl, getSetCCResultType(NVT), Lo, LoOps[1], ISD::SETULT);
    SDValue Carry2 =
        DAG.getSelect(dl, NVT, Cmp2, DAG.getConstant(1, dl, NVT), Carry1);
...
void DAGTypeLegalizer::ExpandIntRes_ADDSUBCARRY(SDNode *N, SDValue &Lo,
                                                SDValue &Hi) {
  // Expand the subcomponents.
  SDValue LHSL, LHSH, RHSL, RHSH;
  SDLoc dl(N);
...
void DAGTypeLegalizer::ExpandIntRes_SADDSUBO_CARRY(SDNode *N, SDValue &Lo,
  ...
  unsigned CarryOp =
      N->getOpcode() == ISD::SADDO_CARRY ? ISD::ADDCARRY : ISD::SUBCARRY;
...
void DAGTypeLegalizer::ExpandIntRes_LLROUND_LLRINT(SDNode *N, SDValue &Lo,
  ...
  if (N->getOpcode() == ISD::LLROUND || N->getOpcode() == ISD::STRICT_LLROUND) {
    if (VT == MVT::f32)
    ...
    if (Scale < NVTSize) {
      ...
      SDValue HLAdjusted =
          DAG.getNode(ISD::SRL, dl, NVT, ResultHL,
                      DAG.getConstant(Scale - NVTSize, dl, ShiftTy));
...
void DAGTypeLegalizer::ExpandIntRes_SADDSUBO(SDNode *Node, SDValue &Lo,
  ...
  default:
    llvm_unreachable("Node has unexpected Opcode");
  case ISD::SADDO:
    CarryOp = ISD::SADDO_CARRY;
    break;
  case ISD::SSUBO:
    CarryOp = ISD::SSUBO_CARRY;
    break;
  }
  ...
  if (HasCarryOp) {
    ...
    Lo = DAG.getNode(Node->getOpcode() == ISD::SADDO ? ISD::UADDO : ISD::USUBO,
                     dl, VTList, {LHSL, RHSL});
    ...
    SDValue SignsMatch =
        DAG.getSetCC(dl, OType, LHSSign, RHSSign,
                     Node->getOpcode() == ISD::SADDO ? ISD::SETEQ : ISD::SETNE);
...
void DAGTypeLegalizer::ExpandIntRes_XMULO(SDNode *N, SDValue &Lo, SDValue &Hi) {
  ...
  if (N->getOpcode() == ISD::UMULO) {
    ...
    EVT HalfVT = LHSLow.getValueType(), BitVT = N->getValueType(1);
    ...
    SDValue OneInHigh =
        DAG.getNode(ISD::BUILD_PAIR, dl, VT, HalfZero, One.getValue(0));
    ...
    SDValue TwoInHigh =
        DAG.getNode(ISD::BUILD_PAIR, dl, VT, HalfZero, Two.getValue(0));
    ...
    if (RHSLo == RHSHi) {
      if (ConstantSDNode *RHSCST = dyn_cast<ConstantSDNode>(RHSLo)) {
        if (RHSCST->isAllOnesValue()) {
          ...
          NewLHS =
              DAG.getNode(ISD::AND, dl, LHSLo.getValueType(), LHSLo, LHSHi);
          NewRHS = RHSLo;
          return;
        }
      }
